...
class FDTDLattice1D:
    ...
    def _apply_gravitational_metric(self, mass_kg: float):
        ...
        if np.any(valid_e):
            n_r_e = np.array([calculate_refractive_strain(mass_kg, r)
                              for r in radius_e[valid_e]])
        ...
        if np.any(valid_h):
            n_r_h = np.array([calculate_refractive_strain(mass_kg, r)
                              for r in radius_h[valid_h]])
