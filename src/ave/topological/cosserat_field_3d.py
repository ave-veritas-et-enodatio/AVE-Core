...
def _energy_density_bare(
    ...
    W_cauchy = (2.0 / 3.0) * trace_eps**2 + jnp.sum(eps_sym**2, axis=(-1, -2))  # [DEMOTED 2026-08-11 - R40-B2a: NEEDS RE-DERIVATION, not dead; dated note at the end of this file]
...
class CosseratField3D:
    ...
    def cfl_dt(self) -> float:
        ...
        Longitudinal (bulk)      c_L = √((K + 4G/3)/rho) with K = 2G →
                                        c_L = √(10G/(3·rho)) ≈ 1.826·c_T.  DEMOTED 2026-08-11 — R40-B1 (dated demotion note at end of file).
...
#   :1789 — "Longitudinal (bulk)      c_L = √((K + 4G/3)/rho) with K = 2G → c_L = √(10G/(3·rho)) ≈ 1.826·c_T."
#       stamped in place at :1790
#       why it dies (audited row rationale, verbatim): A bulk propagation speed used as a physical
#       transit speed on the lattice (frozen DIES text verbatim class): a named branch of 'the
#       linear Cosserat wave equation' folded into the CFL bound; the CFL machinery survives on the
#       remaining (shear/rotational) branches.
#
...
#        IS the coded ½K(∇·u)² at K=2G (½λ=2G/3), driving the velocity-Verlet dynamics (ρü=−∂W/∂u) — the
#        phantom's minting site (#761's own step); constraint-form re-derivation owed (G-ENGINE-FLAG
#        class; also :756).
