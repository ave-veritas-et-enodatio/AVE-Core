"""
K4 V_inc scalar-grade seed — standing longitudinal V on the transverse-readout channel.

Seeds the K4 V_inc (transverse-readout) channel with a standing longitudinal V
structure. Topology-NULL precursor: no planted (2,3) winding.

SCOPE — what this validates, and what it does NOT:
  * The harness ``VacuumEngine3D`` scalar is a ``v_scalar_from_v_inc`` PROJECTION
    of the K4 port voltages (def ``cross_sector_coupling.py:226``) — NOT an
    independent A1 stiffening field. This harness structurally hosts no A1 cage.
  * Therefore this module validates the seed PLUMBING and the CP8 topology-null
    CERTIFICATE only. It does NOT validate (and cannot, on this harness) the A1
    stiffening cage. The cage channel lives on ``master_equation_fdtd.py`` /
    ``crystal_engine.py`` (see ``manuscript/ave-kb/common/engine-capability-map.md``).

F1 falsifier: the seed lands ``A²_V > 0.25·A_yield²`` with ``topology_null=True``
(CP8 — no planted (2,3): ω ≡ 0, |H_bel| ≈ 0).

Lineage: ports the genesis-24 ``_seed_v_partner`` /
``UnifiedGenesisEngine.seed_lane1`` in-plane V-vector shape to the LOOP GAP harness.
"""
