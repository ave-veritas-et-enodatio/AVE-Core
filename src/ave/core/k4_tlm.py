...
class K4Lattice3D:
    ...
    def __init__(self, nx: int, ny: int, nz: int, dx: float = 1.0, nonlinear: bool = False, pml_thickness: int = 0) -> None:
        ...
        # To make it simple: Diamond lattice active sites are exactly those where A has sum(i,j,k) % 4 == 0
        # (with same parity) and B has sum(i,j,k) % 4 == 3 (with same parity? No! if A is all even,
        # + (1,1,1) is all odd. sum is 3.
        ...
        # A nodes have i,j,k all EVEN or all ODD. But sum for A is always exactly 0 mod 4 if all even?
        # No! (1,1,1) has sum 3.
        ...
        # If A connects to B via exactly these 4 vectors, the graph is a set of disjoint Diamond lattices
