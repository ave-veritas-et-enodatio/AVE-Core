...
from ave.core.constants import (
    L_NODE, ALPHA, M_E, C_0, Z_0, V_YIELD, T_EM, HBAR, EPSILON_0, e_charge
)
...
def run_electrostatic_resolution():
    ...
    plt.plot(r_points, energy_spectrum_classical, 'r--', label='Classical 3D Point-Charge Divergence ($\\propto 1/r$)')
    plt.plot(r_points, energy_spectrum_ave, 'b-', linewidth=2, label='AVE Bounded Topological Loop ($T_{EM} \\cdot \\ell_{node}$)')
    
    plt.axvline(L_NODE, color='k', linestyle=':', label='Topological Boundary ($L_{NODE}$)')
    plt.axhline(canonical_rest_mass_energy, color='g', linestyle='-.', alpha=0.5, label='Exact $m_e c^2$ Rest Mass Limit')
