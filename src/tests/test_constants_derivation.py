...
# SI inputs; Three calibration inputs;
# Derived topological; Derived dielectric; Derived macroscopic; Electroweak; CKM; PMNS; Baryon
...
class TestSIConstants:
    ...
    def test_epsilon_from_mu_c(self) -> None:
        assert EPSILON_0 == pytest.approx(1.0 / (MU_0 * C_0**2), rel=1e-12)

    def test_impedance_from_mu_epsilon(self) -> None:
...
class TestDielectricDerivations:
    ...
    def test_nu_vac(self) -> None:
        assert NU_VAC == pytest.approx(2.0 / 7.0, rel=1e-15)

    def test_eta_eq(self) -> None:
...
class TestPredictionTable:
    ...
    def test_galactic_ngc3198(self) -> None:
    ...
    def test_superconductor_bcs_identity(self) -> None:
    ...
    def test_london_depth_order_of_magnitude(self) -> None:
    ...
    def test_gw_lossless_propagation(self) -> None:
    ...
    def test_stellar_tachocline_reflection(self) -> None:
...
class TestDimensionalSanity:
    ...
    def test_l_node_is_femtometer_scale(self) -> None:
        assert 1e-13 < L_NODE < 1e-12  # ~3.86e-13 m

    def test_z0_is_377(self) -> None:
        assert 376 < Z_0 < 378

    def test_t_em_is_sub_newton(self) -> None:
        assert 0.1 < T_EM < 0.5  # ~0.212 N

    def test_v_snap_is_511kv(self) -> None:
        assert 510e3 < V_SNAP < 512e3

    def test_v_yield_is_44kv(self) -> None:
        assert 43e3 < V_YIELD < 45e3

    def test_b_snap_is_giga_tesla(self) -> None:
        assert 1e9 < B_SNAP < 3e9

    def test_h_infinity_positive(self) -> None:
        assert H_INFINITY > 0

    def test_alpha_s_between_zero_and_one(self) -> None:
        assert 0 < ALPHA_S < 1

    def test_nu_vac_is_two_sevenths(self) -> None:
