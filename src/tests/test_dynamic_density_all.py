...
def get_nucleon_coordinates(Z, A, d=0.85):
    if Z == 2 and A == 4:
        ...
        outer = 2.5 * d
        alpha_1 = [
            (-outer+d_stretch, d_stretch, d_stretch), (-outer-d_stretch, -d_stretch, d_stretch),
            (-outer-d_stretch, d_stretch, -d_stretch), (-outer+d_stretch, -d_stretch, -d_stretch)
        ]
        alpha_2 = [
            (outer+d_stretch, d_stretch, d_stretch), (outer-d_stretch, -d_stretch, d_stretch),
...
def generate_dynamic_flux_gif(base_cluster, output_name: str, title: str, grid_res=100, bound=4.5, frames=90):
    ...
    def update(frame):
        ...
        for n in rotated_cluster:
            ...
            ax.scatter(cx, cy, color=n['color'], s=pt_size * depth_scale, marker=pt_marker, linewidth=2, alpha=0.8)
...
if __name__ == "__main__":
    ...
    elements = [
        ...
        { 'cluster': generate_element(3, 7), 'name': 'lithium_7', 'title': "Lithium-7: Asymmetrical Secondary Shell", 'bound': 15.0 },
        { 'cluster': generate_element(4, 9), 'name': 'beryllium_9', 'title': "Beryllium-9: Stretched Dual-Core", 'bound': 8.0 }
    ]

    for el in elements:
        generate_dynamic_flux_gif(el['cluster'], f"{el['name']}_dynamic_flux.gif", el['title'], grid_res=80, bound=el['bound'], frames=90)
