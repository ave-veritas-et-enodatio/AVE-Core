...
class TestUnknot:
    ...
    def test_ropelength_is_2pi(self) -> None:
...
class TestTrefoil:
    ...
    def test_not_planar(self, coords: np.ndarray) -> None:
        """Trefoil must occupy 3D space (z ≠ 0 everywhere)."""
        assert np.max(np.abs(coords[:, 2])) > 0.01

    def test_normalized_bounding(self, coords: np.ndarray) -> None:
...
class TestBorromean:
    ...
    def test_returns_three_rings(self, rings: list[np.ndarray]) -> None:
        """Must return exactly 3 ring arrays."""
        assert len(rings) == 3

    def test_ring_shapes(self, rings: list[np.ndarray]) -> None:
        """Each ring must be [N, 3]."""
        for ring in rings:
            assert ring.shape == (500, 3)

    def test_each_ring_closed(self, rings: list[np.ndarray]) -> None:
        """Each ring must be a closed curve."""
        for ring in rings:
            np.testing.assert_allclose(ring[0], ring[-1], atol=1e-10)

    def test_mutually_perpendicular(self, rings: list[np.ndarray]) -> None:
    ...
    def test_eccentricity_scales(self) -> None:
    ...
    def test_rings_not_identical(self, rings: list[np.ndarray]) -> None:
