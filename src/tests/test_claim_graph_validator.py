...
class TestSchema:
    ...
    def test_missing_required_field_fires(self):
        m = _manifest([
            {"id": "P01", "name": "missing-type", "derivation_label": "ch:x"}
        ])
        ...
        missing_findings = [
            f for f in findings if "missing required fields" in f.message.lower()
        ]
...
class TestLiveManifest:
    ...
    def test_manifest_schema_clean(self):
        ...
        assert criticals == [], (
            "Live manifest has schema violations:\n"
            + "\n".join(f"  [{f.severity}] P={f.entry_id} {f.message}" for f in criticals)
        )
    ...
    def test_manifest_labels_resolve(self):
        ...
        criticals = [f for f in findings if f.severity == "critical"]
        assert criticals == [], (
            "Live manifest has unresolved derivation_labels:\n"
            + "\n".join(f"  P={f.entry_id} {f.message}" for f in criticals)
        )
    ...
    def test_manifest_engine_agrees(self):
        ...
        criticals = [f for f in findings if f.severity == "critical"]
        assert criticals == [], (
            "Live manifest disagrees with engine:\n"
            + "\n".join(f"  P={f.entry_id} {f.message}" for f in criticals)
        )
    ...
    def test_all_entries_use_allowed_types(self):
        ...
        for entry in m["predictions"]:
            assert entry["type"] in ALLOWED_TYPES, (
                f"Entry {entry['id']} uses unknown type: {entry['type']}"
            )
...
class TestOrchestration:
    def test_run_with_all_checks(self):
        ...
        criticals = [f for f in findings if f.severity == "critical"]
        assert criticals == [], (
            "run() reports critical findings on live manifest:\n"
            + "\n".join(f"  [{f.check}] P={f.entry_id} {f.message}" for f in criticals)
        )
