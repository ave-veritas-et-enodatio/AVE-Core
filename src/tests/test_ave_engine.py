...
class TestDerivedConstants:
    ...
    def test_speed_of_light_from_mu_eps(self) -> None:
        """c = 1/√(μ₀ε₀) must be exact."""
        c_derived = 1.0 / np.sqrt(MU_0 * EPSILON_0)
        assert abs(c_derived - C_0) / C_0 < 1e-10, f"c₀ inconsistency: derived={c_derived}, stored={C_0}"

    def test_impedance_from_mu_eps(self) -> None:
    ...
    def test_l_node_is_compton_wavelength(self) -> None:
    ...
    def test_xi_topo_computed(self) -> None:
    ...
    def test_packing_fraction(self) -> None:
        """p_c = 8πα ≈ 0.1834."""
    ...
    def test_proton_mass_ratio(self) -> None:
        """The structural eigenvalue must yield m_p/m_e ≈ 1836.12."""
        assert abs(PROTON_ELECTRON_RATIO - 1836.15) / 1836.15 < 0.005, (
            f"Proton ratio = {PROTON_ELECTRON_RATIO:.4f}, " f"expected ≈ 1836.15 (empirical 1836.15267)"
        )

    def test_proton_eigenvalue_formula(self) -> None:
    ...
    def test_hubble_constant_units(self) -> None:
...
class TestAxiom4Saturation:
    ...
    def test_nonlinear_epsilon_decreases(self) -> None:
        ...
        assert eps_low > eps_mid > eps_high > 0, f"Monotonic decrease violated: {eps_low} > {eps_mid} > {eps_high}"

    def test_nonlinear_e4_correction_dominates(self) -> None:
        ...
        assert ratio > 0.01, f"E⁴ correction ratio = {ratio:.6f} — too small to matter at 50% strain"

    def test_nonlinear_capacitance_diverges(self) -> None:
        ...
        assert c_high > c_mid > c_low > 1.0, f"Capacitance divergence violated: {c_low}, {c_mid}, {c_high}"
        # At 90% saturation, C_eff should be significantly elevated
        assert c_high > 2.0, f"C_eff at 90% = {c_high:.4f}, expected > 2.0"

    def test_nonlinear_wave_speed_slows(self) -> None:
    ...
    def test_nonlinear_impedance_increases(self) -> None:
        ...
        assert z_high > z_low > Z_0, f"Impedance must increase: Z_low={z_low:.2f}, Z_high={z_high:.2f}"

    # ---- SATURATION REGIME: V/V_yield → 1.0 ----

    def test_saturation_epsilon_approaches_zero(self) -> None:
        """At V = 0.9999 V_snap, ε_eff → ≈ 0 (topological destruction limit)."""
        V_near = 0.9999 * V_SNAP
        eps = epsilon_eff(V_near, V_yield=V_SNAP)
        assert eps < EPSILON_0 * 0.02, f"ε_eff at 99.99% = {eps:.2e}, expected near zero"

    def test_saturation_wave_speed_approaches_zero(self) -> None:
        """At saturation, c_eff → 0 (wave packet freezes → mass)."""
        V_near = 0.9999 * V_SNAP
        c_eff = local_wave_speed(V_near, V_yield=V_SNAP)
        assert c_eff / C_0 < 0.15, f"c_eff/c₀ at 99.99% = {c_eff/C_0:.6f}, expected near zero"

    def test_rupture_raises_error(self) -> None:
    ...
    def test_capacitance_rupture_raises_error(self) -> None:
    ...
    def test_vectorized_epsilon(self) -> None:
...
class TestAxiom2Isomorphism:
    ...
    def test_charge_roundtrip(self) -> None:
    ...
    def test_electron_charge_equals_l_node(self) -> None:
        """e → ℓ_node under the isomorphism."""
        x = charge_to_length(e_charge)
        assert abs(x - L_NODE) / L_NODE < 1e-10, f"charge_to_length(e) = {x:.4e}, expected L_NODE = {L_NODE:.4e}"

    def test_impedance_roundtrip(self) -> None:
    ...
    def test_xi_topo_dimensions(self) -> None:
...
class TestGravityOpticalMetric:
    ...
    def test_solar_refractive_index(self) -> None:
    ...
    def test_achromatic_impedance_invariance(self) -> None:
        """Z(r) must equal Z₀ everywhere, regardless of mass or distance."""
        for r in [1e3, 1e6, 1e9, 1e12]:
            z = local_impedance(self.M_SUN, r)
            assert abs(z - Z_0) / Z_0 < 1e-10, f"Z₀ invariance broken at r={r}: Z={z:.6f}, Z₀={Z_0:.6f}"

    def test_achromatic_mu_eps_ratio(self) -> None:
    ...
    def test_dielectric_rupture_inside_horizon(self) -> None:
    ...
    def test_einstein_deflection_sun(self) -> None:
        ...
        assert abs(arcsec - 1.75) / 1.75 < 0.01, f"Deflection = {arcsec:.4f} arcsec, expected 1.75"

    def test_radial_strain_at_horizon(self) -> None:
    ...
    def test_principal_strain_7g(self) -> None:
    ...
    def test_poisson_contraction(self) -> None:
...
class TestProtonEigenvalue:
    ...
    def test_eigenvalue_within_1_percent(self) -> None:
    ...
    def test_eigenvalue_feedback_loop(self) -> None:
...
class TestCrossAxiomConsistency:
    ...
    def test_v_snap_is_saturation_boundary(self) -> None:
    ...
    def test_string_tension_dimension_check(self) -> None:
