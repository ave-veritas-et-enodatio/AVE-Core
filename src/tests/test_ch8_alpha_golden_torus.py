...
class TestMultipoleClosure:
    def test_multipole_sum_equals_closed_form(self) -> None:
    ...
    def test_alpha_cold_reciprocal(self) -> None:
...
class TestGoldenTorusConstraints:
    def test_self_avoidance_constraint(self) -> None:
    ...
    def test_quadratic_root_matches(self) -> None:
...
class TestRopelengthConvergence:
    def test_arc_length_is_positive_and_finite(self) -> None:
        """Sanity check on the trefoil arc-length primitive."""
        L = trefoil_arc_length(GOLDEN_R, GOLDEN_r)
        assert np.isfinite(L)
        assert L > 0

    def test_stage_b_converges_to_golden_torus(self) -> None:
        """
...
class TestVacuumStrainCoefficient:
    def test_delta_strain_positive(self) -> None:
        """
    ...
    def test_delta_strain_small(self) -> None:
        """
    ...
    def test_cmb_correction_bridges_cold_to_codata(self) -> None:
    ...
    def test_delta_strain_derivation_identity(self) -> None:
