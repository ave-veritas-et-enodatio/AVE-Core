...
def construct_helium_4(shift_distance: float):
    placements = [
        {'shift': [ shift_distance,  shift_distance,  shift_distance], 'rot': [0, 0, 0], 'color': '#ff3366'},
        {'shift': [-shift_distance, -shift_distance,  shift_distance], 'rot': [0, 0, np.pi/2], 'color': '#ff3366'},
        {'shift': [-shift_distance,  shift_distance, -shift_distance], 'rot': [np.pi/2, 0, 0], 'color': '#00ffcc'},
        {'shift': [ shift_distance, -shift_distance, -shift_distance], 'rot': [0, np.pi/2, 0], 'color': '#00ffcc'}
    ]
    return NumericallyEvaluateNucleons(NucleonCombiner.assemble_cluster(FundamentalTopologies.generate_borromean_6_3_2, placements))
...
def generate_dynamic_flux_gif(base_cluster, output_name: str, title: str, grid_res=150, bound=4.5, frames=180):
    ...
    def update(frame):
        ...
        ax.imshow(density_field, extent=[-bound, bound, -bound, bound],
        ...
        ax.text(0.02, 0.95, f"Azimuth: {angle * (180/np.pi):.0f} deg\nZ-Slice: 0.0",
