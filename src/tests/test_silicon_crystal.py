...
class TestSiSiBond:
    ...
    def test_bond_energy_within_15_pct(self) -> None:
    ...
    def test_coupling_less_than_1(self) -> None:
        assert K_SI_SI < 1.0

    def test_crystal_coupling_is_k_over_4(self) -> None:
...
class TestDiamondCubicUnitCell:
    ...
    def test_coordination_4(self) -> None:
        assert COORD_NUMBER == 4

    def test_16_bonds(self) -> None:
        bonds = build_unit_cell_bonds()
        assert len(bonds) == 16

    def test_each_atom_has_4_bonds(self) -> None:
    ...
    def test_lattice_constant_within_15_pct(self) -> None:
...
class TestSiliconBandGap:
    ...
    def test_band_gap_within_50_pct(self) -> None:
    ...
    def test_band_gap_within_10_pct(self) -> None:
        """STRETCH TARGET: band gap within 10% of 1.12 eV."""
        result = silicon_band_gap()
        err = abs(result["E_gap_eV"] - E_GAP_EXP) / E_GAP_EXP
        assert err < 0.10, f"Band gap error {err:.1%} > 10%"

    def test_bonding_below_antibonding(self) -> None:
        result = silicon_band_gap()
...
class TestDispersion:
    ...
    def test_bonding_below_antibonding_everywhere(self) -> None:
        q = np.linspace(0, 1, 50)
        E_bond, E_anti = dispersion_periodic_lc(q)
        assert np.all(E_bond <= E_anti + 1e-10)

    def test_all_energies_positive(self) -> None:
        q = np.linspace(0, 1, 50)
        E_bond, E_anti = dispersion_periodic_lc(q)
