...
def construct_helium_4(shift_distance: float):
    placements = [
        {'shift': [ shift_distance,  shift_distance,  shift_distance], 'rot': [0, 0, 0], 'color': '#ff3366', 'label': 'Proton'},
        {'shift': [-shift_distance, -shift_distance,  shift_distance], 'rot': [0, 0, np.pi/2], 'color': '#ff3366'},
        {'shift': [-shift_distance,  shift_distance, -shift_distance], 'rot': [np.pi/2, 0, 0], 'color': '#00ffcc', 'label': 'Neutron'},
...
def construct_lithium_7(shift_distance: float):
    ...
    placements = [
        ...
        # Outer Shell
        {'shift': [outer_shift, -outer_shift, outer_shift], 'rot': [0, np.pi/4, 0], 'color': '#ff99aa', 'label': 'Outer Proton'},
        {'shift': [-outer_shift, -outer_shift, -outer_shift], 'rot': [np.pi/4, 0, 0], 'color': '#99ffee', 'label': 'Outer Neutron'},
...
if __name__ == "__main__":
    ...
    create_nucleus_gif(he4_cluster, 'helium_4_rotation.gif',
                       title="Helium-4 (Alpha Particle)\nTetrahedral Closed Topology", bound=3.0)

    # 2. Lithium-7
    li7_cluster = construct_lithium_7(shift_distance=0.85)
    create_nucleus_gif(li7_cluster, 'lithium_7_rotation.gif',
                       title="Lithium-7 (Dual-Shell)\nAsymmetrical Topological Binding", bound=4.5)
