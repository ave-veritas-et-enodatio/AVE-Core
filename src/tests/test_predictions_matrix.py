...
class TestPreRegisteredSchema:
    ...
    def test_required_fields_present(self, entry: dict):
        ...
        assert not missing, f"Entry {entry.get('id')} missing required fields: " f"{sorted(missing)}"
    ...
    def test_type_is_allowed(self, entry: dict):
        ...
        assert entry["type"] in ALLOWED_TYPES, (
            f"Entry {entry['id']}: type '{entry['type']}' " f"not in {sorted(ALLOWED_TYPES)}"
        )
    ...
    def test_axioms_used_populated(self, entry: dict):
        ...
        assert all(a in (1, 2, 3, 4) for a in axioms), (
            f"Entry {entry['id']}: axioms_used contains invalid values " f"{axioms}; must be in {{1, 2, 3, 4}}"
        )
...
class TestPreRegisteredPaths:
    ...
    def test_research_doc_exists(self, entry: dict):
        ...
        assert full.exists(), (
            f"Entry {entry['id']}: research_doc '{entry['research_doc']}' " f"does not resolve to a file on disk"
        )
    ...
    def test_test_file_exists_or_is_upcoming(self, entry: dict):
        ...
        if path_str in phase_1_tests:
            assert full.exists(), f"Entry {entry['id']}: Phase 1 test_file '{path_str}' " f"must exist in this commit"
...
class TestPreRegisteredIdConvention:
    ...
    def test_id_has_phase_number(self, entry: dict):
        ...
        assert phase_str.isdigit(), (
            f"Entry {eid}: expected P_phaseN_<short> format; " f"could not parse phase number from '{remainder}'"
        )
        phase_num = int(phase_str)
        assert 0 <= phase_num <= 6, f"Entry {eid}: phase number {phase_num} out of range 0..6"
