...
class TestSSMLayers:
    ...
    def test_seven_layers(self) -> None:
        """SSM should have 7 layers."""
        assert len(SSM_LAYERS) == 7

    def test_density_increases_inward(self) -> None:
...
class TestRadialProfile:
    ...
    def test_impedance_decreases_inward(self) -> None:
    ...
    def test_plasma_frequency_increases_inward(self) -> None:
...
class TestBoundaryReflections:
    ...
    def test_tachocline_reflection_nonzero(self) -> None:
        """Tachocline should produce a nonzero Γ."""
        gamma = tachocline_reflection()
        assert abs(gamma) > 0

    def test_photosphere_reflection_nonzero(self) -> None:
        """Photosphere boundary should produce nonzero Γ."""
        gamma = photosphere_reflection()
        assert abs(gamma) > 0

    def test_photosphere_larger_than_tachocline(self) -> None:
...
class TestSolarOpacity:
    ...
    def test_optical_transparent_at_surface(self) -> None:
    ...
    def test_radio_opaque_in_core(self) -> None:
