...
class TestRefractiveIndex:
    ...
    def test_increases_near_horizon(self) -> None:
        rs = schwarzschild_radius(M_SUN)
...
class TestQNMEigenvalue:
    ...
    def test_kerr_higher_frequency_than_schwarzschild(self) -> None:
        f_s = ringdown_frequency(10 * M_SUN, a_star=0.0)
        f_k = ringdown_frequency(10 * M_SUN, a_star=0.7)
        assert f_k > f_s

    def test_Q_and_decay_returns_positive(self) -> None:
...
class TestHawkingTemperature:
    ...
    def test_decreases_with_mass(self) -> None:
...
class TestReggeWheeler:
    ...
    def test_potential_positive_peak(self) -> None:
