...
class TestDerivedBoundaries:
    ...
    def test_linear_boundary_is_sqrt_2alpha(self) -> None:
        assert abs(R_LINEAR_MAX - np.sqrt(2 * ALPHA)) < 1e-12

    def test_nonlinear_boundary_is_sqrt3_over_2(self) -> None:
        assert abs(R_NONLINEAR_MAX - np.sqrt(3) / 2) < 1e-12

    def test_yield_boundary_is_unity(self) -> None:
        assert R_YIELD_MAX == 1.0

    def test_at_linear_boundary_deltaS_equals_alpha(self) -> None:
        """At r = √(2α), the correction ΔS = r²/2 = α."""
        delta_S = R_LINEAR_MAX**2 / 2
        assert abs(delta_S - ALPHA) < 1e-12

    def test_at_nonlinear_boundary_Q_equals_2(self) -> None:
...
class TestTransitionBoundaries:
    ...
    def test_near_linear_nonlinear_boundary(self) -> None:
    ...
    def test_deep_linear_no_boundary(self) -> None:
    ...
    def test_mid_nonlinear_no_boundary(self) -> None:
        """r = 0.5 is far from both boundaries — no transition."""
        info = classify_regime(0.5, 1.0)
        assert not info.near_boundary

    def test_summary_includes_transition(self) -> None:
    ...
    def test_all_transition_names_defined(self) -> None:
...
class TestIdentifyRegime:
    ...
    def test_verbose_prints(self, capsys: Any) -> None:
...
class TestEMDomain:
    ...
    def test_lab_voltage(self) -> None:
        info = em_voltage_regime(1000)  # 1kV
        assert info.regime == REGIME_LINEAR

    def test_ponder05_30kv(self) -> None:
        info = em_voltage_regime(30e3)
        assert info.regime == REGIME_NONLINEAR
        assert abs(info.r - 0.687) < 0.01

    def test_ponder05_43kv(self) -> None:
...
class TestGravityDomain:
    ...
    def test_solar_surface(self) -> None:
        M_sun = 1.989e30
        r_sun = 6.96e8
    ...
    def test_black_hole_horizon(self) -> None:
...
class TestGalacticDomain:
    ...
    def test_derived_a0_used(self) -> None:
    ...
    def test_inner_galaxy_newtonian(self) -> None:
        """Inner galaxy: g_N >> a₀ → Regime IV (Newtonian, no drag)."""
        info = galactic_regime(1e-9)  # 10× a₀
        assert info.regime == REGIME_RUPTURED

    def test_transition_boundary(self) -> None:
    ...
    def test_outer_galaxy_deep_mond(self) -> None:
...
class TestRegimeEquations:
    ...
    def test_nonlinear_equations(self) -> None:
