...
class TestEEBenchNetlist:
    ...
    def test_generates_valid_netlist(self) -> None:
    ...
    def test_includes_lib(self) -> None:
        netlist = compile_ee_bench_dc_sweep()
    ...
    def test_contains_control_block(self) -> None:
        netlist = compile_ee_bench_dc_sweep()
...
class TestLCRNetwork:
    ...
    def _simple_network(self, use_nonlinear: bool = True) -> str:
    ...
    def test_nonlinear_network(self) -> None:
    ...
    def test_linear_network(self) -> None:
    ...
    def test_termination_resistor(self) -> None:
        netlist = self._simple_network()
        assert "R_TERM N_OUT GND 50" in netlist

    def test_ac_analysis_default(self) -> None:
        netlist = self._simple_network()
        assert ".AC DEC" in netlist

    def test_shunt_resistors(self) -> None:
        nodes = [
            {"id": "N_IN"},
            {"id": "N1", "shunt_r": 1e6},
            {"id": "N_OUT"},
...
class TestAminoAcidNetwork:
    ...
    def test_glycine_netlist(self) -> None:
        bonds = [
