...
class TestVacuumGrid:
    ...
    def test_temperature_setting(self) -> None:
        g = VacuumGrid(10, 10, thermal_mode="boundary")
        g.set_temperature(1.0)
        assert g.temperature == 1.0

    def test_bulk_thermal_mode_sets_correctly(self) -> None:
...
class TestNucleonCombiner:
    ...
    def test_rotate_preserves_norm(self) -> None:
    ...
    def test_assemble_cluster_non_empty(self) -> None:
        def dummy_gen(radius: float = 1.0) -> list[np.ndarray]:
