...
from ave.topological.cosserat import (
    M_W_TREE, M_W, M_W_MEV, M_Z_MEV,
    MISMATCH_LOSS, MISMATCH_LOSS_SC, S11_W_BOUND, S11_W_SC,
...
class TestGradedSolverStructure:
    ...
    def test_depth1_matches_hard_saturation(self):
        ...
        Y_hard = build_radial_tree_admittance(depth=1, branch_y=NU_VAC,
                                              boundary_y=0.0, coordination_z=4)
        Y_grad = build_radial_tree_admittance_graded(depth=1, branch_y=NU_VAC,
                                                     shell_boundary_y=[0.0],
                                                     coordination_z=4)
        np.testing.assert_allclose(Y_hard, Y_grad, atol=1e-14,
    ...
    def test_depth1_uniform_matches_standard(self):
        ...
        Y_std = build_radial_tree_admittance(depth=1, branch_y=NU_VAC,
                                             boundary_y=1.0, coordination_z=4)
        Y_grd = build_radial_tree_admittance_graded(depth=1, branch_y=NU_VAC,
                                                    shell_boundary_y=[1.0],
                                                    coordination_z=4)
    ...
    def test_matrix_shape_grows_correctly(self):
        ...
        for depth in range(1, 5):
            ...
            profile = [0.0] * depth
            Y = build_radial_tree_admittance_graded(depth=depth, branch_y=NU_VAC,
                                                    shell_boundary_y=profile,
                                                    coordination_z=z)
            assert Y.shape == (n_expected, n_expected), \
    ...
    def test_matrix_is_symmetric(self):
        ...
        for depth in range(1, 5):
            ...
            Y = build_radial_tree_admittance_graded(depth=depth, branch_y=NU_VAC,
                                                    shell_boundary_y=profile,
                                                    coordination_z=4)
            np.testing.assert_allclose(Y, Y.T, atol=1e-14,
                                       err_msg=f"Not symmetric at depth {depth}")

    def test_diagonal_dominance(self):
        """Each diagonal element >= sum of absolute off-diagonals (passive network)."""
        profile = [0.0, 0.3, 0.7]
        Y = build_radial_tree_admittance_graded(depth=3, branch_y=NU_VAC,
                                                shell_boundary_y=profile, coordination_z=4)
        for i in range(Y.shape[0]):
            off_sum = np.sum(np.abs(Y[i, :])) - abs(Y[i, i])
            assert Y[i, i].real >= off_sum - 1e-12, \
                f"Diagonal dominance violated at node {i}"
    ...
    def test_wrong_profile_length_raises(self):
        ...
        with pytest.raises(ValueError, match="shell_boundary_y"):
            build_radial_tree_admittance_graded(depth=3, branch_y=NU_VAC,
                                                shell_boundary_y=[0.0, 1.0],
                                                coordination_z=4)
...
class TestGradedSolverPhysics:
    ...
    def test_s11_at_shell1_hard_is_negative(self):
        ...
        Y = build_radial_tree_admittance_graded(depth=1, branch_y=NU_VAC,
                                                shell_boundary_y=[0.0], coordination_z=4)
    ...
    def test_harder_boundary_gives_larger_mismatch_loss(self):
        ...
        for y_b in [0.0, 0.3, 0.6, 1.0]:
            Y = build_radial_tree_admittance_graded(depth=1, branch_y=NU_VAC,
                                                    shell_boundary_y=[y_b], coordination_z=4)
        ...
        for i in range(len(s11_list) - 1):
            assert s11_list[i] >= s11_list[i + 1] - 1e-12, \
                f"|S11| not decreasing: {s11_list}"
...
class TestSelfConsistentCorrection:
    ...
    def test_iterations_bounded(self):
        assert self.sc["n_iter"] <= 20, \
            f"Expected convergence in ≤20 iterations, got {self.sc['n_iter']}"
    ...
    def test_sc_loss_less_than_d1_loss(self):
        ...
        assert self.sc["mismatch_loss_sc"] < self.sc["mismatch_loss_d1"], \
    ...
    def test_sc_mw_greater_than_d1_mw(self):
        ...
        assert self.sc["M_W_sc_MeV"] > self.sc["M_W_d1_MeV"], \
            "SC M_W must exceed depth-1 M_W"
    ...
    def test_s11_sc_larger_than_s11_d1(self):
        ...
        assert abs(self.sc["S11_sc"]) > abs(self.sc["S11_d1"]), \
            "Back-saturation must increase |S11|"
    ...
    def test_tree_level_value(self):
        ...
        assert abs(self.sc["M_W_tree_MeV"] - m_w_tree_mev) < 0.01, \
...
class TestModuleConstants:
    ...
    def test_module_mw_equals_sc_result(self):
        ...
        assert abs(M_W_MEV - sc["M_W_sc_MeV"]) < 0.01, \
            f"Module M_W_MEV={M_W_MEV:.4f} != SC={sc['M_W_sc_MeV']:.4f}"

    def test_module_mw_not_equal_to_d1(self):
        """Confirm module-level M_W is NOT the old depth-1 value (regression guard)."""
        sc = w_boson_self_consistent_correction()
        assert abs(M_W_MEV - sc["M_W_d1_MeV"]) > 1.0, \
