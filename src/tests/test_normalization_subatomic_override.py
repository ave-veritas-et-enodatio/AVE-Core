...
class TestThreeObserversAgreeUnderR4:
    ...
    def engine_with_k4_poke(self):
        ...
        engine.k4.V_inc[site[0], site[1], site[2], 0] = engine.V_SNAP * np.sqrt(0.3)
        return engine, site
    ...
    def test_regime_and_node_observers_agree_on_a2_total(self, engine_with_k4_poke):
        ...
        # And they must agree
        assert regime_cap["max_A2_total"] == pytest.approx(node_cap["A2_yield_max"], rel=1e-10)
    ...
    def test_node_and_bond_observers_agree_on_site_a2(self, engine_with_k4_poke):
        ...
        assert node_cap["A2_yield_max"] == pytest.approx(float(bond_a2[alive].max()), rel=1e-10)
    ...
    def test_all_three_observers_report_identical_a2_field(
        ...
        assert cap["max_A2_total"] == pytest.approx(cap["max_A2_k4"] + cap["max_A2_cos"], rel=1e-10)
...
class TestR4PatchEliminatesAlphaConversion:
    ...
    def test_node_observer_does_not_divide_by_alpha(self):
        ...
        engine.k4.V_inc[site[0], site[1], site[2], 0] = engine.V_SNAP * np.sqrt(0.5)
