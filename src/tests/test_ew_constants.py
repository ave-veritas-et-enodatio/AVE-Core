...
class TestGaugeCouplings:
    ...
    def test_alpha_s_from_compliance(self) -> None:
    ...
    def test_alpha_s_exponent_is_spatial_fraction(self) -> None:
...
class TestHiggsMass:
    ...
    def test_n_k4_is_4(self) -> None:
        """K4 unit cell has 4 nodes."""
        assert N_K4 == 4

    def test_lambda_is_one_eighth(self) -> None:
        """λ_H = 1/(2N_K4) = 1/8."""
        assert LAMBDA_HIGGS == 1 / 8

    def test_higgs_mass_is_v_over_2(self) -> None:
        """m_H = v/√N_K4 = v/2."""
        expected = HIGGS_VEV_MEV / np.sqrt(N_K4)
        assert abs(M_HIGGS_MEV - expected) < 1e-6

    def test_higgs_mass_accuracy(self) -> None:
        """m_H ≈ 124,417 MeV (PDG: 125,100, Δ=0.55%)."""
        pdg = 125100  # MeV
        err = abs(M_HIGGS_MEV - pdg) / pdg
        assert err < 0.01  # < 1%

    def test_higgs_mass_from_sm_relation(self) -> None:
