...
def main():
    ...
    for i, (name, rv, color, marker) in enumerate(points):
        ax2.scatter(rv, y_positions[i], c=color, marker=marker,
                   s=120, edgecolors='black', linewidths=0.5, zorder=5)
        ax2.annotate(name, (rv, y_positions[i]),
    ...
    for rx, label in [(1e-15, 'I'), (0.3, 'II'), (0.95, 'III'), (3, 'IV')]:
        ax2.text(rx, 0.97, label, ha='center', va='top', fontsize=12,
    ...
    ax3.plot(r_p05, Q_p05, 'D', color='#FFC107', markersize=10,
            markeredgecolor='black', markeredgewidth=1)
    ax3.annotate(f'PONDER-05\nQ = {Q_p05:.1f}', (r_p05, Q_p05),
                fontsize=9, ha='left', xytext=(10, 0), textcoords='offset points')

    plt.tight_layout()

    # Save
    out_dir = os.path.join(os.path.dirname(__file__), '..', '..',
                          'manuscript', 'vol_1_foundations')
