...
def animate_bullet_cluster():
    ...
    title_text = ax.text(0.5, 1.05, "", transform=ax.transAxes, color='white', 
                         fontsize=14, ha='center', fontweight='bold')
    phase_text = ax.text(0.5, 0.95, "", transform=ax.transAxes, color='cyan', 
    ...
    with writer.saving(fig, output_path, dpi=120):
        for i in range(FRAMES):
            ...
            # Render Core bounds
            ax.scatter([pos_main_x/KPC], [0], color='cyan', marker='o', s=150, edgecolor='white', label='Main Cluster Core')
            ax.scatter([pos_bullet_x/KPC], [0], color='lime', marker='o', s=80, edgecolor='white', label='Bullet Core')
            
            # Add tracking
            ax.set_title("First-Principles Topological Lensing (A0 Metric Extension)", color='white', pad=35)
            ax.text(0.5, 1.02, baryon_state, transform=ax.transAxes, color='gray', fontsize=10, ha='center')
