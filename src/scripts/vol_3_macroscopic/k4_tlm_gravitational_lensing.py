...
from ave.core.k4_tlm import (
    K4Lattice2D, K4Node, build_k4_scattering_matrix, N_PORTS,
)
...
def run_lensing_sim(nx, ny, n_steps, n0, r_core, beam_y, beam_w,
...
def main():
    ...
    result_mod = run_lensing_sim(NX, NY, N_STEPS, n0=0.5, r_core=R_CORE,
                                  beam_y=BEAM_Y, beam_w=BEAM_W,
    ...
    result_ref = run_lensing_sim(NX, NY, N_STEPS, n0=0.0, r_core=R_CORE,
                                  beam_y=BEAM_Y, beam_w=BEAM_W,
                                  wavelength=WAVELENGTH, amplitude=AMPLITUDE)
    ...
    print("\n[3] Strong lens (n₀ = 0.8)...")
    result_strong = run_lensing_sim(NX, NY, N_STEPS, n0=0.8, r_core=R_CORE,
                                    beam_y=BEAM_Y, beam_w=BEAM_W,
                                    wavelength=WAVELENGTH, amplitude=AMPLITUDE)
    ...
    for idx, snap in enumerate(snaps):
        ...
        im = ax.imshow(
            snap.T, cmap='hot', origin='lower',
            extent=[0, NX, 0, NY],
            norm=PowerNorm(gamma=0.4, vmin=0, vmax=vmax),
        )

        # Mass center + core radius
        circle = plt.Circle((cx, cy), R_CORE, color='#00E5FF',
                            fill=False, lw=2, ls='--', alpha=0.7)
        ax.add_patch(circle)
        ax.plot(cx, cy, '+', color='#00E5FF', ms=12, mew=2.5)

        # Straight-line reference
        ax.axhline(BEAM_Y, color='white', ls=':', alpha=0.3, lw=1)
        ax.annotate('un-lensed path', (NX - 5, BEAM_Y + 1.5),
                    color='white', fontsize=7, ha='right', alpha=0.4)

        # n(r) contours
        ax.contour(np.arange(NX), np.arange(NY), n_field.T,
                   levels=[1.1, 1.2, 1.3], colors='#00E5FF',
    ...
    ax = axes2[0, 0]
    im = ax.imshow(n_field.T, cmap='viridis', origin='lower',
                   extent=[0, NX, 0, NY])
    ax.plot(cx, cy, 'w+', ms=14, mew=2.5)
    circle = plt.Circle((cx, cy), R_CORE, color='white',
                        fill=False, lw=2, ls='--')
    ax.add_patch(circle)
    ax.axhline(BEAM_Y, color='red', ls='--', lw=1.5, alpha=0.5,
               label=f'Beam path y₀={BEAM_Y}')
    ...
    im = ax.imshow(ref.T, cmap='hot', origin='lower',
                   extent=[0, NX, 0, NY],
                   norm=PowerNorm(gamma=0.4, vmin=0, vmax=vmax))
    ...
    im = ax.imshow(mod.T, cmap='hot', origin='lower',
                   extent=[0, NX, 0, NY],
                   norm=PowerNorm(gamma=0.4, vmin=0, vmax=vmax))
    circle = plt.Circle((cx, cy), R_CORE, color='#00E5FF',
    ...
    im = ax.imshow(strong.T, cmap='hot', origin='lower',
                   extent=[0, NX, 0, NY],
    ...
    im = ax.imshow(diff.T, cmap='RdBu_r', origin='lower',
                   extent=[0, NX, 0, NY],
                   vmin=-vmax_d * 0.5, vmax=vmax_d * 0.5)
    circle = plt.Circle((cx, cy), R_CORE, color='black',
                        fill=False, lw=1, ls='--', alpha=0.4)
    ax.add_patch(circle)
    ax.axhline(BEAM_Y, color='gray', ls=':', alpha=0.3)
    ax.set_title('Field Difference (Mod − Ref)\nAsymmetric phase → bending',
                 fontsize=12)
    ax.set_xlabel('x')
    ...
    max_p = max(np.max(profile_ref), np.max(profile_mod),
                np.max(profile_str), 1e-10)
    ax.fill_between(y, 0, profile_ref / max_p, alpha=0.2, color='blue',
                    label='Reference')
    ...
    ax.plot(y, profile_str / max_p, 'orange', lw=2, ls='--',
            label='n₀ = 0.8')
