...
def extract_bullet_stills():
    ...
    phases = [
        {"main_x": -900, "bullet_x": 900, "title": "Phase 1: Pre-Collision Vectors\nDark Matter Halos (Back-EMF) Intact"},
        {"main_x": -100, "bullet_x": 100, "title": "Phase 2: Kinematic Crossing\nGas Cores Stall, Linear Halos Superimpose"},
        {"main_x": 400, "bullet_x": -500, "title": "Phase 3: Spatial Offset\nHalos cross through unobstructed (Bullet Cluster Map)"}
    ]
    ...
    for i, phase in enumerate(phases):
        ...
        ax.scatter([phase["main_x"]], [0], color='cyan', marker='o', s=100, edgecolor='white', label='Main Baryonic Core')
        ax.scatter([phase["bullet_x"]], [0], color='lime', marker='o', s=60, edgecolor='white', label='Bullet Baryonic Core')
        
        # Add explicit Annotations
        if i == 1:
            ax.annotate('Linear Superposition\n(No Shock Damping)', xy=(0, 200), xytext=(0, 700),
                        arrowprops=dict(facecolor='white', shrink=0.05), color='white', ha='center')
        if i == 2:
            ax.annotate('Offset Halo\n(True Dark Matter)', xy=(phase["main_x"], 150), xytext=(phase["main_x"], 600),
                        arrowprops=dict(facecolor='white', shrink=0.05), color='white', ha='center')
            ax.annotate('Stalled Gas', xy=(phase["main_x"] - 200, 0), xytext=(phase["main_x"] - 200, -500),
