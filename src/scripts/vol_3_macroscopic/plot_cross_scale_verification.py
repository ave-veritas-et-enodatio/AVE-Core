...
for i, (name, log_scale, agreement, color) in enumerate(domains):
    ax.scatter([log_scale], [0], s=200, c=color, zorder=10,
               edgecolors='white', linewidths=1.5)
    # Alternate label positions
    y_off = 0.4 if i % 2 == 0 else -0.5
    va = 'bottom' if i % 2 == 0 else 'top'
    ax.annotate(f'{name}\n({agreement})',
                xy=(log_scale, 0), xytext=(log_scale, y_off),
...
ax.text(5, -1.05, r'$\mathbf{S(x, x_{yield}) = \sqrt{1 - (x/x_{yield})^2}}$',
        fontsize=14, color='#58a6ff', ha='center', va='center',
        fontweight='bold',
...
# Span arrow
ax.annotate('', xy=(26, -0.85), xytext=(-15, -0.85),
            arrowprops=dict(arrowstyle='<->', color='white', lw=1.5))
ax.text(5, -0.78, '39 orders of magnitude — one operator',
...
ax.set_yticks([])
ax.set_title('Cross-Scale Verification: One Saturation Kernel Across All Physics',
