...
def animate_2d_bullet_cluster():
    ...
    surf = ax.plot_surface(X_kpc, Y_kpc, Z_plot, cmap='magma', linewidth=0, antialiased=True, vmin=0, vmax=0.6, alpha=0.85)

    # Bare Mass Centers as reference posts pushing through
    ax.scatter([pos_main_x/KPC], [pos_main_y/KPC], [1.0], color='cyan', s=100, label='Main Baryonic Core', zorder=10)
    ax.scatter([pos_bullet_x/KPC], [pos_bullet_y/KPC], [0.8], color='lime', s=60, label='Bullet Baryonic Core', zorder=10)

    # Style Axis
    ax.set_title("First-Principles Topological Lensing (Dark Matter) \n 2D Radial Metric Strain", color='white', pad=20)
