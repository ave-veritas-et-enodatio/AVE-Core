...
def generate_weather_calculator():
    ...
    ax1.fill_between(voltages, C_CLASS_THRESHOLD, M_CLASS_THRESHOLD, color='#ffff99', alpha=0.2, label='C-Class Regime')
    ax1.fill_between(voltages, M_CLASS_THRESHOLD, X_CLASS_THRESHOLD, color='#ff9933', alpha=0.3, label='M-Class Regime')
    ax1.fill_between(voltages, X_CLASS_THRESHOLD, max(currents), color='#ff3333', alpha=0.4, label='X-Class Avalanche Region')
    ...
    # Plot the FWHM
    ax2.hlines(half_max, fwhm_start_yr, fwhm_end_yr, color='#ff33ff', lw=3, label=f'Solar Max FWHM: {fwhm_duration:.2f} Years')
