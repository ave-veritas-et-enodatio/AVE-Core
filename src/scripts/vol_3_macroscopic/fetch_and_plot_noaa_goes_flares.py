...
def simulate_empirical_noaa_overlay():
    ...
    ax.plot(years, dynamo_voltage, color='#66ccff', linestyle='-.', lw=1.5, alpha=0.6, label='AC Topological Dynamo (Magnetic Winding)')
    
    # Plot Empirical NOAA GOES Telemetry
    ax.scatter(empirical_years, empirical_intensities, color='#ff3333', marker='x', alpha=0.8, s=40, label='Empirical NOAA GOES (M/X Class Flares)')
    
    # Annotate FWHM Zones
    # Cycle 23
    ax.axvspan(2001.3 - 0.23, 2001.3 + 0.23, color='#ff33ff', alpha=0.2, label='Theoretical 0.46-Year Avalanche FWHM')
    # Cycle 24
