...
for i, (name, val, err, color, sigma) in enumerate(measurements):
    # Error bar
    ax.barh(i, 2*err, left=val-err, height=0.4, color=color, alpha=0.25,
...
ax.text(H_inf_kms + 0.15, len(measurements) - 0.3,
...
ax.set_title('The Hubble Tension: AVE Prediction in Context',
