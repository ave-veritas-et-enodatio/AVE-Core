...
def main():
    ...
    status_text = ax.text(0.05, 0.95, '', transform=ax.transAxes, color='white',
                          fontsize=12, verticalalignment='top',
                          bbox=dict(facecolor='black', alpha=0.7, edgecolor='none'))

    ax.axis('off')
    ax.set_title(r"Sagnac RLVG Validation: Metric Impedance Drag ($\Delta\Phi$)", color='white', pad=20, fontsize=14)
    ...
    def update(frame):
        ...
        status_text_content = (
            ...
            f"-----------------------------------\n"
            fr"Sagnac Phase Shift ($\Delta\Phi$): {delta_phase:.3f} rad"
        )
