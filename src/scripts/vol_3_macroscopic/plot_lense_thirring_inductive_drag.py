...
def generate_lense_thirring_figure():
    ...
    ax1.streamplot(X, Y, Vx, Vy, color='white', linewidth=lw,
                   density=1.5, arrowsize=1.5, arrowstyle='->')

    # Planet circle
    theta = np.linspace(0, 2 * np.pi, 100)
    ax1.fill(R_planet * np.cos(theta), R_planet * np.sin(theta),
             color='#3399ff', alpha=0.6, label='Rotating Body')
    ax1.plot(R_planet * np.cos(theta), R_planet * np.sin(theta),
             color='#33ffcc', lw=2)

    # Rotation arrow inside planet
    ax1.annotate('', xy=(0.4, 0.3), xytext=(-0.3, 0.4),
    ...
    ax1.set_title("Gravitomagnetic Frame-Dragging\n(2D Equatorial Mutual Inductance Map)",
    ...
    ax2.semilogy(r, Omega_AVE_full, color='#33ffcc', lw=3,
                 label='AVE: $\\Omega_{M_{12}} = M_{12}(r) \\cdot I_{mass}$')
    ax2.semilogy(r, Omega_AVE_near, color='#ffcc00', lw=1.5, linestyle=':',
                 alpha=0.5, label='Near-Field: $1/r^2$ (Inductive)')
    ...
    ax2.set_title("Radial Decay: AVE Mutual Inductance vs GR",
                  color='white', fontsize=14, pad=15)
