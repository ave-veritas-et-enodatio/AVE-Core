...
from ave.gravity.galactic_mond_drag import (
    get_a0,
...
def simulate_galactic_curve():
    ...
    print(f"{'R (kpc)':<10} | {'M_enc (M_sun)':<15} | {'g_N (m/s^2)':<15} | {'v_Newton (km/s)':<18} | {'v_Topo (km/s)':<18} | {'Regime'}")
    print("-" * 105)
    ...
    for r_kpc in radii_kpc:
        ...
        print(f"{r_kpc:<10.1f} | {m_enc_s:<15.2e} | {g_n_base:<15.2e} | {v_newt_km:<18.2f} | {v_eff_km:<18.2f} | {regime}")
    ...
    if transition_boundary_kpc:
        ...
        print("This intact geometry actively resists macroscopic velocity vectors, naturally flattening the rotation curve without phantom particle halos.")
    else:
