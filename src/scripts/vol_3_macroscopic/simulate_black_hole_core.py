...
def run_simulation():
    ...
    for r_check, label in [(r_sat*10, "10×r_sat"), (r_sat*2, "2×r_sat"),
                           (r_sat*1.01, "1.01×r_sat"), (r_s, "r_s (EH)")]:
        ...
        print(f"    r = {r_check/r_s:6.2f} r_s: ε₁₁ = {eps:.4f}, S = {S:.4f}, "
              f"n = {n:.4f}, Z = {Z:.2f} Ω")
    ...
    print(f"    Q = ℓ = {Q_mode}")
    print(f"    ω_R·M_g = {omega_R * M_g / C_0:.4f}  (GR exact: 0.3737, Δ = {(omega_R*M_g/C_0 - 0.3737)/0.3737*100:+.1f}%)")

    # ── LIGO Event Comparison ──────────────────────────────────────────────
    print(f"\n  ── LIGO EVENT COMPARISON ──")
    print(f"  {'Event':>12} {'M_final':>8} {'a*':>5} {'f_AVE':>8} {'f_obs':>8} {'Δf':>8} "
    ...
    for name, data in LIGO_EVENTS.items():
        ...
        ligo_results.append((name, data["M_final"], a_star, f_kerr, f_obs,
                             delta_f, tau_ave*1e3, tau_obs*1e3, delta_tau))

        print(f"  {name:>12} {data['M_final']:8.1f} {a_star:5.2f} "
    ...
    ax1.axvline(x=r_sat/r_s, color="#ffaa44", linewidth=2, linestyle="--",
                label=f"$r_{{sat}} = 3.5\\,r_s$")
    ax1.axvline(x=1.0, color="#ff4444", linewidth=1.5, linestyle=":",
                label=f"$r_s$ (Event Horizon)")
    ...
    style_ax(ax2)
    ax2.plot(r_norm, G_shear, color="#44aaff", linewidth=2.5,
             label=r"$G_{shear}/G_0 = S(r)$")
    ax2.plot(r_norm, c_g / C_0, color="#ffaa44", linewidth=2.5,
             label=r"$c_g/c = (1-\varepsilon^2)^{1/4}$")
    ax2.axvline(x=r_sat/r_s, color="#ffaa44", linewidth=2, linestyle="--",
                label=f"Phase transition")
    ax2.axvspan(0, r_sat/r_s, alpha=0.08, color="#ff4444")
    ax2.text(1.5, 0.15, "RUPTURED\n(no shear waves)", color="#ff6666",
             fontsize=10, fontweight="bold", ha="center")
    ax2.text(6, 0.85, "ELASTIC\n(GWs propagate)", color="#44aaff",
             fontsize=10, fontweight="bold", ha="center")
    ax2.set_xlabel(r"$r / r_s$", color=C_TEXT, fontsize=11)
    ax2.set_ylabel("Normalized", color=C_TEXT, fontsize=11)
    ax2.set_title("Lattice Phase Transition:\nShear Modulus & Group Velocity", color=C_TEXT, fontsize=13, fontweight="bold")
    ...
    ax3.plot(r_norm, n_r, color="#44ff88", linewidth=2,
             label=r"$n(r) = 1 + 2GM/(c^2 r)$")
    ...
    ax3.set_title("Symmetric Gravity:\nZ = Z₀ (invariant), n(r) diverges", color=C_TEXT, fontsize=13, fontweight="bold")
    ...
    ax4.text(0.05, 0.95, iso_text, transform=ax4.transAxes,
    ...
    ax5.set_title("QNM Ringdown: AVE vs LIGO\n(Kerr-corrected, ℓ = 2)", color=C_TEXT, fontsize=13, fontweight="bold")
    ...
    ax6.axhline(y=2.725, color="#44aaff", linewidth=1, linestyle="--", alpha=0.7,
                label="CMB: 2.725 K")
    ax6.axhline(y=1e12, color="#ff4444", linewidth=1, linestyle=":", alpha=0.5,
                label=r"$10^{12}$ K (hadron epoch)")
    # Mark 10 M_sun
    ax6.scatter([10], [HBAR * C_0**3 / (8*np.pi*G*10*M_SUN*k_B)],
                color="#44ff88", s=100, zorder=5, edgecolors="#ffffff")
    ax6.text(10, HBAR * C_0**3 / (8*np.pi*G*10*M_SUN*k_B) * 3,
             "10 M☉", color="#44ff88", fontsize=9)

    ax6.set_xlabel(r"$M / M_\odot$", color=C_TEXT, fontsize=11)
    ax6.set_ylabel("Hawking Temperature (K)", color=C_TEXT, fontsize=11)
    ax6.set_title("Hawking Temperature:\nImpedance Boundary Noise", color=C_TEXT, fontsize=13, fontweight="bold")
