...
def generate():
    ...
    ax1.scatter(gear_x, gear_omega, s=sizes, c='#ff6699', edgecolors='white',
                linewidth=1.5, zorder=5, alpha=0.8)

    # Arrows showing slipping
    for i in range(0, len(gear_x) - 1, 2):
        ax1.annotate('', xy=(gear_x[i + 1], gear_omega[i + 1]),
    ...
    ax2.scatter(gear_x2, gear_omega2, s=150, c='#33ffcc', edgecolors='white',
                linewidth=2.0, zorder=5, alpha=0.9, marker='o')

    # Draw rigid coupling lines between gears
    for i in range(len(gear_x2) - 1):
        ax2.plot([gear_x2[i], gear_x2[i + 1]], [gear_omega2[i], gear_omega2[i + 1]],
                 color='#ffcc00', lw=2, alpha=0.5)

    # Mark lambda_L
    ax2.axvline(lambda_L, color='#ffcc00', linestyle='--', lw=2, alpha=0.7,
                label=f'$\\lambda_L = {lambda_L}$')

    ax2.axhline(0, color='white', lw=0.5, alpha=0.3)
    ax2.set_title("Superconductor ($T < T_c$)\n(Phase-Locked Macroscopic Gear Train)", color='white', fontsize=14, pad=15)
    ...
    # Annotation
    ax2.text(1.5, 0.5, "Infinite Macroscopic\nInertia → Perfect\nField Expulsion",
             color='#ffcc00', fontsize=12, ha='center', va='center',
