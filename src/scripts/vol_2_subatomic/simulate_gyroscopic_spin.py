...
def main():
    ...
    def plot_sphere(ax, title, color_line, x_data, y_data, z_data):
        ...
        for idx in [0, len(x_data)//4, len(x_data)//2, 3*len(x_data)//4, -1]:
            ax.quiver(0, 0, 0, x_data[idx], y_data[idx], z_data[idx], 
                      color=color_line, alpha=0.9, arrow_length_ratio=0.1, lw=3)
    ...
    plot_sphere(ax1, "Quantum Dirac Spinor\n(Bloch Sphere $|\psi\\rangle$ Expectation)", 'royalblue', S_x, S_y, S_z)

    # Top Right: 3D Classical Gyroscope
    ax2 = fig.add_subplot(gs[0, 1], projection='3d')
    plot_sphere(ax2, "Classical Macro-Gyroscope\n(Mechanical $3_1$ Defect Precession)", 'darkorange', L_x, L_y, L_z)
