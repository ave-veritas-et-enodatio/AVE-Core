...
def figure_electron() -> None:
    ...
    ax = setup_3d_ax(fig)
    x, y, z, t = unknot()
    plot_phase_loop(ax, x, y, z, t, ELECTRON_COLOR, label=r"Electron  $e^-$  ·  $0_1$ Unknot  ·  0.511 MeV")
...
def figure_muon() -> None:
    ...
    plot_phase_loop(ax, x, y, z, t, MUON_COLOR, label=r"Muon  $\mu^-$  ·  Cosserat Rotation  ·  105.7 MeV")
...
def figure_tau() -> None:
    ...
    plot_phase_loop(ax, x, y, z, t, TAU_COLOR, label=r"Tau  $\tau^-$  ·  Curvature-Twist  ·  1776.8 MeV")
...
def figure_neutrino() -> None:
    ...
    plot_phase_loop(ax, x, y, z, t, NEUTRINO_COLOR, label=r"Neutrino  $\nu_e$  ·  Twisted $0_1$  ·  < 0.8 eV")
...
def figure_proton() -> None:
    ...
    for i, (x, y, z, t) in enumerate(rings):
        ax.plot(x, y, z, color=RING_COLORS[i], linewidth=4, alpha=0.85, label=f"Loop {i+1}")
    ax.text2D(
        0.5,
        0.02,
        r"Proton  $p$  ·  $6^3_2$ Borromean  ·  938.3 MeV",
        transform=ax.transAxes,
        color="white",
        fontsize=11,
        ha="center",
...
def figure_summary_panel() -> None:
    ...
    panels = [
        ...
        (
            "Muon $\\mu^-$\nCosserat Rotation\n105.7 MeV",
            lambda: excited_unknot(mode="rotation"),
            MUON_COLOR,
            "fermion",
        ),
        (
            "Tau $\\tau^-$\nCurvature-Twist\n1776.8 MeV",
            lambda: excited_unknot(mode="curvature"),
            TAU_COLOR,
            "fermion",
        ),
