    ...
    def step(self, n_steps=1):
        ...
        for _ in range(n_steps):
            ...
            if self.l > 0:
                ...
                self.u_E[1:-1] -= (
                    dt**2 * C_0**2 * self.V_cent[1:-1] * self.u_E[1:-1]
                    / (self.eps_r[1:-1] * mu_at_interior)
                )
...
def plot_comparison(sim, path):
    ...
    ax.plot(sim.r_b, P_sim_plot, color=COLORS['ave'], lw=2.5,
    ...
    P_qm /= np.max(P_qm)
    ax.plot(r_qm, P_qm, '--', color=COLORS['qm'], lw=2,
            label='Analytical 1s (QM)', zorder=2)

    ax.axvline(1.0, color='#444444', ls=':', lw=1)
    ax.text(1.05, 0.92, 'a₀', color='#666666', fontsize=10,
            transform=ax.get_xaxis_transform())

    _style_ax(ax, 'r / a₀', 'P(r)  [normalised]',
...
def make_animation(sim, n_warmup, n_frames, steps_per_frame, path):
    ...
    _style_ax(ax, 'r / a₀', '|u(r)|²  [a.u.]',
              'Standing Wave Formation in Nuclear Impedance Cavity')
    ax.set_xlim(0, 15)
    ...
    ax.plot(r_qm, P_qm, '--', color=COLORS['qm'], lw=1.5, alpha=0.4,
            label='1s analytical')
    ...
    anim.save(path, writer='pillow', dpi=120, savefig_kwargs={
...
def make_2d_orbital(sim, n_quantum, l_quantum, m_quantum, path):
    ...
    im = ax.pcolormesh(X, Z, density, cmap='hot', shading='gouraud',
                       vmin=0, vmax=0.5)
    ax.contour(X, Z, density, levels=[0.02, 0.1, 0.3],
               colors='white', linewidths=0.5, alpha=0.3)
    ax.set_aspect('equal')
    _style_ax(ax, 'x / a₀', 'z / a₀',
...
def main():
    ...
    make_animation(sim_s, n_warmup=500, n_frames=150,
                   steps_per_frame=200, path=os.path.join(OUT, 'orbital_1s_formation.gif'))
    ...
    make_2d_orbital(sim_cmp, 1, 0, 0,
                    os.path.join(OUT, 'orbital_1s_cross_section.png'))
    ...
    make_2d_orbital(sim_p, 2, 1, 0,
                    os.path.join(OUT, 'orbital_2p_cross_section.png'))
    ...
    make_2d_orbital(sim_d, 3, 2, 0,
                    os.path.join(OUT, 'orbital_3d_cross_section.png'))
