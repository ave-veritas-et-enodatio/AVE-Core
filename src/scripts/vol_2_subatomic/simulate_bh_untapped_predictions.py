...
def main():
    ...
    ax1.plot(M_range, f_schw, color='#00ccff', linewidth=2, alpha=0.5,
             linestyle='--', label='Schwarzschild ($a_* = 0$)')
    ax1.plot(M_range, f_kerr_07, color='#00ff88', linewidth=2.5,
             label='Kerr-corrected ($a_* = 0.7$)')
    ...
    for i, (name, data) in enumerate(LIGO_EVENTS.items()):
        ...
        ax1.scatter(M_obs, f_obs, color=colors_ligo[i], s=120, marker='*',
                    zorder=10, edgecolors='white', linewidths=0.5,
                    label=f'{name}: {f_obs} Hz')
        ax1.scatter(M_obs, f_ave_kerr, color='#00ff88', s=80, marker='o',
                    zorder=10, edgecolors='white', linewidths=0.5)
    ...
    for i, be in enumerate(band_E):
        ...
        ax2.text(be, 0.85 - i * 0.12, f'n={i+1}', color=color, fontsize=9,
    ...
    ax2.set_title('2. Iron Kα Line Profile\n(Impedance Band Sub-Peaks)', fontsize=13,
                  color='white', pad=10)
    ...
    pcm = ax3.pcolormesh(Theta_map, R_map, Gamma_map,
                         cmap='inferno', vmin=0, vmax=1.0, shading='auto')
    ...
    ax3.annotate('JET\n$\\Gamma \\approx 0$', xy=(0.0, 3.0), fontsize=10,
                 color='#00ff88', fontweight='bold', ha='center')
    ax3.annotate('DISK\n$\\Gamma \\to 1$', xy=(np.pi/2, 5.0), fontsize=9,
                 color='#ff4444', fontweight='bold', ha='center')

    ax3.set_title('3. Jet Launching:\nPolar Impedance Matching', fontsize=13,
                  color='white', pad=20)
    ...
    ax4.plot(M_hawk / M_SUN, T_hawk, color='#ff66cc', linewidth=2.5,
    ...
    for M_t, name, col in targets:
        ...
        ax4.scatter(M_t, T_t, color=col, s=100, zorder=10,
                    edgecolors='white', linewidths=0.5)
        ax4.annotate(name, (M_t, T_t), textcoords="offset points",
                     xytext=(10, 10), color=col, fontsize=9, fontweight='bold')

    ax4.set_xscale('log')
    ax4.set_yscale('log')
    ax4.set_xlabel('Mass [$M_\\odot$]', fontsize=11, color='white')
    ax4.set_ylabel('Hawking Temperature [K]', fontsize=11, color='white')
    ax4.set_title('4. Hawking Temperature\n(Impedance Boundary Noise)', fontsize=13,
                  color='white', pad=10)
    ...
    ax5.plot(h_peak, h_memory, color='#00ff88', linewidth=2.5,
             label=r'$\Delta h = h_{peak} \cdot (h/h_{yield})^2$')
    ax5.axvline(np.sqrt(ALPHA), color='#ffaa00', linestyle='--', alpha=0.7,
                label=f'$h_{{yield}} = \\sqrt{{\\alpha}} \\approx {np.sqrt(ALPHA):.4f}$')

    # Mark LIGO sensitivity
    ax5.axvline(1e-21, color='#ff4444', linestyle=':', alpha=0.5,
                label='LIGO sensitivity ($10^{-21}$)')
    ...
    ax5.set_title('5. GW Memory\n(Residual Lattice Deformation)', fontsize=13,
                  color='white', pad=10)
    ...
    ax6.bar(x_ev - width/2, tau_ave_vals, width, color='#00ff88', alpha=0.8,
            edgecolor='white', linewidth=0.5, label='AVE (Axiom 4 sat.)')
    ax6.bar(x_ev + width/2, tau_obs_vals, width, color='#ff4444', alpha=0.8,
            edgecolor='white', linewidth=0.5, label='LIGO Observed')

    for i, name in enumerate(events):
        err = abs(tau_ave_vals[i] - tau_obs_vals[i]) / tau_obs_vals[i] * 100
        ax6.text(i, max(tau_ave_vals[i], tau_obs_vals[i]) + 0.5,
                 f'Q={Q_vals[i]:.1f}\n{err:.0f}%', ha='center', fontsize=9,
    ...
    ax6.set_title('6. Axiom 4 Cavity Q\n$n_{max} = 1/\\sqrt{\\alpha}$ Saturation',
    ...
    for name, data in LIGO_EVENTS.items():
        ...
        print(f"  {name:12s}: f_Kerr={f_kerr:7.1f} Hz ({err_f:4.1f}%) | Q={Q:.1f} | τ={tau*1e3:.2f} ms ({err_t:.1f}%) | Obs: f={f_obs} Hz, τ={tau_obs*1e3:.1f} ms")
