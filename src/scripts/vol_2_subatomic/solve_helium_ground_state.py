...
def solve_hartree_scf(Z_nuc=2, max_iter=100, mix=0.5, tol=1e-8):
    ...
    for it in range(max_iter):
        ...
        if it % 5 == 0 or dZ < tol:
            print(f"    iter {it:3d}:  Z_eff = {Z_new:.6f}  "
                  f"E_orbital = {res.fun/eV:.6f} eV")
    ...
    return {
        'Z_eff': Z_eff, 'E_orbital': E_orb, 'V_ee': V_ee,
        'E_total': E_total, 'r': r, 'u': u_f, 'V_scf': V_scf_f,
        'history': history,
    }
...
def main():
    ...
    print(f"    E_pert    = E_0 + ⟨V_ee⟩₁   = {E_pert/eV:.4f} eV  "
    ...
    print(f"    E_var             = {E_var/eV:.4f} eV  "
          f"({abs(E_var/eV - E_exp)/abs(E_exp)*100:.2f}% error)")
    ...
    print(f"    E_SCF        = {E_h:.4f} eV  "
          f"({abs(E_h - E_exp)/abs(E_exp)*100:.2f}% error)")
    ...
    print(f"  {'─'*38} {'─'*9} {'─'*7} {'─'*20}")
    print(f"  {'Perturbation (1st order)':<38} {E_pert/eV:>9.4f} {abs(E_pert/eV-E_exp)/abs(E_exp)*100:>6.2f}% {'Axiom 2 (αℏc)':>20}")
    print(f"  {'Variational (Z_eff = 27/16)':<38} {E_var/eV:>9.4f} {abs(E_var/eV-E_exp)/abs(E_exp)*100:>6.2f}% {'Axiom 2 (αℏc)':>20}")
    print(f"  {'Self-consistent Hartree':<38} {E_h:>9.4f} {abs(E_h-E_exp)/abs(E_exp)*100:>6.2f}% {'Axiom 2 (αℏc)':>20}")
    print(f"  {'─'*38} {'─'*9} {'─'*7} {'─'*20}")
    print(f"  {'HF limit (literature)':<38} {-77.87:>9.4f} {abs(-77.87-E_exp)/abs(E_exp)*100:>6.2f}% {'Standard QM':>20}")
    print(f"  {'─'*38} {'─'*9} {'─'*7} {'─'*20}")
    print(f"  {'AVE topological J_s2 (Ch.16)':<38} {-79.00:>9.4f} {abs(-79.00-E_exp)/abs(E_exp)*100:>6.2f}% {'Axioms 2+4 + topo':>20}")
    ...
    ax.set_title('Variational Energy E(Z_eff)', fontsize=13,
    ...
    u = hartree['u']
    P = u**2; P /= np.max(P)
    ax.plot(r_b, P, color=COL['ave'], lw=2.5,
            label=f'He 1s (Z_eff = {hartree["Z_eff"]:.3f})')
    ax.fill_between(r_b, 0, P, color=COL['ave'], alpha=0.1)
    u_bare = hydrogen_like_u(r, 2.0)
    P_b = u_bare**2; P_b /= np.max(P_b)
    ax.plot(r_b, P_b, '--', color=COL['exp'], lw=1.5, alpha=0.7,
            label='Bare Z=2 (no screening)')
    ax.set_xlabel('r / a₀', fontsize=11, color='#cccccc')
    ax.set_ylabel('|u(r)|²', fontsize=11, color='#cccccc')
    ax.set_title('He 1s Radial Wavefunction', fontsize=13,
                 fontweight='bold', color='white', pad=10)
    ax.set_xlim(0, 5)
    ax.legend(fontsize=9, framealpha=0.3)
    ax.tick_params(colors='#aaaaaa', labelsize=9)
    ...
    ax.plot(r_b, V_scf/eV, color=COL['scf'], lw=1.5,
            label='V_scf (e⁻ screening)')
    ax.plot(r_b, (V_nuc+V_scf)/eV, color=COL['ave'], lw=2,
            label='V_total')
    ax.set_xlabel('r / a₀', fontsize=11, color='#cccccc')
    ax.set_ylabel('V(r) [eV]', fontsize=11, color='#cccccc')
    ax.set_title('Effective Potential', fontsize=13,
                 fontweight='bold', color='white', pad=10)
    ax.set_xlim(0, 5)
    ax.set_ylim(-120, 60)
    ...
    fig.suptitle(
        f'AVE Helium: Variational = {E_var/eV:.2f} eV,  '
        f'J_s2 = −79.00 eV,  Exp = {E_exp:.3f} eV',
