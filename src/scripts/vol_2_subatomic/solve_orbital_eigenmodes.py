...
from ave.core.constants import (
    C_0, MU_0, EPSILON_0, M_E, M_PROTON, HBAR, ALPHA,
    L_NODE, V_SNAP, Z_0, e_charge,
)
...
class OrbitalODE:
    ...
    def shoot(self, E):
        ...
        sol = solve_ivp(
            self._ode_rhs, [r_min, self.r_max], [u0, du0],
            args=(E,), t_eval=self.r, method='RK45',
    ...
    def find_eigenvalue(self, n, E_low=None, E_high=None):
        ...
        try:
            E_eigen = brentq(self.shoot, E_low, E_high,
                             xtol=1e-6 * abs(E_low), maxiter=200)
        except ValueError:
            ...
            for i in range(len(vals) - 1):
                if vals[i] * vals[i+1] < 0:
                    E_eigen = brentq(self.shoot, E_scan[i], E_scan[i+1],
                                     xtol=1e-6 * abs(E_scan[i]))
                    return E_eigen
...
def plot_eigenmodes(solver, modes, path):
    ...
    n_modes = len(modes)
    fig, axes = plt.subplots(n_modes, 1, figsize=(11, 4 * n_modes),
                              facecolor='#0a0a14')
    if n_modes == 1:
        axes = [axes]
    ...
    for ax, (n, l) in zip(axes, modes):
        ...
        ax.plot(ode.r_b, P, color=COL['ave'], lw=2.5,
                label='AVE Cavity Eigenmode', zorder=3)
        ...
        P_qm = u_qm**2
        P_qm /= np.max(P_qm)
        ax.plot(r_qm, P_qm, '--', color=COL['qm'], lw=2,
                label='Analytical QM', zorder=2)

        r_peak = ode.r_b[np.argmax(P)]

        _ax(ax, 'r / a₀', 'P(r)',
            f'{n}{"spdfg"[l]}: E = {E_eV:.4f} eV  (QM: {E_exact_eV:.4f} eV)  '
            f'Peak at r = {r_peak:.2f} a₀')
        ax.legend(fontsize=10, loc='upper right', framealpha=0.3)
        ax.set_xlim(0, min(ode.r_b[-1], 5 * n**2))
        ax.set_ylim(0, 1.15)

        print(f'    E_AVE = {E_eV:.6f} eV,  E_QM = {E_exact_eV:.6f} eV,  '
...
def plot_2d_orbital(ode, n, l, m, u, path):
    ...
    im = ax.pcolormesh(X, Z, density, cmap='inferno', shading='gouraud',
                       vmin=0, vmax=0.6)
    ax.contour(X, Z, density, levels=[0.01, 0.05, 0.15, 0.35],
               colors='white', linewidths=0.4, alpha=0.35)
    ax.set_aspect('equal')
    _ax(ax, 'x / a₀', 'z / a₀',
        f'AVE Orbital: {n}{"spdfg"[l]} (m={m})')
    ax.set_xlim(-lim, lim)
    ax.set_ylim(-lim, lim)
...
def make_orbital_animation(ode, n, l, m, u, path, n_frames=120):
    ...
    ax.set_aspect('equal')
    _ax(ax, 'x / a₀', 'z / a₀',
        f'AVE Standing Wave: {n}{"spdfg"[l]} (m={m})')
    ax.set_xlim(-lim, lim)
    ax.set_ylim(-lim, lim)
...
def main():
    ...
    plot_eigenmodes(None, modes,
                    os.path.join(OUT, 'orbital_eigenmode_comparison.png'))
    ...
    for n, l, m in orbitals_2d:
        ...
        plot_2d_orbital(ode, n, l, m, u,
                        os.path.join(OUT, f'{name}_cross_section.png'))
    ...
    make_orbital_animation(ode_1s, 1, 0, 0, u_1s,
                           os.path.join(OUT, 'orbital_1s_oscillation.gif'))
    ...
    make_orbital_animation(ode_2p, 2, 1, 0, u_2p,
                           os.path.join(OUT, 'orbital_2p_oscillation.gif'))
