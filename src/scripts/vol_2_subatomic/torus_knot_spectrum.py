...
def compute_I_scalar(crossing_number, kappa=KAPPA_FS):
    ...
    def objective(params):
        r_opt, n = params
        integral, _ = quad(energy_density, 0, 10 * r_opt,
                           args=(r_opt, n, kappa), limit=100)
        return integral
...
def run_spectrum():
    ...
    for c in crossing_numbers:
        ...
        print(f"  (2,{c:2d}):  {m_MeV:7.1f} MeV  →  {best_name:10s} "
    ...
    for name, (mass, jp, stars) in PDG_RESONANCES.items():
        ...
        ax.text(15.5, mass, f'{name}  ({jp})', fontsize=8,
                va='center', color='#1565C0', alpha=0.8)

    # Plot predictions
    pred_c = [r[0] for r in results]
    pred_m = [r[1] for r in results]
    ax.scatter(pred_c, pred_m, s=120, c='#E53935', zorder=5,
               edgecolors='#B71C1C', linewidths=1.5, label='AVE Prediction')
    ...
    for c, m_pred, pdg_name, pdg_mass, jp, pct in results:
        if c >= 5:
            ax.annotate(f'{m_pred:.0f} MeV\n({pct:+.1f}%)',
                        (c, m_pred), textcoords='offset points',
                        xytext=(-50, 10), fontsize=8, color='#B71C1C',
    ...
    ms_assets = os.path.join(os.path.dirname(__file__), '..', '..',
                             'assets', 'sim_outputs')
