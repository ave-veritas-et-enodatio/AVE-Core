...
def generate_string_mapping():
    ...
    ax1.plot(
        x_continuous,
        np.interp(x_continuous, range(N_nodes), snapshot),
        color="#00ffff",
        linewidth=3,
        label="Abstract Continuous String (M-Theory)",
    )
    ...
    ax1.scatter(
        ...
        s=100,
        zorder=5,
        label="Boundary Conditions (Dirichlet)",
    )
    ...
    ax2.plot(
        ...
        linewidth=2,
        linestyle="--",
        label=r"Continuous Path: Phase Gradient ($\nabla \phi$)",
    )
    ...
    ax2.scatter(
        ...
        zorder=5,
        edgecolors="white",
        label=r"Physical Quantization: Discrete Inductive Nodes ($L$)",
    )
