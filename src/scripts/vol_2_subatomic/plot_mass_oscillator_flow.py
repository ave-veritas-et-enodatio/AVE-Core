...
def create_flowchart():
    ...
    draw_box(
        0.5,
        0.85,
        "1. Ansatz Mass Estimate ($m_0$)\nSet initial particle radius $r_0 \propto 1/m_0$",
        "#00ffff",
    )
    ...
    draw_box(
        0.5,
        0.65,
        "2. Geometric Integrals (FEM)\nCompute dimensionless scalar fields:\n$I_{scalar}$, $I_{vector}$, $I_{tensor}$",
        "#ff00aa",
    )
    ...
    draw_box(
        0.5,
        0.25,
        "4. Physical Feedback (LC Network)\nCalculate Total Potential $V_{total}$\nApply Packing Fraction $p_c$",
        "#ffaa00",
    )
    ...
    plt.suptitle(
        "AVE Framework: Self-Consistent Mass Oscillator",
        color="white",
        fontsize=16,
        fontweight="bold",
        y=0.98,
