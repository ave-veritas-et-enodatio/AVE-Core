...
def generate_hoop_stress_visual() -> None:
    ...
    ax1.legend(
        handles=[
            arrow_legend,
            Line2D([0], [0], color="#00ffff", lw=2, label="1D Inductive String"),
        ],
        loc="center",
        facecolor="black",
        edgecolor="white",
    ...
    ax2.plot(
        ...
        color="#00ff00",
        lw=3,
        label="Numerically Integrated Tension ($T_{sim}$)",
    )
    ax2.axhline(
        T_theoretical,
        color="white",
        linestyle="--",
        lw=2,
        label=r"Analytic Hoop Stress Projection ($F_{tot} / 2\pi$)",
    )
    ...
    plt.suptitle(
        "AVE Continuum Mechanics: Generative Origin of the MOND Limit",
        color="white",
        fontsize=20,
        weight="bold",
        y=0.98,
    ...
    output_path = sim_output("unruh_hawking_hoop_stress.png")
