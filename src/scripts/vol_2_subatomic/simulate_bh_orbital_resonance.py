...
def main():
    ...
    ax1.axvline(rph_grs / rs_grs, color='#ffaa00', linestyle='--', alpha=0.8, label=r'Photon Sphere ($r_{ph}$)')
    ...
    for i, (br, bm) in enumerate(zip(band_radii, band_modes)):
        ...
        ax2.text(br_norm + 0.15, n_at_band + 0.08, f'n={bm}', color=color, fontsize=9,
                 fontweight='bold')
    ...
    ax3.plot(modes_grs, freqs_grs, 'o-', color='#00ccff', markersize=10, linewidth=2,
             label=f'GRS 1915+105 (14 M☉)', zorder=5)

    # Observed QPOs for GRS 1915+105
    observed_qpo_grs = [67.0, 113.0]
    observed_modes_grs = [1, 2]
    ax3.scatter([1], [67.0], color='red', s=150, marker='*', zorder=10,
                label='Observed: 67 Hz', edgecolors='white', linewidths=0.5)
    ax3.scatter([2], [113.0], color='#ff4444', s=150, marker='*', zorder=10,
                label='Observed: 113 Hz', edgecolors='white', linewidths=0.5)

    # Annotate ratio
    if len(freqs_grs) >= 2:
        ratio_12 = freqs_grs[0] / freqs_grs[1]
        ax3.text(1.5, max(freqs_grs[0], 80),
