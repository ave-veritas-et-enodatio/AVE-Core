...
def print_yang_mills_block() -> dict:
    ...
    rows = [
        ...
        ("Part C",  f"Mass gap Δ = {C['gap_MeV']:.4f} MeV > 0",
                                                        _tick(C["gap_positive"])),
        ...
        ("Part E",  "OS3 Reflection Positivity (T=e^{-ℓH}≥0)", _tick(E["OS3_reflection_positivity"])),
    ...
    print()
    print(f"  Cluster decomposition length ξ = {xi_m:.3e} m "
          f"(= κ_FS/3 × ℓ_node, DERIVED MAGIC NUMBER)")
...
def print_navier_stokes_block() -> dict:
    ...
    rows = [
        ...
        ("Step 1", f"Discrete Laplacian ‖∇²‖ = 4/ℓ² = {lap_norm:.3e} m⁻²",
        ...
        ("Step 3", "Picard-Lindelöf guarantees unique global sol.",
                                                             _tick(S3["picard_lindelof_applies"])),
        ("Step 4", "Discrete Laplacian → continuum (O(ℓ²))", _tick(S4["discrete_laplacian_converges"])),
        ...
        ("Step 4", "H¹ per unit length constant (vs N)",     _tick(sobolev["h1_norm_per_length_constant"])),
    ]
...
def print_riemann_block() -> dict:
    ...
    print(f"  Lattice ABCD det          = {recip['det_ABCD']:.4f}  "
...
def print_summary_block() -> None:
    ...
    col_w = [18, 46, 6]
    head = (f"  {'Problem':<{col_w[0]}} {'AVE Status':<{col_w[1]}} "
            f"{'Proven':>{col_w[2]}}")
    print(head)
    print("  " + "─" * (sum(col_w) + 4))
