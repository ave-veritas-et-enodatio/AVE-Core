...
def main():
    ...
    fig, (ax_3d, ax_info) = plt.subplots(1, 2, figsize=(14, 6),
    ...
    def update(frame):
        ...
        if propagating:
            ...
            for i in range(0, len(x) - 1, 2):
                if e_mag[i] > e_max * 0.03:
                    ax_3d.plot([x[i], x[i+1]],
                              [mid + Ey[i], mid + Ey[i+1]],
                              color=colors_e[i], linewidth=2.5,
            ...
            for glow in [4.0, 2.5, 1.5, 1.0]:
                ...
                ax_3d.plot(ring_x, ring_y,
                          color='#ff6622', linewidth=glow * 3,
                          alpha=alpha, solid_capstyle='round')
            ax_3d.plot(ring_x, ring_y,
                      color='#ffaa44', linewidth=2.0,
                      alpha=brightness * 0.9, solid_capstyle='round')

            # Draw the lattice node it sits on
            ax_3d.scatter([mid], [mid], s=100, color='#333366',
                         marker='s', zorder=0, alpha=0.5)

            # Radial field lines (the strain on the node)
            for angle in np.linspace(0, 2 * np.pi, 8, endpoint=False):
                r1 = ring_r * 1.2
                r2 = ring_r * 3.0 * brightness
                ax_3d.plot([mid + r1 * np.cos(angle), mid + r2 * np.cos(angle)],
                          [mid + r1 * np.sin(angle), mid + r2 * np.sin(angle)],
                          color='#ff4422', linewidth=0.8, alpha=0.3 * brightness)

            # Label
            ax_3d.text(mid, mid - 8, 'TRAPPED UNKNOT\n(electron)',
                      ha='center', color='#ffaa44', fontsize=11,
                      fontweight='bold', alpha=brightness)
        ...
        ax_3d.set_title(f"{state}",
                       color=state_color, fontsize=14, fontweight='bold', pad=10)
        ...
        for label, val in info_lines:
            if label:
                ax_info.text(0.1, y_pos, label, color='#8888aa', fontsize=10,
                           fontfamily='monospace', transform=ax_info.transAxes)
                ax_info.text(0.9, y_pos, val, color='#ddddff', fontsize=10,
        ...
        bar_y = 0.35
        ax_info.text(0.1, bar_y + 0.05, "frequency →", color='#666688',
                    fontsize=8, transform=ax_info.transAxes)
        ax_info.barh(bar_y, f_ratio, height=0.03, color=state_color,
                    alpha=0.7, left=0.1)
        ax_info.barh(bar_y, 1.0, height=0.03, color='#222244',
                    alpha=0.3, left=0.1)

        # Scaling law
        ax_info.text(0.5, 0.15, r"$R = \frac{c}{2\pi f}$",
                    color='#aaaacc', fontsize=14, ha='center',
                    transform=ax_info.transAxes)
        ax_info.text(0.5, 0.05, f"Frame {frame+1}/{TOTAL_FRAMES}",
