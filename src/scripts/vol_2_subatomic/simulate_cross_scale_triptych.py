...
def main():
    ...
    ax1.set_title('Electron: Standing de Broglie Waves\n'
    ...
    ax1.annotate('$\\Gamma = -1$\n(self-confinement)',
                 xy=(0.5, 0.6), fontsize=10, color='#ff4444',
    ...
    for r_in, r_out, name in gaps:
        ...
        ax2.text(r_mid, density.max() * 0.9, name, color='#ff4444',
    ...
    ax2.set_title('Saturn: Ring Impedance Bands\n'
                  '$r_{Saturn} = 6.03 \\times 10^{10}$ m',
                  fontsize=13, color='white', pad=10)
    ax2.grid(True, alpha=0.15)

    ax2.annotate('"Density waves" = macroscopic photons',
                 xy=(120, density.max() * 0.15), fontsize=9, color='#00ccff',
                 fontstyle='italic',
                 bbox=dict(boxstyle='round', facecolor='#1a1a3a', alpha=0.8))
    ...
    for i, br in enumerate(band_radii):
        ...
        ax3.text(br / rs, n_r.max() * (0.85 - i * 0.1), f'$n={i+1}$',
    ...
    ax3.set_title('Black Hole: Accretion Disk Bands\n'
    ...
    ax3.annotate('$\\Gamma \\to +1$\n(impedance catastrophe)',
                 xy=(1.5, n_r.max() * 0.5), fontsize=10, color='#ff4444',
                 fontweight='bold',
                 bbox=dict(boxstyle='round', facecolor='#1a1a3a', alpha=0.8))
    ...
    ax4.plot(x, gamma_electron, color='#00ccff', linewidth=2.5,
             label='Electron ($\\Gamma = -1$ confinement)')
    ax4.plot(x, gamma_saturn, color='#ffaa00', linewidth=2.5,
             label='Saturn (resonance impedance bands)')
    ax4.plot(x, gamma_bh, color='#cc44ff', linewidth=2.5,
             label='Black Hole ($\\Gamma \\to +1$ rupture)')

    ax4.axhline(0, color='white', alpha=0.3, linestyle=':')
    ax4.axvline(1, color='white', alpha=0.3, linestyle=':', label='$r = r_{boundary}$')

    ax4.fill_between(x, gamma_electron, 0, where=(gamma_electron < 0),
                     color='#00ccff', alpha=0.1)
    ...
    bars1 = ax5.bar(x_pos - width, [1.0]*4, width, color=colors, alpha=0.6,
                    edgecolor='white', linewidth=0.5, label='Orbital Structure')
    bars2 = ax5.bar(x_pos, [0.8]*4, width, color=colors, alpha=0.4,
                    edgecolor='white', linewidth=0.5, label='"Photon" Emission')
    bars3 = ax5.bar(x_pos + width, [0.6]*4, width, color=colors, alpha=0.3,
                    edgecolor='white', linewidth=0.5, label='"Spectral Lines"')

    # Label each bar
    for i in range(4):
        ax5.text(x_pos[i] - width, 1.05, categories[i], ha='center',
                 fontsize=8, color=colors[i], fontweight='bold')
        ax5.text(x_pos[i], 0.85, emissions[i], ha='center',
                 fontsize=7, color='white')
        ax5.text(x_pos[i] + width, 0.65, spectra[i], ha='center',
                 fontsize=7, color='white')

    # Scale annotation
    for i in range(4):
        ax5.text(x_pos[i], -0.15, f'$r \\sim {scales[i]:.0e}$ m',
    ...
    ax5.set_yticks([])
    ax5.set_xticks([])
    ax5.set_title('Cross-Scale Emission:\nSame $1/d$ Mechanism', fontsize=13,
                  color='white', pad=10)

    # Central annotation
    ax5.text(1.5, 1.35, r'$Z = \sqrt{\mu/\varepsilon}$ at every scale',
    ...
    ax6.bar(x_ev - width, f_schw_vals, width, color='#00ccff', alpha=0.6,
            edgecolor='white', linewidth=0.5, label='Schwarzschild')
    ax6.bar(x_ev, f_kerr_vals, width, color='#00ff88', alpha=0.8,
            edgecolor='white', linewidth=0.5, label='Kerr-corrected')
    ax6.bar(x_ev + width, f_obs_vals, width, color='#ff4444', alpha=0.8,
            edgecolor='white', linewidth=0.5, label='LIGO Observed')

    # Error annotations
    for i, name in enumerate(events):
        err_k = abs(f_kerr_vals[i] - f_obs_vals[i]) / f_obs_vals[i] * 100
        ax6.text(i, max(f_kerr_vals[i], f_obs_vals[i]) + 15,
                 f'{err_k:.0f}%', ha='center', fontsize=10, color='#00ff88',
                 fontweight='bold')
        ax6.text(i, -20, f'$a_* = {a_star_vals[i]}$', ha='center',
                 fontsize=9, color='white')

    ax6.set_xticks(x_ev)
    ax6.set_xticklabels(events, fontsize=10, color='white')
    ax6.set_ylabel('Ringdown Frequency [Hz]', fontsize=12, color='white')
    ax6.set_title('Kerr-Corrected Ringdown\nvs. LIGO Data', fontsize=13,
                  color='white', pad=10)
