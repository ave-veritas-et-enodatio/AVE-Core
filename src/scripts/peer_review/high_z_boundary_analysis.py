...
from ave.core.constants import (
    K_MUTUAL, ALPHA, D_PROTON, HBAR_C_MEV_FM,
    M_P_MEV_TARGET, M_N_MEV_TARGET, V_SNAP, NU_VAC,
)
...
print("=" * 100)
print()
print(f"{'Element':<10s} {'Z':>4s} {'A':>4s} {'CODATA [MeV]':>14s} {'AVE [MeV]':>14s} {'Δ%':>10s} {'n_pair':>8s} {'Regime':>14s}")
print("-" * 100)
...
for name, Z, A, codata_mass in CODATA_MASSES:
    ...
    results.append({
        'name': name, 'Z': Z, 'A': A, 'codata': codata_mass,
        'ave': ave_mass, 'delta_pct': delta_pct, 'n_pairs': n_pairs,
        'regime': regime,
    })
    
    print(f"  {name:<10s} {Z:4d} {A:4d} {codata_mass:14.3f} {ave_mass:14.3f} {delta_pct:+9.4f}% {n_pairs:8d} {regime:>14s}")
...
ax.legend(loc='upper left', fontsize=8, facecolor='#1a1a2e', edgecolor='#333355', labelcolor='white')
ax.text(100, 0.5, 'SUPERHEAVY\nFRONTIER', color='#ff4444', fontsize=14,
...
ax.set_yscale('log')
ax.text(60, 100, 'O(A²) scaling\nmakes brute-force\n1/d tractable\nto Z≈120', 
        color='#aabbdd', fontsize=10, ha='center', va='center')
...
axes[1, 1].legend(handles=legend_elements, loc='upper left', fontsize=9,
                   facecolor='#1a1a2e', edgecolor='#333355', labelcolor='white')

fig.suptitle('High-Z Nuclear Geometry Boundary Analysis\n'
