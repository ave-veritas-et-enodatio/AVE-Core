...
Computes all nuclear constants using BOTH:
  ...
  (B) The empirical CODATA proton mass (M_P_MEV_TARGET)       [ARCHITECTURE REVIEW C3]
...
from ave.core.constants import (
    HBAR, M_E, C_0, e_charge, ALPHA, NU_VAC,
    PROTON_ELECTRON_RATIO, M_P_MEV_TARGET, M_N_MEV_TARGET,
    D_PROTON, D_INTRA_ALPHA, D_NN_EIGENVALUE,
    B_DEUTERON_PREDICTED, OMEGA_0_NUCLEAR, E_0_NUCLEAR, K_COUPLING,
    K_MUTUAL, HBAR_C_MEV_FM, CROSSING_NUMBER_PROTON,
)
...
# ─── (B) EMPIRICAL chain (if we had used M_P_MEV_TARGET) ────────────────
m_p_empirical_kg = M_P_MEV_TARGET * 1e6 * e_charge / C_0**2
...
print(f"  CODATA proton mass (MeV):       {M_P_MEV_TARGET:.6f} MeV")
print(f"  Delta:                          {(m_p_derived_mev - M_P_MEV_TARGET):.6f} MeV"
      f"  ({(m_p_derived_mev / M_P_MEV_TARGET - 1) * 100:+.4f}%)")
print()
print("-" * 90)
...
print("  The 'Empirical' column is what the values WOULD be if M_P_MEV_TARGET were used (old C3 path).")
...
if abs(m_p_derived_mev / M_P_MEV_TARGET - 1) < 0.005:
    ...
    print(f"  ✅ All nuclear constants shift by ≤ {abs(m_p_derived_mev / M_P_MEV_TARGET - 1) * 100:.3f}% — within framework precision.")
else:
