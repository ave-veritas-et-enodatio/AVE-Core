...
def compute_centroid(V_inc, mask_active, axis=0, pml=4):
    ...
    interior[pml : N - pml, pml : N - pml, pml : N - pml] = energy[pml : N - pml, pml : N - pml, pml : N - pml]
...
def main():
    ...
    print(f"  Capture cadence: every {capture_cadence} steps " f"(~{n_steps // capture_cadence} captures)")
    print(f"  V_yield = √α = {V_YIELD:.4f}, A²_yield = {A2_YIELD:.4f}, " f"A²_op14 = {A2_OP14:.4f}")
    ...
    print(f"\n  Engine: A28-corrected coupling (disable_cosserat_lc_force=True, " f"enable_cosserat_self_terms=True)")
    print(f"  Source: SpatialDipoleCPSource RH @ x0=8, ω=1.0, A=0.10·V_SNAP " f"(~{0.10/V_YIELD:.2f}·V_yield)")
    print(f"  Source envelope: 2P ramp + 2P sustain + 2P decay = 6P on, " f"then off for 44P (self-sustenance window)")
    ...
    for step_i in range(n_steps):
        ...
        if step_i % capture_cadence == 0:
            ...
            if step_i % (capture_cadence * 10) == 0:
                ...
                print(
                    f"    t={t_p:5.2f}P  centroid_x={cx if not np.isnan(cx) else 0:6.2f}  "
    ...
    print(f"  Saturation engagement (A² > {A2_OP14:.4f} = √(2α)): " f"{'YES' if saturation_engaged else 'NO'}")
    ...
    if post_shutoff_start < n_steps and trap_y == yc and trap_z == zc:
        ...
        fft_targets = [OMEGA_C * (1.0 - 2 * ALPHA), OMEGA_C, OMEGA_C * (1.0 + 2 * ALPHA), 1.5, 2.96]
    ...
    if len(centroid_arr) > 5:
        ...
        if free_mask.sum() > 3:
            ...
            print(
                f"    Free-propagation v_g (t < 4P): {v_free:.3f} cells/time-unit "
        ...
        if late_mask.sum() > 3:
            ...
            print(f"    Late-time v_g (t > 15P): {v_late:.3f} cells/time-unit " f"(predicted ≈ 0 if trapped)")
            v_drop = (v_free - v_late) / max(v_free, 1e-6) if free_mask.sum() > 3 else 0.0
            print(f"    Velocity drop fraction: {v_drop:.3f} " f"(close to 1.0 if cleanly trapped)")
    ...
    if len(captures) > 10:
        ...
        if late_caps:
            ...
            print(f"    Retention fraction: {retention:.3f} " f"(near 1.0 if self-sustaining; near 0 if dissipates)")
    ...
    if mid_step + 5 < n_steps:
        ...
        if v_sq.max() > 1e-20:
            ...
            print(f"    Opposite-sign wake (Grant's BEMF reframe): " f"{'YES' if opposite_signs else 'NO'}")
        else:
    ...
    if h_pass:
        ...
        if saturation_engaged and topology_match:
            print(f"  → Trap formed (saturation + topology) but frequency mismatch — " f"investigate.")
            ...
            print(f"  → No clear self-trap. Photon may have passed through, " f"dissipated, or dispersed.")
