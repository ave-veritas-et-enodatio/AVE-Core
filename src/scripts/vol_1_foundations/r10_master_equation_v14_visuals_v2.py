...
print(f"  High phase: V_peak = {v_peak_array[high_idx]:.3f}, " f"Low phase: V_peak = {v_peak_array[low_idx]:.3f}")
print()
...
mesh_core = isosurface(V_high, level=0.3 * V_peak_high, color="#ff3030", alpha=0.7)
if mesh_core is not None:
    ax.add_collection3d(mesh_core)
# Outer isosurface: |V| > 0.1 × V_peak_high (the "envelope")
mesh_env = isosurface(V_high, level=0.1 * V_peak_high, color="#ffaa30", alpha=0.25)
if mesh_env is not None:
    ax.add_collection3d(mesh_env)

# Mark center
ax.scatter(
    [CENTER[0]], [CENTER[1]], [CENTER[2]], color="cyan", s=200, marker="*", edgecolors="white", linewidth=2.5, zorder=10
)

# Annotations
ax.text(N - 1, N - 1, N + 2, "Lattice domain edge (computational, PML at +z)", color="#aaaaaa", fontsize=10, ha="right")
ax.text(
    PML,
    PML - 1,
    N - PML - 0.5,
    f"PML shell (sponge-layer absorber, thickness={PML})",
    color="#ff8030",
    fontsize=10,
    ha="left",
)
...
ax.text(CENTER[0], CENTER[1], CENTER[2] - 1, "Soliton core\n(saturated A→1)", color="#ff5050", fontsize=11, ha="center")
...
im = ax.imshow(
    V_slice.T, origin="lower", cmap="RdBu_r", extent=[0, N, 0, N], aspect="equal", vmin=-vmax, vmax=vmax, alpha=1.0
)
...
for edge in ["bottom", "top", "left", "right"]:
    if edge == "bottom":
        rect = Rectangle(
            (0, 0), N, PML, linewidth=2, edgecolor="#ff8030", facecolor="#ff8030", alpha=0.15, hatch="///", zorder=5
        )
    elif edge == "top":
        rect = Rectangle(
            (0, N - PML),
...
lattice_rect = Rectangle((0, 0), N, N, linewidth=3, edgecolor="#aaaaaa", facecolor="none", zorder=6)
...
ax.contour(
    A_slice.T,
    levels=[0.1 * V_peak_high, 0.3 * V_peak_high],
...
ax.text(
    N / 2,
    N - 1.5,
    "← Lattice domain edge (computational)",
    color="#aaaaaa",
    fontsize=11,
    ha="center",
    va="center",
    bbox=dict(boxstyle="round,pad=0.3", facecolor="#0a0a0a", edgecolor="#aaaaaa", alpha=0.8),
)
ax.annotate(
    "PML absorber\n(sponge-layer)",
    xy=(2.5, N / 2),
    ...
    fontsize=11,
    ha="center",
    bbox=dict(boxstyle="round,pad=0.3", facecolor="#0a0a0a", edgecolor="#ff8030", alpha=0.85),
...
ax.annotate(
    ...
    fontsize=11,
    ha="left",
    bbox=dict(boxstyle="round,pad=0.3", facecolor="#0a0a0a", edgecolor="#ffaa30", alpha=0.85),
    arrowprops=dict(arrowstyle="->", color="#ffaa30", lw=1.5),
)
ax.annotate(
    "Soliton core (saturated)",
    xy=(CENTER[0] + 1.5, CENTER[1] + 1.5),
    xytext=(N + 3, CENTER[1] - 3),
    color="#ff5050",
    fontsize=11,
    bbox=dict(boxstyle="round,pad=0.3", facecolor="#0a0a0a", edgecolor="#ff5050", alpha=0.85),
...
ax.set_title(
    f"Equatorial slice (z={CENTER[2]}, high-phase t={snapshots_t[high_idx]:.1f})\n"
...
def update_3d_iso(frame_idx):
    ...
    if V_max > 0.05:
        mesh_core = isosurface(V_state, level=0.4 * V_max, color="#ff3030", alpha=0.7)
        if mesh_core is not None:
            ax3d.add_collection3d(mesh_core)
        # Envelope isosurface
        mesh_env = isosurface(V_state, level=0.15 * V_max, color="#ffaa30", alpha=0.25)
        if mesh_env is not None:
    ...
    ax3d.set_title(
        f"Breathing soliton inside computational lattice\n"
...
for edge_pos in [
    ...
    pos, w, h = edge_pos
    rect = Rectangle(
        pos, w, h, linewidth=1.5, edgecolor="#ff8030", facecolor="#ff8030", alpha=0.15, hatch="//", zorder=5
    )
    ax.add_patch(rect)
...
def update_slice(frame_idx):
    ...
    if V_max_state > 0.05:
        contours_storage["contour"] = ax.contour(
            ...
            levels=[0.1 * V_max_state / V_YIELD, 0.3 * V_max_state / V_YIELD],
            colors=["#ffaa30", "#ff3030"],
            linewidths=[2, 3],
            extent=[0, N, 0, N],
        )
    ...
    ax.set_title(
        f"V(x,y), z={CENTER[2]}, t={snapshots_t[idx]:.1f}, "
...
anim2 = FuncAnimation(fig, update_slice, frames=len(gif_frames), interval=100, blit=False)
...
print(f"  Bulk physics region: ({N - 2*PML})×({N - 2*PML})×({N - 2*PML}) = " f"{(N - 2*PML)**3} cells")
