...
# ═══════════════════════════════════════════════════════════════════════════
# 1. Closed-form holomorphic multipole decomposition
# ═══════════════════════════════════════════════════════════════════════════
PHI = (1.0 + np.sqrt(5.0)) / 2.0         # Golden ratio
R_gt = PHI / 2.0                          # Golden Torus major radius
r_gt = (PHI - 1.0) / 2.0                  # Golden Torus minor radius

# Verify Golden Torus geometric constraints
...
# Multipole shape factors
Lambda_vol = 16.0 * np.pi**3 * (R_gt * r_gt)    # = 4π³
Lambda_surf = 4.0 * np.pi**2 * (R_gt * r_gt)    # = π²
Lambda_line = np.pi * 1.0                        # = π (d = ℓ_node = 1)

alpha_inv_computed = Lambda_vol + Lambda_surf + Lambda_line

# Sanity check against the engine constant
...
print()
print(f"  Λ_vol  = 16π³(R·r)  = {Lambda_vol:.6f}   (= 4π³)")
print(f"  Λ_surf = 4π²(R·r)   = {Lambda_surf:.6f}   (= π²)")
print(f"  Λ_line = π·d        = {Lambda_line:.6f}   (= π)")
print()
...
# ═══════════════════════════════════════════════════════════════════════════
# 3. Trefoil soliton figure at Golden Torus ropelength
# ═══════════════════════════════════════════════════════════════════════════
# Parametric (2,3) torus knot on the Golden Torus
t = np.linspace(0.0, 2.0 * np.pi, 1200)
# Torus knot parameterization with major radius R and minor radius r
...
# Strain proxy: local curvature magnitude
...
scatter = ax.scatter(x, y, z, c=strain, cmap='magma', s=18, alpha=0.92,
