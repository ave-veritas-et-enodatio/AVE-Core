...
def test_v2_cosserat_isolation(N: int = 32, n_steps: int = 30) -> dict:
    ...
    H_drift = max(
        abs(h["H_total"] / history[0]["H_total"] - 1.0) if history[0]["H_total"] != 0 else 0.0 for h in history
    )
...
def test_v3_coupled_interaction(
    ...
    H_drift = max(abs(h["H_total"] / max(abs(H_start), 1e-30) - 1.0) for h in history[3:])
    coupling_e_max = max(h["E_coupling"] for h in history)
    coupling_e_at_peak_V = max(
        h["E_coupling"] for h in history if h["max_V_sq"] == max(hh["max_V_sq"] for hh in history)
    )
...
def render(v1: dict, v2: dict, v3: dict, out: str = "/tmp/coupled_coupling_test.png") -> None:
    ...
    h = v3["history"]
    t = [hh["t"] for hh in h]
    ax.plot(t, [hh["max_V_sq"] / sim_V_SNAP_sq() for hh in h], "-", label="max V² / V_SNAP²", color="#f90", lw=1.3)
    ax.plot(t, [hh["max_A_sq_cos"] for hh in h], "-", label="max A²_cos", color="#47c", lw=1.3)
    ax.plot(t, [hh["max_A_sq_total"] for hh in h], "-", label="max A²_total", color="#c33", lw=1.3)
    ax.set_yscale("log")
    ax.set_title(
        f"V3 — Coupled (photon hits shell, amp={v3['amp_frac']}·V_SNAP)\n"
...
if __name__ == "__main__":
    ...
    summary = {
        "V1_pass": v1["pass"],
        "V1_coupling_e_max": v1["coupling_e_max"],
        "V2_pass": v2["pass"],
        "V2_H_drift": v2["H_drift"],
        "V3_pass": v3["pass"],
        "V3_Q_drift": v3["Q_drift"],
        "V3_H_drift": v3["H_drift"],
        "V3_coupling_e_max": v3["coupling_e_max"],
    }
    print("── Summary ──")
