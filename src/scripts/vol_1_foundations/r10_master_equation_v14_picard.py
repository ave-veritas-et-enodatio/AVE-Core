...
engine.inject_localized_blob(
    center=center,
    radius=2.5,
    amplitude=PEAK_AMP_TARGET * V_YIELD,
    profile="sech",
)
...
for iter_idx in range(N_PICARD_ITERS):
    ...
    V_new = localize_and_renormalize(V_after_relax, center, max_radius=6, peak_target=PEAK_AMP_TARGET)
    ...
    print(
        f"  V_peak after relax = {V_peak_after:.4f}, FWHM = {fwhm_after:.2f}, "
...
test1c_pass = (V_peak_envelope_max / V_peak_init > 0.15) and (
    V_peak_envelope_max - V_peak_envelope_min
) / V_peak_envelope_max < 5.0
# envelope range bounded (radiation-free, no monotonic decay)

print(f"\n  Test 1 (V_peak persistence) — three variants:")
print(
    f"    Test 1a (strict min > 0.3):           min={V_peak_min/V_peak_init:.3f} "
    f"→ {'PASS ✓' if test1a_pass else 'FAIL ✗'}"
)
print(
    f"    Test 1b (mean > 0.2):                  mean={V_peak_mean/V_peak_init:.3f} "
    f"→ {'PASS ✓' if test1b_pass else 'FAIL ✗'}"
)
print(
    f"    Test 1c (envelope bounded, breathing): "
...
axA.plot(picard_history["iter"], picard_history["V_peak_after_relax"], "C2o-", lw=2, ms=10, label="V_peak after relax")
...
axB.plot(history["t"], np.array(history["V_peak"]) / V_peak_init, "C2-", lw=2, label="V_peak / V_peak(0)")
axB.axhline(0.3, color="C3", ls="--", lw=1, label="0.3× (Test 1a)")
axB.axhline(V_peak_mean / V_peak_init, color="C0", ls=":", lw=1, label=f"mean = {V_peak_mean / V_peak_init:.2f}")
...
axC.plot(history["t"], np.array(history["FWHM"]) / fwhm_init, "C4-", lw=2)
...
im = axE.imshow(
    np.abs(V_picard_slice).T, origin="lower", cmap="hot", extent=[0, N, 0, N], aspect="equal", vmin=0, vmax=vmax
)
...
im = axF.imshow(
    np.abs(V_final_slice).T, origin="lower", cmap="hot", extent=[0, N, 0, N], aspect="equal", vmin=0, vmax=vmax
)
...
axH.bar(x_pos - 0.2, measured, 0.4, label="Measured", color="C0", edgecolor="white")
axH.bar(x_pos + 0.2, canonical, 0.4, label="Canonical α⁻¹", color="C3", edgecolor="white")
