#!/usr/bin/env python3
r"""Native electron model v3 — stabilized back-EMF + local avalanche damping.

Tests if a balanced feedback loop (self-limiting gain and local avalanche viscosity)
stabilizes the coupled K4-Cosserat system, achieving stable topological trapping and
persistent spin (omega persistence ratio >= 0.5) without energy runaway.
"""
...
def _score_records(
    ...
    *,
    ...
    if n_pass >= 4 and props["P4b_omega_persistence_ge_0p5"]:
        verdict, outcome = "MODEL_V3_FOUR_PROPERTY_PLUS_CIRCULATION", "A"
    elif n_pass >= 4:
        verdict, outcome = "MODEL_V3_FOUR_PROPERTY_NO_CIRCULATION", "B"
    elif tir_held and localized and n_pass >= 2:
        verdict, outcome = "MODEL_V3_PARTIAL_TRAP", "C"
    elif tir_held:
        verdict, outcome = "MODEL_V3_TIR_ONLY", "D"
    else:
        verdict, outcome = "MODEL_V3_CHANNEL_DESTABILIZED", "E"
...
def run_channel_arm(
    name: str,
    *,
    boundary_leak: bool = True,
    lagrangian_emf: bool = True,
    gain: float = 0.12,
    damping_rate: float = 0.20,
    n: float = 1.8095,
) -> dict[str, Any]:
    ...
    n_lat = N_LATTICE
    cy0 = cz0 = n_lat // 2
    mask = interior_mask(n_lat, PML) & engine.k4.mask_active
    bond_a, bond_port = _find_central_bond(engine)

    records: list[dict[str, Any]] = []
    bond_vi: list[float] = []
    bond_vr: list[float] = []
    last_cx = float(n_lat // 2)
...
def classify(rows: list[dict[str, Any]]) -> dict[str, Any]:
    baseline = next((r for r in rows if r["variant"] == "v3_gain_0.0_damp_0.0"), None)
    ...
    if best["outcome"] == "A":
        agg = "V3_CHANNELS_LANDED_ELECTRON_MODEL"
    elif stable and all(r["properties_passed"] <= 2 for r in stable):
        agg = "V3_CHANNELS_NO_BREAKTHROUGH"
    elif improved_eps:
        agg = "V3_CHANNELS_EPS_IMPROVED"
    else:
        agg = "V3_CHANNELS_DESTABILIZED_TRAP"
    return {
...
def main() -> None:
    ...
    print("Native electron model v3 (stabilized BEMF + local avalanche damping)")
    
    # 10 distinct arms to sweep parameter space efficiently
    arms = [
        ("v3_gain_0.12_damp_0.0", dict(gain=0.12, damping_rate=0.0, n=1.8095, lagrangian_emf=False)),
        ("v3_gain_0.12_damp_0.01", dict(gain=0.12, damping_rate=0.01, n=1.8095, lagrangian_emf=False)),
        ("v3_gain_0.12_damp_0.02", dict(gain=0.12, damping_rate=0.02, n=1.8095, lagrangian_emf=False)),
        ("v3_gain_0.12_damp_0.05", dict(gain=0.12, damping_rate=0.05, n=1.8095, lagrangian_emf=False)),
        ("v3_gain_0.06_damp_0.0", dict(gain=0.06, damping_rate=0.0, n=1.8095, lagrangian_emf=False)),
        ("v3_gain_0.06_damp_0.01", dict(gain=0.06, damping_rate=0.01, n=1.8095, lagrangian_emf=False)),
        ("v3_gain_0.06_damp_0.02", dict(gain=0.06, damping_rate=0.02, n=1.8095, lagrangian_emf=False)),
        ("v3_gain_0.03_damp_0.0", dict(gain=0.03, damping_rate=0.0, n=1.8095, lagrangian_emf=False)),
        ("v3_gain_0.03_damp_0.01", dict(gain=0.03, damping_rate=0.01, n=1.8095, lagrangian_emf=False)),
        ("v3_gain_0.0_damp_0.0", dict(gain=0.0, damping_rate=0.0, n=1.8095, lagrangian_emf=False)),
    ]
    
    rows = []
    for name, kwargs in arms:
        print(f"Running arm: {name} ...", flush=True)
        r = run_channel_arm(name, **kwargs)
        rows.append(r)
    ...
    payload = {
        ...
        "parent": "native_electron_model_v2.py",
        "description": "v3 sweep: stabilized BEMF + local avalanche velocity decay on u_dot, omega_dot",
        "rows": rows,
        "classification": classification,
    }
    out_json = OUT_DIR / "native_electron_model_v3_results.json"
    ...
    for row in rows:
        print(
            ...
            f"  Γ_final={row['gamma_final']}"
            f"  bond R/r={row['pca_bond']['R_over_r']:.2f}"
        )
    print(f"\n  aggregate: {classification['aggregate_verdict']}")
