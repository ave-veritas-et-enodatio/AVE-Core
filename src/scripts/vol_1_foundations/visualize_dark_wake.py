...
def main():
    ...
    for col, pi in enumerate(picks):
        ...
        ax.imshow(wall_vis.T, cmap='Greys', alpha=0.95, origin='lower',
                  extent=[0, sim_no.NX, 0, sim_no.NY], aspect='auto')

        # Particle position
        px = int(sim_no.source_x_start + t_val * sim_no.particle_speed)
        if px < sim_no.wall_x:
            ax.plot(px, sim_no.source_y, 'o', color='#00ff00',
                    markersize=10, markeredgecolor='white',
                    markeredgewidth=1.5, zorder=10)
            ax.annotate('PARTICLE', xy=(px, sim_no.source_y),
                        xytext=(px, sim_no.source_y + 50),
                        fontsize=8, color='#00ff00', fontweight='bold',
                        ha='center',
                        arrowprops=dict(arrowstyle='->', color='#00ff00', lw=1.5))

        ax.set_title(f'No Observer: {time_labels[col]}\n(t = {t_val})',
        ...
        ax2.imshow(wall_vis2.T, cmap='Greys', alpha=0.95, origin='lower',
                   extent=[0, sim_obs.NX, 0, sim_obs.NY], aspect='auto')

        # Observer marker
        ax2.plot(sim_obs.wall_x, sim_obs.slit_2, 's', color='#00ff00',
                 markersize=10, markeredgecolor='yellow',
                 markeredgewidth=2, zorder=10)

        # Particle
        px_o = int(sim_obs.source_x_start + t_val_o * sim_obs.particle_speed)
        if px_o < sim_obs.wall_x:
            ax2.plot(px_o, sim_obs.source_y, 'o', color='#00ff00',
                     markersize=10, markeredgecolor='white',
                     markeredgewidth=1.5, zorder=10)

        ax2.set_title(f'Observer at Slit 2: {time_labels[col]}\n(t = {t_val_o})',
                      color='white', fontsize=11, fontweight='bold')
    ...
    # Annotations
    fig.text(0.01, 0.75, 'NO\nOBSERVER', rotation=90, color='#00ffcc',
             fontsize=16, fontweight='bold', ha='left', va='center')
    fig.text(0.01, 0.28, 'OBSERVER\nAT SLIT 2', rotation=90, color='#ff6b6b',
             fontsize=16, fontweight='bold', ha='left', va='center')
    ...
    wall_big = np.ma.masked_where(~sim_no.wall,
                                   np.ones_like(snap_final) * vmax_f)
    ax_big.imshow(wall_big.T, cmap='Greys', alpha=0.9, origin='lower',
                  extent=[0, sim_no.NX, 0, sim_no.NY], aspect='auto')

    # Slit labels
    ax_big.annotate('SLIT 1', xy=(sim_no.wall_x + 8, sim_no.slit_1),
                    fontsize=12, color='white', fontweight='bold')
    ax_big.annotate('SLIT 2', xy=(sim_no.wall_x + 8, sim_no.slit_2),
                    fontsize=12, color='white', fontweight='bold')
    ...
    fig2.savefig(out2, dpi=250, bbox_inches='tight',
