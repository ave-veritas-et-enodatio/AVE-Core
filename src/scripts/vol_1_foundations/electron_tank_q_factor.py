...
def main():
    ...
    print(f"  L_e             = {r1['L_e_SI_Henry']:.6e}  [H]  " f"(xi_topo^-2 * m_e)")
    print(f"  omega_Compton   = {r1['omega_Compton_SI_rad_per_s']:.6e}  " f"[rad/s]")
    ...
    print(f"  R*r                    = {r2['Rr_product']:.6f}  " f"(expected 1/4 = 0.25)")
    print(f"  Lambda_vol = 16*pi^3 * R*r = {r2['Lambda_vol']:.6f}  " f"(= 4*pi^3)")
    print(f"  Lambda_surf = 4*pi^2 * R*r = {r2['Lambda_surf']:.6f}  " f"(= pi^2)")
    print(f"  Lambda_line = pi*d         = {r2['Lambda_line']:.6f}  " f"(= pi)")
    print(f"  SUM                         = {r2['alpha_inv_Ch8_sum']:.6f}")

    # CODATA reference
    alpha_inv_CODATA = 1.0 / ALPHA
    print(f"\nReference: 1/alpha (CODATA) = {alpha_inv_CODATA:.6f}")
    print(f"Reference: ALPHA_COLD_INV   = {ALPHA_COLD_INV:.6f}  " f"(4*pi^3 + pi^2 + pi)")
