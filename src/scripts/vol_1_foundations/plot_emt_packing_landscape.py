...
K_over_G = np.where(p > p_G,
    (p - p_K) / (p - p_G) * (z0 / (z0 - 2)) * 0.5,
    np.inf)
# Clip for display
...
ax.axhline(2.0, color='#4CAF50', alpha=0.3, linewidth=1, linestyle='--',
           label=r'$K/G = 2$ (Trace-Reversal)')
ax.axhline(5/3, color='#FF9800', alpha=0.3, linewidth=1, linestyle=':',
...
ax.scatter([p_star], [KG_star], s=200, c='#4CAF50', zorder=10,
           edgecolors='white', linewidths=2, label=f'AVE: $p_c = 8\\pi\\alpha \\approx {p_star:.4f}$')

# Mark the Cauchy point
ax.scatter([p_cauchy], [KG_cauchy], s=120, c='#FF9800', zorder=10, marker='D',
...
ax.set_title('Effective Medium Theory: The Trace-Reversal Operating Point',
