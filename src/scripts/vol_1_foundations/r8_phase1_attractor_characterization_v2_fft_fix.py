"""Move 7b / Phase 1 FFT fix — sample at top-|V_inc|² cells (not |ω|² centroid).

Per `P_phase1_attractor_characterization_v2_fft_fix` (frozen at this commit).

FIX TO MOVE 7:
  Move 7 selected 5 sample points displaced ±2 cells from the |ω|² density
  centroid (15, 15, 16). All 5 cells returned IDENTICALLY ZERO V_inc across
  447 timesteps; FFT picked up only lattice-Nyquist numerical artifacts.

  Diagnosis: |ω|² density is shell-distributed (peak ω at non-zero radius
  from lattice center); the centroid sits in the empty hole of the shell.
  V_inc has its own structure with narrow z-extent (2.92 cells); the
  centroid-displaced points landed in zero-V_inc territory. Index parity
  at displaced points was uniformly even, possibly compounding via K4
  sublattice structure.

  FIX: pick the 5 cells with highest Σ_p|V_inc[cell, p]|² at t=200P (i.e.,
  energy-density-based selection on V_inc directly). Guarantees non-zero
  V_inc trace at every sample point.

OTHER 4 EXTRACTIONS (spatial moments, Q-factor, energy partition, topology)
are unchanged from Move 7 and are re-emitted for completeness.
...
OUTPUT_JSON = Path(__file__).parent / "r8_phase1_attractor_characterization_v2_fft_fix_results.json"
...
def select_5_fixed_points(centroid, nx, v_inc_field=None):
    """Pick top-5 highest |V_inc|² cells from the v_inc_field (energy-density
    selection on V_inc directly, not centroid-displaced).

    Falls back to centroid-displaced cross only if v_inc_field is None
    (legacy Move 7 behavior, preserved for reproducibility).
    """
    if v_inc_field is not None:
        # Sum of squared port amplitudes per cell
        v_inc_density = np.sum(v_inc_field ** 2, axis=-1)
        # Find top 5 cells by V_inc² density
        flat = v_inc_density.flatten()
        # argpartition: indices of top 5 (unordered); sort them by value descending
        top5_idx = np.argpartition(flat, -5)[-5:]
        top5_idx = top5_idx[np.argsort(flat[top5_idx])[::-1]]  # descending
        cells = [tuple(np.unravel_index(idx, v_inc_density.shape)) for idx in top5_idx]
        return cells

    # Legacy fallback (Move 7 behavior)
...
def main():
    print("=" * 78, flush=True)
    print(f"  Move 7b / Phase 1 FFT fix — sample at top-5 |V_inc|² cells")
    print(f"  P_phase1_attractor_characterization_v2_fft_fix (frozen extraction)")
    print("=" * 78, flush=True)
    ...
    # ─── Pick 5 fixed points: top-5 |V_inc|² cells (FIX vs Move 7) ───────────
    five_points = select_5_fixed_points(
        moments_omega['centroid'], N_LATTICE, v_inc_field=final_v_inc,
    )
    print(f"  5 fixed points (top-5 |V_inc|² cells):")
    ...
    payload = {
        "pre_registration": "P_phase1_attractor_characterization_v2_fft_fix",
        "test": "Move 7b / Phase 1 FFT fix — top-|V_inc|² cells",
        "N": N_LATTICE,
