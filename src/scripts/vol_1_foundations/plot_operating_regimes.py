...
def create_phase_diagram() -> None:
    ...
    ax.annotate(
        ...
        color="white",
        fontsize=11,
        bbox=dict(boxstyle="round,pad=0.5", fc="#221133", ec="#ffaa00", lw=2),
    )

    ax.annotate(
        r"Absolute Snap Limit ($V_{snap} = 511$ kV)" + "\nPair Production / Event Horizon",
        xy=(rupture_strain, rupture_stress),
        xytext=(0.55, 1.4),
        arrowprops=dict(facecolor="red", shrink=0.05, width=2, headwidth=8, edgecolor="red"),
        color="#ffaaaa",
        fontsize=11,
        bbox=dict(boxstyle="round,pad=0.5", fc="#330000", ec="red", lw=2),
    )
    ...
    ax.set_xlabel(
        "Local Vacuum Strain / Distortion Magnitude ($|\\nabla \Psi|$)",
        color="white",
        fontsize=14,
        labelpad=15,
    )
    ...
    ax.set_title(
        "The Three Operating Regimes of the Universal LC Lattice",
        color="white",
        fontsize=18,
        fontweight="bold",
        pad=20,
