...
def render_summary_plot(result: dict, out: str) -> None:
    ...
    ax.set_xlabel("t")
    ax.set_ylabel("max V² on lattice")
...
def render_animation(result: dict, out: str, fps: int = 10) -> None:
    ...
    im_k4 = axes[0].imshow(frames[0]["A_sq_k4_slice"].T, origin="lower", cmap="inferno", vmin=0, vmax=vmax_k4)
    im_cos = axes[1].imshow(frames[0]["A_sq_cos_slice"].T, origin="lower", cmap="viridis", vmin=0, vmax=vmax_cos)
    im_tot = axes[2].imshow(frames[0]["A_sq_total_slice"].T, origin="lower", cmap="magma", vmin=0, vmax=vmax_tot)
