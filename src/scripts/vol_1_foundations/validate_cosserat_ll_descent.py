...
def run_ll_test(
    nx: int, R_init: float, r_init: float, label: str, max_iter: int = 1500, initial_lr: float = 1e-2
) -> dict:
...
def print_run(info: dict) -> None:
    ...
    print(
        f"  Init (observed):   R={info['initial_obs'][0]:.4f}, r={info['initial_obs'][1]:.4f}, c={info['initial_obs'][2]}"
    )
...
def main():
    ...
    print(
        f"{'near-Golden':<20} {'LL':<12} {info_32_near['ratio']:.4f}     {info_32_near['ratio_gap_pct']:5.2f}      {info_32_near['final_c']:<3}"
    )
    print(f"{'perturbed':<20} {'plain':<12} {'5.2500':<10} {'100.50':<10} {'3':<3}")
    print(
        f"{'perturbed':<20} {'LL':<12} {info_32_pert['ratio']:.4f}     {info_32_pert['ratio_gap_pct']:5.2f}      {info_32_pert['final_c']:<3}"
    )
