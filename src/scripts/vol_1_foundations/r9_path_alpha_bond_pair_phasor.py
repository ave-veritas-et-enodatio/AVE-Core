...
def main():
    ...
    assert A26_GUARD_LOW <= omega_peak_init <= A26_GUARD_HIGH, f"A26 guard FAILED: peak |ω|={omega_peak_init:.4f}"
    ...
    for i in range(N_SELECTION_STEPS):
        ...
        if (time.time() - last_progress) > 30.0:
            ...
            print(f"    [progress] t={t_period:6.1f} P  elapsed {time.time()-t0:.1f}s", flush=True)
            last_progress = time.time()
    ...
    if n_pairs == 0:
        ...
        for k, bp in enumerate(bond_pairs):
            ...
            print(f"    [{k}] {tuple(bp['cell_a'])} ↔ {tuple(bp['cell_b'])}  d={d:.2f}", flush=True)
    ...
    print(
        f"  Recording window: capturing V_inc, V_ref at {n_pairs} bond-pairs' " f"A-side port {SAMPLE_PORT}...",
        flush=True,
    )
    ...
    for k in range(n_pairs):
        ...
        mean_sin, std_sin, chirality_sign = chirality_hilbert(v_inc_traj[:, k], v_ref_traj[:, k])
        ...
        print(
            ...
            f"{std_sin/max(abs(mean_sin), 1e-30):.2f})",
            flush=True,
        )
    ...
    for cluster_key, bp_indices in clusters.items():
        ...
        print(
            f"    median R/r = {cluster_median_R_over_r:.4f} (target {PHI_SQ:.4f}±5%) → "
            f"C1 {'PASS' if c1_pass else 'FAIL'}"
        )
        print(
            f"    chirality = {consensus} of {len(bp_indices)} ({consensus_fraction:.0%}) "
    ...
    print(
        f"  Persistence: {persistence:.0%} (guard {PERSISTENCE_GUARD:.0%}: "
