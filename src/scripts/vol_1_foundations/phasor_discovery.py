...
def principal_axes(points_xy: np.ndarray) -> tuple[float, float, float]:
    ...
    return float(np.sqrt(eigvals[0])), float(np.sqrt(eigvals[1])), float(np.sqrt(eigvals[0] / max(eigvals[1], 1e-30)))
...
def analyze_snapshot(snap, N: int, step_label: int) -> dict:
    ...
    print(f"    {'port':>4s} {'V_rms':>10s} {'I_rms':>10s} " f"{'major':>10s} {'minor':>10s} {'aspect':>10s}")
    p1_ratios = []
    for port in range(4):
        ...
        print(f"    {port:>4d} {v_rms:>10.3e} {i_rms:>10.3e} " f"{smaj:>10.3e} {smin:>10.3e} {ratio:>10.3f}")
    ...
    print(f"    Re-Im cloud: major = {smaj_c:.3e}, minor = {smin_c:.3e}, " f"aspect = {ratio_c:.3f}")
    print(f"    |c| stats: min = {c_mag_min:.3e}, max = {c_mag_max:.3e}, " f"mean = {c_mag_mean:.3e}")
    ...
    print(f"    (4 ports; if {eigvals_corr[0]:.2f} dominates, ports collapse" f" to 1 effective dim)")
    ...
    print(f"    rho range = [{float(rho.min()):.2f}, " f"{float(rho.max()):.2f}]")
    print(f"    z   range = [{float(zs.min()):.2f}, " f"{float(zs.max()):.2f}]")
    ...
    print(f"    (2,3) winding preserved: {frac_aligned * 100:.1f}% of shell " f"sites within pi/4 of theta=2phi+3psi")
...
def main():
    ...
    result = run_and_snapshot(N=64, n_steps=300, snapshot_steps=(100, 200, 300))
    print(f"Lattice N = {result['N']}, steps run = {result['n_steps']}")
    print(f"Seed (R, r) = ({result['R']:.2f}, {result['r']:.3f}) — Golden" f" Torus proportions in real space")

    summaries = []
    for step in sorted(result["snapshots"].keys()):
        summary = analyze_snapshot(result["snapshots"][step], result["N"], step)
