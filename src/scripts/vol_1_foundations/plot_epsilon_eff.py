...
ax1.plot(x, eps_eff, color='#58a6ff', linewidth=2.5,
         label=r'AVE exact: $\varepsilon_{eff} = \varepsilon_0 \cdot S(A)$')
ax1.plot(x, eps_taylor, color='#f0883e', linewidth=2.0, linestyle='--',
         label=r'Euler-Heisenberg ($E^4$ Taylor): $\varepsilon_0[1 - \frac{1}{2}(A/A_y)^2]$')
ax1.plot(x, eps_linear, color='#8b949e', linewidth=1.5, linestyle=':',
...
ax1.annotate(r'$A \to A_{yield}$: $\varepsilon \to 0$', xy=(0.96, 0.08),
             fontsize=10, color='#58a6ff', ha='right',
             bbox=dict(boxstyle='round,pad=0.3', facecolor='#1a1a2e', edgecolor='#58a6ff'))

ax1.set_ylabel(r'Constitutive Permittivity $\varepsilon_{eff} / \varepsilon_0$',
               fontsize=13, color='white')
ax1.set_title(r'Axiom 4: Saturation Observables — $\varepsilon$ Collapse vs $C$ Divergence',
              fontsize=15, color='white', pad=15)
ax1.set_ylim(-0.05, 1.15)
ax1.legend(fontsize=8.5, loc='lower left', facecolor='#161b22',
...
ax2.plot(x, C_eff, color='#f778ba', linewidth=2.5,
         label=r'AVE exact: $C_{eff} = C_0 / S(A)$')
ax2.plot(x, eps_linear, color='#8b949e', linewidth=1.5, linestyle=':',
...
ax2.annotate(r'$A \to A_{yield}$: $C \to \infty$', xy=(0.96, 6.5),
             fontsize=10, color='#f778ba', ha='right',
...
ax2.legend(fontsize=9, loc='upper left', facecolor='#161b22',
           edgecolor='#30363d', labelcolor='white')
...
plt.tight_layout()
output_path = os.path.join(os.path.dirname(__file__), '..', '..', 'assets',
                           'sim_outputs', 'vacuum_dielectric_saturation.png')
