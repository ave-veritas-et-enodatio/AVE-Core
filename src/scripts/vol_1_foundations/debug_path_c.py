...
def main():
    ...
    lattice0 = K4Lattice3D(N, N, N, dx=1.0, pml_thickness=0, nonlinear=False, op3_bond_reflection=True)
    initialize_2_3_voltage_ansatz(lattice0, R=R_target, r=r_target, amplitude=amp)
    ...
    print(f"  p_init = {chi_init['p_measured']:+.3f}  " f"amp_ratio_p = {chi_init['amp_ratio_p']:.4f}")
    print(f"  q_init = {chi_init['q_measured']:+.3f}  " f"amp_ratio_q = {chi_init['amp_ratio_q']:.4f}")
