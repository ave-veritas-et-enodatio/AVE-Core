...
def run_fdtd_slit(nx=600, ny=400, steps=1800, freq=0.06,
                  slit_sep=90, slit_width=14, n_slits=2,
                  observer_damping=0.0, wall_frac=0.35):
    """
    ...
    for sy in slit_positions:
        wall[wall_x:wall_x+wall_t,
             max(0, sy - slit_width//2):min(ny, sy + slit_width//2)] = False
...
def main():
    ...
    for i, freq in enumerate(freqs):
        ...
        ax1.plot(y, cross + i * 0.15, color=colors1[i], linewidth=1.5,
                 label=f'f = {freq}', alpha=0.9)
        ax1.fill_between(y, i * 0.15, cross + i * 0.15,
                         alpha=0.15, color=colors1[i])

    ax1.set_title('Frequency Sweep: Wake λ → Fringe Spacing',
    ...
    for i, sep in enumerate(spacings):
        ...
        ax2.plot(y, cross + i * 0.15, color=colors2[i], linewidth=1.5,
                 label=f'd = {sep} nodes', alpha=0.9)
        ax2.fill_between(y, i * 0.15, cross + i * 0.15,
                         alpha=0.15, color=colors2[i])

    ax2.set_title('Slit Spacing Sweep: Aperture d → Fringe Density',
                  color='white', fontsize=13, fontweight='bold')
    ...
    for i, ns in enumerate(n_slits_list):
        ...
        ax3.plot(y, cross + i * 0.15, color=colors3[i], linewidth=1.5,
                 label=f'N = {ns} slit{"s" if ns > 1 else ""}', alpha=0.9)
        ax3.fill_between(y, i * 0.15, cross + i * 0.15,
                         alpha=0.15, color=colors3[i])

    ax3.set_title('N-Slit Grating: Peak Sharpening with N',
                  color='white', fontsize=13, fontweight='bold')
    ...
    # ═══════════════════════════════════════════════════
    print("\n  Panel 4: Observer impedance sweep (continuous decoherence)...",
          flush=True)
    ...
    labels4 = ['0% (no observer)', '3% (weak probe)',
               '8% (moderate)', '15% (strong)', '30% (destructive)']
    ...
    for i, (z_obs, label) in enumerate(zip(impedances, labels4)):
        ...
        ax4.plot(y, cross + i * 0.12, color=colors4[i], linewidth=1.5,
                 label=label, alpha=0.9)
        ax4.fill_between(y, i * 0.12, cross + i * 0.12,
                         alpha=0.15, color=colors4[i])

    ax4.set_title('Observer Impedance Sweep: Continuous Decoherence',
                  color='white', fontsize=13, fontweight='bold')
    ...
    ax4.text(0.02, 0.98,
             ...
             transform=ax4.transAxes, fontsize=10, color='#00ffcc',
    ...
    for z_obs in z_sweep:
        ...
        cross, _ = run_fdtd_slit(observer_damping=z_obs, steps=1200,
                                 nx=500, ny=350)
    ...
    ax_vis.plot(z_sweep * 100, visibilities, 'o-', color='#00ffcc',
                linewidth=2.5, markersize=8)
    ax_vis.fill_between(z_sweep * 100, visibilities, alpha=0.2, color='#00ffcc')

    # Copenhagen prediction (binary)
    ax_vis.axhline(visibilities[0], color='#ff6b6b', linestyle='--',
                   alpha=0.5, label='Copenhagen: full fringes or none')
    ax_vis.axhline(0, color='#ff6b6b', linestyle='--', alpha=0.5)

    ax_vis.set_xlabel('Observer Impedance Perturbation (%)', color='white',
                      fontsize=13)
    ax_vis.set_ylabel('Fringe Visibility $\\mathcal{V}$', color='white',
                      fontsize=13)
    ax_vis.set_title(
        'AVE Prediction: Decoherence Is Continuous, Not Binary',
        color='white', fontsize=15, fontweight='bold')
    ax_vis.legend(fontsize=11, facecolor='#1a1a1a', edgecolor='#333',
                  labelcolor='white')
