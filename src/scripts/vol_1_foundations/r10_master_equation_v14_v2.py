...
for prof_idx, (profile_name, amp, radius) in enumerate(profiles_to_test):
    print(f"=" * 78)
    print(f"Profile {prof_idx + 1}/{len(profiles_to_test)}: " f"{profile_name} @ A={amp}, R={radius}")
    print(f"=" * 78)
    ...
    for step in range(1, N_STEPS + 1):
        ...
        if step % LOG_CADENCE == 0:
            ...
            if step % (LOG_CADENCE * 20) == 0:
                print(
                    f"    step={step:>4d}  V_peak={history['V_peak'][-1]:.4f}  "
    ...
    print(
        f"    Test 1 (V_peak persistence ratio): min={V_peak_ratio_min:.3f}, "
        f"mean={V_peak_ratio_mean:.3f} → {'PASS' if test1_pass else 'FAIL'}"
    )
    print(
        f"    Test 2 (FWHM stability):           min×={fwhm_min_ratio:.2f}, "
        f"max×={fwhm_max_ratio:.2f} → {'PASS' if test2_pass else 'FAIL'}"
    )
    print(
        f"    Test 3 (n(r) gradient):            n_in={n_center_avg:.4f}, "
        f"n_far={n_far_avg:.4f}, Δ={n_far_avg-n_center_avg:.4f} → "
        f"{'PASS' if test3_pass else 'FAIL'}"
    )
    print(
        f"    Test 4 (Q-factor integral):        Λ_tot={q_total:.1f} vs target "
...
print("=" * 78)
print(f"{'Profile':<14} {'A':>5} {'R':>5} {'V_min':>7} {'V_mean':>8} " f"{'FWHM range':>14} {'PASS':>5}")
print("-" * 70)
for r in all_results:
    print(
        f"{r['profile']:<14} {r['amp']:>5.2f} {r['radius']:>5.2f} "
...
for r in all_results:
    ...
    axA.plot(r["history"]["t"], np.array(r["history"]["V_peak"]) / r["V_peak_init"], lw=1.5, alpha=0.7, label=label)
...
for r in all_results:
    ...
    axB.plot(r["history"]["t"], np.array(r["history"]["FWHM"]) / r["fwhm_init"], lw=1.5, alpha=0.7, label=label)
...
axC.plot(best_history["t"], best_history["V_at_max_initial"], "C2-", lw=2, label="V at initial-peak cell")
...
im = axD.imshow(
    np.abs(V_final_slice).T, origin="lower", cmap="hot", extent=[0, N, 0, N], aspect="equal", vmin=0, vmax=vmax_init
)
axD.plot(center[0] + 0.5, center[1] + 0.5, "c*", ms=15, markeredgecolor="white")
...
im = axE.imshow(
    np.abs(V_init_slice).T, origin="lower", cmap="hot", extent=[0, N, 0, N], aspect="equal", vmin=0, vmax=vmax_init
)
...
axG.bar(x_pos - 0.2, measured, 0.4, label="Best", color="C0", edgecolor="white")
axG.bar(x_pos + 0.2, canonical, 0.4, label="Canonical", color="C3", edgecolor="white")
...
colors_h = ["C2" if p == 4 else "C0" if p == 3 else "C1" if p == 2 else "C3" for p in pass_counts]
...
summary = (
    ...
    f"  MODE: {mode}\n\n"
    f"All-profile pass counts: "
    + ", ".join(f"{r['profile']}/A={r['amp']}={r['n_pass']}/4" for r in all_results)
    + "\n\n"
...
fig.suptitle(f"R10 v14 v2 — Multi-Profile Bound State Test ({mode[:30]})", color="white", fontsize=14, y=0.995)
out_path = sim_output("r10_master_equation_v14_v2.png")
