...
def post_hoc_diagnostics(solver: CosseratField3D) -> dict:
    ...
    refl_field = np.asarray(_reflection_density(u, w, dx, om_y, eps_y) * mask.astype(jnp.float64))
    ...
    shell_gamma_sq_max = (
        float(gamma_sq[shell_mask & np.asarray(mask)].max()) if (shell_mask & np.asarray(mask)).sum() > 0 else 0.0
    )
    exterior_gamma_sq_sum = float(gamma_sq[exterior_mask & np.asarray(mask)].sum())
    total_gamma_sq_sum = float(gamma_sq.sum())
    shell_A_sq_max = (
        float(A_sq[shell_mask & np.asarray(mask)].max()) if (shell_mask & np.asarray(mask)).sum() > 0 else 0.0
    )
...
def run_seed(label: str, R_target: float, r_target: float, N: int = 72, max_iter: int = 500) -> dict:
    ...
    print(f"  Initial: (R, r) = ({R0:.3f}, {r0:.3f}), R/r = " f"{R0/max(r0,1e-9):.3f}, c = {c0}")
    print(f"           E = {E0:.4e}, S11 = {S11_0:.4e}")

    print(f"  Running relax_to_ground_state(max_iter={max_iter}, tol=1e-8, " f"lr=0.01)...")
    ...
    print(f"  Final:   (R, r) = ({R_f:.3f}, {r_f:.3f}), R/r = " f"{ratio_f:.3f}, c = {c_f}")
    print(
        f"           E = {E_f:.4e} (Δ = {E_f-E0:+.4e}), "
    ...
    shell_ext_ratio = diag["exterior_gamma_sq_sum"] / max(diag["total_gamma_sq_sum"], 1e-30)
    print(f"  Post-hoc: shell |Γ|²_max = {diag['shell_gamma_sq_max']:.3f}, " f"exterior/total = {shell_ext_ratio:.3%}")
    print(f"            shell A²_max = {diag['shell_A_sq_max']:.3f}  " f"(→1 means TIR engaged)")
    print(
        f"            Λ: Op10={diag['Lambda_Op10']:.3e}, "
        f"refl={diag['Lambda_refl']:.3e}, Hopf={diag['Lambda_Hopf']:.3e}"
    )
    print(f"            Ch 8 targets: 4π²={4*np.pi**2:.2f}, π={np.pi:.2f}, " f"4π³={4*np.pi**3:.2f}")
...
def classify(res: dict) -> str:
    ...
    ext_ratio = res["exterior_gamma_sq_sum"] / max(res["total_gamma_sq_sum"], 1e-30)
    ...
    if 2.5 < ratio < 2.7:
        ...
        return (
            f"P_X3a partial (R/r=φ² but Γ structure not confirmed: "
            f"shell_Γ²={shell_gamma:.2f}, ext_frac={ext_ratio:.2%})"
        )
    # Non-φ² ratio
    if shell_gamma > 0.9 and ext_ratio < 0.05:
        return f"P_X3b: bound state at R/r={ratio:.3f} (≠ φ²)"
    return f"P_X3b/c: R/r={ratio:.3f}, shell_Γ²={shell_gamma:.2f}, " f"A²={shell_A:.2f}, ext={ext_ratio:.2%}"


def main():
    print("=" * 72)
    print("PHASE 3b X3 — Cosserat energy minimization (rich functional)")
    print("Pre-registered in research/_archive/L3_electron_soliton/" "33_phase3b_x3_energy_analysis.md")
    print("=" * 72)
    ...
    print("=" * 72)
    print(
        f"{'seed':>22} {'R/r_f':>6} {'c':>2} {'shell_Γ²':>9} "
        f"{'ext/tot':>9} {'A²_shell':>9} {'E_final':>11} {'iter':>5}"
    )
    for res in results:
        ratio = res["R_final"] / max(res["r_final"], 1e-9)
        ext_frac = res["exterior_gamma_sq_sum"] / max(res["total_gamma_sq_sum"], 1e-30)
        print(
    ...
    for res in results:
        ...
        save[f"{tag}_energy_history"] = (
            np.array(res["energy_history"]) if "energy_history" in res else np.array(res.get("s11_history", []))
        )
        for key in [
            "R_final",
            "r_final",
    ...
    for res, color in zip(results, colors):
        ...
        if len(hist):
            ...
            ax.plot(np.abs(hist - hist[-1]) + 1e-30, color=color, label=res["label"], linewidth=1)
    ax.set_yscale("log")
    ...
    plt.suptitle("Phase 3b X3 — Cosserat energy min + seed independence", y=1.02)
    plt.tight_layout()
