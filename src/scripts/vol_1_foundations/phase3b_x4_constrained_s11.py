...
def post_hoc_gamma_and_A2(solver: CosseratField3D) -> dict:
    ...
    gamma_sq = np.asarray(_s11_density(u, w, dx, om_y, eps_y) * mask.astype(jnp.float64))
...
def measure_at_amplitude(R_target: float, r_target: float, amp_target: float, N: int = 72) -> dict:
...
def run_amplitude_sweep(
    label: str, R_target: float, r_target: float, amplitudes: list[float], N: int = 72
) -> list[dict]:
    """X4a / X4c — parameter sweep over amplitude at fixed geometry."""
    print(f"\n=== {label}: R/r = {R_target/r_target:.3f} " f"(R={R_target}, r={r_target:.3f}) ===")
    results = []
...
def run_relax_at_amplitude(
    R_target: float, r_target: float, amp_target: float, N: int = 72, max_iter: int = 200
) -> dict:
    ...
    print(f"\n--- X4b relaxation from (R/r={R0/max(r0,1e-9):.3f}, " f"|ω|_peak={amp_target:.3f}) ---")
    ...
    print(
        f"           S11={S11_f:.3e}, shell_Γ²={diag_f['shell_gamma_sq_max']:.3f}, "
...
def plot_results(x4a: list[dict], x4c: list[dict], out_path: str) -> None:
    ...
    ax = axes[0, 0]
    ax.plot(amps_a, [r["shell_gamma_sq_max"] for r in x4a], "bo-", label="X4a: R/r = φ² (Ch 8 Golden Torus)")
    ax.plot(amps_c, [r["shell_gamma_sq_max"] for r in x4c], "gs-", label="X4c: R/r = 2.0 (classical full-Clifford)")
    ax.axhline(1.0, color="red", linestyle=":", label="|Γ|²=1 (TIR)")
    ax.axvline(np.sqrt(3) / 2 * PI, color="gray", linestyle=":", label="Regime II/III = √3/2·π")
    ...
    ax = axes[0, 1]
    ax.plot(
        amps_a, [r["exterior_gamma_sq_sum"] / max(r["total_gamma_sq_sum"], 1e-30) for r in x4a], "bo-", label="X4a φ²"
    )
    ax.plot(
        amps_c, [r["exterior_gamma_sq_sum"] / max(r["total_gamma_sq_sum"], 1e-30) for r in x4c], "gs-", label="X4c 2.0"
    )
    ...
    plt.suptitle("Phase 3b X4 — amplitude sweep at Ch 8 constraints", y=1.00)
    plt.tight_layout()
...
def main():
    ...
    print("Pre-registered in research/_archive/L3_electron_soliton/" "34_x4_constrained_s11.md")
    print("=" * 72)
    ...
    amplitudes = [0.3 * PI, 0.5 * PI, 0.7 * PI, np.sqrt(3) / 2 * PI, PI, 1.2 * PI]
    print(f"\nLattice: N={N}, use_saturation=True")
    print(f"Golden Torus (X4a): R={R_golden:.3f}, r={r_golden:.3f}, " f"R/r={R_golden/r_golden:.3f}")
    print(f"Classical  (X4c): R={R_classical:.3f}, r={r_classical:.3f}, " f"R/r={R_classical/r_classical:.3f}")
    ...
    if electron_candidate:
        print(
            f"X4a electron candidate found: amp={electron_candidate['amp_target']:.3f}, "
    ...
    if classical_candidate:
        print(
            f"X4c classical candidate found: amp={classical_candidate['amp_target']:.3f}, "
    ...
    if electron_candidate and x4b_result:
        ...
        if abs(drift - 1.0) < 0.05 and x4b_result["final_shell_gamma_sq_max"] >= 0.9:
            verdict = "P_X4a.1 + P_X4b: ELECTRON found and stable"
        elif abs(drift - 1.0) >= 0.05:
            verdict = f"P_X4a.1 held but X4b showed R/r drift " f"({drift:.3f}x); electron amplitude not stationary"
        else:
    ...
    if classical_candidate and electron_candidate:
        verdict += (
            " | X4c ALSO found classical candidate — simulation " "CANNOT distinguish electron from classical Hopfion"
        )
    elif classical_candidate and not electron_candidate:
        verdict += " | X4c found classical; X4a did not — " "simulation prefers classical geometry"
    ...
    for i, r in enumerate(x4a):
        for key in ["S11", "shell_gamma_sq_max", "exterior_gamma_sq_sum", "total_gamma_sq_sum", "shell_A_sq_max", "E"]:
