...
def pre_evolve_and_select(engine: VacuumEngine3D, label: str = "engine") -> None:
    ...
    print(f"  [{label}] Pre-evolve + selection: 0 → 15 P " f"({SELECTION_END_STEP} steps)...", flush=True)
    t0 = time.time()
    last = t0
    for step in range(SELECTION_END_STEP):
        engine.step()
        if (time.time() - last) > 30.0:
            print(f"    [{label}] step {step}/{SELECTION_END_STEP}, " f"elapsed {time.time()-t0:.1f}s", flush=True)
            last = time.time()
...
def main() -> None:
    ...
    print(
        f"  Save target: t = {SELECTION_END_P} P "
    ...
    assert A26_GUARD_LOW <= omega_peak_init <= A26_GUARD_HIGH, (
        f"A26 init guard FAIL: peak |ω|={omega_peak_init:.4f} not in " f"[{A26_GUARD_LOW:.4f}, {A26_GUARD_HIGH:.4f}]"
    )
    ...
    print(f"  At t=15 P: peak |ω| = {omega_peak_at_save:.4f}, " f"||V_inc||_2 = {v_inc_l2_at_save:.4e}")
    ...
    print(
        f"  Loaded: engine B at t={engine_b.time:.4f}, "
    ...
    print(f"  ✓ engine_a.state == engine_b.state at t={engine_a.time:.4f} " f"(deterministic evolution preserved)")
    ...
    print(
        f"  State at save: t={SELECTION_END_P} P, peak |ω|={omega_peak_at_save:.4f}, "
        f"||V_inc||_2={v_inc_l2_at_save:.4e}"
    )
    print(
        f"  Round-trip: state arrays match element-wise post-load AND "
