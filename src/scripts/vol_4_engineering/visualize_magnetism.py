...
def plot_magnetism(element_name, Z, A, is_paramagnetic=False):
    ...
    streams = ax.streamplot(x, y, Vx, Vy, color=color, linewidth=1.5, 
                            density=1.5, arrowstyle='->', arrowsize=1.5)
                            
    # Scatter the nodes
    ax.scatter([n[0] for n in nodes_2d], [n[1] for n in nodes_2d], 
               color='white', s=50, edgecolors='black', zorder=5)
    
    title = f"{element_name} Flux Alignment\n"
    title += "Paramagnetic (Asymmetrical Torque Vector)" if is_paramagnetic else "Diamagnetic (Symmetrical Flux Repulsion)"
