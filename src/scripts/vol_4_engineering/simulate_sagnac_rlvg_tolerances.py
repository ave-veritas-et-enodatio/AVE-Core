...
def run_tolerance_sweeps():
    ...
    for ax in axes:
        ...
        ax.axhline(AVE_TARGET_ANOMALY_PHASE, color='#00ffcc', linestyle='-', lw=2, label='AVE Kinematic Signal Target')
        ax.fill_between([1e-15, 1e10], AVE_TARGET_ANOMALY_PHASE, 1e0, color='red', alpha=0.1, label='Noise Drowns Signal')
        ax.fill_between([1e-15, 1e10], 1e-15, AVE_TARGET_ANOMALY_PHASE, color='green', alpha=0.1, label='Clean Detection Zone')

        for spine in ax.spines.values():
            spine.set_edgecolor('#555555')
            
    fig.suptitle("RLVG Experimental System Tolerances to Isolate the AVE Sagnac Anomaly", color='#ffcc00', fontsize=16)
