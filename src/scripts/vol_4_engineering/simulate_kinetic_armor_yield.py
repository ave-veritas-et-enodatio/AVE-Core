...
from ave.core.constants import N_PHI_PACK as PHI_LIMIT  # Axiom 1: V_I Solid (Kepler Conjecture Rigid Hexagonal FCC/HCP packing limit)
...
def plot_kinetic_armor_reaction(history_p, history_phi, history_Z):
    ...
    titles = ["1. Ingress: Kinetic Wave Overloads Baseline Fluid LC",
    ...
    for i in range(3):
        ...
        ax.plot(x_axis, history_p[i], color=colors[i], linewidth=2.5, label='Kinetic Pressure Wave ($\Delta P$)')
        ...
        if len(crystallized_nodes) > 0:
             ax.plot([], [], color='white', linewidth=1.5, label='V_I Structural Snap ($\phi \geq 0.7405$, $Z \to \infty$)')
