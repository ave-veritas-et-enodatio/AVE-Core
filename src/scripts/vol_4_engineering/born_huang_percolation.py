...
def run_analysis():
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        ...
        # Poisson ratio
        nu_vac = np.where(
            (K_vac > 1e-10) & (G_vac > 1e-10),
            (3 * K_vac - 2 * G_vac) / (6 * K_vac + 2 * G_vac),
            np.nan,
        )
    ...
    ax.legend(fontsize=7, facecolor="#161b22", edgecolor="#30363d", labelcolor="white", loc="upper left")
    ...
    ax.plot(p_fine[valid_nu], nu_vac[valid_nu], "-", color="#58a6ff", linewidth=2.5, label="EMT")
    ax.axhline(2 / 7, color="#da3633", linestyle="--", linewidth=2, alpha=0.8, label=r"$\nu = 2/7$")
    ax.axhline(0.5, color="#7c3aed", linestyle=":", alpha=0.4, label=r"$\nu = 1/2$ (fluid)")
    ax.axvline(
        p_c,
        color="#238636",
        linewidth=2.5,
    ...
    fig.suptitle(
        r"P1: Vacuum Operating Point — $K = 2G$ at $p = 8\pi\alpha$",
        fontsize=16,
        color="white",
        y=1.02,
    )
