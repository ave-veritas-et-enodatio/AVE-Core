...
def simulate_log_scale_s_parameters():
    ...
    ax1.set_xlim(1e5, 1e0); ax2.set_xlim(1e5, 1e0)
    
    ax1.plot(r, n_r, color='#FFD54F', lw=3, label=r'Refractive Index ($n \propto \rho_{bulk}$)')
    ax1.plot(r, Z_eft / Z_base, color='#4FC3F7', lw=4, linestyle='--', label=r'EFT Impedance ($Z_0$ Perfectly Matched)')
    ax1.plot(r, Z_flawed / Z_base, color='#E57373', lw=2, linestyle='-.', label=r'Unmatched Dielectric Model ($Z_0$ Drops)')
    
    ax1.set_xscale('log'); ax1.set_yscale('log')
    ax1.set_title('Macroscopic Gravitational Component Divergence (Log-Log Scale)', color='white', fontsize=14, weight='bold')
    ...
    ax2.plot(r, S11_flawed, color='#E57373', lw=3, label=r'Standard Optical Dielectric Reflection ($S_{11} \gg -10$ dB)')
    ax2.plot(r, S11_eft, color='#4FC3F7', lw=4, label=r'AVE Condensate Match ($S_{11} \to -\infty$ dB)')
    
    ax2.set_xscale('log'); ax2.set_ylim(-160, 0)
    ...
    plt.tight_layout(); plt.savefig(os.path.join(OUTPUT_DIR, "log_impedance_s_parameters.png"), facecolor=fig.get_facecolor(), bbox_inches='tight'); plt.close()
