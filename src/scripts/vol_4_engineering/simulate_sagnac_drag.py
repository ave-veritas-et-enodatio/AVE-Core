...
def run_sagnac_sim():
    ...
    im = axes[0].imshow(V_nodes, aspect='auto', cmap='hot', extent=extent)
    ...
    axes[1].annotate('Co-rotating Wave\nArrives Early', xy=(t[np.argmax(V_detector[:2500])]*1e6, 40), xytext=(0.2, 80),
                     arrowprops=dict(facecolor='white', shrink=0.05), color='white', fontsize=10)
    
    axes[1].annotate('Counter-rotating Wave\nArrives Late', xy=(t[np.argmax(V_detector[2500:])+2500]*1e6, 40), xytext=(1.2, 80),
                     arrowprops=dict(facecolor='white', shrink=0.05), color='white', fontsize=10)
