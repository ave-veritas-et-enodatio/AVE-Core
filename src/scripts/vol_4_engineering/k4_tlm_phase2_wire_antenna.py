...
def simulate_wire_antenna(nx, ny, wire_path, n_steps=500,
                          source_freq_norm=0.1, probe_offset=5,
                          alternating_chirality=True):
    """
...
def broadband_simulation(nx, ny, wire_path, n_steps=800,
                         alternating_chirality=True):
    """
...
def main():
    ...
    print("\n[2] (2,3) Trefoil torus knot...")
    trefoil_path = generate_torus_knot_path_2d(
        CENTER_X, CENTER_Y, R=12, r=5, p=2, q=3, n_points=300
    )
    ...
    print("\n[3] (3,5) Torus knot...")
    knot_35_path = generate_torus_knot_path_2d(
        CENTER_X, CENTER_Y, R=12, r=5, p=3, q=5, n_points=400
    )
    ...
    trefoil_chiral = broadband_simulation(
        NX, NY, trefoil_path, n_steps=800, alternating_chirality=False
    )
    ...
    im = ax.imshow(field.T, cmap='magma', origin='lower',
                   extent=[0, NX, 0, NY])
    ...
    ax.axvline(loop_result['f_peak'], color='r', linestyle='--', alpha=0.7,
               label=f"f_peak = {loop_result['f_peak']:.3f}")
    ax.axvline(expected_f_loop, color='g', linestyle=':', alpha=0.7,
    ...
    ax.plot(freq[1:], power[1:] / np.max(power[1:]), 'b-', linewidth=1.5)
    ax.axvline(trefoil_result['f_peak'], color='r', linestyle='--', alpha=0.7,
               label=f"f_peak = {trefoil_result['f_peak']:.3f}")
    ax.axvline(expected_f_trefoil, color='g', linestyle=':', alpha=0.7,
    ...
    ax.plot(freq[1:], power[1:] / np.max(power[1:]), 'b-', linewidth=1.5)
    ax.axvline(knot_35_result['f_peak'], color='r', linestyle='--', alpha=0.7,
               label=f"f_peak = {knot_35_result['f_peak']:.3f}")
    ax.axvline(expected_f_35, color='g', linestyle=':', alpha=0.7,
               label=f"c/(2L) = {expected_f_35:.3f}")
    ...
    im = ax.imshow(h.T, cmap='RdBu_r', origin='lower', vmin=-vmax, vmax=vmax,
                   extent=[0, NX, 0, NY])
    ...
    ax = axes[2, 2]
    labels = ['Loop\n(□)', '(2,3)\nTrefoil', '(3,5)\nKnot']
    f_peaks = [loop_result['f_peak'], trefoil_result['f_peak'],
               knot_35_result['f_peak']]
    f_preds = [expected_f_loop, expected_f_trefoil, expected_f_35]
    wire_lens = [loop_result['wire_length'], trefoil_result['wire_length'],
                 knot_35_result['wire_length']]

    x_pos = np.arange(3)
    bars1 = ax.bar(x_pos - 0.15, f_peaks, 0.3, label='FFT Peak', color='#2196F3')
    bars2 = ax.bar(x_pos + 0.15, f_preds, 0.3, label='c/(2L)', color='#4CAF50',
                   alpha=0.7)
    ax.set_xticks(x_pos)
    ...
    for i, wl in enumerate(wire_lens):
        ax.annotate(f'L={wl}', (x_pos[i], max(f_peaks[i], f_preds[i]) + 0.002),
    ...
    print(f"  Loop:    L={loop_result['wire_length']},   f_peak={loop_result['f_peak']:.4f},  c/(2L)={expected_f_loop:.4f}")
    print(f"  Trefoil: L={trefoil_result['wire_length']},  f_peak={trefoil_result['f_peak']:.4f},  c/(2L)={expected_f_trefoil:.4f}")
    print(f"  (3,5):   L={knot_35_result['wire_length']},  f_peak={knot_35_result['f_peak']:.4f},  c/(2L)={expected_f_35:.4f}")
