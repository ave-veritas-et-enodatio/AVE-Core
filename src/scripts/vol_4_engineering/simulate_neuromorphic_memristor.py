...
def plot_memory_hysteresis(times, V_app, current_I, w_state):
    ...
    ax1.axhline(E_GAP_TRIGGER, color='white', linestyle='--', alpha=0.9, linewidth=1.5, label='AVE Void-Fraction Gap Limit (+0.2158 eV)')
    ax1.axhline(-E_GAP_TRIGGER, color='white', linestyle='--', alpha=0.9, linewidth=1.5, label='AVE Void-Fraction Gap Limit (-0.2158 eV)')
    
    # The physical structure variable w
    ax1_twin = ax1.twinx()
    ax1_twin.plot(times, w_state, color='#FF4444', linewidth=2.5, label='Structural State $w$ ($V_{II} \Rightarrow V_I$)')
    
    ax1.set_title("1. Dielectric Topological Yield tracking Action Potentials", color='white', fontsize=12, fontweight='bold')
    ...
    ax2.set_title("2. I-V Hysteresis: True Artificial Hardware Memory", color='white', fontsize=12, fontweight='bold')
