...
def main():
    ...
    for ch, marker, color in [(+1, 'o', '#2196F3'), (-1, 's', '#F44336')]:
        ...
        ax.scatter(np.real(evals), np.imag(evals), s=80, marker=marker,
    ...
    im = ax.imshow(snap.T, cmap='magma', origin='lower',
                   extent=[0, snap.shape[0], 0, snap.shape[1]])
    ...
    ax.set_title('Energy Conservation\n(boundary-absorbed, never increases)',
                 fontsize=11)
    ...
    print("\n" + "=" * 70)
    all_pass = (
        unitarity[+1]['passed'] and unitarity[-1]['passed'] and
        energy['passed'] and
        eigen[+1]['all_unit'] and eigen[-1]['all_unit']
    )
    if all_pass:
