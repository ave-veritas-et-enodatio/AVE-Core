...
def simulate_torus_knot_3d(N, p, q, R, r, n_steps=400):
    ...
    wire = generate_torus_knot_path_3d(center, center, center, R, r, p, q,
                                       n_points=max(200, 20 * (p + q)))
...
def main():
    ...
    # ── Test 3: (2,3) Trefoil ──
    print("\n[3] (2,3) Trefoil...")
    trefoil_chiral = simulate_torus_knot_3d(
        N, p=2, q=3, R=10, r=4, n_steps=400
    )
    ...
    # ── Test 4: (3,5) Knot ──
    print("\n[4] (3,5) Torus knot...")
    knot_35 = simulate_torus_knot_3d(
        N, p=3, q=5, R=10, r=4, n_steps=400
    )
    ...
    print("\n[5] (7,11) Torus knot...")
    knot_711 = simulate_torus_knot_3d(
        N, p=7, q=11, R=10, r=4, n_steps=400
    )
    ...
    im = ax.imshow(h_diff.T, cmap='RdBu_r', origin='lower',
    ...
    ax.axvline(knot_35['f_peak'], color='r', ls='--', alpha=0.7,
               label=f"f = {knot_35['f_peak']:.3f}")
    ax.set_title(f"(3,5) Knot FFT\nL = {knot_35['wire_length']} nodes",
                 fontsize=11)
    ax.set_xlabel('Normalized Frequency')
    ax.legend(fontsize=9)
    ax.set_xlim(0, 0.15)
    ...
    ax.axvline(knot_711['f_peak'], color='r', ls='--', alpha=0.7,
               label=f"f = {knot_711['f_peak']:.3f}")
    ax.set_title(f"(7,11) Knot FFT\nL = {knot_711['wire_length']} nodes",
                 fontsize=11)
    ax.set_xlabel('Normalized Frequency')
    ax.legend(fontsize=9)
    ax.set_xlim(0, 0.15)
    ...
    ax = axes[3, 0]
    labels = ['(2,3)\nTrefoil', '(3,5)\nKnot', '(7,11)\nKnot']
    f_peaks = [trefoil_achiral['f_peak'], knot_35['f_peak'],
               knot_711['f_peak']]
    wire_lens = [trefoil_achiral['wire_length'], knot_35['wire_length'],
                 knot_711['wire_length']]
    ...
    for i, (fp, wl) in enumerate(zip(f_peaks, wire_lens)):
        ax.annotate(f'L={wl}', (i, fp + 0.001), ha='center', fontsize=8,
                    color='gray')
    ...
    for result, label, color in [
        ...
        ax.plot(wx + [wx[0]], wy + [wy[0]], '-', color=color,
                lw=1.0, alpha=0.7, label=label)
    ...
    ax.plot(pq_ratio, alpha_val * pq_ratio, 'k-', lw=2,
            label=r'$\Delta f/f = \alpha \cdot pq/(p+q)$')
    # AVE predictions for our knots
    knots = [(2, 3), (3, 5), (7, 11)]
    for (p_k, q_k), color in zip(knots, colors):
        pq = p_k * q_k / (p_k + q_k)
        ax.plot(pq, alpha_val * pq, 'o', color=color, ms=10,
    ...
    for result in [trefoil_achiral, knot_35, knot_711]:
        ...
        pq = p_k * q_k / (p_k + q_k)
        pred = alpha_val * pq
        print(f"  ({p_k},{q_k}){'':<10} {result['wire_length']:<8} "
