...
def simulate_memristor_and_skin_effect() -> None:
    ...
    ax2.plot(
        ...
        color="#FFD54F",
        lw=4,
        label=r"AC Metric Skin Depth ($\delta \propto \sqrt{R_{eff}}$)",
    )
    ...
    ax2.set_title(
        "Metric Faraday Cage: The Zero-Impedance Skin Effect",
        color="white",
        fontsize=14,
        weight="bold",
    )
    ...
    ax2.text(
        ...
        color="white",
        fontsize=11,
        bbox=dict(facecolor="#111111", edgecolor="#FFD54F", alpha=0.9, pad=10),
    )
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(OUTPUT_DIR, "memristor_and_skineffect.png"),
...
if __name__ == "__main__":
    simulate_memristor_and_skin_effect()
