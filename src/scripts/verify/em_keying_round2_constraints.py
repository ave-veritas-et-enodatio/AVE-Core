...
def worked_content_W(E_timeseries, dt, coeff=C_W_YIELD, mode="var"):
    """The DERIVED worked content W of a time-series E(t) at a cell (dimensionless, /Ec^2).

    mode='var'  : W_var  = Var_t(E)/Ec^2                 (AC-variance, freq-INDEPENDENT) [WORKED-VAR]
    mode='beat' : W_beat = <(dE/dt)^2>/(wC^2 Ec^2)       (temporal-gradient)             [WORKED-BEAT]
    A field STATIC IN TIME (constant array) -> Var=0 and dE/dt=0 -> W=0 for BOTH modes: BLIND.
    """
...
def constraint_1_muonic(r_cut_factors=(0.5, 1.0, 2.0)):
    """The physical muonic-H atom under the WORKED functional -- passes BY DERIVATION.
...
def main():
    ...
    print("    WORKED content W on the physical (static-in-time) atom fields:")
    for k, v in c1["W_physical_at"].items():
        print(f"      W({k}) = {v:.3e}  (static in TIME -> Var_t(E)=0 -> BLIND, no net-vs-local needed)")
    ...
    print(f"    PASSES (< window)? = {c1['passes']}  -> the round-1 killer DISSOLVES by derivation")
