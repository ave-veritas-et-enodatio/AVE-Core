...
if __name__ == "__main__":
    Z, A = 6, 12
    bound, grid_res, frames = 65.0, 80, 60
    name = "carbon_12"
    title = "Carbon-12 ($^{12}C$): Subcritical $3\\alpha$ Ring Strain"
    
    nodes = get_nucleon_coordinates(Z, A)
    if not nodes:
        print(f"Error: No coordinates for Z={6}, A={12}")
        sys.exit(1)
    ...
    anim.save(os.path.join(outdir, "carbon_12_dynamic_flux.gif"), writer='pillow', fps=15, savefig_kwargs={'facecolor': fig.get_facecolor()})
    print("[*] Done.")
