...
if __name__ == "__main__":
    Z, A = 8, 16
    bound, grid_res, frames = 75.0, 80, 60
    name = "oxygen_16"
    title = "Oxygen-16 ($^{16}O$): $4\\alpha$ Macro-Tetrahedron Strain"
    
    nodes = get_nucleon_coordinates(Z, A)
    if not nodes:
        print(f"Error: No coordinates for Z={8}, A={16}")
        sys.exit(1)
    ...
    anim.save(os.path.join(outdir, "oxygen_16_dynamic_flux.gif"), writer='pillow', fps=15, savefig_kwargs={'facecolor': fig.get_facecolor()})
    print("[*] Done.")
