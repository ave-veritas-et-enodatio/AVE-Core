...
Z, A = 11, 23
bound, grid_res = 160.0, 80
title = "Sodium-23 ($^{23}Na$): X-Z Plane Structural Asymmetry"
...
stream_lines = ax.streamplot(x, z, np.gradient(density)[1], np.gradient(density)[0], color='#aaaaaa', linewidth=1.2, density=1.5, arrowstyle='->', arrowsize=1.5)
...
target = os.path.join(outdir, "sodium_23_dynamic_flux.gif")
