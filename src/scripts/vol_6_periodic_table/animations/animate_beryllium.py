...
if __name__ == "__main__":
    Z, A = 4, 9
    bound, grid_res, frames = 10.0, 80, 60
    name = "beryllium_9"
    title = "Beryllium-9 ($^9Be$): Stretched Dual-Core Strain"
    
    nodes = get_nucleon_coordinates(Z, A)
    if not nodes:
        print(f"Error: No coordinates for Z={4}, A={9}")
        sys.exit(1)
    ...
    anim.save(os.path.join(outdir, "beryllium_9_dynamic_flux.gif"), writer='pillow', fps=15, savefig_kwargs={'facecolor': fig.get_facecolor()})
    print("[*] Done.")
