...
Z, A = 12, 24
bound, grid_res = 130.0, 80
title = "Magnesium-24 ($^{24}Mg$): Dynamic Octahedral Flux"

nodes = get_nucleon_coordinates(Z, A)
if not nodes:
    print(f"Error executing Mg24 nodes")
    sys.exit(1)
...
target = os.path.join(outdir, "magnesium_24_dynamic_flux.gif")
