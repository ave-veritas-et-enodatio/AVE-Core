...
def animate_dt_fusion_transient():
    ...
    def update(frame):
        ...
        if frame < 30:
            ...
            nodes = [
                ...
                np.array([1.0 + np.random.uniform(-0.1, 0.1), 0.2 + np.random.uniform(-0.1, 0.1), 0]) # The parasitic neutron
            ]
        ...
        for n in nodes:
            ...
            ax.scatter(n[0], n[1], color='#00ffcc', s=100, edgecolor='#00ffcc', facecolor='none', linewidth=1.5, alpha=0.9)
    ...
    anim.save(os.path.join(output_dir, "dt_fusion_transient.gif"), writer='pillow', fps=15, savefig_kwargs={'facecolor': fig.get_facecolor()})
