...
if __name__ == "__main__":
    ...
    name = "boron_11"
    title = "Boron-11 ($^{11}B$): Massive Parasitic Halo Strain"
    
    nodes = get_nucleon_coordinates(Z, A)
    if not nodes:
        print(f"Error: No coordinates for Z={5}, A={11}")
        sys.exit(1)
    ...
    anim.save(os.path.join(outdir, "boron_11_dynamic_flux.gif"), writer='pillow', fps=15, savefig_kwargs={'facecolor': fig.get_facecolor()})
    print("[*] Done.")
