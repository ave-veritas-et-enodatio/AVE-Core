...
def animate_beryllium_8_decay():
    ...
    def update(frame):
        ...
        ax.set_title(f"Beryllium-8 ($^8Be \\rightarrow 2\\alpha$) Transient Decay\\nStatus: {status}", color=color, fontsize=16, pad=20)
        ax.tick_params(colors='white')
        
    anim = FuncAnimation(fig, update, frames=frames, interval=80, blit=False)
    anim.save(os.path.join(output_dir, "be8_decay_transient.gif"), writer='pillow', fps=15, savefig_kwargs={'facecolor': fig.get_facecolor()})
...
def animate_tritium_decay():
    ...
    def update(frame):
        ...
        if e_pos is not None:
            rotated_e = np.dot(np.array([[np.cos(angle), 0, np.sin(angle)], [0, 1, 0], [-np.sin(angle), 0, np.cos(angle)]]), e_pos)
        ...
        if e_pos is not None:
            ...
            ax.scatter(rotated_e[0], rotated_e[1], color='yellow', s=150 * depth_scale, marker='o', alpha=1.0)
            target_color = "yellow"
            status = "BETA EMISSION (Relaxing to $^3He$)"
            
        ax.set_title(f"Tritium ($^3H \\rightarrow ^3He + e^-$) Transient Decay\\nStatus: {status}", color=target_color, fontsize=16, pad=20)
        ax.tick_params(colors='white')
        
    anim = FuncAnimation(fig, update, frames=frames, interval=80, blit=False)
    anim.save(os.path.join(output_dir, "h3_decay_transient.gif"), writer='pillow', fps=15, savefig_kwargs={'facecolor': fig.get_facecolor()})
