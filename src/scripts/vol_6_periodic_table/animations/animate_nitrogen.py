...
if __name__ == "__main__":
    ...
    name = "nitrogen_14"
    title = "Nitrogen-14 ($^{14}N$): Optimized Asymmetric Mesh Strain"
    
    nodes = get_nucleon_coordinates(Z, A)
    if not nodes:
        print(f"Error: No coordinates for Z={7}, A={14}")
        sys.exit(1)
    ...
    anim.save(os.path.join(outdir, "nitrogen_14_dynamic_flux.gif"), writer='pillow', fps=15, savefig_kwargs={'facecolor': fig.get_facecolor()})
    print("[*] Done.")
