...
if __name__ == "__main__":
    ...
    name = "lithium_7"
    title = "Lithium-7 ($^7Li$): Asymmetrical Secondary Shell Strain"
    
    nodes = get_nucleon_coordinates(Z, A)
    if not nodes:
        print(f"Error: No coordinates for Z={3}, A={7}")
        sys.exit(1)
    ...
    anim.save(os.path.join(outdir, "lithium_7_dynamic_flux.gif"), writer='pillow', fps=15, savefig_kwargs={'facecolor': fig.get_facecolor()})
    print("[*] Done.")
