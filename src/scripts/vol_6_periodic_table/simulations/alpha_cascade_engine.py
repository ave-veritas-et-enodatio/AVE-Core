...
from ave.core.constants import K_MUTUAL, ALPHA, HBAR, C_0, e_charge, Z_0, M_P_MEV_TARGET, M_N_MEV_TARGET, D_PROTON, ALPHA_HBAR_C, D_INTRA_ALPHA
...
def nuclear_s11_loss(positions_flat, n_alphas, Z_total, A_total):
    ...
    ordered_dists = jnp.array([dists[order[k], order[(k+1) % n_alphas]] 
                                for k in range(n_alphas - 1)])
    ordered_coupling = jnp.array([net_coupling[order[k], order[(k+1) % n_alphas]] 
                                   for k in range(n_alphas - 1)])
