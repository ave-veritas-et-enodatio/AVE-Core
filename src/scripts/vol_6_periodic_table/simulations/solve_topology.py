...
from ave.core.constants import K_MUTUAL, M_P_MEV_TARGET as M_P_RAW, M_N_MEV_TARGET as M_N_RAW, D_PROTON as D_MIN
...
def find_optimal_topology(Z, A, empirical_mass_mev, n_iter=1000):
    ...
    minimizer_kwargs = {"method": "L-BFGS-B", "bounds": bounds, 
                        "args": (Z, A, empirical_mass_mev)}
                        
    # Run global optimization
    print(f"Running Basinhopping Global Optimizer ({n_iter} iterations)...")
    res = basinhopping(objective_function, initial_guess, 
                       minimizer_kwargs=minimizer_kwargs, 
                       niter=n_iter, stepsize=2.0)
