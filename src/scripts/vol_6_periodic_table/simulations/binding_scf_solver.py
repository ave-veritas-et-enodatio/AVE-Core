...
def compute_binding_scf(nodes, V_ref, verbose=False):
    ...
    for p in range(n_pairs):
        for s in range(N_PATH_SAMPLES):
            for k in range(n):
                dist_to_samples[p, s, k] = np.linalg.norm(
                    path_points[p, s] - nodes[k]
                )
    ...
    for iteration in range(MAX_SCF_ITER):
        ...
        if iteration > 0:
            ...
            for k in range(n):
                pairs_with_k = [p for p, (i, j) in enumerate(pair_indices) 
                               if i == k or j == k]
                if pairs_with_k:
        ...
        if verbose:
            print(f"  SCF iter {iteration:3d}: BE={be_current:.6f} MeV  "
...
if __name__ == "__main__":
    ...
    if best_vref and best_err < 0.1:
        ...
        print(f"{'Element':8s} {'CODATA':>12s} {'Bare Err':>10s} {'SCF Err':>10s} "
        ...
        for name, Z, A, emp in ELEMENTS:
            ...
            if not nodes or len(nodes) <= 1:
                print(f"{name:8s} {emp:12.3f} {'+0.0000%':>10s} {'+0.0000%':>10s} "
                      f"{'1.0000':>8s} {'0':>5s} {'✅':>5s}")
                continue
            ...
            print(f"{name:8s} {emp:12.3f} {err_bare:+9.4f}% {err_scf:+9.4f}% "
