...
def plot_flux_inferno(nodes, bounds, z_slice, title, filename, grid_res=120):
    ...
    ax.imshow(density, extent=[-bounds, bounds, -bounds, bounds],
              origin='lower', cmap=cmap, alpha=0.9, vmin=0.0)
    grad_y, grad_x = np.gradient(density)
    ax.streamplot(x, y, grad_x, grad_y, color='#aaaaaa', linewidth=1.2,
                  density=1.5, arrowstyle='->', arrowsize=1.5)
    for cx, cy, cz in nodes:
        depth_scale = np.exp(-np.abs(cz / (bounds / 3.0)))
        ax.scatter(cx, cy, color='#00ffcc', s=300 * depth_scale, marker='+',
