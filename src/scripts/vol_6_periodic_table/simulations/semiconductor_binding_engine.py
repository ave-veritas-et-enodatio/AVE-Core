...
from ave.core.constants import K_MUTUAL, ALPHA, HBAR, C_0, e_charge, L_NODE, PROTON_ELECTRON_RATIO, M_P_MEV_TARGET, M_N_MEV_TARGET, D_PROTON
...
def solve_halo_element(name, n_alpha, Z_core, Z_halo, n_halo, mass_codata,
                       core_func, core_R, verbose=True):
    ...
    for lo, hi in [(1, 500), (0.5, 2000), (0.1, 5000)]:
        try:
            ...
            if e_lo * e_hi < 0:
                ...
                if verbose:
                    print(f"{name:8s} {n_alpha:3d}α+T  R_halo={R_sol:8.3f}d  "
...
if __name__ == "__main__":
    ...
    if "O-16" in solved_R:
        solve_halo_element("F-19", 4, 8, 1, 3, 17692.302,
                          make_tetrahedron, solved_R["O-16"])
    
    # Na-23: Ne-20 core (5α ring) + Tritium halo
    if "Ne-20" in solved_R:
        solve_halo_element("Na-23", 5, 10, 1, 3, 21409.214,
                          lambda R: make_ring(5, R), solved_R["Ne-20"])
    
    # Al-27: Mg-24 core (6α octahedron) + Tritium halo
    if "Mg-24" in solved_R:
        solve_halo_element("Al-27", 6, 12, 1, 3, 25126.501,
                          make_octahedron, solved_R["Mg-24"])
