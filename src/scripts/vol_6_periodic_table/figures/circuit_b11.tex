...
\node[text=aveink, font=\bfseries\Large] at (0, 8.2)
    {B-11 \quad Semiconductor Equivalent Circuit};
\node[text=avemuted, font=\small] at (0, 7.5)
    {$2\alpha + ^3\text{H}$ Core+Halo \quad|\quad $V_R/V_{BR} = N/A$ \quad|\quad $M = N/A$ (Core+Halo)};
...
\node[text=aveink, font=\small, draw=avemuted, dashed, rounded corners=3pt, inner sep=5pt]
    at (0, -5.6) {B-11: Below model threshold \quad (Core+Halo, no avalanche)};
...
\node[small alpha] (c0) at (-6.0, -10.0) {$\alpha$};
\node[small alpha] (c1) at (-3.0, -10.0) {$\alpha$};
\node[small alpha, draw=avevermillion, fill=white!85!avevermillion] (h0) at (-4.5, -11.5) {$n$};

\draw[aveaccent, thick] (c0) -- (c1);
\draw[avevermillion, thick, dashed] (c0) -- (h0); \draw[avevermillion, thick, dashed] (c1) -- (h0);


\node[text=avemuted, font=\tiny, text width=2.6cm, align=left, anchor=west] at (-1.5, -9.5)
    {1 inter-$\alpha$ + 2 bridge\\\textbf{3 junctions}};
...
\node[text=aveink, font=\small, text width=7cm, align=left,
      ...
      at (2.5, -7.6) {
    $M_{\text{nuc}} = 2\, M_\alpha - BE_{\text{net}}$\\[6pt]
    $BE_{\text{net}} = \underbrace{\displaystyle\sum_{i<j}^{1}
    ...
    \textcolor{avevermillion}{Miller:} $M = N/A$ ($V_R/V_{BR} = N/A$)\\[4pt]
    \textbf{Result:} $10,246.100$ MeV ($0.0000\%$ error)
};
