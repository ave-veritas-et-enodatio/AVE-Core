...
\node[small alpha, draw=neonorange, fill=darkbg!80!neonorange] (h0) at (-1.5, -10.0) {$^3\text{H}$};

\draw[neonorange, thick, dashed] (c0) -- (h0) node[midway, above, text=white, font=\tiny] {$M_{\text{halo}}$};
...
\node[text=white, font=\small, text width=7cm, align=left,
    ...
    $BE_{\text{net}} = \underbrace{\displaystyle\sum_{i<j}^{0} 16 \cdot \frac{K}{R_{ij}}}_{\ text{Strong (attractive)}}
