...
\node[text=aveink, font=\bfseries\Large] at (0, 8.2)
    {Li-7 \quad Semiconductor Equivalent Circuit};
\node[text=avemuted, font=\small] at (0, 7.5)
    {$1\alpha + ^3\text{H}$ Core+Halo \quad|\quad $V_R/V_{BR} = N/A$ \quad|\quad $M = N/A$ (Core+Halo)};
...
\node[text=aveink, font=\small, draw=avemuted, dashed, rounded corners=3pt, inner sep=5pt]
    at (0, -5.6) {Li-7: Below model threshold \quad (Core+Halo, no avalanche)};
...
\node[small alpha] (c0) at (-4.5, -10.0) {$\alpha$};
\node[small alpha, draw=avevermillion, fill=white!85!avevermillion] (h0) at (-1.5, -10.0) {$^3\text{H}$};

\draw[avevermillion, thick, dashed] (c0) -- (h0) node[midway, above, text=aveink, font=\tiny] {{$M_{{\text{{halo}}}}$}};


\node[text=avemuted, font=\tiny, text width=2.6cm, align=left, anchor=west] at (-1.5, -9.5)
    {1 core-halo link\\\textbf{1 junction}};
...
\node[text=aveink, font=\small, text width=7cm, align=left,
      ...
      at (2.5, -7.6) {
    $M_{\text{nuc}} = 1\, M_\alpha - BE_{\text{net}}$\\[6pt]
    $BE_{\text{net}} = \underbrace{\displaystyle\sum_{i<j}^{0}
    ...
    \textbf{Result:} $6,533.834$ MeV ($0.0000\%$ error)
};
