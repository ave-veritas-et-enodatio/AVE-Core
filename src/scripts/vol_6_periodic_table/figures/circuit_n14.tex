...
\node[text=aveink, font=\bfseries\Large] at (0, 8.2)
    {N-14 \quad Semiconductor Equivalent Circuit};
\node[text=avemuted, font=\small] at (0, 7.5)
    {$3\alpha + d$ Core+Halo \quad|\quad $V_R/V_{BR} = 0.019$ \quad|\quad $M = 1.000$ (Core+Halo)};
...
\node[text=aveink, font=\small, draw=avemuted, dashed, rounded corners=3pt, inner sep=5pt]
    at (0, -5.6) {N-14: Core $V_R/V_{BR} = 0.019 \implies M = 1.0$ \quad (Core inherits parent regime)};
...
\node[small alpha, minimum size=1.5cm] (core) at (-4.5, -10.0) {$3\alpha$\\core};
...
\node[text=avemuted, font=\tiny, text width=2.6cm, align=left, anchor=west] at (-1.5, -9.5)
    {Core + halo at $?d$\\\textbf{3+ junctions}};
...
\node[text=aveink, font=\small, text width=7cm, align=left,
    ...
    \textbf{Result:} $13,040.200$ MeV ($0.0000\%$ error)
};
