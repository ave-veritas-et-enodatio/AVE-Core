...
\node[text=aveink, font=\bfseries\Large] at (0, 8.2)
    {He-4 \quad Semiconductor Equivalent Circuit};
\node[text=avemuted, font=\small] at (0, 7.5)
    {Single Alpha (Reference Tank) \quad|\quad $V_R/V_{BR} = N/A$ \quad|\quad $M = N/A$ (Reference)};
...
\node[text=avemuted, font=\small, draw=avemuted, dashed, rounded corners=3pt, inner sep=5pt]
    at (0, -5.6) {He-4: $V_R/V_{BR} = N/A \implies M = N/A$ \quad (Deep Small Signal)};
...
\node[small alpha, minimum size=1.5cm] (core) at (-4.5, -10.0) {$1\alpha$\\core};
...
\node[text=avemuted, font=\tiny, text width=2.6cm, align=left, anchor=west] at (-1.5, -9.5)
    {Core + halo at $?d$\\\textbf{0+ junctions}};
...
\node[text=aveink, font=\small, text width=7cm, align=left,
    ...
    \textbf{Result:} $3,727.379$ MeV ($0.0000\%$ error)
};
