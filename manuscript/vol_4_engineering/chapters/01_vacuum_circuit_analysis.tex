...
propagation 2026-05-15).} The translation dictionary above maps EE projection
quantities to ME projections. AVE-QED's universal substrate vocabulary
(canonical at AVE-QED \texttt{appendices/G\_substrate\_vocabulary.tex}; locked
2026-05-14 Grant Q1 closure) extends this to a third column ---
...
Bond & transmission line & spring &
  Default: bond. \emph{Spring} acceptable when invoking Q-G47 buckling pedagogy. \\
\hline
...
(Grant Q1-locked 2026-05-14 evening).} These three integers/half-integer are the
only externally observable invariants at any local $\Gamma = -1$ boundary
(substrate-observability rule canonical per A-026, A-028). This is the no-hair
theorem applied at every scale: from outside a BH, observers see $M, Q, J$; from
outside an electron, observers see $m_e, e, \hbar/2$. Same structure, scale-invariant.
...
(13/13 tests PASS per E-101) provides reference implementations:
\texttt{compute\_M}, \texttt{compute\_Q}, \texttt{compute\_J} for scalar substrate
fields (e.g., from MasterEquationFDTD).

\textbf{Cross-references:}
\begin{itemize}\itemsep 0pt
\item \textbf{Canonical picture-first reference:} the
  \texttt{trampoline-framework.md} KB doc (ground-up build + three
  storage modes + substrate-observability rule + multi-scale boundaries)
\item \textbf{Cosmic-scale application (A-031, 2026-05-15 evening):}
  $\mathcal{J}_{\text{cosmic}} = \Omega_{\text{freeze}} \cdot I_{\text{cosmic}}$
  encodes the cosmological initial condition (Vol~3 Ch~4 \S{}sec:cosmic\_J\_as\_IC)
\item \textbf{Formal upstream:} AVE-QED \texttt{appendices/G\_substrate\_vocabulary.tex}
  \S{}4 (canonical definitions); \texttt{docs/glossary.md} \S{}5m (14-row vocab table)
\item \textbf{L5 status:} A-026 (substrate-observability rule), A-028 (three
  substrate invariants canonical names), A-029 (shared-spring inter-cell coupling),
  A-031 (cosmic-$\mathcal{J}$ as IC; refined per A-034 2026-05-15 evening:
  ``God's Hand'' decoupled into cosmic-parameter horizon vs observable mechanism),
  \textbf{A-034} (Universal Saturation-Kernel Strain-Snap Mechanism, 19-instance
  cross-scale catalog spanning 21 orders of magnitude; see Vol~0 backmatter);
  E-094 / E-101 (this implementation)
\end{itemize}
