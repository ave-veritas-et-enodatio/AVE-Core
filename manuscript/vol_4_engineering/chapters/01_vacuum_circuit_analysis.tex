...
\paragraph{Default Yield Threshold.} Both FDTD and K4-TLM default to $V_{yield} = \sqrt{\alpha}\,V_{snap} \approx 43.65$~kV as the Axiom~4 nonlinear onset (see Section~\% missing reference sec:yield_thresholds in Chapter~2 for the engineering rationale). Subatomic-scale simulations (e.g., bond energy solvers, Yang--Mills confinement) should override with \texttt{v\_yield=V\_SNAP} ($\approx 511$~kV).
