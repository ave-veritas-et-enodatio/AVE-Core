...
% [2026-08-02 DEAD CROSS-REFERENCE REPAIR -- pointer only, no physics content added or
%   removed. Prior wording (git trail): "via the \textbf{Cross-Sectional Porosity
%   ($\Phi_A \equiv \alpha^2$)} derived in Chapter 4."
%   WHY IT WAS DEAD: `grep -rn --include='*.tex' -e 'Phi_A' -e 'porosity' -e 'Porosity'
%   manuscript/vol_0_engineering_compendium/` returns exactly ONE hit -- this line. Vol 0's
%   own Chapter 4 (04_dcve.tex) never mentions Phi_A or porosity, so "Chapter 4" resolved to
%   nothing in this volume. The sentence is inherited verbatim from
%   manuscript/backmatter/01_appendices.tex:53, where the chapter numbering is different.
%   WHERE Phi_A ACTUALLY ENTERS (two-method verified at HEAD -- corpus-wide tex grep above
%   plus a KB grep): manuscript/vol_3_macroscopic/chapters/01_gravity_and_yield.tex:95 --
%   "the effective differential solid angle is modified by the cross-sectional porosity
%   ($\Phi_{A}\equiv\alpha^{2}$)" -- mirrored in the KB at
%   ave-kb/vol3/gravity/ch01-gravity-yield/optical-refraction-gravity.md:54. The pointer is
%   repaired to that site. A prose pointer is used rather than \ref because Vol 0's Vol-3
%   xr-hyper pull is guarded by \IfFileExists and unresolved in a standalone build.
%   FLAG-DON'T-FIX (recorded, NOT adjudicated in this pass; no printed wording changed):
%   the derivation-grade words "derived" and "emerges" in this paragraph sit on a claim the
%   KB grades as an ASSERTED MECHANISM -- ave-kb/vol2/claim-quality.md:665 (clm-nhlo1e),
%   quoted exactly as it reads at HEAD: "the $R^2$ entropy scaling is asserted to be
%   recovered ... No calculation of $S_{BH}=A/(4\ell_P^2)$ ab initio; the porosity-to-area
%   argument is presented as a geometric mechanism, not a derivation. Asserted mechanism."
%   (The board's sec-5 KB-TRUTH row renders that sentence with **bold** emphasis markers
%   that are NOT present in the leaf at HEAD -- content identical, emphasis added; recorded
%   here so the cite chain stays exact.) Claim record
%   ave-kb/.index/claims.jsonl clm-nhlo1e: solidity 0.3, build_status "do not build on,
%   rework needed". The section-level framing is already adjudicated acceptable by
%   clm-yawl6z (quoted in this chapter's register banner above), so the residual debt is
%   sentence-level wording -- routed, not resolved here.
\textbf{The Resolution:} The AVE framework recovers the Holographic Principle via the \textbf{Cross-Sectional Porosity ($\Phi_A \equiv \alpha^2$)} introduced in Volume~3, \emph{Trace-Reversal, Gravity, and Macroscopic Yield} (\kbleaf{ave-kb/vol3/gravity/ch01-gravity-yield/optical-refraction-gravity.md}). While the physical hardware nodes occupy 3D Voronoi volumes, the transmission of kinematic states (signals/information) must traverse the 1D inductive flux tubes. The bandwidth of these connections is geometrically bounded by their 2D cross-sectional area. Applying the Nyquist-Shannon sampling theorem to the substrate graph shows that the effective Information Channel Capacity of the universe is projected onto the 2D bounding surface area of the causal horizon. Thus, the Holographic Principle emerges from discrete network mechanics, averting the $R^3$ divergence.
