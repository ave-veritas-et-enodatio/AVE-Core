...
The AVE framework provides a candidate explanation.  Cytochrome~$c$
...
This yields a tentative prediction: ``mind uploading'' creates a
...
\begin{summarybox}
\noindent\textbf{Status declaration (surfaced from canon 2026-09-06; the summary items below are unchanged).} The content of this chapter is classified in canon as \textbf{hypothesis and prediction, not derived result}. The KB chapter index states verbatim: ``No resultboxes; all content is hypothesis and prediction'' (\kbleaf{ave-kb/vol5/biological-applications/index.md}). The governing claim record is \texttt{clm-8zwyl3} at solidity $0.10$, build status \emph{``refuted, do not use''} (\kbleaf{ave-kb/vol5/claim-quality.md}), which further records that ``\textbf{None of these sections constitute derived numerical predictions}'' and ``Does NOT claim clinical efficacy for any therapy listed (red light, methylene blue, EMDR, creatine, MB+660 nm combination) \ldots{} the AVE explanation is the hypothesis, not a treatment recommendation.'' The cancer, red-light-therapy and methylene-blue items below are to be read at that grade; the consciousness item already carries its own ``hypothesised'' marker.
