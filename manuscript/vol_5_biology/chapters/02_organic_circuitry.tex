...
\begin{enumerate}
    ...
    \end{equation}
    where $E_{HB} = 0.2158$~eV (the hydrogen bond energy, derived from Op4 $\times$ void fraction in \S\ref{sec:ave_water_hb}) and $n_{coop} = 9$ is the cooperative amplification constant.
...
This prediction matches the anomalous density maximum of water at $+4.0^\circ$C to within $0.40\%$---cross-validating the cooperative amplification number $n_{coop} = 9$ (see Appendix \ref{sec:n_coop_derivation} for the full dual derivation from Axioms 1 and 2).
