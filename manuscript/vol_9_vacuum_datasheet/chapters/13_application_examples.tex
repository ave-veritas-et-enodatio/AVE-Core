...
\begin{itemize}
    ...
    \item Preserve the load-bearing framings carried through every preceding chapter: substrate is natural (no engineered applications); $\nu_{vac} = 2/7$ is the Ax~1 LC + Ax~2 $\alpha$ interlink (not promoted); $c_{EM}$ vs $c_{shear}$ are not interchangeable (Pitfall~\#5); Machian $G$ is mixed --- form-derived (the Achromatic-Lens) but value calibrated ($\xi$ calibrated to CODATA $G$; \kbleaf{common/interlock-register.md}), not a primitive axiom; lattice-genesis is single-seed \emph{within our cosmic horizon} (one seed, not many simultaneous disjoint seeds sharing our horizon --- not a claim that ours is the only universe); BH interiors are Regime~IV ruptured plasma (not a B-sublattice), and a melted interior \emph{can} re-crystallize into a daughter cosmology --- nested parent/daughter cosmologies across $\Gamma=-1$\gammaundeclared{} melt$\to$recrystallize boundaries are permitted (Ch.~\ref{ch:vol9_cosmological_characteristics}~$\S$\ref{sec:vol9_cosmo_substrate_rupture}; our own universe is a daughter per \kbleaf{common/omega-freeze-cosmic-grain-cascade.md}).
...
% 🔴 def-tk1xfm STATUS FLIP 2026-08-02 (CRIB-5; Rule 12 -- prior wording PRESERVED verbatim here and
%   in git: "the transducer node is provisional, not yet ratified"). Authority (INVARIANT-S12: the
%   vocabulary register is the authority for def- nodes): ave-kb/common/vocabulary-register.md:441
%   -- "**status:** SOLID -- GRANT-RATIFIED 2026-07-21 (verbatim `[sic]`: *\"ratify def-tk1xfm\"*)",
%   on the framed ratification package quoting the REGIME FENCE ("EXACT, lossless, gain-1,
%   pole-less, invertible co-equality of the mechanical and electrical descriptions BELOW the band
%   edge (omega*tau << 1 / long-wave regime) ... at band-edge scales the distributed-in-bond (arccos
%   TL) and lumped mass-spring descriptions book the delay differently and are no longer co-equal")
%   and the standing STRENGTH CEILING ("identity-by-translation, NOT emerges-from / NOT a
%   derivation ... SOLID means the node is ratified/canonical, NOT that it derives a mechanism",
%   reaffirmed by the Cleave-01 Chern null clm-clvchn). Adjudicated-meaning at :438. Per CRIB-5 the
%   sites carry the RATIFIED status WITH both riders -- not the pre-ratification "exact dimensional
%   isomorphism" absolutism, and not the stale "provisional".
%   OUT-OF-LANE DEBT, FLAGGED NOT FIXED (this lane is manuscript/vol_9_vacuum_datasheet/ only): the
%   Vol-9 KB leaf ave-kb/vol9/ch3-pin-port-configuration/device-circuit-models.md is STALE against
%   the register -- :60 "transduced, `def-tk1xfm` provisional" and :214 "def-tk1xfm is itself
%   `status:proposed`, awaiting auditor + Grant ratification -- NOT yet canonical". This .tex cites
%   that leaf. Until the leaf is repaired the manuscript points at a document that contradicts it.
%   Surfaced for the orchestrator; NOT edited in-lane.
\textbf{Substrate primitives invoked.}
...
\begin{itemize}
    ...
    \item \textbf{The EM-$\Omega$ $\leftrightarrow$ mechanical-$\rho c$ TKI-transformer} (Ch.~\ref{ch:vol9_topological_characteristics} $\xi_{topo}$; \kbleaf{device-circuit-models.md}:205--210): two ports in different units cannot be directly wired --- a domain crossing requires a TRANSDUCER (an electro-mechanical change-of-reference), realized as an ideal transformer (turns-ratio scaling), not a direct wire and not a gyrator. \emph{Status (\texttt{def-tk1xfm}, \textbf{RATIFIED SOLID}, Grant 2026-07-21; \kbleaf{common/vocabulary-register.md}:435--441):} the Topo-Kinematic Isomorphism (Axiom~2) is an exact, lossless, gain-1, pole-less, invertible \textbf{co-equality of the mechanical and electrical descriptions below the band edge} ($\omega\tau \ll 1$, the long-wave regime). Two riders travel with the ratification. \textbf{(a) Regime fence:} the co-equality is regime-scoped --- at band-edge scales the distributed-in-bond (arccos transmission-line) and lumped mass--spring descriptions book the propagation delay differently and are \emph{no longer} co-equal. \textbf{(b) Strength ceiling:} it is \emph{identity-by-translation}, \textbf{not} emerges-from and \textbf{not} a derivation of a mechanism --- SOLID means the node is canonical, not that it derives anything (ceiling reaffirmed by the Cleave-01 Chern null, \texttt{clm-clvchn}). The dynamics (poles $=$ masses, the $Q = 1/\alpha$ radiative leak, the Axiom-4 saturation trap) live in the resonator transfer function, not in the Axiom-2 dictionary.
...
\input{figures/spice_ladder_cells.tex}
...
\midrule
$\alpha$ derivation (\S\ref{sec:vol9_app_electron_lc_alpha}) & $\Gamma=-1$\gammaundeclared{} TIR (Ax~3) & $L_{cell}$, $C_{cell}$, $Z_0$, $\xi_{topo}$ & --- & --- & $u_0^*$ kernel closure & --- & --- & $(2,3)$ trefoil, $0_1$ unknot, $T \to 2T$ & $u_0^*$ magic angle \\
...
%
%   :180  family: Z_bulk=rho*c formula class  [BIAS-DEBT]
%        QUOTE (content verified at HEAD; markup-reduced from the banked audit): the bulk channel $Z_{bulk} = \sqrt{2}\,\rho_{bulk}\, c_0$ at $K =
%        2G$ ($\Gamma_{bulk} \to -1$, confined --- the MASS-``3'')
%       STAMPED AT: :180
%        RATIONALE (verbatim, research/drivers/r40_sweep_worklist_verified.json): Application-chapter
%        recurrence of the three-channel Z=rho*c formula (also :187 2-domain N-port); confined role and
%        mass accounting survive, formula consumes phantom.
