...
\begin{itemize}
    ...
    \item Preserve the load-bearing framings carried through every preceding chapter: substrate is natural (no engineered applications); $\nu_{vac} = 2/7$ is the Ax~1 LC + Ax~2 $\alpha$ interlink (not promoted); $c_{EM}$ vs $c_{shear}$ are not interchangeable (Pitfall~\#5); Machian $G$ is derived (not a primitive axiom); lattice-genesis is single-seed (no multiverse, no baby universes); BH interiors are Regime~IV ruptured plasma (not a B-sublattice or daughter cosmology).
...
\textbf{Cross-chapter chain.} The electron is a $(2,3)$ trefoil in Clifford-torus phase-space coordinates on the K4 substrate (Ch.~\ref{ch:vol9_topological_characteristics}~$\S$\ref{sec:vol9_torus_knot_winding}) with real-space body the $0_1$ unknot at minimum ropelength. At the magic-angle operating point $u_0^*$ (Ch.~\ref{ch:vol9_cosmological_characteristics}~$\S$\ref{sec:vol9_cosmo_magic_angle}), the substrate's bond pair saturates at $\Gamma = -1$ at both ends (Ch.~\ref{ch:vol9_pin_port_configuration}~$\S$\ref{sec:vol9_boundary_saturation_conditions}), forming a TIR-confined LC tank whose stored-energy quality factor is set by the codimensional mode-count assembly at the Golden-Torus geometry. The canonical Theorem~3.1 closure (canonical at \texttt{ave-kb/vol1/ch8-alpha-golden-torus.md}, \texttt{clm-0ktpcn}) is the addition of three independent codimensional Nyquist-cell mode counts at the $\Gamma = -1$ boundary:
...
The three terms are: the $4\pi$ spinor-temporal phase volume from the $T \to 2T \subset SU(2)$ K4-rotation-group chain (Ch.~\ref{ch:vol9_topological_characteristics}~$\S$\ref{sec:vol9_su2_so3_double_cover}); the Clifford-torus 2-area for substrate-spinor spatial half-cover; and the Ampère 1-cycle around the transverse cross-section perimeter at Nyquist-quantized diameter. The cold-lattice value $\alpha^{-1}_{\text{ideal}} = 4\pi^3 + \pi^2 + \pi$ is the substrate's intrinsic value at $T \to 0$, $A_0 = 0$, $S(0) = 1$ (canonical \texttt{ALPHA\_COLD\_INV}, \texttt{src/ave/core/constants.py}:164). The observed CODATA $\alpha^{-1} = 137.035999$ sits below the cold-lattice asymptote by the fractional vacuum strain $\delta_{strain} \approx 2.225 \times 10^{-6}$ documented in \S\ref{sec:vol9_app_delta_strain_cosmic_tcc} below (the Cosserat-Curie thermal-mode-population ASYM correction at $T = T_{CMB}$).

\textbf{Discipline class.} Per \texttt{consistency-vs-emergence} v1.3: the cold-lattice closure $\alpha^{-1}_{\text{ideal}} = 4\pi^3 + \pi^2 + \pi$ is a Class~B substrate-mechanism mapping --- the SM-language fine-structure constant identifies as the substrate's $\Gamma = -1$ TIR-mode count at the electron LC tank's Golden-Torus geometry. The decimal accuracy $|\alpha^{-1}_{\text{CODATA}}/\alpha^{-1}_{\text{ideal}} - 1| \approx 2.2 \times 10^{-6}$ is identified at substrate-mechanism class (Class~B) for the $\delta_{strain}$ correction; the quantitative substrate-statistical-mechanics derivation is open (Q-DELTA-MAP-1-quant; Ch.~\ref{ch:vol9_temperature_characteristics}~$\S$\ref{sec:vol9_temperature_open}).
...
\begin{itemize}
    ...
    \item \textbf{SYM-class scaling} (gravity-class realization): $\mu(r)$ and $\varepsilon(r)$ scale together by the same factor so $Z_0 = \sqrt{\mu/\varepsilon}$ stays invariant; canonical at \texttt{ave-kb/vol3/gravity/ch01-gravity-yield/alpha-invariance-symmetric-gravity.md}, \texttt{clm-3zz0f6} (Ch.~\ref{ch:vol9_pin_port_configuration}~$\S$\ref{sec:vol9_port_class_taxonomy}; Ch.~\ref{ch:vol9_saturation_characteristics}~$\S$\ref{sec:vol9_alpha_invariance}).
    \item \textbf{Machian $G$ as integrated boundary impedance} $G = c^4/(7\xi T_{EM}(u_0^*))$ with dimensionless Machian-hierarchy coupling $\xi = 4\pi(R_H/\ell_{node})\alpha^{-2}$ (Ch.~\ref{ch:vol9_cosmological_characteristics}~$\S$\ref{sec:vol9_cosmo_machian_g}; \texttt{full-derivation-chain.md} \texttt{clm-9oazz0}).
...
\begin{itemize}
    ...
    \item \textbf{Op17 power transmission} $T^2 = 1 - \Gamma^2$ at every substrate port (Ch.~\ref{ch:vol9_pin_port_configuration}~$\S$\ref{sec:vol9_operator_port_behavior}; canonical-source \texttt{ave-kb/common/operators.md} Op17 row).
...
\textbf{Observed phenomenon.} CODATA $\alpha^{-1} = 137.035999$ sits below the cold-lattice substrate asymptote $\alpha^{-1}_{\text{ideal}} = 4\pi^3 + \pi^2 + \pi \approx 137.0363038$ by the fractional vacuum strain coefficient $\delta_{strain} \approx 2.225 \times 10^{-6}$.
...
\begin{itemize}
    ...
    \item \textbf{CMB thermal bath at $T_{CMB} = 2.725$~K} (Ch.~\ref{ch:vol9_temperature_characteristics}~$\S$\ref{subsec:vol9_thermal_population}); Boltzmann factor $\exp(-\hbar\omega_m/k_BT_{CMB}) \sim \exp(-4.3 \times 10^9) \approx 0$ for B-modes.
