...
\begin{itemize}
    ...
    \item \textbf{$\Gamma$ boundary semantics (three-channel).} Substrate boundaries carry channel-subscripted reflection coefficients $\Gamma_{\mathrm{EM}}$, $\Gamma_{\mathrm{shear}}$, $\Gamma_{\mathrm{bulk}}$ with impedances $Z_{\mathrm{EM}} \equiv Z_0$, $Z_{\mathrm{shear}} = \rho c_{\mathrm{shear}}$, $Z_{\mathrm{bulk}} = \rho c_{\mathrm{bulk}}$ (three-impedance law; KB \kbleaf{ave-kb/vol9/ch4-dc-electrical-characteristics/three-channel-impedances.md}). Axiom~3 minimizes $|\Gamma|^2$ per channel. Device equivalent circuits: KB \kbleaf{ave-kb/vol9/ch3-pin-port-configuration/device-circuit-models.md}; PDF figures Ch.~\ref{ch:vol9_pin_port_configuration}~\S\ref{sec:vol9_device_circuit_models}.
...
12 & Cosmological Characteristics       & $R_H/\ell_{node} \sim 10^{39}$; Machian $G$ (mixed); $u_0^* \approx 0.187$ (value back-fit) \\
