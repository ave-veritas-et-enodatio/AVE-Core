...
\begin{itemize}
    ...
    \item Identify Project PONDER-05 at $V_{DC}/V_{yield} = 0.687$ as the canonical bench-scale falsifier of the dielectric specialization mechanism.
...
The substrate's bulk response to local strain is the Axiom~4 universal saturation kernel. The canonical statement is reproduced verbatim from \texttt{manuscript/common\_equations/eq\_axiom\_4.tex} (single source of truth; INVARIANT-S2 mirror at \texttt{manuscript/ave-kb/CLAUDE.md}):
...
At the atomic and bench scale, the dialect-projection $A = \Delta\phi/\alpha$ specializes the kernel to a substrate-native varactor characteristic (\texttt{common\_equations/eq\_axiom\_4.tex}):
...
The substrate carries \textbf{two distinct substrate-native effective wave speeds} at operating point $A_0$, and they are not interchangeable in derivations (canonical at KB leaf \texttt{vol3/gravity/ch01-gravity-yield/alpha-invariance-symmetric-gravity.md} \texttt{clm-8nkvwy} lines 111--113; conflation is the canonical Pitfall~\#5 framework-leakage error caught in the 2026-05-28 Phase 3-A3 WALK-BACK):
...
\noindent
\textbf{Boundary values $r_1$, $r_2$, $r_3$:} derived from first principles in \texttt{ave-kb/vol1/operators-and-regimes/ch7-regime-map/four-regimes.md} ($r_1 = \sqrt{2\alpha} \approx 0.1208$ is the sub-$\alpha$-resolvability limit; $r_2 = \sqrt{3}/2 \approx 0.8660$ is the spin-2 avalanche-onset limit from $Q(r) = 1/S(r) = \ell_{\min} = 2$; $r_3 = 1$ is the Axiom~4 rupture boundary).
...
\noindent
\textbf{Miller multiplication identity:} the substrate-native form is $M(r) = 1/S(r)^2 = 1/(1 - r^2)$, identical to the engineering avalanche-multiplication law $M = 1/(1 - (V_R/V_{BR})^n)$ at $n = 2$. The semiconductor $V_{BR}$ is the engineering name for $V_{yield}$ at the doped-silicon scale; the same kernel governs both.

\section{PONDER-05 Bench-Scale Falsifier}
\label{sec:vol9_ponder_05}

The PONDER-05 differential-saturation-parallax bench measurement is the canonical bench-scale falsifier of the Axiom~4 dielectric specialization. Apparatus: paired DC-biased quartz resonator thrusters across a vertical gravity gradient, $V_{DC} = 30\,\text{kV}$ holding the substrate at $V_{DC}/V_{yield} = 0.687$. At this operating point the kernel predicts $S(0.687) = 0.7266$, giving canonical substrate-physics observables:
...
\begin{itemize}
    ...
    \item Differential thrust $\sim 469\,\mu\text{N}$ at matched-pair geometry
\end{itemize}

The canonical operating-point statement is at KB leaf \texttt{ave-kb/vol4/falsification/ch11-experimental-bench/measurement-hierarchy-snr.md}:66; the substrate-physics derivation at \texttt{ave-kb/vol4/circuit-theory/ch1-vacuum-circuit-analysis/parametric-coupling-kernel.md} (Phase 0c + Phase 2-NA extensions); the divergence-test-matrix row at \texttt{ave-kb/common/divergence-test-substrate-map.md} row \texttt{B7-PONDER-05}; the hardware programme at sibling repo \texttt{AVE-PONDER} (manuscript chapter \texttt{vol\_ponder/chapters/04\_ponder\_05\_dc\_biased\_quartz.tex}).

\textbf{Falsification criterion.} A null result at $V_{DC}/V_{yield} = 0.687$ (no $C_{eff}$ rise within bench noise floor) falsifies the Axiom~4 dielectric specialization directly. The cascade impact is documented in the divergence-test-substrate-map (B7-PONDER-05 cascades to B5/B6 PONDER, B1-VAC-BIREFRINGE, D4-A034, C9-LEVITATION through the shared $V_{yield}$ primitive). See Ch.~15 (Falsification Tests) for the full falsification programme.

\textbf{Operating-point near-optimality.} The PONDER-05 canonical $V_{DC}/V_{yield} = 0.687$ sits within $2.8\%$ of the substrate-mechanical 2D-aperture aperture-aggregate skewness peak at $a^{(2D)}_{peak} = 1/\sqrt{2} = 0.707$ (per \texttt{parametric-coupling-kernel.md} \S 14.5). The canonical operating point --- chosen for the 27.4\% $\varepsilon_{eff}$ collapse + 469\,\textmu N thrust at engineering-conservative bias --- is operationally near-optimal for the amplitude-statistics readout layer as well; the substrate-native Op17 matched-impedance peak structure is shared across all three observables (thrust, $\varepsilon_{eff}$ collapse, aperture-aggregate skewness).

\section{A-034 Universal Kernel Catalog Reference}
\label{sec:vol9_a034_catalog}

The same Axiom~4 kernel governs every cross-scale topological-reorganization event in the substrate. The canonical catalog of record is \texttt{manuscript/ave-kb/common/universal-saturation-kernel-catalog.md} (KB leaf \texttt{clm-gz7ryg}, \texttt{clm-dxdsvt}, \texttt{clm-hvvvop}, \texttt{clm-5fu303}, \texttt{clm-l4o7hv}), mirrored in the Vol~0 backmatter universal saturation-kernel catalog chapter.
...
\textbf{Tightest validations.} BCS $B_c(T) = B_{c0}\, S(T/T_c)$ at $0.00\%$ error across all measured Type-I/II superconductors (ASYM-N($\mu$) class); BH event horizon $\varepsilon_{11}(r) = 1$ matching Schwarzschild $r_s = 2GM/c^2$ exactly; BH merger QNM $\omega_R M_g = 18/49$ at $1.7\%$ from GR exact across 3 LIGO events; planetary-axis kernel scoring $14$--$15/16$ class matches across 16 planetary axis data points (Row 9-a + Row 9-b). The framework simplifies rather than complicates as evidence accumulates: the kernel does not change with scale, only the lattice scale changes.
...
Miller avalanche multiplication $M = 1/(1 - (V_R/V_{BR})^n)$ at $n = 2$ & Substrate Miller identity $M(r) = 1/S(r)^2$; $V_{BR}$ at semiconductor scale is the engineering name for substrate $V_{yield}$. \\
Avalanche breakdown $V_{BR}$ (diode) & Substrate dielectric breakdown at $V_{snap}$ (Schwinger pair-production analog at the doped-silicon scale; Regime III $\to$ IV transition). \\
...
\begin{itemize}
    ...
    \item \textbf{Universal saturation-kernel catalog:} \texttt{manuscript/ave-kb/common/universal-saturation-kernel-catalog.md} (\texttt{clm-gz7ryg}, \texttt{clm-dxdsvt}, \texttt{clm-hvvvop}, \texttt{clm-5fu303}, \texttt{clm-l4o7hv}; cross-scale catalog of record).
    \item \textbf{Four-regime map:} \texttt{manuscript/ave-kb/vol1/operators-and-regimes/ch7-regime-map/four-regimes.md} (\texttt{clm-2dwzib}, \texttt{clm-b2anl4}; regime-boundary derivations from first principles).
    \item \textbf{$\alpha$-invariance under SYM-class:} \texttt{manuscript/ave-kb/vol3/gravity/ch01-gravity-yield/alpha-invariance-symmetric-gravity.md} (\texttt{clm-3zz0f6}; canonical proof).
    ...
    \item \textbf{PONDER-05 substrate-physics anchor:} \texttt{manuscript/ave-kb/vol4/circuit-theory/ch1-vacuum-circuit-analysis/parametric-coupling-kernel.md} (Phase 0c + Phase 2-NA $P(\delta V)$ shape function derivation).
    \item \textbf{PONDER-05 operating-point canonical statement:} \texttt{manuscript/ave-kb/vol4/falsification/ch11-experimental-bench/measurement-hierarchy-snr.md}:66.
    \item \textbf{EE saturation translation:} \texttt{manuscript/ave-kb/common/translation-tables/translation-circuit.md} \S 9 (\texttt{clm-eemap1} META framework).
    \item \textbf{Vol~4 dielectric thresholds:} Vol~4 canonical leaf \texttt{clm-0vxzfu} ($V_{yield}$, $V_{snap}$).
    \item \textbf{Substrate-distinct $\alpha$-modulation under ASYM-class:} \texttt{ave-kb/vol3/cosmology/ch05-dark-sector/delta-strain-cosmic-tcc.md} (\texttt{clm-hp7nlm}, Cosserat-rotation-sector mass-gap thermal-mode-population ASYM mechanism).
\end{itemize}
