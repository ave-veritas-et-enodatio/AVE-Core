...
\begin{itemize}
    ...
    \item Document Project PONDER-05 at $V_{DC}/V_{yield} = 0.687$ as a \emph{material-scale consistency analog} of the kernel shape (a quartz voltage-coefficient-of-capacitance reproducing the saturation-arc form), not a vacuum-kernel falsifier.
...
\noindent
\textbf{Electron operating point (quiescent point) --- datasheet line.} The electron is a \emph{self-biased multi-port LC circuit} parked at a self-set, self-stable quiescent point: the \emph{bias} is the saturation-state $A$ (set by Axiom-4 self-saturation, no external bias network); the \textbf{CHARGE} port (the \emph{mechanical} Cosserat $(2,3)$ micro-rotation $\to Z_{\mathrm{shear}}$, the static reactive charge boundary) self-traps at $V_{yield} = \sqrt{\alpha}\,V_{snap} \approx \qty{43.65}{\kilo\volt}$ --- $Z_{\mathrm{shear}} \to 0$, the $\Gamma = -1$\gammaundeclared{} confinement cage (transverse-$T_2$, \texttt{def-vyvsn1}, Grant 2026-06-30); the \textbf{MASS} port (A1 dilatation $\to Z_{\mathrm{bulk}}$) has its characteristic \emph{energy scale} at $V_{snap} \approx \qty{511}{\kilo\volt}$ (the rest energy $m_e c^2$) but \textbf{operates sub-saturated} inside that cage at $A = V_{yield}/V_{snap} = \sqrt{\alpha} \approx 0.085$ ($S \approx 0.996$, $Z_{\mathrm{bulk}}$ finite --- the $S \to 0$ varactor runaway never fires, which is \emph{why} it binds) --- and the \emph{EM} $\mu$-sector (transduced from that micro-rotation circulation via the Axiom-1 $\omega\!\leftrightarrow\!\mathbf{B}$ / \texttt{def-tk1xfm} dictionary --- RATIFIED SOLID 2026-07-21, exact below the band edge; identity-by-translation, \emph{not} a derivation) reaches $\mu_{eff}\to0$ there, the Meissner onset (a direct $\mu_{eff}$-on-$Z_{\mathrm{shear}}$ wire is forbidden, \kbleaf{ave-kb/vol4/circuit-theory/ch1-vacuum-circuit-analysis/resonant-lc-solitons.md}:129), and the equilibrium $R \cdot r = \tfrac14$ is the stable Q-point (rest-mass $=$ the DC energy stored there). The three-channel impedance network ($Z_{\mathrm{EM}}/Z_{\mathrm{shear}}/Z_{\mathrm{bulk}}$, Ch.~\ref{ch:vol9_pin_port_configuration}~\S\ref{sec:vol9_three_channel_circuit}) is the \emph{small-signal} response linearized around this quiescent point --- completing the EE workflow DC-bias~$\to$~linearize ($\varepsilon_{eff},\mu_{eff},C_{eff}$ at $A$)~$\to$~AC. CONSISTENCY-class re-expression of the operating-point-state + varactor canon; canonical at \kbleaf{ave-kb/vol4/circuit-theory/ch1-vacuum-circuit-analysis/node-up-small-large-signal.md} \S 4b (def-q1escn). \emph{(Whether the Q-point sets $R \cdot r = \tfrac14$ $\alpha$-free, and the transverse self-Meissner build, are PENDING/open --- not asserted here; the channel-impedance-mismatch $Q = 137$ target stays empty.)}

\subsection{Two distinct effective wave speeds}
\label{sec:vol9_two_wave_speeds}

The substrate carries \textbf{two distinct substrate-native effective wave speeds} at operating point $A_0$, and they are not interchangeable in derivations (canonical at \kbleaf{ave-kb/vol3/gravity/ch02-general-relativity/einstein-field-equation.md}, ``Symmetric vs Asymmetric Saturation''; conflating them is the canonical Pitfall~\#5 framework-leakage error):
...
\subsection{Bulk rarefaction floor $\bar{\rho}_{cav} = -1/\varphi$}
...
\end{equation}
This is the bulk analog of the shear-sector $A = A_{yield}$ ceiling. The $-1/\varphi$ value is derived (the $c_{bulk}=0$ root); it is provisional pending experimental confirmation of the bulk rarefaction floor. The bulk-mode clock that freezes at this floor is documented in Ch.~\ref{ch:vol9_ac_electrical_characteristics} \S Temporal Characteristics (Ch.~\ref{ch:vol9_absolute_maximum_ratings} \S Bulk Rarefaction Floor records it as the rarefaction-side absolute limit).

\subsection{$\alpha$-invariance under SYM-class saturation}
\label{sec:vol9_alpha_invariance}

Under SYM-class scaling --- the gravity-class realization where $\mu(r)$ and $\varepsilon(r)$ scale together by the same factor so $Z_0$ stays invariant --- the asymmetry between $c_{EM}$ and $c_{shear}$ makes $\alpha$ \textbf{exactly} invariant. Canonical proof at \kbleaf{ave-kb/vol3/gravity/ch01-gravity-yield/alpha-invariance-symmetric-gravity.md}. The SYM-class realization simultaneously produces gravitational time-dilation via $c_{shear}$ tracking $\sqrt{S}$ (Schwarzschild reduction) while preserving $\alpha$ exactly --- multi-species clock comparisons at different gravitational potentials predict a null result for $\Delta\alpha/\alpha$, consistent with all current experimental bounds.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\textwidth]{saturation_curves.pdf}
\caption{Substrate operating-point characteristic curves vs the normalized saturation $r = A_0/A_{yield}$ (Tab.~\ref{tab:vol9_saturation_curves}), all following from the single Axiom~4 kernel $S(r) = \sqrt{1 - r^2}$: the transverse-T2 permittivity $\varepsilon_{eff}/\varepsilon_0 = S$ softens to $0$; the longitudinal-A1 compliance $C_{eff}/C_0$ and the Maxwell phase velocity $c_{EM}/c_0$ both stiffen as $1/S$ and diverge; the mechanical / rest-mass velocity $c_{shear}/c_0 = \sqrt{S}$ rolls to $0$. The four-regime boundaries $r_1 = \sqrt{2\alpha}$, $r_2 = \sqrt{3}/2$, $r_3 = 1$ are marked. Constants imported from \kbleaf{figures/gen\_saturation\_curves.py}; no observable target hardcoded.}
\label{fig:vol9_saturation_curves}
\end{figure}

\section{Four-Regime Map}
\label{sec:vol9_four_regimes}

The kernel partitions the operating-point axis into four universal regimes. The semiconductor analog --- standard p-n junction operating regions --- is exact, not metaphor (canonical at \kbleaf{ave-kb/vol1/operators-and-regimes/ch7-regime-map/four-regimes.md}):
...
The canonical operating-point statement is at KB leaf \kbleaf{ave-kb/vol4/falsification/ch11-experimental-bench/measurement-hierarchy-snr.md}:66; the substrate-physics derivation at \kbleaf{ave-kb/vol4/circuit-theory/ch1-vacuum-circuit-analysis/parametric-coupling-kernel.md}; the divergence-test-matrix row at \kbleaf{ave-kb/common/divergence-test-substrate-map.md} row \texttt{B7-PONDER-05}; the hardware programme at sibling repo \texttt{AVE-PONDER} (manuscript chapter \kbleaf{AVE-PONDER/manuscript/vol\_ponder/chapters/04\_ponder\_05\_dc\_biased\_quartz.tex}).
...
\textbf{Aperture-aggregate skewness peak (facility-class).} The substrate-mechanical 2D-aperture aperture-aggregate skewness peak sits at $a^{(2D)}_{peak} = 1/\sqrt{2} = 0.707$ (per \kbleaf{parametric-coupling-kernel.md} \S 14.9, the Op17 matched-impedance condition). This is a substrate-geometric forward prediction in the \emph{vacuum} per-node amplitude $a$, and is facility-class to test: the vacuum reaches $a \sim 0.7$ only at $\sim 8\times10^{16}$~V/m. The numerical proximity of the PONDER-05 apparatus ratio $V_{DC}/V_{yield} = 0.687$ to this peak is coincidental --- $0.687$ is the quartz \emph{material's} ratio, while the vacuum per-node $a$ at the PONDER-05 bench is $\sim 10^{-7}$, so the PONDER-05 bench is not a near-optimal probe of the aperture-aggregate skewness observable.

\section{A-034 Universal Kernel Catalog Reference}
\label{sec:vol9_a034_catalog}

The same Axiom~4 kernel governs every cross-scale topological-reorganization event in the substrate. The canonical catalog of record is \kbleaf{manuscript/ave-kb/common/universal-saturation-kernel-catalog.md}, mirrored in the Vol~0 backmatter universal saturation-kernel catalog chapter.

\textbf{Catalog cardinality (canonical).} The kernel governs \textbf{26 canonical cross-scale instances} (17 physical-substrate canonical + 2 biological-substrate + 5 engineered-substrate + 2 physical-substrate companion rows (provisional, pending classification); full enumeration in the catalog). The Vol~0 backmatter and \texttt{eq\_axiom\_4.tex} cite the physical-substrate-only subset as \textbf{19 cross-scale topological-reorganization events} (17 canonical + 2 scoped). Both counts are correct at their respective scopes; the cross-scale span is uniformly \textbf{21 orders of magnitude} (atomic $\sim 10^{-15}\,\text{m}$ to cosmic $\sim 10^{26}\,\text{m}$).

\textbf{Symmetry classification (canonical).} The catalog partitions instances into SYM (vacuum $K = 2G$; $\varepsilon$ and $\mu$ saturate together; 19 instances), ASYM-N (single-sector --- only $\varepsilon$ or only $\mu$; 4 instances: BCS $\mu$-only, plasma $\varepsilon$-only, planetary mag-vs-spin candidate, cosmic-DE $\varepsilon$-only), ASYM-E (engineered $K/G \neq 2$; 1 instance: active topological metamaterials), and TBD (2 scoped instances: galactic spin-axis, LSS spin-axis). The PONDER-05 material-scale consistency analog sits in the SYM class at the DC-biased-piezoelectric engineered row.

% 🔴 VALIDATION-SCOPE CORRECTION 2026-08-02 (CRIB-4; Rule 12 -- the prior sentence is PRESERVED
%   verbatim here and in git: "\textbf{Tightest validations.} BCS $B_c(T) = B_{c0}\, S(T/T_c)$ at
%   $0.00\%$ error across all measured Type-I/II superconductors (ASYM-N($\mu$) class); BH event
%   horizon $\varepsilon_{11}(r) = 1$ matching Schwarzschild $r_s = 2GM/c^2$ exactly; BH merger QNM
%   $\omega_R M_g = 18/49$ at $1.7\%$ from GR exact across 3 LIGO events; planetary-axis kernel
%   scoring $14$--$15/16$ class matches across 16 planetary axis data points (Row 9-a + Row 9-b).").
%   TWO CRIB-4 items fire in this one sentence:
%   (1) RINGDOWN -- Grant Ruling B1 (2026-07-21). The 18/49 eigenvalue at -1.7% is the COLD
%       Schwarzschild (a* = 0) SINGLE-POINT result:
%       ave-kb/vol3/cosmology/ch15-black-hole-orbitals/ave-merger-ringdown-eigenvalue.md:21 gives
%       "omega_R * M_g = ell(1+nu_vac)/x_sat = 18/49 = 0.3673 (GR exact: 0.3737, error 1.7%)" and
%       :24-25 "This cold Schwarzschild eigenvalue SURVIVES the 2026-07-20 correction below"; the
%       CATALOG comparison is RETRACTED at :115/:127/:145-148 -- ":127 'sub-2%' RETRACTED
%       2026-07-20 -- that figure was computed against the corrupt omega_R/omega_I tables +
%       source-frame masses"; ":148 eigenvalue (18/49, -1.7%) is the only surviving
%       zero-free-parameter result" (lines verified at HEAD). Per Rule 12
%       the "across 3 LIGO events" clause is STRUCK WITH NO REPLACEMENT NUMBER -- the retracted
%       frame-mixed catalog match (#774/#780) is not re-fitted here, and no substitute event count
%       or error figure is supplied.
%   (2) BCS -- ave-kb/claim-quality.md:171-182: the 0.00% is a DEFINITIONAL IDENTITY at per-material
%       calibration (per-material T_c, B_c0 inputs), not a fit. Corrected phrasing copied verbatim
%       in shape from the already-repaired exemplar
%       manuscript/vol_1_foundations/chapters/07_regime_map.tex:17-18 ("BCS $B_c(T)$, which matches
%       the kernel form by definitional identity (per-material $T_c$, $B_{c0}$ inputs)").
%   The Schwarzschild-radius row and the planetary-axis row are UNCHANGED (neither is a CRIB-4
%   item). No value in this chapter is refilled, re-fitted, or invented.
\textbf{Tightest validations.} BCS $B_c(T) = B_{c0}\, S(T/T_c)$, which matches the kernel form by \emph{definitional identity} at per-material calibration (per-material $T_c$, $B_{c0}$ inputs --- an identity, not a fit; ASYM-N($\mu$) class); BH event horizon $\varepsilon_{11}(r) = 1$ matching Schwarzschild $r_s = 2GM/c^2$ exactly; BH merger QNM $\omega_R M_g = 18/49$ at $1.7\%$ from GR exact --- the \textbf{cold Schwarzschild ($a_* = 0$) single-point anchor}, which is the only surviving zero-free-parameter ringdown result (Grant Ruling~B1, 2026-07-21; \kbleaf{vol3/cosmology/ch15-black-hole-orbitals/ave-merger-ringdown-eigenvalue.md}); planetary-axis kernel scoring $14$--$15/16$ class matches across 16 planetary axis data points (Row 9-a + Row 9-b). The kernel form does not change with scale across these entries; only the lattice scale changes.

\noindent\emph{Retraction note (Rule~12, 2026-08-02).} This entry previously read ``$1.7\%$ from GR exact \emph{across 3 LIGO events}''. The spinning-remnant / catalog comparison is \textbf{retracted} as a frame-mixed match artifact (\#774/\#780; \kbleaf{vol3/cosmology/ch15-black-hole-orbitals/ave-merger-ringdown-eigenvalue.md}), and \textbf{no replacement number is supplied} --- the cold $a_* = 0$ single point is what survives.
...
\midrule
Variable-capacitance diode (varactor) $C(V)$ characteristic & Substrate operates as substrate-native varactor: $C_{eff}(V_{DC}) = C_0/S(V_{DC}/V_{yield})$. PONDER-05 at $V_{DC}/V_{yield} = 0.687$ is the canonical material-scale consistency analog. \\
...
Miller avalanche multiplication $M = 1/(1 - (V_R/V_{BR})^n)$ at $n = 2$ & Substrate Miller identity $M(r) = 1/S(r)^2$; $V_{BR}$ at the \emph{heavily-doped (Zener) junction} scale maps to the macroscopic-onset axis $V_{yield} = \sqrt{\alpha}\,V_{snap}$ (Vol~4 $V_{yield}$ canonical leaf; same junction-doping split as Ch.~\ref{ch:vol9_breakdown_characteristics} \S EE-translation). \\
Avalanche breakdown $V_{BR}$ (lightly-doped diode) & At the \emph{lightly-doped (avalanche) junction} scale, $V_{BR}$ maps instead to the microscopic absolute-rupture axis $V_{snap}$ (Schwinger pair-production analog; Regime~III$\to$IV transition; Vol~4 $V_{snap}$ axis-disambiguation). The same EE symbol $V_{BR}$ reads on the $V_{yield}$ axis for Zener-doped junctions and on the $V_{snap}$ axis for avalanche-doped junctions. \\
...
\begin{itemize}
    ...
    \item \textbf{Universal saturation-kernel catalog:} \kbleaf{manuscript/ave-kb/common/universal-saturation-kernel-catalog.md} (cross-scale catalog of record).
    \item \textbf{Four-regime map:} \kbleaf{manuscript/ave-kb/vol1/operators-and-regimes/ch7-regime-map/four-regimes.md} (regime-boundary derivations from first principles).
    \item \textbf{$\alpha$-invariance under SYM-class:} \kbleaf{manuscript/ave-kb/vol3/gravity/ch01-gravity-yield/alpha-invariance-symmetric-gravity.md} (canonical proof).
    \item \textbf{Two effective wave speeds ($c_{EM}$ vs $c_{shear}$):} \kbleaf{manuscript/ave-kb/vol3/gravity/ch02-general-relativity/einstein-field-equation.md} (Symmetric vs Asymmetric Saturation).
    \item \textbf{PONDER-05 substrate-physics anchor:} \kbleaf{manuscript/ave-kb/vol4/circuit-theory/ch1-vacuum-circuit-analysis/parametric-coupling-kernel.md} (the $P(\delta V)$ shape-function derivation).
    ...
    \item \textbf{EE saturation translation:} \kbleaf{manuscript/ave-kb/common/translation-tables/translation-circuit.md} \S 9 (EE-as-substrate-native META framework).
    \item \textbf{Vol~4 dielectric thresholds:} Vol~4 canonical leaf ($V_{yield}$, $V_{snap}$).
    \item \textbf{Substrate-distinct $\alpha$-modulation under ASYM-class:} \kbleaf{ave-kb/vol3/cosmology/ch05-dark-sector/delta-strain-cosmic-tcc.md} (Cosserat-rotation-sector mass-gap thermal-mode-population ASYM mechanism).
\end{itemize}
