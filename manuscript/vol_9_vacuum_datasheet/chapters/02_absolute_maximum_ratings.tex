...
The substrate-physics derivation of each rating lives in the canonical leaves cited below; this chapter is a synthesis spec table, not a re-derivation. Numerical values are imported from the canonical-constant module \kbleaf{src/ave/core/constants.py}, never hard-coded.
...
\midrule
$V_{snap}$ & $\num{5.110e5}$ & \si{\volt} & Ax 2 + Ax 4: topological node-pair destruction at $\Delta\phi = m_e c^2$ across one $\ell_{node}$ & \kbleaf{vol1/.../dielectric-snap-limit.md}; \kbleaf{constants.py} \texttt{V\_SNAP} \\ % clm-2dwzib
$V_{yield}$ & $\num{4.365e4}$ & \si{\volt} & Ax 4: macroscopic dielectric nonlinear-onset; $S(V_{yield}) = \sqrt{1 - \alpha} \approx 0.996$; engine default for macroscopic solvers & \kbleaf{vol4/claim-quality.md}; \kbleaf{vol4/.../regimes-of-operation.md}; \kbleaf{constants.py} \texttt{V\_YIELD} \\ % clm-0vxzfu
$E_S$ & $\num{1.323e18}$ & \si{\volt\per\metre} & Ax 4 + Ax 2: Schwinger pair-production critical field; $E_S = V_{snap}/\ell_{node} = m_e^2 c^3/(e\hbar)$ & \kbleaf{vol1/.../dielectric-rupture.md} (QED-anchor + $p_c$ identity); \kbleaf{vol2/.../pair-production-axiom-derivation.md}; \kbleaf{constants.py} \texttt{E\_CRIT} \\ % clm-ezai5b
$E_{yield}$ & $\num{1.130e17}$ & \si{\volt\per\metre} & Ax 4: macroscopic field-onset; $E_{yield} = V_{yield}/\ell_{node} = \sqrt{\alpha}\, E_S$; engine default for FDTD / K4-TLM macroscopic solvers & \kbleaf{vol4/.../regimes-of-operation.md}; \kbleaf{constants.py} \texttt{E\_YIELD} \\ % clm-trgqtf
$B_{snap}$ & $\num{1.892e9}$ & \si{\tesla} & Ax 1 + Ax 4: Cosserat microrotational saturation; $B^2/(2\mu_0) = m_e c^2/\ell_{node}^3$ (energy density = rest energy per cell); $\mu_{eff} \to 0$ (vacuum becomes perfect diamagnet) & \kbleaf{vol1/.../four-regimes.md}; \kbleaf{vol1/.../domain-catalog.md}; \kbleaf{constants.py} \texttt{B\_SNAP} \\ % clm-82dxbj
$T_{melt}$ & $\sim\num{5.93e9}$ & \si{\kelvin} & Ax 4 + thermal-bath: $k_B T \approx m_e c^2$; lightest topological excitation (electron-positron pair) thermally accessible; mode-freezeout regime ends, vacuum heat-capacity falls (Debye-class) & \kbleaf{vol3/.../mode-counting-heat-capacity.md}; \kbleaf{common/temporal-saturation-regime-classifier.md} \\ % clm-uu6dl5
\bottomrule
\end{tabular}
...
\noindent
\emph{Grade disclosure --- the provenance axis is a \textbf{route}, not a confidence.} ``DERIVED'' below says how a line was obtained, not how strongly the corpus stands behind it; those are separate axes and the second one is not in either table. Two of the canonical claims cited in the ratings table above sit at KB build-band \texttt{do-not-build} (``do not build on, rework needed'') and are flagged here rather than left to the reader to look up: \texttt{clm-ezai5b} (the pair-production rupture mechanism behind the $E_S$ row --- an asserted-partial mechanism whose quantitative rate is out of scope at the leaf) and \texttt{clm-uu6dl5} (the mode-counting heat-capacity content behind the $T_{melt}$ row --- not separately measurable, band riding a composite downstream validation). The \emph{numbers} in both rows are unaffected by this note and are unchanged; what is disclosed is the grade of the substrate-mechanism claims they cite.
...
$\bar{\rho}_{cav}$ & per-cell (bulk) & DERIVED ($c_{bulk}=0$ root) --- provisional & --- \\
\bottomrule
\end{tabular}
...
\medskip
\noindent\textbf{$E_S = m_e^2 c^3/(e\hbar) \approx \qty{1.32e18}{\volt\per\metre}$.} The Schwinger pair-production critical field. In standard QED this is the field at which vacuum pair creation becomes non-perturbative; in AVE this is the substrate's full-rupture electric field, identical to $V_{snap}/\ell_{node}$ as required by axiomatic consistency. The substrate mechanism (canonical at \kbleaf{vol2/.../pair-production-axiom-derivation.md}): at $E = E_S$, the local saturation amplitude $A^2 \to 1$ at adjacent A/B K4 nodes, $\Gamma \to -1$\gammaundeclared{} TIR walls form at both nodes, the local phase velocity $c_{local} \to 0$, and the blocked longitudinal kinetic energy shatters sideways into two contra-rotating Beltrami vortices --- the electron-positron pair, each of mass $m_e$. This is \emph{not} the Breit-Wheeler ``virtual photons collide and a pair pops from vacuum'' framing; it is mechanical impedance-wall redirection of linear KE into transverse curl, with parity forcing one LH + one RH outcome. Above $E_S$, additional pair-production events cascade.

\medskip
\noindent\textbf{$B_{snap} = \sqrt{2\mu_0 m_e c^2 / \ell_{node}^3} \approx \qty{1.89e9}{\tesla}$.} The magnetic-rotation node-destruction field, defined by the energy-density balance $B^2/(2\mu_0) = m_e c^2/\ell_{node}^3$ (one electron rest mass per K4 unit cell). Above $B_{snap}$, the Cosserat microrotational sector saturates: the effective permeability $\mu_{eff} \to 0$, the substrate becomes a perfect diamagnet (the Meissner effect of the vacuum itself), and the magnetic-flux topology ruptures. Laboratory magnets ($\sim \qty{10}{\tesla}$) sit at $B/B_{snap} \sim \num{5e-9}$ (deep Regime~I); magnetar surface fields ($\sim \qty{e10}{\tesla}$) reach $B/B_{snap} \sim 5$ (Regime~IV). The B-sector $B_{snap}$ is the dual of the E-sector $E_S$; both are Axiom~4 saturation evaluated at $A = A_{yield}$, in two different sector projections. (This \emph{energy-density-matched} $B_{snap}$ is distinct from the \emph{field-amplitude-matched} duality scale $E_{yield}/c \approx \qty{3.77e8}{\tesla}$ used in birefringence-facility duality arguments; the two answer different questions and sit on the exact, $\alpha$-free bridge $B_{snap}/(E_{yield}/c) = \sqrt{8\pi} = 5.013257$ --- KEEP-BOTH, \kbleaf{vol4/falsification/ch11-experimental-bench-falsification/pvlas-static-b-verdict.md}, \texttt{clm-bdualb}. Do not conflate the two.) \emph{Naming note:} the engineering name $B_{dual}$ for the field-amplitude-matched scale $E_{yield}/c$ is a \textbf{proposed, gated coinage} (\texttt{def-bdual1}, \kbleaf{ave-kb/common/vocabulary-register.md}; \texttt{status: proposed}, verified 0 prior corpus hits, GATED on auditor + Grant per \textsc{invariant-s12} --- a coinage is never seeded SOLID). The \emph{bridge identity} $\sqrt{8\pi}$ (\texttt{clm-bdualb}) is a separately-verified claim; only the noun $B_{dual}$ is pending ratification, so the scale is written here as $E_{yield}/c$ with $B_{dual}$ as the proposed shorthand.

\medskip
\noindent\textbf{$T_{melt} \sim m_e c^2 / k_B \approx \qty{5.93e9}{\kelvin}$.} The pair-production thermal threshold: the temperature at which thermal-bath equipartition energy approaches the lightest topological excitation's rest energy ($m_e c^2$), and electron-positron pairs become spontaneously thermally accessible. Below $T_{melt}$, the vacuum-lattice heat capacity is Dulong-Petit-flat (every node mode equipartitioned); approaching $T_{melt}$, lattice modes begin to freeze \emph{into} pair-production rather than out of it (the reverse-Debye direction at the topological-excitation scale). A stricter operational threshold $T_{pair} = 2 m_e c^2/k_B$ governs thermal decoherence of topological qubits (per \kbleaf{common/temporal-saturation-regime-classifier.md} line 223); $T_{melt}$ as quoted here is the substrate-physics threshold for spontaneous pair-creation, while $T_{pair}$ is the engineering-application threshold for topological-qubit decoherence. Cosmologically, $T_{melt}$ corresponds to the era $t \sim 1$~s after lattice-genesis (universe-cooling crossing the electron-positron annihilation epoch).

\end{resultbox}

\section{Bulk Rarefaction Floor $\bar{\rho}_{cav}$ (provisional)}
...
\begin{warningbox}[$\bar{\rho}_{cav}$ --- provisional]
\textbf{Status: provisional.} The bulk-sector rarefaction floor is recorded here for completeness of the compression/rarefaction limit pair; it is a derived value (see below) carried provisionally pending canonical confirmation.
\end{warningbox}
...
\begin{itemize}
    ...
    \item \textbf{Regime II (Nonlinear)} --- $\sqrt{2\alpha} \leq r < \sqrt{3}/2 \approx 0.866$ ($0.500 < S < 0.993$). Axiom~4 corrections become observable; Euler-Heisenberg analog at the leading quadratic correction. \emph{A genuine vacuum Regime~II reading is facility-class}: the per-node ratio reaches $r \sim 0.7$ only at fields of order \qty{8e16}{\volt\per\metre} (canonical at \kbleaf{vol4/circuit-theory/ch1-vacuum-circuit-analysis/op14-local-clock-modulation.md}). The PONDER-05 dielectric-saturation bench-tester does \emph{not} put the vacuum in Regime~II; its $V_{DC}/V_{yield} = 0.687$ is the quartz \emph{material's} per-node ratio (a consistency-class varactor analog of the saturation-kernel shape), while the vacuum per-node amplitude at that bench is $a \sim 10^{-7}$ --- deep Regime~I (canonical at \kbleaf{vol4/circuit-theory/ch1-vacuum-circuit-analysis/parametric-coupling-kernel.md}).
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\textwidth]{absmax_quarter_arc.pdf}
\caption{The Axiom~4 saturation kernel $S(r) = \sqrt{1 - r^2}$ on the operating-point axis $r = A/A_c$, with the four-regime bands shaded (Regime~I Linear $r < r_1 = \sqrt{2\alpha}$; II Nonlinear; III Avalanche $r_2 = \sqrt{3}/2 \le r < r_3$; IV Ruptured $r \ge r_3 = 1$). The five absolute-maximum ratings (Tab.~\ref{tab:vol9_absmax}) all sit at $r_3 = 1$. The deep-Regime-I markers show the largest currently achievable laboratory ratios ($E/E_S \sim 10^{-8}$, $B/B_{snap} \sim 10^{-8}$, $T/T_{melt} \sim 10^{-3}$; \S\ref{sec:vol9_absmax_table}), all $\gtrsim 8$ orders of magnitude below the rupture boundary. Boundary values derive only from canonical constants via \kbleaf{figures/gen\_absmax\_quarter\_arc.py}; no observable target is hardcoded.}
\label{fig:vol9_absmax_quarter_arc}
\end{figure}

The engineering operating-margin recommendation for any device or substrate-physics measurement is therefore: \textbf{stay in Regime~I unless the experimental design explicitly targets a vacuum Regime~II measurement}. A vacuum Regime~II measurement requires the per-node $V/V_{yield} \gtrsim 0.121$, which at $\ell_{node} = \qty{3.86e-13}{\metre}$ corresponds to local field gradients of order \qty{e16}{\volt\per\metre} sustained over scales below one node spacing --- a facility-class condition, achievable only in resonant cavity geometries with very high $Q$. (The PONDER-05 bench reaches this ratio only in the quartz \emph{material}, not in the vacuum; see \S\ref{sec:vol9_absmax_margins} and \S\ref{sec:vol9_absmax_falsification}.) Regime~III and IV are accessed only by nature (the deep interior of nucleons, neutron stars, black holes, the early universe) and by the autoresonant Schwinger-bypass programme (PROJECT-ZENER-04, AVE-Propulsion Ch.~5, canonical at \kbleaf{vol4/simulation/ch15-autoresonant-breakdown/}).
...
\begin{itemize}
    \item \textbf{PONDER-05 dielectric-saturation bench-tester} --- DC-biased quartz whose $V_{DC}/V_{yield} = 0.687$ is the quartz \emph{material's} per-node ratio. This is a material-scale consistency-class falsifier: it tests the SHAPE of the Axiom~4 saturation kernel via the quartz $C(V)$-arc (a consistency-class varactor analog), not a vacuum operating point. The vacuum per-node amplitude at this bench is $a \sim 10^{-7}$ (deep Regime~I); a genuine vacuum Regime~II reading is facility-class, requiring fields of order \qty{8e16}{\volt\per\metre}. Canonical at \kbleaf{vol4/circuit-theory/ch1-vacuum-circuit-analysis/op14-local-clock-modulation.md} and \kbleaf{vol4/circuit-theory/ch1-vacuum-circuit-analysis/parametric-coupling-kernel.md}. Vol~9 Ch.~15 \emph{Falsification Tests} (Ch.~\ref{ch:vol9_falsification_tests}) carries the full programme.
    \item \textbf{Schwinger pair-production direct test} --- not currently within laboratory reach; ELI-NP and similar ultra-intense laser facilities approach $E/E_S \sim 10^{-3}$. The substrate-physics prediction for the pair-production rate at sub-$E_S$ fields (canonical at \kbleaf{vol2/.../pair-production-axiom-derivation.md}) is consistent with the standard QED Schwinger rate as a leading-order limit, but the AVE substrate-mechanism predicts additional Duffing-autoresonant gating structure that becomes resolvable as $E/E_S \to 1$. Vol~4 PROJECT-ZENER-04 (\kbleaf{vol4/falsification/ch11-experimental-bench-falsification/project-zener-04.md}) is the bench-scale falsifiable proxy via autoresonant rupture-tracking at $E \ll E_S$.
    \item \textbf{Magnetar emission classification} --- the $B \gtrsim B_{snap}$ regime is the astrophysical setting in which the magnetic-saturation limit could be reached. Magnetar surface fields place soft-gamma-repeater, magnetar-burst, and starquake events near $B/B_{snap} \sim O(1)$; AVE substrate-physics predicts distinct emission-spectrum signatures of Cosserat-rotation node destruction vs.~standard-model neutron-star-crust quakes (a forward prediction, provisional and pending experimental confirmation; the distinguishing test, not the corroboration, is the load-bearing claim).
...
%
%   :105  family: cavitation EOS via wave speed  [BIAS-DEBT]
%        QUOTE (content verified at HEAD; markup-reduced from the banked audit): The bulk compressional wave speed obeys $c_{bulk}^2 = c_0^2\,[\,1 +
%        \bar{\rho}/(1 - \bar{\rho}^2)\,]$
%       STAMPED AT: :105
%        RATIONALE (verbatim, research/drivers/r40_sweep_worklist_verified.json): Rarefaction floor
%        rho_cav = -1/phi is derived from a bulk wave-speed law (':98 the bulk (density / compressional)
%        wave speed vanishes and the medium cavitates'); surviving content = a rarefaction stiffness
%        limit of the bound response's constitutive law; stated mechanism is a propagation speed. Already
%        provisional/candidate-class.
