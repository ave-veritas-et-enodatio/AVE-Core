...
The cell's AC response is a 2$\times$2 chiral transfer function. Its co-polarized component is the canonical lossy resonator
\begin{equation}
...
\end{equation}
The pole's distance from the $j\omega$ axis, $\alpha\omega_0/2$, IS the per-cycle radiative leak $1/Q = \alpha$ through the TIR boundary (\kbleaf{vol4/circuit-theory/ch1-vacuum-circuit-analysis/theorem-3-1-q-factor.md}:81, \texttt{clm-rtdmsn}). Bode: peak $20\log_{10}Q = \qty{42.7}{\decibel}$ at $\omega_C$, bandwidth $\Delta\omega = \alpha\omega_0$. The off-diagonal of the full matrix is the parity-odd winding coupling ($S_{LR}\ne S_{RL}^*$); its magnitude is STATED, pending the chiral-crystal engine.
...
\textbf{Scope and classification (\texttt{consistency-vs-emergence}).} The CVR sweep is a \textbf{consolidation / consistency} re-expression of already-derived canon (the LC tank, $\alpha=1/Q$, the magnetic-branch $\Gamma=-1$); it introduces no new substrate primitive. The one AVE-distinct corollary is $|\Gamma|^2=1-\alpha$ (the radiative leak made geometric) and the parity-odd chiral $S$; the $Q=1/\alpha$ Bode peak and the LC$\to E=mc^2$ recovery are CONSISTENCY (re-expressions). \textbf{Carried flags:} the sector-attribution question (magnetic $\mu_{eff}\to0$ PRIMARY vs capacitive $C_{eff}\to\infty$ --- same $Z=Z_0\sqrt S$ trajectory, \texttt{clm-lv3uw1} vs \texttt{clm-kezk9z}); the exponent defect; the chiral-coupling magnitude STATED. The compression-extreme wall here is one end of the lattice extreme-map (\S the phase-diagram chapter; \kbleaf{vol3/cosmology/ch15-black-hole-orbitals/lattice-extreme-bh-rationality.md}).
