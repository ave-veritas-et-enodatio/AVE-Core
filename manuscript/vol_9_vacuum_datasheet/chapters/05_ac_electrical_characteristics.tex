...
Each bond connecting nearest-neighbour K4 nodes carries distributed transmission-line topology with per-cell inductance and capacitance set by the vacuum primitives (Axiom~1; \texttt{translation-circuit.md} \S 4 substrate-primitive-to-EE-component catalog):
\begin{align}
L_{cell} &= \mu_0\, \ell_{node} \\
C_{cell} &= \varepsilon_0\, \ell_{node}
\end{align}
These are the substrate's own canonical EE primitives, not phenomenological fits. $\mu_0$, $\varepsilon_0$, and $\ell_{node}$ are canonical at \texttt{src/ave/core/constants.py:78--206}; the per-cell forms are forced by Axiom~1's K4-node intrinsic-LC-oscillator structure.
...
\end{equation}
Evaluated at the canonical primitives ($c_0 = 2.998\times 10^8$~m/s; $\ell_{node} = \hbar/(m_e c) \approx 3.862\times 10^{-13}$~m):
\begin{align}
\omega_C &\;\approx\; 7.76 \times 10^{20} \text{ rad/s} \\
f_C \;=\; \frac{\omega_C}{2\pi} &\;\approx\; 1.23 \times 10^{20} \text{ Hz}
\end{align}
...
The substrate bond is a distributed transmission-line element. Its characteristic impedance is the canonical substrate-native vacuum impedance (\texttt{translation-circuit.md} \S 4 Op1 anchor):
\begin{equation}
Z_0 \;=\; \sqrt{\frac{\mu_0}{\varepsilon_0}} \;\approx\; 376.73\;\Omega \qquad \text{(canonical at \texttt{src/ave/core/constants.py:81})}
\end{equation}
...
\hline
$Z_0$ & $\sqrt{\mu_0/\varepsilon_0} \approx 376.73$ & $\Omega$ & cold-lattice; $A_0 = 0$; $\omega \ll \omega_C$ & Bond TLM characteristic Z (Axiom~1; \texttt{constants.py:81}) \\
$L_{cell}$ & $\mu_0 \ell_{node} \approx 4.85\times 10^{-19}$ & H & per bond, cold-lattice & Axiom~1 per-cell inductance \\
$C_{cell}$ & $\varepsilon_0 \ell_{node} \approx 3.42\times 10^{-24}$ & F & per bond, cold-lattice & Axiom~1 per-cell capacitance \\
$\omega_C$ & $c_0/\ell_{node} \approx 7.76\times 10^{20}$ & rad/s & cold-lattice & Bond LC natural frequency; substrate Nyquist ceiling \\
$f_C$ & $\omega_C/(2\pi) \approx 1.23\times 10^{20}$ & Hz & cold-lattice & Same in Hz \\
$k_{Brillouin}$ & $\pi/\ell_{node} \approx 8.13\times 10^{12}$ & rad/m & substrate K4 lattice & Substrate Brillouin-zone edge \\
$v_{phase}$ & $c_0$ ($A_0 = 0$) & m/s & cold-lattice; $\omega \ll \omega_C$ & Maxwell phase velocity (cold) \\
$v_{group}$ & $c_0$ ($A_0 = 0$) & m/s & cold-lattice; $\omega \ll \omega_C$ & Group velocity (cold) \\
$\varepsilon_{eff}$ & $\varepsilon_0\, S(A_0)$ & F/m & operating point $A_0$ & INVARIANT-S2 Ax 4 small-signal \\
$\mu_{eff}$ & $\mu_0\, S(A_0)$ & H/m & operating point $A_0$ & INVARIANT-S2 Ax 4 small-signal \\
$C_{eff}$ & $C_0 / S(A_0)$ & F & operating point $A_0$ & Varactor C-vs-V at substrate scale \\
$Z_{eff}$ (Op14) & $Z_0/\sqrt{S(A_0)}$ (ASYM-class) & $\Omega$ & operating point $A_0$, ASYM scaling & Op14 dynamic impedance \\
$c_{EM}(A_0)$ & $c_0 / S(A_0)$ & m/s & operating point $A_0$ & \textbf{Maxwell phase velocity}; enters $\alpha$ \\
$c_{shear}(A_0)$ & $c_0\, \sqrt{S(A_0)}$ & m/s & operating point $A_0$ & \textbf{Mechanical / group / rest-mass velocity}; Op16; Schwarzschild reduction \\
\end{tabular}

\vspace{0.5em}
\noindent
\textbf{Canonical sources.} $\mu_0$, $\varepsilon_0$, $c_0$, $\ell_{node}$, $Z_0$ at \texttt{src/ave/core/constants.py:78--206} via the \texttt{ave-canonical-source} skill. Operating-point small-signal forms at CLAUDE.md INVARIANT-S2 (\texttt{manuscript/ave-kb/CLAUDE.md}). Op14 / Op16 at \texttt{ave-kb/common/operators.md} and \texttt{vol4/circuit-theory/ch1-vacuum-circuit-analysis/op14-local-clock-modulation.md} (\texttt{clm-1eg13f}). $c_{EM}$ / $c_{shear}$ disambiguation at \texttt{clm-8nkvwy} (lines 111 and 113) and \texttt{clm-3zz0f6}.
...
Standard EE AC vocabulary maps to its substrate-physics origin via the EE-as-substrate-native META framework (canonical leaf \texttt{translation-circuit.md} \S 2, \texttt{clm-eemap1}; agent-discipline skill \texttt{ave-ee-first-mapping} v1.0). The substrate is itself an LC network at minimal DOF; EE AC quantities are substrate-native, not analogies.
...
Reactance $X = \omega L_{cell} - 1/(\omega C_{cell})$ & Per-bond reactive response with substrate-native $L_{cell}, C_{cell}$ & Axiom~1; \texttt{translation-circuit.md} \S 4 per-cell row \\
...
Quality factor $Q$ & Substrate Q-factor at the $\Gamma = -1$ saturation boundary; Op21 $Q = \ell$ multi-mode mode-counting & \texttt{vol4/circuit-theory/ch1-vacuum-circuit-analysis/op21-multi-mode-mode-counting.md}; \texttt{theorem-3-1-q-factor.md} \\
