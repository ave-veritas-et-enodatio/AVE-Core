...
The operating-point machinery above (\S\ref{sec:vol9_operating_point}) is the bias; this section is the full small-signal characterization of the substrate cell as a self-made nonlinear resonant cavity --- the \emph{Vacuum Circuit Analysis} (VCA) sweep. The electron mass-cage is a high-$Q$ resonant LC tank closed by a $\Gamma = -1$\gammaundeclared{} wall on the \textbf{A1 longitudinal dilatation} ($Z_{bulk} \to 0 \Rightarrow \Gamma_{bulk} = -1$; canonical \kbleaf{vol1/dynamics/ch4-continuum-electrodynamics/master-equation.md}:95) --- \emph{not} the dielectric-rupture branch ($\varepsilon_{eff}\to0$, $Z\to\infty$, $\Gamma=+1$\gammaundeclared{}, open, a different object). The magnetic branch is the chirality/spin sign-selector, not the cage mechanism; this keeps the cage out of the $T_2$ charge-winding (A1 $\perp$ T2). All six views below are projections of one constitutive spine.
...
\end{equation}
It leaks exactly $\alpha = 1/Q$ per cycle, so the reflected power is $1-\alpha$ and $|\Gamma|$ sits just \emph{inside} the unit circle --- \textbf{the hair by which $\Gamma$ falls short of $|\Gamma|=1$\gammaundeclared{} is $\alpha$}, the fine-structure constant read off the chart. A perfect-conductor boundary gives $|\Gamma|=1$\gammaundeclared{} identically; the substrate wall carries a radiative leak $=\alpha$ built into the reflection --- but at the \emph{value} level $\alpha$ is set by the calibrated $Q$, so this is a re-expression of the $Q=1/\alpha$ calibration identity, not an independent prediction of the number.
...
Bulk (volumetric, $K$) & $c_{bulk} = c_0\sqrt{1 + \bar{\rho}/(1-\bar{\rho}^2)}$ & stiffens at $\bar{\rho}\to+1$; freezes at $\bar{\rho}_{cav} = -1/\varphi$ & compressional / density wave; rarefaction relation (\kbleaf{AVE-Propulsion/.../04\_superluminal\_transit.tex}:86); distinct from shear. \textbf{[DEMOTED 2026-08-11 --- R40-B1; dated demotion note at the end of this chapter]} \\
\bottomrule
\end{tabular}
...
Bulk = compressional & $\tau_{bulk} = \tau_0 / \sqrt{1 + \bar{\rho}/(1-\bar{\rho}^2)}$ & inverse & dilates toward $\bar{\rho}_{cav} = -1/\varphi$ \textbf{[DEMOTED 2026-08-11 --- R40-B1; dated demotion note at the end of this chapter]} \\
...
Quality factor $Q$ & Substrate Q-factor at the $\Gamma = -1$\gammaundeclared{} saturation boundary; Op21 $Q = \ell$ multi-mode mode-counting & \kbleaf{vol4/circuit-theory/ch1-vacuum-circuit-analysis/op21-multi-mode-mode-counting.md}; \kbleaf{theorem-3-1-q-factor.md} \\
...
\begin{itemize}
  \item \textbf{Line~542} (``three distinct wave sectors at three distinct speeds''), realised at the bulk row of the sector-speeds table (line~553, a ``compressional / density wave'' sector at $c_{bulk}$ with the rarefaction speed law) and at the bulk row of the sector-clocks table (line~571, a bulk \emph{transit} clock, tick $= \ell_{node}/c_{sector}$ per line~537). Stamped in place at lines~553 and~571. The bulk \emph{wave sector} and its \emph{transit clock} both require the propagating bulk DOF; the EM and shear rows are untouched, and the constitutive stiffness relation is tabled separately (Ch.~2 / Ch.~14 rows) and is \emph{not} demoted here.
\end{itemize}
...
% --------------------------------------------------------------------------
%   :542  QUOTE: The substrate carries three distinct wave sectors at three distinct speeds
%          STAMPED AT: :553, :571
%          RATIONALE: Table row :553 asserts a 'compressional / density wave' sector at c_bulk with rarefaction
%          speed law, and :571 gives it a clock (tick = l_node/c_sector transit semantics, :537). The bulk
%          WAVE-SECTOR and its transit clock require the propagating DOF; the constitutive stiffness relation
%          is tabled separately (ch02/ch14 rows).
%          ALSO COVERED: none. :553 and :571 are named in the audited row and are the two
%          lines this pass STAMPS (bulk wave-sector row + bulk transit-clock row), so they are
%          not additionally-covered sites; :537 is cited only for the tick = l_node/c_sector
%          semantics and is the GENERAL tau_relax definition, not a bulk-sector claim -- not demoted.
% --------------------------------------------------------------------------
