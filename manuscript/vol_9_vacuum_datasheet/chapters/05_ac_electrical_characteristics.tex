...
\end{align}
These are the substrate's own canonical EE primitives, not phenomenological fits. $\mu_0$, $\varepsilon_0$, and $\ell_{node}$ are canonical at \kbleaf{src/ave/core/constants.py:78--206}; the per-cell forms are forced by Axiom~1's K4-node intrinsic-LC-oscillator structure.
