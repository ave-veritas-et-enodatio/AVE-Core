...
\begin{equation}
Z_0 \;=\; \sqrt{\frac{\mu_0}{\varepsilon_0}} \;\approx\; 376.73\;\Omega \qquad \text{(canonical at \kbleaf{src/ave/core/constants.py:98} \texttt{Z\_0})}
\end{equation}
...
\subsection{Reflection: $|\Gamma|^2 = 1-\alpha$ (instrument-echo scope)}
\label{subsec:vol9_cvr_reflection}

\begin{warningbox}[\texttt{Rule-12} echo-scope --- $|\Gamma|^2=1-\alpha$ is the \emph{instrument echo} of a baked $Q=1/\alpha$, not an independently derived AVE-distinct result]
This subsection previously titled the $|\Gamma|^2 = 1-\alpha$ corollary ``the AVE-distinct result.'' That headline is \textbf{rescoped} (text below preserved per Rule~12). The radiative leak read off the Smith chart is computed from a \textbf{baked} electron $Q = 1/\alpha$ (\kbleaf{src/scripts/vol\_9\_device/cvr\_ee\_sweep/cvr\_model.py}:72, \texttt{Q\_TANK = 1/ALPHA}); the electron-$Q$ value $137$ is \textbf{not independently derived} (the cold-cage FDTD ring-down gives $Q_{\mathrm{ringdown}} \approx 30.8 \neq 137$, and the closed-port eigenframe gives $Q \to \infty$). Per the value-scoped status at \kbleaf{vol4/circuit-theory/ch1-vacuum-circuit-analysis/theorem-3-1-q-factor.md}:21, $\alpha$ stays an \textbf{echo at the value level}: $Q = 1/\alpha$ is a definitional identity that predicts no independent number. Every display computed from that one baked $Q$ (the Smith-gap $1-\alpha$, the Bode peak, the $S_{21}$ notch) reads the same $137$ --- plotting more instruments does not escape the echo. See the three-reading cross-pointer below (mirroring Ch.~\ref{ch:vol9_pin_port_configuration} \S Build-A isolation eigensolver).
\end{warningbox}
...
\end{equation}
It leaks exactly $\alpha = 1/Q$ per cycle, so the reflected power is $1-\alpha$ and $|\Gamma|$ sits just \emph{inside} the unit circle --- \textbf{the hair by which $\Gamma$ falls short of $|\Gamma|=1$ is $\alpha$}, the fine-structure constant read off the chart. A perfect-conductor QED boundary gives $|\Gamma|=1$ identically; the substrate wall carries a radiative leak $=\alpha$ built into the reflection --- but at the \emph{value} level $\alpha$ rides the baked $Q$, so this is the instrument echo of the $Q=1/\alpha$ identity, not an independent prediction of the number.

\medskip\noindent
\textbf{The electron-$Q$ is not independently derived (three-reading cross-pointer).} The $\alpha^{-1} = 137$ value the displays above all inherit carries the same three canonical readings recorded for the Build-A isolation eigensolver (Ch.~\ref{ch:vol9_pin_port_configuration} \S Build-A; full content at \kbleaf{vol2/particle-physics/ch01-topological-matter/electron-bound-resonator-coverage.md} and \kbleaf{vol4/circuit-theory/ch1-vacuum-circuit-analysis/theorem-3-1-q-factor.md}):
\begin{itemize}
  ...
  \item \textbf{$\alpha^{-1}=137$ is the loaded/radiative $Q$, not the intrinsic resonator $Q$.} Since the intrinsic mode is lossless, $137$ is the coupling out through the EM port ($|\Gamma_{\mathrm{EM}}|^2=1-\alpha$, a fraction $\alpha$ leaking per bounce) --- a coupling coefficient, not a lifetime. This retracts no prior; $\alpha$ stays an echo at the value level and derivability of the loaded-$Q$ value remains open.
  \item \textbf{$\alpha$ stays echo --- the cold-cage $Q\approx30.8\neq137$ and the eigenframe $Q\to\infty$ both stand as reported rows.} No $\alpha$ echo$\to$chord promotion; the re-posed ``derive the loaded $Q$'' test is circular (the engine leak is literally \texttt{1.0-alpha}), so no new slot is minted.
\end{itemize}
...
\noindent
\textbf{Per-DOF dispersion split --- $(q\ell_{node})^2$ matter vs $(q\ell_{node})^4$ photon (figure: Phase-C create).} The per-DOF node constitutive layer beneath the scalar cell (each translation DOF an $(L_i,C_i)$ reactance) produces the dispersion tell that distinguishes the matter channel ($\propto (q\ell_{node})^2$) from the photon channel ($\propto (q\ell_{node})^4$); the demo \kbleaf{src/scripts/vol\_9\_device/per\_dof\_node\_dispersion\_demo.py} computes the three regimes (isotropic / deviatoric / high-$k$) and emits them as structured data (\texttt{\_output/per\_dof\_node\_dispersion\_demo.json}). It does \textbf{not} currently emit a plot, so the dispersion-split figure is a \textbf{Phase-C create} (a new plotting pass on the existing demo output), not wired in this Phase-A overhaul. The constitutive layer is documented at \kbleaf{ave-kb/vol9/ch3-pin-port-configuration/per-dof-vacuum-node-circuit.md}.
...
\midrule
$Z_0$ & $\sqrt{\mu_0/\varepsilon_0} \approx \num{376.73}$ & \si{\ohm} & cold-lattice; $A_0 = 0$; $\omega \ll \omega_C$ & Bond TLM characteristic Z (Axiom~1; \texttt{constants.py:98} \texttt{Z\_0}) \\
