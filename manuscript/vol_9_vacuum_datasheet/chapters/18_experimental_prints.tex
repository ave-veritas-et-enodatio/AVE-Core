...
\begin{warningbox}[PROVISIONAL / WORK-IN-PROGRESS --- kit STLs are not yet print/mate-validated]
...
\end{enumerate}
Mesh remediation is tracked. Treat the kit as a topology preview, not print-ready hardware.
\end{warningbox}
...
\begin{enumerate}
    ...
    \item \textbf{Joinery:} press each bond barbell peg into the hollow socket at the matching port collar mouth (friction-fit; 0.05\,mm/side radial interference is the \emph{design target} --- the shipped WIP bond OD equals the socket bore, so actual interference is $0\,\mathrm{mm}$; see PROVISIONAL banner).
