...
Run:  PYTHONPATH=src ./.venv/bin/python gen_bankable_falsification_windows.py
Output: bankable_falsification_windows.{pdf,png}  (vector + raster, house style).
"""
...
# central BH
ax.add_patch(plt.Circle((0, 0), R_S, color=style.COLORS["data"], zorder=5))
ax.text(0, 0, "BH", color="white", ha="center", va="center", fontsize=8, zorder=6)

# characteristic radii as circles
circ_specs = [
    (R_PHOTON, style.COLORS["muted"], "--", r"photon sphere $3GM/c^2$ (GR$\equiv$AVE)"),
    (R_ISCO_GR, style.COLORS["comparison"], "-", r"GR ISCO $6GM/c^2$"),
    (R_SAT_AVE, style.COLORS["ave"], "-", r"AVE $r_{\mathrm{sat}}=7GM/c^2$ ($=3.5\,r_s$)"),
]
...
ax.fill(
    ...
    color=style.COLORS["ave"], alpha=0.13, zorder=2,
)
ax.annotate(
    "disk-edge\nwindow\n$\\Delta r = GM/c^2$",
    xy=(0, 6.5), xytext=(4.6, 6.6), fontsize=8, ha="left",
    arrowprops=dict(arrowstyle="->", color=style.COLORS["ave"], lw=1.0),
)
ax.set_xlabel(style.axis_label("Radius", "r", "GM/c^2"))
ax.set_ylabel(style.axis_label("Radius", "r", "GM/c^2"))
...
ax.annotate(
    ...
    arrowprops=dict(arrowstyle="<->", color=style.COLORS["muted"], lw=1.2),
)
ax.text(
    0.18, (G_STAR_AVE + G_STAR_SM) / 2,
    f"$\\Delta g_*={G_STAR_SM - G_STAR_AVE:.2f}$\n(24 fewer\nfermionic DOF)",
    fontsize=7.6, va="center", ha="left",
)
ax.set_ylim(0, 122)
ax.set_ylabel(style.axis_label("Effective DOF", "g_*", ""))
...
out = Path(__file__).resolve().parent / "bankable_falsification_windows"
written = style.save(fig, out)
print(f"[fig] wrote {[str(p) for p in written]}")
