...
\begin{circuitikz}[american, scale=0.85, transform shape]
    % --- The node LC tank (shared) ---
    \draw (0,0) node[left] {node} to[short, o-] (1,0);
    \draw (1,0) to[L, l=$L_{\mathrm{eff}}(I)$, i>=$I$] (3.5,0);
    \draw (3.5,0) to[short] (5.0,0);
    \draw (4.25,0) to[C, l=$C_{\mathrm{eff}}(V)$, *-] (4.25,-1.6) node[ground] {};
    \draw (5.0,0) to[short, -o] (6.0,0) node[right] {};
    % grade labels
    \node[align=center, font=\scriptsize] at (2.25, 0.95) {$\mu$-grade: keyed on $I$};
    \node[align=center, font=\scriptsize] at (4.25, 1.25) {$\varepsilon$-grade: keyed on $V$};
\end{circuitikz}
