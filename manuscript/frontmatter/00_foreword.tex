...
\vspace{0.5em}
\noindent \textbf{The combined construct.} Taken together, the three threads motivate a single model: a discrete medium that is simultaneously a Cosserat micropolar solid~\cite{cosserat1909deformables} and a network of LC oscillators --- the \textbf{chiral Laves K4 Cosserat crystal} $\mathcal{M}_A$ (also called the Chiral SRS Net), with long-range $I4_1 32$ chiral space-group order. Each node is \textbf{micropolar}, carrying \textbf{six intrinsic degrees of freedom}: three translational (capacitive coupling $\varepsilon_0$, identified with the electric field $\mathbf{E}$) $\perp$ three microrotational (inductive coupling $\mu_0$, identified with the magnetic field $\mathbf{B}$); the translation $\leftrightarrow$ E and rotation $\leftrightarrow$ B couplings make each node an LC oscillator. The scalar/longitudinal channel and the micro-rotation winding are kept distinct: the former carries mass (the A1 sector), the latter carries charge (the T2 sector); they are separate objects (A1 $\perp$ T2, canonical at \kbleaf{vol1/dynamics/ch4-continuum-electrodynamics/master-equation.md}), with the electron modelled as the unknot dilatation-mass carrying the $(2,3)$ winding.
