...
Historically, the framework was presented as a single-parameter EFT (taking $\alpha$ as the one empirical input after $\ell_{node}$ is identified with the electron's Compton wavelength). The closure is at \textbf{Class B substrate-mechanism manifestation level}: $\alpha$ is expressed in closed form by the Golden Torus geometry of the electron soliton (real-space unknot $0_1$ with $(2,3)$ phase-space Clifford-torus winding pattern; Ch.~\ref{ch:alpha_golden_torus}, \kbleaf{vol1/ch8-alpha-golden-torus.md}), $\alpha^{-1}_{\text{ideal}} = 4\pi^3 + \pi^2 + \pi$, with a CMB-induced thermal strain coefficient $\delta_{\text{strain}}$ accounting for the $\sim 2 \times 10^{-6}$ residual to the CODATA value. The value's scale ($\sim 1/137$) is forced by the Compton-resonance trapping condition, while its exact value rests on the one substrate-geometric identification ($R \cdot r = 1/4$) that the substrate does not independently select --- a named identification, not a first-principles derivation and not a zero-parameter closure. The framework is thus a Class B substrate-mechanism manifestation zero-parameter-\emph{target} EFT: all 26 Standard Model constants follow from four axioms + the topological requirement that the smallest stable soliton is the unknot $0_1$ + the named substrate-canonical identification from the bond LC tank.
