...
Historically, the framework was presented as a single-parameter EFT (taking $\alpha$ as the one empirical input after $\ell_{node}$ is identified with the electron's Compton wavelength). The zero-parameter closure is \textbf{proposed but contingent on the open Clifford-torus embedding-selection item} (zero-parameter contingent on the open embedding-selection item --- NOT the half-cover chain, which is substrate-derived; see Ch.~\ref{ch:alpha_golden_torus} chapter-head gating note + \kbleaf{vol1/ch8-alpha-golden-torus.md} \S``Remaining open formal-rigor sub-item (THE gating item)''): $\alpha$ itself is derived from the S$_{11}$-minimum Golden Torus geometry of the electron soliton (real-space unknot $0_1$ with $(2,3)$ phase-space Clifford-torus winding pattern; Ch.~\ref{ch:alpha_golden_torus}), $\alpha^{-1}_{\text{ideal}} = 4\pi^3 + \pi^2 + \pi$, with a CMB-induced thermal strain coefficient $\delta_{\text{strain}}$ accounting for the $\sim 2 \times 10^{-6}$ residual to the CODATA value. With the embedding-selection step closed, the framework would be a \textbf{zero-parameter} EFT: all 26 Standard Model constants follow from four axioms plus the topological requirement that the smallest stable soliton is the unknot $0_1$. Until that step closes from the classical axioms alone, $\alpha$ operationally remains the framework's one calibration input.
