...
% 🔴 [VALUE REFRESH 2026-08-02 -- manuscript-side propagation of the KB value-refresh 2026-07-13.
%  Rule 12: prior wording PRESERVED here byte-verbatim and in git, NOT deleted:
%    "evaluates the 1D scalar trace to $\mathcal{I}_{scalar}^{(cold)} \approx 1185\,m_e$, yielding a
%     proton ratio of $\approx 1872$ (approximately $2\%$ above the empirical value)"
%  That triplet is the PRE-Ax4-saturation-refactor result. Authority (byte-mirror of the sentence
%  below): ave-kb/vol2/particle-physics/ch02-baryon-sector/thermal-softening.md:11, with the dated
%  strike banner at :13 ("Live solver ... gives $\mathcal{I}_{scalar}^{(cold)} = 1170.59\,m_e \to$
%  ratio $1849.70$ ($+0.7377\%$ vs CODATA $1836.153$)"); cross-confirmed at
%  ch02-baryon-sector/proton-identification.md:13. NO new value is minted here -- every digit below
%  is transcribed from the leaf. This also removes a self-contradiction with :219 of this chapter,
%  which already prints the corrected "+0.74% with zero baryon input".]
...
\begin{enumerate}
    % 🔴 [RELABEL + PROVENANCE 2026-08-02 (CRIB-1). This is the VOLUME'S FIRST use of the symbol, so
    %  the definitional clause lands here. Rule 12 -- prior wording preserved byte-verbatim and in
    %  git: "\item $\nu_{vac} = 2/7$ --- the Poisson ratio of the chiral LC lattice, which sets the
    %  anharmonic Gr{\"u}neisen parameter ...". Two things were wrong with the old form: the SYMBOL
    %  (the corpus relabelled it nu_Hill in #589 -- engine src/ave/core/constants.py:397 "N_NU ...
    %  nu_Hill (Poisson ratio, Axiom 2; isotropic V-R-H avg at GR-imported K=2G)" and :626
    %  "NU_VAC ... nu_Hill (isotropic Voigt-Reuss-Hill average at K=2G)"; KB theorem-thesaurus.md
    %  :227 and srs-band-structure.md throughout) and the ATTRIBUTION ("of the chiral LC lattice"
    %  reads lattice-emergent, which theorem-thesaurus.md:228 explicitly denies: the Voigt-Reuss-Hill
    %  average is "★**Not a theorem and not a bound**"). VALUE UNCHANGED at 2/7 -- this is a naming
    %  and provenance correction only, with zero numerical consequence anywhere in the volume.
    %  Naming status re-verified at HEAD: theorem-thesaurus.md:132 carried an AUDIT-FLAG on the
    %  Voigt/Reuss classification, and that flag is DISCHARGED at :133 by "★RULING LANDED --
    %  2026-08-01, Grant ruling D3". The residual discipline is the homonym caveat at :227 ("Hill --
    %  three distinct objects"), which the printed clause below carries.]
    \item $\nu_{\text{Hill}} = 2/7$ --- the vacuum Poisson ratio at the $K = 2G$ operating point
    (form-derived; \emph{value} GR-imported, see
    \kbleaf{ave-kb/common/form-deriving-value-importing.md}), which sets the anharmonic
    Gr{\"u}neisen parameter governing the coupling between thermal fluctuations and elastic
    stiffness. \emph{Naming, used throughout this volume:} the subscript is ``Hill'' because $2/7$
    is an \textbf{isotropic Voigt--Reuss--Hill average} --- an averaging \emph{choice}, not a
    lattice-emergent bound and not a theorem. Do not read it as the Hill (1963) apparent-modulus
    \emph{ordering}, which is a different object sharing the name
    (\kbleaf{ave-kb/common/theorem-thesaurus.md}, ``Hill --- three distinct objects'').
...
The product evaluates to $\delta_{th} = \frac{\nu_{\text{Hill}}}{\kappa_{cold}} \times \frac{2}{\pi} = \frac{2/7}{8\pi} \times \frac{2}{\pi} = \frac{1}{14\pi^2} \approx 0.00721$. Together with the gradient saturation inside the energy functional, this produces proton mass agreement to better than $0.1\%$ and covers the baryon ladder through crossing number $c = 15$ with maximum error $\sim 2.4\%$. \emph{Read ``covers'' as consistency, not ensemble-validation} (scope correction 2026-06-19, see the section-level note below the prediction table): the load-bearing positive is the single proton hit + the curved ladder FORM, not a six-state forward-validation.

\begin{figure}[h]
    \centering
    \includegraphics[width=1.0\textwidth]{thermal_skyrmion_comparison.png}
    % 🔴 [VALUE REFRESH 2026-08-02 -- companion to the :81 refresh; same KB authority. Rule 12:
    %  prior caption wording PRESERVED byte-verbatim and in git, NOT deleted:
    %    "the radial energy integrand $r^2\mathcal{E}(r)$ decreases by approximately 2\%, shifting
    %     $\mathcal{I}_{scalar}$ from $\sim 1185\,m_e$ to $\sim 1160\,m_e$."
    %  Corrected endpoints are transcribed from the KB, not minted here: the COLD trace
    %  $1170.59\,m_e$ (thermal-softening.md:11,:13) and the thermally-CORRECTED trace
    %  $\approx 1162\,m_e$ (proton-identification.md:68; src/ave/core/constants.py I_SCALAR_1D =
    %  1161.987). The standalone "approximately 2\%" figure is STRUCK rather than replaced -- it is
    %  a derived quantity of the two endpoints and no KB leaf carries a refreshed percentage; the
    %  ratio is recoverable from the two printed endpoints.
    %  ⚠ FLAG-DON'T-FIX (owed, NOT executed in this lane): the plotted PNG
    %  (thermal_skyrmion_comparison.png, generator src/scripts/vol_2_subatomic/plot_thermal_skyrmion.py)
    %  is itself PRE-refresh -- the curves it draws are the 1185/1160 pair. Regenerating the figure
    %  is a driver run, outside a manuscript-reconciliation lane. Until it is regenerated the caption
    %  is correct and the artwork is stale; this is disclosed rather than silently reconciled.]
    \caption{Cold ($\kappa = 8\pi$) vs.\ thermally corrected ($\kappa_{eff} = 8\pi(1-\delta_{th})$) Skyrmion profiles. Left: the hedgehog profile $f(r)$ broadens slightly under thermal softening. Right: the radial energy integrand $r^2\mathcal{E}(r)$ decreases under the softening, shifting $\mathcal{I}_{scalar}$ from the cold trace $\approx 1170.6\,m_e$ to the thermally corrected trace $\approx 1162\,m_e$. \emph{(Artwork pre-dates the 2026-07-13 value refresh and is queued for regeneration; the numbers quoted here are the current solver values.)}}
