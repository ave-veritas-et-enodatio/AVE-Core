...
is computational (run via \kbleaf{src/ave/axioms/yang\_mills.py::verify\_osterwalder\_schrader}),
...
\begin{itemize}
    ...
    \item \textbf{Proof.}  Translational invariance of the vacuum
          ...
          electron's $(2,3)$ Clifford-torus winding pattern (phase-space
          quantization layer; real-space ontology is unknot $0_1$ per Vol 1
          Ch~\ref{ch:alpha_golden_torus}) winds by angle $\pi$ per crossing.
          Exchanging two such windings accumulates $e^{i\pi} = -1$:
          \begin{equation}
...
\begin{itemize}
    ...
    \item \textbf{Proof.}  At the knot boundary, $\Gamma = -1$\gammaundeclared{} (total
...
constant in Appendix~\ref{app:derived_topological_numerology}.
