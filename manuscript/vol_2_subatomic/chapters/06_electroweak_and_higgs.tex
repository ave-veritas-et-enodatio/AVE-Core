...
\begin{equation}
    C_2 = -\frac{197}{144} + \frac{\pi^2}{12} - \frac{3\,\zeta(3)}{4} + \frac{\pi^2 \ln 2}{2} = -0.328\,479\ldots
\end{equation}

\paragraph{AVE mapping.}
In the QED framework, $C_2$ arises from one-loop photon propagator insertions (vertex and vacuum-polarisation diagrams). On the AVE lattice, the isomorphic computation is the \textbf{K4 lattice Green's function at two-loop order}:

\begin{itemize}
    \item The first-order $\alpha/(2\pi)$ is the \emph{on-site} capacitive correction (single node visited by the unknot).
    \item The second-order arises from the perturbation propagating to \emph{nearest-neighbor} nodes and back. On the K4 unit cell, each vertex has 3 neighbors. The round-trip coupling through the Poisson ratio ($\nu_{\text{vac}} = 2/7$) attenuates each path by $\nu_{\text{vac}}^2 = 4/49$.
    \item The transcendentals $\pi^2$, $\zeta(3)$, and $\ln 2$ are expected to emerge from the lattice propagator sums over all two-hop paths on the infinite K4 tiling, with the $\zeta(3)$ term arising from the 3D lattice sum convergence.
\end{itemize}

\paragraph{Current status.}
The na\"ive single-path Poisson estimate gives $C_2^{\text{na\"ive}} = -6/(49\pi) \approx -0.039$, roughly $8\times$ too small. This indicates that multi-path interference on the complete graph (which is absent from a simple round-trip estimate) contributes the dominant fraction of $C_2$. Computing the exact lattice propagator on the K4 tiling is a well-defined computational target---equivalent to evaluating the discrete Green's function of a 3+1D nearest-neighbor graph at two-loop order. This is documented as an open research problem.
