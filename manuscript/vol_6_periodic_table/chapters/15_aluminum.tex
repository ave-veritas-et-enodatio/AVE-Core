...
The empirical CODATA nuclear mass for Aluminum-27 is $25126.501$ MeV. This mass is the input; the solver fits the single scalar halo distance $R_{halo}$ that reproduces it within the $(Z,A)$-forced $6\alpha + T$ topology (the topology is zero-parameter; the halo distance is the one fitted degree of freedom).

The fit converges to a tolerance below $0.000001\%$ on $R_{halo}$; the Tritium halo bonds to the primary $Z$-axis (North Pole) Alpha centroid of the underlying Octahedron, bounding outward to $R_{halo} = 52.6d$. Aluminum-27 is a core-plus-halo fit nucleus, not one of the closed-shell exact-geometric-identity cases; its end-to-end topological mass ($25132.926$ MeV) agrees with CODATA to $0.02557\%$ (see the summary table, Ch.~\ref{ch:summary}, and the Scope note in \S\ref{box:internal_peer_scope}).
...
\begin{figure}[htbp]
    ...
    \caption{The Aluminum-27 topology rendered across the $X-Z$ plane. The $6\alpha$ Octahedral core pushes the Tritium array up the $Z$-axis. The visual maps the asymmetric moment created by the $52.6d$ ($R_{\text{halo}} = 52.605d$) offset gap.}
...
\begin{figure}[htbp]
    ...
    \caption{The Aluminum-27 SPICE architecture. The 351 unique dynamic parameters couple the core 24 nucleon array linearly into the 3 discrete nodes of the polar Tritium halo.}
...
Aluminum-27 ($6\alpha + ^3\text{H}$) is the third core-plus-halo element. The semiconductor engine fixes the Magnesium-24 octahedral core at $R_{\text{core}} = 78.0\,d$ (Small Signal, $V_R/V_{BR} = 0.040$, $M = 1$), then fits the single scalar Tritium halo distance: $R_{\text{halo}} = 52.605\,d$. The $0.000\,000\%$ figure is the fit-convergence tolerance on $R_{\text{halo}}$, not the mass-prediction error---Aluminum-27 is a core-plus-halo fit nucleus (NOT in the closed-shell exact set), and its end-to-end topological mass agrees with the CODATA $25\,126.501$ MeV to the $0.02557\%$ recorded in the summary table.

The Period-3 halo evolution runs $R_{\text{halo}} = 398d$ (Fluorine) $\to$ $50d$ (Sodium) $\to$ $53d$ (Aluminum). As the core grows from $4\alpha$ to $5\alpha$ to $6\alpha$, the halo contracts dramatically and then stabilizes. The Sodium-to-Aluminum shift of only $2.4d$ indicates that the $5\alpha \to 6\alpha$ core expansion has negligible effect on halo proximity---the octahedral core is already dense enough to saturate the core-halo attraction. This geometric plateau explains why post-transition metals (Al, Ga, In) share moderate, similar electronegativities rather than exhibiting the extreme variation seen across Halogens and Alkali metals.
