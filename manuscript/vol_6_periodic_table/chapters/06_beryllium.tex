...
\end{equation}
\end{resultbox}
\noindent This resolves the prior $-7.1\%$ residual with zero fit constants \emph{given the standard filling order} ($1s^2\,2s^2$), which AVE asserts as an input rather than derives.  The correction is gated to $n_\text{adjacent} = 1$ (pure $s$-shell inner core), ensuring it does not fire for period~3 atoms where the SIR boundary model applies (see Vol.~2).  The quoted accuracy is verified against the engine's reference values under the pinned solver snapshot; it is not asserted as a NIST-validated prediction.
