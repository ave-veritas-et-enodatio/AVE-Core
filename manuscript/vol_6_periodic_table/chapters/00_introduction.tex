...
% [2026-06-15 KB-reconciliation (vol_6 brief §A.4, parallel-site of §D.1) -- prior wording preserved per Rule 12]: read "it converges to $0.0001\%$ error using the axiom-derived coupling constant $K$ and no empirical fit." SUPERSEDED: "no empirical fit" contradicts this file's own methodology note (:21) + :58 -- Cr-52 is solved at its own per-nucleus fitted R like every nucleus; the 0.0001% is a fitting tolerance on R, not an ab-initio error. This paragraph was missed when the chapter was hardened at :18/:21/:58 (byte-identical twin executive-abstract.md:46). K being axiom-derived is unchanged.
When the semiconductor engine solves Chromium-52 as a centered icosahedron (12 icosahedral vertices + 1 central alpha), it converges to a $0.0001\%$ fitting tolerance using the axiom-derived coupling constant $K$, with the inter-alpha distance $R$ fit per nucleus (methodology note, \S above). The Fibonacci sequence ($1, 1, 2, 3, 5, 8, 13, \ldots$), whose consecutive ratios converge to $\varphi$, is therefore not an injected mathematical artifact but an emergent structural constant of the nuclear topology at the 13-alpha shell. The ``Fibonacci lattice'' used as a numerical proxy in the heavy element catalog is an accidental approximation of what the actual icosahedral ground-state geometry requires.
