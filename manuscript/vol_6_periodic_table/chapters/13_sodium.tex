...
\begin{itemize}
    ...
    \item Understand how the semiconductor engine maps the alkali-metal energetics to a single mechanical variable: $R_{\text{halo}}$.
\end{itemize}
\end{objectivebox}


Sodium-23 ($Z=11, A=23$) begins the Alkali Metal series following the balanced Neon-20 ($5\alpha$) noble gas. The structural geometry of Sodium illustrates the framework's mechanism cleanly: Sodium-23 is geometrically nearly identical to Fluorine-19, yet sits chemically on the exact opposite side of the periodic table (this internal structural mapping is peer-with-standard chemistry, not a distinct prediction; see the Scope note in \S\ref{box:internal_peer_scope}). The contrast resolves through a single mechanical variable, as the topological paradox below shows.
...
\section{The Core Proximity Effect ($398d$ vs $50d$)}
...
\begin{itemize}
    \item \textbf{Fluorine-19 ($R_{halo} = 398d$):} The $4\alpha$ core is relatively small and weakly inductive. The Tritium halo is violently repelled far out into space, creating a massive, unstable $\sim335$ fm reactive whip. Thus, Fluorine acts as a profound receiver.
...
\begin{figure}[htbp]
    ...
    \caption{The topological vacuum flux slice for Sodium ($^{23}_{11}\text{Na}$) highlighting the polar geometric offset. The highly inductive Neon-20 ($5\alpha$) Bipyramid core pulls the Tritium array in to a compact $50.171d$ radius, differentiating its chemical reactivity from the Fluorine $398d$ geometry.}
...
Sodium-23 ($5\alpha + ^3\text{H}$) is the second core-plus-halo element after Fluorine-19. The semiconductor engine fixes the Neon-20 bipyramidal core at $R_{\text{core}} = 81.158\,d$ (Small Signal, $V_R/V_{BR} = 0.032$, $M = 1$), then solves for the single fitted scalar---the Tritium halo distance---that reproduces the CODATA mass of $21\,409.214$ MeV. The result is $R_{\text{halo}} = 50.171\,d$; the $0.000\,000\%$ is the optimizer's convergence tolerance on $R_{\text{halo}}$, not an ab-initio mass-prediction error. (Sodium-23 is a core-plus-halo fit nucleus; its end-to-end mass residual against CODATA is the $0.02436\%$ reported in the summary table, Ch.~\ref{ch:summary}.)

The chemical paradox resolved: Sodium and Fluorine share the same structural template ($n\alpha + ^3\text{H}$), yet their $R_{\text{halo}}$ values differ by a factor of $8\times$ ($398d$ vs $50d$). Fluorine's weak $4\alpha$ core repels the halo to an enormous distance, creating a reactive whip. Sodium's dense $5\alpha$ core binds the halo inward to a tight $50d$ lock, producing a compact, rigid bulge. This is why Sodium strips its outer electron easily (low ionization energy) but never aggressively seeks additional bonds (low electronegativity). Within the framework, the alkali-metal energetics map to a single mechanical variable: $R_{\text{halo}}$.
...
\begin{itemize}
    ...
    \item Within the framework, the alkali-metal energetics map to a single mechanical variable: $R_{\text{halo}}$.
\end{itemize}
\end{summarybox}
