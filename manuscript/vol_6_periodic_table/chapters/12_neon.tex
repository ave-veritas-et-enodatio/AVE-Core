...
\begin{figure}[h]
    ...
    % [2026-06-15 KB-reconciliation (vol_6 brief §D.2/§A.4) -- prior wording preserved per Rule 12]: read "These geometries are solved from the CODATA mass target with zero free parameters---the engine finds the unique radius at which the pairwise $\sum K/d_{ij}$ sum reproduces the empirical mass defect." SUPERSEDED: self-contradictory in the same sentence -- "the unique radius" the engine finds IS the fitted scalar R (R_bipyramid=81.181d at :23; the chapter's own "Addressing the Curve-Fitting Fallacy" section :26 states R is "tuned until the math matches the empirical mass limit"). Topology is zero-parameter; R is the one fitted DOF. Aligns the caption with 00_introduction.tex:21 + the chapter body.
