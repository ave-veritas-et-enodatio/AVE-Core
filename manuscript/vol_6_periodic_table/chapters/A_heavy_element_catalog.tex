...
\begin{objectivebox}
\begin{itemize}
    \item Apply the semiconductor avalanche binding model to all heavy elements ($Z = 15$ to $Z = 119$) with no additional empirical constants beyond the light-element framework---the per-nucleus inter-alpha distance $R$ remains the one fitted scalar, and the Fibonacci-proxy elements carry two global packing coefficients.
    \item Identify Iron-56 as the minimum mass-per-nucleon (the fusion endpoint) and understand the superheavy stability margin.
\end{itemize}
\end{objectivebox}
...
By substituting the empirical term $d$ with the exact topological derivation of the proton gyroscopic spin radius ($d = 4\hbar/m_pc \approx 0.841\text{ fm}$), this engine uses \textbf{no empirical mass-fitting constants}: the coupling constant ($K$), Coulomb strength ($\alpha\hbar c$), breakdown capacity ($V_{BR}$), avalanche exponent ($n=5$), and internal charge radius ($d$) are all derived rigidly from the topological graph structure. The one fitted geometric degree of freedom is the per-nucleus inter-alpha distance $R$ (``the operational $R$'' identified by the solver below); the Fibonacci-proxy elements additionally carry the two empirical packing coefficients of $r_{core}=d(15+0.95A)$ (see \S\ref{sec:high_z_frontier}).

As discussed in Section~\ref{sub:R_coupled_cavity}, the inter-alpha distance $R$ is determined by the topology acting as a coupled cavity resonator. The numerical solver identifies the operational $R$ by mapping the structural geometry to the standing wave wavelength of the required kinetic binding energy. Elements such as Sulfur-32 ($M = 32.8$), Calcium-40 ($M = 32.9$), Gallium ($M > 100$), Germanium ($M > 100$), and Arsenic ($M > 100$) explicitly require the non-linear \textbf{Large Signal regime}. Entries marked with residual error $> 0.01\%$ use the spherical Fibonacci packing model (a geometric proxy); exact Platonic/Archimedean solutions are not yet tabulated for these.
...
% [2026-08-02 NON-CLAIM SCOPE PROPAGATION -- clm-nk6c43. ADDITIVE ONLY: nothing in the paragraph
%  above is retracted or reworded; its per-tier breakout and its word "typically" already match
%  the claim record. What was absent is the claim record's stated MAXIMUM. Authority, verbatim,
%  from manuscript/ave-kb/vol6/claim-quality.md:122 (clm-nk6c43 "Heavy Element Catalog
%  ($Z=15$-$119$): Accuracy Tiers, Not a Uniform Bound", solidity 0.50, build_status
%  "use as input only, don't build deeper"), under _Specific Non-Claims and Caveats_:
%    "Does NOT claim \"$< 0.5\%$ across 105 elements\" is a uniform predictive accuracy.
%     Individual-element errors range from 0.000% (Tier A) up to 1.465% (Cl-35, in the table)
%     and 0.898% (Zn-65) and 0.849% (Rb-85). The \"$< 0.5\%$\" headline is a TYPICAL, not a
%     maximum. Several Tier-C entries exceed it."
%  and :125 -- "Do not present a global \"AVE achieves $< 0.5\%$ across $Z=15$-$119$\" claim
%  without per-tier breakout."
%  All three maxima were re-verified against THIS chapter's own full element table (Chlorine (Cl)
%  35 = 1.465%; Zinc (Zn) 65 = 0.898%; Rubidium (Rb) 85 = 0.849%) -- no number is imported.
%  BOARD-STATE FLAG: the section-5 disposition tag for this site is [REFUTED -- dropped], and the
%  refutation is correct on its own limb (the paragraph DOES carry the per-tier breakout and DOES
%  say "typically"). This sentence is executed under the wave-2 dispatch's explicit EXECUTE
%  instruction for the clm-nk6c43 non-claim scope and is strictly ADDITIVE, so it does not
%  resurrect the refuted limb. Surfaced for orchestrator adjudication, not resolved in-lane.]
The ``typically below $0.5\%$'' figure is a \emph{typical}, not a maximum: several Fibonacci-proxy entries exceed it. The largest residuals in the table below are Chlorine-35 at $1.465\%$, Zinc-65 at $0.898\%$, and Rubidium-85 at $0.849\%$. Accordingly no global ``$<0.5\%$ across $Z=15$--$119$'' accuracy claim is made here without the per-tier breakout above (\kbleaf{manuscript/ave-kb/vol6/claim-quality.md} \texttt{clm-nk6c43}, solidity $0.50$, ``use as input only, don't build deeper'').

\begin{figure}[h]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/mass_error_vs_Z.png}
    \caption{\textbf{AVE Mass Prediction Accuracy vs.\ Atomic Number.} Green markers indicate exact semiconductor engine solutions ($0.000\%$ error). Orange: Large Signal (avalanche) regime. Cyan: sub-$0.1\%$ Fibonacci packing solutions. Yellow: $0.1$--$0.5\%$ residual. Red: $>0.5\%$ error, Fibonacci-proxy entries without a tabulated exact geometry.}
...
\begin{footnotesize}
\begin{longtable}{r l r r r r p{3.5cm}}
...
\begin{figure}[h]
    ...
    \begin{minipage}[t]{0.48\textwidth}
        ...
        \caption{\textbf{Iron-56}: $[Ar]\, 3d^6\, 4s^2$. Minimum mass-per-nucleon (the fusion endpoint). Six $3d$ solitons at $60°$ plus two $4s$ solitons at $180°$. FCC-14 packing ($0.0001\%$ error).}
...
The following figures show the vacuum strain density and flux streamlines for the same six elements, computed from the nucleon coordinates returned by the AVE element simulator. All spatial coordinates are in units of the proton spin radius $d = 4\hbar / m_p c \approx 0.841\text{ fm}$, derived from the physics engine. Nucleon positions for Z~$\geq$~16 use the spherical Fibonacci alpha-packing model\textemdash a geometric proxy that preserves the $1/r$ mutual impedance topology; exact Platonic/Archimedean solutions are not yet tabulated for each of these elements.
...
\begin{figure}[h]
    ...
    \end{minipage}
    \caption{\textbf{Iron-56}: FCC-14 packing at minimum mass-per-nucleon (the fusion endpoint) ($0.0001\%$ error). 14 alpha clusters visible in the flux plot.}
...
\begin{figure}[h]
    ...
    \begin{minipage}[t]{0.48\textwidth}
        ...
        \caption{\textbf{Iron-56}: $14\alpha$ FCC-14 architecture. Minimum mass-per-nucleon (the fusion endpoint). 1540 connections across 91 inter-alpha pairs.}
...
\begin{itemize}
    ...
    \item The transition from Small Signal ($M \approx 1$) to Large Signal ($M \gg 1$) at Sulfur-32 marks the onset of avalanche multiplication, with Iron-56 representing the minimum mass-per-nucleon (the fusion endpoint).
...
\begin{exercisebox}
\begin{enumerate}
    \item Using the semiconductor avalanche model, explain why Iron-56 sits at the minimum mass-per-nucleon (the fusion endpoint) of the stellar-fusion sequence.
