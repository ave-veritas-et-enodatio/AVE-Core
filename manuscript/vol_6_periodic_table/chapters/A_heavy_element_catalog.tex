...
% [2026-06-15 KB-reconciliation (vol_6 brief §D.3/§A.4, heavy-Z parallel of B_high_z_boundary:22) -- prior wording preserved per Rule 12]: read "this engine operates with \textbf{zero empirical mass-fitting parameters}. The coupling constant ($K$), Coulomb strength ($\alpha\hbar c$), breakdown capacity ($V_{BR}$), avalanche exponent ($n=5$), and internal charge radius ($d$) are all derived rigidly from the topological graph structure." SUPERSEDED: the listed CONSTANTS are indeed axiom-derived, but "zero empirical mass-fitting parameters" omits (i) the per-nucleus inter-alpha distance R, which the next paragraph itself identifies as "the operational R" the solver finds to reproduce the binding energy (= a per-nucleus fit), and (ii) for the Fibonacci-proxy elements, the empirical packing coefficients 15 and 0.95 in $r_{core}=d(15+0.95A)$ (B_high_z_boundary.tex:12). Scoped to "no empirical CONSTANTS" + explicit R/Fibonacci disclosure.
