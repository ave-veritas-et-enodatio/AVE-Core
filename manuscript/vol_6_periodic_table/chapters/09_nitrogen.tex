...
% [2026-06-15 KB-reconciliation (vol_6 brief §A.4 "other") -- prior wording preserved per Rule 12]: read "In the AVE framework, \textbf{the exact 3D shape of an atomic nucleus can be mathematically derived from first principles} simply by executing a global minimization search on the network's reactive impedance." SUPERSEDED: overclaims "first principles" -- the global minimization (next paragraphs) is constrained to reproduce the empirical CODATA mass, so the absolute geometry/scale is fit per nucleus, not predicted ab-initio. The shape CLASS (topology) is the genuine prediction; the per-nucleus scale is fit (00_introduction.tex:21 methodology note; vol6/claim-quality.md:529; byte-identical twin structure-isotope-stability.md:16).
