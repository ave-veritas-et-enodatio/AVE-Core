...
In previous models, atomic shape is either guessed from shell models or assumed as a liquid drop. In the AVE framework, \textbf{the 3D shape \emph{class} of an atomic nucleus is derived from first principles} by executing a global minimization search on the network's reactive impedance, constrained to reproduce the empirical CODATA mass. The topology (shape class) is the genuine prediction; the absolute per-nucleus scale is fit, not an ab-initio mass prediction (cf.\ the fit-vs-prediction methodology note in the Introduction).
...
For Nitrogen-14, executing a Basinhopping global optimizer to search the 42-dimensional spatial phase space (3 spatial coordinates for 14 interacting nucleons) yields a converged topological architecture whose fitted inter-alpha scale reproduces the CODATA target mass of $13040.204$ MeV to within $0.024\%$ (topological $13043.280$ MeV). The structure is asymmetrical, stretched, and heterogeneous, consistent with the picture that at Z=7 the nucleus behaves less like a rigid crystal and more like a fluid, reactive, multi-path scattering network.
...
In the AVE picture this irregular orbital topology re-expresses Nitrogen's half-filled $p$-shell stability and its high first ionization energy relative to Oxygen as the geometric incompressibility of three orthogonal standing-wave tracks sharing a single asymmetric nuclear strain field---the same effect standard shell models attribute to exchange energy (peer-with-QM, see \S\ref{box:internal_peer_scope}).


\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{figures/nitrogen_14_topology.png}
    \caption{Nitrogen-14 orbital knot topology. The five $n=2$ (L-shell) electrons ($2s^2\,2p^3$) sit at $72^\circ$ on the $n=2$ track, distinct from the two $n=1$ core electrons (seven total). The half-filled $p$-shell creates three orthogonal standing-wave tracks on the asymmetric nuclear strain field.}
...
\begin{itemize}
    ...
    \item In semiconductor terms, Nitrogen is a biased junction device with a permanent geometric dipole moment, which in the AVE picture maps to its half-filled $p$-shell stability (peer-with-QM).
\end{itemize}
\end{summarybox}
