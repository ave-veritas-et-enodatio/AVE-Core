...
In the AVE picture the tetrahedral $4\alpha$ geometry of Oxygen-16 is the structural account of its most famous macroscopic property: its role as a strong chemical oxidizer (peer-with-QM, see \S\ref{box:internal_peer_scope}).
...
Oxygen-16 ($4\alpha$) is the most compact even-even nucleus in the semiconductor engine. Its tetrahedral geometry---4 alpha clusters with 6 inter-alpha coupling pairs, all at identical separation---has a single geometric degree of freedom ($R_{\text{tet}} = 33.383\,d$), which the engine locks to reproduce the CODATA target of $14\,895.080$ MeV, closing to a $0.000\,000\%$ geometric fit-residual (structurally zero by solver construction, per the Platonic-progression geometric-identity case; distinct from the mass-prediction error in the validation set). The predicted content is the $(Z,A)$-forced tetrahedral topology and the axiom-derived $K$.
