...
Progressing sequentially up the binding curve, Magnesium-24 ($Z=12, A=24$) perfectly balances 12 protons and 12 neutrons. Therefore, like Oxygen-16 ($4\alpha$) and Neon-20 ($5\alpha$), Magnesium-24 closes an absolute scalar shell boundary and is constructed exclusively from identical Alpha geometries, yielding exactly $6\alpha$.
...
\noindent\textbf{[DEMOTED 2026-08-03 --- structural/interpretive, not a quantitative alloy prediction] (Rule~12: the paragraph above is preserved verbatim; no number is edited, deleted or replaced).} The paragraph above reads as a causal derivation of an engineering material property from nuclear topology (``derives directly from\ldots'', ``precisely because\ldots'', ``at comparable bond strength\ldots is the direct macroscopic manifestation''). That register exceeds the governing claim entry for this whole per-element ``Topological Area of Interest'' family, which explicitly non-claims it: \emph{``Does NOT claim quantitative predictions of bond enthalpies, electronegativity scales (Pauling / Mulliken), reaction kinetics, or material constants from the topology alone. The mapping is structural / interpretive''} (\kbleaf{ave-kb/vol6/claim-quality.md}, \texttt{clm-f8k2um}). \textbf{Read the paragraph in the demoted register:} what the $6\alpha$ octahedral closure supplies is a \emph{structural} account --- a closed, zero-dipole, symmetric core --- \emph{consistent with} Magnesium's role as the lightest structural metal. \textbf{What is NOT claimed:} that alloy strength-to-weight ratio, bond strength, or any other material constant is \emph{predicted} by the topology. In particular the clause ``at comparable bond strength'' is an \emph{assumed} premise imported from materials data, not a topological output; the $24/27 = 89\%$ figure is a nucleon-count ratio (arithmetic on $A$), not a strength prediction; and no quantitative strength-to-weight number is derived anywhere in this chapter. Same governing boundary as the Neon discharge-spectrum entry (\texttt{12\_neon.tex}:65) and the other per-element behaviour summaries.
...
Magnesium-24 ($6\alpha$) constructs as a perfect Octahedron with 15 inter-alpha coupling pairs. The semiconductor engine solves $R_{\text{oct}} = 78.0\,d$ at $V_R/V_{BR} = 0.040$, $M = 1.000$ (Small Signal), reproducing the CODATA mass of $22\,335.793$ MeV to a $0.000\,0\%$ fitting tolerance on the single per-nucleus scalar $R$. The $6\alpha$ closed shell is an exact-geometric-identity case: $R$ snaps to the unique impedance-matched octahedral vertex set (see the fit-vs-predict methodology note in Ch.~\ref{ch:intro}, and the Scope note in \S\ref{box:internal_peer_scope}), distinct from the core-plus-halo fit nuclei whose summary-table residuals are nonzero.
...
\end{equation}
\end{resultbox}
\noindent This resolves the prior $+3.5\%$ residual with zero fit constants given the standard $[\text{Ne}]\,3s^2$ filling order (an asserted input, not derived here); the accuracy is gated against the engine's pinned reference values rather than against a NIST-exact target.  The correction is gated to $n_\text{adjacent} \geq 2$ (inner shell has $p$-subshells), ensuring mutual exclusivity with the Be-type hierarchical cascade.  See Vol.~II for the full derivation.
