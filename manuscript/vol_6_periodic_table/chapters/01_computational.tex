...
Tritium ($1p, 2n$) lacks the necessary geometric symmetry to fold into a tight topological knot. The solver proves that to match its empirical mass defect ($8.48$ MeV), its nodes must be stretched to a wide $\sim 3.5d$ separation. This results in a critically low Topological $Q$-factor of just $3.20$. To eliminate this extreme parasitic strain, the topology spontaneously ejects a unit of phase (an electron via $\beta$-decay) to transition into the stable Helium-3 ($^3He$) lattice, which boasts a tight, highly symmetrical $Q=19.52$ footprint. The empirical mass-energy difference for this transition is $m(^3\text{H})c^2 - m(^3\text{He})c^2 \approx 0.529$ MeV, of which the $\beta$-endpoint kinetic energy is $\sim 18.6$ keV. The framework's contribution here is \textit{mechanistic} --- the $Q$-factor jump ($3.20 \to 19.52$) explains \textit{why} the transition occurs spontaneously and which direction it runs; the released energy \textit{magnitude} is taken from empirical mass-energy data, not derived independently here.
...
\begin{figure}[htbp]
    ...
    \caption{\textbf{Radioactive Decay via Q-Factor Optimization.} \textit{Left:} Tritium's unstable topology collapses into the tighter Helium-3 structure; the $Q$-factor jump ($3.20 \to 19.52$) is the framework's mechanistic driver, with the empirical mass-energy difference ($\approx 0.529$ MeV) accounting for the released energy. \textit{Right:} Beryllium-8 represents a broken inductive bridge; without a central neutron to mediate the structural tension, it instantly cleaves into two Alpha cores.}
