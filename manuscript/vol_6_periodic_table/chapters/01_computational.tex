...
% [2026-06-15 KB-reconciliation (vol_6 brief §A.4, parallel-site) -- prior wording preserved per Rule 12]: read "Every parameter derives from the four AVE axioms with zero empirical fits." SUPERSEDED: "every parameter" overclaims -- it omits the per-nucleus geometric DOF. Reconciled to the honest KB twin semiconductor-nuclear-analysis.md:14 (which scopes it to NON-GEOMETRIC constants and discloses the per-nucleus R-fit in its ⚠ methodology note at :11). This .tex chapter carried no R-disclosure.
...
\begin{table}[htbp]
\centering
% [2026-06-15 KB-reconciliation (vol_6 brief §A.4, parallel-site) -- prior caption preserved per Rule 12]: read "All parameters are axiom-derived; zero empirical fits." SUPERSEDED: scopes to the NON-GEOMETRIC constants; the per-nucleus inter-alpha distance R is fit (see §\ref{sec:semiconductor_nuclear} + semiconductor-nuclear-analysis.md:11/:128). Predicted masses below are reproduced at the per-nucleus fitted R.
