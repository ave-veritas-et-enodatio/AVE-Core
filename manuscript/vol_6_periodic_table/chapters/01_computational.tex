...
\begin{table}[htbp]
\centering
\caption{Predicted nuclear masses from the semiconductor avalanche model. The non-geometric constants are axiom-derived (zero empirical fits); the per-nucleus inter-alpha distance $R$ is fit to reproduce each CODATA mass. The avalanche multiplier $M$ remains at unity for $Z \le 14$ (Small Signal) and jumps to $32.8$ at $Z=16$ (Large Signal).}
