...
\begin{itemize}
    ...
    \item Reproduce CODATA masses to sub-0.01\% for light elements ($Z \leq 14$) using a single fitted inter-alpha distance $R$ per nucleus within the $(Z,A)$-forced alpha-cluster topology and axiom-derived coupling constants --- the accuracies are fitting tolerances on $R$, not ab-initio prediction errors (methodology note, Chapter~\ref{ch:intro}).
\end{itemize}
\end{objectivebox}
...
Within the AVE framework, "dissipation" here is \textbf{boundary/aperture radiative delivery} --- energy leaving the defect \emph{through its geometric perimeter} into the surrounding lattice, an Axiom-3-legal \textbf{port}, not a bulk friction inside the knot. $Q$ is calculated as the ratio of Total Internal Mutual Inductance ($U_{stored}$) to the Effective Topological Radius ($R_{eff}$).

\noindent\textbf{Mechanism relabel (2026-08-04, ruling execution).} The prior wording of the sentence above named the mechanism \emph{``acoustic drag (vacuum friction)''} --- bulk-loss vocabulary for a boundary object, which invites the crossing-resistor reading Axiom~4's lossless substrate forbids. The replacement mechanism was \emph{identified}, not chosen: the 2026-07-17 Regime-IV dissipation audit graded this section \textbf{radiative-port, port-only}, and under the transfer-cost theorem (\kbleaf{ave-kb/common/transfer-cost-theorem.md}) a boundary port is a \textbf{delivery} terminal --- the substrate does not heat, the energy leaves. \textbf{One caveat travels with it:} the tabulated $Q$ contains \emph{no dissipated-power term} --- it is $U_{stored}/R_{eff}$, a stored-energy-over-geometric-length ratio --- so it ranks \textbf{confinement/compactness} (He-4 tight, Li-7 leaky) and must not be read as $\omega U/P$.
...
At nuclear separations ($d \sim r_{\text{proton}} \approx 0.88$ fm), the nucleon strain fields are close enough that the current distributions deform to concentrate flux toward the adjacent coil---the well-known \textbf{proximity effect} in EE transformer theory. The first-order flux-concentration correction to the mutual coupling is $1/(1 - \alpha/3)$, where $\alpha/3$ is the isotropic 3D spatial average of the single-crossing coupling factor $\alpha$ over the three spatial directions (the substrate proximity-effect correction to $M_{12}$).
...
The nuclear binding problem maps precisely onto the large-signal analysis of semiconductor junction devices. This is not an analogy---the vacuum LC lattice \textit{is} the dielectric medium, the nucleon knots \textit{are} the junction dopants, and the Coulomb field \textit{is} the reverse-bias voltage. All non-geometric constants ($K_{mutual}$, $\alpha$, $D_{intra}$, breakdown voltages) derive from the four AVE axioms with zero empirical fits; the remaining geometric degree of freedom is the per-nucleus inter-alpha distance $R$, fit so the $\sum K/d_{ij}$ summation reproduces each CODATA mass --- so the tabulated $\le 0.03\%$ accuracies are fitting tolerances on $R$, not ab-initio mass-prediction errors.
...
\hline
$l_\text{node}$ & \texttt{L\_NODE} & $3.862\times10^{-13}$ & m & Axiom 1 \\
...
$m_p/m_e$ & \texttt{PROTON\_ELECTRON\_RATIO} & $1836.1$ & --- & Ax 1--3 + electron-sector inputs ($m_e,\alpha$) + $\delta_{th}=1/(14\pi^2)$ \\
...
\begin{table}[htbp]
\centering
% [2026-08-02 SCOPE PROPAGATION -- the \textbf{Error} column below is the GEOMETRIC RESIDUAL,
%  not a mass/binding prediction error. No row, number or column is changed; the caption gains
%  the column's referent. Authority, verbatim, from
...
%     ... (structurally zero by solver construction). The load-bearing physics test EXISTS in
%     sibling doc [vol6/framework/mass-defect-summary.md]:8-24 ..."
%  This volume prints BOTH quantities for the SAME nuclei under the same word "Error":
%  00_summary_table.tex:33 gives Si-28 as 0.01907% (the CODATA mass residual; identical at
%  manuscript/ave-kb/vol6/framework/mass-defect-summary.md:30) while the Si-28 row of the table
%  below gives 0.0002% (the geometric residual). Both are correct for their own quantity; only the
%  shared label was ambiguous. The caption's fit-vs-predict disclosure is UNCHANGED and is a
%  separate debt already discharged.]
\caption{Predicted nuclear masses from the semiconductor avalanche model. The non-geometric constants are axiom-derived (zero empirical fits); the per-nucleus inter-alpha distance $R$ is fit to reproduce each CODATA mass. The avalanche multiplier $M$ remains at unity for $Z \le 14$ (Small Signal) and jumps to $32.8$ at $Z=16$ (Large Signal). \textbf{The Error column is the geometric residual} of the converged cluster positions against the assumed Platonic/Archimedean coordinates (structurally zero by solver construction), \emph{not} a binding-energy prediction error against experiment; the CODATA mass residuals for these same nuclei are the separate, larger figures in the validation table of Ch.~\ref{ch:summary} (e.g.\ Si-28: $0.01907\%$ there vs.\ $0.0002\%$ here). Canonical: \kbleaf{manuscript/ave-kb/vol6/appendix/geometric-inevitability/platonic-progression.md}.}
...
\begin{itemize}
    \item \textbf{Triangle} (C-12, 3$\alpha$): Low vertex density $\rightarrow$ $V_R/V_{BR} = 0.019$ $\rightarrow$ deep Small Signal.
    \item \textbf{Tetrahedron} (O-16, 4$\alpha$): Moderate packing $\rightarrow$ $V_R/V_{BR} = 0.030$ $\rightarrow$ Small Signal.
    \item \textbf{Pentagonal bipyramid} (Si-28, 7$\alpha$): High packing but open faces $\rightarrow$ $V_R/V_{BR} = 0.050$ $\rightarrow$ boundary of Small Signal. \textit{Silicon's topology sits at the boundary of the Small-Signal regime; in the AVE picture this co-locates with its role as a switchable-bandgap semiconductor --- highly stable in bulk, yet close enough to the breakdown threshold to be doped and switched dynamically.}
...
Tritium ($1p, 2n$) lacks the necessary geometric symmetry to fold into a tight topological knot. The solver proves that to match its empirical mass defect ($8.48$ MeV), its nodes must be stretched to a wide $\sim 3.5d$ separation. This results in a critically low Topological $Q$-factor of just $3.27$. To eliminate this extreme parasitic strain, the topology spontaneously ejects a unit of phase (an electron via $\beta$-decay) to transition into the stable Helium-3 ($^3He$) lattice, which boasts a tight, highly symmetrical $Q=19.04$ footprint. The empirical mass-energy difference for this transition is $m(^3\text{H})c^2 - m(^3\text{He})c^2 \approx 0.529$ MeV, of which the $\beta$-endpoint kinetic energy is $\sim 18.6$ keV. The framework's contribution here is \textit{mechanistic} --- the $Q$-factor jump ($3.27 \to 19.04$) explains \textit{why} the transition occurs spontaneously and which direction it runs; the released energy \textit{magnitude} is taken from empirical mass-energy data, not derived independently here.
...
\begin{figure}[htbp]
    ...
    \caption{\textbf{Radioactive Decay via Q-Factor Optimization.} \textit{Left:} Tritium's unstable topology collapses into the tighter Helium-3 structure; the $Q$-factor jump ($3.27 \to 19.04$) is the framework's mechanistic driver, with the empirical mass-energy difference ($\approx 0.529$ MeV) accounting for the released energy. \textit{Right:} Beryllium-8 represents a broken inductive bridge; without a central neutron to mediate the structural tension, it instantly cleaves into two Alpha cores.}
...
\begin{itemize}
    ...
    \item The computational engine reproduces CODATA masses to sub-0.01\% for light elements ($Z \leq 14$) using a single fitted inter-alpha distance $R$ per nucleus within the $(Z,A)$-forced alpha-cluster topology and axiom-derived coupling constants; the accuracies are fitting tolerances on $R$.
\end{itemize}
\end{summarybox}
