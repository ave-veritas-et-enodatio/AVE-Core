...
\begin{enumerate}
    % [2026-06-15 KB-reconciliation (vol_6 brief §A.4) -- prior wording preserved per Rule 12]: read "...Maximum error: $0.12\%$ (Si-28). Zero free parameters." SUPERSEDED: the analytic regime solves each nucleus at a per-nucleus fitted inter-alpha distance R (e.g. R_bipyramid=81.181d, 12_neon.tex:23); the topology is zero-parameter but R is the one fitted scalar, so these accuracies are fitting tolerances on R, not ab-initio prediction errors (00_introduction.tex:21; mass-defect-summary.md:14).
    \item \textbf{Analytic Regime ($Z \leq 14$):} Each nucleus is solved with a hand-crafted, physically motivated geometry (tetrahedral alpha clusters, bipyramids, octahedra). Mean accuracy: $|\Delta| = 0.055\%$. Maximum error: $0.12\%$ (Si-28). Topology zero-parameter (forced by $(Z,A)$); one fitted scalar $R$ per nucleus (the quoted accuracies are fitting tolerances on $R$).
...
\begin{itemize}
    % [2026-06-15 KB-reconciliation (vol_6 brief §D.3) -- prior wording preserved per Rule 12]: read "The model uses \textbf{zero fitted parameters} beyond the axiom-derived $K_{mutual}$, $d_{proton}$, and $\alpha\hbar c$." SUPERSEDED: omits the two empirical packing coefficients (15 and 0.95) in the Fibonacci-regime density-scaled radius $r_{core}=d(15+0.95A)$ (this section, item 2 above) -- those coefficients are NOT axiom-derived. The Fibonacci/superheavy regime carries no PER-NUCLEUS fit (the radius is a closed-form function of A), but the two global packing coefficients are empirical.
    \item The model uses \textbf{no per-nucleus fitted parameter} in this Fibonacci/superheavy regime beyond the axiom-derived $K_{mutual}$, $d_{proton}$, and $\alpha\hbar c$; its only empirical inputs are the two global Fibonacci-packing coefficients ($15$ and $0.95$) in the density-scaled radius $r_{core}=d(15+0.95A)$ above, which are not axiom-derived.
