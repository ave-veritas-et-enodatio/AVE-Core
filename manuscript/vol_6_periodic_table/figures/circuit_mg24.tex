...
\node[text=aveink, font=\bfseries\Large] at (0, 8.2)
    {Mg-24 \quad Semiconductor Equivalent Circuit};
\node[text=avemuted, font=\small] at (0, 7.5)
    {$6\alpha$ Octahedron \quad|\quad $V_R/V_{BR} = 0.040$ \quad|\quad $M = 1.000$ (Small Signal)};
...
\node[text=aveink, font=\small, draw=avemuted, dashed, rounded corners=3pt, inner sep=5pt]
    at (0, -5.6) {Mg-24: $V_R/V_{BR} = 0.040 \implies M = 1.0$ \quad (Deep Small Signal)};
...
\node[small alpha, draw=aveink, fill=white!85!aveink] (p1) at (-4.5+0.3, -10.0+2.5) {$\alpha_N$};
\node[small alpha, draw=aveink, fill=white!85!aveink] (p2) at (-4.5-0.3, -10.0-2.5) {$\alpha_S$};
...
\node[text=avemuted, font=\tiny, text width=2.6cm, align=left, anchor=west] at (-1.5, -9.5)
    {4 ring + 8 pole-eq\\2 diagonal + 1 pp\\\textbf{15 junctions}};
...
\node[text=aveink, font=\small, text width=7cm, align=left,
    ...
    \textcolor{avevermillion}{Miller:} $M = 1.000$ ($V_R/V_{BR} = 0.040$)\\[4pt]
    \textbf{Result:} $22,335.793$ MeV ($0.0000\%$ error)
};
