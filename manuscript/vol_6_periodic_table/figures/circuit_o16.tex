...
\node[text=aveink, font=\bfseries\Large] at (0, 8.2)
    {O-16 \quad Semiconductor Equivalent Circuit};
\node[text=avemuted, font=\small] at (0, 7.5)
    {$4\alpha$ Tetrahedron \quad|\quad $V_R/V_{BR} = 0.030$ \quad|\quad $M = 1.000$ (Small Signal)};
...
\node[text=aveink, font=\small, draw=avemuted, dashed, rounded corners=3pt, inner sep=5pt]
    at (0, -5.6) {O-16: $V_R/V_{BR} = 0.030 \implies M = 1.0$ \quad (Deep Small Signal)};
...
\node[small alpha] (n0) at (-4.5, -10.0+2.3) {$\alpha$};
\node[small alpha] (n1) at (-4.5-2.0, -10.0-1.5) {$\alpha$};
\node[small alpha] (n2) at (-4.5+2.0, -10.0-1.5) {$\alpha$};
\node[small alpha] (n3) at (-4.5, -10.0+0.3) {$\alpha$};
...
\node[text=avemuted, font=\tiny, text width=2.6cm, align=left, anchor=west] at (-1.5, -9.5)
    {6 tetrahedral edges\\\textbf{6 junctions total}};
...
\node[text=aveink, font=\small, text width=7cm, align=left,
    ...
    \textbf{Result:} $14,895.080$ MeV ($0.0000\%$ error)
};
