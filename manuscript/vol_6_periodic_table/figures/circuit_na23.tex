...
\node[text=aveink, font=\bfseries\Large] at (0, 8.2)
    {Na-23 \quad Semiconductor Equivalent Circuit};
\node[text=avemuted, font=\small] at (0, 7.5)
    {$5\alpha + ^3\text{H}$ Core+Halo \quad|\quad $V_R/V_{BR} = 0.032$ \quad|\quad $M = 1.000$ (Core+Halo)};
...
\node[text=aveink, font=\small, draw=avemuted, dashed, rounded corners=3pt, inner sep=5pt]
    at (0, -5.6) {Na-23: Core $V_R/V_{BR} = 0.032 \implies M = 1.0$ \quad (Core inherits parent regime)};
...
\node[small alpha, minimum size=1.5cm] (core) at (-4.5, -10.0) {$5\alpha$\\core};
\node[small alpha, draw=avevermillion, fill=white!85!avevermillion] (halo) at (-1.0, -10.0) {$^3\text{H}$};

\draw[avevermillion, thick, dashed] (core) -- (halo) node[midway, above, text=aveink, font=\tiny] {$M_{\text{halo}}$};


\node[text=avemuted, font=\tiny, text width=2.6cm, align=left, anchor=west] at (-1.5, -9.5)
    {Core + halo at $50.171d$\\\textbf{10+ junctions}};
...
\node[text=aveink, font=\small, text width=7cm, align=left,
    ...
    \textbf{Result:} $21,409.214$ MeV ($0.0000\%$ error)
};
