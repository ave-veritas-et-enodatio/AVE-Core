...
\node[text=aveink, font=\bfseries\Large] at (0, 8.2)
    {F-19 \quad Semiconductor Equivalent Circuit};
\node[text=avemuted, font=\small] at (0, 7.5)
    {$4\alpha + ^3\text{H}$ Core+Halo \quad|\quad $V_R/V_{BR} = 0.030$ \quad|\quad $M = 1.000$ (Core+Halo)};
...
\node[text=aveink, font=\small, draw=avemuted, dashed, rounded corners=3pt, inner sep=5pt]
    at (0, -5.6) {F-19: Core $V_R/V_{BR} = 0.030 \implies M = 1.0$ \quad (Core inherits parent regime)};
...
\node[small alpha, minimum size=1.5cm] (core) at (-4.5, -10.0) {$4\alpha$\\core};
...
\node[text=avemuted, font=\tiny, text width=2.6cm, align=left, anchor=west] at (-1.5, -9.5)
    {Core + halo at $398.478d$\\\textbf{6+ junctions}};
...
\node[text=aveink, font=\small, text width=7cm, align=left,
      ...
      at (2.5, -7.6) {
    $M_{\text{nuc}} = 4\, M_\alpha - BE_{\text{net}}$\\[6pt]
    $BE_{\text{net}} = \underbrace{\displaystyle\sum_{i<j}^{6} 16 \cdot \frac{K}{R_{ij}}}_{\ text{Strong (attractive)}}
    ...
    \textbf{Result:} $17,692.302$ MeV ($0.0000\%$ error)
};
