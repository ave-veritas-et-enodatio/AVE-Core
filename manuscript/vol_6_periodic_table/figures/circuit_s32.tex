...
\node[text=aveink, font=\bfseries\Large] at (0, 8.2)
    {S-32 \quad Semiconductor Equivalent Circuit};
\node[text=avemuted, font=\small] at (0, 7.5)
    {$8\alpha$ Cube \quad|\quad $V_R/V_{BR} = 0.994$ \quad|\quad $M = 32.800$ (Large Signal)};
...
\node[text=avevermillion, font=\small, draw=avemuted, dashed, rounded corners=3pt, inner sep=5pt]
    at (0, -5.6) {S-32: $V_R/V_{BR} = 0.994 \implies M = 32.8$ \quad (\textbf{AVALANCHE})};
...
\node[small alpha] (f0) at ({-4.5+-1.2}, {-10.0+1.2}) {$\alpha$};
\node[small alpha] (f1) at ({-4.5+1.2}, {-10.0+1.2}) {$\alpha$};
\node[small alpha] (f2) at ({-4.5+1.2}, {-10.0+-1.2}) {$\alpha$};
\node[small alpha] (f3) at ({-4.5+-1.2}, {-10.0+-1.2}) {$\alpha$};
\node[small alpha, draw=avevermillion, fill=white!85!avevermillion] (b0) at ({-4.5+-2.2}, {-10.0+2.2}) {$\alpha$};
\node[small alpha, draw=avevermillion, fill=white!85!avevermillion] (b1) at ({-4.5+2.2}, {-10.0+2.2}) {$\alpha$};
\node[small alpha, draw=avevermillion, fill=white!85!avevermillion] (b2) at ({-4.5+2.2}, {-10.0+-2.2}) {$\alpha$};
\node[small alpha, draw=avevermillion, fill=white!85!avevermillion] (b3) at ({-4.5+-2.2}, {-10.0+-2.2}) {$\alpha$};

\draw[aveaccent, thick] (f0)--(f1)--(f2)--(f3)--cycle;
\draw[aveaccent, thick] (b0)--(b1)--(b2)--(b3)--cycle;
\draw[aveaccent] (f0)--(b0); \draw[aveaccent] (f1)--(b1); \draw[aveaccent] (f2)--(b2); \draw[aveaccent] (f3)--(b3);


\node[text=avemuted, font=\tiny, text width=2.6cm, align=left, anchor=west] at (-1.5, -9.5)
    {12 cube edges\\+16 face/body diag.\\\textbf{28 junctions}};
...
\node[text=aveink, font=\small, text width=7cm, align=left,
      ...
      at (2.5, -7.6) {
    $M_{\text{nuc}} = 8\, M_\alpha - BE_{\text{net}}$\\[6pt]
    $BE_{\text{net}} = \underbrace{\displaystyle\sum_{i<j}^{28} 16 \cdot \frac{K}{R_{ij}}}_{\ text{Strong (attractive)}}
    ...
    \textcolor{avevermillion}{Miller:} $M = 32.800$ ($V_R/V_{BR} = 0.994$)\\[4pt]
    \textbf{Result:} $29,855.525$ MeV ($0.0000\%$ error)
};
