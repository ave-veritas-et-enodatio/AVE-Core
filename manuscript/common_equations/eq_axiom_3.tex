...
\end{equation}
This is the temporal-gauge (Weyl-gauge, $A_0$-free) form of the Maxwell Lagrangian: standard Maxwell dynamics are recovered on the Gauss-constrained initial-data surface --- $\nabla\cdot(\varepsilon_0\partial_t\mathbf{A})$ is a constant of motion of these equations; \textbf{its pin to a source value is OWED to the bound-sector source law, not held in written canon} (the Substrate DC Bias --- \texttt{eq\_axiom\_5.tex}, KB \texttt{axiom-register.md}; restructured axiom$\to$source law per R55, 2026-08-24: no ratified clause yet writes the pin --- clause S deposits the \emph{A1 dilatation} flux $\oint_S \mathbf{u}\cdot\hat{\mathbf{n}}$, a mechanical-sector object distinct from the EM Gauss function, and the source law's own forbids-block bars any EM/winding-sector deposit --- a NAMED-OPEN riding the DEFERRED FORK-1 variable-class fork), and it is in any case not pinned by an equation of motion of this action (the written action carries no scalar-potential/multiplier term). Recovered as the substrate's effective action in the linear regime ($A \ll A_{yield}$). Under finite strain, $\varepsilon_0$ and $\mu_0$ become nonlinear functions of local strain via Axiom 4's saturation kernel ($\varepsilon_{eff} = \varepsilon_0\, S(A)$, $\mu_{eff} = \mu_0\, S(A)$).
...
% Pre-edit wording (content-verbatim; \texttt{} wrappers and underscore escapes
% normalized for comment readability, byte-exact text in git): "pinned to its source value by the
...
% PRIOR-RECORD ACKNOWLEDGMENT: the 2026-08-10 fragment above already held most of
% this — the clause-S narrowing was withdrawn at seven sites on the same physics
% (S deposits A1 mass; the law's own forbids-block bars "any EM/winding-sector
% deposit", so clause S CANNOT be the pin by the ratified text itself). The R55
% panel's genuine delta is only the grade: "which clause is open" -> "NO clause
% yet writes it — owed". Not a new discovery; a corrected grade on a recorded one.
% CITING-SITE MAP (the :22 in-place rewrite): two frozen records quote the pre-edit
%  ':22' wording verbatim — research/2026-08-12_boundary-response_result.md:137 and
%  research/2026-08-12_boundary-response_prereg-FROZEN.md:277. Per Rule 12 neither is
%  rewritten; their quotes resolve to the preserved verbatim in THIS fragment, not to
%  :22. (R55 round-2 restated the :22 body again — owed-not-held grade, repairing an
%  assert-and-deny reading; the pre-edit wording above is unchanged origin/main text.)
