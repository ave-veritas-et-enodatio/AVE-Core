% Rule-12 reframe (2026-06-21, Grant THEORY-CURRENCY ruling): the +4 degC density
% maximum is a STATISTICAL cooperative two-state (LDL/HDL) result, NOT a closed-form
% T_0(1-alpha) output; and the (1-alpha) Axiom-2 alpha-loading is an ASSERTED
% alpha-calibration (consistency identification / ECHO), not an independently derived
% chord. The melting derivation, 279.5 K bare eigenmode, and Axiom-4 saturation
% correction are KB-consistent and unchanged. Canon: water-anomaly-lc-partition.md.
...
\begin{itemize}
    ...
    \item Understand how all seven bulk water properties emerge from a single proton $LC$ eigenmode---every input is engine-derived or a fundamental constant ($m_\text{H}$, $\hbar$, $k_B$, $\alpha$), with the $(1-\alpha)$ step an asserted $\alpha$-calibration rather than a derived chord.
    \item Explain the density anomaly as a statistical cooperative two-state (LDL/HDL) average---a competition between melting contraction and thermal expansion near the theoretical melting eigenmode---not a closed-form output.
\end{itemize}
\end{objectivebox}
...
$m_\text{H}$ & $1.673 \times 10^{-27}$ kg & Proton mass & Imported constant \\
...
This bare eigenmode $T_0 = 279.5$~K is the \emph{harmonic} LC resonance; it sets the temperature \emph{scale} of the density-anomaly domain. The $+3.98^{\circ}$C density maximum itself emerges as a statistical cooperative two-state (LDL/HDL) result, not as the closed-form $T_0(1-\alpha)$ (\kbleaf{vol3/condensed-matter/ch11-thermodynamics/water-anomaly-lc-partition.md}). The bare eigenmode overestimates the melting point by $+2.3\%$ because it neglects the Op4 anharmonicity.
...
\begin{enumerate}
  ...
  \item \textbf{Axiom~2 $\alpha$-loading} ($+0.88\% \to +0.14\%$): the eigenmode is loaded by $(1-\alpha)$. This $\alpha$-loading is an \emph{asserted} $\alpha$-calibration (a consistency identification), not an independently derived chord: no derivation links the EM coupling $\alpha$ to H-bond LC-mode loading.
\end{enumerate}
...
\begin{itemize}
  ...
  \item The bare eigenmode $T_0 = 279.5$~K sets the temperature \emph{scale} of the
        density anomaly; the $+4^{\circ}$C density maximum is a statistical cooperative
        two-state (LDL/HDL) result ($\sim +3.98^{\circ}$C observed), not a closed-form output.
...
lone pair (which is a totally reflected wave, $\Gamma = -1$\gammaundeclared{}, with weight $1$).
...
$Z_\text{H}$) and two open ports (lone pairs, $\Gamma = -1$\gammaundeclared{}) has an asymmetric 
...
Classical models struggle to reproduce the density of liquid water (998~kg/m$^3$ at 20$^\circ$C) from its molecular geometry without invoking arbitrary empirical packing radii. If one treats a water molecule as a continuous solid boundary (like a 3-sphere exact geometric union), its bare spatial volume is rigidly $5.56\ \AA^3$. Dividing this by the $\varphi_{FCC}$ Kepler packing fraction yields an unphysical packed density of nearly 4000~kg/m$^3$ (roughly $4\times$ the observed value).

In AVE (Axiom~1), spatial volume is not the geometric hull of a particle, but the volumetric capacity of a loaded discrete network cell. The macroscopic liquid-state (State II) volume is derived without spatial bounds, relying strictly on the $K=2G$ topological collapse (Axiom~2).

\subsection{State I: The Open Lattice ($V_I$)}
The rigid, unyielded State I is a tetrahedral lattice dictated by the Op4 pairwise minimum distance ($d_{OO} = d_{OH} + d_{hb} = 2.727\ \AA$). The unit cell volume natively occupied by one molecule in this expanded topology is:
\begin{equation}
...
\end{equation}
The resulting macroscopic density is $m_{H_2O} / V_{avg} = 1005.8$~kg/m$^3$. This reproduces the experimental liquid-state density to $+0.7\%$, entirely devoid of empirical collision or hard-sphere parameters.
