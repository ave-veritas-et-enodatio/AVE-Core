...
The factor of $2/3$ relates the total 3D thermal energy to the pressure--volume product, via the equipartition of energy over the three Cartesian axes. Evaluating:
\begin{equation}
    R = \frac{PV}{nT} = N_A \cdot k_B = 6.022 \times 10^{23} \times 1.381 \times 10^{-23} = 8.314 \text{ J/(mol$\cdot$K)}
\end{equation}

This is the exact CODATA value~\cite{codata2018} of the universal gas constant. The identity $R = N_A k_B$ recovers the empirical gas constant to $<0.001\%$ as a consistency check: the LC energy-balance mapping reproduces $PV = nRT$ with no corrected gas law, while $k_B$ and $N_A$ enter as the imported SI scaling constants. The forced structure is the mapping (pressure as LC energy density, temperature as RMS grid noise); the dimensionful values are calibration inputs.
...
\begin{itemize}
    ...
    \item The Universal Gas Constant ($R = N_A k_B$) follows mechanically as the macroscopic (per-mole) projection of the per-node LC thermal scaling; $k_B$ and $N_A$ enter as the imported SI calibration constants.
\end{itemize}
\end{summarybox}
...
\begin{enumerate}
    ...
    \item Explain dynamically why Boltzmann's constant uniquely bounds the minimum per-node thermodynamic resolution limit of the substrate discrete grid.
\end{enumerate}
\end{exercisebox}
