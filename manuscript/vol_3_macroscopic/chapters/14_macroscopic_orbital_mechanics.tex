...
where $\delta_{in}, \delta_{out}$ are the declinations of the incoming and outgoing hyperbolic asymptotes. The $(\cos \delta_{in} - \cos \delta_{out})$ structure is the AVE-substrate derivation of the empirical fit form via the Sagnac-loop integral between endpoint projections. \textbf{Scope correction (2026-05-18 walk-back):} earlier versions of this section wrote the geometric factor as $\cos(\alpha_{geo})\cos(\delta_{geo})$ with the headline ``intrinsically outputs $\Delta V \approx 13.4$ mm/s without fitting.'' Direct verification against the six-spacecraft anchor set (driver \kbleaf{src/scripts/verify/flyby\_anomaly\_anderson\_anchor.py}) found: (a) literal $\cos(\alpha)\cos(\delta)$ notation produced wrong-sign or wildly-off predictions for ALL 6 anchor spacecraft; correct geometric factor is $(\cos\delta_{in} - \cos\delta_{out})$; (b) ``13.4 mm/s'' is the NEAR-specific evaluation ($V_\infty = 6.851$ km/s + NEAR geometry), not a universal AVE prediction. Match across 6-spacecraft anchor set: 2/6 within 1$\sigma$ (Galileo~II, Cassini), 3/6 within 2$\sigma$ (adds NEAR); Galileo~I, Rosetta~I, MESSENGER are $>2\sigma$ outliers. The Sagnac-RLVE Regime-IV stator-boundary MECHANISM is preserved (consistent with the Gravitational Stator framework at \kbleaf{vol3/cosmology/ch06-solar-system/plasma-standoff-vs-gravitational-stator.md} and the sibling-observable Earth magnetic dipole derivation at \kbleaf{vol3/applied-physics/ch13-geophysics/geodynamo-vca-back-emf.md}); only the geometric notation and per-spacecraft framing are corrected. Pioneer anomaly mention removed (different observable --- long-duration trajectory, not perigee passage $\Delta V$). \textbf{Categorical discrimination vs GR Lense-Thirring:} AVE Regime-IV ($\sim 10^{-6}$ coupling) is 6 orders of magnitude stronger than GR Lense-Thirring ($\sim 10^{-12}$); the AVE prediction lies in the right order of magnitude across the anchor set, while GR is categorically too small. The 2/6 1$\sigma$ match is a partial match, not a categorical falsification.
...
superconductivity (0.00\% error \textit{Axiom~4 manifestation: the kernel IS $B_c(T)$, not a fitted prediction}), BH ring-down (1.7\% from GR), and
