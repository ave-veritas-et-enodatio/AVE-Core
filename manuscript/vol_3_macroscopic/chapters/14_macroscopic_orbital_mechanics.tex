...
\noindent\textbf{Scope correction (2026-06-15 driver-honesty re-sweep, LF-03).} The overlaid flare timeline is a \emph{synthesized} reconstruction of the Solar Cycle 23/24/25 X/M-class structure (a Monte-Carlo placeholder generated in \texttt{fetch\_and\_plot\_noaa\_goes\_flares.py}), \emph{not} a live fetch of the NOAA GOES catalog; despite the figure filename \texttt{noaa\_goes\_empirical\_validation.png}, this is an \emph{illustrative} comparison and does not by itself constitute empirical validation. The $0.46$-yr FWHM is the AVE forward prediction; falsification against the real NOAA GOES archive is the pending live-fetch deliverable. The prose below is retained with this scope.
