...
When softened by the Axiom~4 saturation operator $S = \sqrt{1 - r_\mathrm{surf}^2}$ at 25$^\circ$C, the derived surface tension is $67.31$ mN/m. The interfacial strain $r_\mathrm{surf} = \sqrt{k_B T / E_\mathrm{hb}}$ carries no factor of $\tfrac{1}{2}$, in contrast to the bulk-oscillator strain $r_\mathrm{bulk} = \sqrt{k_B T / (2 E_\mathrm{hb})}$ used for the eigenmode (Section~\ref{sec:proton_eigenmode}): a surface bond is a single-sided open termination ($\Gamma = +1$) loaded against the full well depth, whereas a bulk oscillator partitions thermal energy equally between its two conjugate (kinetic and potential) reservoirs, halving the effective strain denominator.
