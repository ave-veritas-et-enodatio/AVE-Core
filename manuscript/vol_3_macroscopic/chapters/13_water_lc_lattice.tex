...
\begin{objectivebox}
\begin{itemize}
    \item Derive seven macroscopic water observables from a single Op4 spring constant $k_{\text{hb}} = 2.247$~N/m with no per-observable fitting; the bond geometry and eigenmode are cross-volume derived primaries; the $+4$\textdegree{}C density maximum is a statistical cooperative-lattice result of the two-state LDL/HDL partition, not a closed-form output.
...
This chapter derives \emph{seven} independent macroscopic observables of bulk water from a single first-principles input: the H-bond spring constant $k_\mathrm{hb} = 2E_\mathrm{hb}/d_\mathrm{hb}^2 = 2.247$~N/m, obtained from the Op4 hydrogen bond well depth $E_\mathrm{hb} = 0.2158$~eV and equilibrium displacement $d_\mathrm{hb} = 1.7545$~\AA.  No fitted parameters and no Standard Model constructs are employed. Every prediction traces to the four AVE axioms and the universal operator set. One of the seven --- the $+3.98$\textdegree{}C density maximum --- is \emph{not} a closed-form output: it is a statistical cooperative-lattice average of the two-state LDL/HDL partition (a 3D cooperative-lattice simulation result, not a 1D closed form; see Prediction~2 and \kbleaf{vol3/condensed-matter/ch11-thermodynamics/water-anomaly-lc-partition.md}). The remaining six are closed-form.
...
Surface tension
    & Energy stored in the reflection coefficient at an open-circuit termination ($\Gamma = +1$\gammaundeclared{}). One K4 port is unterminated at the surface. \\
Boiling point
...
Here ``mode'' denotes a full LC oscillator (kinetic plus potential), which stores $k_B T$ by equipartition. This is distinct from the per-quadratic-degree-of-freedom $\tfrac{1}{2}k_B T$ bookkeeping used in the vacuum-lattice heat-capacity accounting; the two conventions count different objects and are not in conflict.

This mapping provides a complete translation between macroscopic fluid thermodynamics and microwave network analysis. Every quantity on the left is computable with the tools on the right.
...
\midrule
$T(\rho_\mathrm{max})$ & $T_0$ (scale) & Cooperative two-state average (statistical) \\
...
% Rule-12 reclassification (2026-06-21, Grant THEORY-CURRENCY ruling): the (1-alpha)
% "Axiom-2 vacuum-lattice loading" is reclassified from a derived chord to an ASSERTED
% alpha-calibration (consistency identification / ECHO). No derivation links the EM
% coupling alpha to hydrogen-bond LC-mode loading. Body preserved per ave-walk-back;
% epistemic-status framing corrected. Canon: water-anomaly-lc-partition.md.
The factor $(1-\alpha)$ is an asserted $\alpha$-calibration loading (a consistency identification, not an independently derived chord: the substrate does not derive that the EM coupling $\alpha$ loads the H-bond LC modes), applied to refine the bare eigenmodes toward their observed values ($\alpha = 7.297 \times 10^{-3}$).
...
In a \textbf{crystal}, all K4 junctions are aligned. The 3D projection averages $\cos^2\theta$ over the tetrahedral geometry, giving $1/\sqrt{3}$. In a \textbf{liquid}, the random orientations of the K4-TLM junctions require averaging the \emph{phase} (not power) of the transmitted wave through the disordered network. Because phase (not intensity) is averaged, the relevant quantity is the mean of the projection factor $\cos\theta$ taken directly over the misalignment angle $\theta$ uniformly distributed across the forward quadrant $[0,\pi/2]$:
\begin{equation}
    \langle \cos\theta \rangle_\mathrm{phase}
    = \frac{\int_0^{\pi/2} \cos\theta\, d\theta}{\int_0^{\pi/2} d\theta}
    = \frac{1}{\pi/2}
    = \frac{2}{\pi}
\end{equation}
This single composite average is the liquid phase velocity factor:
\begin{equation}
\boxed{
...
% Rule-12 demotion (2026-06-21, Grant THEORY-CURRENCY ruling): the +4 degC density
% maximum is a STATISTICAL cooperative-lattice result of the two-state LDL/HDL
% partition, NOT a closed-form T_0(1-alpha) prediction. The boxed closed-form was
% removed; the harmonic eigenmode is retained only as the temperature SCALE.
% Canon: water-anomaly-lc-partition.md (statistical-property derivation + 2026 LLCP).
The bare harmonic eigenmode $T_0 = 279.45$~K sets the temperature \emph{scale} of the density-anomaly domain. The $+3.98$\textdegree{}C density maximum itself, however, is \textbf{not} a closed-form 1D output: it emerges as a \emph{statistical cooperative average} of the two-state liquid partition (expanded low-density-liquid, LDL, vs.\ collapsed high-density-liquid, HDL). Because infinite 3D cooperative melting grids are computationally NP-hard, no single 1D polynomial yields $3.98$\textdegree{}C without explicit 3D cooperative-lattice simulation; the density peak is fundamentally a supercritical statistical average of the dual-geometry competition (\kbleaf{vol3/condensed-matter/ch11-thermodynamics/water-anomaly-lc-partition.md}). The harmonic-peak \emph{scale} (not the closed-form output) is $T_0(1-\alpha) = 279.45 \times 0.99270 = 277.41$~K $= 4.26$\textdegree{}C.

\subsection{Result}
Observed (X-ray LLCP-corroborated): $T(\rho_\mathrm{max}) = 3.98$\textdegree{}C. The statistical cooperative two-state average reproduces this anomaly; the harmonic-peak scale lies $+0.28$\textdegree{}C above it. This is \textbf{statistical agreement} with the observed maximum, not a closed-form prediction error.
...
\textbf{Why saturation $S(r)$ does not apply here}: the enthalpy of vaporisation is a \emph{bulk} quantity that disrupts the complete bond inventory --- it integrates the full Op4 well depth $E_\mathrm{hb}$ from the strained operating point all the way to dissociation. Because the path covers the entire well, the fixed geometric depth is the conserved quantity and the instantaneous restoring force (which $S(r)$ softens) cancels out of the integral. The surface tension of Prediction~5 is by contrast an \emph{interfacial} quantity: it is the reflected energy held at a terminated port at the \emph{finite} operating strain, so it does track the softened stiffness through $S(r)$ and is evaluated at fixed temperature rather than integrated to dissociation. This is the physical criterion that makes $\Delta H_\mathrm{vap}$ saturation-immune while surface tension is saturation-softened. As a consistency check, applying $S(r)$ to $\Delta H_\mathrm{vap}$ ($r = 0.273$, $S = 0.962$ at the boiling point) would \emph{overshoot} to $-2.2\%$ error, confirming that the integrated well depth and the instantaneous spring stiffness are distinct quantities.
...
The calculation of surface tension requires abandoning classical definitions of microscopic ``lever-arms'' and orientational averaging. Instead, the boundary obeys the continuum LC matrix topology mapped via Delesse's stereological principle \cite{delesse1847procede}, by which the fractional area of an ideal 2D mathematical boundary cross-sectioning a 3D phase equals the volume fraction occupied by that phase. The relevant packing fraction here is the substrate topological packing fraction $P_C = p^* = 8\pi\alpha \approx 0.1834$ (the operating-point packing of the chiral K4 lattice), not a water-specific matrix property.
...
When softened by the Axiom~4 saturation operator $S = \sqrt{1 - r_\mathrm{surf}^2}$ at 25$^\circ$C, the derived surface tension is $67.31$ mN/m. The interfacial strain $r_\mathrm{surf} = \sqrt{k_B T / E_\mathrm{hb}}$ carries no factor of $\tfrac{1}{2}$, in contrast to the bulk-oscillator strain $r_\mathrm{bulk} = \sqrt{k_B T / (2 E_\mathrm{hb})}$ used for the eigenmode (Section~\ref{sec:proton_eigenmode}): a surface bond is a single-sided open termination ($\Gamma = +1$\gammaundeclared{}) loaded against the full well depth, whereas a bulk oscillator partitions thermal energy equally between its two conjugate (kinetic and potential) reservoirs, halving the effective strain denominator.
...
For a molecule to decouple from the lattice, all $z/3 = 4/3$ effective K4 junction ports must reach resonance simultaneously. The isotropy factor $z/3$ counts the tetrahedral coordination $z = 4$ projected onto the three spatial channels. It appears here as a \emph{multiplier}: the energy required to excite all 3D channels is $z/3$ times the single-bond eigenmode:
\begin{equation}
\boxed{
...
$T(\rho_\mathrm{max})$            & 4.26\textdegree{}C   & 3.98\textdegree{}C   & $+0.28$\textdegree{}C & two-state cooperative avg.\ \\
...
\begin{summarybox}
\begin{itemize}
    \item Seven independent macroscopic observables of bulk water derive from a single Op4-derived spring constant $k_\mathrm{hb} = 2.247$~N/m; the H-bond-coupled modes carry an asserted $\alpha$-calibration loading, and the $+4$\textdegree{}C density maximum is a statistical cooperative-lattice result.
    \item Three distinct phase boundaries emerge from one eigenmode: the density maximum (cooperative two-state average), melting (single-port saturation, $T_\mathrm{sat}(1-\alpha)$), and boiling (multi-port junction decoupling, $(z/3)\,T_0$).
    ...
    \item The 3D LC lattice topology (via the substrate packing fraction $P_C = 8\pi\alpha$) grounds properties such as surface tension in uniform geometric limits rather than heuristic orientational averages.
\end{itemize}
\end{summarybox}
