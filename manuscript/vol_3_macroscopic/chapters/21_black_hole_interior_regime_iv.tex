...
is the EM horizon, not a $\Gamma = -1$\gammaundeclared{} reflector. The shear and bulk channels
are treated separately: at $r_{sat} = 7GM/c^2 = 3.5\,r_s$ the shear modulus
collapses ($G_{shear} \to 0$, $Z_{shear}, Z_{bulk} \to 0$, $\Gamma = -1$\gammaundeclared{}).
...
\begin{enumerate}
    ...
    \item Unlike particle confinement (asymmetric saturation, $\Gamma = -1$\gammaundeclared{}),
    ...
    \emph{Sign-relativity declaration (2026-08-05; Grant ruling 2026-08-04):}
    the signed $\Gamma_{bulk}$ here carries its \textbf{reference plane},
    \textbf{projection} and \textbf{profile} declarations --- in particular
    ...
    branch, while on the fenced $\rho_{eff} = \rho_0/S^3$ branch (FORK-3(b),
    \textbf{open}) the same $c_{bulk} \to 0$ coexists with
    $Z_{bulk} \to \infty$. Axiom~3 forces only $|\Gamma| = 1$\gammaundeclared{}; the sign is
    computed from a certified instrument's branch-derived wall row, never
    chosen. Canonical: \kbleaf{ave-kb/common/wall-taxonomy.md} \S10.
    \emph{Kernel-collapse self-termination, CHANNEL-SCOPED (Grant ruling
    2026-08-05; \kbleaf{\_orchestration/docket-entries/2026-08-05-ruling-kernel-collapse-rescope.md}, PR~\#897):} at this wall every transport coupling riding the \textbf{Fence re-point, 2026-08-06 --- this routing note now points at the v2 record.} The citable ruled text is the versioned re-issue \kbleaf{\_orchestration/docket-entries/2026-08-06-ruling-kernel-collapse-rescope-v2.md}:13--29, which \textbf{carries the cross-grade combine-member fence inside the ruled text itself}; the 2026-08-05 v1 record is preserved and gains a dated pointer to it. \textbf{The earlier ``the omission is at the ruling \ldots routed to Grant for a possible re-issue'' language is resolved --- the re-issue happened}, so the fence is now carried at the ruling and no print site has to supply it. \textbf{Nothing about the physics moves:} the carve-out is still conditional on the per-grade ($L^\infty$-across-grades) member and the cross-grade combine rule is still canon-OPEN. Delta declaration and the corrected engine-residence map: \kbleaf{\_orchestration/docket-entries/2026-08-06-ruling-kernel-collapse-rescope-v2-correction.md} C1/C2 --- the engine codes the saturation amplitude \textbf{three} ways across two live functionals plus a separate objective, so ``the member the engine actually codes'' is over-broad and the carve-out's receipt is \textsc{structural}, not numerical.
    \textbf{strain kernel} --- $G$, $K$ on \emph{both} density branches, and
    the $u \leftrightarrow \phi$ coupling $G_c$ --- disconnects at the last
    bond, giving total reflection whose phase is computed from the
    branch-derived row \textbf{at the declared plane} and which is
    \textbf{independent of the density profile} (receipts PR~\#888, PR~\#890,
    both Tier-2 verified). \textbf{Certification carried, not compressed:} the
    three load-bearing theorems are measured \emph{exact} (zero, not a
    tolerance), while the discrete row is \textsc{row-not-certified} pending
    the named \texttt{G-RHO2} repair. \textbf{Currency 2026-08-06 --- which run this quotes (doc-lane refresh pass).} The \textsc{row-not-certified} state above is the \textbf{v1 run's} verdict and stands as that run's true measurement (\kbleaf{research/2026-08-05\_last-bond-kernel-collapse\_result.md}:24). \textbf{It is superseded as the current state:} the repair that v1's own \S1.3 named --- inject $k_0 = \varepsilon\,\omega\,Z_1$, not $\varepsilon\,k_{cold}$ --- has run, and TASK~2 is \textsc{row-certified}, read from the landed result document itself rather than from a PR title (\kbleaf{research/2026-08-05\_last-bond-g-rho2-rerun\_result.md}:30; PR~\#902, merge commit \texttt{b06cbeb1}). A certified \emph{row} does \textbf{not} certify the \emph{premise scan}, which stays \textsc{scan-not-certified} with no bin adjudicated, and no compressed ``confirmed'' wording is licensed here. \textbf{The rotational channel is carved
    out:} $\gamma\,S_\kappa$ is \textbf{unwalled at $r_{sat}$} --- its wall is
    a $\kappa$-amplitude surface this strain bias does not reach ($S_\kappa$
    at the wall measured $1$ to every digit double precision carries at
    physical gradients, $0.99998$ only at an unphysical one-node ceiling) ---
    so shear-to-rotation conversion cannot happen \emph{at} the wall and is an
    \emph{approach} question whose size no lane has computed.
    \textbf{Cross-grade fence --- the condition the carve-out rides on, and it
    is NOT in the ruled text of PR~\#897} (added by the documentation lane
    2026-08-05; the omission at the ruling is routed to Grant). The carve-out
    holds on the \textbf{separate-kernel} ($L^\infty$-across-grades) member of
    an \emph{open} fork: canon records the \textbf{cross-grade combine rule as
    underdetermined at $O(\alpha)$} (\kbleaf{ave-kb/common/axiom-register.md}).
    Under that member the strain grade sets the wall and $S_\kappa$ is
    untouched there; under the \textbf{normalized-$L^2$-across-grades} member
    every grade rides \emph{one} kernel and the carve-out does \textbf{not}
    stand. Neither receipt lane chooses the member. Regime~IV stays
    fenced out for every mirrored channel. Canonical:
    \kbleaf{ave-kb/common/wall-taxonomy.md} \S10.2.
...
every scale where a $\Gamma = -1$\gammaundeclared{} boundary forms:
...
\item \textbf{Our cosmic interior $\to$ pre-genesis state:} we sit inside
  our cosmic $\Gamma = -1$\gammaundeclared{} boundary (cosmic horizon $R_H \approx 10^{26}$~m,
...
%
%   :200  family: Z_bulk = rho*c_bulk formula  [BIAS-DEBT]
%        QUOTE (content verified at HEAD; markup-reduced from the banked audit): $Z_{bulk} = \rho_{bulk} c_{bulk} \to 0$ is the constant-$\rho$
%        (RHO-A) branch
%       STAMPED AT: :200
%        RATIONALE (verbatim, research/drivers/r40_sweep_worklist_verified.json): Sign-relativity
%        declaration is carve-friendly (Axiom 3 forces only |Gamma|=1) but both declared branches compute
%        Z_bulk from a bulk propagation speed; formula-level re-derivation owed — the RHO-A vs FORK-3(b)
%        profile fork itself may be mooted when c_bulk is not a speed.
