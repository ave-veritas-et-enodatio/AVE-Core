...
\item \textbf{Our cosmic interior $\to$ pre-genesis state:} we sit inside
  ...
  scale). The mechanism that set those observables at lattice genesis
  (\textit{``God's Hand''} in the AVE canonical framing) is fundamentally
  inaccessible from inside.

\item \textbf{Same epistemic horizon, different scales.} The crystallization
  phase transition that produced our substrate IS the wall --- no observer
  exists within the resulting substrate from a position outside it. This
  is structurally identical to a Kerr BH astronomer's relationship to the
  interior: they characterize the boundary, not what set it.
\end{itemize}

This cross-cutting framing connects Ch~\ref{ch:bh_interior} to Vol~3 Ch~4
(Generative Cosmology cosmic-boundary canonical), AVE-QED App~F
(multi-scale Machian network), and the canonical
\texttt{trampoline-framework.md} \S{}1.3.7 (\textit{God's Hand and the cosmic IC}).
The substrate-observability rule is canonical at every scale AND canonical
when applied to ourselves --- the framework's irreducible epistemic horizon.
