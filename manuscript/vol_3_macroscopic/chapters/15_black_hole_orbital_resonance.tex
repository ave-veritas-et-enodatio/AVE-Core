...
\textbf{A-034 gravitational anchors (SYM).} This chapter contains
TWO of A-034's strongest empirical anchors in the 26-instance catalog:
(1) BH event horizon at $\varepsilon_{11}(r) = 2GM/(c^2 r) = 1$ matches
the Schwarzschild radius \textbf{exactly} (canonical AVE-native derivation);
(2) the Schwarzschild BH merger ring-down QNM eigenvalue
$\omega_R M_g = 18/49 = 0.3673$ agrees with GR exact $0.3737$ to
\textbf{1.7\%} with \textbf{zero free parameters} --- the \textbf{cold
Schwarzschild ($a_* = 0$) single-point} anchor, and per Grant Ruling~B1
(2026-07-21) the \emph{only} surviving zero-free-parameter ring-down content.
\textbf{Reconciled per-occurrence 2026-08-05 (Ruling B1, \#780; prior wording
in the git trail).} What this paragraph previously booked here --- a
$-0.45\%$ mean frequency / $-0.47\%$ mean decay-time agreement against three
LIGO events (GW150914, GW170104, GW151226) via the Phase-3+5 v2 formula, read
inside a ``validated only for $a_* = 0.3$--$0.8$'' band --- is
\textbf{retracted} as a frame-mixed match artifact, and \textbf{no
replacement number is claimed}. Full detail and the driver
\kbleaf{src/scripts/vol\_3\_macroscopic/ligo\_ringdown\_driver.py} below.
...
(NOAA validation pending live fetch), and cosmic K4 crystallization. The BH event horizon is
...
In the AVE framework, the electron is a self-trapped photon---a topological $0_1$ unknot confined at the lattice node scale ($\ell_{node}$) by a $\Gamma = -1$\gammaundeclared{} total internal reflection boundary. The event horizon of a Schwarzschild black hole is governed by isomorphic confinement physics operating at cosmological scale via a \textbf{lattice phase transition}. This phase transition is \emph{channel-resolved}: the EM-transverse channel stays matched ($\Gamma_{EM} = 0$, symmetric gravity $Z_{EM} \equiv Z_0$), while the shear and bulk channels undergo an impedance collapse ($G_{shear} \to 0 \Rightarrow Z_{shear}, Z_{bulk} \to 0 \Rightarrow \Gamma_{shear} = \Gamma_{bulk} = -1$) at the deeper strain-unity boundary $r_{sat} = 7GM/c^2$ (developed in the Saturation Boundary subsection below). The earlier ``not an impedance mismatch'' framing was the EM channel only; the shear/bulk channels do short.
...
\paragraph{Kernel-collapse self-termination at the wall --- CHANNEL-SCOPED (Grant ruling 2026-08-05).} The reflection above is now carried by a \emph{measured wall row}, and its scope is channel-scoped rather than universal. At any strain-saturation wall, every transport coupling that rides the \textbf{strain kernel} --- the shear modulus $G$, the bulk modulus $K$ on \emph{both} branches of the open density fork, and the $u \leftrightarrow \phi$ coupling $G_c$ --- disconnects at the last bond: \textbf{total reflection, with the phase computed from the branch-derived row at the declared plane, and independent of the density profile}. Receipts: PR~\#888 and PR~\#890, both Tier-2 verified; the ruled scope is the kernel-collapse re-scope ruling, \kbleaf{\_orchestration/docket-entries/2026-08-05-ruling-kernel-collapse-rescope.md} (PR~\#897, landed). \textbf{Certification status, carried and not compressed} (the standing print-certification rule, 2026-08-05): the three load-bearing theorems are \emph{measured exact} --- last-bond stiffness $0.0$, residual $\lvert\Gamma_{LB}+1\rvert = 0.0$ over $1680$ swept harmonic points, zero spread across the entire beyond-wall grid, and zero separation between the two density branches --- while the discrete \emph{row} is \textsc{row-not-certified}, pending the named \texttt{G-RHO2} repair (an injection point sized wrong at freeze; the repair is named, the tolerance was not moved and the injections were not re-sited). \textbf{And the rotational channel is carved out --- the carve-out is part of the claim, not a footnote to it.} The Cosserat micro-rotation transport $\gamma\,S_\kappa$ is \textbf{unwalled at $r_{sat}$}: its own wall is a $\kappa$-\emph{amplitude} surface, which this DC strain bias does not reach. Measured, $S_\kappa$ at the wall is $1$ to every digit double precision carries at every physical gradient scale, and $0.99998$ only at an unphysical one-node ceiling. Shear-to-rotation conversion therefore cannot occur \emph{at} the wall --- the door rides the strain kernel and closes with it --- and is confined to the graded approach, where $G_c S_\varepsilon$ is finite; its size is one computable number and \textbf{no lane has computed it}. \textbf{Cross-grade fence --- the condition the carve-out rides on, and it is NOT in the ruled text of PR~\#897} (added by the documentation lane 2026-08-05; the omission at the ruling is routed to Grant). The carve-out holds on the \textbf{separate-kernel} ($L^\infty$-across-grades) member of an \emph{open} fork: canon records the \textbf{cross-grade combine rule as underdetermined at $O(\alpha)$} (\kbleaf{ave-kb/common/axiom-register.md}). Under that member the strain grade sets the wall and $S_\kappa$ is untouched there; under the \textbf{normalized-$L^2$-across-grades} member every grade rides \emph{one} kernel and the carve-out does \textbf{not} stand. Neither receipt lane chooses the member. Regime~IV stays fenced out for every mirrored channel. Canonical: \kbleaf{ave-kb/common/wall-taxonomy.md} \S10.2. \textbf{Currency 2026-08-06 --- which run this quotes (doc-lane refresh pass).} The \textsc{row-not-certified} state above is the \textbf{v1 run's} verdict and stands as that run's true measurement (\kbleaf{research/2026-08-05\_last-bond-kernel-collapse\_result.md}:24). \textbf{It is superseded as the current state:} the repair that v1's own \S1.3 named --- inject $k_0 = \varepsilon\,\omega\,Z_1$, not $\varepsilon\,k_{cold}$ --- has run, and TASK~2 is \textsc{row-certified}, read from the landed result document itself rather than from a PR title (\kbleaf{research/2026-08-05\_last-bond-g-rho2-rerun\_result.md}:30; PR~\#902, merge commit \texttt{b06cbeb1}). A certified \emph{row} does \textbf{not} certify the \emph{premise scan}, which stays \textsc{scan-not-certified} with no bin adjudicated, and no compressed ``confirmed'' wording is licensed here. \textbf{Fence re-point, 2026-08-06 --- this routing note now points at the v2 record.} The citable ruled text is the versioned re-issue \kbleaf{\_orchestration/docket-entries/2026-08-06-ruling-kernel-collapse-rescope-v2.md}:13--29, which \textbf{carries the cross-grade combine-member fence inside the ruled text itself}; the 2026-08-05 v1 record is preserved and gains a dated pointer to it. \textbf{The earlier ``the omission is at the ruling \ldots routed to Grant for a possible re-issue'' language is resolved --- the re-issue happened}, so the fence is now carried at the ruling and no print site has to supply it. \textbf{Nothing about the physics moves:} the carve-out is still conditional on the per-grade ($L^\infty$-across-grades) member and the cross-grade combine rule is still canon-OPEN. Delta declaration and the corrected engine-residence map: \kbleaf{\_orchestration/docket-entries/2026-08-06-ruling-kernel-collapse-rescope-v2-correction.md} C1/C2 --- the engine codes the saturation amplitude \textbf{three} ways across two live functionals plus a separate objective, so ``the member the engine actually codes'' is over-broad and the carve-out's receipt is \textsc{structural}, not numerical.
...
Impedance (per channel) & $Z_{bulk} \to 0$ at knot core & $Z_{EM} = Z_0$ (SYM); $Z_{shear}, Z_{bulk} \to 0$ at $r_{sat}$ \\
\bottomrule
\end{tabularx}
\end{center}
...
The AVE impedance band solver ({\kbleaf{orbital\_resonance.py}}) predicts quantised mode frequencies at each standing-wave resonance radius. Table~\ref{tab:qpo_comparison} compares the predicted mode spectrum against four systems with published RXTE twin HF QPO detections.
...
Frame-dragging shifts the prograde saturation boundary inward, but the K4 Cosserat lattice provides a rigid skeleton fraction that does NOT yield to rotational stress. Per the (2,3) torus-knot topology shared between electron and BH (KB \kbleaf{electron-bh-isomorphism.md}), the cavity has TWO components: a rigid $\nu_{vac}$ fraction set by K4 elasticity, plus a compliant $(1 - \nu_{vac})$ fraction that scales with the photon-orbit radius.

\begin{resultbox}{Kerr-Corrected Ringdown (v2 --- Cosserat back-reaction, 2026-05-18)}
\begin{equation}
...
\noindent\textbf{Limits}: at $a_* = 0$ (Schwarzschild), $r_{ph}^+ = 3M \Rightarrow x_{sat} = 7$ recovering the cold eigenvalue $18/49$. At $a_* \to 1$ (extremal), $r_{ph}^+ \to M \Rightarrow x_{sat} \to 2 + 5/3 \approx 3.67$ (cavity floored by Cosserat elasticity, not pure photon sphere). \textbf{Superseded v1 formula} (pre-2026-05-18, over-predicted spin correction by $\sim$13\% mean): $f_{ring}(a_*) = f_{ring}(0) \cdot r_{ph,\text{Schw}}/r_{ph}^+(a_*)$. Treated entire cavity as compliant (no rigid skeleton fraction); diagnosed via Phase-2 LIGO ringdown comparison and refined per Grant adjudication 2026-05-18 (Cosserat Poisson-ratio back-reaction).

\subsection{Kerr Quality Factor}

For a spinning remnant, the continuous topological strain gradient convolutes asymmetrically with the QNM mode, reducing the effective radiation rate.  The v1 decay rate was obtained from the non-reciprocal phase shift:
\begin{equation}
    \omega_I = \frac{\omega_R - m\,\Omega}{2\,\ell}, \qquad
    r_\Omega = r_{ph}(a_*) \cdot \sqrt{1 + \nu_\mathrm{vac}}
\end{equation}
where $\Omega$ is the asymmetric impedance convolution rate (formerly interpreted as Lense-Thirring angular velocity) at the Poisson-augmented photon sphere $r_\Omega$.

\paragraph{Phase-5 refinement (2026-05-18, lattice-Q preservation).} The v1 formula gave $\tau_{v1}$ values that systematically under-predicted LIGO observed $\tau$ by 10--14\% per event. Per Cosserat-skeleton reasoning: the rigid K4 lattice impedance sets the damping Q-factor independent of the spin-dependent cavity radius; Q is therefore invariant across the v1$\to$v2 cavity-radius refinement. Cavity-radius shift propagates inversely into $\tau$ via Q-preservation:
\begin{resultbox}{v2 Decay Time via Lattice-Q Preservation}
\begin{equation}
    Q_{v2} = Q_{v1} \quad\Longrightarrow\quad \tau_{v2}(a_*) = \tau_{v1}(a_*) \cdot \frac{\omega_{R,v1}(a_*)}{\omega_{R,v2}(a_*)}
\end{equation}
\end{resultbox}
\noindent The same rigid-skeleton mechanism that PARTIALLY resists spin-induced cavity shrinkage in v2 $\omega_R$ is the \emph{full} mechanism behind Q preservation. This $\tau$ value is a \textbf{post-hoc Q-preservation refinement} (2026-05-18): the v1 formula under-predicted the LIGO-observed $\tau$ by 10--14\% per event, and the lattice-Q-preservation step was introduced afterward to reconcile the discrepancy. It is \textbf{not} a parameter-free forward prediction of the decay time; unlike the Schwarzschild $\ell=2$ eigenvalue above, it is a fitted reconciliation of the spinning-remnant case against the same LIGO data it is compared to.

\begin{tcolorbox}[colback=white, colframe=black, title=\textbf{Scope: post-hoc consistency check, not a forward discriminator}]
% [2026-08-05 #780 per-occurrence reconciliation EXECUTED in the body of this box -- the retracted figures are no longer quoted here as current truth-source text.]
The Schwarzschild $\ell=2$ QNM eigenvalue $18/49 \approx 0.3673$ (vs GR exact $0.3737$, \textbf{1.7\% error}, \textbf{zero free parameters}) is the genuine forward prediction of this section, and per Grant Ruling~B1 (2026-07-21) it is the \emph{only} surviving zero-free-parameter ring-down content --- the \textbf{cold Schwarzschild $a_* = 0$ single point}. The \emph{spinning-remnant} (Kerr) comparison against LIGO below is a \textbf{post-hoc consistency check}, not a forward AVE-vs-GR discriminator; its per-event agreement is achieved \emph{after} the Phase-5 Q-preservation fit and must be read accordingly. What the truth-source card (\kbleaf{manuscript/ave-kb/vol3/claim-quality.md} \texttt{clm-395gps}) still carries unstruck is that the Kerr correction ``inherits the Schwarzschild AVE structure via a phenomenological photon-sphere shift formula''. \textbf{Reconciled per-occurrence 2026-08-05 (Ruling B1, \#780; prior wording in the git trail).} This box previously quoted that same card for a \textbf{10--18\%} frequency / 10--14\% decay-time spinning-remnant scope and for a ``matches GR sub-2\% only for $a_* = 0.3$--$0.8$'' Kerr-quality-factor band. \textbf{Both clauses are struck at the card itself} (\texttt{:202} corrected, \texttt{:204} retracted) as frame-mixed pre-correction artifacts computed against a corrupt Kerr table and source-frame masses (\#774), and \textbf{no replacement number is claimed here}.
\end{tcolorbox}

\medskip\noindent
Post-hoc consistency comparison against three LIGO detections (Phase-3 v2 $\omega_R$ + Phase-5 v2 $\tau$, 2026-05-18):

\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabularx}{\textwidth}{@{}l c c c c c c c@{}}
\toprule
\textbf{Event} & $\mathbf{M_{final}}$ & $\mathbf{a_*}$ & \textbf{$f_\mathrm{AVE\text{-}v2}$} & \textbf{$f_\mathrm{obs}$} & $\boldsymbol{\Delta f}$ & \textbf{$\tau_\mathrm{AVE\text{-}v2}$} & \textbf{$\tau_\mathrm{obs}$} \\
\midrule
GW150914 & 62.0 $M_\odot$ & 0.67 & \textbf{246.0 Hz} & 251 Hz & \textbf{$-2.0$\%} & \textbf{3.95 ms} & 4.0 ms \\
GW170104 & 48.7 $M_\odot$ & 0.64 & \textbf{308.2 Hz} & 312 Hz & \textbf{$-1.2$\%} & \textbf{3.02 ms} & 3.0 ms \\
GW151226 & 20.8 $M_\odot$ & 0.74 & \textbf{764.0 Hz} & 750 Hz & \textbf{$+1.9$\%} & \textbf{1.39 ms} & 1.4 ms \\
\bottomrule
\end{tabularx}
\end{center}
% [2026-08-05 #780 per-occurrence reconciliation EXECUTED on the line below -- the table above is retained as the printed record of the superseded comparison, per Rule 12.]
\noindent \textbf{Reconciled per-occurrence 2026-08-05 (Ruling B1, \#780; prior wording in the git trail).} This line previously banked the table above as a \textbf{frequency mean $-0.45\%$, decay-time mean $-0.47\%$ per event} result. Both means are \textbf{retracted} as frame-mixed compensating-error artifacts (\#774), the table above is retained only as the printed record of the superseded comparison, and \textbf{no replacement number is claimed}. Live-fire driver at \kbleaf{src/scripts/vol\_3\_macroscopic/ligo\_ringdown\_driver.py}; full Phase-3+5 outputs in research \kbleaf{ligo-ringdown-driver-design.md} \S 8--10.

\paragraph{Phase-4 spin sweep + mass-independence --- RETRACTED, and what replaced it.} \textbf{Reconciled per-occurrence 2026-08-05 (Ruling B1, \#780; prior wording in the git trail).} This paragraph previously reported that the v2 mapping generalizes across the Berti+Cardoso+Will 2006 Kerr QNM curve with 9 of 11 swept spin values $a_* \in \{0.0, 0.1, \ldots, 0.95\}$ passing at $|\text{dev}| < 3\%$, with per-event mass-independence hits on GW190521 and GW170729, inside a ``validated spin range $a_* = 0.3$--$0.8$''. \textbf{The entire sweep was run against the corrupt Kerr table} (\#774): against corrected Kerr the v2 mapping sits below true Kerr across $a_* = 0.6$--$0.95$ and \textbf{no PASS survives} (\kbleaf{ave-kb/vol3/cosmology/ch15-black-hole-orbitals/ave-merger-ringdown-eigenvalue.md}). \textbf{No replacement per-event number is claimed.} What Ruling~B1 puts in its place is not a catalog match but a forward prediction on the same axis: near-extremal remnants are \textbf{no longer deferred} to Option~B (full spheroidal cavity) --- the standing v1 mapping ($x_{sat,v1} = 7\,r_{ph}^+/3M$) gives $\omega_R M_g \to 54/49 \approx 1.102$ as $a_* \to 1$, a \textbf{$+10.2\%$ overshoot of the GR zero-damped-mode limit $m/2 = 1$}, promoted under B1 from routed candidate to \textbf{LIVE FORWARD PREDICTION} testable at $a_* \gtrsim 0.9$ (none in the current catalog, whose maximum is $a_* \approx 0.74$). \textbf{Provenance rider}: the AVE-distinct content is the \emph{deviation form} --- the overshoot rising toward the ZDM limit, against the retired v2 mapping's floor --- while the absolute $54/49$ embeds the GR-imported $\nu_{vac} = 2/7$ via $K = 2G$ (\kbleaf{ave-kb/common/form-deriving-value-importing.md}; PR~\#261). Canonical: \kbleaf{ave-kb/vol3/cosmology/ch15-black-hole-orbitals/first-principles-predictions.md}.
...
The rupture physics described above is implemented in two modules of the \kbleaf{regime\_4\_rupture/} package:
\begin{itemize}
    \item \kbleaf{rupture\_solver.py}: Computes the lattice rupture state
    ...
    \item \kbleaf{black\_hole\_jets.py}: Models the energy redirection
...
\subsection{Gravitational Wave Memory: A Null Prediction on the Substrate's Latch Channel}

\noindent\textbf{Re-scoped 2026-08-05 (Ruling R1, 2026-08-03; prior wording in the git trail).} This subsection previously read the observed metric offset as the \emph{permanent plastic deformation} of the LC lattice driven past its elastic limit, with a boxed $\Delta h_{memory} = h_{peak}(h_{peak}/h_{yield})^2$ scaling law and a metal-yield analogy. \textbf{That reading is retracted on three independent counts} --- parity, anhysteresis and magnitude --- and the boxed law goes with it: a boxed result is a claim, and this quantity is not derived. What replaces it is narrower and testable.

After a gravitational wave passes, the local metric is observed to retain an offset---so-called ``memory'' or residual strain. General Relativity predicts this (the Christodoulou memory, $\sim 10^{-22}$ for a GW150914-class event) as a second-order effect of the wave's own \emph{energy flux}. \textbf{The AVE substrate, as canonized, contributes no memory of its own through its constitutive latch}---and that is a testable statement.

\noindent\textbf{The substrate cannot rectify.} The Axiom~4 kernel $S(A) = \sqrt{1 - A^2}$ is \emph{even} in $A$, so the modulated strain response is \emph{odd} in $h$. An odd nonlinearity driven by a symmetric wave produces odd harmonics and \textbf{exactly zero DC}: $\cos^3\omega t = \tfrac34\cos\omega t + \tfrac14\cos 3\omega t$, so $\langle\cos^3\omega t\rangle = 0$. Rectification requires a diode; a lossless, even, single-valued substrate does not have one.

\noindent\textbf{The substrate cannot retain.} The same kernel is \textbf{anhysteretic}---zero enclosed loop area, hence no remanence (\kbleaf{ave-kb/common/engine-capability-map.md} \S3.3). The response is a single-valued function of the instantaneous strain, so $h \to 0$ returns it identically to zero for \emph{every} drive history. Nothing is held.

\noindent\textbf{The bound, if one insists on a residual.} The most generous estimate---taking the entire leading sub-yield nonlinearity and treating it as though it accumulated---is $\tfrac12 h_{peak}(h_{peak}/h_{yield})^2$, which for GW150914 ($h_{peak} = 10^{-21}$, $h_{yield} = \sqrt{\alpha} = 0.0854$, so $h_{peak}/h_{yield} = 1.2\times10^{-20}$) is $6.9\times10^{-62}$: about \textbf{39 orders of magnitude} below the GR value. Both the strict statement (zero) and the generous bound sit far below anything observable.

\begin{resultbox}{Forward Null Prediction --- Substrate Latch Contribution to GW Memory}
\begin{equation}
    \Delta h_{\text{memory}}^{\text{latch}} = 0
    \quad\text{(strictly)}, \qquad
    \left|\Delta h^{\text{latch}}\right| \lesssim
...
\noindent\textbf{What would falsify the latch null.} A LISA or pulsar-timing-array detection of gravitational-wave memory \emph{in excess of} the GR/Christodoulou prediction, scaling as $h^3$ with an $h_{yield} = \sqrt{\alpha}$ normalisation, would falsify the lossless-anhysteretic substrate of this framework. A detection \emph{at} the GR level discriminates nothing here until the flux-channel derivation lands: that is what the open carve above means, and it is stated rather than left for a reader to discover.
...
\begin{figure}[h!]
    ...
    \caption{\textbf{Untapped Impedance-Topology Predictions.} Panels derived from the AVE impedance topology; the Schwarzschild $\ell=2$ QNM eigenvalue is zero-free-parameter. \textbf{RETRACTED-CONTENT NOTICE (2026-08-05).} \textbf{Two of the six plotted panels plot retracted quantities, and this raster has deliberately \emph{not} been re-plotted}: the retractions carry \emph{no replacement numbers} by ruling, so regenerating the panels would require minting content no receipt supplies. \textbf{(1) Merger ringdown vs.\ LIGO --- the plotted spinning-remnant comparison is RETRACTED} as a frame-mixed compensating-error artifact (Ruling~B1, \#774/\#780); the curve and its per-event agreement are the printed record of a superseded comparison, not a current result, and no replacement is claimed. \textbf{(5) GW memory --- the plotted ``residual lattice deformation above $h_{yield} = \sqrt{\alpha}$'' law is RETRACTED} on parity, anhysteresis and magnitude (Ruling~R1, 2026-08-03); the substrate's \emph{latch} contribution is a null $\lesssim 10^{-61}$ against a GR expectation $\sim 10^{-22}$, and the source-side flux channel is an \emph{open} derivation --- see the re-scoped subsection above. Panels (2) Iron K$\alpha$ line profile showing discrete sub-peaks at impedance band energies, (3) jet impedance map (polar impedance matching, $\Gamma \approx 0$, produces a natural jet channel), (4) Hawking temperature from impedance boundary noise leakage, and (6) scale invariance of the same $1/d$ topology from the electron unknot ($10^{-13}$ m) to M87* ($10^{13}$ m) are unaffected by either retraction.}
    \label{fig:untapped}
\end{figure}
...
\midrule
1 & LC lattice, $Z_0 = \sqrt{\mu_0/\varepsilon_0}$ & \textbf{Full} & Symmetric Gravity (EM channel $Z_{EM} = Z_0$) \\
...
4 & Saturation ($V_{SNAP}$, viscosity) & \textbf{Full} & Phase transition, $Q = \ell$ (cold $a_* = 0$ anchor --- B1), $\tau_{ring}$ \\
\bottomrule
\end{tabularx}
\end{center}
...
\noindent\textbf{Spin scope (Ruling~B1, 2026-07-21; propagated to print 2026-08-05).} The table below applies the cold $a_* = 0$ anchor \emph{outside its scope}: all three events sit at $a_* = 0.64$--$0.72$, so the flat $Q = \ell = 2$ column is the cold anchor read through at catalog spins --- exactly the reading B1 scoped out, and the $38$--$43\%$ $\tau$ errors are its signature. \textbf{Nothing in the table is re-adjudicated}; it stays as the printed record. Under B1 the standing spin story is the m$\Omega$ law $\omega_I = (\omega_R - m\Omega)/(2\ell)$, an \textbf{OPEN near-miss tension} and the named next ring-down work --- not an un-attempted correction. \textbf{Open flag, not a ruling} (\#808 F7/F8; B1 did not adjudicate it): even the cold value is convention --- the $1/\ell$-per-cycle leak constant is a scaling assertion, never a computed radiated power, and the cold-$Q$ derivation is the named next work. Canonical: \kbleaf{ave-kb/vol3/cosmology/ch15-black-hole-orbitals/axiom-coverage-audit.md}.

\medskip\noindent
Comparison against three LIGO events at the bare Schwarzschild $Q = \ell$ approximation (no spin correction):
...
\end{tabularx}
\end{center}
\noindent\textbf{Spin scope (Grant Ruling~B1, 2026-07-21; reconciled per-occurrence 2026-08-05, \#780 --- prior wording in the git trail).} The bare Schwarzschild $Q = \ell = 2$ eigenvalue above is the cold-cavity forward prediction (zero free parameters) \textbf{at the cold $a_* = 0$ anchor, and only there}: it is the $\Omega \to 0$ limit of the spin-dependent damping law $\omega_I = (\omega_R - m\Omega)/(2\ell)$, and at the catalog spins of the table above the flat reading would fail badly --- \textbf{the spin story is the m$\Omega$ law, not flat $Q = \ell$} (\kbleaf{ave-kb/vol3/cosmology/ch15-black-hole-orbitals/qnm-quality-factor.md}; the axiom-manifestation Op21 mode-counting content of $Q = \ell$ is unchanged by B1 --- only its spin scope is pinned). The spinning-remnant Kerr correction to $Q$ remains a \textbf{post-hoc refinement}: the v1$\to$v2 lattice-Q preservation mechanism (Cosserat skeleton sets the Q-factor invariant of cavity-radius refinement) was introduced after the fact to reconcile the v1 shortfall against LIGO data; see the \emph{Kerr Quality Factor} subsection above for the v2 formula $\tau_{v2} = \tau_{v1} \cdot (\omega_{R,v1}/\omega_{R,v2})$. This paragraph previously banked that comparison at a $-0.47\%$ mean $\tau$ error per event against a $-6.94\%$ standard-GR contrast, inside a ``validated only for $a_* = 0.3$--$0.8$'' band. \textbf{All three figures are retracted} --- the $\tau$ chain rides the same source-frame masses and a separately-corrupt $\omega_I$ table, so neither the match nor the outperforms-GR contrast survives --- and \textbf{no replacement number is claimed}.
...
Classical General Relativity predicts that matter falling into a black hole will accelerate indefinitely, culminating in a point singularity at $r = 0$ characterized by infinite spatial curvature and infinite mass density in zero volume. In the AVE topological framework, the classical geometry is bounded by the physical elastic limit of the substrate LC resonant lattice.
...
%
%   :31  family: ringdown polar solver (bulk-outgoing condition)  [BIAS-DEBT]
%        QUOTE (content verified at HEAD; markup-reduced from the banked audit): two channels radiating at different speeds mean a single
%        shear-channel outgoing factor leaves the bulk channel's residual amplitude suppressed
%        \emph{beyond all orders}
%       STAMPED AT: :31
%        RATIONALE (verbatim, research/drivers/r40_sweep_worklist_verified.json): The polar-family
%        instrument's radial equation and outgoing boundary condition consume a bulk radiation branch at
%        k_P; under the carve the instrument premise is void and the polar cavity-mode question is owed a
%        constraint-native (divergence-slaved) re-derivation — plausibly explaining the lane's
%        no-stable-root pathology.
...
%
%   :31  family: ringdown spheroidal P-SV fundamental  [BIAS-DEBT]
%        QUOTE (content verified at HEAD; markup-reduced from the banked audit): with the P and SV polarizations \emph{coupled}, so the compressional
%        A1 dilatation channel and the shear channel move together
%       STAMPED AT: :31
%        RATIONALE (verbatim, research/drivers/r40_sweep_worklist_verified.json): The
%        toroidal-vs-fundamental carve claims the un-built spheroidal branch (P-SV coupled, elastodynamic
%        analogue) carries the true ell=2 fundamental; the P polarization content dies under the carve,
%        and which polar family exists must be re-derived — the 'read ring-down as a cavity mode' hedge
%        survives.
%        RESOLUTION: the demoted carrier is the propagating A1 / bulk branch; under Axiom 5 clause G that
%        slot is the bound response, so the re-derivation must be re-posed on the bound-sector
%        constitutive law (bias eps_11, bound response u_0, mechanism gloss back-reaction) rather than on
...
%        replacement is owed, not held.
%
%   :57  family: electron-sector label (bulk-channel TIR)  [BIAS-DEBT | PAST-WALL]
%        QUOTE (content verified at HEAD; markup-reduced from the banked audit): the electron's confinement (bulk-channel TIR, $\Gamma_{bulk} = -1$
%        at the knot boundary where $Z_{bulk} \to 0$)
%       STAMPED AT: :57
%        RATIONALE (verbatim, research/drivers/r40_sweep_worklist_verified.json): Electron knot-core
%        cage: total-internal-reflection of what incident bulk wave? Label-level re-derivation with
%        past-wall scope declared (prereg's electron-sector family); the boundary statement itself
%        survives as |Gamma|=1.
%        RESOLUTION: the demoted carrier is the propagating A1 / bulk branch; under Axiom 5 clause G that
%        slot is the bound response, so the re-derivation must be re-posed on the bound-sector
%        constitutive law (bias eps_11, bound response u_0, mechanism gloss back-reaction) rather than on
...
%        nor adjudicated.
%
%   :68  family: Z_bulk = rho*c_bulk formula  [BIAS-DEBT]
%        QUOTE (content verified at HEAD; markup-reduced from the banked audit): The bulk-longitudinal channel collapses with it ($c_{bulk} \to 0
%        \Rightarrow Z_{bulk} \to 0 \Rightarrow \Gamma_{bulk} = -1$)
%       STAMPED AT: :68
%        RATIONALE (verbatim, research/drivers/r40_sweep_worklist_verified.json): The reflection survives
%        sign-invariant, but the mechanism chain runs through c_bulk as a propagation speed and Z_bulk =
%        rho_bulk*c_bulk (RHO-A) / sqrt(K*rho_eff) (FORK-3(b)) — the prereg's canonical formula-level
%        NEEDS-RE-DERIVATION example.
%        RESOLUTION: the demoted carrier is the propagating A1 / bulk branch; under Axiom 5 clause G that
%        slot is the bound response, so the re-derivation must be re-posed on the bound-sector
%        constitutive law (bias eps_11, bound response u_0, mechanism gloss back-reaction) rather than on
%        a compression wave. BIAS-DEBT: this row turns on finite-speed bias dynamics, so the resolution
...
%        replacement is owed, not held.
%
%   :344  family: GW-memory A1 flux route  [BIAS-DEBT]
%        QUOTE (content verified at HEAD; markup-reduced from the banked audit): that energy sources the A1 dilatation channel. Whether AVE
%        reproduces the GR/Christodoulou value through that flux channel is an OPEN derivation
%       STAMPED AT: :344
%        RATIONALE (verbatim, research/drivers/r40_sweep_worklist_verified.json): Surviving content:
%        memory is source-side and the medium-latch null stands. But the OPEN route is framed as an A1
%        'flux channel' — a port the carve removes; the re-derivation should be a bound-response
%        DC-displacement statement (T2 energy density in the driver inventory), routed.
%        RESOLUTION: the demoted carrier is the propagating A1 / bulk branch; under Axiom 5 clause G that
%        slot is the bound response, so the re-derivation must be re-posed on the bound-sector
%        constitutive law (bias eps_11, bound response u_0, mechanism gloss back-reaction) rather than on
%        a compression wave. BIAS-DEBT: this row turns on finite-speed bias dynamics, so the resolution
