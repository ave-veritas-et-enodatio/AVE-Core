...
\begin{figure}[h!]
    ...
    \caption{\textbf{Untapped First-Principles Predictions.} All panels derived from the AVE impedance topology with zero free parameters. (1) Merger ringdown frequency vs. LIGO observations: v2 Cosserat-back-reaction formula achieves \textbf{$-0.45\%$ mean $\omega_R$ + $-0.47\%$ mean $\tau$} per event (Phase 3+5 closure 2026-05-18; outperforms GR Kerr QNM for $\tau$). (2) Iron K$\alpha$ line profile showing discrete sub-peaks at impedance band energies. (3) Jet impedance map: polar impedance matching ($\Gamma \approx 0$) produces a natural jet channel. (4) Hawking temperature from impedance boundary noise leakage. (5) GW memory strain as residual lattice deformation above $h_{yield} = \sqrt{\alpha}$. (6) Scale invariance: the same $1/d$ topology from the electron unknot ($10^{-13}$ m) to M87* ($10^{13}$ m).}
    \label{fig:untapped}
\end{figure}
...
\end{tabularx}
\end{center}
\noindent\textbf{Updated 2026-05-18 (Phase 5 closure):} the bare Schwarzschild $Q = \ell = 2$ approximation above is the cold-cavity prediction; the Kerr correction to $Q$ \textbf{IS} now derived via the v1$\to$v2 lattice-Q preservation mechanism (Cosserat skeleton sets Q-factor invariant of cavity-radius refinement). See the upgraded \emph{Kerr Quality Factor} subsection above for the v2 formula $\tau_{v2} = \tau_{v1} \cdot (\omega_{R,v1}/\omega_{R,v2})$ and the corresponding LIGO comparison table at $-0.47\%$ mean $\tau$ error per event (outperforms standard GR Kerr QNM at $-6.94\%$). The 38-43\% errors above are an artifact of the cold $Q = \ell$ approximation, NOT a residual gap in the AVE framework.
