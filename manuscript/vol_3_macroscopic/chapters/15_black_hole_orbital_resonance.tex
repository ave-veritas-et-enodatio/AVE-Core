...
% [2026-07-21 #780 pointer: the -0.45%/-0.47% and 10-18%/sub-2% figures quoted below are RETRACTED (MATCH-ARTIFACT, #774; register corrected #780) -- see the dated correction note near the section head; per-occurrence reconciliation routed to the manuscript-reconciliation program.]
The Schwarzschild $\ell=2$ QNM eigenvalue $18/49 \approx 0.3673$ (vs GR exact $0.3737$, \textbf{1.7\% error}, \textbf{zero free parameters}) is the genuine forward prediction of this section. The \emph{spinning-remnant} (Kerr) comparison against LIGO below is a \textbf{post-hoc consistency check}, not a forward AVE-vs-GR discriminator. Per the truth-source card (\kbleaf{manuscript/ave-kb/vol3/claim-quality.md} \texttt{clm-395gps}): AVE ``does NOT claim sub-percent agreement on ringdown frequency for spinning remnants. The Kerr-corrected LIGO ringdown comparisons \dots show \textbf{10--18\%} frequency error and 10--14\% decay-time error, not the sub-percent precision the Schwarzschild case suggests.'' The Kerr correction ``inherits the Schwarzschild AVE structure via a \textbf{phenomenological photon-sphere shift formula}''; and the Kerr quality factor ``matches GR sub-2\% only for $a_* = 0.3$--$0.8$; behaviour outside this spin range is not validated.'' The sub-percent per-event agreement in the table below is achieved \emph{after} the Phase-5 Q-preservation fit and should be read accordingly.
\end{tcolorbox}
...
\end{center}
% [2026-07-21 #780 pointer: the -0.45%/-0.47% and 10-18%/sub-2% figures quoted below are RETRACTED (MATCH-ARTIFACT, #774; register corrected #780) -- see the dated correction note near the section head; per-occurrence reconciliation routed to the manuscript-reconciliation program.]
\noindent \textbf{Frequency mean $-0.45\%$, decay-time mean $-0.47\%$ per event}. Live-fire driver at \kbleaf{src/scripts/vol\_3\_macroscopic/ligo\_ringdown\_driver.py}; full Phase-3+5 outputs in research \kbleaf{ligo-ringdown-driver-design.md} \S 8--10.

\paragraph{Phase-4 spin sweep + mass-independence (2026-05-18).} v2 generalizes across the full Berti+Cardoso+Will 2006 Kerr QNM curve: 9 of 11 swept spin values $a_* \in \{0.0, 0.1, \ldots, 0.95\}$ PASS at $|\text{dev}| < 3\%$ vs GR reference; divergence onset $a_* \geq 0.90$ (near-extremal regime, currently unattested in LIGO catalog). Mass-independence confirmed at $7\times$ range: GW190521 ($M = 142\ M_\odot$, IMBH-class) matches at $-0.25\%$ per v2; GW170729 ($a_* = 0.81$) matches at $+1.77\%$. These agreements are the \emph{post-hoc} spinning-remnant consistency check (Phase-5 Q-preservation fit); the validated spin range is $a_* = 0.3$--$0.8$ per \texttt{clm-395gps}, and near-extremal remnants ($a_* > 0.90$) are deferrable to Option B (full spheroidal cavity) when LIGO/ET detects such an event.
...
\end{tabularx}
\end{center}
\noindent\textbf{Updated 2026-05-18 (Phase 5 closure):} the bare Schwarzschild $Q = \ell = 2$ eigenvalue above is the cold-cavity forward prediction (zero free parameters). The spinning-remnant Kerr correction to $Q$ is a \textbf{post-hoc refinement}: the v1$\to$v2 lattice-Q preservation mechanism (Cosserat skeleton sets Q-factor invariant of cavity-radius refinement) was introduced after the fact to reconcile the v1 shortfall against LIGO data. See the \emph{Kerr Quality Factor} subsection above for the v2 formula $\tau_{v2} = \tau_{v1} \cdot (\omega_{R,v1}/\omega_{R,v2})$ and the corresponding LIGO comparison table at $-0.47\%$ mean $\tau$ error per event (the standard GR Kerr QNM comparison figure for the same events is $-6.94\%$ mean; both are \emph{post-hoc} spinning-remnant consistency checks, not forward predictions). Per \texttt{clm-395gps}, this Kerr comparison is validated only for $a_* = 0.3$--$0.8$ (10--18\% frequency error otherwise); the larger cold-$Q=\ell$ discrepancies in the table above reflect that the cold approximation is not the Kerr-refined case.
