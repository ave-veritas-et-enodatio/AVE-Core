...
\begin{figure}[h!]
    ...
    \caption{\textbf{Untapped Impedance-Topology Predictions.} Panels derived from the AVE impedance topology; the Schwarzschild $\ell=2$ QNM eigenvalue is zero-free-parameter, while the spinning-remnant comparison in panel (1) is a post-hoc consistency check (see text). (1) Merger ringdown frequency vs. LIGO observations: the v2 Cosserat-back-reaction formula reaches \textbf{$-0.45\%$ mean $\omega_R$ + $-0.47\%$ mean $\tau$} per event (Phase 3+5 closure 2026-05-18) --- a post-hoc Kerr consistency check, not a forward prediction. (2) Iron K$\alpha$ line profile showing discrete sub-peaks at impedance band energies. (3) Jet impedance map: polar impedance matching ($\Gamma \approx 0$) produces a natural jet channel. (4) Hawking temperature from impedance boundary noise leakage. (5) GW memory strain as residual lattice deformation above $h_{yield} = \sqrt{\alpha}$. (6) Scale invariance: the same $1/d$ topology from the electron unknot ($10^{-13}$ m) to M87* ($10^{13}$ m).}
    \label{fig:untapped}
\end{figure}
