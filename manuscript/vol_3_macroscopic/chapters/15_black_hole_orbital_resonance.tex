...
%        QUOTE (byte-exact at HEAD): two channels radiating at different speeds mean a single
...
%        QUOTE (byte-exact at HEAD): with the P and SV polarizations \emph{coupled}, so the compressional
...
%        QUOTE (byte-exact at HEAD): the electron's confinement (bulk-channel TIR, $\Gamma_{bulk} = -1$
...
%        QUOTE (byte-exact at HEAD): The bulk-longitudinal channel collapses with it ($c_{bulk} \to 0
...
%        QUOTE (byte-exact at HEAD): that energy sources the A1 dilatation channel. Whether AVE
