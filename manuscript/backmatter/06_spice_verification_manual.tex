...
\emph{Scope (2026-06-15, reconciliation):} ``derives \ldots from four axioms''
refers to the \emph{forms} and relations; absolute values additionally ride on
the framework's three calibration inputs $\{m_e\,(\equiv \ell_{node}), \alpha, G\}$.
The solver-independence principle below (``traces solely to the input axioms'')
likewise certifies that the relations are solver-independent, not that the
values are parameter-free.
...
The six-row translation table (Chapter~1, Table~the main text)
...
\begin{equation}
    C_{eff}(V) = \frac{C_0}{S(V)} = \frac{C_0}{\sqrt{1 - (V/V_{snap})^2}}
...
+   / sqrt(1 - min((V(A,B)/V_SNAP)**2, 0.9999))}
\end{verbatim}
\end{codebox}

\noindent \textbf{Sector-keying fix (2026-07-03, VALUE CHANGE).} The divergent
$C_0/S$ metric varactor is the \emph{longitudinal-A1 bond compliance}, which
diverges at $V_{snap} = m_e c^2/e \approx 511$~kV, \emph{not} at the transverse-T2
yield wall $V_{yield} \approx 43.65$~kV (Grant-ratified grade-fork, 2026-06-30;
\texttt{def-vyvsn1}; Vol~4 Ch~1 nonlinear-vacuum-capacitance sector note). The
prior keying on $V_{yield}$ was a mis-scoping by the factor $1/\sqrt{\alpha}
\approx 11.7$; the A1 varactor knee moves $43.65$~kV~$\to 511$~kV. The
\texttt{min(\ldots, 0.9999)} clamp prevents the argument of \texttt{sqrt} from
reaching zero during numerical iteration, which would produce a divide-by-zero.
This is a numerical guard only; the physics demands $V < V_{snap}$ (otherwise the
A1 mass core has completed). The separate TVS-Zener rupture at $V_{yield}$ (the T2
yield wall) is unchanged.
...
\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{l l l p{0.38\linewidth}}
\toprule
...
\texttt{V\_SNAP} & $V_{snap}$ & 510998.95 V         & A1 bond-compliance divergence voltage (metric varactor knee) \\
\texttt{V\_YLD}  & $V_{yield}$ & 43651.85 V         & Transverse-T2 dielectric yield / rupture voltage \\
\texttt{I\_YMAX} & $I_{max}$  & 124.384 A           & Current saturation limit \\
\bottomrule
\end{tabular}
\end{center}

\noindent The physical values $V_{snap} = m_e c^2/e = 510998.95$ V and
$V_{yield} = \sqrt{\alpha}\,V_{snap} = 43651.851844\ldots$ V are the canonical
\texttt{ave.core.constants} values (\texttt{V\_SNAP}, \texttt{V\_YIELD}).
The current threshold $I_{max} = \xi_{topo}\,c = 124.384$ A is \emph{not} a
\texttt{constants} symbol: it lives in the engine module as
\texttt{ave.core.fdtd\_3d.I\_MAX\_MU}
(\texttt{src/ave/core/fdtd\_3d.py}:69, \texttt{I\_MAX\_MU = XI\_TOPO * C\_0}).
\textbf{The A1 metric varactor keys on
$V_{snap}$; the TVS-Zener rupture and the thixotropic memristor key on
$V_{yield}$} (the sector split, 2026-07-03).
% DEAD-CITE REPAIR (2026-08-03, imax-mechanical lane).  The sentence above
% formerly read, verbatim (Rule 12): "The physical values $V_{snap} = m_e c^2/e
% = 510998.95$ V, $V_{yield} = \sqrt{\alpha}\,V_{snap} = 43651.851844\ldots$ V
% and $I_{max} = \xi_{topo}\,c = 124.384$ A are the canonical
% \texttt{ave.core.constants} values."  That attribution is FALSE for $I_{max}$:
% ave.core.constants.I_MAX DOES NOT EXIST (two-method verified at origin/main
% 66fc7e69 -- hasattr() False; grep of src/ave/core/constants.py exits 1).
% Re-pointed to the live symbol ave.core.fdtd_3d.I_MAX_MU.  No symbol is minted
% in constants.py by this repair, deliberately: minting one would force the open
% A4 / I_max homonym ruling.  The V_snap / V_yield half of the sentence was and
% remains correct.

\begin{quote}
\small
\textbf{Open-fork caveat on the $I_{max}$ value (2026-08-03).}  The
$124.384$ A default is the \emph{convection} reading of $I_{max}$
($\xi_{topo}c$, Ax2 TKI at $v=c$).  The corpus also uses the name $I_{max}$
for a \emph{displacement} reading, $V_{yield}/Z_0 = 115.870$ A --- the
``FPB slew rating'' (\texttt{manuscript/ave-kb/common/operators.md}:145).
The two differ by exactly $4\pi\sqrt{\alpha} = 1.073476$, i.e.\ $+15.2\%$ in
the \emph{quadratic} Axiom-4 kernel argument $(I/I_{max})^2$.  Which reading
the $\mu$-grade denominator should carry is Grant's A4 ruling and is
\textbf{still open}
(\texttt{research/2026-07-10\_operator-typing-pass\_result.md}:112).  Three-sense
map and hazard box: \texttt{manuscript/ave-kb/common/theorem-thesaurus.md}
\S6, the $I_{max}$ row.  This manual keeps $124.384$ A --- the repair is to the
cite and the disclosure, not to the number.
\end{quote}
% FLAG-1 correction (2026-07-03, ave-canonical-source): the prior 43653.7 V /
% 124.4 A defaults were DRIFTED; corrected to the constants.py canonical values.
% The prior claim "No constants are hardcoded in the .lib file" was FALSE — the
% .lib DOES hardcode these as subckt defaults (now the canonical values, still
% hand-written). The durable fix is to GENERATE the .lib from constants.py
% (SPICE-charter design-(d): .lib.template + substitute-at-test-time); until then
% these defaults are a maintained copy, not a generated one.
The default literals above are also carried as subcircuit-parameter defaults in
the hand-written \texttt{.lib}; until the generated-\texttt{.lib} mechanism
(charter design-(d)) lands, they are a maintained copy of their canonical
sources --- \texttt{constants.py} for \texttt{V\_SNAP}/\texttt{V\_YLD},
\texttt{fdtd\_3d.py} for \texttt{I\_YMAX} --- not an auto-substituted one.
% Same 2026-08-03 dead-cite repair: this sentence formerly read "a maintained
% copy of the \texttt{constants.py} values", which is the same false attribution
% for the I_YMAX row only.  Split by symbol; V_SNAP/V_YLD unchanged.
...
\subsection{Topological Hardware Subcircuits}

The core simulation methodology relies on generalizing any complex hardware topology into interconnected \texttt{AVE\_VACUUM\_CELL} subcircuits, making their Axiom~4 compliance mathematically explicit across any arbitrary configuration.
...
\begin{warningbox}[ADJUDICATION-PENDING (2026-07-03 sector-keying pass)]
The \texttt{AVE\_EE\_BENCH} subcircuit and this worked example implement the
\emph{divergent} $C_0/S$ (longitudinal-A1 compliance) form keyed on $V_{yield}$,
but the canonical EE bench (Vol~4 Ch~17 ee-bench-netlist) measures the
\emph{collapse} $C_0\!\cdot\!S$ (transverse-T2 dielectric permittivity) roll-off
--- the LCR-bench capacitance. These are the two orthogonal reactances of the
FLAG-2 sector split (A1 $\perp$ T2), so this section models a different sector
than the EE bench actually reads, \emph{and} the keying question (A1~$\Rightarrow
V_{snap}$ vs T2~$\Rightarrow V_{yield}$) rides on which sector it is. This is
surfaced for adjudication (which sector is the EE bench modelling?) rather than
force-resolved. The A1 metric-varactor knee elsewhere in this manual is fixed to
$V_{snap}$; this EE-bench section is left as-is pending the sector ruling.
\end{warningbox}
