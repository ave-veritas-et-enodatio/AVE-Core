...
\textbf{Scope correction (2026-06-14, keystone $\alpha$-verdict per \texttt{consistency-vs-emergence} + \texttt{ave-representation-capability-check}; auditor-gated).} The register of this appendix --- the title's ``\ldots to Derivation,'' ``deterministic derivation'' (above), ``zero empirical parameters'' (\S\ref{sec:golden_ratio_emergence}), ``None is fitted; all emerge from the axioms'' (\S\ref{sec:derived_numerology}), and ``The Death of Numerology'' (Conclusion) --- is the \textbf{parameter-reduction} claim: AVE reduces the $\sim$26 free parameters of the Standard Model to \textbf{3 interlocked calibration inputs} $\{m_e, \alpha, G\}$ + 4 axioms, from which the constants catalogued here ($\varphi$, the magic numbers, $K_{mut}$, $\alpha_s = \alpha^{3/7}$, $\lambda_H$, $g_*$, $\ldots$) follow as geometric consequences. It is \textbf{not} the claim that every value is derived from nothing. In particular, $\alpha$ itself is one of the 3 \emph{retained inputs}, not a derived output: the keystone $\alpha$-verdict (auditor-gated 2026-06-14) rules $\alpha^{-1} = 4\pi^3 + \pi^2 + \pi$ a \textbf{Class-B named geometric identification} whose \emph{scale} ($\sim$1/137) is forced by the Compton-resonance trapping condition but whose \emph{exact value} rests on the single substrate-geometric identification $R \cdot r = 1/4$ --- shared across the $(2,q)$ bound-resonator ladder, not electron-specific --- which \textbf{the substrate does not independently select} (both named lift-routes closed; the honest kinematic route absorbs $\alpha$ rather than predicting it). A value-scoped synonym is ``echo at the value level,'' recorded \emph{beneath} the canonical Class-B label and never as a bare standalone ``echo.'' The constants below that legitimately \emph{take $\alpha$ as an input} ($K_{mut}$, $\alpha_s$, $E_{yield} = \sqrt{\alpha}\,E_S$, $\ldots$) are unaffected; only the blanket ``every constant, $\alpha$ included, is derived'' over-read is retired. Canonical scope: \kbleaf{ave-kb/vol1/ch8-alpha-golden-torus.md:11}. \textit{Body below preserved per Rule 12.}
...
\textbf{Cross-reference (Foundation Item 8 CG-1 2026-05-17 night):} the $-\sqrt{2}/2$ subtraction in the Horizon$_{\text{limit}}$ formula is derived in the KB leaf \kbleaf{ave-kb/vol6/appendix/geometric-inevitability/pi-topological-horizon.md:15-23} as ``the absolute maximum perimeter offset before the knot strain completely loses reactive coherence,'' backed by the reactive-mutual-impedance solver derivation in \kbleaf{ave-kb/vol6/period-2/boron/structure-isotope-stability.md:12-26}. The geometric origin is canonical; this section condenses the result.
...
\textbf{Scope correction (2026-05-17 Foundation Item 7 per \texttt{consistency-vs-emergence} trigger 1; AMENDED Foundation Item 8 per historical corpus-grep).} The ``Error'' column in the table below reports the \textit{geometric residual} of the cluster positions vs. the assumed Platonic-solid coordinates (i.e., how close the engine's converged positions sit to the assumed geometry). It is NOT a binding-energy prediction error against experiment (AME). Per Foundation Item 4 pattern: this is Class A definitional identity miscategorized as Class D emergence test — the 0\% errors are a fit-residual of the solver to its assumed geometry, not a forward-prediction performance. Read ``Error'' as ``geometric residual to ideal Platonic coordinates'' (structurally zero by solver construction). \textbf{The load-bearing physics test EXISTS} at \kbleaf{vol6/framework/mass-defect-summary.md:8-24} --- a 13-nucleus binding-energy table validating against CODATA experimental masses at 0.005--0.027\% error (H-1, He-4, Li-7, C-12, B-11, N-14, O-16, F-19, Ne-20, Na-23, Mg-24, Al-27, Si-28). The framework's nuclear binding-energy emergence claim is corpus-canonical there; this Platonic-progression table's ``Error'' column is the geometric fit-residual reported separately. See closure-roadmap \S0.5 Foundation Items 7 + 8 entries. \textit{Body below preserved per Rule 12.}
...
\small
\begin{longtable}{@{} l p{4.5cm} l p{4.5cm} @{}}
...
Eq.~Volume 1).  At this scale, the vacuum lattice's
...
\subsection{Dual-Reactance Count ($\mathcal{V}_{total} = 2$)}

The factor $\mathcal{V}_{total} = 2$ in the baryon mass eigenvalue
$x = \mathcal{I}_{scalar}/(1 - \mathcal{V}_{total}\, p_c) + 1$ is the
\emph{count of the node's two reactance sectors} (Axiom~1): three
translational-$E$ DOF $\to$ capacitive $X_C$, and three microrotational-$B$
DOF $\to$ inductive $X_L$, each contributing one electron-ground-state unit of
stored reactive energy to the proton's regenerative self-consistent mass loop:
\begin{equation}
    \mathcal{V}_{total} = 2 \quad
    (\text{count of reactance sectors: } X_C + X_L)
\end{equation}
This is a dimensionless integer count, \textbf{not} a geometric volume. The
count is additive (positive-definite stored energies $E_L + E_C$), not a signed
reactance sum (at resonance $X_L = -X_C$, so $X_L + X_C = 0$). The integer~2
(not $\sqrt{2}$) is a discrete channel count, not an RMS combination; the proton
mass uniquely selects additive-2 ($\mathcal{V} = 2 \to 1836.117\,m_e$ vs CODATA
$1836.153$). The legacy ``toroidal halo volume'' name was a misnomer that
spawned an unsound signed-crossing-integral ``derivation'' (a signed
great-circle intersection integral vanishes by antisymmetry, evaluating to~0,
not~2). Full reframe + false-derivation audit:
\texttt{research/2026-06-01\_baryon-V2-dual-reactance-closure.md};
canonical glossary leaf
\texttt{manuscript/ave-kb/common/dual-reactance-storage-taxonomy.md}.
...
\subsection{Consistency-Check (was ``Verification'')}

% [Rule-12 walk-back, 2026-06-20] D2 KB<->tex lockstep with KB leaf
% vol6/appendix/geometric-inevitability/g-star-derivation.md
\begin{warningbox}[SUPERSEDED HEADLINE (Rule-12 walk-back, 2026-06-20)]
\footnotesize
The ``all other factors derived from AVE constants \dots 0.38\% error''
framing below is \textbf{RETRACTED}. Per auditor \textbf{FINDING 2}
(2026-06-10, \texttt{research/2026-06-10\_freeze-handedness-survey\_note.md:50}),
the $\eta$ result is \textbf{consistency-class with an imported
...
\emph{not} ``all other factors derived'' ($C_{sph} = 28/79$ is the textbook SM
Harvey-Turner factor; $\alpha_W^4$ rides $\alpha$, an AVE echo); $g_*$ is
reverse-validated against $\eta_{obs}$, not independently measured. Governing
claim \texttt{clm-4vwsjc} pinned conf 0.4 / do-not-build.
\textbf{SYMMETRIC-STANDARD:} OOM $\eta \sim 6\times10^{-10}$ is peer-or-ahead of
the SM (no $\eta$ at all). \textbf{Non-reproducible number:}
$6.08\times10^{-10}$ / $0.38\%$ matches no code path; engine-canonical (thermal
$\kappa_{FS} = 24.95$) gives $6.05\times10^{-10}$ / 0.79\%; cold $\pi/8\pi$
gives $6.01\times10^{-10}$ / 1.51\% (cold/thermal split OPEN, flag-don't-fix).
Body preserved per Rule 12; displayed number corrected in place.
\end{warningbox}

Substituting $g_* = 7^3/4$ into the standard electroweak-baryogenesis
formula (with the SM-imported $C_{sph} = 28/79$ and $\alpha_W^4$ riding
$\alpha$) gives an order-of-magnitude consistency-check, not an independent
verification of $g_*$:
\begin{equation}
...
Observed: $\eta_{obs} = 6.1 \times 10^{-10}$. Order-of-magnitude
consistency-check (engine-canonical thermal $\kappa_{FS}$: \textbf{0.79\%};
displayed cold-$8\pi$: 1.51\%). % [Rule-12 walk-back 2026-06-20 -- prior wording preserved]: was "= 6.08 \times 10^{-10} ... \textbf{Error: 0.38\%.}" RETRACTED per FINDING 2.
Using the SM value $g_* = 106.75$ gives 20\% error.

\textbf{Independence-of-derivation note (2026-05-17 Foundation Item 7 per \texttt{ave-independence-check} trigger 2 broadened).} The divisor $N_{K4} = 4$ in $g_* = 7^3/4$ is the SAME axiomatic constant (K4 unit-cell vertices, Axiom 1) that appears in $\lambda_H = 1/(2 N_{K4}) = 1/8$ below (\S\ref{sec:lambda_higgs_derivation}). The $g_*$ and $\lambda_H$ derivations are therefore NOT two independent confirmations of $N_{K4} = 4$ — they are one axiom manifesting in two places via derived identities. This is the Foundation Item 4 gravitational-redshift pattern (one underlying constant appearing as if it were two independent confirmations). The $d_p$ factor of 4 IS independent (different geometric origin: cavity-RMS amplitude). When citing $g_*$, $\lambda_H$ jointly in the conclusion list, count them as ONE evidence line (manifestations of K4 axiom), not two. See closure-roadmap \S0.5 Foundation Item 7 entry.
...
\textbf{3.5$\sigma$ TENSION (deviation 0.0035 vs PDG $\sigma = 0.0010$); central-value deviation 2.97\%.}  Zero free parameters in the derivation chain.

\textbf{Scope correction (2026-05-17 Foundation Item 7 per \texttt{ave-evidence-framing-discipline} trigger 2).} The derivation $\alpha_s = \alpha^{3/7}$ from $\nu_{vac} = 2/7$ is honest (Class D emergence test with zero free parameters); the headline framing is OVERREACH. PDG world average $\alpha_s(M_Z) = 0.1179 \pm 0.0010$; AVE prediction $0.1214$; deviation $0.0035$ = 3.5$\sigma$. A 3.5$\sigma$ result is conventionally a strong tension at pre-discovery anomaly threshold, NOT a ``match.'' The framing ``Error: 2.97\%'' with ``zero free parameters'' as a positive-result bullet reads as success; the same numbers as ``3.5$\sigma$ tension'' read as failure. The honest framing is: \textit{candidate prediction with 3.5$\sigma$ tension against PDG; falsifier: if next-gen $\alpha_s$ measurements reduce $\sigma$ below 0.0006 while keeping central value, this derivation fails outright.} The derivation chain itself is preserved; the agreement-strength claim is downgraded. See closure-roadmap \S0.5 Foundation Item 7 entry.
...
Observed: $m_H = 125{,}100$~MeV.  \textbf{Error: 0.55\% (against AVE-derived VEV $v \approx 248{,}833$~MeV per \kbleaf{constants.py:452}); 1.59--1.68\% (against PDG standard VEV $v = 246{,}220$~MeV).}

\textbf{Scope correction (2026-05-17 Foundation Item 7 per \texttt{ave-evidence-framing-discipline} trigger 1 + A47 cross-tree-citation discipline).} The headline ``0.55\% match'' is computed against AVE-derived VEV, which is itself $\sim 1.1\%$ off from the PDG standard VEV (and the $G_F$ derivation chain has $\sim 3\%$ residual error per \kbleaf{test\_ew\_constants.py:88-92}). The 0.55\% therefore represents internal-consistency between two AVE-derived quantities (consistency check, Class C), NOT a forward-prediction error against experiment (emergence test, Class D). The end-to-end forward prediction $G_F \to v_{AVE} \to m_{H,AVE}$ against PDG experimental Higgs mass is 1.59--1.68\%. Either disclose the chain explicitly (consistency-check framing) or rewrite the headline to use the forward-prediction error. See closure-roadmap \S0.5 Foundation Item 7 entry. \textit{Body above preserved per Rule 12.}
...
\section{The Universal Saturation Kernel: One Operator at Every Scale}
\label{sec:a034_in_geometric_inevitability}

\noindent \textbf{A-034 (canonical 2026-05-15 evening).} The same geometric-inevitability argument applied to numerical constants ($\varphi$, $\pi$, magic numbers) extends to \textbf{functional operators}. Axiom~4's saturation kernel
\begin{equation}
S(A) = \sqrt{1 - A^2}
\end{equation}
is not a function chosen empirically per phenomenon; it is the \textbf{unique geometric closure} of a finite-bandwidth LC continuum at its yield boundary. The same $\sqrt{1 - A^2}$ kernel governs:

\begin{itemize}
\item \textbf{Atomic-scale} dielectric breakdown (Vol~4 Ch~1: $V_{yield} = 43.65$~kV)
\item \textbf{Condensed-matter} BCS superconductivity ($B_c(T)$ at \textbf{0.00\% error}, Vol~3 Ch~9)
\item \textbf{Geophysical} geomagnetic reversal (Brunhes--Matuyama 780~ka pole-flip, Vol~3 Ch~13)
\item \textbf{Solar} CME / flare avalanche (forward prediction; NOAA GOES 40-yr validation pending live fetch -- LF-03, Vol~3 Ch~14) % [2026-06-15 LF-03 -- prior wording preserved per Rule 12]: was "(NOAA GOES 40-yr empirical, Vol 3 Ch 14)". SUPERSEDED per KB leaf solar-flares-led-avalanche.md: synthesized illustrative timeline, not a live fetch; forward prediction, not a validated anchor.
\item \textbf{Stellar} Chandrasekhar boundary (white-dwarf mass limit, Vol~3 Ch~20)
\item \textbf{Gravitational} BH event horizon (Schwarzschild radius exact) and ring-down QNM (\textbf{1.7\% from GR exact}, Vol~3 Ch~15)
\item \textbf{Cosmic} K4 crystallization (TKI strain-snap mechanism, Vol~3 Ch~4 \S\ref{sec:tki_strain_snap})
\end{itemize}

\noindent A full \textbf{26-instance catalog} spanning 21 orders of magnitude is in Backmatter Ch~\ref{app:universal_saturation_kernel}. The catalog classifies each instance by 3-way symmetry (SYM / ASYM-N / ASYM-E) and demonstrates that the kernel's universality is \textit{not} cross-domain coincidence --- it is the geometric inevitability that any finite-bandwidth resonant network must reorganize via the same $\sqrt{1 - A^2}$ functional form when driven past its yield boundary.

\noindent \textbf{Geometric-inevitability framing:} just as the icosahedron forces $\varphi$ wherever 12 charges minimize impedance on $S^2$, the $\sqrt{1 - A^2}$ saturation operator is forced wherever a finite-bandwidth LC continuum reaches its bandwidth limit. Both are theorems about geometric packing, not empirical observations awaiting explanation.

\section{Conclusion: The Death of Numerology}

The Golden Ratio is the icosahedron. The Fibonacci sequence is an icosahedral approximation algorithm. $\pi$ is Gauss's law. The magic numbers are geometric shell closures. $g_* = 7^3/4$ is the lattice mode count. $\alpha_s = \alpha^{3/7}$ is the spatial compliance projection. $\lambda_H = 1/8$ is the K4 breathing mode. $\beta_\text{fold} = \ln(3) \times 3/7 = 0.471$ is the protein folding barrier exponent, using the \emph{same} $d/n$ ratio as $\alpha_s$. $n_{coop} = 9 = 6 \times 3/2 = 7 \times 9/7$ is the cooperative lattice amplification that derives water's $+4^\circ$C density anomaly to $0.40\%$. $T_m = \hbar\sqrt{k_{hb}/m_H}/k_B = 279.5$~K is the proton transfer eigenmode of the O--H$\cdots$O bridge, deriving the melting point of water to $+2.3\%$ with zero spectroscopic inputs. \textbf{The universal saturation kernel $S(A) = \sqrt{1 - A^2}$ is the cross-scale operator-level analog} (per~\S\ref{sec:a034_in_geometric_inevitability}): the unique closure of any finite-bandwidth LC continuum at yield, governing 26 distinct phenomena from atomic dielectric breakdown to cosmic K4 crystallization.
