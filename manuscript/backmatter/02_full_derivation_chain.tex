...
crystallization, etc. Per A-034 (canonical), these are 19 manifestations
...
\begin{enumerate}
    \item Each ``Layer'' derives \textit{only} from quantities established in
          preceding layers.
    \item The three canonical hardware scales are themselves derived
          (Layer~8 + Vol.~1 Ch.~\ref{ch:alpha_golden_torus} Golden Torus
          $\alpha$ derivation), closing the loop to genuinely zero parameters ---
          \textbf{contingent on the open Clifford-torus embedding-selection item (see gating note above).}
...
$\mathcal{M}_A$ --- a 3D crystallized substrate of nodes at pitch $\ell_{node}$,
...
Charge $q$ is a discrete geometric dislocation in $\mathcal{M}_A$. The Burgers
...
this dislocation field are particles: $(2,3)$ torus knot = electron, $(2,5)$
cinquefoil = proton; the $(2,q)$ family generates the baryon ladder.
\end{axiom}
...
19 cross-scale topological-reorganization events spanning 21 orders of magnitude
(empirical anchors: BCS $B_c(T)$ at 0.00\% error, BH event horizon =
Schwarzschild $r_s$ exact, BH ring-down QNM = $18/49$ at 1.7\% from GR,
NOAA GOES 40-yr solar-flare validation). See Backmatter
...
Finally, the three canonical hardware scales are \emph{conjectured} to be
geometrically derived---not independent empirical inputs---which \emph{would}
reduce the framework to \textbf{zero free parameters}. This reduction is
\textbf{contingent on the open Clifford-torus embedding-selection item} (see the
gating note at the head of this chapter): until that step is proven from the
classical axioms alone, $\alpha$ remains operationally the one calibration input.
The proposed closure for
$\alpha$ is the Golden Torus derivation in Vol.~1 Ch.~\ref{ch:alpha_golden_torus};
the three-regime argument there produces $\alpha^{-1} = 4\pi^3 + \pi^2 + \pi$
(Nyquist + self-avoidance + spin-1/2 half-cover of
Clifford torus --- the half-cover regime carrying the open embedding-selection
step), with CMB thermal strain closing to the CODATA value.

\paragraph{$\alpha$ is \emph{conjectured}-derived (gated on the open embedding-selection item).}
The full derivation is given in Vol.~1, Ch.~\ref{ch:alpha_golden_torus}
\textit{``Zero-Parameter Closure: $\alpha$ from the Golden Torus''}. Three distinct physical regimes --- Nyquist resolution at tube
diameter $d = 1\,\ell_{node}$, self-avoidance at trefoil crossings
$R - r = 1/2$, and the spin-1/2 half-cover of the standard Clifford torus
$\mathbb{T}^2 \subset S^3 \subset \mathbb{C}^2$ giving $R \cdot r = 1/4$ ---
...
\end{equation}
with a CMB-induced thermal strain $\delta_{\text{strain}} \approx 2.225 \times 10^{-6}$ accounting for the residual to the CODATA value $137.035999$.
...
\textbf{Result:} The AVE framework is a closed, structurally zero-parameter Topological
Effective Field Theory (contingent on the open Clifford-torus embedding-selection
item --- see gating note at chapter head --- and on Layer~8 thermal closure of $\delta_{strain}$
...
\end{itemize}
\noindent\textbf{Honest framing of ``zero free parameters.''} The chain reduces 26 SM parameters to a 3-element bounding set $\{m_e, \alpha, G\}$ + four axioms, which is then claimed to close to zero parameters at Layer~8. The headline ``26 / 26 derived'' is correct \emph{conditional on Layer~8 closure holding}; without that closure, the count is ``25 of 26 expressed as functions of three bounding limits, of which one ($m_e$) is the input scale.'' The Layer-8 closure depends on (a) the Golden Torus cold-lattice $\alpha^{-1}_{ideal} = 4\pi^3+\pi^2+\pi$ derivation (Ch.~\ref{ch:alpha_golden_torus}) and (b) the thermal running $\alpha^{-1}(T)$: existence and sign predicted; the magnitude $\delta_{strain}$ at $T_{CMB}$ is one currently-fitted scalar (back-subtracted from CODATA), pending derivation from $G_{vac}$ + equipartition. See Ch.~\ref{ch:alpha_golden_torus} predicted/fitted disclosure. Every quantity marked $\checkmark$ is computed by the physics engine at import time with zero per-parameter curve-fitting; the input scale is calibrated once and propagated. The scope of the ``no curve-fitting'' claim is the SM-parameter table only; nuclear masses (Vol~6) are a separate one-fit-per-nucleus structural claim — see Vol~6 introduction methodology note.
...
\bigskip
\noindent
\textbf{Empirical inputs (bounding limits):} 3 --- $\{m_e, \alpha, G\}$. Each is \emph{claimed} emergent at Layer~8 (Ch.~\ref{ch:alpha_golden_torus} Golden Torus); the closure rests on the cold-lattice $\alpha^{-1}_{ideal} = 4\pi^3+\pi^2+\pi$ derivation, plus a thermal-running correction whose existence and sign are predicted but whose magnitude ($\delta_{strain}$ at $T_{CMB}$) is one currently-fitted scalar --- see Ch.~\ref{ch:alpha_golden_torus} predicted/fitted disclosure.\\
