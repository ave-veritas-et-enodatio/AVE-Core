...
scales are themselves derived from first principles: $\alpha$ from the Golden
Torus S$_{11}$-minimum geometry of the trefoil electron soliton (Vol.~1,
...
\textit{Underlying mechanism (substrate topology):} the physical vacuum
operates as a dense, non-linear electromagnetic LC resonant network
$\mathcal{M}_A(V, E, t)$, evaluated as a \textbf{Trace-Reversed Chiral LC
Network} (micropolar continuum) in the macroscopic limit.
\end{axiom}

\begin{axiom}[Axiom 2: Fine Structure]
...
\textit{Underlying mechanism (topo-kinematic isomorphism):} charge $q$ is
identically a discrete geometric dislocation (a localized phase twist) within
$\mathcal{M}_A$. The fundamental dimension of charge is \textit{length}:
$[Q] \equiv [L]$, with topological conversion constant
$\xi_{topo} = e/\ell_{node}$.
\end{axiom}

\begin{axiom}[Axiom 3: Gravity]
Newton's constant emerges as the Machian boundary impedance:
\begin{equation}
    G = \frac{\hbar c}{7\,\xi\, m_e^2}
    \label{eq:axiom3_gravity_chain}
\end{equation}
...
\textit{Underlying mechanism (effective action principle):} the system evolves
to minimize the macroscopic hardware action; the dynamics are encoded in the
...
\begin{equation}
    ...
    \label{eq:axiom4_saturation_chain}
\end{equation}
\textit{Underlying mechanism (non-linear Born-Infeld dielectric):} a
\textbf{squared limit} ($n=2$), aligning with the $E^4$ scaling of
Euler--Heisenberg QED and suppressing $E^6$ divergences. The constitutive
permittivity collapses as $\varepsilon_{eff} = \varepsilon_0\,S$ while the
energy-absorbing differential capacitance diverges:
\begin{equation}
    C_{eff}(\Delta\phi)
...
Finally, the three canonical hardware scales are themselves shown to be
geometrically derived---not independent empirical inputs---formally reducing
the framework to \textbf{zero free parameters}.  The definitive closure for
...
from first principles (Nyquist + self-avoidance + spin-1/2 half-cover of
Clifford torus), with CMB thermal strain closing to the CODATA value.
...
$(R, r) = (\varphi/2, (\varphi-1)/2)$. The holomorphic multipole
decomposition at this geometry yields
\begin{equation}
