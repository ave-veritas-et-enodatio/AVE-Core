...
Ch.~8 \textit{``Zero-Parameter Closure: $\alpha$ from the Golden Torus''});
$\ell_{node}$ from the Nyquist resolution of
...
\begin{enumerate}
    ...
    \item The three canonical hardware scales are themselves derived
          (Layer~8 + Vol.~1 Ch.~8 Golden Torus $\alpha$ derivation),
          closing the loop to genuinely zero parameters.
...
$\alpha$ is the Golden Torus derivation in Vol.~1 Ch.~8;
...
The full derivation is given in Vol.~1, Ch.~8
...
uniquely, and the Poisson ratio $\nu_{\text{vac}} = 2/7$ follows.
