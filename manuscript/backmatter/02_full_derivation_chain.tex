...
\begin{axiom}[Axiom 1: Substrate Topology (Chiral Laves K4 Crystal)]
The physical vacuum IS a \textbf{chiral Laves K4 crystal} $\mathcal{M}_A$ ---
a 3D crystallized substrate of nodes at pitch $\ell_{node}$, governed by the
right-handed $I4_1 32$ chiral space group, with 4-fold K4 nearest-neighbor
connectivity. Each node has two intrinsic DOFs: translational (capacitive
$\varepsilon_0$ = E-field) and rotational (inductive $\mu_0$ = B-field),
making every node a native LC oscillator. The substrate is structurally a
Cosserat micropolar crystal AND a discrete LC resonant network, the same
reality in mechanical vs.~EM dialects. In the macroscopic continuum limit:
a \textbf{Trace-Reversed Chiral LC Network} supporting intrinsic spin and
trace-free transverse EM waves. Two phases: crystallized (Regimes I--III,
observable universe) and ruptured plasma (Regime IV, BH interior + pre-genesis);
connected by Axiom 4's saturation kernel.
\end{axiom}
...
The substrate, in its continuum limit, evolves to extremize the macroscopic
substrate action $S_{AVE}$. Two co-canonical forms:
