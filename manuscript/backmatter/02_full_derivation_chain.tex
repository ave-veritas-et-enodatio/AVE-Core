...
\begin{equation}
    ...
    \label{eq:lagrangian}
\end{equation}
is the \emph{temporal-gauge} (Weyl-gauge, $A_0$-free) form of the Maxwell Lagrangian in vector-potential form --- standard Maxwell dynamics are recovered on the Gauss-constrained initial-data surface, the constraint being supplied by Axiom 5 (Substrate DC Bias, \texttt{eq\_axiom\_5.tex}; clause attribution left open, riding the deferred FORK-1) rather than by an equation of motion of this action (rescoped 2026-08-10, R43) --- recovered as the
...
\begin{equation}
    ...
    \label{eq:nu_kin}
\end{equation}
% [2026-08-02 NON-CLAIM TAG -- wave-2 backmatter reconciliation; board finding
%   [MEDIUM][overclaim] at 02_full_derivation_chain.tex:744. Prior wording preserved per
%   Rule 12 (git trail), verbatim:
%     "(Nearly identical to liquid water---a non-trivial structural prediction.)"
%   Authority (verified at HEAD two-method: `git grep -n "liquid water"` + `sed` line read),
%   ave-kb/vol1/claim-quality.md:650, clm-crbl60 Specific Non-Claims -- "The $\nu_{kin}$
%   'close to liquid water' comparison is an order-of-magnitude consistency observation, not
%   a derivation that the vacuum *behaves like* water in any operationally relevant sense."
%   FORK DISCLOSED: the paired strengthen-by at :667 offers TWO mutually-exclusive remedies,
%   "Drop **or** formalize" -- which is why the board tagged this [ruling-needed] rather than
%   [mechanical]. The lane brief selects the :650 non-claim (formalize, do not drop), so the
%   comparison stays and the "prediction" grade is struck. NO VALUE REFILL: no water viscosity
%   figure, ratio or temperature is introduced, and nu_kin's own value is unchanged.
%   KB-mirror co-lag recorded, not fixed (KB lane owns it; KB is truth-source): the identical
%   un-tagged sentence is still carried at ave-kb/common/full-derivation-chain.md:640, and the
%   softer twin "close to that of liquid water" at
%   ave-kb/vol1/dynamics/ch4-continuum-electrodynamics/lc-electrodynamics.md:71 with its print
%   mirror vol_1_foundations/chapters/04_continuum_electrodynamics.tex:211 (a different lane's
%   file -- not edited here).
(An \emph{order-of-magnitude consistency observation}: $\nu_{kin}$ lands near the kinematic viscosity of liquid water. It is \textbf{not} a prediction and \textbf{not} a claim that the vacuum behaves like water in any operationally relevant sense --- non-claim per \kbleaf{ave-kb/vol1/claim-quality.md}, \texttt{clm-crbl60}. Scope tagged 2026-08-02; the comparison itself is unchanged.)
...
$|\Gamma| \to 1$\gammaundeclared{} faster than the mode spacing shrinks.
