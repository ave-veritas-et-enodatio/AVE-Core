...
\subsection{\texttt{hardware/} --- APU Hardware Architectures}

\begin{itemize}
    \item \texttt{geometric\_diode.py}: Asymmetric geometrical choke limits via Axiom 4 rupture bounds.
    \item \texttt{soliton\_memory.py}: Faddeev-Skyrme soliton memory kinks locking to FCC packing fraction constraints.
    \item \texttt{phase\_locked\_routing.py}: Ideal $\Gamma=0$ continuous waveguide simulation.
    \item \texttt{continuous\_fpga.py}: O(1) solver natively executing continuous geometric routing matrices.
    \item \texttt{resonance\_cavity.py}: Cavity acoustic bounds based on practical/vacuum lattice limits.
    \item \texttt{axiomatic\_io.py}: Klopfenstein geometric impedance steppers matching Continuous Vacuum limits to Discrete limits.
    \item \texttt{spice\_apu\_exporter.py}: Equivalent topological sub-circuit mapper strictly enforcing universal $S(V)$ saturation bounds.
    \item \texttt{spice\_models/ave\_vacuum\_cell.lib}: Canonical ngspice behavioral model library implementing the Axiom~4 kernel as a universal vacuum cell subcircuit.  All domain-specific SPICE models are wiring topologies of this cell (see Appendix~\ref{app:spice_verification}).
\end{itemize}
