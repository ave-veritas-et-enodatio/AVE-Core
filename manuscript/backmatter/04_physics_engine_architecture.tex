...
|     |-- black_hole_core.py       Saturated-core interior
|     |-- cavitation_collapse.py   Cavitation / collapse dynamics
|     |-- electrostatic_core.py    Saturated electrostatic core
|-- regime_4_rupture/             Lattice rupture regime (S = 0) (3 modules)
...
|-- gravity/                     11 modules: solar impedance, galactic MOND
|                                  drag, GW propagation + detector, stellar
|                                  interior, neutrino MSW, planetary
|                                  magnetosphere, Lense-Thirring, orbital LC
|                                  damping, hyperbolic kinematics,
|                                  two-way backreaction
|-- topological/                 19 modules: Faddeev-Skyrme, Borromean,
|                                  Cosserat (+ 3D field), K4 holonomy + K4
|                                  Cosserat coupling, srs-DEC (+ punctured),
|                                  FR braid exchange, helicity observer,
|                                  charge quantization, soliton bond,
|                                  combiner, tensors, operator registry,
|                                  vacuum engine, mixing, entanglement
|                                  thread, moving-front freeze-in
|-- condensed/                   5 modules: silicon crystal, Si/Ge/GaAs
|                                  doping, BJT mechanics
|-- nuclear/                     8 modules: Si nucleus + per-species atoms
|                                  (B, C, P, As, Ga, Ge, Si)
|-- plasma/                      cutoff.py, superconductor.py
|-- qed/                         birefringence.py, brillouin_cutoff.py
|-- thermal/                     f6_bath_meter.py

Tier 2c: DISPATCH, BENCH AND GATE INFRASTRUCTURE
|-- facade/unified_engine.py     Regime-dispatch facade over the certified
|                                  backends (reuses, never re-implements)
|-- bench/                       8 modules: channel-agnostic bench model,
|                                  apparatus, adopters, SNR, sweep, SPICE
|                                  runner, birefringence, validate
|-- validation/                  6 modules: equation audit, planted source,
|                                  reconcile gate, runtime independence,
|                                  spectral liveness, structural degeneracy
|-- utils/                       4 modules: fast winding extractor, genesis
|                                  parallel runner, topology genus, v8
|                                  instrumentation
|-- viz/style.py                 Manuscript figure house style

Tier 3: SOLVERS (consume Tiers 1+2, never re-derive operators)
   [core/ holds 58 modules and solvers/ 31; the rows below are a
    representative selection, not the full listing]
...
|-- solvers/phase_space_winding   Phase-space winding extraction
...
domain-specific constitutive adapters; Tier~2c provides the regime-dispatch
facade, the bench model and the gate/validation harnesses.
