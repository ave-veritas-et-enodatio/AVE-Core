...
\begin{itemize}
    ...
    \item \textbf{Macroscopic Dielectric Yield Stress:} $\tau_{yield} = \frac{e^2\, \mathcal{V}_{total}}{8\pi \varepsilon_0\, \ell_{node}^4} \approx 1.04 \times 10^{22}\text{ Pa}$
          \quad (simplified from $\rho_{bulk}\, c^2 \cdot \mathcal{V}_{total} \cdot p_c/(8\pi)$; $\mathcal{V}_{total} = 2.0$ is the dual-reactance count ($X_C + X_L$ sectors; see \texttt{common/dual-reactance-storage-taxonomy.md}), not an integrated volume; full derivation in Vol 1 Ch 4 \S\ref{sec:dielectric_yield})
\end{itemize}
