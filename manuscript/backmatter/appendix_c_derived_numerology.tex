...
\textit{Axiom trace:} Axiom~1 LC mechanism + $K=2G$ operating point at Axiom~2's $\alpha$ → Cauchy boundary conditions → $\nu_{vac} = 2/7$.
...
\textit{Axiom trace:} Axiom~1 LC mechanism + Axiom~2 ($\alpha \equiv p_c/8\pi$, taken as input) → EMT $K/G = 2$ → quadratic solve → $z_0 \approx 51.25$ located at $\alpha$ by construction (consistency illustration, not an independent $\alpha$).
...
\end{equation}
\textit{Axiom trace:} Axiom 4 ($\mathcal{S}^2 + r^2 = 1$) → Avalanche $M = 1/\mathcal{S}^2 \rightarrow n=2$ → Axiom 1 LC mechanism + Axiom 2 ($K=2G$ at $\alpha$) gives $\nu_{vac}=2/7$ → $n_{3D} = 38/21$.
...
\end{equation}
\textit{Axiom trace:} Axiom 1 LC mechanism (S-matrix for uniform isotropic lattice intersections) → cascade efficiency $\eta = 3/4$ $\rightarrow C_K = 4/3$.
