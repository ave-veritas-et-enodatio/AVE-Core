...
- **Lattice-genesis single-seed cosmology** — the observable universe emerged from a single chiral seed crystallizing under $\hat{\Omega}_{freeze}$. **Single-seed is scoped *within our cosmic horizon*** (one seed event, not many simultaneous disjoint seeds); it is **NOT** a claim ours is the only universe. Our cosmic $\Gamma=-1$ horizon is itself a parent-BH Schwarzschild radius, so our universe is a **daughter** of a parent cosmology, and further daughters are **permitted** (nested parent/daughter across $\Gamma=-1$ melt→recrystallize boundaries; [`common/cosmic-parameter-horizon-a031-refinement.md`](../../common/cosmic-parameter-horizon-a031-refinement.md) + [`common/omega-freeze-cosmic-grain-cascade.md`](../../common/omega-freeze-cosmic-grain-cascade.md)).
- **BH interior = ruptured-plasma phase** — substrate locally in Regime IV at cosmological strain; shear modulus phase-transitions $G_{shear} \to 0$. **SUBSTRATE SINGLE-MEDIUM THROUGHOUT — RUPTURED-PLASMA, NOT a separate B-sublattice.** The interior is a **melt** (a hot $\varepsilon$ free-store where the parent K4 crystal has dissolved); a melt **can** cool and re-crystallize into a daughter K4 lattice — a **daughter cosmology is permitted** (our own universe is such a daughter; [`common/omega-freeze-cosmic-grain-cascade.md`](../../common/omega-freeze-cosmic-grain-cascade.md) §3.3). The $\Gamma=-1$ horizon is a **two-way-opaque symmetric membrane**; because the interior is unobservable from outside, no claim is made about whether a *specific* BH has re-crystallized — the cascade is permitted, not proven-universal.
...
| CLAUDE.md INVARIANT-S2 | Axiom 4 universal saturation kernel verbatim; operating-point gauge-relativity statement; two effective wave speeds discipline ($c_{EM}$ vs $c_{shear}$); SYM-class α-invariance. (Note: INVARIANT-S2 is the AVE-axiom-numbering invariant; it does **not** carry cosmic-genesis / single-seed framing — that is the cosmology-sector canon at [`common/omega-freeze-cosmic-grain-cascade.md`](../../common/omega-freeze-cosmic-grain-cascade.md).) |
