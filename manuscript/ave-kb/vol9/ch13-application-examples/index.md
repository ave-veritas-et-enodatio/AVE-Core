...
| Schwarzschild + Machian $G$ as gradient-index TL | $G_{vac}$, $K=2G$, $\nu_{vac}=2/7$, $c_{EM}$ vs $c_{shear}$ (Pitfall \#5), SYM-class $\alpha$-invariance, Machian-G closed form (mixed: derived form, calibration-fitted $\xi$ value; `ilk-gravmb`) | Ch~3 + Ch~7 + Ch~9 + Ch~12 → $c_{group} = c_0\sqrt{1-r_s/r}$ + exact $\alpha$-invariance + $G = c^4/(7\xi T_{EM})$ (form derived; $\xi$ from CODATA $G$) | [`vol3/gravity/ch01-gravity-yield/alpha-invariance-symmetric-gravity.md`](../../vol3/gravity/ch01-gravity-yield/alpha-invariance-symmetric-gravity.md) (`clm-3zz0f6`); [`common/full-derivation-chain.md`](../../common/full-derivation-chain.md) (`ilk-gravmb` — 2026-07-20 remap from phantom `clm-9oazz0`, matching the correct `ilk-gravmb` usage in the row's own in-cell tag + the §44 Machian-G bullet); [`common/trampoline-framework.md`](../../common/trampoline-framework.md) §5.5 (Machian bulk integral) |
| $W$/$Z$ gauge bosons → transformer-leakage cutoff modes | $\gamma_c$ Cosserat couple-stress, $l_c = \sqrt{\gamma_c/G_{vac}}$ weak-force range, Perpendicular Axis Theorem at bond cylinder $J=2I$, $\nu_{vac}=2/7$ | Ch~9 + Ch~10 + (Ch~15 joint kill-switch) → $m_W/m_Z = \sqrt{7}/3 ≈ 0.882$ + $\sin^2\theta_W = 2/9$ + below-cutoff Yukawa | [`vol2/particle-physics/ch05-electroweak-mechanics/gauge-boson-masses.md`](../../vol2/particle-physics/ch05-electroweak-mechanics/gauge-boson-masses.md) (`clm-5zuo7g`); [`common/trampoline-framework.md`](../../common/trampoline-framework.md) line~192 (rotation-sector mass-gap $m_\omega$) |
...
| **Worked cell analysis (SPICE phase-1 ladder)** | RC/LC `.tran`; Poisson `.OP` (MNA = srs Laplacian); 1D LC-ladder dispersion `ka = 2·asin(ω/2ω₀)`; biased small-signal `C_ss = C₀/S(V_b)³` (large-vs-small-signal split); Ax4 metric varactor keyed $V_{snap}$ | Ch~3 (varactor) + Ch~4 ($L_{cell}/C_{cell}/Z_0$) + Ch~7 (Ax4 kernel) → ngspice-46 exercises the cell models, each rung analytic-target vs measured | `research/2026-07-04_spice-phase1-ladder_result.md`; netlists `src/scripts/vol_4_engineering/spice_ladder_artifacts/`; figure `manuscript/vol_9_vacuum_datasheet/figures/spice_ladder_cells.tex` |
...
- **Machian $G$ is MIXED** (G-ruling 2026-06-14, `ilk-gravmb`) — form-derived (the Achromatic-Lens), value-fitted ($\xi$ back-solved from CODATA $G$; Chain B$'$ open); canonical at `common/full-derivation-chain.md` (id `ilk-gravmb`, 2026-07-20 remap from phantom `clm-9oazz0` — consistent with this bullet's own `ilk-gravmb` tag above); not a primitive axiom. NOT "echo" — the derived-form half stands. *(Mirrors the §-20 table row + 13.tex; the high-salience row was reconciled first, this carry-forward bullet follows per the parallel-site gate.)*
...
- [`common/full-derivation-chain.md`](../../common/full-derivation-chain.md) — canonical closed-form for $G$ + Machian-hierarchy coupling $\xi$ (`ilk-gravmb` — 2026-07-20 remap from phantom `clm-9oazz0`, matching the :44 Machian-G MIXED bullet)
- [`common/trampoline-framework.md`](../../common/trampoline-framework.md) — Cosserat rotation-sector mass-gap line~192; Machian-G bulk integral §5.5
