...
**Discipline:** This leaf is the **source of truth** for the vacuum-node IM3 / harmonic-distortion datasheet characterization. The CODE demonstration is `src/scripts/vol_9_device/im3_vacuum_harmonic_distortion.py` (results `manuscript/vol_9_vacuum_datasheet/results/im3_vacuum_harmonic_distortion.json`), all constants pulled live from `ave.core.constants` (no hardcoded canon), kernel-Taylor coefficients validate-on-known with HALT-on-mismatch. Research provenance: [`research/2026-06-24_e4-im3-vacuum-distortion.md`](../../../../research/2026-06-24_e4-im3-vacuum-distortion.md).
...
> 🚩 **FLAG — DO NOT FIX (2026-08-03). Same-form keying question, routed to Grant; nothing above or below is edited.** The ε-grade bullet at `:79` writes the **divergent** $C_{eff} = C_0/S(A_V)$ with $A_V = V/V_{yield}$ (and `:34` defines $A \equiv E/E_{yield} = V/V_{yield}$, $E_{yield} = \sqrt{\alpha}E_{crit}$). Per the ratified `def-vyvsn1` grade-fork (Grant 2026-06-30, [`../../vol4/circuit-theory/ch1-vacuum-circuit-analysis/nonlinear-vacuum-capacitance.md`](../../vol4/circuit-theory/ch1-vacuum-circuit-analysis/nonlinear-vacuum-capacitance.md):18), the *divergent* $C_0/S$ belongs to the **longitudinal-A1 bond compliance** (bound $V_{snap}$), while $V_{yield}$ is the **transverse-T2 self-trap wall** whose constitutive law rolls off as $C_{diel} \propto S$. That upstream already prints $V_{snap}$; this leaf still prints $V_{yield}$.
>
> **What is and is not exposed, stated precisely rather than assumed.** The **chord-candidate content of §4 is NOT exposed**: the E-vs-B asymmetry turns on *which drive variable keys which grade* (voltage keys ε, circulating current keys μ), which is orthogonal to *which wall normalizes* $A$. The static-B categorical zero survives either keying, because it comes from $I_{vac}=0$, not from the choice of denominator. **What IS exposed if the fork resolves A1/re-key** is the *magnitude* leg — the $A$-definition at `:34` and every $\delta n$ that rides on $A^2$, including the headline $\delta n$ and the bankable ratio at `:73`. **That exposure is named here, not evaluated here:** computing a replacement magnitude would presuppose the ruling. Full statement of the fork, the recomputed $1/\alpha$ consequences at the sibling leaf, and the routed question: [`../../vol4/circuit-theory/ch1-vacuum-circuit-analysis/parametric-coupling-kernel.md`](../../vol4/circuit-theory/ch1-vacuum-circuit-analysis/parametric-coupling-kernel.md) (head banner, same commit).
>
