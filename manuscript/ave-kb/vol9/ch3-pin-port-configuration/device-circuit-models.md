...
| (iv) | `fig:vol9_graded_network_op_sweep` | **Operating-point sweep** — channel impedances vs saturation $S$: $Z_{\mathrm{EM}}$ flat (matched), $Z_{\mathrm{bulk}}=\sqrt2\,Z_0\to0$ and $Z_{\mathrm{shear}}\to0$ as $A\to1$. *To be generated by the coupled-network derivation task.* |
