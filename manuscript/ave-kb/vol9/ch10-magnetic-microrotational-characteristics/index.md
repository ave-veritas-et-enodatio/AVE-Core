...
The substrate's $I4_1 32$ chirality enters the magnetic/microrotational sector as a **non-reciprocal inter-tank coupling**. Per the Fork-A node-coupling result (PR #321 merged; canonical at [`device-circuit-models.md`](../ch3-pin-port-configuration/device-circuit-models.md):159-165,201-227 and `research/2026-06-20_node-circulator-coupling.md`), a **conservative skew-Hermitian circulator coupling EXISTS** between the bulk (mass/A1) and shear (charge/Cosserat) sublattice tanks — it is bounded/lossless (norm conserved to $1.1\times10^{-12}$/40k steps, does NOT pump), transfers helicity (100% bulk$\to$shear, vs the failed graft-v3 $\sim$2%), and acts on the poloidal winding (ON $\ne$ OFF). The verdict is **PARTIAL**:
...
- [`vol9/ch3-pin-port-configuration/device-circuit-models.md`](../ch3-pin-port-configuration/device-circuit-models.md):159-165,201-227 (`def-ch1crc`, `def-0pt1ac`) — the chiral circulator (non-reciprocal inter-tank, 3-port) vs the optical-activity gyrator (reciprocal 2-port); Fork-A PARTIAL verdict, non-reciprocity magnitude imposed-echo.
