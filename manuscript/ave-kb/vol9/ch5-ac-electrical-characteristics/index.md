...
| `src/ave/core/constants.py:78-206` (via `ave-canonical-source` skill) | $\mu_0$, $\varepsilon_0$, $Z_0$, $c_0$, $\ell_{node}$ canonical primitives |
...
PONDER-05 (DC-biased quartz at $V_{DC}/V_{yield} = 0.687$) is the canonical bench-scale falsifier of the operating-point $A_0$ small-signal modulation mechanism. At $V_{DC}/V_{yield} = 0.687$: $S(A_0) \approx 0.727$, so $\varepsilon_{eff}/\varepsilon_0 \approx 0.727$; quartz LC-resonance shift vs zero-bias is the bench-scale observable. Canonical pointer at CLAUDE.md INVARIANT-S2 small-signal block. Hardened build artifacts live in the `ave-veritas-et-enodatio` private repository per `vol4/index.md` repo-scope note; the AC-characteristic spec that PONDER-05 tests is documented in this chapter. Cross-reference: Vol 9 Ch.15 falsification programme.
