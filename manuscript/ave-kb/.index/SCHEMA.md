...
```typescript
{
  ...
  real_or_fitted: string,        // "real-geometric-constraint" | "fitted-identification" — the chord/echo classification
...
**`real_or_fitted` is the chord/echo axis (definitive rule, INVARIANT-S13).** `real-geometric-constraint` = a **chord**: the substrate independently forces the relation, so it removes one degree of freedom from the calibration-parameter count (its `derived_endpoint` becomes dependent). `fitted-identification` = an **echo**: a named identification the substrate does NOT independently select (a consistency match), so it buys **NO** parameter reduction — the count is unchanged. This is `consistency-vs-emergence` made machine-enforced per mechanism rather than narrated. A `derived_endpoint` is meaningful only on a `real-geometric-constraint`; on a `fitted-identification` it is informational (the constant that WOULD become dependent if the mechanism were ever lifted fitted→real).
...
**LIVE independent-parameter count (replaces the prose-stated count).** The framework's calibration-parameter count was previously stated only in prose (`clm-5xon03`: 26 SM parameters → `{m_e, α, G}` + 4 axioms). It is now ALSO a DERIVED number the verifier recomputes and CI-asserts:

```
independent_count = (# claim nodes with build_band == "input-only")
                    − |{ derived_endpoint(m) : m is a REAL-tagged interlock-mechanism
                                               ...
                                               and that claim is itself input-only }|
```

A `real-geometric-constraint` mechanism is a **chord**: it removes one DOF (its `derived_endpoint` becomes dependent) and lowers the count. A `fitted-identification` mechanism is an **echo**: it removes NONE, so the count is unchanged ("an echo buys no parameter reduction; only a chord does"). The interlock register declares an `expected-independent-count:` in its meta header; `verify-kb-metadata` recomputes and **hard-fails on mismatch**. So a tag flip (`fitted`↔`real`) visibly moves the count and breaks the assertion until the author deliberately updates it — the loud-on-drift gate applied to the framework's headline parameter-economy claim.

> **Semantics caveat (flagged for adjudication, 2026-06-14).** "input-only nodes" above is the literal `build_band == "input-only"` set (143 at materialization), which is a BUILD-READINESS band, NOT the calibration-parameter enumeration `{m_e, α, G}` (3) of `clm-5xon03`. The two are different quantities ("input" has two meanings). The MECHANISM (fitted→no-drop, real→−1, CI-asserted, tag-flip-sensitive) is delivered faithfully against the literal `build_band` set; whether the canonical denominator should instead be a scoped calibration-parameter subset is an OPEN adjudication item (the count function is written so the node set is swappable).
