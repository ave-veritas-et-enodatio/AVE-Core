...
**Sort key.** Records are sorted by `(node_type, id)` — explicit grouping by ASCII order of the discriminator: axioms, then claims, then **definitions**, then experiments, then invariants, then **support** (alphabetical: axiom < claim < **definition** < experiment < invariant < support). The Stage-2 `def-` materialization (LIVE 2026-06-08) inserts the `definition` group between `claim` and `experiment`, so no pre-existing record's relative order changed — a new group was inserted, the existing nodes stay byte-identical.
