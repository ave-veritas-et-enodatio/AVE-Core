...
Fluorine-19 ($4\alpha + ^3\text{H}$) is the most extreme core-plus-halo nucleus in the periodic table. The semiconductor engine fixes the Oxygen-16 tetrahedral core at $R_{\text{core}} = 33.383\,d$ (Small Signal, $V_R/V_{BR} = 0.030$, $M = 1$), then fits the Tritium halo offset that reproduces the CODATA mass of $17\,692.302$ MeV. The result: $R_{\text{halo}} = 398.478\,d$, a convergence to $0.000\,000\%$ fitting tolerance on the single fitted halo scalar (not an ab-initio mass-prediction error).
