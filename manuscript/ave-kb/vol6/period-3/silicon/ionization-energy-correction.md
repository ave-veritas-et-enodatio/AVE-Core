...
\begin{enumerate}
    ...
    \item **Op3 $\to$ Op10 bridge (Malus's law):**  The reflected power fraction maps to a complementary projection angle: $\cos\theta = 1 - 2|\Gamma|^2 = 1 - 2 \times 0.250 = 0.500$, giving $\theta = 60.0^\circ$.
    \item **Op10 (Junction projection):**  $Y = c(1 - \cos\theta)/(2\pi^2)$ with $c = 2$ crossings: $Y = 2 \times 0.500 / (2\pi^2) = 0.0507$.
...
\begin{itemize}
    ...
    \item Silicon-28 sits at the Small-Signal boundary ($V_R/V_{BR} = 0.050$)---the AVE-mechanism correlate of being deep in the linear regime yet externally modulable; it is the heaviest symmetric alpha-cluster element that operates in the fully linear regime.
    \item The first ionization energy ($8.147$~eV vs $8.152$~eV, $-0.06\%$) is reproduced by Op10 junction projection at the co-resonant $n = 2$ shell boundary (Correction~C), confirming the systematic pattern from Aluminum.
\end{itemize}
\end{summarybox}
