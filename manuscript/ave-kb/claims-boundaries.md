...
> **References:** Bound asserted at invariant level — see `LIVING_REFERENCE.md` Axiom 3 entry ("α invariance" sub-bullet, and the "Lattice decomposition" sub-bullet for the temporal/spatial split) and `LIVING_REFERENCE.md` Pitfall #5. Note: [`CLAUDE.md`](./CLAUDE.md) INVARIANT-S2 lists the four AVE axioms in their bare form; the α-invariance sub-bullet under Axiom 3 is in LIVING_REFERENCE.md, not in CLAUDE.md. Supporting derivation steps appear in `vol1/dynamics/` (saturation kernel, master equation) and `vol3/gravity/` (symmetric-gravity mapping); these establish the cancellation mechanism but do not explicitly state the invariance result. The α thermal-running effect derivation is in [`vol1/ch8-alpha-golden-torus.md`](vol1/ch8-alpha-golden-torus.md). Followup logged: leaf-level explicit statement of the invariance bound is a KB content gap (see `kb-claims-boundaries-followups.md`).
