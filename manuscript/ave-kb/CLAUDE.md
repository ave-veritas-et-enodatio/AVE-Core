...
**Operating-point state and small-signal modulation:** beyond the 6 spatial DOFs per node (Ax 1), each LC tank carries a saturation-amplitude state $A$ — its operating point along the Axiom 4 kernel. This state is **gauge-relative**: only spatial gradients of $A$ across the substrate are physically observable, not absolute per-node values. Small-signal transverse propagation through a region at operating point $A_0$ sees modulated effective parameters $\varepsilon_{eff} = \varepsilon_0 S(A_0)$, $\mu_{eff} = \mu_0 S(A_0)$, $C_{eff} = C_0/S(A_0)$, $c_{eff} = c_0\sqrt{S(A_0)}$ — the same varactor-bias mechanism producing refractive-index gradients across all scales (Op14 local clock modulation, Op16 universal wave speed). Gravity is the SYM-class realization: $\mu(r)$ and $\varepsilon(r)$ scale together so $Z_0$ stays invariant, but $c_{eff}$ tracks $\sqrt{S(r)}$, reducing to Schwarzschild $c\sqrt{1-r_s/r}$ in the weak-field limit. This is **not** a 7th spatial DOF — it is the dynamical state of the LC tank, analogous to DC bias on a semiconductor varactor. PONDER-05 (DC-biased quartz at $V_{DC}/V_{yield} = 0.687$) is the canonical bench-scale falsifier of this mechanism.
