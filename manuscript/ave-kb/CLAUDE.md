...
**Asymmetric scaling (ASYM-class)** — when $\mu$ and $\varepsilon$ scale by different factors (canonical at clm-8nkvwy:112; applies to strong-field large-amplitude saturation regimes) — α is NOT invariant; this is the canonical mechanism for substrate-distinct α-modulation predictions. Low-amplitude thermal-bath regimes (e.g., $\delta_{strain}$ at $T_{CMB}$) require either ASYM scope-extension or a third substrate-mechanism path (Q-DELTA-MAP-1 open).
