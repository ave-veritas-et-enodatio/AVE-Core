...
- solidity: 0.70 (ok to build on, see caveats) [= min(0.70, 1.00)]
- rationale: The constitutive step $\eta_{eff}(V)=\eta_0$ below / $0$ above $V_{yield}$ is well-posed, and the yield-stress numeral $\tau_{yield}=\rho_{bulk}c^2\mathcal{V}_{total}\,\alpha\approx1.04\times10^{22}$ Pa closes arithmetically. But it imports several framework-derived inputs — $\rho_{bulk}$, the dual-reactance count $\mathcal{V}_{total}=2.0$ ($X_C + X_L$ reactance sectors, Axiom 1; counted, not integrated; NOT a FEM-integrated volume — the prior "FEM-verified halo volume" provenance was fabricated and is dropped, see `common/dual-reactance-storage-taxonomy.md`), and the porosity identity $p_c/(8\pi)=\alpha$ — and the leaf discloses the step is idealized (transition width unspecified). A clean constitutive derivation gated by clearly-disclosed imports. (Open structural item: the yield *stress* currently inherits the value 2 from $\mathcal{V}_{total}$; the corpus does not yet derive that the yield scales with the reactance-sector count — see `common/dual-reactance-storage-taxonomy.md` §τ_yield open item.)
- strengthen-by:
