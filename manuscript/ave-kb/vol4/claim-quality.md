...
- _Specific Non-Claims and Caveats_
  ...
  - **The three-way varactor-convention tangle is OPEN (surfaced for Grant, NOT resolved).** (a) node-up:105/:370 ($C_0/S(A_V)$ = ε-grade varactor, DERIVED) vs (b) `CLAUDE.md`:73/:75 (assigns $C_0/S$ to A1, $\varepsilon_0 S$ to T2, calls both "small-signal") vs (c) `device-circuit-models.md`:60 (chord $C_0/S$ vs small-signal differential $C_0/S^3$, A1-scoped). The M3 coefficients (chord $C_0\!\cdot\!S$ → $1-\tfrac12 A_0^2$; $\mathrm{d}Q/\mathrm{d}V$ tangent → $1-\tfrac32 A_0^2$) are carried KEEP-BOTH, **neither crowned** — M3 is convention-robust (the kill holds in every convention), but which object is "the small-signal C" is a corpus-consistency call for Grant. See the RESULT § DISCIPLINE ⚑ merged flag block.
