...
> **★ Keying correction + status update (KEEP-BOTH; roll-off ruling ratified 2026-07-06/07, PR #562/#558).** Per the ruling, the **divergent** $C_0/S$ form IS the **longitudinal-$A_1$ bond compliance keyed on $V_{snap}\approx511$ kV** — its kernel argument is $V/V_{snap}$, NOT $V/V_{yield}$. The $V_{yield}$-keyed object is the *transverse-$T_2$ dielectric* $C_{diel}=C_0 S$ (roll-off, ×S), a DISTINCT reactance an across-gap LCR meter reads. The $V_{yield}$-keyed **divergence** statements in this claim (the equation above and the "diverges as $V\to V_{yield}$" claim below) are preserved as the **superseded mixed form** (divergent $C_0/S$ ÷S shape with the transverse $V_{yield}$ key). Canonical: [`dielectric-plateau-prediction.md`](./falsification/ch12-falsifiable-predictions/dielectric-plateau-prediction.md):25-38.
...
- _Specific Non-Claims and Caveats_
  - **Sector (Q1 = (B), Grant-ratified 2026-06-15; `research/2026-06-15_ceff-epsilon-monotonicity_result.md`):** this $C_0/S$ is the **longitudinal-A1 bond compliance** ($1/k_a$), NOT the transverse dielectric capacitance — orthogonal reactances sharing the EE name "capacitance" (INVARIANT-S2 sector split). The ×S bench/SPICE leaf [ee-bench-netlist](./simulation/ch17-hardware-netlists/ee-bench-netlist.md) measures the **transverse dielectric** $C_{diel}=\varepsilon_{eff}A/d\propto S$ (rolloff) — a DISTINCT quantity, also correct. The Leaf-references footer therefore spans both signs. **✅ RESOLVED 2026-07-06/07 (PR #562/#558; superseded wording preserved KEEP-BOTH):** an **across-gap** precision LCR couples to the **transverse dielectric** (rolloff, ×S, keyed on $V_{yield}\approx43.65$ kV); the longitudinal-$A_1$ compliance (spike, ÷S) is keyed on the higher $V_{snap}\approx511$ kV and is NOT read across-gap. The sign of the deviation is itself the sector discriminator ([`dielectric-plateau-prediction.md`](./falsification/ch12-falsifiable-predictions/dielectric-plateau-prediction.md):27,38). *(Superseded: "whether a physical precision-LCR couples to the longitudinal compliance (spike, ÷S) or the transverse dielectric (rolloff, ×S) is an **OPEN measurement-sector question flagged to Grant** … left as-is pending that ruling (no silent leaf-frontmatter split)." — the ruling has since been made.)*
...
- strengthen-by:
  ...
  - <s>Resolve in-leaf the constitutive-parameters-individually-collapsing vs ratio-preserved distinction so the BH-echo branch is not in apparent tension with $\Gamma=0$ everywhere.</s> *(superseded 2026-06-17 — resolved via channel-split above; preserved per Rule-12.)*  <!-- rule12-freeze: base=2e1f5257414492b8667b4530d44a09686127bf32 region=above offset=0 lines=14 bytes=1882 sha256=e7c14db07f856ab7df03167ff86078b97fd182ab7b38efe4099d1844444187d3 -->
...
- _Specific Non-Claims and Caveats_
  ...
  - The active acoustic-metric compression mechanism (3D standing tensor shockwave producing $n > 1$) is **proposed** as the engineering pathway; the framework provides the threshold ($n^* = 1.114$) but no leaf-level derivation that any specific apparatus achieves it. 🔴 **[DEMOTED 2026-08-11 — R40-B2a: NEEDS RE-DERIVATION, not dead; dated note at the end of this file]**
...
> 🔴 **QED-NORMALIZATION CORRECTION (2026-07-03; Rule-12 — body preserved below, git is the trail).**  <!-- rule12-freeze: base=3924169aa6aeb191e911294e41af176ee75531ae region=below offset=172 lines=81 bytes=8640 sha256=b0241bc5fe3e8f4313fb0f181d1d006b300e8ec4ea85f0109c1741b69a50dffa -->
...
> 🔵 **OPTION-B FOOTING CONSOLIDATION (2026-07-07; Grant-fired — the body below is re-frozen to the
> current consistent-instantaneous footing; the 🔴/🔵 notes above are preserved verbatim as the dated
> convention-history record).** PR #545 deliberately left this entry's body under the header-supersession
> chain (Option A); Grant has now fired **Option B**: the body is consolidated to the **current headline**
> carried by the canonical leaf's FOOTING RE-FREEZE note — on a single consistent **instantaneous** footing
> (the instantaneous SVE kernel $-\tfrac12 A^2$ paired against the **instantaneous** one-loop QED coefficient
> $2\alpha/(15\pi)$):
> $$\boxed{\;\frac{\delta n_{AVE}}{\delta n_{QED}}=\frac{15\pi}{4\alpha^2}=\frac{3.75\pi}{\alpha^2}\approx2.2\times10^5\;}$$
> **Two-step history, stated once (KEEP-BOTH):** $7.5/\alpha^3\approx1.93\times10^7$ (v1 original — understated /
> mismatched QED denominator $(3/45)\alpha^2$) $\to$ $7.5\pi/\alpha^2\approx4.42\times10^5$ (v2, QED-normalization
> corrected, propagating / **mixed** footing — the 🔴 note above) $\to$ $3.75\pi/\alpha^2\approx2.2\times10^5$ (v3
> current, consistent **instantaneous** footing; **exactly half** v2, the $\langle\cos^2\rangle=\tfrac12$ carrier
> average). **Numbering disambiguation:** this note's chain labels (v1 $=7.5/\alpha^3$, v2 $=7.5\pi/\alpha^2$,
> v3 $=3.75\pi/\alpha^2$) differ from the **Letter's** numbering used by the canonical leaf's preserved 🔵
> FOOTING RE-FREEZE note, in which the SAME $7.5\pi/\alpha^2$ object is the Letter's **'v1'** — so this note's
> **v2** $\equiv$ the Letter's **'v1'**. v1 and v2 are preserved verbatim in the 🔴/🔵 notes above; the
> OpenTimestamps anchor `claim-prereg-ots/claims_by_hash.md` carries that object — this note's v2 (= the
> Letter's 'v1') $7.5\pi/\alpha^2$ value — **byte-identical, not re-stamped**.
> The 🔵 differential-observable promotion above **stands** (the par$-$perp differential remains the canonical
> falsifier observable); only its printed VALUE is re-frozen here (v1$\to$v3). **Solidity and confidence are
> UNCHANGED — this is a display-footing consolidation, NOT a physics change:** no order of magnitude and **no
> falsifier verdict** moves (the SVE $P_{flip}\sim5\times10^{-3}$ headline is footing-invariant; the kill
> criterion $P_{flip}<10^{-8}$ is unchanged). Provenance: the canonical leaf
> [`vacuum-birefringence-e4`](./falsification/ch12-falsifiable-predictions/vacuum-birefringence-e4.md) FOOTING
> RE-FREEZE note; the flagship Letter `papers/2026_birefringence_letter/provenance.md` §9 (Arm-2 re-freeze,
> Grant ruling verbatim) + §10 item-4 (leaf/register reconciliation).

- **AVE (canonical falsifier observable — the par$-$perp DIFFERENTIAL):** $\delta n_{bir}=n_\parallel-n_\perp\approx-\tfrac12 (E/E_{yield})^2$ (uniaxial probe tensor $\varepsilon_{ij}=\varepsilon\delta_{ij}+2\varepsilon'E_{0i}E_{0j}$ = exact differential of the scalar Ax-4 kernel; DERIVED, OQ-1). **QED (differenced the same way, on the consistent instantaneous footing):** $\delta n_{QED}\approx(2\alpha/15\pi)\,(E/E_{crit})^2$ (the instantaneous one-loop Euler-Heisenberg coefficient — PVLAS-$A_e$-anchored via the $E\leftrightarrow cB$ duality; its $\langle\cos^2\rangle=\tfrac12$ carrier average is the propagating headline $\alpha/15\pi$). **Both are $E^2$-leading**; the discriminator is the COEFFICIENT, not the exponent. Matched field-independent ratio $\delta n_{AVE}/\delta n_{QED}=15\pi/(4\alpha^2)=3.75\pi/\alpha^2\approx2.2\times10^5$ (v3, consistent instantaneous footing — see the Option-B note above; the v1 $7.5/\alpha^3\approx1.93\times10^7$ and v2 $7.5\pi/\alpha^2\approx4.42\times10^5$ are convention history). **(KEEP-BOTH secondary — the isotropic single-arm:** the scalar common-mode shift $\delta n_{iso}=\sqrt S-1\approx-\tfrac14 A^2$ is what a birefringence polarimeter is BLIND to — the **single-arm-retardance-probe** pairing for a distinct interferometric single-pass-phase instrument class, NOT the polarimeter/ellipsometer headline; its ratio (quoted $\approx4.14\times10^6$ on the v1 footing, paired against the QED parallel single-mode $7/45$) is a mismatched-observable traceability figure carried as v1 convention history, NOT re-frozen here.)
- _Specific Claims_
  - High-intensity laser interferometry specifying a transverse FIELD $E$ (not a gap-voltage $\Rightarrow$ no per-node conflation, $A=E/E_{yield}$ directly). AVE's vacuum saturates at $E_{yield}=V_{yield}/\ell_{node}\approx 1.13\times10^{17}$ V/m with an O(1) (un-suppressed) nonlinearity; QED at $E_{crit}\approx 1.32\times10^{18}$ V/m with an $\alpha^2\sim 5\times10^{-5}$ loop suppression $\Rightarrow$ **AVE predicts $\delta n_{bir}\approx 2.2\times10^5\times$ QED at the matched differential observable at any field** (field-independent structural ratio $3.75\pi/\alpha^2$, v3 consistent instantaneous footing; the v1 $7.5/\alpha^3\approx1.93\times10^7$ and its isotropic-single-arm companion $\approx4.14\times10^6$ are convention history — see the Option-B note above).
...
- _Specific Non-Claims and Caveats_
  ...
  - An $E^2$ slope does **NOT** falsify AVE (QED is also $E^2$-leading). A **QED-sized differential coefficient** ($\delta n_{bir}\sim(2\alpha/15\pi)(E/E_{crit})^2$, $\sim 2.2\times10^5\times$ smaller than the AVE prediction at the same field) **falsifies AVE**; an AVE-sized coefficient falsifies QED at this observable. Two-sided. *(v3 consistent instantaneous footing; the v1 "$\sim\alpha^2$, $\sim10^6\times$" form is convention history — see the Option-B note above.)*
  - The discriminator is facility-class: at $E\sim 10^{14}$ V/m (extreme-laser-reachable) AVE gives the **par$-$perp differential** $\delta n_{bir}\approx 3.9\times10^{-7}$ (high-finesse-cavity measurable) vs the instantaneous-footing $\delta n_{QED}=(2\alpha/15\pi)(E/E_{crit})^2\approx 1.78\times10^{-12}$; the AVE-distinct margin is the $\sim2.2\times10^5$ coefficient gap ($15\pi/(4\alpha^2)$), present at ALL fields, not a regime-gated exponent change. *(v3; the v1 single-arm "$\delta n\approx2.0\times10^{-7}$ vs $\delta n_{QED}\approx5\times10^{-14}$, $\sim10^6$ gap" is convention history — see the Option-B note above.)*
...
- rationale: The discriminator is the COEFFICIENT of the (shared $E^2$-leading) index shift, not the exponent. AVE's $\delta n=\sqrt{S}-1\approx -\tfrac14(E/E_{yield})^2$ (the $n=\sqrt{\varepsilon_{eff}/\varepsilon_0}$ identity applied to the Ax-4 kernel) carries an O(1) coefficient against an un-suppressed yield field $E_{yield}\approx 1.13\times10^{17}$ V/m; QED's Euler-Heisenberg $\delta n\approx a_{EH}\alpha^2(E/E_{crit})^2$ is $\alpha^2$-loop-suppressed against $E_{crit}\approx 1.32\times10^{18}$ V/m. The field-independent matched-differential ratio $15\pi/(4\alpha^2)=3.75\pi/\alpha^2\approx 2.2\times10^5$ (v3 consistent-footing; the v1 single-arm figure $1/(4\,a_{EH}\,\alpha^3)\sim 10^6$ is convention history — see the Option-B note) is AVE-distinct at ALL fields (forward driver `birefringence_coefficient_discriminator.py`). The historical "$\Delta n\propto E^4$" framing was a $\sqrt{\varepsilon}$ conflation (it Taylor-expanded the permittivity DEPTH $1-S=+A^2/2$, itself $E^2$-leading, not the index shift) — corrected; an $E^2$ slope does not falsify AVE.
- strengthen-by:
  - **OQ-1 field→cavity-phase coupling: DERIVED (CLOSED 2026-06-21).** The prior "Gaussian-overlap asserted" residual is closed — the coupling is the exact differential of the scalar Ax-4 kernel (uniaxial tensor → cavity round-trip ellipticity), $g$ pinned per apparatus config. Remaining residuals: (a) CHECK-3 gated-cavity $\tau_{rt}$ factor-2 / "recovers both finesse and temporal overlap" approximation; (b) polarimetry-floor validate-on-known still owed against a published cavity. The COEFFICIENT ($3.75\pi/\alpha^2$, v3) depends on neither. Derivation: `research/2026-06-21_oq1-field-to-cavity-phase-coupling-derivation.md`; driver `src/scripts/vol_9_device/oq1_field_to_cavity_phase_coupling.py` (validate-on-known PASS: PVLAS $A_e$ to 0.35%).
  - Establish a concrete high-intensity-laser facility path (FIELD-specified, not gap-voltage) reaching $E\gtrsim 10^{14}$ V/m where $\delta n_{bir}^{AVE}\sim$ high-finesse-cavity measurable and the $2.2\times10^5$ matched-differential coefficient gap is resolvable against the QED instantaneous ($2\alpha/15\pi$) baseline.
  - Pin the QED one-loop instantaneous coefficient $2\alpha/(15\pi)$ (PVLAS-$A_e$-anchored; carrier-averaged $\alpha/(15\pi)$ propagating, $\alpha/(30\pi)$ static-duality) under the facility's exact field-polarization and temporal geometry to tighten the ratio band. (The earlier AVE-side differenced $3/45$ estimate understated the **static-duality** coefficient $\alpha/(30\pi)$ by $1/(2\pi\alpha)\approx21.8$ — the 🔴 note computes the factor against that form; the consolidated chain instead uses the instantaneous $2\alpha/(15\pi)$, against which the understatement factor is $2/(\pi\alpha)\approx87.2$, and $1/(\pi\alpha)\approx43.6$ against the propagating $\alpha/(15\pi)$.)
...
- _Specific Non-Claims and Caveats_
  ...
  - **↗ DOUBLY-PROTECTED LEADING-ORDER ISOTROPY (2026-07-04, PR #516 MERGED — the parent-condition derivation).** The **VALIDATE-ON-KNOWN leading-order isotropy** of this leaf — the acoustic-speed spread across directions $=0$ at the isotropic-bond point $k_s=k_a$, the "emergent-Lorentz isotropy point" — is now **doubly protected**. (i) It is **cubic-symmetry-automatic** at leading order (a rank-2 velocity tensor is forced isotropic by any cubic point group — the standing argument). (ii) The isotropic-bond point $k_s=k_a$ itself is now **Axiom-3-FORCED**: it is the UNIQUE stiffness ratio where the srs net's internal-boundary acoustic reflection $\Gamma_{internal}(\rho_{bond})$ vanishes ($\rho_{bond}=k_a/k_s=1$ to machine precision, knob-free — the ½/¼ knife passes). So the point at which leading-order isotropy holds is no longer a hand-chosen locus but an axiom consequence (MECHANISM-DERIVED, axiom-manifestation class). **SCOPE (KEEP-BOTH — does NOT un-demote anything):** this strengthens the LEADING-ORDER ISOTROPY statement ONLY. The 🟡 weak-C DEMOTION of the $(q\ell)^4$ photon-DISPERSION tell (gate `wejkhvnfb`, OPEN, caveat above) is **UNCHANGED** — the parent-condition arc derives the isotropy/MATCH pinning, NOT the zone-edge decoupling; solidity is UNCHANGED. Provenance: `research/2026-07-04_parent-condition-match-forces-balance_result.md` (VERDICT BOX §1–§2 + §5), driver `src/scripts/vol_4_engineering/parent_condition_match_forces_balance.py`.
...
> **🔴 DEMOTED (dissipative sub-claim) + FALSIFIER FLIPPED 2026-07-19 (deep-space reactive-bulk ruling, Rule-12 — claims/caveats above PRESERVED unedited).** This claim's trapping mechanism — detritus that "**loses transverse kinetic energy**" / experiences an inductive "**drag spike**", "**shedding orbital energy**" — is the sub-yield bulk resistive loss **DEMOTED** by the ruling (deep-space bulk coupling to slow matter = **lossless / pure-reactance**; Ax3 forbids a bulk resistor, `../common_equations/eq_axiom_3.tex`:24). Two consequences: **(1)** the *impedance-shear / termination-shock* framing (a reflection at an impedance step) can survive as a **reactive** event; the *energy-shedding drag* that stalls the body is retracted. **(2)** The leaf's own falsification target — a Pioneer-transit "sudden spike in transit drag" — is **FLIPPED into a discriminator**: the demoted mechanism predicts a `v`-scaling drag-spike; the reactive ruling predicts **zero energy-shedding drag below the (Cherenkov/Mach) radiation threshold** (a reactive reflection, not a thermalizing stall). A measured `v`-scaling drag-spike would falsify the reactive reading; its absence falsifies the resistive stall. The belt/Oort structure re-derives as **band-structured reactive coupling** (structure-at-resonances; cf. Kirkwood cavity modes `../vol3/cosmology/ch06-solar-system/kirkwood-gaps-cavity-modes.md` at `<0.3%`), **SPEC'd, not run**. Walk-record: `research/2026-07-19_deep-space-reactive-bulk-walk_RECORD.md`. Leaf flags: `boundary-trapping-test.md`, `advanced-protocols.md` (same date). Solidity **unchanged** (already 0.30, "do not build on").  <!-- rule12-freeze: base=aa626690705f170874ee85e923ed170f212e57f3 region=above offset=0 lines=225 bytes=27087 sha256=b641753bbca41340ccde602203dbf665bdf0a5beb599e0ebdd66bdd27c8d16e5 -->
...
> **🔴 SECTOR RELABEL — 2026-08-03 (Oort containment-retraction lane, repair pass; Rule 12 — the claim block at `:1120` above is left BYTE-UNTOUCHED, as the merged 2026-07-19 banner at `:1133` asserts in print: *"claims/caveats above PRESERVED unedited"*). No grade field moves; no id minted.**  <!-- rule12-freeze: base=36662d8596dd69ebb17f1baa947a64bc21b859f3 region=above offset=0 lines=15 bytes=2804 sha256=363bfe5e3469e7ce86fe27e4682fdedeb749701cf855f0d8635a06d7ea46ebf9 -->
>
...
- _Specific Claims_
  ...
  - The real AVE test is the **E-route** (static-$\mathbf E$ / HIBEF-class facility field), which biases the $V$-keyed varactor (R2) and gives the OQ-1 differential ratio $\delta n_{AVE}/\delta n_{QED}=3.75\pi/\alpha^2\approx2.2\times10^5$ (clm-pp3qwf, v3 consistent-footing; the v1 $7.5/\alpha^3\approx1.93\times10^7$ is convention history).
- _Specific Non-Claims and Caveats_
  - AVE does **not** claim the PVLAS null confirms AVE — a null is *consistent with* AVE (and with QED's tiny $\sim10^{-23}$ at 5 T being below sensitivity). The discriminating measurement is the E-route, where AVE and QED diverge by $\sim10^5$.
...
- _Specific Non-Claims and Caveats_
  ...
  - The birefringence-coefficient MAGNITUDE $2.2\times10^5=3.75\pi/\alpha^2$ (v3 consistent-footing; v1 $1.93\times10^7=7.5/\alpha^3$ is convention history) is an **$\alpha$-echo** at the value level (claim-quality.md:399,405). The AVE-distinct chord is existence-of-saturation (tree-level $O(1)$) + the static-B exact-zero.
...
- depends-on:
  ...
  - clm-pp3qwf (E-route birefringence differential coefficient $3.75\pi/\alpha^2$)
...
## ε-grade EM keying is CHARGE-KEYED (mean-square, DC-included) at leading order; excursion/variance keying DERIVED-AGAINST
<!-- id: clm-chgky3 -->

A three-round adversarially-reviewed (thrice-fixed) **blind** derivation routes the ε-grade (transverse-T2 permittivity) EM-keying question to **[DERIVED: CHARGE-KEYED]** (single-cell + lattice-rigid; with a UNIFORM-bias gauge-observability RIDER). The ε-varactor nonlinearity keys on the **MEAN-SQUARE** of the instantaneous field amplitude at the cell — **DC-INCLUDED** — at leading (2nd) order: $\langle 1-S_\varepsilon(A_V)\rangle \stackrel{O(A^2)}{=} \tfrac12\langle A_V^2\rangle$, and $\langle A_V^2\rangle = a_0^2 + \tfrac12 a_1^2$ (for $A_V(t)=a_0+a_1\cos\omega t$) **keeps** the DC baseline $a_0^2$, whereas the variance $\operatorname{Var}_t(A_V)=\tfrac12 a_1^2$ **drops** it. The **excursion/variance** member (the round-2 DC-blind "worked-keyed" candidate) is **DERIVED-AGAINST**: all four candidate mechanisms fail to deliver a lossless ε-side DC-block — M1 (topology): no series-C in the shunt-C T2 path (the canonical series-$L$-bond / shunt-$C$-node unit passes the held DC to the varactor node, a two-topology time-domain computation); M2 (ledger): a two-route energy ledger closes with zero residual, the held energy parked as charge in the saturating shunt capacitor, no linear spectator mode to hide it on; M3 (slide): the small-signal object shifts DOWN nonzero under held bias in *every* convention, and the only relaxation ($\tau_{relax}$ hysteresis) dissipates → Ax3-forbidden. The lattice-level zero-mode question is **settled CLOSED (rigid)** across the whole counted band $A \le 1/\sqrt2$ (K4 carries shear stiffness $k_s>0$). This CONFIRMS the corpus R2 statement (node-up:218, "A static $\mathbf E$ … it loads $\varepsilon$"; clm-vca7r1) at the local ledger and DERIVES-AGAINST the round-2 KEEP-BOTH `R2′` (worked-keyed / DC-blind) candidate. **FORM-level derivation / CONSISTENCY class** (network-topology + axiom-kernel-argument identity — NOT emergence: no new dimensionful number is minted).

- _Specific Claims_
  - **Charge-keyed at leading order.** $\langle 1-S_\varepsilon(A_V)\rangle=\tfrac12\langle A_V^2\rangle$ at 2nd order (sympy `mean_leading = a_0^2/2 + a_1^2/4 = \tfrac12\langle A_V^2\rangle$); the DC-only deficit at $a_1\to0$ is $1-\sqrt{1-a_0^2}=a_0^2/2+O(a_0^4)>0$ for any $a_0>0$ — a held DC alone loads $\varepsilon$. Frequency-independent (numeric-vs-symbolic $\max_{\rm rel}=2.0\times10^{-16}$ across $\omega/\omega_C\in\{10^{-3},10^{-2},0.1,0.5\}$; no $(\omega/\omega_C)^2$ rate factor).
  - **Variance member DERIVED-AGAINST by the network, not selected against a target.** A hypothetical variance-keyed kernel gives $\tfrac12\operatorname{Var}_t=0$ on a held DC; the canonical sqrt-kernel gives $1-\sqrt{1-a_0^2}>0$ on the SAME held DC (they disagree, $0.0202\ne0$ at $a_0=0.2$ — the gate can fire). One mechanism (the ε element is a shunt keyed on the static-capable node potential, with no lossless way to shed a held bias) explains all four M0/M1/M2/M3 verdicts.
  - **Slow-ramp persists (Grant's falsifier → H1).** A field ramped over seconds then held retains the local ε-deficit ($=1-S(E/E_{yield})$, a function of the *settled* amplitude, not the ramp rate); the stress is parked as charge in the saturating shunt cap, and a lossless "forget" would need either a soft zero-restoring mode (none — M3/lattice-rigid) or the dissipative $\tau_{relax}$ relaxation (Ax3-forbidden).
  - **UNIFORM-bias gauge-observability RIDER (CONSISTENCY re-expression of INVARIANT-S2).** The cell IS locally deficient under any held bias; readout is differential. A spatially-**uniform** held bias self-cancels on readout ($A$-state gauge-relative, `CLAUDE.md`:75; PHASE-ONLY north-star, `common/claim-quality.md`:1318). A **non-uniform** held field ($\nabla A\ne0$: atomic Coulomb, bench fringe) IS readable via the Op14 Meissner-asymmetric impedance mirror $Z_{eff}=Z_0\sqrt{S_\mu/S_\varepsilon}$ ($S_\varepsilon<1,S_\mu=1$ ⟹ $\Gamma\ne0$; `CLAUDE.md`:75, `operators.md`:54). This is the discriminating readout that makes the rider falsifiable, not metaphysics.
- _Specific Non-Claims and Caveats_
  - **Leading-order only (Jensen gap at $O(A^4)$).** The exact $\langle 1-S\rangle=\tfrac12\langle A_V^2\rangle$ identity is 2nd-order; at $O(A^4)$ a Jensen gap opens (the sqrt kernel is concave). Both objects keep the DC baseline $a_0^2$, so the H1-vs-H2 (charge-vs-excursion) routing is unchanged by the higher-order gap — but the *exact* $\tfrac12\langle A_V^2\rangle$ equality is not claimed beyond leading order.
  - **Lattice-rigidity rests on a FLAGGED cross-lattice $C_{44}$ BORROW.** The magnitude on the $k_s>0$ rigidity (K4-leaf qualitative structure, `k4-bloch-dispersion-quartic.md`:58) is borrowed from the ratified chiral **srs-z3** net's saturated Born–Huang tensor ($C_{44}=0.17661$ at $A=0$, $0.09213$ at $A_{wall}=0.9$, $0.02536$ at $0.99479$; clm-mfb2ax) — a **borrow of a sibling-lattice magnitude, NOT a K4-native $C_{44}$ derivation**. It plausibly transfers because both nets carry $k_s>0$; the shipped 1-D consistency-encoding driver is NOT an independent K4-tensor verification.
  - **The three-way varactor-convention tangle is RESOLVED (Grant-ratified 2026-07-06; caveat text PRESERVED below, status updated not deleted).** (a) node-up:105/:370 ($C_0/S(A_V)$ = ε-grade varactor, DERIVED) vs (b) `CLAUDE.md`:73/:75 (assigns $C_0/S$ to A1, $\varepsilon_0 S$ to T2, calls both "small-signal") vs (c) `device-circuit-models.md`:60 (chord $C_0/S$ vs small-signal differential $C_0/S^3$, A1-scoped). The M3 coefficients (chord $C_0\!\cdot\!S$ → $1-\tfrac12 A_0^2$; $\mathrm{d}Q/\mathrm{d}V$ tangent → $1-\tfrac32 A_0^2$) were carried KEEP-BOTH, **neither crowned** — M3 is convention-robust (the kill holds in every convention). **✅ RESOLVED 2026-07-07 (Grant-ratified 2026-07-06; payload as enumerated in PR #558):** payload (ii) crowns the tangent $dQ/dV$ as "*the* small-signal" (both sectors) and payload (iii) identifies the chord/tangent pair as the ⊥/∥ polarization eigenmodes of one saturable-dielectric tensor — so "which is *the* C" dissolves at the tensor level (both real, per-polarization; the observable is their split $\delta n_{bir}=-\tfrac12 A_V^2$, sympy-proven, `semiconductor_cv_dip.py::eigenmode_check`; vs Letter Appendix A). The three sources reconcile: (a) was the A1-form-on-T2-key cross-wire (fixed by payload (i)); (b) is correct with "small-signal" read as the crowned tangent; (c) is the crowned-tangent/large-signal-chord distinction. Landed at node-up-small-large-signal.md §1 :105 supersession block + §4 :229 closure note (`analysis/land-varactor-supersession`). Provenance: `research/2026-07-07_semiconductor-cv-dip_RESULT.md` (h); PR #558; the RESULT § DISCIPLINE ⚑ merged flag block. **Solidity is UNCHANGED (0.55):** this resolution touches cap (iii) only; the binding gate is the clm-mfb2ax srs-$C_{44}$ borrow (cap ii), unaffected.
  - **Single-cell + lattice-rigid scope.** The verdict is derived at the single-cell ledger and extended to the lattice via the no-zero-mode (rigidity) argument across the counted band $A\le1/\sqrt2$; the floppy $A\to1$ wall lies wholly inside the excluded interior ($r<r_{turn}$) and is never counted.
  - **CONSISTENCY / FORM-level, NOT emergence.** No new dimensionful constant; the constants used ($\omega_C,\ell_{node},Z_0,E_{yield}$) are imported and appear only in dimensionless bands and the slow-ramp timescale. The uniform-bias rider is a re-expression of the canonical INVARIANT-S2 (the PHASE-ONLY north-star mechanized), not a new mechanism.
  - **Confirms clm-sve3xc as COMPLETE at atomic scales (does not overturn it).** The muonic Coulomb field is static-in-time but spatially non-uniform, so it LOADS under charge-keying — reproducing #539 [C-EXCLUDED] (the interior-excluded bracket integral overshoots the 2.3 µeV CREMA window by $\sim6.6\times10^5\times$; per the round-3 computed band-split, ~103% of that magnitude rides the sub-pitch band [159.6, 386.2] fm — the open [B-AVE] lattice-scale regime-boundary arm's territory — while the super-pitch remainder alone still overshoots by ~2×10⁴×, so [C-EXCLUDED] is robust to a lattice-pitch cutoff; `research/2026-07-06_em-keying-round3-eps-dc-mechanism_RESULT.md` finding [18]). The uniform-bias gauge rider does NOT rescue the muon ($\nabla A\ne0$). So the round-2 conditional-PASS does not become derived; the Letter's protective hard-cutoff reading and clm-sve3xc [C-EXCLUDED] are the complete story.
...
- confidence: 0.78
- depends-on:
  - clm-vca7r1 (the node-up V/I-keyed varactor asymmetry leaf; R2 static-E operating point this closure confirms)
  - clm-sve3xc (the #539 [C-EXCLUDED] static-E exclusion this closure confirms as complete at atomic scales)
  - clm-mfb2ax (the srs-z3 saturated Born–Huang tensor whose $C_{44}$ the lattice-rigidity quantification borrows across-lattice; register home of the FLAGGED cross-lattice borrow, "use as input only, don't build deeper")
  - Axiom 4 (the saturation kernel $S=\sqrt{1-(A/A_{yield})^2}$ whose ε-specialization $\varepsilon_{eff}=\varepsilon_0 S$ is the T2 kernel keyed on the static-capable $A_V=V/V_{yield}$)
- solidity: 0.55 (use as input only, don't build deeper) [= min(0.78, 0.55)]
- rationale: A FORM-level / CONSISTENCY-class closure of a keying fork, blind-derived against a FROZEN prereg (freeze commit `942c950b`, git ordering = freeze proof) with no post-hoc criterion drops (Rule 11 honest closure) and thrice fixed under adversarial review (the #542/#546/#547 arc). The routing is forced by the network — four independent mechanisms (M0 axiom-argument, M1 two-topology time-domain DC-response, M2 two-route energy ledger, M3 convention-robust small-signal object) each fail to deliver a lossless ε-side DC-block, and one mechanism (the ε element is a shunt keyed on the static-capable node potential) explains all four; the lattice zero-mode is settled CLOSED (rigid) across the whole counted band. Two independent code paths (sympy analytic + numpy time-domain) + four ReconcileGates (all can-fire proven) + a live counterfactual; 12 tests. **Caps (why own-confidence is 0.78, not higher):** (i) the exact $\tfrac12\langle A_V^2\rangle$ identity is LEADING-ORDER only — a Jensen gap opens at $O(A^4)$ (the charge-vs-excursion routing is unchanged by it, but the exact equality is not claimed beyond 2nd order); (ii) lattice-rigidity is quantified via a cross-lattice $C_{44}$ borrow from the srs-z3 saturated Born–Huang tensor (clm-mfb2ax), NOT a K4-native derivation — this dependency edge is now **DECLARED** in depends-on and is **binding**: the 0.55 gate is the srs-borrow priced mechanically (the weakest dependency's final solidity flows through `min`), not a prose cap, so this leaf inherits clm-mfb2ax's "use as input only, don't build deeper" band. Upgrade path: a K4-native saturated elastic-tensor derivation (the strengthen-by) would drop this edge and re-price the leaf's solidity upward automatically; (iii) the three-way varactor-convention tangle (node-up:105/:370 vs `CLAUDE.md`:73/:75 vs `device-circuit-models.md`:60) is **RESOLVED (Grant-ratified 2026-07-06, payload as enumerated in PR #558)** — was OPEN with the M3 chord-vs-tangent coefficient carried KEEP-BOTH, neither crowned; now closed by payload (ii) (the tangent $dQ/dV$ is crowned "*the* small-signal", both sectors) + payload (iii) (chord/tangent = the ⊥/∥ polarization eigenmodes of one saturable-dielectric tensor, so the question dissolves at the tensor level; the observable is their split, sympy-proven `semiconductor_cv_dip.py::eigenmode_check`), landed at node-up-small-large-signal.md §1 :105 + §4 :229; (iv) single-cell + lattice-rigid scope. **Solidity and own-confidence are UNCHANGED by this ratification (conservative):** the cap-(iii) resolution does NOT touch the binding gate. Own-confidence 0.78 reflects the four disclosed caps; the FINAL solidity is 0.55 [= min(own 0.78, clm-vca7r1 0.85, clm-sve3xc 0.80, clm-mfb2ax 0.55)] — the binding gate at 0.55 is the clm-mfb2ax srs-C₄₄ borrow (cap ii), which is untouched by the varactor-convention ratification, so the leaf stays in the "use as input only, don't build deeper" band and the K4-native re-derivation (strengthen-by item 2) remains what lifts it. CONSISTENCY-class, not emergence: no dimensionful number minted; the constants are imported and appear only in dimensionless bands.
- strengthen-by:
...
- **`:161`** — stamped at `:161`. *(family: parent-condition acoustic boundary; banked `uncertain`)*  ⚑ **BIAS-DEBT**
  ...
  ```text
  Register home of the parent-condition claim (echoed at :655/:663 anchor, vol4 ch1 index:49, vol9 ch1:21); uncertain whether Γ_internal's incident object is a propagating axial branch — mechanism check owed, static k-ratio survives.
  ```
...
- **`:369`** — stamped at `:369`. *(family: driven compression mechanism; banked `uncertain`)*  ⚑ **BIAS-DEBT** · ⚑ **R49 4π-CONVENTION**
  ...
  Audited rationale, verbatim from the banked worklist:
  ```text
  A standing compression wave presupposes counter-propagating bulk waves; the surviving content is a source-driven bound bias-strain pattern (pumped elliptic response) — the mechanism sentence must re-derive on that footing or the pathway dies. Uncertain because the site is a proposal, not a banked mechanism.
  ```

  **Resolution.** The demoted carrier is the propagating A1 / bulk branch; under Axiom 5 clause G that slot is the **bound response**, so the re-derivation must be re-posed on the bound-sector constitutive law (bias $\varepsilon_{11}$, bound response $\mathbf{u}_0$, mechanism gloss back-reaction) rather than on a compression wave. **⚑ BIAS-DEBT:** this row's re-derivation turns on finite-speed bias dynamics, so the resolution is the ratified axiom **with THE BIAS PROPAGATION THEOREM standing** (clause (c1)) — the replacement is *owed, not held*. **⚑ R49:** where this row's re-derivation runs through the elliptic bias law, that law carries the **declared 4π source convention** (canonical declaration in `gordon-optical-metric.md`, claim `clm-rd9cjm`); a correction of that class is an R31-style dated correction, not a re-ratification.
