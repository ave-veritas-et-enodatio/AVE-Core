...
- _Specific Non-Claims and Caveats_
  ...
  - **Confirms clm-sve3xc as COMPLETE at atomic scales (does not overturn it).** The muonic Coulomb field is static-in-time but spatially non-uniform, so it LOADS under charge-keying — reproducing #539 [C-EXCLUDED] (the interior-excluded bracket integral overshoots the 2.3 µeV CREMA window by $\sim6.6\times10^5\times$). The uniform-bias gauge rider does NOT rescue the muon ($\nabla A\ne0$). So the round-2 conditional-PASS does not become derived; the Letter's protective hard-cutoff reading and clm-sve3xc [C-EXCLUDED] are the complete story.
...
- rationale: A FORM-level / CONSISTENCY-class closure of a keying fork, blind-derived against a FROZEN prereg (freeze commit `942c950b`, git ordering = freeze proof) with no post-hoc criterion drops (Rule 11 honest closure) and thrice fixed under adversarial review (the #542/#546/#547 arc). The routing is forced by the network — four independent mechanisms (M0 axiom-argument, M1 two-topology time-domain DC-response, M2 two-route energy ledger, M3 convention-robust small-signal tangent) each fail to deliver a lossless ε-side DC-block, and one mechanism (the ε element is a shunt keyed on the static-capable node potential) explains all four; the lattice zero-mode is settled CLOSED (rigid) across the whole counted band. Two independent code paths (sympy analytic + numpy time-domain) + four ReconcileGates (all can-fire proven) + a live counterfactual; 12 tests. **Caps (why 0.78, not higher):** (i) the exact $\tfrac12\langle A_V^2\rangle$ identity is LEADING-ORDER only — a Jensen gap opens at $O(A^4)$ (the charge-vs-excursion routing is unchanged by it, but the exact equality is not claimed beyond 2nd order); (ii) lattice-rigidity is quantified via a FLAGGED cross-lattice $C_{44}$ borrow from the srs-z3 saturated Born–Huang tensor (clm-mfb2ax), NOT a K4-native derivation; (iii) the three-way varactor-convention tangle (node-up:104/:360 vs `CLAUDE.md`:73/:75 vs `device-circuit-models.md`:60) is OPEN — the M3 chord-vs-tangent coefficient is carried KEEP-BOTH, neither crowned, surfaced for Grant; (iv) single-cell + lattice-rigid scope. Solidity 0.78 [= min(own 0.78, clm-sve3xc 0.80, clm-vca7r1 0.85)] — the own-confidence 0.78 (below the parent floor) is the binding gate, reflecting the four disclosed caps. CONSISTENCY-class, not emergence: no dimensionful number minted; the constants are imported and appear only in dimensionless bands.
- strengthen-by:
  - Resolve the three-way varactor-convention tangle (crown one object as "the small-signal C" across node-up:104/:360, `CLAUDE.md`:73/:75, `device-circuit-models.md`:60) — a Grant corpus-consistency adjudication, after which the M3 coefficient stops being KEEP-BOTH.
