...
- _Specific Claims_
  ...
  - **LIGO GW150914 black-hole echoes** ($\sim 0.29$ s spacing): at the **shear/bulk** rupture boundary $r_{sat}=7GM/c^2$ the reflection coefficient hits $\Gamma_{shear}=\Gamma_{bulk}=-1$ ($G_{shear}\to0\Rightarrow Z_{shear}=\rho c_{shear}\to0$, an Op3 short — **shear-mode**, *not* an $\varepsilon$-sector "dielectric rupture") → the horizon is a hard reflective boundary for shear/GW modes, not a one-way membrane → echoes are *predicted* by AVE (the EM channel stays matched, $\Gamma_{EM}=0$). *(2026-06-17 walk-back: channel-labelled; mechanism is shear, see [`lattice-extreme-bh-rationality.md`](../vol3/cosmology/ch15-black-hole-orbitals/lattice-extreme-bh-rationality.md):75.)*
