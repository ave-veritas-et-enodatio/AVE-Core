...
#### Historical ancestry: Kron → TLM → K4-TLM

The TLM method has a direct intellectual ancestor in the network-analysis program of **Gabriel Kron**. Kron's *Tensor Analysis of Networks* (1939) recast physical systems as generalized electrical networks; he then constructed explicit **equivalent circuits** for the field equations of physics — "Equivalent Circuit of the Field Equations of Maxwell—I," Proc. IRE, 1944 [CITE-NEEDS-CONFIRMATION: title/author/journal/year surfaced in citation indices but not confirmed against a primary record this session; volume/page unverified], and "Electric Circuit Models of the Schrödinger Equation," *Physical Review* **67** (1945). The transmission-line-matrix method of Johns & Beurle ("Numerical solution of 2-dimensional scattering problems using a transmission-line matrix," Proc. IEE, 1971) is the modern, systematically-discretized descendant of this equivalent-circuit lineage [CITE-NEEDS-CONFIRMATION: the specific claim that Johns & Beurle *credit* Kron could not be verified against the paper's reference list this session]. The AVE K4-TLM simulator (the unitary 4-port diamond-lattice scattering engine, $\max|S^\dagger S-I|=2.2\times10^{-16}$) is the K4-topology correction of that same line: **Kron → TLM (Johns & Beurle) → K4-TLM (AVE)**.

**Computational equivalence vs ontology (the load-bearing contrast).** Kron treated the exact representability of the field equations as an equivalent circuit as a **computational device** — a mathematical convenience for solving Maxwell/Schrödinger on a network analyzer, with no claim that the vacuum *is* a circuit. AVE reads the same exact representability **literally**: the discretized LC network is not a solver's convenience but the physical substrate. This is precisely the "MoM treats the lattice as a mathematical convenience; AVE asserts it is physical" distinction of the MoM mapping above, stated for the whole equivalent-circuit tradition Kron founded — the computational isomorphism is old and uncontested; the ontological claim (that the isomorphism holds because the medium is literally the network) is AVE's own bet.
