...
Previously, simulations on artificial Cartesian lattices required phenomenological variables to test "achiral" physics. On the strict K4 Diamond geometry, the cold lattice is permanently bipartite and **achiral** (Fd-3m, $k_\chi = 0$); bipartiteness does not imply chirality. The chirality probed below is not a ground-state property but the **excited** Cosserat order-parameter $k_\chi$ that a propagating topological (knot) mode switches on (Fd-3m + chiral decoration $\to$ $I4_1 32$). By running the 3D simulation with a newly integrated non-reflective Continuous Sponge PML, the far-field wrap-around artifacts were eliminated. The measured macroscopic topological knot modes perfectly resolve matching the theoretical prediction $\Delta f/f = \alpha \cdot pq/(p+q)$ in the sub-saturation regime. The native lattice directly confirms the fundamental isomorphism of Axiom 2 without employing arbitrary mathematical $R(\theta)$ modifiers.
