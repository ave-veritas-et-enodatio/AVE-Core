...
- **Single-channel wavelength-counting** ($\ell$ wavelengths around one fixed boundary; one Nyquist-cell category): one resonance, linewidth $\propto 1/\ell$. BH ringdown, QNM, knot-mode isomorphism, the universal substrate-Q derivation procedure (`regime-eigenvalue-method.md`) all live here. $Q = \ell$ is the linewidth of ONE confined mode. **[Spin-scope on the BH-ringdown instance only, Grant Ruling B1 2026-07-21 (propagated 2026-07-31): at the BH-ringdown fire, flat $Q = \ell$ is the **cold $a_* = 0$ anchor** — $\ell$ is an integer **mode count** — cavity-deformable (`common/vocabulary-register.md:261` (def-quant3), sense (1) at `:271`), **not** a deformation-invariant winding. **The discriminator is its INTEGRALITY, not its sense class:** an integer can only jump discretely, while the physical $Q = \omega_R/2\omega_I$ is an impedance ratio that moves continuously with the strain profile; an integer that can only jump cannot track a ratio that moves smoothly, so at catalog spins the flat reading fails at $\bar D_Q = -38\%$ and the m$\Omega$ law $\omega_I = (\omega_R - m\Omega)/(2\ell)$ carries the spin dependence. **B1 did NOT re-adjudicate the Op21 mode-counting identity itself** — only its BH spin scope. Scope banner: [`qnm-quality-factor.md`](../../../vol3/cosmology/ch15-black-hole-orbitals/qnm-quality-factor.md); model banner [`vol3/claim-quality.md`](../../../vol3/claim-quality.md) `:204`.]** **[#808 scoping F7/F8 — OPEN FLAG, not a ruling; B1 did not adjudicate this, and it is separate from the spin scope above:** the $1/\ell$ leak fraction derived in §2 below is a *scaling assertion* with its constant set to 1 by convention, never a computed radiated power. Source `research/2026-07-30_qlaw-derivation_scoping.md` §1.5 — surfaced, unadjudicated.**]**
...
|---|---|---|---|---|
| BH ringdown | Saturation at $r_{sat}$ Axiom-4 phase transition | **Single channel** | Tangential $\ell$-wavelength shear mode (one resonance) | $Q = \ell$ per ringdown mode (cold $a_*=0$ anchor — B1) |
...
2. **BH ringdown** at $r_{sat}$ Axiom-4 phase transition boundary — single-channel $Q = \ell$ per ringdown mode, **spin-scoped to the cold $a_* = 0$ anchor** (Ruling B1, 2026-07-21; the m$\Omega$ law carries the spin dependence — this scopes the BH *instance*, not the Op21 identity) (canonical at [`qnm-quality-factor.md`](../../../vol3/cosmology/ch15-black-hole-orbitals/qnm-quality-factor.md) + [`axiom-coverage-audit.md`](../../../vol3/cosmology/ch15-black-hole-orbitals/axiom-coverage-audit.md))
...
- [**QNM Quality Factor from Lattice Phase Transition**](../../../vol3/cosmology/ch15-black-hole-orbitals/qnm-quality-factor.md) (clm-395gps) — BH-ringdown-scale Op21: $Q = \ell, \omega_I = \omega_R/(2\ell)$ (cold $a_*=0$ anchor — B1, 2026-07-21)
...
**Numerical-value axis: Class 4 observable consistency** (unchanged from prior Phase 3-A2 closure). The numerical match $\alpha^{-1}_{\text{ideal}} = 4\pi^3 + \pi^2 + \pi \approx 137.0363$ to CODATA $\alpha^{-1} \approx 137.036$ within $\delta_{strain} \approx 2.22 \times 10^{-6}$ is a substrate-prediction-vs-measurement consistency. The substrate-prediction values $(R, r, d) = (\varphi/2, (\varphi-1)/2, 1)$ are forced by the three-substrate-regime derivation at `ch8-alpha-golden-torus.md:31-93` (Phase 1-2 closures), NOT by Phase 3-A4 work.

> **🔴 VALUE-SCOPED CAVEAT (2026-06-15, keystone $\alpha$-verdict; Rule 12 — the paragraph above is preserved unedited).** "Forced by the three-substrate-regime derivation" names the **Class-B substrate-mechanism provenance** of the geometry $(R,r,d)$ — which remains canonical — but per the keystone $\alpha$-verdict (auditor-gated 2026-06-14) it does **not** make the *value* substrate-selected. The *scale* ($\sim$1/137) is forced by the Compton-resonance trapping condition; the *exact value* $4\pi^3+\pi^2+\pi$ rests on the single identification $R\cdot r = 1/4$ — shared across the $(2,q)$ bound-resonator ladder, not electron-specific — which **the substrate does not independently select** (both named lift-routes closed; the honest kinematic route absorbs $\alpha$, forcing $R \cdot r \to 4\pi^2\alpha \neq 1/4$). $\alpha$ is one of the 3 *retained inputs*; the value closes as **"echo at the value level," recorded beneath the canonical Class-B label** and never as a bare standalone "echo." Canonical scope: `../../../vol1/ch8-alpha-golden-torus.md:11`; in-volume anchor: `theorem-3-1-q-factor.md:19`.  <!-- rule12-freeze: base=417f1330187c0a6ee2105ccde26d410b80b45a47 region=above offset=67 lines=173 bytes=19553 sha256=fda39decc7823dafbd6e7770d8c2a2bf1576994a399841bbdb33a5c9c4f92a4a -->
