...
>
> **Sectoral threshold note (Grant-gated — see the BLOCKING fork at def-vyvsn1).** Per #416 the single-electron confining $\Gamma=-1$ wall and the charge/transverse-$T_2$ self-trap sit at $V_{\text{yield}}$, while $V_{\text{snap}}$ is the longitudinal-A1 saturation + Schwinger/pair energy. ⚠ The grade-attribution of $V_{\text{yield}}$ is an UNADJUDICATED fork (KB-A1 [`nonlinear-vacuum-capacitance.md`](nonlinear-vacuum-capacitance.md):16 vs engine-$\varepsilon_{\text{eff}}$ + pair-production:100); and $V_{\text{yield}}=\sqrt{\alpha}\cdot V_{\text{snap}}$ EXACTLY, so the two are NOT independent per-sector thresholds (the $\sqrt{\alpha}$ is an $\alpha$-echo). Flag-don't-fix — Grant physics call. CONSISTENCY-class throughout; charge=Link STANDS; the channel-impedance-mismatch $Q$ target stays EMPTY (see :131). See `research/2026-06-24_engine-reroute-epic-summary.md`.
