...
>
> - **$\Gamma_{spinor} = -1$** — the topological $2\pi\to4\pi$ spinor-sign / perfect-short **stability** wall. **Class-invariant: ALL fermions have it** (electron AND proton). This is the "$\Gamma=-1$ wall" of the Pauli/TIR derivation above. It is NOT the EM leak.
...
>
> $\alpha$ here is the **UNIVERSAL** EM coupling — it is NOT renamed $1/Q$ and NOT factored out as a per-instance variable. Only the electron's $Q$ VALUE is the instance field. **Open homonym (for adjudication, NOT resolved here):** whether the impedance-short $\Gamma=-1$ derived above (from $Z_{core}\to0$) is *identically* the topological $2\pi\to4\pi$ spinor sign, or a numerically-coincident distinct $-1$, is a pending ruling.
