...
<!-- kb-frontmatter
kind: leaf
claims: [clm-vca7r1, clm-chgky3]
-->

> 🔴 **QED-NORMALIZATION CORRECTION (2026-07-03; Rule-12 — body preserved below, git is the trail).**  <!-- rule12-freeze: base=46162d6589f063f1cbc90712e1a7d24d613ca098 region=below offset=13 lines=37 bytes=3800 sha256=b10be7a490057e7ad837211a959093e9bd743d7867e60b0cd0283a2c73665e26 -->
> The OQ-1 differential coefficient "$7.5/\alpha^3\approx1.93\times10^7$" below carries an understated QED
> denominator ($(3/45)\alpha^2$ too small by $1/(2\pi\alpha)\approx21.8$ vs the PVLAS-anchored magnetic leg).
...
> $7.5\pi/\alpha^2\approx4.42\times10^5$, exactly double via the $\langle\cos^2\rangle=\tfrac12$ carrier average;
> $15\pi/\alpha^2\approx8.85\times10^5$ static-E; no order of magnitude or falsifier verdict changes).
> The R2/varactor node-up derivation and the $\alpha$-echo grade are UNAFFECTED (only the value moves).
> Canonical:
...
> 🔴 **SECTOR-KEYING SUPERSESSION (2026-07-07, `analysis/land-varactor-supersession`; Grant-ratified 2026-07-06, payload as enumerated in PR #558; Rule-12 — body below PRESERVED, git is the trail).** The `[Resultbox]` keyed-argument duality below (this :105 resultbox) and the §5 ledger row (:370 — same treatment, see the pointer there) label $C_{eff}(V)=C_0/S(A_V)$, $A_V=V/V_{yield}$ the "$\varepsilon$-grade varactor." This **cross-wires A1's form onto T2's key**: the diverging $C_0/S$ (↑) is the **longitudinal-A1 bond compliance** and is keyed on **$V_{snap}$** ($m_ec^2/e\approx511$ kV, where $C_{eff}\to\infty$; [`nonlinear-vacuum-capacitance.md`](nonlinear-vacuum-capacitance.md):16, [`CLAUDE.md`](../../../CLAUDE.md):73), NOT the "$\varepsilon$-grade." The **transverse-T2 permittivity** is $\varepsilon_{eff}=\varepsilon_0 S$ (↓, ROLLS OFF), keyed on **$V_{yield}$** ($\sqrt\alpha\,V_{snap}\approx43.65$ kV, the Cosserat self-trap wall; `def-vyvsn1`, [`nonlinear-vacuum-capacitance.md`](nonlinear-vacuum-capacitance.md):18). **The corrected split (Grant-ratified sector split, [`CLAUDE.md`](../../../CLAUDE.md):73):**  <!-- rule12-freeze: base=fc097d8f02a647baefd55d680800fa4cade83769 region=below offset=0 lines=193 bytes=27533 sha256=f11aca2568ed40d2084eb4f90e5e4a6e1edb1cee3b933dfbc9f2c81e15dd5b7e -->
> - **A1 longitudinal bond compliance:** $C_{eff}=C_0/S(V/V_{snap})$, DIVERGES at $V_{snap}$; small-signal tangent $C_{ss}=dQ/dV=C_0/S^3$ ([`device-circuit-models.md`](../../../vol9/ch3-pin-port-configuration/device-circuit-models.md):60). *Operational:* the small-signal compliance is the **tangent** $C_0/S^3$ (crowned); the **chord** $C_0/S$ is the large-signal secant. Device reading: turn-on / channel-inversion capacitance.
> - **T2 transverse permittivity:** $\varepsilon_{eff}=\varepsilon_0 S(V/V_{yield})$, ROLLS OFF at $V_{yield}$; small-signal tangent $\varepsilon_{ss}=dD/dV=\varepsilon_0(S-A_V^2/S)$. *Operational:* chord $\varepsilon_0 S$ (leading $1-\tfrac12 A_V^2$) = the perpendicular probe eigenmode $n_\perp=\sqrt S$; tangent $\varepsilon_0(S-A_V^2/S)$ (leading $1-\tfrac32 A_V^2$) = the parallel eigenmode $n_\parallel$ — the **chord/tangent split IS the birefringence** $\delta n_{bir}=-\tfrac12 A_V^2$ (corpus-resolved; Letter Appendix A).
>
> **Scope of what ratifying this note settles (three-part payload — enumerated so the self-description matches the body above).** Grant is adjudicating all three of:
> - **(i) sector-keying correction** — the A1 divergent *form* $C_0/S$ (keyed $V_{snap}$) vs the T2 rolloff *form* $\varepsilon_0 S$ (keyed $V_{yield}$); A1's form was cross-wired onto T2's key at this :105 resultbox and the :370 ledger row.
> - **(ii) operational crowning** — per the C-V measurement discipline, the **tangent** $dQ/dV$ is "*the* small-signal" capacitance/permittivity and the **chord** $Q/V$ is the large-signal secant, for BOTH sectors (A1 $C_0/S^3$ crowned; T2 $\varepsilon_0(S-A_V^2/S)$ crowned).
> - **(iii) eigenmode resolution of the chord/tangent fork** — the T2 chord $\sqrt S$ IS the perpendicular index $n_\perp$ and the T2 tangent $\sqrt{S-A_V^2/S}$ IS the parallel index $n_\parallel$; BOTH are real, per-polarization, and their split IS the birefringence $\delta n_{bir}=-\tfrac12 A_V^2$ (sympy-proven, [`semiconductor_cv_dip.py`](../../../../../src/scripts/verify/semiconductor_cv_dip.py)`::eigenmode_check`; vs Letter Appendix A, Eq. A5-A8).
>
> **Consequence:** ratifying this note **closes** the §4 :229 three-way "which is *the* small-signal C" convention question — via (ii) (the tangent is crowned "the small-signal" by the measurement definition) **and** (iii) (the chord/tangent fork was never a contradiction; the two objects are the ⊥/∥ polarization eigenmodes of one saturable-dielectric tensor, so "which is *the* C" dissolves at the tensor level — both are real, and the observable is their difference). The **sector-keying** cross-wire (i) is the narrow KB-edit target at this :105 resultbox and the :370 ledger row. Provenance: [`research/2026-07-07_semiconductor-cv-dip_RESULT.md`](../../../../../research/2026-07-07_semiconductor-cv-dip_RESULT.md) (a)/(c)/(h); Grant-ratified 2026-07-06 (payload as enumerated in PR #558).
...
> **↗ STRENGTHEN — the R1 symmetric-internal null has a TRANSLATIONAL-ELASTIC sibling (2026-07-04, PR #518 MERGED — the matter-stiffening derivation).** The R1 result "$S_\varepsilon=S_\mu=S\Rightarrow Z=Z_0$ (reflectionless)" is the **EM** ($\varepsilon$/$\mu$) face of a symmetric-loading transparency. The SAME symmetric-loading logic on the **translational-elastic** bond pair (axial $k_a$ vs shear $k_s$ — BOTH capacitive springs of the same bond, PR #516) gives the elastic sibling: **$S_{axial}=S_{shear}\Rightarrow\rho_{eff}=\rho_{cold}$** (the bond-stiffness ratio is unchanged), via the canon-forced composition $\rho_{eff}=\rho_{cold}\cdot(S_{axial}/S_{shear})$. So a **symmetrically-loaded** (both-channel-equal) drive — the radiation / pure-AC $\langle A\rangle=0$ case — is **transparent to the bond-stiffness ratio** exactly as R1 is transparent to $Z$: $\rho_{eff}=\rho_{cold}$ for every amplitude (confirmed two independent ways — symmetric-internal $S_{axial}=S_{shear}$, and the displacement-pump null `clm-clvchn`). Only **asymmetric** loading ($S_{axial}\ne S_{shear}$, matter) moves $\rho_{eff}$ off the cold point. **Scope (honest, mandatory):** this is the FORM-level radiation-vs-matter split (radiation leaves $\rho$ invariant, matter stiffens it); it does **NOT** derive any matter VALUE — the specific matter $\rho^\ast\approx9.77$ / the /7 value stays GR-imported (PR #506), and — the load-bearing regime caveat carried verbatim (flag-don't-fix) — **driving the saturated $\rho_{eff}$ to 9.77 is NOT proven to land the same $\nu=2/7$ / $K=2G$ elastic tensor** as setting the cold $\rho=9.77$ (a running arc tests exactly this cold-vs-saturated gap; this does NOT pre-judge it). Provenance: [`research/2026-07-04_matter-stiffening-rho_result.md`](../../../../../research/2026-07-04_matter-stiffening-rho_result.md) (VERDICT BOX radiation control + §6 + §7).

> **↗ STRENGTHEN + RESOLUTION (2026-07-04, PR #521 MERGED — the saturated-elastic-tensor arc; the STRENGTHEN banner above is PRESERVED — KEEP-BOTH).** Two upgrades from the saturated arc, both landing on this R1-sibling: **(1) the sibling now has its own TENSOR form** — the "$S_{axial}=S_{shear}\Rightarrow\rho_{eff}=\rho_{cold}$" transparency above is not merely a $\rho$-level (ratio) statement: the saturated Born-Huang tensor computed on $\Phi_b(A)$ is the cold tensor with $\rho\to\rho_{eff}$, so at $S_{axial}=S_{shear}$ (radiation / $\langle A\rangle=0$) the **FULL saturated small-signal Cauchy tensor equals the cold tensor**, not just the ratio — symmetric loading is elastically transparent **at the tensor level**, the exact elastic analog of R1's $Z=Z_0$. **(2) the "running arc" the caveat above deferred to has landed** — verdict **[SAME-TENSOR-POINT]**: driving the saturated $\rho_{eff}$ to $9.7734$ DOES land the same cold $\nu=2/7$ / $K=2G$ tensor (homogeneous degree-1 map $\Rightarrow$ dimensionless ratios degree-0; VS2/VS3 $\le4\times10^{-8}$). **So the cold-vs-saturated gap CLOSES — MODEL-SCOPED** to the small-signal swapped-springs model (fixed geometry); the **RESIDUAL STAYS OPEN** (initial/residual pre-stress + bias-induced geometry change OMITTED, § MODEL SCOPE). **VALUE GRADE UNCHANGED:** CONSISTENCY finding, NOT a value derivation — $\rho^\ast\approx9.77$ / $K=2G$ stays GR-imported (PR #506/PR#261), crossing amplitude $A_{wall}=0.99479$ canon-undistinguished. Provenance: [`research/2026-07-04_saturated-elastic-tensor_result.md`](../../../../../research/2026-07-04_saturated-elastic-tensor_result.md) (VERDICT BOX + § MODEL SCOPE + §9 auditor-queue R1-sibling row); driver `src/scripts/vol_1_foundations/saturated_elastic_tensor.py`.
...
<!-- claim-quality: clm-chgky3 -->
> **↗ STRENGTHEN + SCOPE (2026-07-06, PR #542/#546/#547 arc — the round-3 blind ε-side DC-mechanism derivation; ADDITIVE, KEEP-BOTH — the :218 R2 statement above is PRESERVED verbatim, this note only scopes its observability).** A three-round adversarially-reviewed (thrice-fixed) blind derivation **CONFIRMS the :218 R2 statement at the local cell ledger**: a static $\mathbf E$ **is** a real operating-point bias for the $V$-keyed varactor — it loads $\varepsilon$. Round 3 derives *WHY* and *WHAT it keys on*: the ε-grade (transverse-T2 permittivity) nonlinearity keys on the **MEAN-SQUARE** of the instantaneous field amplitude at the cell — **DC-included ([DERIVED: CHARGE-KEYED])**, at leading (2nd) order (the $\langle 1-S_\varepsilon\rangle = \tfrac12\langle A_V^2\rangle$ identity is 2nd-order; a Jensen gap opens at $O(A^4)$, DC baseline $a_0^2$ retained either way). The **excursion/variance** member (a DC-blind "worked-keyed" candidate) is **DERIVED-AGAINST**: all four candidate mechanisms fail to deliver a lossless ε-side DC-block (M1 — no series-C in the shunt-C T2 path; M2 — the two-route energy ledger closes with the held energy parked as charge in the saturating shunt capacitor, zero residual, no linear spectator mode; M3 — the small-signal object shifts DOWN nonzero under held bias in *every* convention, and the only relaxation, the $\tau_{relax}$ hysteresis, dissipates → Ax3-forbidden), and the lattice-level zero-mode is **settled CLOSED (rigid)** across the whole counted band $A \le 1/\sqrt2$ (K4 carries shear stiffness $k_s>0$, `k4-bloch-dispersion-quartic.md`:58 verbatim *"A pure central-force model ($k_s=0$) would carry soft transverse-acoustic branches"* — so the floppy zone is the $k_s=0$ pathology, not the canonical substrate). **This CONFIRMS R2 and DERIVES-AGAINST the round-2 KEEP-BOTH `R2′` (worked-keyed / DC-blind) candidate** — the canonical network does not force it, and a lossless ε-side Lenz-dual DC-block does not exist.
>
> **UNIFORM-vs-NON-UNIFORM observability scoping (the :218 "and shifts $n$" clause, made precise — CONSISTENCY-class re-expression of INVARIANT-S2, NOT a keying change).** The cell IS locally deficient under *any* held bias (charge-keyed). But **readout is differential**: a spatially-**UNIFORM** held bias parks a real local deficit that **self-cancels on readout** — the $A$-state is gauge-relative (`manuscript/ave-kb/CLAUDE.md`:75 verbatim *"only spatial gradients of $A$ across the substrate are physically observable, not absolute per-node values"*; the PHASE-ONLY north-star, `manuscript/ave-kb/common/claim-quality.md`:1318 verbatim *"All measurement is AC — a uniform DC bias is gauge-relative and self-cancels (= relativity, with the constructive-relativity mechanism: wave-made rulers/clocks ride the same offset). Every AVE-distinct observable is an AC reading of a DC gradient or topology — differential BY PRINCIPLE, not by experimental limitation (the PHASE-ONLY north-star, mechanized)."*), so a co-located wave-made ruler rides the same offset and the uniform shift is gauge-unobservable. A **NON-uniform** held field (an atomic Coulomb field, a bench fringe) has a nonzero $\nabla A$ and IS readable — via exactly the R2 **Op14 Meissner-asymmetric impedance mirror** of §2/§4 above: `manuscript/ave-kb/CLAUDE.md`:75 verbatim *"A **static-E-only drive is ASYMMETRIC** … it loads the $\varepsilon$ / capacitive sector only ($S_\varepsilon < 1$, $S_\mu = 1$)"*, giving $Z_{eff}=Z_0\sqrt{S_\mu/S_\varepsilon}$ (`operators.md`:54 canonical asymmetric form) so *"$Z$ **changes**, so the boundary reflects ($\Gamma \neq 0$): this is the vacuum-impedance-mirror bench mechanism (Vol 4 Ch 11)"* (`manuscript/ave-kb/CLAUDE.md`:75). That $\Gamma\ne0$ reflection at an ε-gradient boundary is the readable observable the :218 "shifts $n$" clause names — **differential by principle**. So the :218 "loads ε and shifts $n$" stands: it loads locally for *any* held E; the shift is readable for a *non-uniform* held E and gauge-hidden for a *uniform* one.
>
> **⚑ FLAGGED-OPEN — the three-way varactor-convention tangle (surfaced for Grant, NOT resolved here).** The corpus carries a **three-way** tension around $C_0/S(A_V)$ and what "small-signal C" means, across (a) this leaf's :105/:370 ($C_0/S(A_V)$ = the ε-grade varactor, **DERIVED**), (b) `manuscript/ave-kb/CLAUDE.md`:73/:75 (assigns $C_0/S$ to A1, $\varepsilon_0 S$ to T2, and calls **both** "small-signal" in one sentence), and (c) `device-circuit-models.md`:60 (verbatim *"the large-signal chord/secant varactor $C_{\mathrm{eff}}=C_0/S$ vs the small-signal differential $C_{\mathrm{ss}}=\mathrm{d}Q/\mathrm{d}V=C_0/S^3$"*, A1-scoped). The M3 coefficient consequence (chord $C_0\!\cdot\!S(A_0)$ → leading $1-\tfrac12 A_0^2$ vs $\mathrm{d}Q/\mathrm{d}V$ tangent → leading $1-\tfrac32 A_0^2$) is carried **KEEP-BOTH, neither crowned** — M3 is convention-robust (all shift DOWN nonzero, the kill holds in every convention), but which object is "the small-signal C" is a **corpus-consistency call OPEN for Grant**. See the merged flag block in the RESULT doc, § DISCIPLINE ⚑.
>
> **✅ CLOSED (2026-07-07, Grant-ratified 2026-07-06; payload as enumerated in PR #558; body above PRESERVED per KEEP-BOTH — this is a status note, not a deletion).** The three-way "which is *the* small-signal C" convention question is **RESOLVED** by the varactor sector-keying supersession (§1 :105 block above), via **(ii)** and **(iii)** of its three-part payload: **(ii) operational crowning** — per the C-V measurement discipline the **tangent** $dQ/dV$ is "*the* small-signal" object and the **chord** $Q/V$ is the large-signal secant, in BOTH sectors (A1 tangent $C_0/S^3$ crowned; T2 tangent $\varepsilon_0(S-A_V^2/S)$ crowned); **(iii) eigenmode resolution** — the chord/tangent fork was never a contradiction: the T2 chord $\sqrt S$ IS the perpendicular index $n_\perp$ and the T2 tangent $\sqrt{S-A_V^2/S}$ IS the parallel index $n_\parallel$; **both are real, per-polarization**, so "which is *the* C" **dissolves at the tensor level** — the two objects are the ⊥/∥ polarization eigenmodes of one saturable-dielectric tensor, and **the observable is their split** $\delta n_{bir}=-\tfrac12 A_V^2$ (sympy-proven, [`semiconductor_cv_dip.py`](../../../../../src/scripts/verify/semiconductor_cv_dip.py)`::eigenmode_check`; vs Letter Appendix A). The (a)/(b)/(c) sources tabulated above are reconciled: (a) node-up:105/:370 was the A1-form-on-T2-key cross-wire (fixed by payload (i)); (b) `CLAUDE.md`:73/:75 is correct once "small-signal" is read as the crowned tangent; (c) `device-circuit-models.md`:60's chord-vs-differential is exactly the crowned-tangent/large-signal-chord distinction. Provenance: [`research/2026-07-07_semiconductor-cv-dip_RESULT.md`](../../../../../research/2026-07-07_semiconductor-cv-dip_RESULT.md) (h); PR #558; this landing (`analysis/land-varactor-supersession`). The `clm-chgky3` cap (iii) and strengthen-by item 1 are updated to RESOLVED to match.
>
> **Consistency-vs-emergence: CONSISTENCY-class** — the CHARGE-KEYED verdict is a consistency identity of the network topology + the axiom-level kernel-argument definition (no new dimensionful number; the constants $\omega_C,\ell_{node},Z_0,E_{yield}$ are imported and appear only in dimensionless bands and the slow-ramp timescale). Provenance: [`research/2026-07-06_em-keying-round3-eps-dc-mechanism_RESULT.md`](../../../../../research/2026-07-06_em-keying-round3-eps-dc-mechanism_RESULT.md) (ROUTED BIN + § DISCIPLINE ⚑ merged flag block; drivers `src/scripts/verify/em_keying_round3_mechanism.py` + `..._comparison.py`, 12 tests); round 2 [`research/2026-07-05_em-keying-round2-worked-cell_RESULT.md`](../../../../../research/2026-07-05_em-keying-round2-worked-cell_RESULT.md) (the `[SELECTED-NOT-DERIVED]` this round resolves toward charge); round 1 [`research/2026-07-05_em-saturation-keying-functional_RESULT.md`](../../../../../research/2026-07-05_em-saturation-keying-functional_RESULT.md). Claim register: clm-chgky3.
...
|---|---|---|
| $C_{eff}=C_0/S(A_V)$, varactor keyed on $V$ | **DERIVED** (🔴 sector-keying superseded — see §1 block) | Axiom 4 dielectric specialization (`manuscript/ave-kb/CLAUDE.md`:73). 🔴 **SECTOR-KEYING SUPERSESSION (2026-07-07; Grant-ratified 2026-07-06, payload per PR #558; Rule-12, row content preserved).** The diverging $C_0/S$ (↑) form is the **A1 longitudinal bond compliance** keyed on **$V_{snap}$** ($C_{eff}\to\infty$; `nonlinear-vacuum-capacitance.md`:16), NOT the $\varepsilon$-grade; the **T2** permittivity is $\varepsilon_0 S$ (↓) keyed on $V_{yield}$ (`def-vyvsn1`, :18). Same cross-wire as the §1 :105 resultbox — full corrected split, the (i)/(ii)/(iii) three-part payload, and the §4 :229 closure are in the §1 supersession block above. |
...
> **⚑ Cross-ref (2026-08-04, additive — nothing above is edited, no number moves).** This leaf is the site of the corpus's **fraction-bar kind-mismatch** in $A_I$: the Resultbox at `:136` fixes the denominator as $I_{max}=\xi_{topo}c$ — a **convection-amplitude** ceiling — while `:146`–`:150` fixes the numerator's kind as the **circulating current**, *"a rate/flux variable, $I\propto\oint\mathbf H\cdot d\boldsymbol\ell$"*. Each half is correct about its own half; what no leaf states is the conversion that makes them commensurate. **Ruled 2026-08-04 (R3):** $A_I$'s two sides must be of the **same kind**, and any leaf assembling $A_I$ must declare the kind on both sides. **Which sense of $I_{max}$ the $\mu$-grade denominator carries is A4 and remains OPEN** — the ruling lands the requirement, not the choice. Full record, with both $I_{max}$ senses, their exact $4\pi\sqrt\alpha$ ratio and the tier assignments: [`common/theorem-thesaurus.md`](../../../common/theorem-thesaurus.md) §6, the $I_{max}$ row.
...
- **`:59`** — stamped at `:59`. *(family: Z_bulk=ρc_bulk formula)*  ⚑ **BIAS-DEBT**
  ...
  ```text
  Characterizes Z_bulk as ρ×speed — the phantom formula; the mechanical-domain tag survives, the speed factor is owed a re-derivation (also touches :370 trio).
  ```
