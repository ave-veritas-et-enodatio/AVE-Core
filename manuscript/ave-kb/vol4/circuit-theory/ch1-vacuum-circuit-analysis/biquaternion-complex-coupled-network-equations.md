...
> 🚩 **FLAG — DO NOT FIX (2026-08-03). Same-form keying question, routed to Grant; nothing above is edited.** The $\varepsilon$-leg of the resultbox at `:99` writes the **divergent** $C_{eff} = C_0/S(A_V)$ with $A_V = V/V_{yield}$. Per the ratified `def-vyvsn1` grade-fork (Grant 2026-06-30, [`nonlinear-vacuum-capacitance.md`](nonlinear-vacuum-capacitance.md):18), the *divergent* $C_0/S$ form is the **longitudinal-A1 bond compliance**, bounded at $V_{snap} = 510{,}998.95$ V — while $V_{yield} = \sqrt{\alpha}V_{snap} = 43{,}651.85$ V is the **transverse-T2 self-trap wall**, whose own constitutive law is the *rolling-off* $C_{diel} \propto S$, not $C_0/S$. That upstream leaf's resultbox already prints $V_{snap}$; this one still prints $V_{yield}$. **Sharpened by this leaf's own label:** the resultbox is titled *"Keyed saturation (Grant-ratified sector split)"*, so a Grant-ratification label sits on a keying that a later Grant ruling re-keyed. **This banner takes no side.** The keying is genuinely forked and the fork is a physics ruling, not a notation propagation — see the full statement of the fork, the recomputed $1/\alpha$ consequences, and the routed question at [`parametric-coupling-kernel.md`](parametric-coupling-kernel.md) (head banner, same commit). **Routed to Grant (verbatim):** *"is the α-slew pump squeezing the BOND (A1 compliance — re-key, and sub-yield stops meaning what the leaf says) or twisting the TRANSVERSE sector (T2 — V_yield stands and only the divergent-form notation is wrong)?"* The $L_{eff}$ / $A_I$ leg of the same resultbox is **not** in question — the current-keyed inductor grade is untouched by this fork.
...
$Z_{\mathrm{bulk}}=\rho_{\mathrm{bulk}}c_{\mathrm{bulk}}=\sqrt2\,\rho_{\mathrm{bulk}}c_0$ at $K=2G$ are 🔴 **[DEMOTED 2026-08-11 — R40-B2a: NEEDS RE-DERIVATION, not dead; dated note at the end of this file]**
...
> **[Resultbox]** *Conserved chiral bulk↔shear coupling (α-free)* 🔴 **[DEMOTED 2026-08-11 — R40-B2a: NEEDS RE-DERIVATION, not dead; dated note at the end of this file]**
>
> $$
...
- **`:147`** — stamped at `:147`. *(family: Z_bulk=ρc_bulk formula)*  ⚑ **BIAS-DEBT**
  ...
  ```text
  Prereg-named formula class: the channel admittance Y_bulk survives as a reactance of the bound sector, but Z_bulk=ρ·c_bulk consumes a bulk propagation speed.
  ```
...
- **`:166`** — anchor `:166` is display math — **stamped at `:163`** (the enclosing block's title line; the math line is byte-untouched). *(family: bulk↔shear coupling term; banked `uncertain`)*
  ...
  Audited rationale, verbatim from the banked worklist:
  ```text
  Treats V_A1 as an independent dynamical field with its own energy in a two-field coupling Hamiltonian; under the carve V_A1 is bound/slaved, so the coupling must re-derive as parametric-on-a-response, not field-field.
  ```
