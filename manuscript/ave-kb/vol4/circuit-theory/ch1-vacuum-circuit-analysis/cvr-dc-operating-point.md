...
> **★ Keying correction (KEEP-BOTH; roll-off ruling ratified 2026-07-06/07, PR #562/#558; propagated 2026-07-13).** Per the ruling, the **divergent** $C_0/S$ form this datasheet plots (the "vacuum varactor," $C_{eff}\to\infty$) IS the **longitudinal-$A_1$ bond compliance** — its saturation-kernel argument is $V/V_{snap}$, **keyed on $V_{snap}\approx511$ kV**, NOT $V/V_{yield}$. The $V_{yield}$-keyed object is the *orthogonal* **transverse-$T_2$ dielectric** $C_{diel}=C_0 S$ (roll-off, $\times S$) — a DISTINCT reactance an across-gap LCR meter reads. The head wording above — "the operating-point bias $A_0 = |V|/V_{yield}$" together with "The constitutive law is the **vacuum varactor** $C_{eff}(A_0)=C_0/S(A_0)$" — keys the divergent ($\div S$) form on $V_{yield}$ and is preserved KEEP-BOTH as the **superseded mixed form** (divergent $\div S$ shape carrying the transverse $V_{yield}$ key). This leaf's own §6 already carries the corrected sector split. Canonical: [`dielectric-plateau-prediction.md`](../../falsification/ch12-falsifiable-predictions/dielectric-plateau-prediction.md):25-38; pattern exemplar `vol4/claim-quality.md` clm-vjv4zf.
...
> **★ Keying disambiguation (KEEP-BOTH; ruling 2026-07-06/07, PR #562/#558; propagated 2026-07-13; see §1).** This paragraph correctly sets the cage DC bias on the snap scale ("$m_e c^2 = e\,V_{snap}$, $V_{snap}\approx511$ kV") — the longitudinal-$A_1$ frame in which the divergent $C_0/S$ varactor lives. But it also names "the saturation/yield onset is $V_{yield}=\sqrt{\alpha}\,V_{snap}\approx43.65$ kV" and then places the electron so that it "sits at a residual $A_\star$ where $\sqrt{S_\star}\approx\alpha/4$ — i.e. essentially at saturation ($A_\star\to1$)" **on that same $C_0/S$ curve** — so the $A_\star\to1$ saturation of the divergent varactor is the **$V_{snap}$ (longitudinal-$A_1$) wall**, NOT the transverse-$T_2$ "$V_{yield}$ yield onset" the sentence also names ($A_\star=V/V_{snap}$, not $V/V_{yield}$). The two saturation scales are distinct reactances; the conflated wording is preserved KEEP-BOTH. Canonical: [`dielectric-plateau-prediction.md`](../../falsification/ch12-falsifiable-predictions/dielectric-plateau-prediction.md):25-38.
