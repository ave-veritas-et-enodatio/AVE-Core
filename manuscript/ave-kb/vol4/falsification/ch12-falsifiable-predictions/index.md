...
| [Vacuum Photon–Photon (χ³/FWM) Channel](vacuum-photon-photon-channel.md) | The Letter's FOURTH testable consequence; in-band four-wave-mixing cross section from the same saturable-$\varepsilon$ kernel as birefringence; low-frequency EFT below $\omega_0 = m_e c^2/\hbar$; two completed in-band searches leave it unconstrained (clm-gg4wmx) |
| [K4 Bloch Dispersion & $(q\ell_{node})^4$ Anisotropy](k4-bloch-dispersion-quartic.md) | Quartic (not quadratic) cubic-harmonic photon anisotropy $\Xi(\hat q)=\hat q_x^4+\hat q_y^4+\hat q_z^4$, symmetry-protected by diamond-cubic ($Fd\bar3m$); K4-specific FORM, CONSISTENCY-class; the weak-C surviving forward prediction (clm-k4d4ph) |
