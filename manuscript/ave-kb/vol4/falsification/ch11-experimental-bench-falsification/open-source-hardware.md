...
> 🔴 **ROUND-2 SUPERSESSION (Grant-adjudicated 2026-06-04; propagated 2026-08-01. Rule 12 — the round-1 protocol below is preserved verbatim, git is the trail).** The protocol's *"AVE-distinct discriminators are (i) substrate-independence … and (ii) enantiomer parity sign-flip"* is **superseded: BOTH legs are RETIRED to consistency-class (FORM-SHARED-RETIRE).** The correct counterfactual is not a coupled-line model but a classical **reciprocal Pasteur (chiral/bianisotropic) medium** — *"from sugar water to a copper-helix PCB"* — whose rotatory power $\kappa$ is a geometric property of the handed inclusion rather than of the host $\varepsilon$ (⇒ medium-independent shift, leg i) and which sends $\kappa\to-\kappa$ under mirroring (⇒ enantiomer sign-flip, leg ii). Receipts: `AVE-HOPF/research/2026-06-04_hopf-round2-chiral-counterfactual-result.md:189-191` (§5 verdict table) + `:20-23` (mechanism); Grant decision at `_orchestration/experimental/2026-06-04_round2-adjudications.md:40` (*"retire to consistency-class … **ADD** the 2-port S₂₁-vs-S₁₂ reciprocity sweep"*).  <!-- rule12-freeze: base=71a45c526473eb458d99beb4ee8868a528ceb9fb region=below offset=0 lines=25 bytes=5406 sha256=604e2434d290757ccf1c7de0ca911a393c7b73cece925be8e86e7e24b3e12954 -->
>
> **What survives:** the **2-port S₂₁-vs-S₁₂ reciprocity sweep** — a **linear-null in the accessible regime** (the AVE-distinct regime is **above-yield**), per `README.md:295`. It requires a **2-port S₂₁/S₁₂ measurement**, which the 1-port SMA edge-launch board described below does **not** provide. Consequently the closing sentence *"AVE chiral coupling is falsified at this scale if the medium and sign legs are null"* **no longer holds** — a null on those legs is now the *expected* consistency-class outcome, not a falsification.

**The Falsification Protocol (discriminator re-framed 2026-06-03 audit, `AVE-HOPF/docs/analysis/2026-06-03_hopf_antenna_hardened_prereg.md`; ⚠ SUPERSEDED IN PART by the round-2 box above):** Measure all six antennas with a calibrated VNA (SOL calibration) in air, then submerge in mineral oil ($\varepsilon_r \approx 2.1$); enantiomer-paired (L vs R). Plot $\Delta f/f$ vs. $pq/(p+q)$. **The AVE-distinct discriminators are (i) substrate-independence** — identical $\Delta f/f$ in air vs oil vs vacuum (a classical coupled-line coupling scales with surrounding $\varepsilon$) — **and (ii) enantiomer parity sign-flip** — opposite-sign $\Delta f$ for a mirror-image winding (classical mutual inductance is parity-invariant). A linear-through-origin slope $\approx \alpha$ is **consistency-class, NOT confirmation**: the $pq/(p+q)$ shape is generic parallel-$\Gamma$ algebra (form-shared with a classical coupled-line model, non-discriminating), and slope=$\alpha$ is the framework's own CODATA-injected $\alpha$ (`src/ave/core/constants.py` `ALPHA`) read back, NOT an α-measurement. AVE chiral coupling is falsified at this scale **if the medium and sign legs are null** (not merely if the slope is off). Control null-shift tolerance $|\Delta f/f| \le 3\times10^{-3}$.

### Project PONDER-01: The Solid-State Micro-Drive
<!-- claim-quality: clm-7tynm2 -->
