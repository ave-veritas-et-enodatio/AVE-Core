...
**(ii) Open-loop Fizeau-style drift is cubic-symmetry-suppressed by $\sim 10^{-22}$.** Even if a non-closed-loop or path-asymmetric configuration is used to extract a Fizeau-class drift, the substrate lattice's diamond-cubic ($Fd\bar{3}m$) symmetry suppresses observable anisotropy at low momentum $q \ll \pi/\ell_{node}$ to $\delta_{aniso} \sim (q\ell_{node})^4$. At optical wavelengths ($\lambda \sim 1$ μm, $q \sim 10^7$ m$^{-1}$):
...
The rotor-based Sagnac-RLVE protocol ([`sagnac-rlve.md`](sagnac-rlve.md)) is a **rotor-local mutual-inductance test** in *mechanism* (a spinning rotor injects a localized drift $v_{network} = v_{rotor} \cdot (\rho_{rotor}/\rho_{bulk}) = 0.38$ m/s into the surrounding substrate via mass-density-dependent coupling, producing the $\Psi_{W/Al} = 7.15$ material-contrast signal; the uniform 370 km/s bulk-frame motion of Earth integrates to zero around the rotor loop). The mechanism distinction from C17's uniform-bulk preferred-frame drift is correct and unchanged. **However, A2 also retired to corroborative-null status (2026-06-03 audit) — it is NOT a surviving forward kill-switch.** The same $\kappa = \rho_{rotor}/\rho_{bulk}$ applied to **Earth-as-rotor** (also a rigid rotor, curl $= 2\Omega \neq 0$ — not the uniform-bulk case C17's geometry nulls) predicts a $+7\times10^{-4}$ rotation-rate bias ($\kappa_{earth} = 6.97\times10^{-4}$) already excluded by ring-laser-gyro Earth-rotation geodesy (G-ring/ROMY/GINGER, agreeing with VLBI/IERS at ~1 part in $10^8$) by $7\times10^4\times$. A2 and C17 land in the **same lifecycle bin (corroborative null)** with *different* suppression physics (A2: rotor-local vorticity excluded by existing RLG data; C17: bulk preferred-frame cubic-symmetry-suppressed). KB anchor: `AVE-PONDER/research/2026-06-03_sagnac-rlve-fog-question-verdict.md`. Both leaves are consistent with the cohesive narrative in the preferred-frame-and-emergent-lorentz leaf §4 (A2/C17 classification) + §6 (matrix implications).
