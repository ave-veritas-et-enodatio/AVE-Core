...
> **Cross-reference flag (2026-06-03 audit; surfaced for adjudication, NOT unilaterally retired).** This B-field-pickup variant shares the **same** rotor-local entrainment chain ($v_{vac} \approx 0.038$ m/s $= v_{tan} \cdot \kappa$, $\kappa = \rho_{rotor}/\rho_{bulk}$, $\rho_{bulk}$ substrate-derived `RHO_BULK` at `src/ave/core/constants.py:619`) as the **A2-SAGNAC fiber-optic Sagnac-RLVE**, which **retired forward "kill-switch" → corroborative-null** (2026-06-03 audit, `AVE-PONDER/research/2026-06-03_sagnac-rlve-fog-question-verdict.md`): the same $\kappa$ applied to Earth-as-rotor predicts a $+7\times10^{-4}$ rotation-rate bias already excluded by ring-laser-gyro Earth-rotation geodesy by $7\times10^4\times$. The Earth-rotor absolute-magnitude tension therefore **also bears on this B-pickup observable** (it inherits the same $\kappa$); the "amplitude scales linearly with RPM + $180°$ phase flip" structure survives as a **self-consistency** scaling check, not a forward kill-switch. The verdict scoped explicitly to A2-SAGNAC; whether to formally retire this B-pickup variant alongside it is the auditor's adjudication. NOT silently resolved here.
