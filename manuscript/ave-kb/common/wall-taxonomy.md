...
# The Wall Taxonomy: $\Gamma = \pm 1$ level-sets of DC operating-point fields, indexed by phase-space axis
...
- **Two rows carry live open flags and they are carried in the row, not in a footnote** — row 1's coordinate-artifact-vs-physical discriminator and row 6's two-$\ell_c$ symbol collision (§6).

## §1 — Provenance

### The question, and the framings (Grant, 2026-08-03, verbatim `[sic]`)
...
**The canonization was fired on Grant's GO of 2026-08-03, verbatim `[sic]`: *"Should the wall table go into KB?"*** — treated as ratification of the table's placement, on the same reading as the 2026-08-02 paperclip precedent ([`electron-plumbing-primer.md`](electron-plumbing-primer.md) Step 3.5, whose Grant-verbatim / orchestrator-refinement split this section copies deliberately).

### What is Grant's and what is the walk's

Following the Step-3.5 attribution pattern exactly: **the three framings above are Grant's, quoted verbatim.** Everything in §§2–4 and §7 below is **orchestrator walk-level refinement added in the same conversation** — it is kept in separately-labelled sections on purpose, and it is **not canon**. The rows in §5 are neither: each row is a **pointer at an existing canonical site**, and the site's own status governs.
...
Every wall in the table below is the same *bookkeeping* statement in a different coordinate: **a channel's impedance runs to an extreme, so its reflection coefficient runs to $\pm1$, on some axis of phase space.** What differs row to row is *which axis*:
...
**Do not read "every wall is $\Gamma\to\pm1$" as "every wall is set by the same mechanism."** Canon has already ruled against that, in a dedicated tag:
...
|---|---|---|---|---|---|---|
| **1** | $r_{sat} = 7GM/c^2$ | **space** | shear (and bulk) | $-1$ short | The horizon-analog. AVE puts the saturation boundary at $3.5\,r_s$, **not** $r_s$ — *"a **falsifiable prediction** for any high-gravity observational test"*. ⚑ walk-level rider below | [`ave-bh-horizon-area-theorem.md`](../vol3/cosmology/ch15-black-hole-orbitals/ave-bh-horizon-area-theorem.md):33 (boxed $r_{sat}=7GM/c^2$), :37 (the falsifiability line); [`bulk-impedance-at-saturation-boundary.md`](../vol3/cosmology/ch15-black-hole-orbitals/bulk-impedance-at-saturation-boundary.md):51 ($G_{shear}\to0 \Rightarrow \Gamma_{shear}=-1$), :48–54 (the three-channel table) |
...
| **6** | $\ell_c = \sqrt{\gamma_c/G_{vac}}$ | **length scale** | couple-stress / micropolar bending | evanescent below cutoff → Yukawa $e^{-r/\ell_c}/r$ | The weak-force range. A structure asked for on a scale below $\ell_c$ cannot propagate: *"Any physical excitation operating **below** a medium's natural cutoff frequency becomes an **Evanescent Wave**"*, and the static field equation turns from Laplace into massive Helmholtz. ⚑ **CARRIES AN OPEN IN-CODE SYMBOL COLLISION — see §6 flag 2; do not use this row without it** | [`gauge-boson-masses.md`](../vol2/particle-physics/ch05-electroweak-mechanics/gauge-boson-masses.md):39; the collision flagged in-code at [`constants.py`](../../../src/ave/core/constants.py):331–337 |
