...
> | the discrete **row** | **`ROW-NOT-CERTIFIED`** *(the v1 run — superseded; see the currency note two rows below)* — `G-RHO2` FAILS (fitted exponent `0.0037…` against a frozen `[1.9, 2.1]`) on an injection point sized wrong at freeze; **the named repair is `k_0 = ε·ω·Z_1`, not `ε·k_cold`**, and the tolerance was not moved nor the injections re-sited | `:24`, `:52`, `:78` |
