...
- **Driver-script honesty audit** — partial cleanup 2026-05-17 across `vlbi_impedance_parallax.py` (C13 retirement), `sagnac_geo_parallax.py` (C17 + C18 retirements), and `electron_interferometry_parallax.py` (C11 factor-7 correction). Class C overclaim-only fixes remaining: `simulate_borromean_baryon.py` ("938.27 MeV" annotation without computation), `simulate_chiral_network.py` ("proves Weak Force parity violation"), `simulate_string_theory_mapping.py` ("formally proves"), `simulate_helical_confinement.py` ("proving the physics"), `simulate_electron_topology.py` ("computes exact U(1) parameterization"), `generate_astrophysical_plots.py` (plots literals labeled as AVE-distinct). Plus cross-repo: `ponder_05_gravity_parallax.py` in AVE-PONDER has same factor-7 bug; queued for PONDER working tree.
