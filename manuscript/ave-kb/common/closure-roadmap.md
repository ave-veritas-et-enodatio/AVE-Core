...
- **DAMA amplitude formula derivation — ENERGY SCALE + MODULATION CLOSED zero-parameter 2026-05-17 evening; rate magnitude open with sharpened gap.** Per [`research/2026-05-17_C14-DAMA_amplitude_result.md`](../../../research/2026-05-17_C14-DAMA_amplitude_result.md) (partial-closure result doc) building on prereg working hypothesis. **CLOSED**: $E_{refresh} = h \cdot v_{cmb}/\ell_{node} = 3.96$ keV (sits in DAMA 2-6 keV window, zero tuning); annual modulation $\Delta E/E = \Delta v_{orbit}/v_{cmb} = 4.05\%$ (matches observed envelope). Both inputs canonical: $v_{cmb} = 370$ km/s (Q-G24 preferred-frame), $\ell_{node} = 3.86 \times 10^{-13}$ m (derived from $\alpha$ + electron Compton). Substrate refresh-quantum has slow-quasi-particle dispersion $E = p v_{cmb}$ (NOT $E = pc$). **Picture refinement surfaced for Grant adjudication**: Mössbauer Debye-Waller analysis rules out substrate-pitch absorption ($f = e^{-1.22\times10^4} = 0$, completely suppressed) and supports atomic-spacing envelope absorption ($f = 0.996$, recoilless). AM-radio analog: substrate refresh acts as carrier at $\nu = 9.6 \times 10^{17}$ Hz / $hν = 4$ keV; crystal absorbs modulation envelope at atomic-spacing wavelength. **REMAINING (sharpened)**: (a) rate magnitude — naive cross-section calc gives ~$10^{45}$ events/s/kg vs DAMA empirical ~$10^{-7}$; requires Q-factor / coherence-bandwidth derivation (corpus has no template for events/s/kg; closest is Hulse-Taylor pattern at `src/ave/gravity/orbital_lc_damping.py:62-71`); (b) crystal-quality dependence (COSINE/DAMA tension) — interferometric Q at substrate refresh rate; testable via cross-correlation of crystal-quality metrics across NaI batches. **First positive zero-parameter ENERGY-SCALE result for DAMA at substrate refresh rate.** C14 row updated: energy + modulation U-C (forward-prediction CONFIRMED); rate magnitude U-D pending Q-factor derivation. Foreword update DEFERRED until rate-bridge closes or DAMA-vs-COSINE crystal-quality cross-correlation provides a second corroborating prediction.
