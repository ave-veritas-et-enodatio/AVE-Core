...
- **C13-VLBI-DARK** ([`divergence-test-substrate-map.md:411`](divergence-test-substrate-map.md)) claims "dark matter IS continuous geometric stretching of $377\Omega$ vacuum impedance baseline." Audit (2026-05-16 during C18 cascade walk) flagged that this contradicts the achromatic-impedance-matching theorem ([`achromatic-impedance-matching.md:24`](../vol3/gravity/ch03-macroscopic-relativity/achromatic-impedance-matching.md) + [`z0-derivation.md:79`](../vol4/circuit-theory/ch1-vacuum-circuit-analysis/z0-derivation.md)) which proves $Z_{local}(r) \equiv Z_0$ exactly. **Adjudication pending Grant physics-judgment call.**
- **C11-MACH-ZEHNDER** ([`divergence-test-substrate-map.md:409`](divergence-test-substrate-map.md)) claims a 35-rad shift on 1-m macroscopic Mach-Zehnder from $n_s/n_t = 9/2$ refractive split. This is a directional anisotropy probe at optical wavelength — potentially subject to the same cubic-symmetry suppression that retired C17. **Audit not yet spawned.** Cascade implication: if C11 retires, the $\nu_{vac} = 2/7$ triangulation (C1 + C11 + C12) loses a leg.
