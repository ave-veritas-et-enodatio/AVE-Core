...
- **Driver-script honesty audit** — extended cleanup 2026-05-17 across `vlbi_impedance_parallax.py` (C13 retirement), `sagnac_geo_parallax.py` (C17 + C18 retirements), `electron_interferometry_parallax.py` (C11 factor-7 correction), `ponder_05_gravity_parallax.py` cross-repo on AVE-PONDER (same factor-7 fix), and 5 Class C overclaim-only scripts (`simulate_borromean_baryon.py`, `simulate_chiral_network.py`, `simulate_string_theory_mapping.py`, `simulate_helical_confinement.py`, `simulate_electron_topology.py`) — language softened to match what the code actually computes (visualization, not formal proof/derivation). **Remaining: Class B `generate_astrophysical_plots.py`** (plots literals labeled as AVE-distinct; either re-wire to actual operators or remove/relabel) + broader ~380-script sweep outside the candidate-list heuristic.
- **CMB axis $(174°, -5°)$ citation gap** (flagged 2026-05-17 audit) — corpus references "Planck data, ≈ (l=174°, b=-5°)" but does not pin a specific publication. The cited methodology reference (Land+Magueijo 2005) gives a DIFFERENT axis value $(237°, 63°)$. Pre-execution pin needed before C5 driver runs: identify the specific Planck-data analysis + paper that gives $(174°, -5°)$. Likely candidates: Planck 2018 PR3 low-ℓ multipole alignment analysis, or a specific CMB anomaly axis from the Planck collaboration's "Isotropy and statistics of the CMB" paper series.
...
- **DAMA amplitude formula derivation — Q1+Q2 RESOLVED 2026-05-17 evening, refresh-rate working hypothesis landed.** Per [`research/2026-05-17_C14-DAMA_amplitude_prereg.md`](../../../research/2026-05-17_C14-DAMA_amplitude_prereg.md) (updated 2026-05-17 evening per Grant physical-intuition session). **Working hypothesis**: DAMA is a high-Q acoustic interferometer measuring Earth's local refresh-rate modulation in the K4 discrete substrate. K4 lattice refresh rate $\sim v_{wind}/\ell_{node} \approx 9.6 \times 10^{17}$ Hz per unit area (Earth's frame, 370 km/s through CMB-rest substrate); annual ±15 km/s modulates by ~4%; coherent crystal lattice acts as long-baseline interferometer. Solids (G > 0) sustain coherent phase-lock → detect; liquids (G = 0) have no coherent baseline → no detection (matches XENONnT). Q1 resolved: both bulk-density and shear-modulus framings correct as conditional (bulk density sets magnitude × shear-support binary gate). Q2 resolved: CMB-dipole frame (370 km/s) wins; lattice doesn't rotate with galaxy. **Remaining gaps**: (a) proportionality constant from $\nu_{kin} \times \kappa_{crystal} \times \Delta v_{wind}$ (m³/s²) to cpd/kg/keV — estimated 1-2 sessions of derivation work; (b) crystal-quality dependence to explain COSINE/DAMA tension — interferometric Q at substrate refresh rate ~10¹⁷ Hz may differ between NaI batches; testable via cross-correlation of crystal-quality metrics against observed modulation amplitudes.
...
- **5-galaxy → SPARC catalog ingestion** (C13a row): current "validation" is 5 hard-coded galaxies in [`galactic_rotation.py`](../../../src/ave/regime_3_saturated/galactic_rotation.py); promotion to full forward-prediction status requires real SPARC catalog (175+ galaxies, public) ingestion + benchmark.
- **Bullet-cluster offset distance derivation** (C13b row): PREREG landed 2026-05-17 at [`research/2026-05-17_C13b_bullet_cluster_prereg.md`](../../../research/2026-05-17_C13b_bullet_cluster_prereg.md). Same shape as DAMA Q1+Q2 — green-field with load-bearing physics tension. Three discriminator-bearing interpretations for Grant adjudication: (α) propagating sub-luminal TT shockwave (requires κ-coupling-reduced $v_T$ at cluster scale; no precedent in corpus); (β) standing TT-mode at collision site, lensing tracks coherent substrate-mode decay (DAMA-template reframe; AVE-Fusion has standing-Tensor-Shockwave precedent); (γ) Vol 1 Ch 4 TT-shockwave framing is stale, Vol 3 Ch 5 η_eff halo superposition wins (matches existing driver `simulate_bullet_cluster_fdtd.py` which is also mislabeled — same anti-pattern as retired vlbi_impedance_parallax.py). Adjacent flags: $G_{vac}$ corpus discrepancy ([`lc-electrodynamics.md:35`](../vol1/dynamics/ch4-continuum-electrodynamics/lc-electrodynamics.md) vs [`derived-numerology.md:52,56`](../vol2/appendices/app-f-solver-toolchain/derived-numerology.md) — factor 7.7 off, derived-numerology says lc-electrodynamics is wrong but uncorrected); bullet-cluster driver mislabel + 3 sibling animation scripts inheriting it.
