...
| Op16 | Universal Wave Speed | $c_{shear} = c_0\cdot\sqrt{S}$ | 317 | CANONICAL — Vol 1 Ch 6 §1.14 narrative ("Freezes wave propagation dynamically as $S \to 0$") + grep-verified explicit formula at ≥2 cross-citations: vol_2 ch 7 line 1032 + lines 985-993 (label `eq:c_shear`, $c_{eff} = c_0\cdot S^{1/2}$), **[KB canonical: Op14 Local Clock Modulation](../vol4/circuit-theory/ch1-vacuum-circuit-analysis/op14-local-clock-modulation.md)** (substrate-native time-dilation via $\omega_{local} = \omega_{global}\sqrt{1-A^2}$; cross-volume parallel to gravitational $\tau_{local} = n(r)\tau_{unstrained}$). Gravitational analog: $c \cdot \sqrt{1-A^2} \equiv$ Schwarzschild $c \cdot \sqrt{1-r_s/r}$ in $r_s/r \ll 1$ limit. ⚑ **VACATED CITE (2026-08-07) — the two vol-2 ch-7 cross-citations in this cell do NOT carry $c_0\sqrt{S}$ at HEAD, and three mutually-inconsistent forms are in play. The Op16 form is NOT adjudicated here. See §8 FLAG-CEFF-CITE at the end of this leaf before citing this row's formula or its receipts.** |
...
| Op19 | Refractive Index | $n(r) = 1 + \nu_{vac}\cdot\varepsilon_{11}$ | 319 | CANONICAL — explicit equation Vol 1 Ch 6 §1.18 + cross-citations: `eq_gravity_derived.tex` (canonical Symmetric Gravity $n(r) = 1 + 2GM/(rc^2)$ form post-axiom-homologation), Vol 3 Ch 3. $\nu_{vac} = 2/7$ (Poisson ratio: 2 compliance / 7 total modes). Disambiguation of "linear" vs Ax4: [`def-ln3str`](vocabulary-register.md); assembly [`bond-lc-constitutive-grading.md`](../vol3/gravity/ch03-macroscopic-relativity/bond-lc-constitutive-grading.md). Dated WALK note after this table — not a change to the formula. |
...
> **Dated note (2026-08-28, WALK; not a change to the Op19 formula above).** Op19 is linear in $\varepsilon_{11}$ by construction (photoelastic / kinematic). It is **not** the Ax4 Taylor series; the kernel's index shift is $O(A^2)$ ([`four-regimes.md`](../vol1/operators-and-regimes/ch7-regime-map/four-regimes.md):110–114; [`research/2026-07-31_anisotropy-observable_scoping.md`](../../../research/2026-07-31_anisotropy-observable_scoping.md):893). Pointers: [`def-ln3str`](vocabulary-register.md), [`bond-lc-constitutive-grading.md`](../vol3/gravity/ch03-macroscopic-relativity/bond-lc-constitutive-grading.md).
...
|---|---|---|
| **ε / C** | **swing-ε** (C keyed on $V$): $S(A)=\sqrt{1-A^2}$ — **CANONICAL** (Op2/Op14 family; the Letter's kernel; bench analog = varactor). Particle-tier endpoint: Schwinger / pair production at $V_{SNAP}$. | **slew-ε** (C keyed on $I$ = charge-rate; bench analog = dielectric relaxation / finite polarization rate): **MEASURED — INTERFACE-SCOPED; BULK VERTEX OPEN** (fork A two-tone run, PR #610). Banked the ★ parity theorem (even kernel ⇒ pure χ³ ⇒ difference tone forbidden sub-yield = inversion-symmetry meter, `clm-invmtr`) + the A⁶ χ³ law (`clm-a6chi3`); the **co-located interface** reading was the artifact (flat by construction, $q_{raw}=2.93$), while at bulk separation (sep≥3) the beat **collapses 17.1 orders tracking the physical skin suppression** (vindicating the frozen $O_{skin}$ bulk model) — so the *frequency* vertex is NOT probed and the slew-ε **population stays OPEN** pending the 3D clamp-free run. Canonical: [`universal-saturation-kernel-catalog.md` § The parity theorem](universal-saturation-kernel-catalog.md). |
...
## §8 — ⚑ FLAG-CEFF-CITE — the Op16 wave-speed cite is VACATED (dated 2026-08-07; NOT adjudicated; routed for derivation)

**Placement note.** EOF append plus a one-token ⚑ marker inside the `:56` cell — both line-count-neutral, so none of the ~90 inbound `operators.md:NN` cites (25 at `:57`, 29 at `:61`, 17 at `:145`) are rotted by this flag.

**What is vacated, and why "vacated" rather than "wrong".** The Op16 row at `:56` states $c_{shear} = c_0\cdot\sqrt{S}$ and offers two vol-2 ch-7 cross-citations as its receipt. **Re-read at HEAD, neither cited site carries that form, and the label attribution is crossed.** Per the vacated-cite pattern, the *receipt* has drifted past the state it certified — so the argument for the stated form is **dead, not refuted**. The form may still be right; it is simply **no longer supported by what it cites**. Nothing here rules on which form is correct.

**The three forms now in play — each quoted verbatim from HEAD, none preferred:**

| # | Site | Verbatim at HEAD | Form |
|---|---|---|---|
| 1 | [`operators.md`](operators.md):56 (Op16, this leaf) | `c_{shear} = c_0\cdot\sqrt{S}`, receipted as *"grep-verified explicit formula at ≥2 cross-citations: vol_2 ch 7 line 1032 + lines 985-993 (label"* `eq:c_shear`, `c_{eff} = c_0\cdot S^{1/2}` *")"* | $c_0\sqrt{S}$ |
| 2 | `manuscript/vol_2_subatomic/chapters/07_quantum_mechanics_and_orbitals.tex`:991, label at `:994` | `= c_0\,\sqrt{1 - A^2}` under `\label{eq:c_eff}` | $c_0\sqrt{1-A^2}$ |
| 3 | same file `:1032`, label at `:1033` | `\boxed{c_S \equiv c}` under `\label{eq:c_shear}` — the surrounding text reads *"The speed of light \emph{is} the shear wave speed."* | $c_S \equiv c$ (**no saturation factor at all**) |
| 4 | [`claim-quality.md`](claim-quality.md):396 | *"Master Equation FDTD's non-linear d'Alembertian carries"* `c_{eff}(V) = c_0/\sqrt{S(A)}` | $c_0/\sqrt{S}$ (**rises** as $S\to0$) |

**The three specific cite defects, stated plainly:**

1. **Label crossed.** `:56` attributes the label `eq:c_shear` to the *"lines 985-993"* block. At HEAD that block carries `\label{eq:c_eff}` (`:994`); `\label{eq:c_shear}` is a **different equation** at `:1033`.
2. **Neither receipt carries $\sqrt S$.** `:56` reports the cited content as $c_{eff}=c_0\cdot S^{1/2}$. Site 2 reads $c_0\sqrt{1-A^2}$ and site 3 reads $c_S\equiv c$. (Under the canonical kernel $S(A)=\sqrt{1-A^2}$ site 2 is $c_0\,S$ — **stating that identification is not adjudicating it**; whether the ch-7 $A$ is the same normalized amplitude is part of what the derivation lane must settle.)
3. **The row is internally inconsistent.** The same `:56` cell's gravitational analog uses $c\cdot\sqrt{1-A^2}$ — i.e. **the site-2 form**, not the $c_0\sqrt S$ the row headlines.

**Direction of the disagreement is not cosmetic.** Forms 1 and 2 make the speed **fall** toward saturation; form 4 makes it **rise** ($c_{eff}\to\infty$ as $S\to0$); form 3 makes it **constant**. These are different physics at the saturation core, not different notations.

**Disposition: ROUTED, NOT RULED.** No form is preferred, no site is edited, no receipt is repaired. Until a derivation lane returns, **do not cite the Op16 row's formula or its two vol-2 cross-citations as mutually supporting**, and do not treat any of the four sites as confirming another.
