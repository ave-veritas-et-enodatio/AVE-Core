...
| Op19 | Refractive Index | $n(r) = 1 + \nu_{vac}\cdot\varepsilon_{11}$ | 319 | CANONICAL — explicit equation Vol 1 Ch 6 §1.18 + cross-citations: `eq_gravity_derived.tex` (canonical Symmetric Gravity $n(r) = 1 + 2GM/(rc^2)$ form post-axiom-homologation), Vol 3 Ch 3. $\nu_{vac} = 2/7$ (Poisson ratio: 2 compliance / 7 total modes). **Not the Ax4 Taylor series** — photoelastic / kinematic, linear in $\varepsilon_{11}$; the kernel's index shift is $O(A^2)$. Disambiguation: [`def-ln3str`](vocabulary-register.md); assembly [`hop-lc-constitutive-grading.md`](../vol3/gravity/ch03-macroscopic-relativity/hop-lc-constitutive-grading.md) |
