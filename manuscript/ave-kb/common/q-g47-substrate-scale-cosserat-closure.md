...
- Vol 3 Ch 1's **Machian** $\xi_M \sim 10^{44}$ ($8.15\times10^{43}$; cosmic-scale impedance integral, $G = c^4 / (7\xi_M T_{EM})$ — its cell-count $R_H/\ell_{node}\sim10^{38}$ is a *factor inside* $\xi_M$, lifted by $4\pi\alpha^{-2}$, not $\xi$ itself)
