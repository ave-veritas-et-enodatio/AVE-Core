...
> **Fork reframe (2026-07-20, #751 — the XOR is DISSOLVED; the row prose above is preserved KEEP-BOTH).** The J(ω) derivation (`research/2026-07-20_jomega-derivation_result.md` §5/§6, merged #751; sibling #758) shows the "finite-area memristive loop **vs** zero-area saturating reactance" **XOR is the wrong question** (FLAG-2): a lossless (γ=0) second-order reactive `S` traces a **FINITE** pinched loop with **exactly zero dissipated work** (the W_diss=0 leg is a definitional identity; the fireable ledger is the independent drive-work closure W_drive≈W_diss ≤2%). So **loop area ≠ dissipation** — a finite ∮ does NOT imply a resistor, and "finite loop" no longer disjoins "zero-area reactance." World (c) (an axiom-level resistor / rate-independent plastic loss) stays **EXCLUDED** (corroborated, not new). What remains genuinely OPEN, both UNDETERMINED, is (i) the **coupling-model fork** — on-site (Ohmic) vs strain (super-Ohmic) S→bath coupling, which decides world-(b)-transductive vs world-(a)-reactive at the crossing, pending the engine constitutive tensor; and (ii) the **physical bath SCOPE** (0D few-mode reactive-return vs ∞-lattice transductive), routed to Grant. The frozen (a-ledger)/(b-ledger) net-per-cycle-transfer criterion is **DEGENERATE** (a:0/4, b:1/4 super-Ohmic-∞ only). Grant's reversible-reactive lean is unchanged and still not banked either way; the crux stays at #59 Flag F (never RESOLVED).
...
- **The generalization (canonized 2026-08-04, `clm-xfrcst`):** [`transfer-cost-theorem.md`](transfer-cost-theorem.md) — the crossing-arrow licence above, lifted from the yield crossing to *any* regime/state/channel crossing: within-SYSTEM reactive exchange is free however many channels it crosses; an arrow exists **iff** energy crosses the SYSTEM boundary into a continuum-counted port. The MODE-loss row (`:59`) and the SYSTEM-loss row (`:60`) of the table above are its free and costed halves respectively, and its $R_{rad}$-as-**delivery** reading is what makes the `:60` port row Ax3-legal rather than a smuggled friction. **This leaf is unedited by that canonization** — no row moves, no grade moves.
