...
**[Spin-scope, B1 (2026-07-21; propagated 2026-07-31).** Step 5 and Step 6 are not alternatives for a *rotating* system: for the BH-ringdown fire, flat $Q = \ell$ (Step 5) is the **cold $a_* = 0$ anchor** — the $\Omega \to 0$ limit of Step 6 — because $\ell$ is an integer **mode count** (cavity-deformable, def-quant3 sense (1), `common/vocabulary-register.md:261`); the discriminator is its **INTEGRALITY**, not its sense class — an integer can only jump discretely, while the physical $Q$ is an impedance ratio that moves continuously with the strain profile, and an integer that can only jump cannot track a ratio that moves smoothly. Reading Step 5 through at catalog spins is the banked $\bar D_Q = -38\%$ failure (corrected-Kerr $Q$ rises $3.07 \to 3.49$). Model banner: [`vol3/claim-quality.md`](../vol3/claim-quality.md) `:204`.**]**
