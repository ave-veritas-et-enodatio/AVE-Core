...
> $\ell$ is an integer **mode count** — cavity-deformable (def-quant3 sense (1),
> `common/vocabulary-register.md:261` (def-quant3), sense (1) at `:271`, *"Failure mode: ionization destroys them"*), **not** a
> deformation-invariant winding. **The discriminator is its INTEGRALITY, not its sense class:** an
> integer cannot drift continuously with strain — it can only jump discretely — while
...
| 5 | Cinquefoil $(2,5)$ | 938 | 1519 | 141 |
| 7 | Septafoil $(2,7)$ | 1261 | 2859 | 229 |

**Validated: $c = 5$ (proton).**
The proton mass (938 vs. 938.27 MeV, $0.00\%$), the $N(1520)$ resonance (1519 vs. 1520 MeV, $-0.1\%$), and the charged pion (141 vs. 140 MeV, $+0.7\%$) all emerge from first principles with zero tuning.

**Predictions: $c = 3$ and $c = 7$.**
The trefoil soliton at 637 MeV is *not* the electron (which is an unknot $0_1$, not a torus knot); it corresponds to a hypothetical light baryon or constituent diquark. The septafoil at 1261 MeV identifies with $\Delta(1232)$ ($+2.35\%$); higher rungs of the ladder match $\Delta(1600)$, $\Delta(1900)$, and $N(2190)$ in turn (full ladder in App. B).
...
The AVE framework **reduces the Standard Model's ~26 independent empirical parameters to ~3 interlocked geometric inputs** $\{\ell_\mathrm{node}/m_e, \alpha, G\}$, with every other physical quantity then reducing to geometric ratios of the lattice pitch $\ell_\mathrm{node}$ through these exchange rates (full scorecard + named residuals in the [Full Derivation Chain](full-derivation-chain.md)). Lifting even those three inputs to geometric outcomes --- a **structurally zero-parameter** framework --- is the stated **target**, gated on the open gaps tracked in that scorecard.
...
**[Spin-scope, B1 (2026-07-21; propagated 2026-07-31).** Step 5 and Step 6 are not alternatives for a *rotating* system: for the BH-ringdown fire, flat $Q = \ell$ (Step 5) is the **cold $a_* = 0$ anchor** — the $\Omega \to 0$ limit of Step 6 — because $\ell$ is an integer **mode count** (cavity-deformable — `common/vocabulary-register.md:261` (def-quant3), sense (1) at `:271`); the discriminator is its **INTEGRALITY**, not its sense class — an integer can only jump discretely, while the physical $Q$ is an impedance ratio that moves continuously with the strain profile, and an integer that can only jump cannot track a ratio that moves smoothly. Reading Step 5 through at catalog spins is the banked $\bar D_Q = -38\%$ failure (corrected-Kerr $Q$ rises $3.07 \to 3.49$). Model banner: [`vol3/claim-quality.md`](../vol3/claim-quality.md) `:204`.**]**
...
| First excitation | Cinquefoil $c = 5$ (proton) | $\ell = 3$ (angular multipole — **NOT** an overtone) |

🔴 *[**index cross-wire corrected 2026-07-31** (#814 compact audit); **cross-sector residue repaired 2026-07-31** (#816 compact audit, F1). The "First excitation" cell previously read "$\ell = 3$ **(1st GW overtone)**" — struck. $\ell$ is the **ANGULAR** index, and $\ell = 3$ is a **different angular multipole**, not an overtone of $\ell = 2$; the two indices cannot be identified. The corpus already scopes the $Q = \ell$ claim to the **fundamental** on exactly this ground — [`vol3/claim-quality.md:205`](../vol3/claim-quality.md): *"for higher modes ($\ell > 2$) this disagrees with GR overtone structure — the claim is the fundamental mode, not the full QNM spectrum."* **SECTOR TAG — why the cell now carries the de-claim ONLY.** In the **ATOMIC** sector the overtone-analog index is Op6's radial node count $n_r = n - l - 1$ ([`radial-eigenvalue-solver.md:160`](../vol2/quantum-orbitals/ch07-quantum-mechanics/radial-eigenvalue-solver.md); [`operators.md:189`](operators.md), *"Op6 (eigenvalue target → radial nodes $n_r$), Op10 ($c=3$ invariant → angular nodes $l$)"*) — **both cites are atomic-sector leaves**, so $n_r$ is **not** an object of the BH column and was removed from it. A gravitational QNM overtone ($n \ge 1$) is a **damping-ladder** label, not a bound-state radial node count, and **the corpus supplies no BH overtone spectrum at all** — `research/2026-07-20_ringdown-systematics_derivation.md:79`: *"the corpus derives `ω_R ∝ ℓ` and `Q=ℓ` only for the **fundamental** (n=0). It supplies **no** overtone spectrum."* **The row's eigenvalue content is untouched** — only the parenthetical gloss is corrected.]*
...
**[Spin-scope, B1 (2026-07-21; propagated 2026-07-31).** The gravitational-scale side of this isomorphism holds at the **cold $a_* = 0$ anchor** only. That is the isomorphism's own limit, not an add-on. **Both sides are integers** — the particle-side crossing number $c$ and the BH-side mode number $\ell$ — and **integrality is the discriminator**: neither can drift continuously with strain; both can only jump discretely. The BH-side $\ell$ is a cavity-deformable **mode count** (`common/vocabulary-register.md:261` (def-quant3), sense (1) at `:271`). **Whether the particle-side $c$ is the deformation-invariant winding class (def-quant3 sense (2)) is register-OPEN and is not settled here:** `def-3638f2` carries status *ambiguous* on exactly this question (real-space knot **CROSSING** number vs the phase-space $(2,3)$ Clifford-torus **WINDING** pattern), and `def-kn0t01` locks the *phase-space* qualifier before any $(2,q)$ label attached to a particle name. Surfaced, not adjudicated. The *physical* $Q = \omega_R/2\omega_I$ is an impedance ratio that moves continuously — an integer that can only jump cannot track it — so at catalog spins the flat reading fails at $\bar D_Q = -38\%$ (corrected-Kerr $Q$ rises $3.07 \to 3.49$) and the spin dependence is carried by the m$\Omega$ law $\omega_I = (\omega_R - m\Omega)/(2\ell)$. *(Physical reading = orchestrator-walk provenance 2026-07-31, not canon.)* Model banner: [`vol3/claim-quality.md`](../vol3/claim-quality.md) `:204`; scoping doc `research/2026-07-30_qlaw-derivation_scoping.md` §1.5.**]**

**[#808 scoping F7/F8 — OPEN FLAG, not a ruling; B1 did not adjudicate this.** The $\sim 1/\ell$ per-cycle leak quoted above is a *scaling assertion* with its constant fixed to 1 by convention, never a computed radiated power. Source `research/2026-07-30_qlaw-derivation_scoping.md` §1.5 — surfaced, unadjudicated.**]**
...
|---|---|---|
| Acoustic Bulk Modulus Cavity | Atomic Orbital | The electron is an impedance mismatch ($\Gamma=-1$) that traps a longitudinal wave, bouncing at $E=V(r)$. 🔴 **[DEMOTED 2026-08-11 — R40-B2a: NEEDS RE-DERIVATION, not dead; dated note at the end of this file]** |
...
- **`:214`** — *"| Longitudinal waves | Propagate | May propagate |"*
  Stamped in place at `:214`.
  **Why it dies (audited row rationale, verbatim):** The elastic (ε₁₁<1, cold sub-yield) column flatly asserts longitudinal-wave propagation in the vacuum lattice — the phantom branch verbatim; the ruptured-column half is past-wall out of scope.
...
- **`:468`** — stamped at `:468`. *(family: acoustic-cavity-orbital)*  ⚑ **BIAS-DEBT**
  ...
  ```text
  Orbital = 'Acoustic Bulk Modulus Cavity' trapping a bouncing longitudinal wave (and :469 'lossless acoustic standing-wave' WKB) — the Γ=−1 boundary/resonance role survives, the standing wave must be re-homed to a native sector.
  ```
