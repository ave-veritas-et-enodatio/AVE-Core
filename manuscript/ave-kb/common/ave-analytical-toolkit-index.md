...
| **Op21 Quality Factor Phase Transition** | $Q = \ell$ (substrate-foundational mode-counting at $\Gamma = -1$, derived; Class B substrate-mechanism manifestation rigor); $Q \sim 1/\ln(Z_1/Z_0)$ (Cooper-pair-phase-transition specialization, conjectured-not-derived, logged as Q-OP21-BARDEEN-1 framework-extension candidate) | [`operators.md` Op21 line 61](operators.md) + [KB canonical: Op21 Multi-Mode Mode-Counting](../vol4/circuit-theory/ch1-vacuum-circuit-analysis/op21-multi-mode-mode-counting.md) | Per-mode substrate Q at $\Gamma = -1$ saturation/TIR boundary (cross-scale canonical: electron LC tank multi-codim assembly; BH ringdown [spin-scoped to the cold $a_* = 0$ anchor — Ruling B1, 2026-07-21] / knot-mode isomorphism / universal substrate-Q derivation procedure); Cooper-pair-phase-transition specialization-with-explicit-reduction-NOT-yet-derived (Q-OP21-BARDEEN-1) |
...
| **CVR EE Sweep** (the worked resonator instance) | $H(s)$ pole at $-\alpha\omega_0/2$; $Q=1/\alpha$; $\|\Gamma\|^2=1-\alpha$; 2×2 chiral $S_{LR}\ne S_{RL}^*$ | [`cvr-transfer-function.md`](../vol4/circuit-theory/ch1-vacuum-circuit-analysis/cvr-transfer-function.md) + 4 companion views | Full circuit characterization of the electron tank: DC op-point → AC $H(s)$ → Smith → phasor → root-locus (six computed views) |
...
- CVR full sweep: the electron tank as $H(s)$ (pole $-\alpha\omega_0/2$, $Q=1/\alpha$, BW $=\alpha\omega_0$), wall reflectivity $|\Gamma|^2=1-\alpha$ on the Smith chart, computed across six views (the worked resonator instance; cvr-transfer-function.md + 4 companions, vol4/circuit-theory/ch1; re-runnable `src/scripts/vol_9_device/cvr_ee_sweep/`)
...
| **B_snap** | B_snap = √(2μ₀ m_e c² / ℓ_node³) ≈ 1.89e9 T | [`src/ave/core/constants.py`](../../../src/ave/core/constants.py) (symbol `B_SNAP`) | Magnetic saturation rupture field |
...
- K4 discrete vs continuum Maxwell — discrete dispersion has Nyquist cutoff at k_max = π/ℓ_node. **✅ $k_{max}$ contradiction RESOLVED (empirical settle, branch `analysis/2026-06-16-k4-zone-edge-nyquist-settle` @ `b72045d4`; claim-text relabel pending Grant, flag-don't-fix):** this π/ℓ_node is the correct *order and origin* of the propagating WAVEVECTOR edge. [`boundary-observables-m-q-j.md`](boundary-observables-m-q-j.md):87's $0.577/\ell_{node}$ is **NOT a competing $k_{max}$** — it is the band's low-$k$ phase-**velocity** factor $1/\sqrt3 = c(k\to0)/c_{link}$ (the 3D-TLM network-velocity projection, `chiral_lattice_dynamics.py:48`), dimensionally length/time, a *velocity* mislabeled as a wavevector (the $1/\sqrt3=0.577$ coincidence enabled it). Sweeping `measure_dispersion` to the band-top ($\omega$ max, $d\omega/dk\to0$) gives the real edge: $\sqrt2\,\pi\approx4.44/\ell_{node}$ on the chiral srs axis, $\sqrt3\,\pi\approx5.44$ on the diamond-cubic control (the $\sqrt n$ are crystallographic plane-spacing projections of the single 1-bond $\pi/\ell_{node}$ edge). So there is **one** propagating wavevector edge (the $\pi/\ell_{node}$ family) and one mislabeled velocity, not two competing bounds.
