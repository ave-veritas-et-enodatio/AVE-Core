...
> **⚠ $k_{\max}$ value is corpus-contested (flag-don't-fix, 2026-06-16).** The $0.577/\ell_{\text{node}}$ used here is the *empirically-measured K4 propagating-mode cutoff* ($=1/\sqrt3$, the TLM network-velocity factor), whereas [`ave-analytical-toolkit-index.md`](ave-analytical-toolkit-index.md):179 and [`paley-wiener-hilbert.md`](../vol1/dynamics/ch3-quantum-signal-dynamics/paley-wiener-hilbert.md):10 give the *idealized continuum-cubic* Brillouin edge $\pi/\ell_{\text{node}}$ — a **different physical bound**, ~5.4× larger, sharing the one "$k_{\max}$" label. **This exemption argument is unaffected either way:** the interior $k\approx6.36/\ell_{\text{node}}$ exceeds **both** bounds, so the interior is sub-Nyquist regardless. Which value is canonical for the *general* admissibility threshold is a Grant adjudication; `measure_dispersion` (`src/ave/core/chiral_lattice_dynamics.py:85`) settles it empirically by sweeping to the zone edge.
