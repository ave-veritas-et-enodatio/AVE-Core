...
> **★ The $\mathcal{M}$-row EE-projection "inductance $L$" is a TKI translation-image, NOT a sector-ownership claim (collapse-batch T6; Grant ruling 2026-07-11 — "follow the most robust description according to the claims").** The $\mathcal{M}$ (mass) row's EE projection "inductance $L$" is the **TKI translation-image** $L = \xi_{topo}^{-2}\,m$ (mass's kinematic *dictionary image*; "identity-by-translation, NOT a derivation" per def-tk1xfm) — equivalently the Lenz inertial-resistance reading ([`newtonian-inertia-as-lenz.md`](../vol2/particle-physics/ch01-topological-matter/newtonian-inertia-as-lenz.md), `clm-jwyy6l`). It is a dictionary entry, **not** a statement that rest mass lives in the inductive-reactance sector. The Grant-ratified sector ownership governs and is the more robust claim: **rest mass is the A1 longitudinal DILATATION** (the $X_C$-side / bulk-dilatation depression, $Z_{\text{bulk}}\to0$ — [`master-equation.md:20`](../vol1/dynamics/ch4-continuum-electrodynamics/master-equation.md) "A1 dilatation-MASS"; `constants.py:72`), while the *physical* inductive reactance $X_L$ is the **micro-rotation / spin FLYWHEEL** sector ([`dual-reactance-storage-taxonomy.md:221`](dual-reactance-storage-taxonomy.md) "$X_L$ = the FLYWHEEL … NOT the rest-mass store"). **MASS (A1) $\perp$ CHARGE/spin (T2) — never one phasor** (def-portmp). So the $\mathcal{M}$-row "inductance $L$" (translation-image) and the physical $X_L$ flywheel are DIFFERENT objects; do **not** read "mass and spin are both inductance." KEEP-BOTH: the table cell is unchanged; this note supplies the owed disambiguation.
...
| Black-hole event horizon ($r = 2GM/c^2$) | Schwarzschild horizon | All matter → de-bonded state *(R9 rename 2026-08-06; BH interior = **de-bonded** — bonded once, released — as against the primordial **pre-bond** state that never bonded; gloss *pre-geodesic*)* | $M$ (ADM mass), $Q$, $J$ (Kerr-Newman) |
...
> **🔴 BOUNDARY-REGISTER TAG (2026-07-14 — the six rows conflate FOUR boundary registers under one heading; KEEP-BOTH, table unchanged).**
> "Same mechanism at all scales" holds for the **observability rule** ($\mathcal{M},\mathcal{Q},\mathcal{J}$ = the three boundary integrals) — it does **NOT** hold for **what SETS each boundary**, which is one of four distinct registers (the register-discipline "conflation map"):
> - **AMPLITUDE LEVEL-SET** — Electron, Nucleus, Atom rows: the boundary is the locus where the saturation amplitude hits threshold ($S(A)\to0$, $\Gamma\to-1$; the leaf's own opening definition). *Nucleus* confinement is additionally **topological** (the $6_2^3$ linkage); *Atom* "which boundary is the $\Gamma=-1$ skin" is **Q-G43 OPEN**.
> - **PRESSURE-BALANCE locus** — Planetary magnetopause row: $B^2/(2\mu_0) = \tfrac12\rho_{sw}v_{sw}^2$ (`clm-3kmt3p`, [`../vol3/cosmology/ch06-solar-system/planetary-magnetopause-standoff.md`](../vol3/cosmology/ch06-solar-system/planetary-magnetopause-standoff.md):12). This is **NOT** an amplitude level-set; the geodynamo reads its reflection as a $\Gamma=+1$ **open-circuit** boundary (voltage-doubling), a defensible pressure-boundary convention — see [`../vol3/applied-physics/ch13-geophysics/geodynamo-vca-back-emf.md`](../vol3/applied-physics/ch13-geophysics/geodynamo-vca-back-emf.md):12,16.
> - **COORDINATE RADIUS** — Black-hole and Cosmic-horizon rows: $r = 2GM/c^2$ (a metric locus / Schwarzschild radius), **not** a lattice-amplitude threshold.
> - (**BALANCE LOCUS** — the fourth register — appears at the *Solar* rung of the trampoline six-scale table (heliopause ram-balance / Oort $r=\sqrt{GM/a_0}$), not tabulated here; cf. the hollow-vortex balance locus, `hollow-vortex-binding.md:49`.)
>
> So the electron TIR wall (amplitude level-set) and the Schwarzschild horizon (coordinate radius) are the **same observability object** ($\mathcal{M},\mathcal{Q},\mathcal{J}$) sitting on **different boundary registers**; do not read "same $\Gamma=-1$ saturation surface" as "same boundary-setting mechanism." Mirrors the trampoline six-scale note at [`trampoline-framework.md`](trampoline-framework.md) §7.
