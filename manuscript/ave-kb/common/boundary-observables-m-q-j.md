...
> **✅ $k_{\max}$ RESOLVED — the $0.577/\ell_{\text{node}}$ above is a VELOCITY, not a wavevector (empirical settle @ `b72045d4`; claim-text relabel pending Grant, flag-don't-fix).** $0.577 = 1/\sqrt3$ is the band's low-$k$ phase-velocity factor $c(k\to0)/c_{\text{link}}$ (the 3D-TLM network-velocity projection, `chiral_lattice_dynamics.py:48`) — dimensionally length/time, **not** a $k_{\max}$. The actual propagating Nyquist WAVEVECTOR edge is the $\pi/\ell_{\text{node}}$ family: measured $\sqrt2\,\pi\approx4.44/\ell_{\text{node}}$ on the chiral srs axis ($\sqrt3\,\pi\approx5.44$ on the cubic control). **The exemption is reinforced, not weakened:** the interior $k\approx6.36/\ell_{\text{node}}$ exceeds the corrected wavevector edge ($1.43\times$ srs, $1.17\times$ cubic), so it still sits above where the lattice can propagate. The old "$6.36$ vs $0.577$" comparison was a category error (wavevector vs velocity). The claim text's "$0.577/\ell_{\text{node}}$ as $k_{\max}$" should be relabeled to the $\pi$-family edge — Grant rules; the index copy (`claims.jsonl` clm-sjjvhf rationale) carries the same string and must move in lockstep.
