...
**The buckling direction sets chirality.** In an isotropic environment, the buckling direction would be chosen randomly. In AVE, the parent black-hole rotation imposes a **global directional bias** on the **pre-bond state** *(R9 rename, 2026-08-06; formerly "pre-geodesic plasma", gloss *pre-geodesic*: no bonds → no impedance network → no wave paths → no metric)* at the moment of K4 crystallization (cf. Q-G21, Q-G35). All bonds freeze in the same rotational buckling direction. **This is the substrate's frozen chirality.**
...
The buckled state is the substrate's **rest configuration**: the lattice geometry that the pre-bond state settles into when cooled during cosmic crystallization (cosmology "bottle in freezer" mechanism).

**Hypothesis** (Grant Lindblom, 2026-05-13 late evening, formalized in Q-G47 setup doc §5):

> **$K = 2G$ is the elastic configuration at which the cooled K4 lattice has minimum free energy.** The substrate doesn't have $K = 2G$ because GR demands it (top-down) or because FTG-EMT self-consistency checks it (back-derived); it has $K = 2G$ because that's the **thermodynamic attractor** of the cooling pre-bond state.
...
The Pythagorean constraint on a buckled bond's tip position has the **same functional form** as the AVE saturation kernel $S(A) = \sqrt{1-A^2}$ (Axiom 4). *(2026-07-02 arc update: this is **more than a pedagogical correspondence** — the buckling √ is the α-free axial projection of a fixed-arc-length bond, and it does **not** require the network-level Q-G47 calc, which never emits $S(A)$; a single over-braced strut suffices. See below.)*

**Axiom 4 remains postulated** — but the 2026-07-02 Axiom-4 arc (`research/2026-07-02_axiom4-{buckling-kernel,moduli-hierarchy}_result.md`, PRs #459/#460) sharpened *what* is postulated. $S(A)=\sqrt{1-A^2}$ is the **α-free axial projection of a fixed-arc-length bond bowing**, and for any finite stretch/bend ratio $\rho=k_a/k_s$ the bond settles at a self-consistent operating arc-length $arc^*$, making $S(A)=\sqrt{1-(A/arc^*)^2}$ an **exact geometric √ — the *form* is FORCED, α-free, robust across $\rho$.** What stays postulated is the **yield anchor $arc^*$**, which inherits from the **GR-imported K=2G** ($\rho$ is K=2G-set; inextensibility $k_{stretch}\gg k_{bend}$ does not hold — $\rho\in[2,5.3]$ is a stubby strut, not an elastica). *(The $\nu=(3K-2G)/(2(3K+G))|_{K=2G}=2/7$ relationship is a **consequence** of K=2G, not the kernel's justification — the same GR-imported value that anchors $arc^*$.)* **FORM-derived / VALUE-imported**, the framework's signature (cf. G, α).
...
**Beyond $A = 1$: Regime IV.** The bond does not break. Instead, the substrate **phase-transitions** to a different state — **Regime IV de-bonded state** (the K4 lattice's parent medium) *(R9 rename, 2026-08-06 — formerly "pre-geodesic plasma"; gloss *pre-geodesic*. Regime IV is reached from inside the crystal, so this endpoint is the **de-bonded** history)*. This is the same phase found in BH interiors (**de-bonded** — bonded once, released) and pre-K4 cosmology (**pre-bond** — never bonded); one phase, two histories (cosmology "bottle in freezer" framework). The substrate beyond $A = 1$ is **not damaged; it is in a different phase**.

> **[FLAG 2026-06-11 clarification — Vol 9 Ch 14 §True Phase Diagram]** This sentence equates "Regime IV" with "pre-geodesic plasma." These are **distinct coordinates**, and conflating them is the issue Vol 9 Ch 14 §True Phase Diagram resolves: *pre-geodesic plasma* is a **thermodynamic phase** (the parent medium the crystal froze out of — it **predates** the crystal, per [`cosmic-axes-and-frames-glossary.md:75`](cosmic-axes-and-frames-glossary.md), "supercooled pre-geodesic plasma"); *Regime IV* is the **excitation-path endpoint** *within* the solid ($r \to 1$, $S \to 0$, the local rupture terminus). They **coincide** at the BH-interior rupture trajectory — driving local strain to $A = 1$ lands on the melt boundary — but the regime axis cannot *label* a phase that exists before the crystal it is built on. The substrate's true thermodynamic phase diagram (solid / melt / candidate cavitated, on the $(T, \bar\rho)$ plane) carries the melt and the candidate cavitated phase as regions, with the four-regime $r$-map a sub-structure of the solid only. Body preserved (Rule 12). **⚑ RULE-12 DATED SUPERSESSION, 2026-08-07 (R20) — this flag's BODY IS PRESERVED and NOT rewritten, and it carries the retired term BY DESIGN.** Ruled at [`2026-08-07-rulings-f3-f5-f1.md`](../../../_orchestration/docket-entries/2026-08-07-rulings-f3-f5-f1.md), Grant verbatim: *"I think we might want the full term 'pre-geodesic plasma' fully removed right?"*. **The term is now removed from LIVE vocabulary corpus-wide; this 2026-06-11 flag is history, so its wording stands.** Read it with the current words substituted: **pre-bond state** for the primordial parent medium (never bonded), **de-bonded state** for the BH-interior / Regime-IV endpoint (bonded once, released). **Pointer treatment under R20, and it is FORCED rather than chosen: a Rule-12 preserved body keeps its pointer pinned to the preserved HISTORY entry; only LIVE prose follows the rename.** So the glossary relay cited from inside this flag stays at [`cosmic-axes-and-frames-glossary.md:75`](cosmic-axes-and-frames-glossary.md) — the struck entry, whose body preserves the quoted string *"supercooled pre-geodesic plasma"* byte-verbatim — and it is deliberately NOT moved to the dated **v2** entry at `:169`, because `:169` carries the RENAMED text and this flag's quoted words may not be re-quoted. History cites history. *(A first pass of this R20 edit did re-point it to `:169`, which made the quotation cite a line not containing it; reverted.)* The one pointer that does move is internal: the sentence this flag quotes at `:267` no longer reads *"pre-geodesic plasma"*. **The flag's physics is untouched by any of this** — its whole content is that a PHASE and an EXCITATION COORDINATE are distinct objects, which is exactly as true under the new words. *(Supersedes the 2026-08-06 R9 rename note that stood here and argued the quotation must stay verbatim.)*
...
3. **Cosserat twist factor** ($f_{\text{Cosserat}}$): the chirality ($I4_1 32$ handedness) couples microrotational sector to translational sector. When you compress, the fabric twists in a specific direction. ("trampoline chirality coupling") — 🔴 **CORRECTED 2026-08-05 (PR #890, Tier-2-verified): the sentence *"When you compress, the fabric twists in a specific direction"* is FALSE AT LEADING ORDER, on this bullet's own carrier.** [`research/2026-08-05_srs-twist-coefficient_result.md`](../../../research/2026-08-05_srs-twist-coefficient_result.md) measured a homogeneous squeeze of the chiral **srs-z3** cell — the D1-ratified carrier, *"`I4₁32`, Wyckoff-8a, z=3"* (`:8`), i.e. the same space group this bullet names — and returned **`NO-TWIST`** (`:14`): *"A homogeneous squeeze of the chiral srs-z3 cell induces **NO macroscopic micro-rotation at all**"* (`:16`), $\tau = 3.3\times10^{-33}$ isotropic and $1.2\times10^{-17}$ uniaxial `[001]`, machine zero on every load and both enantiomorphs. **It is a symmetry theorem, not a small number:** point group **432** is the one non-centrosymmetric class that is not piezoelectric, and containing only proper rotations its axial rank-3 tensor vanishes with its polar one — *"**No homogeneous strain of any symmetry can turn the srs carrier**"* (`:22`). **What survives, and it is not what this bullet says:** the chirality *is* real and parity-odd, but it enters **two gradient orders down** — $\kappa/\varepsilon = \hat c_2\,q^2/\ell_{node}$, exponent **fitted** at `2.0009`/`1.9983`/`1.9869` along `[001]`/`[110]`/`[111]`, $\hat c_2$ sign-flipping exactly under enantiomorph swap. **The bullet's picture is therefore an $O(q^2)$ GRADIENT statement, and as written — a response to *homogeneous* compression — it has no leading-order referent.** ⚑ **Not adjudicated here and routed:** whether $f_{\text{Cosserat}}$ and the bridge-stands-up construction at `:452` were load-bearing on the leading-order reading is a separate question this note does not answer.
...
- **Canonical manuscript anchors**:
  ...
  - Vol 3 Ch 21 (BH Interior Regime IV) — de-bonded-state framing *(R9 rename 2026-08-06; BH interior = de-bonded, the primordial phase = pre-bond)*
