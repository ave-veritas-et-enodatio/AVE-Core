...
- **The miss-ledger (an instrument, not a lament).** The orchestrator's hopeful-interior-mechanism record is **0-for-7** (P4c; canonical booking per `_orchestration/2026-07-10_rulings-docket.md`:480,491 — the X43 → 0-for-8 and C13b → 0-for-9 increments stay **Grant-gated** per docket :271,:272,:286, so the booked figure is 0-for-7) — and that track record is itself cited as evidence when weighing the next pretty mechanism. This is *why* §5 (standing negatives) is a first-class navigational surface: the program's measured edge is honest fences + identity compressions + the bench/beam, NOT interior mechanism-hunting.
