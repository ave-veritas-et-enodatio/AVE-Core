...
- *Closed:* the "substrate selects a bore" reading; the X37 bond-bore fork. *Opened (Grant ontology Q, ~~surfaced not landed~~):* is the 1/9 per-vertex reflection a real network loss or an idealization a distributed merge smooths out? 🔴 *[ADJUDICATED — Grant soft-nod 2026-07-13, PR #669 MERGED; T4 fork close]* **bin (i) HOMOGENIZATION-SPLIT**, peer-class **CONSISTENCY / peer-with-SM** (the ordinary periodic-medium transparency physics, reproduced for the chiral srs z=3 net — NOT an AVE-distinct chord): the srs z=3 per-vertex Γ=−1/3 is a **real reactive event** (meter reproduces |Γ|²=1/9 exactly at the single vertex) but is **homogenized away for in-band collective carriers** (long-λ backscatter suppressed to σ≈0.12 of the incoherent limit; instantaneous coherent min ~0.004) and **resolves near the band edge** (R(k) crosses the bare-tee |Γ|²=1/9 at k·ℓ≈1.85, climbing past it toward the zone boundary). Bins (ii) real-at-all-k and (iii) null-everywhere both refuted. **Still-open sub-item carried:** the band edge is **NOT independently located** — the dispersion probe reached only k·ℓ≤0.83 (linear acoustic window), so "band edge" labels the high-k end of R(k)'s monotone rise, not a measured v_g→0. RESULT: `research/2026-07-13_srs-vertex-ksweep-backscatter_RESULT.md`.
...
- **The miss-ledger (an instrument, not a lament).** The orchestrator's hopeful-interior-mechanism record is ~~**0-for-6/7**~~ **0-for-9** (P4c) 🔴 *[D3 booking — Grant "classify and book" ruling 2026-07-13; resolves the prior 0-for-6/7 hedge]* — and that track record is itself cited as evidence when weighing the next pretty mechanism. This is *why* §5 (standing negatives) is a first-class navigational surface: the program's measured edge is honest fences + identity compressions + the bench/beam, NOT interior mechanism-hunting.
  - **Booked 2026-07-13 (Grant-ruled classifications):** increment **8 = X43 ringdown-port** (2026-07-11) — a frozen $\omega^5$ Sargent-form prediction; the engine returned an electric-dipole $\omega^3$ / no-law above band → **CLEAN NEGATIVE, class = frozen-form miss**, the ringdown-port picture retired (`research/2026-07-11_x43-ringdown-port_result.md`). Increment **9 = C13b Bullet-Cluster** (2026-07-11) — **−4.8σ MISS, kernel-independent** (η_eff dormant in the core, $g_{eff}/g_N=1.000$) → **class = forward-prediction miss, honest liability standing** (`research/2026-07-11_C13b_bullet_cluster_result.md`). Ledger now reads **0-for-9**.
  - *(Collision note: the sibling `docs/eod-reconciliation-hygiene-batch` lane books this same :404 hedge to a canonical **0-for-7**; this edit supersedes that to **0-for-9** by adding the two Grant-ruled increments — Grant reconciles the two at merge.)*
