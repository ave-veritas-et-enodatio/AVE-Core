...
micropolar elastic tube: pure torsional ($G_{vac}J$, longitudinal) and 🔴 **[DEMOTED 2026-08-11 — R40-B2a: NEEDS RE-DERIVATION, not dead; dated note at the end of this file]**
...
**$\alpha$ at Class B substrate-mechanism manifestation (Q-EMBED-SEL-1 Phase 1+2+3; honest-α relabel 2026-06-02).**
The closed form is in Vol 1 Ch 8 (Closed-Form $\alpha$ from the Golden Torus). Three distinct physical regimes produce three independent equations that solve uniquely to the Golden Torus geometry. Honest scope: α's value (α⁻¹ = 4π³+π²+π) is closed-form geometry whose scale (~1/137) is forced by the Compton-resonance trapping condition, but whose exact value rests on the one substrate-geometric identification $R \cdot r = 1/4$ that the substrate does NOT independently select — Class B, not a first-principles "derivation" and not a "Zero-Parameter Closure" (see Vol 1 Ch 8 §"Class B caveat (honest-α relabel)"):
...
- **`:302`** — stamped at `:302`. *(family: acoustic-mass-ratio-WZ; banked `uncertain`)*  ⚑ **BIAS-DEBT**
  ...
  ```text
  Evanescent micropolar-tube modes are shear-family, but the bending stiffness E_vac imports ν and 'acoustic dispersion' framing must show no dynamical volumetric-compliance participation — the K-as-dynamical-reservoir check is owed.
  ```
