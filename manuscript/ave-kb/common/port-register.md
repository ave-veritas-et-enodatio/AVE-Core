...
- **`:49`** — stamped at `:49`. *(family: Z_bulk formula / port speed)*  ⚑ **BIAS-DEBT**
  Quoted claim (content verified at HEAD; markup-reduced from the banked audit):
  ```text
  $Z_{bulk} = \rho\,c_{bulk}$ ... $\sqrt2\,c$ ($V_{LONG}$; $K=2G$ magic-angle PORT/impedance mode) ... gapless acoustic; edge $2\sqrt2\,c/\ell_{node}$
  ```
  Audited rationale, verbatim from the banked worklist:
  ```text
  Prereg-explicit: Z_bulk=rho*c_bulk owes formula-level re-derivation; the gapless band-edge entry presumes a spectrum branch the carve removes; covers the :126 FLAG-A label-flag (provenance case (b): the speed fracture is carve-explained, no fact-of-the-matter).
  ```
...
- **`:75`** — stamped at `:75`. *(family: F6 A1 dilatation-port instrument; banked `uncertain`)*  ⚑ **BIAS-DEBT**
  Quoted claim (content verified at HEAD; markup-reduced from the banked audit):
  ```text
  REACTIVE when bath-coupled (stores-returns) / RADIATIVE only under a deliberate lossy $\mathrm{Re}(Z)$ friction-plant termination
  ```
  Audited rationale, verbatim from the banked worklist:
  ```text
  UNCERTAIN — the collar's scalar read of the A1 projection survives as accounting/reactive; but an engineered radiative Re(Z) termination on the dilatation projection presupposes an extractable independent A1 port, which the carve denies (NO port); what a friction-plant termination reads under a bound-response A1 is owed.
  ```

  **Resolution.** The demoted carrier is the propagating A1 / bulk branch; under Axiom 5 clause G that slot is the **bound response**, so the re-derivation must be re-posed on the bound-sector constitutive law (bias $\varepsilon_{11}$, bound response $\mathbf{u}_0$, mechanism gloss back-reaction) rather than on a compression wave. **⚑ BIAS-DEBT:** this row's re-derivation turns on finite-speed bias dynamics, so the resolution is the ratified axiom **with THE BIAS PROPAGATION THEOREM standing** (clause (c1)) — the replacement is *owed, not held*.

- **`:76`** — stamped at `:76`. *(family: J(omega) infinite-lattice drain; banked `uncertain`)*  ⚑ **BIAS-DEBT**
  Quoted claim (content verified at HEAD; markup-reduced from the banked audit):
  ```text
  ∞-lattice → "RADIATIVE" only for the super-Ohmic (C2) coupling model (drains via Op3 transduction — still Ax3-lossless microscopically, not a resistor)
  ```
  Audited rationale, verbatim from the banked worklist:
  ```text
  UNCERTAIN — row is channels 3/4; if the ∞-lattice drain route is channel-3 it consumes a propagating bulk mode; quantitative half already RE-BANKED bin-(iii) DEGENERATE, so only the scope-split ordering needs re-homing (possibly channel-4/shear-carried).
  ```

  **Resolution.** The demoted carrier is the propagating A1 / bulk branch; under Axiom 5 clause G that slot is the **bound response**, so the re-derivation must be re-posed on the bound-sector constitutive law (bias $\varepsilon_{11}$, bound response $\mathbf{u}_0$, mechanism gloss back-reaction) rather than on a compression wave. **⚑ BIAS-DEBT:** this row's re-derivation turns on finite-speed bias dynamics, so the resolution is the ratified axiom **with THE BIAS PROPAGATION THEOREM standing** (clause (c1)) — the replacement is *owed, not held*.

- **`:87`** — stamped at `:87`. *(family: Q1 Reading-A row state)*  ⚑ **BIAS-DEBT**
  Quoted claim (content verified at HEAD; markup-reduced from the banked audit):
  ```text
  Standing physics is now Reading A (independent far-field bulk radiative port + O(1) coupling); the banked exclusion is LIVE — excluded at 9–110σ (Hulse-Taylor)
  ```
  Audited rationale, verbatim from the banked worklist:
  ```text
  Frozen-bin example: a register row recording the imported reading's state; under the carve the 9–110σ exclusion re-reads as the phantom's self-exclusion (receipt FOR the carve, not against the framework); covers :91 (Reading-B's owed-mechanism debt, discharged by the constraint), :109 tally, :127 bulk+shear double-count flag (bulk attribution void, GW=shear-only resolves it).
  ```
