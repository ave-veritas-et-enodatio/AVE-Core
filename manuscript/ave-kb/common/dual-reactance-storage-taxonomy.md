...
| 2 | **K/G = 2** | bulk/shear modulus ratio at the EMT trace-reversal operating point → √2 longitudinal/photon speed ratio | ratio 2 (→ √2) | [`../vol1/claim-quality.md`](../vol1/claim-quality.md):1391 (clm-uu1qbo); [`../vol1/axioms-and-lattice/ch1-fundamental-axioms/zero-parameter-universe.md`](../vol1/axioms-and-lattice/ch1-fundamental-axioms/zero-parameter-universe.md):32 🔴 **[DEMOTED 2026-08-11 — R40-B2a: NEEDS RE-DERIVATION, not dead; dated note at the end of this file]** *(cite excerpts, byte-exact at the cited lines — appended because the R40-B2a stamp modified this line and `verify-new-cite-excerpts` re-classifies its pre-existing cites as added: `The EMT argument below is a *downstream consequence* of this closure: once $\alp`)* |
...
> **🔴 $X_L$ = the FLYWHEEL (spin/frequency-regulation) sector, NOT the rest-mass store (2026-06-20, Rule 12 — the "magnetic → $X_L$ → rest mass" gloss at ~:164, the §"branch" table row at ~:198, and all bodies above PRESERVED unedited; Grant-ratified mass-sector ruling; EXTENDS the 2026-06-18 PR#260 B3-DEGENERATE banner above).** Where this leaf glosses the **Magnetic** branch ($\mu_{eff}\to0$) inductive sector "$X_L$" as **rest mass**, the **$X_L$ = the inductive FLYWHEEL = the T2 / Cosserat micro-rotation ($\omega$) frequency-regulation (spin) sector** — the local frequency regulator / Park-dq FOC rotating frame, whose mass-gap is the Compton/Larmor *clock* gap, **NOT** the rest-mass store. *(⚠ 2026-06-20 CORRECTION: this flywheel clock is the **spin/Larmor** clock — NOT the gravitational time-dilation clock, which is the Op14 A1-saturation clock $\omega_{local}=\omega\sqrt{1-A^2}$ [`op14-local-clock-modulation.md`](../vol4/circuit-theory/ch1-vacuum-circuit-analysis/op14-local-clock-modulation.md):11; two distinct clocks, see [`master-equation.md`](../vol1/dynamics/ch4-continuum-electrodynamics/master-equation.md):20 "TWO DISTINCT CLOCKS" note.)* The **rest mass is the A1 longitudinal DILATATION** (the $X_C$-side / bulk-dilatation depression, $Z_{bulk}\to0$; [`master-equation.md`](../vol1/dynamics/ch4-continuum-electrodynamics/master-equation.md):20 "A1 dilatation-MASS"), held at $90°$ to T2 by the **GRADE orthogonality** (**A1 ⊥ T2**). The flywheel regulates the *frequency that SETS* the mass (Compton $f = mc^2/\hbar$ → A1 depression depth); mass stays A1. *(⚠ 2026-06-20 CORRECTION: FOC does NOT FORCE A1 ⊥ T2 — the temporal-within-tank "FOC d-q" reading is RETRACTED [`clm-533gvm`](claim-quality.md), solidity 0.30; the canonical FOC homes [`helium-symmetric-cavity.md`](../vol2/quantum-orbitals/ch07-quantum-mechanics/helium-symmetric-cavity.md):52-56 are SPATIAL inter-object 90°. A1 ⊥ T2 stands FOC-INDEPENDENT on the grade decomposition; REFUTED flag at [`master-equation.md`](../vol1/dynamics/ch4-continuum-electrodynamics/master-equation.md):20.)*  <!-- rule12-freeze: base=d4c28cf8b86ec2887a29ccd870a9f4cc6ec83034 region=above offset=0 lines=31 bytes=2251 sha256=eea98e38c696bdbe8a4c9b6b813ede3ad758f3cabe63eb3366591846fec72291 -->
>
...
- **`:51`** — stamped at `:51`. *(family: FLAG-A-speed-split)*  ⚑ **BIAS-DEBT**
  ...
  ```text
  K/G=2 as static import survives, but the '→ √2 longitudinal speed' derivation step mints a bulk propagation speed from it — formula-level consumption (clm-uu1qbo family; repeated at :245).
  ```
