...
> **🔴 $X_L$ = the FLYWHEEL (spin/frequency-regulation) sector, NOT the rest-mass store (2026-06-20, Rule 12 — the inline "magnetic branch ($\mu_{eff}\to0$, $X_L$) is the rest-mass branch" gloss above PRESERVED unedited; Grant-CONFIRMED mass-sector ruling; SYNCS this site to the PR#260 banner at [`dual-reactance-storage-taxonomy.md`](dual-reactance-storage-taxonomy.md):189).** Where this overview glosses the **magnetic** branch inductive sector "$X_L$" as **rest mass**, the **$X_L$ = the inductive FLYWHEEL = the T2 / Cosserat micro-rotation ($\omega$) spin sector** (the spin/Larmor clock), whose mass-gap is the Compton/Larmor *clock* gap, **NOT** the rest-mass store. The **rest mass is the A1 longitudinal DILATATION** depression ($Z_{bulk}\to0$; [`master-equation.md`](../vol1/dynamics/ch4-continuum-electrodynamics/master-equation.md):20 "A1 dilatation-MASS"). The dual-reactance **COUNT** ($X_C + X_L$, $\mathcal{V}_{total}=2.0$) — the load-bearing point of this $\tau_{yield}$ provenance — is **UNAFFECTED**; only the "$X_L$ = rest mass" *label* is re-scoped to the flywheel sector. Consistency-class relabel, body preserved per Rule-12.
