...
> **Channel-subscript note (2026-06-10, Grant rename-queue adjudication R5 — paragraph above preserved unedited):** the "longitudinal-shear" signature here is the **SHEAR channel** (the Cosserat deviatoric $\tau_{zx}$, a shear stress with longitudinal propagation direction) — explicitly **NOT** the bulk-volumetric/dilatational **longitudinal-V** grade (the "3"). The bare "longitudinal" is channel-subscripted to **shear** per the dark-sector registry (Rule 3, four-way ambiguity: bulk / shear / EM-forbidden / port). Registry §5 R5.
