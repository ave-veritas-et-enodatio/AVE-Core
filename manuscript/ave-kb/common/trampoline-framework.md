...
> **⚑ CURRENT-STATUS FLAG (2026-07-03 cosmic-rotation exposure audit, docket D — surfaced per flag-don't-fix, NOT resolved here).** The five observability channels above (CMB low-multipole; large-scale-structure spin correlations; Hubble-flow anisotropy; cosmic shear; frame-dragging clock comparison) carry **NO quantified amplitude** anywhere in the corpus, and have **NOT been checked against the CMB-isotropy / Bianchi cosmic-rotation bounds** (the Barrow / Gödel / Saadeh $\omega/H \lesssim 10^{-9}$ lineage — a corpus-wide grep returns zero hits for it). Two cross-wiring cautions the list should be read against:
> - **The canonical present-day residual is STATIC, not an ongoing rotation.** The Kerr-BH framing here ("frame-dragging", "gravitomagnetic effects") reads as a persisting boundary angular momentum sourcing ongoing frame-drag, but the canonical present-day imprint of $\Omega_{\text{freeze}}$ is the STATIC over-bracing $u_0$ (§1.3.6, this leaf, the $u_0 = \rho\,\Omega_{\text{freeze}}^2\,r_{\text{node}}^2 / 2K_0$ freeze-in), plus the $I4_132$ chirality handedness and the frozen grain axis. Per the 2026-06-05 trampoline audit ([`research/2026-06-05_trampoline-metaphor-audit-result.md:60`](../../../research/2026-06-05_trampoline-metaphor-audit-result.md)) **"$\omega = 0$ at rest is an EXACT FIXED POINT of the even-in-$\omega$ coupling … Net $\omega$ is an excited-soliton flywheel property, never a rest DC field … its only per-node imprint is the STATIC over-bracing $u_0$."** So $\Omega_{\text{freeze}}$ does not collide head-on with the $\omega/H$ rotation bounds at leading order — but the frame-dragging-flavored channels above advertise ongoing-rotation observables whose amplitudes are unstated and could silently bite.
> - **The spin-down ledger is explicitly UNWRITTEN.** Where the genesis rotation went ("$\Omega_{\text{freeze}} = \mathcal{J}_{\text{cosmic}}/I_{\text{today}}$ with the dilution/spin-down dynamics") is not derived; it is flagged as inference-not-leaf in the 2026-06-07 dilution-trajectory scope ([`research/2026-06-07_cosmic-operating-point-dilution-trajectory-scope.md:142`](../../../research/2026-06-07_cosmic-operating-point-dilution-trajectory-scope.md)). Until that ledger and a per-channel amplitude (or explicit zero) against the Bianchi bounds is written, these five channels are OPEN-GAP, not tested predictions. Full context: `research/2026-07-03_historical-exposure-audits_synthesis.md` §D. The list is left intact; only its status is flagged.
...
**(B2 re-scope 2026-06-14):** $\alpha$ and $G$ (Routes 1–2) **fix** $u_0^*$ — they are the fit inputs ($u_0^*$ back-solved from CODATA $\alpha$, $G$), so their mutual agreement is guaranteed by construction and is *not* a test; $\mathcal{J}_{\text{cosmic}}$ (Route 3), measured *independently* of $\alpha$, $G$, **tests** whether it lands on the same $u_0^*$ — pass = chord, fail = echo. This is the sharp falsification test: one genuinely independent route ($\mathcal{J}_{\text{cosmic}}$) testing a number that two others set. <!-- 🔴 Rule-12 2026-06-19 B2: prior wording "All three must give the same u0* or the framework is wrong. This is a sharp falsification test: AVE commits to a single value of one parameter, derivable from three independent observational routes." superseded — α, G are fit inputs (not independent), so their agreement is by construction; J_cosmic is the one independent test. Per the 2026-06-14 B2 re-scope. -->
...
> **🔴 FLYWHEEL-CLOCK GAP, NOT THE REST MASS (2026-06-20, Rule 12 — line above PRESERVED unedited; Grant-ratified mass-sector ruling).** The rotation-sector "mass gap" $m_\omega^2 = 4 G_c / I_\omega$ is the **FLYWHEEL's frequency / clock gap** (the Compton/Larmor clock of the T2 / Cosserat micro-rotation $\omega$ regulator), **re-scoped from "the rest mass."** The rest-mass *store* is the orthogonal **A1 longitudinal DILATATION** (the depression, [`master-equation.md`](../vol1/dynamics/ch4-continuum-electrodynamics/master-equation.md):20 "A1 dilatation-MASS"; [`lattice-extreme-bh-rationality.md`](../vol3/cosmology/ch15-black-hole-orbitals/lattice-extreme-bh-rationality.md):28), held at $90°$ to T2 by the FOC orthogonal decoupling (**A1 ⊥ T2**). The $\omega$-gap regulates the *frequency that SETS* the mass (via Compton $f = mc^2/\hbar$); the mass itself stays A1. Consistency-class re-scope, not a new chord. Body preserved per Rule-12.
...
**The half-cover paradox.** ~~When you rotate the field by $2\pi$ (one full turn), the frame must rotate by $\pi$ (half turn) for the bond network to re-close in the same configuration.~~ ~~This is the SU(2) → SO(3) double cover: 720° of field rotation = 360° of frame rotation = identity.~~ **This is the SU(2) → SO(3) double cover: 720° of FRAME rotation = 360° of FIELD rotation = identity.** **🔴 DIRECTION-OF-THE-2 CORRECTION (2026-08-06, Rule 12 — the line is PRESERVED and the first clause struck IN PLACE; Grant ruling R16, FLAG-D STRUCK. No line inserted or removed: ~136 corpus lines cite this leaf below this point, so a line-count change here would manufacture the exact FLAG-C anchor-drift the same pass is repairing).** **Corrected first clause:** rotate the **frame** by $2\pi$ and the **field** advances $\pi$ — the FRAME is the full-cover side and carries the 2, exactly as this section's own A-008 resolution states four lines below ($m_{\text{Cosserat}} = 2$ is the frame twist rate; $\omega_C = m_e = 1$ is the field frequency). **Canonical direction:** the medium's rotational-sector gap is the HIGHER frequency, $\hbar\omega_m = 2 m_e c^2 = 1.022$ MeV; the observable is the half-cover, $m_e$ (observable) $=$ medium$/2$. Ruling [`2026-08-06-rulings-final-batch.md`](../../../_orchestration/docket-entries/2026-08-06-rulings-final-batch.md):48 — *"FLAG-D: STRIKE. The A-008 direction is canonical"*; basis record [`2026-08-05-a008-factor-propagation.md`](../../../_orchestration/docket-entries/2026-08-05-a008-factor-propagation.md):56 — *"direction-of-the-2 prose"*. **⚑ FINDING, SURFACED NOT FIXED — the ruling's stated rationale does not reproduce at HEAD.** R16 names the strike target as *"first clause vs its own second clause"*. Read at this commit, the two clauses are **mutually consistent**, not contradictory: the first gives field:frame $= 2\pi:\pi = 2{:}1$ and the second gives $720°{:}360° = 2{:}1$ — the same ratio, the same direction. What the first clause actually contradicts is the **A-008 resolution below it** ($:225$–$:227$), which puts the 2 on the frame. So the second clause carries the struck direction too. It is **left byte-untouched** because R16 struck one clause and this lane does not widen a ruling; the discrepancy is recorded here rather than silently resolved. → ruling author. **🔴 R21 — THE STRIKE EXTENDS TO THE SECOND CLAUSE (2026-08-07, Rule 12 — both clauses PRESERVED struck-in-place, no line inserted or removed; ~136 corpus lines cite this leaf below this point).** Grant ruling R21, [`2026-08-07-rulings-f3-f5-f1.md`](../../../_orchestration/docket-entries/2026-08-07-rulings-f3-f5-f1.md) — *"follow rec."* **Corrected wording, verbatim from R21:** *"This is the SU(2) → SO(3) double cover: 720° of FRAME rotation = 360° of FIELD rotation = identity."* **Why the first correction was not enough:** the two original clauses were mutually CONSISTENT — both put the 2 on the FIELD — and R21 rules that BOTH are backwards against the Grant-adjudicated A-008 resolution four lines below (`:225`–`:227`: the FRAME carries the 2). **PROVENANCE CORRECTION, carried at R21 and not re-derived here:** R16's stated rationale (*"first clause vs its own second clause"*) originated in the A-008 propagation record's FLAG-D table entry and was carried into R16 verbatim by the orchestrator; the actual inconsistency was always both-clauses-vs-the-A-008-resolution. **R16's SUBSTANCE is unchanged** — the canonical direction is still ħω_m = 2m_ec² = 1.022 MeV with the observable at medium/2. This supersedes the 2026-08-06 note above, which recorded the second clause as surfaced-not-fixed pending exactly this call.
...
|---|---|---|---|
| 1 — Electromagnetic (α) | $\alpha$ to 12 decimals (CODATA) | $u_0^*$ via Q-G47 + magic-angle closure | Q-factor identity for $\alpha$; the linked Q-G19α g-2 closure reaches printed-precision agreement with PDG **[ppm label struck 2026-08-03 per Grant ruling; no ppm number substituted]** **only conditional on the $n_q$-additivity postulate** (parameter-free Route B forward is $+4.0\%$ off) |
...
**The single-cosmological-parameter claim (B2 re-scope):** $\alpha$ and $G$ **fix** $u_0^*$ — they are the fit inputs ($u_0^*$ back-solved from CODATA $\alpha$, $G$), so their mutual agreement is guaranteed by construction and is *not* a test; $\mathcal{J}_{\text{cosmic}}$ (route 3), measured *independently* of $\alpha$, $G$, is the **one genuinely independent test** — if it lands on the $\alpha$–$G$-fit $u_0^*$ the chord is real, if not it is an echo. This is the sharpest commitment AVE makes — one genuinely independent observational path ($\mathcal{J}_{\text{cosmic}}$) testing a number set by two others. <!-- 🔴 Rule-12 2026-06-19 B2 re-scope: prior wording "all three routes must give the same u0*. Any pairwise inconsistency falsifies the framework. This is the sharpest commitment AVE makes — three independent observational paths constraining one number" superseded — two of three (α, G) are fit inputs (not independent), so their agreement is by construction; J_cosmic is the one independent test (pass=chord/fail=echo). Per the 2026-06-14 B2 re-scope; mirrors the two hubs + clm-dsb560. -->
...
**Order-of-magnitude check:** $R_H / \ell_{\text{node}} \approx 10^{26}/10^{-13} = 10^{39}$ (precise derivation value $\approx 3.456\times10^{38}$). Combined with geometric factors $\sim 10^3$, this gives a predicted hierarchy $\sim 10^{42}$ — matching observed $\alpha/\alpha_G \approx 4.2 \times 10^{42}$ qualitatively. **AVE's prediction for why gravity is so weak: the cosmic horizon is far away. Not fine-tuning; geometry.**

**What's needed for quantitative test:**
1. Explicit $T_{EM}(u_0^*)$ — Q-G47 substrate-scale closure now canonical via FTG-EMT (Path C: p* = 8πα at z_0 = 51.25 verified to 0.003%); $T_{EM}$ at the operating point follows from substrate constitutive tensor + magic-angle amplitude
...
> **Sign convention (gauge, not physics).** The bare *sign* of $\Gamma$ is not a substrate observable — Axiom 3 minimizes $|\Gamma|^2$, never $\Gamma$. The same total-reflection wall reads $\Gamma_V = +1$ (open / voltage antinode) from the vacuum *outside* and $\Gamma = -1$ (short / displacement node) from *inside* the trapped region, related by the reflection-sphere Möbius map $Z \leftrightarrow 1/Z$ ($\Gamma \to -\Gamma$). Physical, gauge-invariant content: $|\Gamma| = 1$ and $Z_{\text{eff}} \to \infty$ (measured — observable-battery 2026-06-06; proven — the Smith chart is the $SL(2,\mathbb{C})$ spinor action). The legacy "$\Gamma = -1$" label throughout the corpus denotes this wall in the inside (displacement) convention. *(Scope-note 2026-08-18, Wave-2 D8: "proven" here = the Möbius/SL(2,ℂ) identity — textbook mathematics, adjudicated consistency-class at `research/2026-06-06_biquaternion-node-algebra-result.md` §5.3; it passes no G-gate and asserts no substrate-physics status beyond the adjudicated echo ceiling.)*
...
> **🔴 MASS-SECTOR SCOPE-NOTE — $\mathcal{M}$ EE-projection is the A1 DILATATION, not the inductance flywheel (2026-06-20, Rule 12 — table above PRESERVED unedited; Grant-ratified mass-sector ruling).** The §4.3 invariants table maps $\mathcal{M}$ (rest mass $mc^2$) to the EE projection "inductance $L$ / inertia." **Re-scope (consistency-class, not a new chord):** the rest-mass *store* is the **A1 longitudinal DILATATION** — the bowling-ball trampoline *depression* itself, the standing bulk-reactance wave's **mutual couple to the bulk lattice** ($K$ = the inter-node bulk modulus; $Z_{bulk}\to0 \Rightarrow \Gamma_{bulk}=-1$ at the cage; canonical at [`lattice-extreme-bh-rationality.md`](../vol3/cosmology/ch15-black-hole-orbitals/lattice-extreme-bh-rationality.md):28, [`master-equation.md`](../vol1/dynamics/ch4-continuum-electrodynamics/master-equation.md):20 "A1 dilatation-MASS"). The "**inductance $L$**" naming belongs to the **FLYWHEEL = the T2 / Cosserat micro-rotation ($\omega$) frequency-regulation (spin) sector** — the local frequency regulator / Park-dq field-oriented-control rotating frame — which is **NOT** the rest-mass store. *(⚠ 2026-06-20 CORRECTION: the flywheel clock is the **spin/Larmor** clock — NOT the gravitational time-dilation clock, which is the Op14 A1-saturation clock $\omega_{local}=\omega\sqrt{1-A^2}$, set by the A1 operating point not the flywheel; two distinct clocks, see [`master-equation.md`](../vol1/dynamics/ch4-continuum-electrodynamics/master-equation.md):20 "TWO DISTINCT CLOCKS" note.)* **A1 ⊥ T2** is the GRADE orthogonality that holds mass (A1) and spin/charge (T2) at $90°$ so they share no $(V_{inc},V_{ref})$ phasor — the mechanism behind the two-"3"s no-double-count guard. *(⚠ 2026-06-20 CORRECTION: the canonical FOC homes [`helium-symmetric-cavity.md`](../vol2/quantum-orbitals/ch07-quantum-mechanics/helium-symmetric-cavity.md):52-56, [`de-broglie-standing-wave.md`](../vol2/quantum-orbitals/ch07-quantum-mechanics/de-broglie-standing-wave.md):13 are SPATIAL inter-object 90°; FOC does NOT FORCE A1 ⊥ T2 — the temporal-within-tank "FOC d-q" reading is RETRACTED, `clm-533gvm` solidity 0.30. A1 ⊥ T2 stands FOC-INDEPENDENT on the grade decomposition. See the REFUTED flag at [`master-equation.md`](../vol1/dynamics/ch4-continuum-electrodynamics/master-equation.md):20.)* The depictable bowling-ball *depression* in this doc IS the A1 mass-cage and stays correct; only the "$L$/inductance" label on the $\mathcal{M}$ row is re-scoped to the flywheel sector. Lepton tower: more Cosserat torsion → faster flywheel → higher regulated frequency → (Compton $f = mc^2/\hbar$) deeper A1 depression → more mass. Body preserved per Rule-12.
...
| Scale | Node count $\ell/\ell_{\text{node}}$ | Boundary | Internal solitons | Observable $\mathcal{M}$ | Status |
|---|---|---|---|---|---|
| Electron ($\ell \sim 10^{-13}$ m) | $\approx 1$ (by construction — electron scale $\sim \ell_{\text{node}}$; calibration anchor) | Horn-torus tube wall (TIR at saturation) | Unknot single soliton | $m_e c^2 = T_{EM} \ell_{\text{node}}$ | canonical (Vol 2 Ch 1) |
...
🔴 *[node-count column added 2026-07-13 (KEEP-BOTH — the $\ell \sim \ldots$ meters column is preserved unedited): the new "Node count $\ell/\ell_{\text{node}}$" column tabulates the dimensionless scale ratio. Electron $\approx 1$ (by construction) and cosmic $\approx 3.456\times10^{38}$ ($=R_H/\ell_{\text{node}}$) are identities/calibration; nucleus $\approx 2.6\times10^{-3}$ is the sub-node ratio. The atom $1/\alpha \approx 137$ carries **Q-G43 OPEN** (the boundary-identification is open, not the ratio), and solar/BH are **Q-G44 OPEN** / TAG-missing. **Open rows are NOT presented as settled.**]*
...
| Q-G19α g-2 closure: $+4.0\%$ parameter-free (Route B forward), printed-precision postulate-conditional ($n_q$-additivity) **[ppm label struck 2026-08-03 per Grant ruling; no ppm number substituted]** | Vol 2 Ch 6 §6.2 substrate derivation | dark-resonance × kernel-asymmetry analytical-correlation closure; symmetric Route B forward is $C_2 = -0.3416$ ($+4.0\%$), saliency $\delta = -3\alpha/2$ reaches printed precision only with the $n_q$-additivity postulate **[ppm label struck 2026-08-03 per Grant ruling; no ppm number substituted]** | Vol 2 Ch 6 §6.2 |
...
|---|---|---|
| $\alpha = 1/(4\pi^3 + \pi^2 + \pi)$ | Q-G19α g-2 closure is **$+4.0\%$ off PDG parameter-free** (symmetric Route B forward, no postulate/no fit); reaches **printed-precision agreement only conditional on the $n_q$-additivity postulate** **[ppm label struck 2026-08-03 per Grant ruling; no ppm number substituted]** | Parameter-free result derived; the Stage-2 headline is postulate-gated and carries no defensible ppm label (struck 2026-08-03) |
...
**The framework's claim** (per §5.6 + §1.3.7; B2 re-scope 2026-06-14): $\alpha$, $G$, and $\mathcal{J}_{\text{cosmic}}$ are three observational windows on the same $u_0^*$ — and thus on the same cosmological initial-data parameter $\Omega_{\text{freeze}}$. **$\alpha$ and $G$ FIX $u_0^*$** (fit inputs — $u_0^*$ is back-solved from CODATA $\alpha$, $G$ — so their mutual agreement is guaranteed by construction, NOT a test); **$\mathcal{J}_{\text{cosmic}}$, measured independently of $\alpha$, $G$, is the one genuinely independent TEST** of whether it lands on the same $u_0^*$ (pass = chord, fail = echo). <!-- 🔴 Rule-12 2026-06-19 B2: prior wording "α, G, and J_cosmic all derive from the same u0* ... Any consistent AVE derivation MUST give the same u0* from THREE independent observational routes. Pairwise inconsistency between any two routes falsifies the single-parameter cosmological-anchoring claim." superseded — α, G are fit inputs (not independent), so their agreement is by construction; J_cosmic is the one independent test. Per the 2026-06-14 B2 re-scope. -->

**What the gravitational hierarchy reveals.** $\alpha/\alpha_G \approx 4 \times 10^{42}$ has no explanation in standard physics — it's a brute fact. AVE's framework structurally predicts this ratio comes from the cosmic-to-lattice scale ratio $R_H / \ell_{\text{node}} \sim 10^{39}$ (precisely $\approx 3.456\times10^{38}$) via the Machian impedance integral. **No fine-tuning required**: gravity is weak because the cosmic boundary is far away.

**Three-route consistency check as the central falsification test (B2 re-scope 2026-06-14).** AVE commits to one number ($u_0^*$ at the magic-angle operating point) that $\alpha$ and $G$ **fix** (fit inputs, agree by construction) and that the **independent** $\mathcal{J}_{\text{cosmic}}$ route **tests** (pass = chord, fail = echo). This is the framework's sharpest empirical claim — and the cleanest falsification test it currently admits. <!-- 🔴 Rule-12 2026-06-19 B2: prior wording "derivable from three independent observational paths" superseded — α, G are fit inputs (not independent); J_cosmic is the one independent test. Per the 2026-06-14 B2 re-scope. -->
...
- **α + G + cosmic $\mathcal{J}$ joint cosmological anchoring** (Grant adjudication 2026-05-15, A-030 + A-031; B2 re-scope 2026-06-14): α and G **fix** the $\Omega_{\text{freeze}}$ operating point $u_0^*$ (fit inputs, agree by construction), cosmic $\mathcal{J}_{\text{cosmic}}$ independently **tests** it (pass = chord, fail = echo) <!-- 🔴 Rule-12 2026-06-19 B2: prior "all three derive from single Ω_freeze" superseded; α, G are fit inputs, J_cosmic is the one independent test. -->
...
|---|---|---|
| α + G + $\mathcal{J}_{\text{cosmic}}$ quantitative three-route closure | partial: α route closed via Path C FTG-EMT (Q-G47 canonical 2026-05-16); G route corpus-canonical (Vol 3 Ch 1 Machian); 𝒥 still cosmic-formation-dependent per A-031 cosmic-parameter horizon | A-030, A-031 |
