...
- $\delta_{strain} \approx 2.225\times 10^{-6}$ — **status: structure predicted (existence + sign of α thermal running below cold-lattice asymptote); magnitude at $T_{CMB}$ is one currently-fitted scalar** (back-subtracted from CODATA, pending substrate-physics derivation gated on **Q-DELTA-MAP-1** substrate-saturation-class adjudication for low-amplitude electromagnetic thermal-bath loading; see [`research/2026-05-28_phase-3-a3-delta-strain-machian-projection-result.md`](../../../research/2026-05-28_phase-3-a3-delta-strain-machian-projection-result.md) §3 for the 2026-05-27 Phase 3-A3 WALK-BACK + candidate substrate-mechanism paths P1/P2/P3). Same predicted/fitted pattern as Vol 6's R per nucleus. See [Vol 1 Ch 8](../vol1/ch8-alpha-golden-torus.md) disclosure.
...
- **$\alpha$ closure (Vol 1 Ch 8 Golden Torus):** $\alpha^{-1}_{ideal} = 4\pi^3 + \pi^2 + \pi$ at the cold-lattice asymptote, from the trefoil's three-regime decomposition. Acyclic if the Golden Torus geometry is established independently of $\alpha$'s measured value (the three regimes are individually grounded in the chapter; this can be checked separately). The thermal running $\alpha^{-1}(T) < \alpha^{-1}_{ideal}$ at $T > 0$ is a structural prediction (sign and existence). *Magnitude-derivation gap:* $\delta_{strain}$ at $T_{CMB}$ is currently back-subtracted from CODATA, definitional given the engine's `DELTA_STRAIN = 1 - (1/ALPHA)/ALPHA_COLD_INV`. The cold-lattice $\alpha$ closure itself is acyclic; the magnitude of the thermal correction is fitted (one scalar) pending substrate-physics derivation gated on **Q-DELTA-MAP-1** substrate-saturation-class adjudication (see [`research/2026-05-28_phase-3-a3-delta-strain-machian-projection-result.md`](../../../research/2026-05-28_phase-3-a3-delta-strain-machian-projection-result.md) §3). Same predicted/fitted pattern as Vol 6 (predicted: structure; fitted: one scalar) — different physics, same disclosure shape.
...
The forward DAG is acyclic. Of the three Layer-8 back-edges, the $G$ closure is acyclic conditional on the other two; the $m_e$ closure depends on whether "smallest stable soliton" is well-defined without circular reference to $m_e$; the $\alpha$ cold-lattice closure ($\alpha^{-1}_{ideal} = 4\pi^3+\pi^2+\pi$) is acyclic by inspection. The thermal-running magnitude $\delta_{strain}$ at $T_{CMB}$ is currently a fitted scalar (one scalar bridging cold-lattice α to CODATA; same predicted/fitted pattern as Vol 6's R per nucleus). Deriving $\delta_{strain}$ from substrate primitives — via **Q-DELTA-MAP-1** substrate-saturation-class adjudication for low-amplitude electromagnetic thermal-bath loading + magnitude derivation within the chosen class — would upgrade the $\alpha$ closure from "structure predicted, magnitude fit" to fully zero-parameter. That substrate-physics derivation is the principal outstanding rigour gap, not a calculational error elsewhere in the chain. See [`research/2026-05-28_phase-3-a3-delta-strain-machian-projection-result.md`](../../../research/2026-05-28_phase-3-a3-delta-strain-machian-projection-result.md) §3 for the 2026-05-27 Phase 3-A3 WALK-BACK + candidate substrate-mechanism paths.
...
|---|---|---|---|
| **δ_strain magnitude at T_CMB** | One fitted scalar (≈ 2.225×10⁻⁶) currently back-subtracted from CODATA. Structure predicted (existence + sign of substrate spatial-metric response to finite-$T$ photon-bath loading); magnitude pending. | Upgrades the α closure from "structure predicted, magnitude fit" to absolutely zero-parameter. Requires **Q-DELTA-MAP-1** substrate-saturation-class adjudication for low-amplitude electromagnetic thermal-bath loading (CMB at 2.725 K), then magnitude derivation within the chosen class — substrate-mechanism-direction-setting first, derivation second. The 2026-05-27 Phase 3-A3 WALK-BACK established that the prior "G_vac + equipartition" SM-vocabulary framing leaks classical thermal-expansion + heat-capacity equipartition mechanism onto a substrate-physics problem; substrate-native framing requires Q-DELTA-MAP-1 saturation-class adjudication first. | [Vol 1 Ch 8](../vol1/ch8-alpha-golden-torus.md) §Status disclosure; this document §Back-edges; [`research/2026-05-28_phase-3-a3-delta-strain-machian-projection-result.md`](../../../research/2026-05-28_phase-3-a3-delta-strain-machian-projection-result.md) §3 |
