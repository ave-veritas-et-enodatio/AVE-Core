...
> **⚠️ Gating note (2026-05-28).** AVE's zero-parameter closure is **contingent on one open formal step**: that ropelength-minimality on K4 uniquely selects the canonical Clifford-torus embedding $r_1 = r_2 = 1/\sqrt{2}$ fixing $R \cdot r = 1/4$. The spin-½ half-cover itself is treated as substrate-derived via the K4 → 2T ⊂ SU(2) → SO(3) chain (see [`vol1/ch8-alpha-golden-torus.md`](../vol1/ch8-alpha-golden-torus.md) steps 1–4); the unproven piece is embedding-selection, not the half-cover postulate in isolation. Until embedding-selection is closed from the classical axioms alone, $\alpha$ is operationally the one calibration input and $4\pi^3 + \pi^2 + \pi$ is a Class-4 consistency check. **All "zero free parameters" / "structurally zero-parameter" / "absolutely zero-parameter" framing below — including the Layer-8 back-edges §, Outstanding Rigour Gaps table, and the A-034 empirical-anchors §"Methodological significance" — should be read as gated on this item.** The δ_strain magnitude derivation (Q-DELTA-MAP-1-quant) is a *separate* outstanding rigour gap from the embedding-selection step; both must close to lift the framework from "structurally zero-parameter, conditional" to "absolutely zero-parameter". Canonical anchor: [`vol1/ch8-alpha-golden-torus.md`](../vol1/ch8-alpha-golden-torus.md) §"Remaining open formal-rigor sub-item".
