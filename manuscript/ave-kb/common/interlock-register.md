...
- **definition_uniqueness (criterion 1):** UNIQUE-FORM **with two flagged duplications**. The one universal tractable FORM is `α⁻¹ = 4π³ + π² + π` (= `Λ_vol + Λ_surf + Λ_line`; `vol1/ch8-alpha-golden-torus.md:11`), a unique dimensionless skeleton. **FLAG 1 — `def-vyvsn1` grade-attribution fork (BLOCKING, flag-don't-fix):** the α-keyed yield threshold `V_yield = √α · V_snap` (`eq_calibration_constants.tex:63-67`; `constants.py:462`) has an UNRESOLVED sector-attribution — `nonlinear-vacuum-capacitance.md:14,16` puts the V_yield varactor on the LONGITUDINAL-A1 bond compliance, while `constants.py` + `pair-production-axiom-derivation.md:100` (#416) put it on the TRANSVERSE Cosserat-T2 dielectric. Opposite grade-assignments of the same threshold; Grant-gated, `def-vyvsn1` status PROPOSED ([`vocabulary-register.md:660-670`](vocabulary-register.md)). This fork does NOT change α's ECHO class (the √α is the imported α-echo either way per `research/2026-06-24_forka-alpha-flip.md:75`) but it muddies the single-definition claim until adjudicated. **FLAG 2 — definition-display dup:** `eq_calibration_constants.tex` is `\input` TWICE in the foreword (`frontmatter/00_foreword.tex:50` and `:61`), so the α definition-display (`eq_calibration_constants.tex:51-55`) renders doubly — a display duplication, not a second derivation.
- **routes_status (criterion 2):** **OPEN — 3+ named routes closed-negative, flip-condition LIVE (NOT exhausted).** Per the Grant-RATIFIED closed≠exhausted rule: the named lift-routes to force the α *value* are each REFUTED, but the route-space is NOT provably empty.
