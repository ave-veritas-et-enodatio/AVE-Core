...
- **Test type:** existing-data re-analysis (SPARC catalog public; 175+ galaxies); current hard-coded-5-galaxy fit is partial-PASS.
- **Substrate:** [`simulate_galactic_rotation_curve.py`](../../../src/scripts/vol_3_macroscopic/simulate_galactic_rotation_curve.py) exists and uses AVE-distinct physics. Engine: [`ave.regime_3_saturated.galactic_rotation`](../../../src/ave/regime_3_saturated/galactic_rotation.py) + [`ave.gravity.galactic_mond_drag`](../../../src/ave/gravity/galactic_mond_drag.py). **GAP:** no SPARC catalog ingestion; 5-galaxy table hard-coded.
...
### C13b. Bullet cluster lensing as Transverse-Tensor acoustic shockwave on Gordon optical metric

- **AVE predicts:** bullet cluster's lensing-baryon spatial offset (matter and lensing separated post-collision) emerges from TT-tensor strain wave $h_\perp$ propagating on the Gordon optical metric and decoupling from baryons. The TT shockwave passes through baryonic matter and creates the lensing signature at the original collision site, while baryons continue with momentum. **Qualitative-only — no offset distance numeric derived.**
- **Standard predicts:** DM halo passes through baryons during collision; lensing tracks the halo, baryons continue with momentum. Both AVE and WIMP DM correctly predict the spatial offset qualitatively.
- **Discriminator:** quantitative offset distance + temporal evolution. Currently AVE corpus is qualitative-only.
- **Test type:** existing-data corroborative (qualitative); needs derivation for quantitative discrimination.
- **Substrate:** [`simulate_bullet_cluster_fdtd.py`](../../../src/scripts/vol_3_macroscopic/simulate_bullet_cluster_fdtd.py) exists; uses $a_0$ + saturation kernel + 1D FDTD; shows $g_{eff} - g_N$ wake but **no offset distance magnitude derivation**.
- **KB anchors:** [`../vol1/dynamics/ch4-continuum-electrodynamics/bullet-cluster.md`](../vol1/dynamics/ch4-continuum-electrodynamics/bullet-cluster.md).

### C13c. META row — three DM-mechanism framings coexist without formal unification

- **AVE corpus state:** three distinct framings invoke three different operators, all called "AVE DM":
  - **(i)** $\eta_{eff}$ unsaturated-lattice drag (galactic rotation, Vol 3 Ch 5) — drives C13a observable
  - **(ii)** TT-tensor acoustic shockwave $h_\perp$ on Gordon optical metric (bullet cluster, Vol 1 Ch 4) — drives C13b observable
...
- **Open theoretical work:** no formal proof of equivalence under Cosserat substrate (Ax 1). Under careful derivation, mechanisms (i), (ii), and operationally-related saturation derivations ought to be unifiable as different limits of the same K4 micropolar physics, but the corpus does not show this explicitly. Closure would unify the three DM observables (galactic rotation, bullet-cluster lensing, DAMA modulation) under one substrate derivation.
- **Discriminator:** META — no single observable, but closure (or failure to close) propagates to all DM-class predictions. NULL = framework retains three coexisting framings without unification; PASS = single substrate derivation produces all three observables.
...
| C13a-GAL-ROT | Galactic rotation curves via $a_0 + \eta_{eff}$ saturation kernel (5-galaxy hard-coded catalog, ~17% offset) | C / existing-data | I↔III | $a_0 = c H_\infty / (2\pi) \approx 1.07 \times 10^{-10}$ m/s² + $g_{eff} = g_N + \sqrt{g_N a_0}\sqrt{1 - g_N/a_0}$ + saturation kernel $\eta_{eff}$ — Unruh-Hawking hoop stress derivation in [`mond-hoop-stress.md:25-31`](../vol1/dynamics/ch4-continuum-electrodynamics/mond-hoop-stress.md); Tully-Fisher $v^4 = GMa_0$ as deep-MOND limit | Ax1+Ax4 (**M**) — Vol 1 Ch 4 hoop-stress derivation dies if $a_0$ wrong; Ax1 K4 lattice survives with revised drag mechanism | Cascade: C13b (bullet cluster shares $a_0$); C14-DAMA (different operator but same DM-substrate-mechanism family); C13c (three-mechanism unification gap). Currently partial-PASS at hard-coded-5-galaxy fit; needs real SPARC catalog ingestion. | U-D vs WIMP particulate DM; S-D vs MOND empirical-only | ~17% offset on 5-galaxy fits per [`multi-galaxy-validation.md:10-16`](../vol3/cosmology/ch05-dark-sector/multi-galaxy-validation.md); $a_0$ derived value 10.7% off empirical $1.2 \times 10^{-10}$ m/s² | [`../vol1/dynamics/ch4-continuum-electrodynamics/mond-hoop-stress.md`](../vol1/dynamics/ch4-continuum-electrodynamics/mond-hoop-stress.md); [`../vol3/cosmology/ch05-dark-sector/multi-galaxy-validation.md`](../vol3/cosmology/ch05-dark-sector/multi-galaxy-validation.md); driver: [`src/scripts/vol_3_macroscopic/simulate_galactic_rotation_curve.py`](../../../src/scripts/vol_3_macroscopic/simulate_galactic_rotation_curve.py) |
| C13b-BULLET | Bullet cluster lensing as Transverse-Tensor acoustic shockwave on Gordon optical metric | C / existing-data | I↔II | TT-tensor strain wave $h_\perp$ on Gordon optical metric decouples from baryons → lensing-baryon spatial offset per [`bullet-cluster.md:8-12`](../vol1/dynamics/ch4-continuum-electrodynamics/bullet-cluster.md). Qualitative-only — no offset distance numeric derived | Ax1 (**C**) — Vol 1 Ch 4 bullet-cluster mechanism dies; Ax1 + TT-wave physics survives with different mechanism | Cascade: C13a (shares $a_0$ in FDTD driver); C13c (three-mechanism unification gap). NULL = bullet-cluster offset emergent only at qualitative level; quantitative offset distance needs derivation. | U-C qualitative; S-D quantitative pending derivation | **TBD pin offset distance** (KB leaf is qualitative; FDTD driver shows $g_{eff} - g_N$ wake but no offset magnitude derivation) | [`../vol1/dynamics/ch4-continuum-electrodynamics/bullet-cluster.md`](../vol1/dynamics/ch4-continuum-electrodynamics/bullet-cluster.md); driver: [`src/scripts/vol_3_macroscopic/simulate_bullet_cluster_fdtd.py`](../../../src/scripts/vol_3_macroscopic/simulate_bullet_cluster_fdtd.py) |
| C13c-DM-MECHANISM-UNIFY | META: three DM-mechanism framings coexist without formal unification | META / theoretical-gap | META | Three framings invoke three different operators: (i) $\eta_{eff}$ unsaturated-lattice drag (galactic rotation, Vol 3 Ch 5); (ii) TT-tensor acoustic shockwave $h_\perp$ (bullet cluster, Vol 1 Ch 4); (iii) $377\Omega$ impedance stretching (Vol 3 Ch 5 §192-200, AUDIT-RETIRED 2026-05-16 because contradicts $Z_0$-invariance theorem). All claimed as "AVE DM" but no formal proof of equivalence under Cosserat substrate (Ax 1) exists in the corpus. | All four axioms (**N**) — corpus-coherence gap; no single-shot kill but framework coherence stakes if mechanisms diverge under careful derivation | Cascade: C13a (uses operator i); C13b (uses operator ii); C14-DAMA (separate operator). NULL = open theoretical work; closure would unify three DM observables under one substrate derivation. | U-C (theoretical-gap row; corroborative-cumulative) | n/a — META row tracking corpus-coherence gap | [`../vol3/cosmology/ch05-dark-sector/index.md`](../vol3/cosmology/ch05-dark-sector/index.md); [`../vol1/dynamics/ch4-continuum-electrodynamics/index.md`](../vol1/dynamics/ch4-continuum-electrodynamics/index.md); cohesive narrative pending — see [closure-roadmap.md §0.5](closure-roadmap.md) DM-mechanism-unification open item |
| C14-DAMA-MATERIAL | DAMA refresh-rate interferometer (NaI vs Sapphire vs Ge vs liquid Xe) | C / both | I | **Refresh-rate working hypothesis (Grant 2026-05-17 evening):** DAMA is a high-Q acoustic interferometer measuring Earth's local refresh-rate modulation in the K4 discrete substrate. K4 lattice refresh rate $\sim v_{wind}/\ell_{node} \approx 9.6 \times 10^{17}$ Hz per unit area (Earth's frame, 370 km/s through CMB-rest substrate); annual orbital ±15 km/s modulates by ~4%; coherent crystal lattice acts as long-baseline interferometer for the discrete refresh signal. Solids (NaI: G ≈ 15 GPa) sustain coherent phase-lock → detect signal; liquid Xe (G = 0) has no coherent baseline → no detection (matches XENONnT null). Energy scale (keV) from coherent multiplication of single-node-encounter momenta (~2 eV/encounter × ~10³ atomic-pair coherent baseline → ~keV recoils). Q1+Q2 of [`research/2026-05-17_C14-DAMA_amplitude_prereg.md`](../../../research/2026-05-17_C14-DAMA_amplitude_prereg.md) resolved; proportionality constant + crystal-quality dependence are the remaining derivation gaps. | Ax1+Ax4 (**C**) — Vol III dark-sector chapter dies; WIMP particulate resurrects | Cascade: C13c META (three-mechanism unification — refresh-rate framing now provides the κ-coupling-limb explicitly); D4-A034 (Ax4 kernel). NULL = swapped-crystal amplitude scales with WIMP cross-section. Refresh-rate framing testable: COSINE/DAMA tension is now a CRYSTAL-QUALITY question (interferometric Q at substrate refresh rate ~10¹⁷ Hz). | U-D pending amplitude proportionality derivation; refresh-rate mechanism load-bearing | NaI / Sapphire / Ge densities cited; **proportionality constant TBD** (refresh-rate working hypothesis gives substrate-native scaling $\nu_{kin} \times \kappa_{crystal} \times \Delta v_{wind}$; dimensional bridge to cpd/kg/keV is the 1-2 session derivation gap) | [`../vol3/cosmology/ch05-dark-sector/multi-galaxy-validation.md` lines 30-34](../vol3/cosmology/ch05-dark-sector/multi-galaxy-validation.md); refresh-rate working hypothesis at [`../../../research/2026-05-17_C14-DAMA_amplitude_prereg.md`](../../../research/2026-05-17_C14-DAMA_amplitude_prereg.md); driver: [`src/scripts/vol_3_macroscopic/vlbi_impedance_parallax.py`](../../../src/scripts/vol_3_macroscopic/vlbi_impedance_parallax.py) (computes $\kappa$ ratios; rewrite to refresh-rate framing pending) |
...
| Regime | Count | Tests |
|---|---|---|
| **I** (linear sub-yield) | 16 | A1, A2, B4, C3, C6, C7, C10, C11, C12, C14, C15, C17, C18, C19, D1, D5 |
