...
| C14-DAMA-MATERIAL | DAMA Parallax & Crystal Phonon Modulation (NaI vs Sapphire vs Ge $\kappa_{crystal}$) | C / both | I | DM wind modulates DAMA via bulk dielectric $\kappa_{crystal}$ coupling. **Note (2026-05-16 audit):** crystal-density ratios $\kappa = \rho_{crystal}/\rho_{bulk}$ are computable but the **amplitude formula itself is NOT derived in the corpus** (TBD-pin). Falls into the same derivation-gap class as the retired Jupiter-VLBI claim — descriptive-only, no observable-amplitude derivation. Per the 2026-05-16 DM cohesive narrative (C13c META row), this is the DAMA-mechanism limb of the three-mechanism unification open item. | Ax1+Ax4 (**C**) — Vol III dark-sector chapter dies; WIMP particulate resurrects | Cascade: now in C13-family (C13c-DM-MECHANISM-UNIFY META row tracks DAMA + galactic rotation + bullet cluster as three-mechanism coexistence); D4-A034 (Ax4 kernel). NULL = swapped-crystal amplitude scales with WIMP cross-section. | U-D pending amplitude derivation | NaI / Sapphire / Ge densities cited; **no explicit amplitude formula — TBD pin** | [`../vol3/cosmology/ch05-dark-sector/multi-galaxy-validation.md` lines 30-34](../vol3/cosmology/ch05-dark-sector/multi-galaxy-validation.md); driver: [`src/scripts/vol_3_macroscopic/vlbi_impedance_parallax.py`](../../../src/scripts/vol_3_macroscopic/vlbi_impedance_parallax.py) (computes $\kappa$ ratios but no amplitude) |
