...
| C3-MUON-DELTA | **WALK-BACK 2026-05-18 late evening** — Q-G27 factor-2 conversion error corrected; AVE forward prediction $+502 \times 10^{-11}$ in 4.6σ tension with Fermilab Run-3 on e+e- baseline; saliency mechanism preserved (substrate $\sqrt{3/7}$ shared with $m_\mu$ at 1.24%) | complete (mechanism preserved; arithmetic corrected) | driver built [`src/scripts/verify/muon_g2_fermilab_anchor.py`](../../../src/scripts/verify/muon_g2_fermilab_anchor.py) (commit 06e42f4); Outcome B confirmed → walk-back executed | partial-PASS at mechanism level (Cosserat-saliency forward-computes $+502 \times 10^{-11}$); 4.6σ tension with Fermilab Run-3 on e+e- baseline (BMW-baseline-conditional); Run-4/5 at ±10 ppm + BMW-vs-e+e- adjudication needed for true discrimination | open (forward prediction documented; discrimination awaits Fermilab Run-4/5 ~2026-2027 + BMW-vs-e+e- SM-baseline adjudication) |
...
| C3-MUON-DELTA | driver built; walk-back executed | free (Fermilab data public) | **WALK-BACK 2026-05-18 late evening** — driver [`src/scripts/verify/muon_g2_fermilab_anchor.py`](../../../src/scripts/verify/muon_g2_fermilab_anchor.py) (commit 06e42f4) surfaced factor-2 conversion error in corpus Q-G27; corrected forward prediction $\Delta a_\mu^{(2)} = \Delta C_2 \cdot (\alpha/\pi)^2 = +502 \times 10^{-11}$ with $\delta_\mu = -3\alpha/2 - \alpha\sqrt{3/7}/(2\pi) = -0.01171$; saliency mechanism preserved (forced by lepton-mass derivation sharing $\sqrt{3/7}$ at 1.24%) | **Fermilab Muon g-2 Run-1+Run-2+Run-3** — Phys. Rev. Lett. 131:161802 (2023) for Run-1+Run-2 baseline ($a_\mu = 0.00116592055(24)$); Run-3 tension +245(56)×10⁻¹¹ for AVE comparison vs SM | Y in tension direction at current Run-3 precision (AVE forward $+502 \times 10^{-11}$ vs +245(56) is 4.6σ above on e+e- baseline; BMW lattice baseline would put AVE in deeper tension); Run-4/5 needed for sub-1% true discrimination | (i) BMW lattice vs e+e- SM-baseline dispute (AVE forward prediction is in tension with BOTH baselines at current precision); (ii) Run-4/5 timeline ~2026-2027 expected; (iii) Q-G19α n_q-additive postulate downstream — full Route B engine implementation `verify/electron_g2_petermann.py` deferred (multi-session) for independent verification | Honest forward prediction $+502 \times 10^{-11}$ documented across leaf + matrix + closure-roadmap; await Fermilab Run-4/5 release; await BMW-vs-e+e- adjudication; if AVE-vs-Fermilab gap persists at Run-4/5 precision, audit n_q-additivity postulate (Q-G19α) as next candidate mechanism walk-back | 2026-05-18 late evening (WALK-BACK: factor-2 conversion error corrected; saliency mechanism preserved; tension framing now honest) |
...
flowchart TD
    ...
    ALPHA -->|"δ_μ = -3α/2 - α√(3/7)/(2π) = -0.01171<br/>ΔC_2 × (α/π)² = +502×10⁻¹¹<br/>(Q-G27 Cosserat preserved; factor-2 walked back 2026-05-18 late evening)"| C3["C3-MUON-DELTA<br/>Q-G27 Cosserat saliency canonical<br/>AVE forward Δa_μ⁽²⁾ = +502×10⁻¹¹<br/>vs Fermilab Run-3 +245(56)×10⁻¹¹ → 4.6σ over (e+e- baseline)<br/>BMW-baseline-conditional; Run-4/5 ~2026-2027 for sub-1%"]
