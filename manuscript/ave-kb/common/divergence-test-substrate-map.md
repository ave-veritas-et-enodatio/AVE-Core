...
| C1-BH-RING | BH horizon $r_{sat} = 3.5 r_s$ + ringdown $\omega_R M_g = 18/49$; **Kerr-corrected via (2,3) topology + Cosserat back-reaction (Phase 3 v2, 2026-05-18 PASS)** | C / existing-data | III↔IV | Buchdahl-bound + $\nu_{vac} = 2/7$ Poisson ratio from K4 ($r_{sat} = 7 M_g$; $\omega_R M_g = \ell(1+\nu_{vac})/x_{sat} = 18/49$); **spinning case**: $x_{sat}(a_*) = 2 + 5 \cdot r_{ph}^+(a_*)/3M$ — rigid $\nu_{vac}$ K4 skeleton fraction + compliant $(1-\nu_{vac})$ photon-orbit fraction per (2,3) electron-BH isomorphism | Ax1+Ax4 (**M**) — Ax4 survives with revised $\nu_{vac}$; K4 lattice untouched | **Cascade through ν_vac=2/7:** tight triangulation with C12-G-STAR ($g_*=7^3/4$ uses 7-mode compliance) and C11-MACH-ZEHNDER ($n_s=9/7, n_t=2/7$ ARE the ν_vac numbers); D2-RHO-LAMBDA (via $G = \hbar c/(7\xi m_e^2)$); B5-PONDER-01 (thrust scales with $\nu_{vac}$); D4-A034 turbulence subrow ($n_{3D} = 38/21$). Cascade: D4 A-034 catalog (BH ringdown is canonical row), D3 geometric entropy (uses horizon area). **3-row triangulation on ν_vac NOW EMPIRICALLY CONSISTENT (Phase 3 PASS 2026-05-18)**: C1 -0.45% mean across 3 LIGO events; C11 249.64 rad live-fire; C12 awaits LISA. Single FAIL still = revision needed. | U-D for $\nu_{vac}$; S-D against modified-gravity ringdown theories | **AVE-v2 -0.45% mean, max 2.0% per event vs LIGO (GW150914 -2.0%, GW170104 -1.2%, GW151226 +1.9%)** — Phase 3 PASS at GR-class precision (GR Kerr QNM +0.34% mean per Berti+Cardoso+Will 2006). Cold AVE 0.3673 vs GR 0.3737 (1.7% from GR). v1 simplified formula (superseded) was 10-18% over per event. | [`../vol3/cosmology/ch15-black-hole-orbitals/ave-bh-horizon-area-theorem.md`](../vol3/cosmology/ch15-black-hole-orbitals/ave-bh-horizon-area-theorem.md); refined formula at [`ave-merger-ringdown-eigenvalue.md`](../vol3/cosmology/ch15-black-hole-orbitals/ave-merger-ringdown-eigenvalue.md) (Phase 3 v2 on `analysis/ligo-ringdown-driver` branch commit 4673963) |
...
| C1-BH-RING | partial (Phase 1+2+3 design+execution doc landed 2026-05-18 on `analysis/ligo-ringdown-driver` branch) | complete (driver design + 3-event GW150914/GW170104/GW151226 test geometry) | code-written + data-collected (`src/scripts/vol_3_macroscopic/ligo_ringdown_driver.py` runs v1, v2, GR-QNM Berti reference; LIGO observed values embedded) | **PASS (Phase 3, 2026-05-18)** — v2 refined formula -0.45% mean, max 2.0% per event vs LIGO; matches GR Kerr QNM precision band | Core (LIGO branch) |
...
flowchart TD
    ...
    NU -->|"factor 7 = 1/ν_vac"| C1["C1-BH-RING<br/>r_sat = 7 M_g<br/>ω_R M_g = 18/49 (cold, 1.7% from GR)<br/>Phase 3 v2 -0.45% mean vs LIGO (PASS 2026-05-18)<br/>x_sat = 2 + 5·r_ph+(a*)/3M (Cosserat back-reaction)"]
