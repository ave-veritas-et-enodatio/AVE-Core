...
| C14-DAMA-MATERIAL | DAMA refresh-rate interferometer (NaI vs Sapphire vs Ge vs liquid Xe) | C / both | I | **Energy scale + modulation CLOSED zero-parameter 2026-05-17 evening** ([result doc](../../../research/2026-05-17_C14-DAMA_amplitude_result.md)): $E_{refresh} = h \cdot v_{cmb}/\ell_{node} = h \cdot (370\text{ km/s})/(3.86 \times 10^{-13}\text{ m}) = 3.96$ keV — sits in DAMA 2-6 keV window with **zero tuning** (both $v_{cmb}$ and $\ell_{node}$ canonical). Annual modulation $\Delta E/E = \Delta v_{orbit}/v_{cmb} = 15/370 = 4.05\%$. Substrate refresh-quantum has slow-quasi-particle dispersion $E = p v_{cmb}$ (NOT $E = pc$). **Picture refinement (awaiting Grant plumber check)**: Mössbauer Debye-Waller analysis at room-T rules out substrate-pitch absorption ($f = e^{-1.22 \times 10^4} = 0$) and supports atomic-spacing envelope absorption ($f = 0.996$, recoilless). AM-radio analog: substrate refresh acts as carrier at $\nu = 9.6 \times 10^{17}$ Hz; crystal absorbs modulation envelope at atomic-spacing wavelength. **Rate magnitude TBD (sharpened gap)**: naive rate ~$10^{45}$ events/s/kg vs DAMA empirical ~$10^{-7}$ — requires Q-factor / coherence-bandwidth resonance-line derivation (corpus has no template; closest is Hulse-Taylor pattern adaptation). Refresh-rate working hypothesis from prereg holds: solids (NaI G≈15 GPa) sustain coherent phase-lock → detect; liquid Xe (G=0) → no coherent baseline → no detection (matches XENONnT null). | Ax1+Ax4 (**C**) — Vol III dark-sector chapter dies; WIMP particulate resurrects | Cascade: C13c META (three-mechanism unification — refresh-rate framing now provides the κ-coupling-limb explicitly + energy scale anchors it); D4-A034 (Ax4 kernel). NULL = swapped-crystal amplitude scales with WIMP cross-section. Refresh-rate framing testable: COSINE/DAMA tension is now a CRYSTAL-QUALITY question (interferometric Q at substrate refresh rate ~10¹⁷ Hz). | **Energy + modulation: U-C (forward-prediction CONFIRMED, zero-parameter match to DAMA window + envelope width 2026-05-17)**; rate magnitude: U-D pending Q-factor derivation | NaI / Sapphire / Ge densities cited; **energy scale $h v_{cmb}/\ell_{node} = 3.96$ keV CONFIRMED in DAMA window (zero-parameter); modulation 4.05% CONFIRMED; rate magnitude TBD pending Q-factor / coherence-bandwidth derivation** | [`../vol3/cosmology/ch05-dark-sector/multi-galaxy-validation.md` lines 30-34](../vol3/cosmology/ch05-dark-sector/multi-galaxy-validation.md); refresh-rate working hypothesis at [`../../../research/2026-05-17_C14-DAMA_amplitude_prereg.md`](../../../research/2026-05-17_C14-DAMA_amplitude_prereg.md); **partial-closure result at [`../../../research/2026-05-17_C14-DAMA_amplitude_result.md`](../../../research/2026-05-17_C14-DAMA_amplitude_result.md)**; driver: [`src/scripts/vol_3_macroscopic/vlbi_impedance_parallax.py`](../../../src/scripts/vol_3_macroscopic/vlbi_impedance_parallax.py) (computes $\kappa$ ratios; rewrite to refresh-rate framing pending) |
