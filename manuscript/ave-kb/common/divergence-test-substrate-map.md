...
| C1-BH-RING | BH horizon $r_{sat} = 3.5 r_s$ + ringdown $\omega_R M_g = 18/49$; **Kerr-corrected via (2,3) topology + Cosserat back-reaction (Phase 3 v2 + Phase 4 spin-sweep, 2026-05-18 PASS)** | C / existing-data | III↔IV | Buchdahl-bound + $\nu_{vac} = 2/7$ Poisson ratio from K4 ($r_{sat} = 7 M_g$; $\omega_R M_g = \ell(1+\nu_{vac})/x_{sat} = 18/49$); **spinning case**: $x_{sat}(a_*) = 2 + 5 \cdot r_{ph}^+(a_*)/3M$ — rigid $\nu_{vac}$ K4 skeleton fraction + compliant $(1-\nu_{vac})$ photon-orbit fraction per (2,3) electron-BH isomorphism | Ax1+Ax4 (**M**) — Ax4 survives with revised $\nu_{vac}$; K4 lattice untouched | **Cascade through ν_vac=2/7:** tight triangulation with C12-G-STAR ($g_*=7^3/4$ uses 7-mode compliance) and C11-MACH-ZEHNDER ($n_s=9/7, n_t=2/7$ ARE the ν_vac numbers); D2-RHO-LAMBDA (via $G = \hbar c/(7\xi m_e^2)$); B5-PONDER-01 (thrust scales with $\nu_{vac}$); D4-A034 turbulence subrow ($n_{3D} = 38/21$). **3-row triangulation on ν_vac NOW EMPIRICALLY CONSISTENT** (Phase 3+4 PASS 2026-05-18): C1 covers full LIGO BBH catalog (a*<0.85); C11 249.64 rad live-fire; C12 awaits LISA. Single FAIL still = revision needed. | U-D for $\nu_{vac}$; S-D against modified-gravity ringdown theories | **Phase 3 (3 events): AVE-v2 -0.45% mean, max 2.0% per event vs LIGO (GW150914, GW170104, GW151226). Phase 4 (spin sweep + extended): v2 PASSES at \|dev\| < 3% vs GR Kerr QNM for 9 of 11 swept spin values across a* in [0, 0.85]; mass-independence confirmed at GW190521 (M=142 M⊙, IMBH-class) at -0.25%; GW170729 (a*=0.81) at +1.77%. v2 covers entire currently-detected LIGO BBH catalog (a_final < 0.85). Divergence onset a* ≥ 0.90 (near-extremal regime, currently unattested) — Option B (full spheroidal cavity) deferrable.** v1 simplified formula superseded. | [`../vol3/cosmology/ch15-black-hole-orbitals/ave-bh-horizon-area-theorem.md`](../vol3/cosmology/ch15-black-hole-orbitals/ave-bh-horizon-area-theorem.md); refined formula + spin sweep at [`ave-merger-ringdown-eigenvalue.md`](../vol3/cosmology/ch15-black-hole-orbitals/ave-merger-ringdown-eigenvalue.md) (Phase 3+4 on `analysis/ligo-ringdown-driver` commits 4673963 + c9a8db6) |
...
| C1-BH-RING | partial (Phase 1+2+3+4 design+execution doc landed 2026-05-18 on `analysis/ligo-ringdown-driver` branch) | complete (driver design + 3-event Phase-3 + spin-sweep Phase-4 + extended-event GW170729+GW190521 test geometry) | code-written + data-collected (`src/scripts/vol_3_macroscopic/ligo_ringdown_driver.py` runs v1, v2, GR-QNM Berti reference, Phase-4 spin sweep + IMBH-class events; commits 4673963 + c9a8db6) | **PASS (Phase 3+4, 2026-05-18)** — v2 refined formula -0.45% mean Phase 3; Phase 4 spin sweep PASSES \|dev\| < 3% vs GR Kerr QNM for 9 of 11 swept spin values across a* in [0, 0.85]; covers entire LIGO BBH catalog at GR-class precision | Core (LIGO branch) |
...
| C1-BH-RING | Core (LIGO branch) | free (LIGO public data; no fab) | `src/scripts/vol_3_macroscopic/ligo_ringdown_driver.py` on `analysis/ligo-ringdown-driver` branch (commit 4673963); implements AVE v1 simplified + v2 refined Kerr correction + GR Berti+Cardoso+Will 2006 QNM reference table | LIGO observed values from `ave-merger-ringdown-eigenvalue.md` table + KB-cited LIGO collaboration values for GW150914 (251 Hz), GW170104 (312 Hz), GW151226 (750 Hz) | Y — Phase 3 v2 -0.45% mean, max 2.0% per event matches LIGO at GR-class precision (GR Kerr QNM +0.34% mean) | v2 refined formula validated at moderate spin (0.64-0.74); higher-spin events (GW190521 a*≈0.84) needed to test whether v2 generalizes beyond moderate-spin regime; decay-time (tau) predictions for spinning case not yet refined for v2 mechanism | **PHASE 3 COMPLETE**. Next: (a) cherry-pick driver+KB-anchor updates back to L3 / cross-link in foreword; (b) Phase 4 extend to GW190521 + GW190426 for higher-spin coverage; (c) refine decay-time prediction τ(a*) for v2 cavity geometry | 2026-05-18 |
...
flowchart TD
    ...
    NU -->|"factor 7 = 1/ν_vac"| C1["C1-BH-RING<br/>r_sat = 7 M_g<br/>ω_R M_g = 18/49 (cold, 1.7% from GR)<br/>Phase 3 v2 -0.45% mean vs LIGO<br/>Phase 4 spin sweep PASS a* < 0.85<br/>x_sat = 2 + 5·r_ph+(a*)/3M (Cosserat back-reaction)<br/>covers entire LIGO BBH catalog 2026-05-18"]
