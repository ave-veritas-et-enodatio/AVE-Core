...
- **residual_content:** **the L2-norm / fixed-radius energy constraint.** The shape is a theorem OF this norm; the **norm is the residual axiom**. Two named identifications bridge the forced *dynamical* LC L2 invariant to the Axiom-4 *static* 7-mode RMS argument $A$ ($A^2=\varepsilon^2+\kappa^2+V^2$): **GAP-1** (the saturation ceiling keys on the L2 *energy* norm rather than L∞/Nyquist-peak or L1) and **GAP-2** ($A_{yield}$ is a HARD fixed radius, giving the vertical tangent, not a soft asymptotic cap). Both gaps are the SAME residual posit in two guises — *the reactive amplitude lives on a fixed-radius circle in an L2 (energy) metric.* A natural, EE-native, lossless-reactive posit — but a posit. **The axiom does not vanish; its content SHRINKS and RELOCATES** from "the substrate obeys this specific curve" to "saturation is a fixed-radius L2 constraint on the reactive amplitude." **Content reduction, NOT count reduction: the axiom count stays 4 (NOT 4→3).**
- **derived_by:** the derivation result `research/2026-07-02_axiom4-forced_result.md` (branch `analysis/axiom4-saturation-forced` @ `7170f40e`) — a structural/derivational result about the axiom's internal content (no scored `clm-` node minted; the result is a not-yet-canonized research doc, deliberately per Grant's hold). Prior art: the **Q-G47** gate (`common/q-g47-substrate-scale-cosserat-closure.md`) closed only the OPERATING POINT $u_0^*$ where $S(A^*)=0$ — it took the shape $S(A)$ and the $A^*=1$ boundary as INPUT, it did NOT derive the shape.
...
- **OPEN — the FULL reduction is an orchestration EPIC, not yet run.** Whether $A$ is *definitionally* the dynamical energy-phase (which would make the L2 norm FORCED, not just identified — closing GAP-1 + GAP-2 from substrate structure, promoting CONDITIONALLY-FORCED → FORCED) is the epic's thesis, **unproven here**. Grant's framing: "saturation is both static and dynamic" — the static 7-mode RMS $A$ and the dynamic LC energy-phase $(V_{inc},\Phi_{link})$ are two faces of the same L2 object; proving they ARE the same object is the epic. Until it runs, Axiom 4 is SHAPE-DERIVED **(conditional)** and the residual L2-norm posit stays axiomatic. **Do NOT restate Axiom 4 as a theorem in `axiom-definitions.md` or `eq_axiom_4.tex`** — that is gated on the epic + Grant. The corpus's standing honest self-statements are preserved and consistent: [`trampoline-analogy-primer.md:192`](trampoline-analogy-primer.md) "**Axiom 4 remains postulated**"; [`common/claim-quality.md:805`](claim-quality.md) "the kernel is the **postulated** Axiom 4 form." This SHAPE-DERIVED (conditional) status SHARPENS those (it names *why* they are correct — the norm-choice is the hidden posit — and *how much* reduction is available), it does not overturn them.
