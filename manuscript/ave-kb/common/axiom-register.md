...
- **residual_content:** *(re-pinned 2026-07-02 by the reduction epic + DP-3 — supersedes the earlier "the L2-norm / fixed-radius energy constraint" wording, which was imprecise.)* The L2 norm is **FORCED within each dynamical reactance tank** (Ax 1 tank + Ax 3 lossless — for a 2-state lossless LC tank, peak/energy/action all collapse to the same L2; there is no L∞ or L1 conserved quantity, so the within-tank norm is a theorem, NOT a choice). The genuine residual axiomatic content is therefore **not "the L2 norm"** but: **(i) the CROSS-SECTOR COMBINE RULE** — the combine is **per-yield-normalized** (each sector against its own yield; the *single-radius-L2 total* $A^2=\varepsilon^2+\kappa^2+V^2$ at one shared $V_{\text{SNAP}}^2$ is **FALSIFIED** by the electron (68× off) and the operative engine, which uses per-yield-normalized L2 *within* a grade and separate $S_\mu,S_\varepsilon$ kernels = L∞ *across* grades). Whether the cross-grade aggregation is L∞-max or normalized-L2 is **UNDERDETERMINED** — degenerate to $O(\alpha)$ at the electron's $\sqrt\alpha$ operating point; a non-$\alpha$-suppressed discriminator is the one open piece. **(ii)** the **hard-radius** character (Ax 1 forces a ceiling *exists*, not that it is *hard*/vertical-tangent). **(iii)** the per-sector yield **ratio $\sqrt\alpha$** ($V_{\text{yield}}/V_{\text{snap}}$) — an **imported** calibration α-echo (`def-vyvsn1`, "both CALIBRATION, not derived"), consumed by every candidate rule, derived by none. **Content reduction, NOT count reduction: the axiom count stays 4 (NOT 4→3).**
- **derived_by:** three structural/derivational research results (no scored `clm-` minted; not-yet-canonized research docs per Grant's hold): (1) `research/2026-07-02_axiom4-forced_result.md` (branch `analysis/axiom4-saturation-forced` @ `7170f40e`) — the within-tank L2 forcing + the L^p 1-to-1 map; (2) `research/2026-07-02_axiom4-reduction-epic_result.md` (merged PR #455) — FULL reduction **REFUTED**, the within-tank L2 promoted to airtight, the residual relocated to the cross-sector combine rule; (3) `research/2026-07-02_axiom4-combine-rule_result.md` (branch `analysis/axiom4-combine-rule`, PR #457) — the combine is per-yield-normalized (single-radius-L2 falsified), cross-grade aggregation underdetermined at $O(\alpha)$, $\sqrt\alpha$ imported. Prior art: the **Q-G47** gate (`common/q-g47-substrate-scale-cosserat-closure.md`) closed only the OPERATING POINT $u_0^*$ where $S(A^*)=0$ — it took the shape $S(A)$ and the $A^*=1$ boundary as INPUT, it did NOT derive the shape.
...
- **EPIC RAN (2026-07-02) → FULL reduction REFUTED; the residual is now pinned about as tight as the physics allows.** The full-reduction thesis ("$A$ is *definitionally* the dynamical energy-phase, making the L2 norm forced not identified") was tested and **does not hold as a 4→3 reduction**: (1) the static↔dynamic bridge closes only under a definitional re-reading of $A$ (as the envelope/energy, not the instantaneous snapshot — DP-1, now adopted); (2) the cross-sector combine's single-radius-L2 reading is **falsified** (electron + engine), leaving a per-yield-normalized structure whose cross-grade aggregation (L∞ vs normalized-L2) is **underdetermined** by the electron; (3) the hard-radius character and the $\sqrt\alpha$ yield ratio stay residual/imported. So Axiom 4 stays SHAPE-DERIVED **(conditional)**, count 4. **The one open piece** is a non-$\alpha$-suppressed discriminator for the cross-grade aggregation. **Do NOT restate Axiom 4 as a theorem in `axiom-definitions.md` or `eq_axiom_4.tex`** — the reduction is a content reduction, not a shape derivation; the residual combine-rule posit stays axiomatic. The corpus's standing honest self-statements remain correct and are now *explained*: [`trampoline-analogy-primer.md:192`](trampoline-analogy-primer.md) "**Axiom 4 remains postulated**"; [`common/claim-quality.md:805`](claim-quality.md) "the kernel is the **postulated** Axiom 4 form."
