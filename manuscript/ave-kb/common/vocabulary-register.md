...
- **verification:** VERIFIED the varactor-bias operating-point reading verbatim at [`CLAUDE.md`](../CLAUDE.md):75 and the electron load-line ($V_{snap}\approx511$ kV, $V_{yield}=\sqrt\alpha V_{snap}\approx43.65$ kV) at `cvr-dc-operating-point.md`:43-45. The $R\cdot r=\tfrac14$-sets-the-Q-point **α-free** (the α-flip) is **PENDING/open**, not asserted. **★ GATE DISCHARGED 2026-08-10 (R43): ADOPTED and SOLID** — *"The proposed `quiescent point` def-node PROMOTES to ratified under this naming"*; the prior "GATED on auditor + Grant review — NOT adopted, NOT SOLID" line is superseded and lives in git. **Scope of the promotion:** the NAME and the operating-point reading only. The α-flip stays PENDING/open exactly as below — ratifying the vocabulary does not ratify the α-free claim. *(α-flip STATUS, FORK-A 2026-06-24 — PENDING STANDS: the self-biased **pressure-equilibrium** route returns closed-NEGATIVE → ECHO — fixes a scale not the product $R\cdot r$, α re-enters via the √α ladder + $Z_0\propto\alpha$ area-bridge, relabel of the conservative $H_{couple}$ slosh; [`research/2026-06-24_forka-alpha-flip.md`](../../../research/2026-06-24_forka-alpha-flip.md). SCOPE = that named route ONLY, **NOT** route-exhaustion; the α-flip stays PENDING.)* **⛔ NOT-A-VERIFICATION-RESULT — the block that follows is TAGGED INSIDE this field, not part of it. It is here, and not in a bullet of its own, only because external gate fixtures anchor this register by line number (`research/drivers/iomega_law_number_check.py` cites `:526`), so the repair is kept line-count-neutral; it is NOT part of the verify-before-cite result this field is defined to hold at `:61`.** ⟦BEGIN TAGGED NON-VERIFICATION BLOCK⟧ **gravity-scope rider (2026-08-28, PR #1033) — RESTATEMENT, WALK-assembled, UNAUDITED, NO RULING; the earlier label "Grant-signed picture" is DEMOTED and the finding is kept.** *Receipt, stated exactly:* an assembly of three existing canon legs — this node's own **R43** ratification (the naming ruling quoted at `:500`) × the gravity-domain **input** mapping at [`domain-catalog.md`](../vol1/operators-and-regimes/ch7-regime-map/domain-catalog.md):45-47 (*"Amplitude $A$ | Principal radial strain $\varepsilon_{11} = 7GM/(c^2 r)$"*, *"Control parameter | $r = \varepsilon_{11}$"*), scored `clm-82dxbj` at **solidity 0.63** and used **input-only** (the catalog supplies the operating-point mapping; nothing is read back out of it) × the varactor-bias operating-point clause at [`CLAUDE.md`](../CLAUDE.md):75. **Assembled in a chat walk with Grant 2026-08-28 — agreement recorded, reasoning not; no adversarial pass has run; no docket ruling exists for it.** Under the 2026-09-03 convention (*adversarial pass **and** physical/logical review with Grant*) only the second half's chat agreement exists, so **"signed" is not the right word and is not used.** *Content, unchanged:* gravitational $\varepsilon_{11}$ is read at gravity scope as the DC Q-point of the A1 tank (allowed AC swing / varactor bias), **not** a slide of the graph vertex. Clause Q names the substrate-scope $\varepsilon_{11}=0$ away from defects; a mass is a defect that holds a local $\varepsilon_{11}$ as the cell's Q-point. Does **not** coin "Q-point gravity." Does **not** change the locked meaning or the SOLID status. Home of the constitutive inventory: [`bond-lc-constitutive-grading.md`](../vol3/gravity/ch03-macroscopic-relativity/bond-lc-constitutive-grading.md) (former filename [`hop-lc-constitutive-grading.md`](../vol3/gravity/ch03-macroscopic-relativity/hop-lc-constitutive-grading.md) is a Rule-12 stub). **★ AXIOM-5 NOUN SEAM the rider inherits — its unqualified "this **same noun**" is now qualified.** Axiom 5 carries two clauses with two objects. Clause **G (bias coupling / bridge)**, [`eq_axiom_5.tex`](../../common_equations/eq_axiom_5.tex):73 verbatim — *"The operating-point **bias** $\varepsilon_{11}$ is the bound sector's potential, and the **bound response** $\mathbf{u}_0$ is its gradient"*. Clause **Q (quiescence --- the DC operating point)**, same file `:82`-`:87` — the sourceless substrate at *"$\nabla\cdot\pi = 0$, $\theta = 0$, $\varepsilon_{11} = 0$ ... away from defects"*, i.e. *"the **quiescent reference (Q-point)** that makes the potentials defined and clause G's elliptic solve well-posed"*. The register lists them as **separate canonical nouns** — [`translation-circuit.md`](translation-tables/translation-circuit.md):1107-1108, verbatim: *"Canonical nouns authored here: **the bound response** ($\mathbf{u}_0$), **the bias** ($\varepsilon_{11}$), the **DC operating point / quiescent point (Q-point)**"* (three nouns listed there; two are the pair at issue). **Both sides:** clause G's own wording is *"the **operating-point** bias"*, which is why the fusion is tempting and may be right; but clause Q's Q-point is the **zero**, sourceless reference while clause G's bias is the **nonzero** potential at a defect, so *"gravitational $\varepsilon_{11}$ IS the Q-point"* is a claim about two nouns, not a restatement of one. **UNADJUDICATED — recorded, not resolved.** It is **not** licensed by R43, which ruled the *name* (Q-point vs the "ground" gloss) and said nothing about clause G's bias. ⟦END TAGGED NON-VERIFICATION BLOCK⟧
...
## linear (bond TL $L,C$ vs strain) *(ambiguous — DISAMBIGUATION VIEW, 2026-08-28; three non-interchangeable senses; not a coinage; 2026-08-28 surface form was "hop")*
<!-- id: def-ln3str -->

- **term:** linear (bond TL $L$, $C$ vs strain) — surface form; object is the **bond** (`def-b0nd01`). Write the sense, never bare "linear $L,C$." The 2026-08-28 label "hop" was TLM slang for one graph step along that bond; retired as a live label (collision with hopping unknot / port hop).
- **adjudicated-meaning:** **DISAMBIGUATION VIEW, RESTATEMENT-grade.** The question *"under what strain do bond TL $L$ and $C$ scale linearly?"* has **three non-interchangeable answers already in canon**; this node separates them and adds no physics. **(K) Kernel / Ax4.** $S(r)=\sqrt{1-r^2}$ is even; first correction $O(r^2)$. Regime I ($r<\sqrt{2\alpha}$) drops that correction, so the SYM table freezes $\varepsilon_{\mathrm{eff}}=\varepsilon_0$, $\mu_{\mathrm{eff}}=\mu_0$, $C_{\mathrm{eff}}=C_0$. Solar-system gravity is deep Regime I. Kernel $\Delta S\sim10^{-10}$ at the limb is **not** a derived bond $\delta L/L$. **(P) Photoelastic / Op19.** $n=1+\nu_{\mathrm{vac}}\varepsilon_{11}$ is linear in strain by construction. Under SYM, $\mu$ and $\varepsilon$ co-scale with $n$ (achromatic matching). Bond TL $L,C$ co-scale with $n$ **on the gapless T2/EM channel**, where $L=\mu_0$ and $C=\varepsilon_0$ is a **Class A identity** (`translation-circuit.md`:811-812; `docs/glossary.md`:96); for the **whole three-channel bond** ($Z_{EM}$/$Z_{shear}$/$Z_{bulk}$, `def-gv1net`) no single $(L,C)$ exists to identify, so there the map is Phase-1 and unlicensed. Same weak-field band as (K); **different expansion**. Coefficient $\nu_{\mathrm{vac}}=2/7$ is GR-imported. **(Z) SYM co-scale.** **Same channel condition as (P), stated conditionally for that reason.** On the T2/EM channel $Z=\sqrt{L/C}=Z_0$ is pinned for all gravity-class loading, so $L\propto C$ at every strain until the loading class leaves SYM (definitional for that TL pair). That is linear-in-each-other, not linear-in-strain. For the whole three-channel bond there is no one $L/C$ to pin ($Z_{shear}$, $Z_{bulk}$ are mechanical/acoustic, not in $Z_0$ units — `resonant-lc-solitons.md`:133), so (Z) is **not** unconditional there. **Rule:** never fuse (K) with (P). Photoelasticity $\neq$ the kernel Taylor series. Cosserat / gap shunt is $S_\gamma$ (prereg $S$ in $\Omega^2=S/C$), not the kernel $S(r)$.
...
- **canonical-home:** [`bond-lc-constitutive-grading.md`](../vol3/gravity/ch03-macroscopic-relativity/bond-lc-constitutive-grading.md) §3 (the three-sense assembly) + [`four-regimes.md`](../vol1/operators-and-regimes/ch7-regime-map/four-regimes.md):110-114 (even Taylor) + [`operators.md`](operators.md):59 (Op19) + [`achromatic-impedance-matching.md`](../vol3/gravity/ch03-macroscopic-relativity/achromatic-impedance-matching.md):20-28 (SYM co-scale) + `def-b0nd01` (the bond noun; this register)
...
- **open-ambiguity-flag:** YES — **three registers, KEEP-ALL. This node redefines nothing; it separates:**
  ...
  - **(P) Op19 / photoelastic linear-in-$\varepsilon_{11}$.** `operators.md:59`; `transverse-refractive-index.md:23`; `achromatic-impedance-matching.md:20-28` ($\mu'=n\mu_0$, $\varepsilon'=n\varepsilon_0$ — licensed as a bond-TL $L,C$ map **on the T2/EM channel only**, where `translation-circuit.md:811-812` already grades $L=\mu_0$, $C=\varepsilon_0$ Class A; **not** licensed for the whole three-channel bond). Sharpened: `2026-07-31_anisotropy-observable_scoping.md:893` (*"photoelasticity is linear in strain while the kernel's index shift is quadratic in amplitude — the two are not the same expansion"*); `2026-08-11_gravity-linearity-audit_result.md:24` (*"The Ax4 kernel appears **nowhere** in that chain"*).
  - **(Z) SYM $L\propto C$.** `achromatic-impedance-matching.md:20-28`; INVARIANT-S2 SYM row `translation-circuit.md:117`. Holds at every gravity-class strain, not only small $r$ — **on the T2/EM channel**, under the same channel condition as (P); `def-gv1net`:715-726 and `three-channel-impedances.md`:20-22 are why it does not extend to the whole bond.
...
- **verification:** DISAMBIGUATION VIEW, not a coinage. Coinage-grep of the original node title `linear (hop L,C vs strain)` / `def-ln3str` / `hop-lc-constitutive` at write time: **0 prior hits**. The 2026-08-28-later surface-form rename to "bond TL" reuses SOLID `def-b0nd01` (not a new coinage; Check 1 same-meaning reuse). Bare "linear" is the overload this node exists to split — not grepped as a coinage. Receipts re-read at write: `four-regimes.md:110-114` even Taylor **verbatim**; `regime-equation-sets.md:19` $C_{\mathrm{eff}}=C_0$ in column I; `operators.md:59` Op19 **verbatim**; `achromatic-impedance-matching.md:20-28` $\mu'=n\mu_0$, $\varepsilon'=n\varepsilon_0$; `nonlinear-vacuum-capacitance.md:33-34` quadratic-leading; `domain-catalog.md:50` solar Regime I; anisotropy `:893` and gravity-linearity-audit `:24` as quoted in the flag. `clm-b2anl4`, `clm-rd9cjm`, `clm-07kd5v`, `clm-3zz0f6` each resolve. **Grade honesty:** WALK assembly of canon legs (Grant asked the question 2026-08-28; PR #1033). Does **not** derive the Op19 coefficient, does **not** plant $(2,1,\tfrac12)$, does **not** slide vertices. **★ 2026-08-28 option-1 fence, CHANNEL-SCOPED 2026-09-06:** the fence as it landed at `f943a034` (*bond TL $L,C$ co-scale with $n$ is an unlicensed identification, not a licensed map*) was **right for the whole three-channel bond and wrong by omission for the gapless T2/EM channel the lane prereg §0 names** (`research/2026-08-28_qpoint-lc-constitutive_prereg-FROZEN.md`:19) — on that channel $L=\mu_0$, $C=\varepsilon_0$ is canon's **Class A identity** (`translation-circuit.md`:811-812; `docs/glossary.md`:96). Repaired scope above and at `bond-lc-constitutive-grading.md` §2.1. The `f943a034` flip inverted `d1b55e87`'s equally-unqualified *so hop $L$ and $C$ co-scale with $n$*; it is disclosed by frozen-prereg amendment **A2** (`:222-224`), but that audit has **no charter and no verdict document in the tree** — the greenlight is author-stated. Kernel $\Delta S$ is not bond $\delta L/L$; Cosserat shunt is $S_\gamma$, not kernel $S(r)$. **★ 2026-08-28 vocab:** live noun is the bond (`def-b0nd01`); "hop $L,C$" retired as a live label; `hop-lc-constitutive-grading.md` is a Rule-12 stub; `def-ln3str` id unchanged.
