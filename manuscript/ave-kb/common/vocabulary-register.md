...
- **adjudicated-meaning:** two voltage thresholds on the Axiom-4 saturation kernel, in **two orthogonal sectors** ($A_1 \perp T_2$). **$V_{\text{yield}} = \sqrt{\alpha}\cdot V_{\text{snap}} \approx 43.65$ kV** = the **transverse Cosserat ($T_2$) self-trap wall** (Grant 2026-06-30 ruling): the transverse micro-rotation crosses Axiom-4 onset, $\Gamma\to-1$. **$V_{\text{snap}} = m_e c^2/e \approx 511$ kV** = the **longitudinal $A_1$ sector fully saturates** ($A^2=1$, $Z_{\text{core}}\to0$, the A1 varactor bond compliance $C_{\text{eff}}=C_0/S$ diverges HERE); this is the mass-completion + Schwinger / pair-nucleation energy. **The A1 mass channel does NOT saturate at $V_{\text{yield}}$** — it saturates at the $1/\sqrt\alpha\approx11.7\times$-higher $V_{\text{snap}}$. **RIDER — SEPARATE PROVENANCE, NOT part of the 2026-06-30 ruling (attribution corrected 2026-09-07; the physical claim is unchanged and unweakened).** *The single-electron confining $\Gamma=-1$ TIR cavity self-creates at $V_{\text{yield}}$.* That clause STANDS — but it does not descend from the 2026-06-30 grade-attribution ruling, inside whose stamped sentence it was printed until now. Its actual line of descent is the **canonical 4-property electron definition, Property 3** (`electron-identification.md:32` §1.3, verbatim *"The lattice self-creates a perfect TIR mirror at the wave's location"*, carrying its own 2026-06-24 stamp *"Property 3 STANDS"*), which reached this node through the **#416 / Grant 2026-06-24 sectoral correction** (commit `73a78a82`, where it was written *"per the #416 ruling; the single-electron confining $\Gamma=-1$ TIR wall is here"*). **What the 2026-06-30 ruling actually settled is narrower: which sector's constitutive law is keyed on 43.65 kV.** The Grant-facing question was frozen before any derivation line as a *grade-attribution* fork — `2026-06-30_electron-portmap-derivation_prereg_FROZEN.md:173`, verbatim *"if the inward-leg sign or the brace-presence is genuinely AMBIGUOUS at the symbolic level"* / *"depends on an un-adjudicated grade-attribution of"* — and the routing that raised it names it *"the V_yield sector-attribution fork"* (`2026-06-24_engine-reroute-epic-summary.md:82`). The string "confin" does not occur in that fork clause. **This rider moves an ATTRIBUTION only**: it adjudicates no physics, changes no status, and takes no position on which sector owns the cage — see open-ambiguity-flag item (4).
...
- **status:** SOLID — adjudicated (Grant 2026-06-30). The formerly-BLOCKING grade-attribution fork is RESOLVED = T2: $V_{\text{yield}}$ is the transverse-$T_2$ self-trap wall; the longitudinal-A1 compliance bound is at $V_{\text{snap}}$. Canonized in prose at `pair-production-axiom-derivation.md:93-104` (T2 horn) and reconciled at `nonlinear-vacuum-capacitance.md:18` — the A1-horn re-keying banner, verbatim *"Grade-fork RESOLVED = T2"* / *"That is **corrected**"* (re-pinned 2026-09-07 from `:16`, which is the varactor sentence one block ABOVE the banner, not the reconciliation itself).
- **canonical-home:** `pair-production-axiom-derivation.md:93-104` (sectoral prose, Grant 2026-06-30 T2 ruling); `nonlinear-vacuum-capacitance.md:18` (the ruling banner keying the A1 varactor on $V_{\text{snap}}$, verbatim *"bond compliance below diverges at the higher"*; re-pinned 2026-09-07 from `:16`); `src/ave/core/constants.py:455,:464` (calibration); the electron-binding derivation `research/2026-06-30_electron-portmap-derivation_result.md` §5.
- **clm-cross-links:** *(none verified-specific yet)*
- **open-ambiguity-flag:** **PARTIAL — the sector fork (2) is RESOLVED; the α-lock (1) STANDS as an honest caveat.** **(1) ALPHA-LOCK, not two free thresholds (STANDS):** $V_{\text{yield}}/V_{\text{snap}} = \sqrt{\alpha}\approx0.0854$ EXACTLY (`constants.py:464`). They are NOT two independent per-sector thresholds — the transverse-$T_2$ onset sits at $\sqrt{\alpha}$ of the longitudinal-$A_1$ scale. Do NOT import "one free threshold per sector"; the $\sqrt{\alpha}$ relation is itself a VALUE-level $\alpha$-echo. **Consequence of the ruling:** the electron's A1 mass core operates at strain $A=V_{\text{yield}}/V_{\text{snap}}=\sqrt\alpha$ — an $\alpha$-echo operating point (Class-C), sub-saturated ($S(\sqrt\alpha)=\sqrt{1-\alpha}\approx0.996$), which is why the mass channel does not run away. **(2) sector-attribution fork — RESOLVED = T2 (Grant 2026-06-30):** the earlier conflict — KB leaf `nonlinear-vacuum-capacitance.md:18` (re-pinned 2026-09-07 from `:16`; verbatim *"The prior wording keyed this A1 varactor's saturation-completion voltage on"*) had the $V_{\text{yield}}$ varactor on the LONGITUDINAL-A1 bond compliance, while `constants.py` + `pair-production-axiom-derivation.md:102` put $V_{\text{yield}}$ on the TRANSVERSE-$T_2$ self-trap — is now settled: **$V_{\text{yield}}$ is the T2 wall**; the A1 varactor's divergence was re-keyed to $V_{\text{snap}}$, removing the conflict. **(3) keep DISTINCT** from the C_eff/ε_eff saturation-reactance sector split at `ave-kb/CLAUDE.md:73` (a different object — the saturation reactances, not the two-event threshold ladder). **(4) CAGE OWNERSHIP — OPEN, ROUTED TO GRANT (opened 2026-09-07; provenance lane — nothing here adjudicates physics or changes a status).** This node's SOLID stamp is correct **on its actual scope**: which sector's constitutive law is keyed on 43.65 kV. It is **not** a ruling on which sector *owns the confining cage*, and the node must not be read as if cage ownership were settled (the confinement clause's true descent is the RIDER under adjudicated-meaning above). Four questions are open, and this entry decides none of them. **(4a)** Does commit `2ff5ff41` stand as a Grant ruling on cage OWNERSHIP? It landed 2026-07-01 01:17 and Grant merged it 11 minutes later; it wrote the two lines now at `resonant-lc-solitons.md:140-141` — verbatim *"The $\Gamma=-1$ confinement cage is the transverse-$T_2$/charge self-trap at $V_{\mathrm{yield}}$"* and *"The A1 mass is held by this transverse-$T_2$/charge self-trap boundary"*. Only Grant can vacate a Grant-attributed decision; this lane left those two lines untouched. **(4b)** If it stands, does it SUPERSEDE `master-equation.md:106` (verbatim *"The electron mass-cage is the **A1 longitudinal dilatation**"*), or do both hold in different frames? PR #260's own result doc offers a frame-split of exactly this shape — *"provenance = T2"* and *"state = A1"*, *"Not a contradiction — two frames of one mode."* (`2026-06-15_wall-branch-fork_result.md:42`). **(4c)** The live NESTING contradiction inside ONE resultbox: `resonant-lc-solitons.md:140` puts the A1 mass *"inside it sub-saturated"* — a nesting — while `:145` of the same resultbox states *"two orthogonal self-confining sectors (A1-mass + Cosserat-charge), NOT a nesting"*. Both in print, five lines apart, unreconciled. **(4d)** Is `electron-identification.md:32` (Property 3) STALE? It keys the self-created confining cavity to *"Axiom 4 engages: $C_{eff} \to \infty$, $Z_{local} \to 0$, $\Gamma \to -1$"* **at $V_{\text{yield}}$** — but $C_{\text{eff}}=C_0/S\to\infty$ is precisely the **A1 varactor divergence that the 2026-06-30 ruling re-keyed to $V_{\text{snap}}$** (fork (2) above; `nonlinear-vacuum-capacitance.md:18`). So the canonical SOURCE of the confinement clause states its mechanism in the sector that same ruling moved. Whether Property 3's conclusion survives on a different mechanism, or its $V_{\text{yield}}$ keying is stale, is unadjudicated. **Routed to Grant; tracked at `_orchestration/open-items/2026-09-07-cage-ownership-provenance.md`.**
- **verification:** VERIFIED the Grant 2026-06-30 T2 ruling landed in prose at `pair-production-axiom-derivation.md:93-104`; VERIFIED the A1-horn reconciliation (varactor re-keyed $V_{\text{yield}}\to V_{\text{snap}}$) at `nonlinear-vacuum-capacitance.md:18` (verbatim *"Grade-fork RESOLVED = T2"*; re-pinned 2026-09-07 from `:16`); VERIFIED the calibration definitions at `constants.py:455,:464` and the $V_{\text{yield}}=\sqrt{\alpha}\cdot V_{\text{snap}}$ alpha-lock ($\sqrt\alpha=0.0854$). Status SOLID — fork (2) resolved, α-lock (1) retained as caveat. See `research/2026-06-30_electron-portmap-derivation_result.md` + `research/2026-06-24_engine-reroute-epic-summary.md`.
