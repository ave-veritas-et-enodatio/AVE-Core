...
- **adjudicated-meaning:** *(PROPOSED, gated)* the lock that groups the node's **6 Axiom-1 spatial DOFs + the saturation-state $A$** into four sectors/ports, so the same node can be read in EE port language without sector drift: **MASS** = the A1/dilatation (trace-of-translation) port → $Z_{\mathrm{bulk}}$; **CHARGE** = the Cosserat **micro-rotation** port (the $\mu$/inductive sector) → $Z_{\mathrm{shear}}$; **$\varepsilon$** = the capacitive/electric (displacement/Coulomb) port; **deviatoric-shear** = the transverse-mechanical port. Effective params modulate with $S(A)$: $\varepsilon_{eff}=\varepsilon_0 S$, $\mu_{eff}=\mu_0 S$, $C_{eff}=C_0/S$ ([`CLAUDE.md`](../CLAUDE.md):73,:75). **MASS (A1) $\perp$ CHARGE (T2)** ([`master-equation.md`](../vol1/dynamics/ch4-continuum-electrodynamics/master-equation.md):20) — the two homonymous "3"s are orthogonal ports, never one phasor.
...
- **verification:** VERIFIED the 6-DOF + $A$-state content at [`CLAUDE.md`](../CLAUDE.md):73,:75 and $A1\perp T2$ at `master-equation.md`:20. The MASS/CHARGE/$\varepsilon$/$\mu$ role attribution re-expresses the three-channel ROLES (`resonant-lc-solitons.md`:118-125, themselves Grant-gated where the $V_{yield}$ fork applies). **GATED on auditor + Grant review — NOT adopted, NOT SOLID.**
...
## V_yield / V_snap *(proposed)*
<!-- id: def-vyvsn1 -->

- **term:** V_yield / V_snap (the two Axiom-4 saturation thresholds)
- **adjudicated-meaning:** *(proposed — records the #416 sectoral two-event reading; the sector-attribution of $V_{\text{yield}}$ carries a BLOCKING fork, see below)*: two voltage thresholds on the Axiom-4 saturation kernel. **$V_{\text{snap}} = m_e c^2/e \approx 511$ kV** = the longitudinal $A_1$ sector fully saturates ($A^2=1$, $Z_{\text{core}}\to0$); this is the mass-completion + Schwinger / pair-nucleation energy. **$V_{\text{yield}} = \sqrt{\alpha}\cdot V_{\text{snap}} \approx 43.65$ kV** = the Axiom-4 saturation **onset** (transverse Cosserat $T_2$ self-trap per the #416 ruling; the single-electron confining $\Gamma=-1$ TIR wall is here). $A_1 \perp T_2$.
- **axis:** sectoral saturation thresholds (longitudinal-A1 vs transverse-T2 onsets)
- **dimension/type:** voltage (both); BOTH are **CALIBRATION, not derived** ($V_{\text{snap}}\equiv m_e c^2/e$ definitional with $m_e$ in voltage units; $V_{\text{yield}}\equiv\sqrt{\alpha}\cdot V_{\text{snap}}$, the $\sqrt{\alpha}$ being the imported $\alpha$-echo). `src/ave/core/constants.py:450` (V_SNAP), `:462` (V_YIELD).
- **status:** proposed — Grant-gated. Records the #416 sectoral reading (canon in prose at `pair-production-axiom-derivation.md:98-101`, supersedes the prior purely-temporal "two stages") but is **NOT yet a settled def-node** because of the fork below.
- **canonical-home:** `pair-production-axiom-derivation.md:98-101` (sectoral prose, Grant 2026-06-24 / #416); `src/ave/core/constants.py:450,:462` (calibration).
- **clm-cross-links:** *(none verified-specific yet)*
- **open-ambiguity-flag:** YES — **(1) ALPHA-LOCK, not two free thresholds:** $V_{\text{yield}}/V_{\text{snap}} = \sqrt{\alpha}\approx0.0854$ EXACTLY (`constants.py:462`). They are NOT two independent per-sector thresholds — the charge-sector onset sits at $\sqrt{\alpha}$ of the mass-sector scale. Do NOT import "one free threshold per sector"; the $\sqrt{\alpha}$ relation is itself a VALUE-level $\alpha$-echo. **(2) BLOCKING sector-attribution fork (flag-don't-fix, Grant physics call):** the KB leaf `nonlinear-vacuum-capacitance.md:14,:16` attributes the $V_{\text{yield}}$ varactor ($C_{\text{eff}}=C_0/S$) to the **LONGITUDINAL-A1 bond compliance** ("NOT the transverse dielectric"), while `constants.py` ties $V_{\text{yield}}$ to $\varepsilon_{\text{eff}}\to0$ (the **TRANSVERSE dielectric**) and `pair-production-axiom-derivation.md:100` (#416) puts $V_{\text{yield}}$ at the transverse Cosserat-$T_2$ self-trap. These are opposite grade-assignments of the same threshold — a pre-existing KB(A1)-vs-engine($\varepsilon_{\text{eff}}$) inconsistency that must be adjudicated by Grant BEFORE this node is canonized. **(3) keep DISTINCT** from the C_eff/ε_eff saturation-reactance sector split at `ave-kb/CLAUDE.md:73` (a different object — the saturation reactances, not the two-event threshold ladder).
- **verification:** VERIFIED the #416 sectoral prose at `pair-production-axiom-derivation.md:98-101`; VERIFIED the calibration definitions at `constants.py:450,:462` and the $V_{\text{yield}}=\sqrt{\alpha}\cdot V_{\text{snap}}$ alpha-lock; VERIFIED the sector-attribution conflict (KB-A1 `nonlinear-vacuum-capacitance.md:14,:16` vs engine-$\varepsilon_{\text{eff}}$). Status PROPOSED / Grant-gated — the fork (2) blocks canonization. See `research/2026-06-24_engine-reroute-epic-summary.md`.
