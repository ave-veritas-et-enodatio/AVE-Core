...
- **status:** ambiguous — the surface form is overloaded across four registers with no single locked sense; the **influence-class register is already canon** (`translation-circuit.md:115-116`), so this node adjudicates the *overload*, not the physics. **GATED on Grant review.**
...
- **open-ambiguity-flag:** YES — **four registers, KEEP-ALL. This node redefines nothing; it separates:**
  ...
  - conflicting sites: influence-class canon `translation-circuit.md:115,116`; anatomy restatement `envelope-anatomy.md:88,119`; instrument sense `cleave-01-requirements-boundary-conditions.md:200`; Letter optical sense `main.tex:1229`; mass-ledger sense `common-mode-twist-ledger.md:10`; A1 mode-grade `fork_b_saturation_tank.py:30`, `node_scattering_multiplicity.py:95,193`, `port-register.md:93`.
...
## tube phase *(proposed — the (2,3) family parameter; ARC-ADOPTION, not a coinage)*
<!-- id: def-tubalf -->

- **term:** tube phase (the (2,3) winding-family parameter; walk/epic glyph "α" — see the glyph flag)
- **adjudicated-meaning:** *(PROPOSED, gated)* the phase parameter of the (2,3) winding **family**: ψ → ψ + α maps one family member to another. It is **NOT an invariant of the winding** — a uniform frame change / time-origin shift preserves the topological class while moving α; only the class is the winding. Its operational consequence is the **α-agnostic imposition guard** (static-existence epic §5 guard 8, verbatim): *"Imposing a specific α imposes MORE than the winding and smuggles un-owned structure into the relaxed state. The G1-walked imposition must be α-agnostic (the class, or the family)."* **EE-native first — inside the register's lock cage:** the free absolute phase of an oscillator sitting at a fixed frequency ratio to a reference; specifying that phase specifies the reference, not the ratio. ⚑ **The draft's "a synthesizer locked N:M" framing is WITHDRAWN** — see the read-guard. **Space declaration (A46):** a phase-carrier angle on the phase-space portrait; **not** a real-space angle, and specifically not the real-space **tube radius** — see the glyph/word flag.
- **axis:** phase-carrier; NOT a substrate-noun ontology node (hence no `ontology-grade`)
...
## ratio lock *(ambiguous — RE-SCOPED at audit: a RESTATEMENT + disambiguation pointer, NOT a coinage)*
<!-- id: def-rtlock -->
...
## static existence *(ambiguous — RE-SCOPED at audit: two objects with OPPOSITE status polarity)*
<!-- id: def-stexst -->
...
[`peierls-nabarro-paradox.md`](../vol2/appendices/app-b-paradoxes/peierls-nabarro-paradox.md):12
