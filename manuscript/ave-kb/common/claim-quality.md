...
- _Specific Claims_
  - One five-step chain (identify $\varepsilon_{11}$ → locate $r_{sat}$ → apply $\nu_{vac}$ Poisson correction → eigenfrequency from $\omega = \ell c/r_{eff}$ → $Q = \ell$) reproduces eigenvalues across multiple scales using the same operators. **[Spin-scope, Ruling B1 2026-07-21 (propagated 2026-07-31; nothing struck, scope added): for the BH-ringdown fire, flat $Q = \ell$ is the **cold $a_* = 0$ anchor** only — $\ell$ is an integer **mode count** (cavity-deformable — `common/vocabulary-register.md:261` (def-quant3), sense (1) at `:271`); the discriminator is its **INTEGRALITY**, not its sense class — an integer can only jump discretely, while the physical $Q$ is an impedance ratio that moves continuously with the strain profile, and an integer that can only jump cannot track a ratio that moves smoothly, so at catalog spins the flat reading fails at $\bar D_Q = -38\%$ (corrected-Kerr $Q$ rises $3.07 \to 3.49$) and the m$\Omega$ law carries the spin dependence. Model banner: [`vol3/claim-quality.md`](../vol3/claim-quality.md) `:204`; scoping doc `research/2026-07-30_qlaw-derivation_scoping.md` §1.5.]**
  ...
  - ~~The Kerr $Q$ correction (co-rotating frame decomposition) reproduces GR to sub-2% for $a_* \in [0.3, 0.8]$.~~ **🔴 RETRACTED 2026-07-21 (Grant Ruling B1; original struck, preserved per Rule 12).** The "sub-2%" was computed against the **corrupt $\omega_R$/$\omega_I$ Kerr reference tables + source-frame masses** (#774). Corrected picture: the topological **flat $Q = \ell = 2$ is scoped to the $a_* = 0$ cold anchor** (the $\Omega \to 0$ limit of the m$\Omega$ law) and would **fail at $\bar D_Q = -38\%$** at catalog spins; the **spin-refined m$\Omega$ law** $\omega_I = (\omega_R - m\Omega)/(2\ell)$ under the standing v1 mapping lands at **$-5.44\%$** (Resultbox) / $-4.57\%$ (ZAMO) — an **OPEN near-miss tension**, the named next ringdown work, NOT a sub-2% match and NOT a zero-free-parameter benchmark. The m$\Omega$-law *structure* (incl. superradiance at $\omega_R = m\Omega$) is the standing Q-law under B1 and is not retracted; only the accuracy number is. Honest grade: [`vol3/claim-quality.md`](../vol3/claim-quality.md) `clm-395gps` (model banner at `:204`) and [`ave-merger-ringdown-eigenvalue.md` § GRANT RULING B1](../vol3/cosmology/ch15-black-hole-orbitals/ave-merger-ringdown-eigenvalue.md) (solidity 0.55, disclosed-phenomenological, *"NOT a zero-free-parameter benchmark"*).
- _Specific Non-Claims and Caveats_
  ...
  - The Kerr $Q$ formula degrades at $a_* > 0.9$ (error grows to ~40% at $a_* = 0.99$); the LIGO-band claim does not extend to extremal Kerr. **[Corrupt-reference inheritance, 2026-07-21: the "~40% at $a_* = 0.99$" is read off the same corrupt Kerr table whose "sub-2%" LIGO-band claim is retracted above (#774); and the "LIGO-band claim" it references no longer exists as a match. Not an honest corrected-Kerr deviation.]**
...
- strengthen-by:
  ...
  - Verify the longitudinal-acoustic G projection inherits the Kirkwood-Frohlich angular factor cleanly (flagged open in §6) 🔴 **[DEMOTED 2026-08-11 — R40-B2a: NEEDS RE-DERIVATION, not dead; dated note at the end of this file]**
...
> **🔴 CONTESTED — dated demotion 2026-07-19 (Rule-12; claim body + prior Quality block preserved below).** The requested simulation (see strengthen-by, "Support the ≥100-Compton-period residue persistence with a simulation") was run in the unbanked moving-front freeze-in arc (archive tag `archive/analysis/moving-front-freezein` @ `f647f58b`; `research/2026-06-30_moving-front-freezein_result.md`), recovered in the 2026-07-19 branch scrub. Outcome: (a) **persistence CONTESTED** — engine realization gave real-space ω-defect persistence **≤ 3.04 Compton periods** (~30× short of the ≥100 Cp asserted here); single named mechanism = the re-solidified Cosserat solid is linear-elastic with **no topological-pinning term** (`cosserat_field_3d.py:1999`, `_bulk_accel → _bare_linear_gradient`). CONTESTED not REFUTED because the result doc frames it as an **OPEN A44 fork** (engine gap vs corpus over-claim, not adjudicated) and the negative is resolution-limited (N=12–16; §7 asks for N≥32 re-confirmation before corpus-verdict promotion). (b) **direction FLAGGED backwards-as-stated** — the derivation + run give fast→freeze / slow→heal (S_min `0.04→0.19→0.56` with v_front), the OPPOSITE of the "crossing takes ≥ τ_relax → FREEZES" (slow→freeze) worded in the claim body (and the "slow-crossing regime" caveat below); routed to Grant/auditor. (c) **the memristive S-lag mechanism itself is CONFIRMED** rate-dependent. **confidence demoted `0.50 → 0.20` ⇒ derived solidity `0.30 → 0.20`** (bottom of the "do not build on" band; deliberately NOT the <0.20 "refuted" band, since CONTESTED not refuted). HOME leaf banner: `dark-wake-bemf-foc-synthesis.md` §1.2. Flag-don't-fix: both directions and both readings surfaced verbatim; nothing here retro-edits the frozen claim body.  <!-- rule12-freeze: base=f909e4e0028a02514d93fc5c1fa37b81d1d248dc region=below offset=25 lines=738 bytes=98836 sha256=19501f7123d10db21da1732cf8c720fd0caa033637087b2e61a9fbe207ab6960 -->

> **🔴 RULED 2026-07-20 (RULING 2 of the 2026-07-20 ratification batch — supersedes the CONTESTED status above for the BARE-loop branch; Rule-12 KEEP-BOTH, claim body + prior Quality block preserved).** Grant ratified 2026-07-20 (**Grant-verbatim, [sic]: "2 ratified"**); ruling *content* = the **orchestrator's walk, ratified in chat** (attribution split — the wording here is the ratified walk, NOT Grant-verbatim). **(a) DIRECTION RULED fast→freeze** — the body's slow→freeze prose ("crossing takes ≥ τ_relax → FREEZES") is BACKWARDS on its own mechanism: τ_relax is the state's *slew limit* (sample-and-hold — fast sweep holds/quenches, slow sweep tracks/anneals), and the engine's S_min-vs-v_front monotone rise (`0.04→0.19→0.56`) is the receipt. **(b) PERSISTENCE RULED REFUTED-AS-STATED for BARE ω-loops** — the seed-controlled N≥32 discharge (all ≤ 3.264 Cp, ~30× short of ≥100 Cp) closes the resolution-limit caveat the CONTESTED status rested on; nothing pins a bare ω-loop in a linear-elastic bulk (consistent with the localization-is-boundary settlement), so the **A44 fork resolves for bare loops toward CORPUS-OVER-CLAIM** (Ax1 does not promise pinning for a bare non-soliton loop; no engine pinning term is "missing", no Ax5 owed — corpus over-reach, not engine-violates-Ax3). **(c) SCOPED OPEN — soliton-dressed defects** (a loop carried by an actual (2,3)-winding soliton, untested by this arc). The CONFIRMED memristive S-lag mechanism is untouched. **Confidence demoted `0.20 → 0.15`** (into the "refuted, do not use" band [0.00, 0.20) per the house build-status legend) ⇒ derived solidity recomputes to `0.15`. Value rationale: **0.15, not lower** — refuted-as-stated for the *tested* bare-loop case (below the 0.20 "do not build" floor), but held mid-refuted-band (not 0.00–0.10) because the direction is *corrected* (fast→freeze salvages the mechanism's internal logic rather than voiding it), the memristive S-lag half is CONFIRMED, and the **soliton-dressed branch stays OPEN** — the claim is refuted as-stated for bare loops, not annihilated wholesale. HOME leaf banner: `dark-wake-bemf-foc-synthesis.md` §1.2 (dated RULED addendum). Flag-don't-fix preserved (both readings surfaced verbatim).
...
### Quality
- confidence: 0.15
<!-- confidence demoted 2026-07-20 from 0.20 → 0.15 (RULING 2, 2026-07-20 ratification batch; supersedes the 2026-07-19 CONTESTED value). The seed-controlled N≥32 discharge (≤3.264 Cp vs ≥100 Cp asserted) closes the resolution-limit caveat the CONTESTED status rested on; persistence is RULED REFUTED-AS-STATED for BARE ω-loops and the A44 fork resolves toward corpus-over-claim; direction RULED fast→freeze. 0.15 lands in the "refuted, do not use" band [0.00, 0.20) per the build-status legend, held MID-band (not 0.00–0.10) because the mechanism's S-lag half is CONFIRMED, the direction is corrected-not-voided, and the soliton-dressed branch stays OPEN. Prior chain (preserved): 2026-07-19 demoted 0.50 → 0.20 (CONTESTED). Drives derived solidity → 0.15. -->
- depends-on:
...
In a phase-only substrate there is **no spatial frame beneath the node graph**: position is defined only ON the graph, as a pattern of relative phase offsets across nodes. A localized excitation IS such a pattern, so its energy goes where its host nodes go — it **cannot hold station against the material displacement of its nodes** (station-keeping would require anti-advective node-to-node hopping, which presupposes the very sub-graph spatial anchor phase-only denies). Consequence for **Fork ρ** (*is trapped-energy mass acoustic inertia, or gravitating-only?*, routed from the `#782` RVE bench): trapped-energy patterns **participate in a carrier's oscillatory material motion** (the kinematic half, unconditional); *if* that swayed energy carries **nonzero acoustic inertia per unit sector-crossed `T2` energy** (the C-load premise, open `clm-m5swh9`) the trapped mass **loads** the effective inertia `ρ_eff` — so the **direction leans Reading A (MATCHED) conditional on the C-load premise**, degenerating to Reading B (`ρ_eff/ρ_0 = 1`) if that ledger entry is `~0`. Canonical home [`relative-offset-principle.md`](./relative-offset-principle.md); walk provenance `research/2026-07-21_fork-rho-walk_RECORD.md`; Grant-ratified at the DIRECTION level (as interpreted in RECORD §5 — his verbatim names neither "Fork-ρ" nor "Reading A") in-chat 2026-07-21 (verbatim `[sic]`: "yes the relative offset makes sense, and seemsmlike a major concept for the cannon to adjudicate and define"). 🔴 **[DEMOTED 2026-08-11 — R40-B2a: NEEDS RE-DERIVATION, not dead; dated note at the end of this file]**
...
- _Specific Claims_
  ...
  - **Corollary C (Fork-ρ direction, split):** *(C-kin, unconditional)* trapped-energy patterns participate in the carrier's oscillatory material motion — no pinned escape from material tracking; *(C-load, CONDITIONAL on `clm-m5swh9`)* IF the swayed trapped energy carries nonzero acoustic inertia per unit sector-crossed `T2` energy THEN it loads `ρ_eff` (`ρ_eff/ρ_0 = 1 + β·φ > 1`), leaning Reading A (MATCHED) — if that ledger entry is `~0`, Reading A degenerates to the engine-native Reading B (`ρ_eff/ρ_0 = 1`).
- _Specific Non-Claims and Caveats_
  - **DIRECTION only.** Does NOT claim a MAGNITUDE — the loaded inertia value, `β`, and the sector-crossed `c²` choice are the separate OPEN claim `clm-m5swh9` (β INFEASIBLE-ON-LATTICE; routed to the #788 rulings items 10/11 — Protocol E structural `ρ_eff` term + continuum `E=mc²` import). Do not read the direction as a magnitude prediction.
  - **WALK-RATIFIED, pending adversarial review.** Grant ratified the DIRECTION and the promote-to-canon; the leaf is PROPOSED (Grant-ratification-at-merge). The wording is the orchestrator's walk (attribution: the argument is walk-level; the two quoted strings are Grant-verbatim). Not an emergence result; ~~not yet adversarially reviewed~~ *(2026-07-26: the adversarial review RAN — PR #787, 9/9 findings confirmed, repairs landed; see the ★STATUS NOTE on the rationale below. Grade HELD at 0.50.)*
  - **Does NOT introduce a preferred-frame observable.** The crystal frame (≈ CMB-isotropy frame) exists and is unchanged; the principle says the EXCITATION does not inherit it — a boosted breather is taken (in the continuum/sub-band-edge limit) as an equally valid lossless solution, an **ungraded dynamical premise** (discrete breathers radiate over the PN barrier; see the leaf's §"The principle"), not established here. This is consistent with, not a revision of, `preferred-frame-and-emergent-lorentz.md`.
  - **Does NOT re-open the #782 verdict.** BIN-4 is invariant across `β`; the banked bare-node `r_Z` values are **neither a floor nor a ceiling** of the physical `r_Z` (the `K`-side is a KUBC upper bound, #782 result:12 §7.1; the ρ-side net sign is open — a two-term decomposition per the #788 scoping §4, where Protocol E measures the net structural term). The bench's `K_eff` numbers are untouched.
...
- rationale: A walk-ratified DIRECTION with a clean structural argument (the anti-advection reductio follows from the phase-only substrate with no free steps for a uniform/global displacement; for a LOCALIZED carrier the far undisturbed lattice is an on-graph frame, so exclusion of local station-keeping leans additionally on Corollary A index-pinning, not on pure logic — leaf §"anti-advection impossibility"), Grant-ratified in-chat at the direction level and promoted to a proposed canon leaf — but PENDING adversarial review and with the MAGNITUDE explicitly open (`clm-m5swh9`). Graded 0.5 (not higher) because it is pre-review and the corollary set has not yet been stress-tested against the corpus's real-space "cannot advect / does not move" statements (surfaced as flag-don't-fix revisit sites, not yet adjudicated); graded 0.5 (not lower) because the core reductio is structurally clean over Axiom 1 and the direction is Grant-ratified. Consistency/organizing-principle class, the same standing as the AC/DC carve it mechanizes on the excitation side. **★STATUS NOTE (2026-07-26): the adversarial review RAN 2026-07-21 (PR #787, 9/9 findings confirmed) — the repairs landed (Corollary C split into C-kin unconditional / C-load conditional; the r_Z consequence re-scoped to neither-floor-nor-ceiling; the corpus "cannot advect" tension RE-LABELED as self-transport nulls + formulation artifacts rather than a contradiction — the ADJUDICATION itself is still routed to auditor/Grant, `relative-offset-principle.md`:59 "this leaf reframes none of them", and strengthen-by leg 1 below STANDS). The grade is HELD at 0.50 and the "pending/pre-review" phrasing above is RETAINED verbatim pending a formal re-grade — this note records that the review is no longer pending, without retitling the claim (anchor-slug stability).** *(2026-07-28 addendum: the routed ADJUDICATION has since been made — Grant ruled 2026-07-26 to PROPAGATE the re-label — so "strengthen-by leg 1 below STANDS" is superseded: leg 1 is now **DISCHARGED** below, and `relative-offset-principle.md:59` carries the dated superseding update. Grade still HELD at 0.50.)*
- strengthen-by:
  - ~~adversarial review of the corollary set against the corpus "cannot advect / does not move / pinned by its own frozen clock" sites (genesis-chord-falsification-ledger.md:144; motion-stability-bemf-longitudinal; annihilation-evaporation TRANSPORT-ABSENT) — reconcile material-tracking vs phase-velocity-surfing or flag a genuine tension~~ — **DISCHARGED 2026-07-26** by Grant's ruling (verbatim `[sic]`: *"for the relabel, let's propagate/ (a)"*) on the #787 review: the reconciliation is **material-tracking, NO genuine tension**. Those sites are (i) **no-SELF-transport / no-re-hosting-DOF** results for the saturated core *at their formulations* — which **SUPPORT** Corollary A rather than contradict it (a core with no re-hosting DOF is the dynamical content of index-pinning) — plus (ii) **formulation-anchor nulls** (bond strains boost-invariant under a uniform material shift in the small-displacement model; the #788 structural absences, `research/2026-07-21_beta-tracking-feasibility_scoping.md` §3). They are **not** a "can't-surf-a-drive-at-phase-velocity ⇒ can't-track-material-displacement" result. Landed carves (all ADDITIVE; **no June verdict overturned** — GENUINE-FALSIFICATION, PIN-even-longitudinal, UNRESOLVED, PROPAGATION_WITH_TIR_AT_WALL and NULL-mobility all stand): `genesis-chord-falsification-ledger.md` ★SCOPE CARVE on the motion-stability section; `research/2026-06-04_motion-stability-bemf-longitudinal-result.md`; `research/2026-06-11_annihilation-evaporation_result.md`; plus the sweep-missed `research/2026-06-08_native-electron-propagation-adjudication.md`, `research/2026-07-09_superband-carrier-fork_result.md`, `research/figures/photon_vs_electron_cells.py`, `src/scripts/vol_1_foundations/moving_defect_transport_gate.py`. Grade **HELD at 0.50** — the discharge re-labels the sites, it adds no new evidence for the direction. **★ 2026-07-28 claim-strength harmonization (adversarial review of the propagation lane).** Read *"SUPPORT Corollary A rather than contradict it"* above at exactly the strength the ruling licensed — **NOT COUNTEREVIDENCE**. It is **not** positive evidence for the corollary: none of the carved runs drove the material-tracking axis, so on that axis each is **evidentially NULL**, and a null cannot double as support. The carve sites now state that strength explicitly. Separately, the *"formulation-anchor nulls"* clause is **narrowed**: the `#788` family is defined by uniform-shift invariance, which does **not** obtain for the 2026-06-04 longitudinal run (spatially varying Variant-B drive, measured nonzero core strain, live kernel response) — that label is **WITHDRAWN** for that run and residual artifact language is scoped to the material-tracking axis only. Grade **still HELD at 0.50** (no re-grade in either direction).
  - ~~Grant-ratification-at-merge of the leaf + the "relative offset" name (and the deferred `def-` register row, post-#781 batch)~~ — **DISCHARGED 2026-07-26**: #787 merged 2026-07-21 (leaf + name ratified); `def-r0ffst` landed in #794.
  - close `clm-m5swh9` via the #788-routed magnitude paths (Protocol E structural `ρ_eff`, item 10; continuum `E=mc²` import, item 11) — the β feasibility lane reported INFEASIBLE-ON-LATTICE
...
Given the walk-ratified DIRECTION (`clm-hu1jjw`: trapped-energy mass loads `ρ_eff`), the **magnitude** of that loading is **OPEN**. The effective inertia per pattern is **emergent field momentum** and **SECTOR-CROSSED**: the trapped energy is `T2`/swing-class energy inside the `A1` compression carrier's budget, and **which sector's `c²` divides `E`** to give the loaded inertia is not automatic (`A1 ⊥ T2`, `master-equation.md:20`; sector-ownership cross-wiring watch). The tracking ratio `β = v_pattern-centroid / v_local-material` and the effective `M` were routed to a β feasibility lane, which has since reported **INFEASIBLE-ON-LATTICE** (three structural absences — no mass-energy equivalence, no advective pattern transport, imposed-grade-sets-the-answer; `research/2026-07-21_beta-tracking-feasibility_scoping.md` §3); the magnitude is split (the #788 rulings, Grant-approved 2026-07-21) into **item 10** (the structural `ρ_eff` term via Protocol E, lattice-accessible) + **item 11** (the trapped-energy term as an explicit `E=mc²` import on the continuum radial-solver lane, not lattice-derived). Canonical home [`relative-offset-principle.md`](./relative-offset-principle.md) §"Honest scope".
...
- _Specific Claims_
  ...
  - **The protection locus (NOT a quantization mechanism):** winding integers live on the whole **punctured disk** (any `A≠0` supports `π₁(S¹)=ℤ`); the rim confers **maximal PROTECTION** — amplitude freeze-out removes the `A→0` unwinding channel — not existence. The rim↔charge coupling is a locus-of-protection consistency reading over the STATIC-Link canon (`charge=Link(∂Ω,F)`, #416), not a claim that reaching the rim dynamically closes the `(2,3)` integer.
...
- _Specific Non-Claims and Caveats_
  ...
  - **The MAPPING is what Grant ratified**, over his prior sign-flip reading (parked, no claim). Ratification does NOT extend to `clm-satnec` or `clm-zdual1`. PROPOSED leaf (Grant-ratification-at-merge); ~~the `def-` register row is DEFERRED to a post-#781 batch~~ *(DISCHARGED 2026-07-26: #790 merged 2026-07-21; `def-satrim` landed in #794.)*
  - **NOT a quantization MECHANISM — the dynamical charge-winding tests read NEGATIVE.** The rim↔charge coupling is a *protection*-locus consistency reading over the STATIC-Link canon (`charge=Link(∂Ω,F)`, the #416 two-natured ruling, `research/2026-06-24_engine-reroute-epic-summary.md:5,20,24`), NOT a claim that reaching the rim dynamically closes the `(2,3)` integer. The corpus's nearest *dynamical* tests read NEGATIVE at both loci — #415 real-space eigensolve (winding bled off the bound mode) and #417 phase-space carrier-lock (`(−5,−5)=(1,1)`-class, not `(2,3)`); `the-abandoned-interior.md:113` ("the `(2,3)` winding does NOT dynamically 'close' at any `V` — it is a **static Clifford-torus / Link texture**").
...
- solidity: 0.55 (use as input only, don't build deeper) [= min(0.55, 1.00)]
- rationale: A consistency-class / organizing reading, not a derivation — a near-theorem over three objects the corpus already owns (the Axiom-4 kernel's circle shape; the frozen local clock at yield, genesis ledger:145; the phase-space `(2,3)` winding, electron-identification §1). Grant ratified the MAPPING in-chat over his prior sign-flip reading. It carries **exactly ONE new ungraded premise** — the geometric factorization that the single-mode disk-rim is one factor of the corpus's `(2,3)`-carrying Clifford torus. That factorization's registered two-circles adversarial check has now **RUN (this review, PR #790)**: the `π₁` (topological-class) factorization reading **SURVIVED** — it is radius-blind and does **not** touch the Golden-Torus `R·r=¼` metric, and **no** metric identification of the rim-radius with the `(R,r)=(φ/2,(φ−1)/2)` semi-axes was ever claimed. Graded 0.55 (above the relative-offset reading's 0.50) because it rides on already-canonical objects with **no ungraded *dynamical* premise** and its one geometric premise survived review; 0.55 (not higher) because it is PROPOSED/pre-review, Grant ratified a mapping not a derivation, and the quantization-locus half is scoped to a **locus-of-PROTECTION** consistency reading (the nearest dynamical charge-winding tests read NEGATIVE — #415 real-space, #417 phase-space carrier-lock; charge = the static Link, #416). The reading's solidity is deliberately NOT bled onto the two candidate reads (`clm-satnec` OPEN, `clm-zdual1` 0.50). **★STATUS NOTE (2026-07-26): the adversarial review RAN 2026-07-21 (PR #790, 10/10 findings confirmed) — the repairs landed (the quantization-locus half re-worded from an EXISTENCE claim to a locus-of-PROTECTION consistency reading, with the #415/#417/#416 static-Link negatives engaged; the π₁ factorization survived as topological-not-metric). The grade is HELD at 0.55 and the "PROPOSED/pre-review" phrasing above is RETAINED verbatim pending a formal re-grade — this note records that the review is no longer pending, without retitling the claim (anchor-slug stability).**
- strengthen-by:
  - ~~Grant-ratification-at-merge of the leaf + the `saturation-rim-inversion` name (and the deferred `def-` register row, post-#781 batch)~~ — **DISCHARGED 2026-07-26**: #790 merged 2026-07-21 (leaf + name ratified); `def-satrim` landed in #794.
  - adversarial review of the disk/rim reading against the corpus's phase-space-winding sites (`the-abandoned-interior.md:46,:111`; `substrate-perspective-electron.md:243,247`; `mathematical-topology-of-mass.md:30`) — the two-circles / disk-factor-of-torus leg has now RUN (PR #790 review: `π₁`-factorization survived, radius-blind, no metric identification claimed); the remaining strengthener is a full pass against the other phase-space-winding sites
...
photon) and $10G/(3\rho)$ (×2, P-wave). The **load-bearing structural statement** is that the carrier's 🔴 **[DEMOTED 2026-08-11 — R40-B2a: NEEDS RE-DERIVATION, not dead; dated note at the end of this file]**
...
- _Specific Non-Claims and Caveats_
  ...
  - **NOT superluminal transport.** The relativistic form's validity window closes BEFORE its own relativistic regime opens ($k_{\text{break}}/k_{\text{rel}} = 0.387$, $0.424$) and the full-BZ carrier group velocity stays below $c_{EM}$ everywhere (sampled max $\approx 0.61$, a lower bound; supremum uncertified). The mismatch is in the low-energy effective theory's invariant speed.
...
At the swing-railed core the impedance **ordering of the channels flips** versus cold vacuum — the core is the impedance **DUAL** of the medium. This is a **re-reading of an already-Grant-ratified split** (INVARIANT-S2, Grant-ratified 2026-06-15, `resonant-lc-solitons.md:41`), not a new result: the **longitudinal-A1 bond compliance** `C_eff = C_0/S → ∞` shorts the bulk channel (`Z_bulk = √(L/C_comp) → 0`, `Γ_bulk = −1` at the knot core, `electron-bh-isomorphism.md:26`); the **transverse-T2 permittivity** `ε_eff = ε_0·S → 0` (the swing-ε kernel, `fdtd_3d_jax.py:109-113`) opens the EM channel (`Z_wave = √(μ/ε) → ∞`). Both `|Γ|=1`, opposite boundary phase (`Γ=−1` short vs `Γ=+1` open) — the dual. Canonical home [`saturation-rim-inversion.md`](./saturation-rim-inversion.md) §"The channel-duality inversion".
...
- _Specific Non-Claims and Caveats_
  - **The DUALITY SUBSTRATE is already Grant-ratified** (INVARIANT-S2, `C_0/S`↑ vs `ε_0·S`↓ is a sector split, NOT a sign error, `resonant-lc-solitons.md:41`); this claim re-expresses it as "the impedance dual." The re-expression / framing was NOT ratified by the 2026-07-21 mapping.
...
- **`:561`** — stamped at `:561`. *(family: G-projection-label; banked `uncertain`)*
  ...
  ```text
  Strengthen-by to-do frames the G-anisotropy projection as longitudinal-ACOUSTIC; if the projection is static tensor algebra it drops to static class — the to-do must be re-posed under the carve first.
  ```
...
- **`:1530`** — stamped at `:1530`. *(family: Fork-rho-C-load)*
  ...
  Audited rationale, verbatim from the banked worklist:
  ```text
  Register copy of Fork-ρ: the C-load arm (and :1536 ρ_eff loading) is defined by a compression-carrier probe with no referent under the carve — fork survives only re-posed.
  ```
...
- **`:1625`** — stamped at `:1625`. *(family: band-branch-P-wave)*  ⚑ **BIAS-DEBT**
  ...
  Audited rationale, verbatim from the banked worklist:
  ```text
  clm-2bkp7v enumerates a √(10/3)c P-branch in the canonical Bloch spectrum; the carrier/photon k·p content is shear-sector and untouched, but the spectrum statement is owed a constrained (P-branch-projected) re-derivation.
  ```
...
## The Relational-Observable Cancellation Identity (PROPOSED — Grant-ratification-at-merge)
<!-- id: clm-relcnc -->

**IF** an observable is relational — a ratio (or offset-form difference) against a reference that **co-transforms exactly** under an influence, which the phase-only / relational-observable epistemology asserts of every admissible observable (`clm-acdc07` (i): *"all measurement is AC"*; rulers and clocks are wave-made of the medium) — **THEN** the common-mode component of that influence **cancels identically**: algebraically ($gx/gr = x/r$; $(x+g)-(r+g) = x-r$), at every scale, to all orders (the influence never enters the observable), with zero parameters (nothing matched, tuned, or renormalized). The **differential complement** — whatever couples asymmetrically across signal and reference — survives in full and is the entirety of what the observable transduces (the canon row pair, *"Uniform external field | Common-mode bias"* / *"External field gradient | Differential bias"*, `translation-circuit.md:115-116`). EE-native statement: **the substrate is an ideal differential instrument whose CMRR is infinite BY IDENTITY, not engineered** (canon EP row, *"CMRR is infinite BY IDENTITY … the tide is the DIFFERENTIAL mode"*, `translation-circuit.md:156`). Canonical home [`relational-cancellation-identity.md`](./relational-cancellation-identity.md); walk provenance `research/2026-08-24_frame-invariance-observer-walk_RECORD.md` §1 (*"The theorem that appeared three times in one day"*); minted on Grant's R57 GO (2026-08-24, verbatim `[sic]`: *"lets pursue the theorem from A1"*, with his A1 framing *"lead with EE native, is this a universal scale invariant?"*).

- _Specific Claims_
  - **The cancellation identity** (above), with its premise exposed in **two legs**: leg 1, *class membership* — which observables are relational (the epistemology: all of them, integers included, a winding being a loop-sum of phase differences); leg 2, *exact co-transformation* — which influences actually co-transform the reference EXACTLY (a per-instance physical fact, falsifiable per instance). The theorem's mathematical content is one line of algebra; its physical content is entirely the premise.
  - **The universality KIND: BY IDENTITY, not dynamical.** Universality is inherited from the definition of the observable class — a categorically different kind than the Axiom-4 kernel's dynamical universality (A-034: contingent, instance-falsifiable, buys numbers). This identity is never falsified by an instance — only an instance's premise-MEMBERSHIP can fail; it buys organizing power and ZERO numbers.
  - **The differential complement is the physical content:** a relational observable rejects the common mode exactly and transduces the differential mode entirely — the identity form of *"every AVE-distinct observable is an AC reading of a DC gradient or topology."*
  - **Premise-membership instances banked at their own grades** (leaf §3, 9 instances + 2 WALK rows): chart-floor uniform grading MEASURED (Γ_end = −1/3 exact all orders, *"max deviation 5.6e-17"*; differential split *"agree to 1e-13"*; `research/2026-08-24_smith-annulus_result.md:367-369, 381-383, 386-387`) incl. the negative-space case 2b (a "symmetric" bias with the bond held cold is a hidden differential — the floor MOVES at O(ε²), `:373-380`); (2,3) carrier-ratio invariance under uniform clock rescale ALGEBRA-exact with its MEASURED differential complement (*"2:3→0.65"*, 0.644 vs 0.667, `research/2026-06-24_engine-phase-space-winding_result.md:29, 44-45`, #417); srs vertex counting CANON (*"immune to symmetric transformation"*, `translation-circuit.md:189`, `clm-v3port`, invariance-not-value per the in-lattice caveat); WEP CMRR row CANON (`translation-circuit.md:156`); α exactly invariant under SYM co-scaling — *"$\varepsilon$ and $c$ carry compensating $n \cdot S$ factors that cancel in the $\alpha$ expression"* (`manuscript/ave-kb/claim-quality.md:141`, `clm-3zz0f6`, solidity 0.85, the highest-graded instance; canon books the row as CMRR at `translation-circuit.md:706`); the A-state gauge-relativity invariant (*"only spatial gradients of $A$ … are physically observable"*, `manuscript/ave-kb/CLAUDE.md:75`); the common-mode twist ledger (`clm-cmtwst` 0.50, CONDITIONAL, premise SUPERSEDED — carried as a premise-FORM exhibit); AC/DC measurement principle CANON (`clm-acdc07`); relative-offset Corollary B CANON-PROPOSED (`clm-hu1jjw`, HELD 0.50).
- _Specific Non-Claims and Caveats_
  - **★ THE ALGEBRA IS EXTERNAL PRIOR ART — no novelty is claimed and none may be inferred.** The cancellation is the incumbents' own statement under at least five names: global-phase unobservability (QM), gauge redundancy (field theory), general covariance (GR), only-dimensionless-ratios-are-measurable (Weyl/Duff-class dimensional analysis), CMRR (EE) — leaf §4. The AVE-forced content is the PREMISE, not the algebra. **Explicitly NOT claimed:** that AVE derives gauge invariance, relativity, or any interaction — the identity is **not generative**, and canon's own R43 rescope removes full local U(1) from the written action (*"not full local U(1) gauge invariance, which is not a symmetry of the written action"*, `manuscript/ave-kb/vol1/axioms-and-lattice/ch1-fundamental-axioms/kirchhoff-network-method.md:43`). The constructive/mechanism half also has prior art in-corpus (`physics-lineage-map.md:473`, T13 Lorentz–Larmor).
  - **★ DOES NOT DISCHARGE THE STANDING SECTOR-BY-SECTOR DEBT.** The identity holds GIVEN exact co-transformation; whether real rulers and clocks co-transform exactly, sector by sector, is open corpus debt — *"derive exact self-cancellation sector-by-sector, or state where it breaks and at what magnitude"* (`physics-lineage-map.md:473`), and *"weak-C exactness is a prediction, not a theorem"* (`:587`). No "infinite by identity" phrasing sourced to this claim may be read as retiring that debt.
  - **Zero numbers, ever.** Consistency / organizing-principle class. `clm-relcnc` must NEVER appear as a step in a value derivation — such a cite is a misuse by construction (identity-not-derivation discipline, the `Q_TANK = 1/ALPHA` precedent).
  - **"Identity form" does not upgrade `clm-acdc07`.** Canon books that carve as *"an organizing PRINCIPLE not a theorem (solidity 0.55)"* (`form-deriving-value-importing.md:372`); writing its measurement statement in identity form changes nothing about that booking, and this entry books in the same class at 0.50.
  - **The MEASURED tags are premise-membership receipts, not dynamical-universality evidence.** They confirm one influence co-transformed one reference exactly; the algebra was never at stake (the walk's own tautology counter-arm is carried in the leaf, §2). Nor are they evidence for the phase-only epistemology, which is this claim's premise (that would be circular).
  - **`preferred-frame-and-emergent-lorentz.md` is NOT an instance** — it is the CONTRAST exhibit (leaf §2): a *dynamical, finite* suppression (*"$\delta_{aniso} \sim (q\ell_{node})^4 \approx 2.2 \times 10^{-22}$"*) of a frame that *is* detectable.
  - **Integer observables: common-mode blindness only.** A winding is blind to a common-mode offset because it is a loop-sum of differences; its robustness under *differential* perturbation is topological protection (*"integer protection is topological"*, `axiom-register.md:555`), a different mechanism this claim does not cover.
  - **No observer-effect derivation.** The 2026-08-24 walk §4 conjecture stays un-promoted, its 6-question sweep un-run, its four counter-arms cited live in the leaf (§5.1). **No gravity-decoherence claim** (Penrose–Diósi adjacency named, not engaged). **No collision asserted with the ohmic Born/decoherence thread** (`clm-ldmvwi` owns the probability/click axis; reconciliation UNBUILT).
  - **WALK legs stay WALK:** *"identity lives in the frame-invariant part"* (walk §2) and the chart-normalization *"relativity = the physics of only-ratios-observable"* assembly (walk §1) are tabled at WALK grade and not promoted (leaf §3 rows 10–11).
  - **The A1⊥T2 fourth-register fence:** this claim's "common-mode" is an INFLUENCE class, never the port/irrep A1 common-mode MODE grade (= the physical mass sector, *"common-mode scalar/longitudinal (dilatation, mass)"*, `port-register.md:37`).
  - **Branch dependency (HARD GATE):** the R57 docket entry that carries this claim's minting authority, plus `def-cmdiff` and `def-rtlock`, exist only on open PR #1011 (`kb/2026-08-24-walk-vocab-register`). **This entry lands only after #1011 merges.** Ruling state: R57 **A5** closes the `def-rtlock` keep-vs-decline fork as *"KEEP-as-restatement"* (status stays ambiguous) — there is no live DECLINE. The leaf cites canon rows directly, so no instance depends on a `def-` node resolving.
  - **The uniform component is not declared unreal** — the DC sector stays AVE's physical-medium-state bet (`clm-acdc07`); this identity is the mechanism of its unobservability from inside, not of its absence, and that bet is a commitment carried elsewhere, not a consequence proved here.

> **Leaf references:** [relational-cancellation-identity](./relational-cancellation-identity.md).

### Quality
- confidence: 0.50
- depends-on:
  - Axiom 1 (Substrate Topology — the phase-only node graph; no sub-graph anchor to read an absolute against, so admissible observables are relational)
  - clm-acdc07 — the AC/DC carve (the measurement principle this claim states in identity form; solidity 0.55)
  - clm-v3port — the z=3 counting floor (instance 3's canonical anchor; solidity 0.83)
- solidity: 0.50 (use as input only, don't build deeper) [= min(0.50, 0.55)]
- rationale: The algebra is exact, trivial, and **external prior art** — deliberately so; the load-bearing content is the premise carve (which influences co-transform exactly, per instance) and the universality-KIND distinction, which are organizing-principle class. Graded at the proposed-entry convention (0.50, the `clm-hu1jjw` precedent): PROPOSED pending Grant-ratification-at-merge and pre-adversarial-review; not higher because the claim's value is organizational, its instances are membership receipts rather than evidence for anything contingent, and the premise's sector-by-sector exactness is itself standing open debt (`physics-lineage-map.md:473`); not lower because every instance receipt is verified read-and-cite and the two canon anchors (`clm-acdc07`, the EP row) already state the physics this claim consolidates. Consistency-class; buys zero numbers by construction.
- strengthen-by:
  - **Gate first:** PR #1011 merged (the R57 ruling record on `main`), then Grant-ratification-at-merge of the leaf + this entry, then adversarial review (the leaf's grades move only through that gate)
  - the routed computable step from the walk (RECORD §5): the actual mode-pair detuning of the bond-pair tank under an imposed ∇ε₁₁ — a new MEASURED differential-complement instance if it lands (tagged UNDERIVED-EXTENSION at its T3 first-pass formulas until then)
  - post-#1011: the conditional `def-cmdiff` cross-link, and the `def-rtlock` cross-link (KEEP-as-restatement per R57 A5)
  - post-ratification: a `foundational-machinery-register.md` row citing this leaf as home; a `theorem-thesaurus.md` row for the ALGEBRA half (E7, that register's lane)
