...
- confidence: 0.65
- depends-on:
  - clm-0ktpcn — Golden Torus α Derivation (solidity 0.50) [α appears at multiple layers; α-bottleneck cascades through chain]
...
- confidence: 0.65
- depends-on:
  - clm-0ktpcn — Golden Torus α Derivation (solidity 0.50) [α appears in BH $r_{eff} = r_{sat}/(1+\nu_{vac})$, pion via $m_p$, etc.]
- solidity: 0.50 (use as input only, don't build deeper) [= min(0.65, 0.50)]
...
- confidence: 0.70
- depends-on:
  - clm-0ktpcn — Golden Torus α Derivation (solidity 0.50) [α-dependent constants throughout: $V_{yield}$, $p_c$, etc.]
