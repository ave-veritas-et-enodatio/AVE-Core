...
- _Specific Claims_
  ...
  - **Ladder theorem:** $V=\int E\,d\ell=\sum_{\text{cells}}E\cdot\ell_{node}$ — the two registers are one ladder at two scales, not competitors. Both knees are real: the **field knee** $(2\alpha)^{-1/4}=2.877\,\ell_{node}$ (cell/dress, step register, measured as the $r99$ outer edge in PR #696) and the **voltage knee** $(2\alpha)^{-1/2}=8.278\,\ell_{node}$ (interaction/port, ladder register); the second is the square of the first, as a line integral relates to its step.
...
- _Specific Non-Claims and Caveats_
  ...
  - **The Op4/Op14 $(d_{sat}/r)^2$ port expression is a CANDIDATE integrated ladder — REGISTERED-not-run.** The Op4 ladder integral (integrate the per-cell field dress radially vs the closed-form $Z_0(1-(d_{sat}/r)^2)^{-1/4}$) is owed; a match closes the fork at the VALUE level, a mismatch flags a canonical-operator defect (flag-don't-fix).
...
- _Specific Claims_
  ...
  - **FORM (Grant-ruled):** the two families are two criteria (storage vs response), and the criterion axis is ORTHOGONAL to the register axis (clm-strreg) — a 2×2 the corpus populates in 3 of 4 cells ($h_{yield}$ = step×storage; $V_{YIELD}$ = ladder×storage; the two knees = response).
