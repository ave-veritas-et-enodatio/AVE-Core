...
> **🟢 Design-walk EXECUTED (2026-07-19, Rule-12 — supersedes the "DOS-balance = recommended direction" status below).** The design walk this section recorded as IN PROGRESS **happened.** Grant ruled (in-chat) that the T2 sink couples **locally as a static, pre-occupied noise floor** — Grant-verbatim "*my gut says its couples through a static noise floor*" `[sic]` + "*so wffectively constant*" `[sic]`. Under that reframe the **DOS-balance A/B fork is MOOT**: growth/node-genesis re-homes to the cosmological rate rung (`Γ=3Hρ_latent`), and the local arrow turns on **dephasing against a standing random floor**, not on balancing bath-vs-lattice DOS. The counting-arrow **resolution path is now the THERMAL-FLOOR configuration** — a pre-occupied random-phase bath (branch `feat/f6-thermal-floor-arm`, in flight), adjudicated by a revival-vs-ρ kill-shape. The **design-walk-owed slot (★SCOPE HONESTY, below) is now EXECUTED.** The "DOS-balance = recommended direction" wording in this section is **preserved below per Rule-12** as the superseded prior status. Full arc + attribution: `research/2026-07-19_noise-floor-arrow-walk_RECORD.md`; docket: RULING 22 + the F6 NOISE-FLOOR ARROW continuation (`_orchestration/2026-07-10_rulings-docket.md`). Recorded as §8c.10.
