...
> - **§2 capability matrix = VERIFIED-STATE** — every cell grounded against a file:line / PR anchor, or a verbatim-text anchor for the loop-gap doctrine cells (see [`figures/engine_capability_matrix.yaml`](figures/engine_capability_matrix.yaml)).
...
On the **STANDALONE-K4** plant, energy conservation `E_lat+E_bath` is an **algebraic identity**, not an empirically-survived outcome: the 4-port scatter matrix is **amplitude-independent** — `build_scattering_matrix` (`src/ave/core/k4_tlm.py:64`) returns `S_ij = 0.5 − δ_ij` for the equal-admittance node junction *regardless* of `z_local` (the `2y/y_total = 2y/4y = ½` cancellation at `:88-92`, and the `z≈1` fast-path at `:82`; `_S_base = build_scattering_matrix(1.0)` is set at `:255`). So the S(A)-kernel-in-scatter pump the §B1 risk feared is **inexpressible** on this junction: even under `nonlinear=True`, the Op14 saturation block (gated at `:335`) rebuilds `_S_field[idx] = build_scattering_matrix(z_strained)` (`:368`) — which *still* returns `0.5 − δ` — and routes the amplitude-dependent `z_strained` **only** into `z_local_field` (`:365`), which feeds the lossless Op3 bond-impedance mismatch in `_connect_all`, **never the scatter kernel**. The global rescale removes exactly the bath's gain (arithmetic-exact). *(Superseded phrasing, preserved 2026-07-19 per Rule-12: "`nonlinear=True` is a NO-OP in `K4Lattice3D` unconditionally" — imprecise. `nonlinear=True` is not a no-op: it updates `S_field`/`z_local_field` and rebuilds `_S_field`. What is code-true is narrower — the rebuilt scatter kernel is amplitude-independent because `build_scattering_matrix` is `z_local`-independent for the equal-admittance junction, so no amplitude gets into the scatter S(A) path.)* **★SCOPE CAVEAT (load-bearing):** the certificate is scoped to STANDALONE-K4. A **CoupledK4Cosserat** arm (Cosserat↔V exchange outside the `E_lat` ledger) **or ANY genuine irreversible ε→T2 depletion primitive** BREAKS the identity ⇒ the **W-battery must be RE-VALIDATED** before such an arm integrates the meter (charter §B-post-review R-1, `research/2026-07-16_f6-bath-meter_CHARTER.md:263`).
...
### §8c.8 — RULING 22 (Grant in-chat 2026-07-19): KEEP THE INSTRUMENT (Grant-ruled); DOS-BALANCE = recommended direction, design-walk IN PROGRESS → **EXECUTED 2026-07-19 (DOS-balance MOOT; resolution = the thermal-floor configuration — §8c.10)**

> **🟢 Design-walk EXECUTED (2026-07-19, Rule-12 — supersedes the "DOS-balance = recommended direction" status below).** The design walk this section recorded as IN PROGRESS **happened.** Grant ruled (in-chat) that the T2 sink couples **locally as a static, pre-occupied noise floor** — Grant-verbatim "*my gut says its couples through a static noise floor*" `[sic]` + "*so wffectively constant*" `[sic]`. Under that reframe the **DOS-balance A/B fork is MOOT**: growth/node-genesis re-homes to the cosmological rate rung (`Γ=3Hρ_latent`), and the local arrow turns on **dephasing against a standing random floor**, not on balancing bath-vs-lattice DOS. The counting-arrow **resolution path is now the THERMAL-FLOOR configuration** — a pre-occupied random-phase bath (branch `feat/f6-thermal-floor-arm`, in flight), adjudicated by a revival-vs-ρ kill-shape. The **design-walk-owed slot (★SCOPE HONESTY, below) is now EXECUTED.** The "DOS-balance = recommended direction" wording in this section is **preserved below per Rule-12** as the superseded prior status. Full arc + attribution: `research/2026-07-19_noise-floor-arrow-walk_RECORD.md`; docket: RULING 22 + the F6 NOISE-FLOOR ARROW continuation (`_orchestration/2026-07-10_rulings-docket.md`). Recorded as §8c.10.  <!-- rule12-freeze: base=5eb71f7d9fac87daf3d83b100eec4b1f00730772 region=below offset=0 lines=3 bytes=388 sha256=0ca355dd4ff596675976d221b3c6ac82227474b2cd96e9a4f2ee5fcdfbb2575b -->

**Grant-verbatim (the four words that rule):** "*yes, keep the instrument*" (+ "*fully document in kb following proper skills and rules, lets also update the kb engine map*"). **What the verbatim rules:** the instrument STAYS — it rules **OUT** the R2 meter-edit and endorses documenting the existing certified meter. It does **not**, by itself, confirm DOS-balance as the mechanism.

> **🟡 Attribution note (2026-07-19, Rule-12 — the "RULED DIRECTION / the resolution path IS DOS-BALANCE" wording below is preserved as superseded).** Grant-RULED: keep the instrument / rule out R2. The **DOS-BALANCE framing** is the **orchestrator's recommended path**, which Grant **engaged into a design walk** — that walk was **IN PROGRESS at this batch**. The direction confirmation **rides the walk's outcome**; the **upgrade-to-RULED slot is left explicitly open**. Superseded wording preserved: the §8c.8 header formerly read "the F6 fork = DOS-BALANCE on the existing instrument"; the ruling sentence formerly read "The counting-arrow resolution path **is** DOS-BALANCE ON THE EXISTING CERTIFIED INSTRUMENT"; the Fork table's DOS-balance row formerly read "**RULED DIRECTION**." See docket RULING 22 for the same softening.  <!-- rule12-freeze: base=5c8572c0567e99fef023056ef38771e0b47c31f7 region=below offset=56 lines=23 bytes=1389 sha256=6c695477769f967ac56458c39d040818dd95358bb30225d854cbce06b8a3ec6a -->

**Recommended direction (orchestrator; Grant-engaged, walk IN PROGRESS).** The recommended counting-arrow resolution path is **DOS-BALANCE ON THE EXISTING CERTIFIED INSTRUMENT** — attack the **equipartition root** (bath-DOS ≫ participating-lattice-DOS drives the full discharge) by **balancing the mode counts** (candidate knobs, direction-only: fewer bath modes near the line / larger collar / more lattice participation). The instrument is **kept** (Grant-ruled, not rebuilt); the DOS-balance *mechanism* is the recommendation the walk is adjudicating. The other forks are **NOT taken:**

| Fork | Ruling |
|---|---|
| **KEEP the instrument (rule out R2 meter-edit / rule out rebuild)** | **RULED** (Grant-verbatim "yes, keep the instrument") |
| **DOS-balance on the existing instrument** | **RECOMMENDED DIRECTION** (orchestrator; Grant engaged the design walk — **upgrade-to-RULED pending the walk's outcome**; attack the equipartition root by balancing mode counts) — **★MOOT as of 2026-07-19** (the design-walk resolved the fork *away* from DOS-balance to the static-noise-floor / thermal-floor configuration; see the 🟢 note + §8c.10) |
...
**★SCOPE HONESTY (mandatory).** The ruling fixes the **DIRECTION only.** The design walk — *what exactly gets balanced, and how* — was **OWED with Grant in-chat BEFORE any design lane fires** (walk-the-picture-first) — **now EXECUTED (2026-07-19; see the 🟢 note above + §8c.10): the walk resolved the fork *away* from DOS-balance to the static-noise-floor / thermal-floor configuration.** Any resulting configuration will still need its **own validation extension on the meter certificates** before it can bank a counting-arrow result — in particular the §C X5 tare-wording gap (docket ENTRY 18 F7) **rides that future extension**, not this ruling. Docket: RULING 22 + the F6 NOISE-FLOOR ARROW continuation (`_orchestration/2026-07-10_rulings-docket.md`). **Owed to auditor lane:** land the RULING-22 fork direction + the DOS-balance-walk-owed note (implementer surfaces; auditor lands) — **→ LANDED 2026-07-19 (this batch): §8c.8 EXECUTED note + §8c.10.**
...
**Counting-arrow QUESTION: OPEN** (favorable single-comb evidence; collapse unshown; nothing emergence-class). **Resolution status: instrument KEPT (Grant-ruled, rules out R2); DOS-balance = the RECOMMENDED direction (orchestrator), which Grant engaged into a design walk that was IN PROGRESS at this batch — upgrade-to-RULED rides the walk's outcome (§8c.8, RULING 22). Design-walk OWED with Grant.** → **EXECUTED 2026-07-19 (the noise-floor ruling; recorded in §8c.10).** Receipts = the four PRs (#722/#724/#726/#727) + their result docs.

### §8c.10 — 2026-07-19: the noise-floor ruling (the §8c.9 design-walk, EXECUTED)

> **🟢 SUPERSEDED-IN-PART pointer (2026-07-20, post-wave KB batch).** The "**thermal-floor arm … in flight**" status in the body below is STALE: the arm has since FIRED and been re-banked to the tri-form verdict — see **§8c.11** (landed via #738 D5, ratified by #734 merge). This §8c.10 records the *ruling* (still current); its *arm-in-flight* clause is superseded by §8c.11. Rule-12: body preserved verbatim.

**2026-07-19 — the noise-floor ruling (the owed §8c.9 design-walk, resolved).** Grant ruled (in-chat) that the T2 sink couples **locally as a static, pre-occupied noise floor** ("effectively constant") — the Johnson-Nyquist vacuum-noise-floor row (`translation-circuit.md:151`). Consequence: the full-discharge pathology was **bath-emptiness** (a cold comb drains to zero; a floor relaxes *to* the floor, clamp moot); the **local arrow** = coherent returns dephasing into the floor's random phases (counting-only, static); growth/node-genesis **re-homes** to the cosmological rate rung (`Γ=3Hρ_latent`), which makes the prior **DOS-balance A/B fork MOOT** (§8c.8's recommended-direction slot resolves *away* from DOS-balance). The **thermal-floor arm** (branch `feat/f6-thermal-floor-arm`, in flight) adjudicates via a **revival-vs-ρ kill-shape** (`ρ=0` positive control = the #726 dips, 14.9%→35.5%; NO-SUPPRESSION = the live falsifier). Floor provenance (level + randomness) is **upstream** (past-hypothesis analog; crystallization-frontier candidate). Full arc + attribution: `research/2026-07-19_noise-floor-arrow-walk_RECORD.md`; docket: the F6 NOISE-FLOOR ARROW continuation (`_orchestration/2026-07-10_rulings-docket.md`).

### §8c.11 — 2026-07-19: the thermal-floor arm FIRED (the §8c.9/§8c.10 arm, resolved to a tri-form verdict; ratified by #734 merge)

**2026-07-19 — the thermal-floor arm fired.** The revival-vs-ρ arm that §8c.9/§8c.10 recorded as "in flight" (branch `feat/f6-thermal-floor-arm`) has RUN and been re-banked post-review (adversarial review `wf_932a21ad`). Result: `research/2026-07-19_f6-thermal-floor-arm_result.md` (merged #734). **Tri-form verdict** (three honest statements; the single "NO-SUPPRESSION" frozen-tree label is disclosed DEGENERATE — interior clips to 0):
- **(i)** the STRONG walked hypothesis — "revivals DIE as the floor rises past the signal" — is **EXCLUDED**: POST-HOC coherent revival at ρ=5 = `0.923 ± 0.167` (jackknife, N=6), **5.5σ die-to-zero / ~2.8σ ≥halving**; flat `0.899 → 0.923` across [0,5]. Grant's ride-on-top alternative corroborated at the STRONG-form level.
- **(ii)** the reactive-floor arrow mechanism is **STRUCTURALLY INEXPRESSIBLE** on a non-interacting (lossless-reactive Caldeira–Leggett) bath — identity-class, the #721-W2 shape (an algebraic identity, not an empirical falsification).
- **(iii)** a **PARTIAL suppression ≤~30%** at ρ=5 is **UNCONSTRAINED** (jackknife σ≈`0.167`, N=6 ⇒ 2σ-detectable drop ~37% of the cold revival).

**Grant RATIFIED BY MERGE of #734 (2026-07-19):** (a) the §D-post floor-battery **flip** — frozen-literal `FLOOR-LEDGER-ARTIFACT` (un-amended §D.D3 binary on the `Δω=0.030` ladder to ρ=30) → corrected `FLOOR-METER-VALID-BAND[0,5]` (§Dp reconciliation + bounded-band class); **both readings banked** (flag-don't-fix — Grant could have overruled the flip and stopped the arm, and did not; `research/2026-07-19_f6-floor-battery_result.md`); and (b) the tri-form verdict framing (the earlier single-headline "empirical falsification" is demoted; honest weight = (i)+(ii)).

**★ The ARROW QUESTION REMAINS OPEN.** The reactive-floor route is closed *structurally* (ii), not the whole question. Surviving candidates (SPEC only, not built): **interacting-bath thermalization** (a floor that dephases a coherent revival needs a phase-carrying interaction — mode-coupling / nonlinearity / genuine dissipation, absent in the lossless-reactive comb; breaks the #721 identity ⇒ full re-cert) and **the X40-class *click*** (boundary/topological minting — the one-way-at-an-event arrow).

**FD leg — banked FENCED (no claim).** The floor-injected fluctuation / relaxation ratio rises smoothly with ρ (**FD ratio `0 → 0.129`** across ρ ∈ [0,5]); routed to the ℏ-as-FD open, **excluded from the §4 verdict**; no fluctuation-dissipation theorem asserted. Nothing banked at emergence-class; meter + K4 engine byte-untouched (floor = config-only). **SEM caveat (2026-07-20, #752):** the `0.129` absolute is an **N_seeds=6 SEM artifact** (`std/√N` ⇒ scales `∝ 1/√N_seeds`; calibration + ensemble-size-dependent, NOT a forced number); the **√ρ SHAPE is N-invariant and analytically exact** (equipartition ½ amplitude-law, CONSISTENCY-class / Johnson-Nyquist, no chord) — see `research/2026-07-20_hbar-as-fd_result.md` (merged #752). Read the shape, not the absolute.

### §8c.12 — 2026-07-20: the post-wave arrow/fork state (pointer-class; ratified-by-merge of #742/#743/#744)

**2026-07-20 — where the arrow and yield-fork questions stand after the 07-19/20 merge wave.** Pointer-class only (nothing built, nothing new-canon); the load-bearing docs carry the derivations. Three standing states:

- **(a) TWO arrow-candidate SPECs landed, both BUILD-GATED (Grant walks the gate on each; nothing fired).**
  - **Interacting-bath thermalization** (`research/2026-07-19_arrow-candidate-interacting-bath_SPEC.md`, ratified-by-merge #742): the lattice's own mode–mode coupling is the **shared-`S(A)` intermodulation** — a Kerr/four-wave-mixing cubic, coefficient `1/4` FIXED by the `(1−A²)^(−1/4)` kernel (DERIVED, no VALUE import). Dephasing is DERIVED-YES but **thresholded**: at the certified MILD point it is `~40×` sub-threshold (`δω_nl/Δω ≈ 0.025`), which **retro-predicts the #734 null**; clearing the Chirikov overlap needs the near-knee (`A ≳ 0.63`). If built it **breaks the #721 identity-conservation certificate ⇒ full re-cert.** Genuine fork on the operating point + a phase-carrying port (a SECOND barrier — see B3 / §8c.11 lineage). **🟢 RULED 2026-07-20 (RULING 3, 2026-07-20 ratification batch; Grant-verbatim [sic] "3 ratified", ruling content = orchestrator's walk ratified in chat):** the SPEC's Decision-2 pre-test-physics question is answered — the **T2 port is a PHASED ARRAY** (many independent local contacts, statically-random per-contact phases), which **composes with** the #734 "effectively-constant" aggregate ruling (piston = aggregate level, array = port geometry). The structural-inexpressibility thereby **rescopes to instrument-class** (arrow live on the real substrate, closed at the scalar-port instrument); the phase-carrying build is **LICENSED but PARKED** (no build without a separate Grant go).
  - **X40-class click** (`research/2026-07-19_arrow-candidate-x40-click_SPEC.md`, ratified-by-merge #743): a **count-coordinate** (topological `b_1` mint) arrow — expressible on the lossless substrate where the floor's modal-phase coordinate was barred. Loschmidt-class; chord-vs-echo undecided until its Loschmidt + `Δω`-invariance gates run. The certified meter's hard-zero back-reaction **clamp is adjudicated a NUMERICAL GUARD** (fails all three positive click signatures), NOT a derivable click — #726's NO-INFORMATION reading stands. **🟢 RULED 2026-07-20 (RULING 4, 2026-07-20 ratification batch; Grant-verbatim [sic] "4 ratified", ruling content = orchestrator's walk ratified in chat):** the SPEC's **Decision 2 = 2a** — the radiated `9/10` terminates into the **matched real port `R_rad ≡ Z_0`** (Ax3-legal SYSTEM-loss row, a real system-boundary port, NOT a bulk valve). The **cost-of-genesis ledger is thereby well-defined** (radiated majority = genesis cost; trapped `1/10 = f_E = 1/N` is **mesh-geometry, NOT `m_e`** — mass stays A1/definitional); the **keystone fence rides** (formation-route claims stay echo-scoped). The click-arm gate is CLEARED; the **BUILD stays PARKED** (no build without a separate Grant go).
- **(b) Flag-F RE-BANKED (`research/2026-07-19_flag-f-s-dynamics-derivation.md` §0, ratified-by-merge #744).** The near-yield `S(t)` derivation EARNS strictly less than its retracted "World (a) REACTIVE — RESOLVED" one-liner: **Eq 2.1's Markovian first-order reduction is derivably UNLICENSED at `ωτ~1`**; the **isolated node is second-order reactive** (kinetic-term / `I_S` provenance OPEN — R-5); the **axiom-level resistor is EXCLUDED**. The **(a)/(b) world-fork (reactive vs transductive) at the crossing STAYS OPEN pending the z=3 bath spectral density `J(ω)`** (never computed). Flag F **PARTIALLY discharged**, awaiting Grant ratification.
- **(c) The counting-arrow QUESTION remains OPEN** (§8c.7/§8c.9/§8c.11 lineage): the reactive-floor route is closed *structurally* (§8c.11 (ii)); the two open candidates are (a) above. The #734 result that named both candidates: `research/2026-07-19_f6-thermal-floor-arm_result.md`.

### §8c.13 — 2026-07-20: F1 (coupled K4↔Cosserat) DEFECT-fix reproduction gate PASSED (ratified-by-merge #759)

**2026-07-20 — the F1 DEFECT fix reproduces on current main.** The F1 DEFECT fix (Grant 2026-07-15 — `external_z_local` set `True` under `CoupledK4Cosserat` so the shared front `√(S_μ/S_ε)` survives `k4.step()` and reaches bond Γ; commits `79435ee3`/`20395fe2`) was re-run as a reproduction gate on `origin/main` @ **`e2e870c0`** (`research/2026-07-20_f1-verification-rerun_result.md`, merged #759). **PASSED:** the F1-materiality-critical `v_inc_peak` (the V-spatial peak the D1–D4 genesis-node-birth gates read) is **bit-stable to every banked digit** across all three seed modes at both fidelities; both frozen bins (smoke `i_A_SUPPORTED`, production `ii_A_WEAKENED`) reproduce; the priority-1 flip-risk did NOT materialize; F1-path regression + coupled/genesis/gpersist tests all GREEN (55). *(NB — this is the F1 **DEFECT fix**, distinct from the capability-map "F1 ruling" on the `S^0.5` exponent.)* **Two disclosed, verdict-invariant drifts, both banked BOTH / routed to Grant:** the `pair`-smoke `φ_persist` moved −6.4 % (stays ≫ 0.80 gate); genesis-24's `dE_V` drifts +10.8…+42.2 % (F1-attributable, amplified difference of two ~5.9 quantities) while the **qualitative `C1` verdict is invariant** and `max_V_inc ≈ 0.37` is bit-stable. The genesis-24 banked-JSON receipt-refresh is a **GRANT CALL**, not actioned (§Contradictions C1; see the two F1-NOTE stale-cite refreshes).
...
- **`:29`** — **NOT STAMPED — R39 BYTE-FENCE ROUTED** (the anchor sits inside a Rule-12 preserved body; this entry IS the routed ledger note and the span is byte-untouched). *(family: electron-sector engine labels)*  ⚑ **BIAS-DEBT** · ⚑ **PAST-WALL**
  Quoted claim (content verified at HEAD; markup-reduced from the banked audit):
  ```text
  rest mass = the self-trapped longitudinal-bulk wall; $c_{eff}\to\infty$ at the saturated core self-creates the $\Gamma=-1$ TIR cage
  ```
  Audited rationale, verbatim from the banked worklist:
  ```text
  Prereg electron-sector expectation: label-level re-derivation with past-wall scope declared (carve is cold-linear; saturated interior out of scope); mass=A1 accounting and the boundary-cavity localizer survive; the bulk-self-focus route is independently FALSIFIED (carve-consistent). Covers :50 crystal_engine "transverse-shear→longitudinal-bulk conversion" (presumes an independent bulk wave field to convert into) and :66.
  ```
...
- **`:31`** — **NOT STAMPED — R39 BYTE-FENCE ROUTED** (the anchor sits inside a Rule-12 preserved body; this entry IS the routed ledger note and the span is byte-untouched). *(family: 3-channel coupling DOF)*  ⚑ **BIAS-DEBT**
  Quoted claim (content verified at HEAD; markup-reduced from the banked audit):
  ```text
  the electron lives across all three impedance channels; a complete engine must carry and couple them
  ```
  Audited rationale, verbatim from the banked worklist:
  ```text
  Under the carve a complete engine implements the bulk slot as a constraint/controlled source, not an independently carried channel — the DOF requirement re-derives; covers the :37 pointer restating Q1 as "OPEN ... independent far-field radiative port" (premise = the phantom; also STALE vs port-register's 2026-07-20 revert, a currency flag independent of the carve).
  ```
