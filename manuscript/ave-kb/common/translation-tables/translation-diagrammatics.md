...
> where a home carries a 🔴/🟡 marker or an echo tag that BEARS ON the cited content, the row says so (banners on unrelated content in the same file are not restated). The particle zoo
...
| Virtual particle / "exchange" | Reactive near-field / standing longitudinal $V$ (the spine's replacement wording); the vocabulary register **replaces** the term outright | [`substrate-native-terminology.md:56`](../substrate-native-terminology.md) (FAIL-replace row); near/far split at [`dark-back-reaction-taxonomy.md:45`](../dark-back-reaction-taxonomy.md) — *"the QED self-energy (near) vs radiation-reaction (far) split"*, LOCKED decision-B |
...
| External line (photon/boson leg) | Propagating far-field mode on the matched continuum ($Z_0$ everywhere, $\Gamma = 0$ at every bond). External FERMION legs are the other object — $\Gamma=-1$ TIR-confined BoundResonators, not matched modes | [`photon-identification.md:113`](../../vol1/dynamics/ch4-continuum-electrodynamics/photon-identification.md) §3 — *"reflection coefficient $\Gamma = 0$ at every bond; no scattering or reflection"* — canonical; weak-C regime reading canonized, DEC-01 primitive-vs-regime OPEN |
| Loop integral | Mode-sum over the first Brillouin zone — finite by mode-count, no counterterm; the same integrand diverges in the continuum | [`brillouin-zone-uv-cutoff.md:53`](../../vol2/quantum-orbitals/ch07-quantum-mechanics/brillouin-zone-uv-cutoff.md) — clm-1wmyx3; the BZ leaf's §3 distinct-cutoff discipline: the loop bound is the spatial $k_{max}=\pi/\ell_{node}$, channel-independent — "consistent, not conflicting" with the routed ω-edge tension (⚑ `wall-taxonomy.md:171`, unadjudicated, ω-axis) |
...
| Pair-production vertex | Axiom-4 saturation **rupture at the yield boundary** — the medium's breakdown vertex; the WKB exponent is QED's by construction. EE join (the EE spoke's cells, pointered not duplicated): Zener field-tunneling (spontaneous) / Miller avalanche (seeded), $M = 1/S^2$ (Op22) | WKB half: [`q-g18-schwinger-pair-wkb.md`](../../vol2/particle-physics/ch01-topological-matter/q-g18-schwinger-pair-wkb.md) — clm-lj4ok5, structurally closed at WKB level. EE join + 2026-08-05 split ruling: [`translation-circuit.md:127-128`](translation-circuit.md) — *"split ruled 2026-08-05"* (Op22 pointered there → `operators.md:62`, the *"universal avalanche factor"* row) |
...
| Vacuum polarization / running α | Both classical routes closed WRONG-FORM (#685 power-law-not-log; #693 spectator no-log); rows re-tagged SCOPED IMPORT; AVE-distinct content = α stops running sub-Compton + structural Landau-pole removal; retarded + inductive channels declared NOT probed | [`q-g20f-vacuum-polarization.md`](../../vol2/particle-physics/ch06-electroweak-higgs/q-g20f-vacuum-polarization.md) — clm-bqtasn, solidity 0.60 input-only |
...
| Detector-side line termination (the "measurement") | The medium's Ohmic boundary termination (Joule extraction + threshold-crossing, the home's own nouns); Born-rule click probability derived end-to-end, no Born input. EE-spoke name: "detector capture work-function" (`translation-circuit.md:143`) | [`ohmic-decoherence-born.md`](../../vol1/dynamics/ch3-quantum-signal-dynamics/ohmic-decoherence-born.md) — clm-ldmvwi, solidity 0.65; AC/sign-symmetric scope |
...
1. **The internal-line-only bosons are the evanescent ones.** W/Z appear in diagrams
   ...
   modes of the Cosserat rotational sector — $W^\pm$ longitudinal-torsional, $Z^0$
   transverse-bending ($\gamma_c$ = the microrotational **bending** inductance) ([`gauge-boson-masses.md`](../../vol2/particle-physics/ch05-electroweak-mechanics/gauge-boson-masses.md),
...
- **α per vertex**: $Q_{tank} = 1/\alpha$ is a **definitional identity, never a derivation**
  (`physics-lineage-map.md:394` — *"cited as identity NOT derivation"*; the baked constant ruled culpable for the #41 circularity
  and CI-scrubbed, `interlock-register.md:302`). Any reading of "why the expansion parameter is α" that routes through
...
- **$a_e = \alpha/2\pi$** is PEER-WITH-Schwinger — SM's value equally rides measured α; the
  ...
  ($+502\times10^{-11}$) is **in 4.6σ tension above Fermilab** on the e⁺e⁻ baseline
  (BMW-baseline-conditional, so not refuted) — a liability rider, not a win
  (`vol2/claim-quality.md:195,1504,1516`).
- **W/Z rows** were re-typed `derived_prediction → consistency_check` (2026-08-19, D9-1;
  `consistency-manifest.yaml:143,168`); the public derived-predictions category is empty as
  of that date (`README.md:215`, category (iv): *"the framework outputs a novel numerical value"*).
- **Longitudinal sector**: $\nabla\!\cdot\!\mathbf{A}$ is gauge (def-l0ngdu). The
  "$\nabla\!\cdot\!\mathbf{u}$ propagates" half is the **R40-B2a phantom verbatim**
  (🔴 DEMOTED 2026-08-11, `substrate-native-terminology.md:144`) — what survives is
  **physical-and-bound ≠ deleted**, re-posed on the Axiom-5 clause-G bound response. The A1
  compression scalar is physical and never framed in QED-vector terms; no longitudinal photon.

## §5 — Gap ledger (six-lane corpus pull, 2026-08-23; negative results are search-derived — items 5 and 8 are the corpus-registered opens)
...
8. "One lattice cutoff or four?" — routed to Grant at `wall-taxonomy.md:171`, unadjudicated.
   That tension lives on the ω-axis; the loop-integral row's spatial $k_{max}$ bound is
   channel-independent per the BZ leaf's §3 ("consistent, not conflicting").
