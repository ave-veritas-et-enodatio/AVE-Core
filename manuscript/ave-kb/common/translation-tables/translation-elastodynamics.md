...
|---|---|---|
| Mode conversion at a boundary (P↔S at an impedance contrast) | Mode conversion at a $\Gamma$-wall (A1↔T2 at a substrate impedance boundary) | **WATCH** — no means-test; candidate only 🔴 **[DEMOTED 2026-08-11 — R40-B2a: NEEDS RE-DERIVATION, not dead; dated note at the end of this file]** |
...
- **`:31`** — stamped at `:31`. *(family: P↔S mode conversion watch; banked `uncertain`)*  ⚑ **BIAS-DEBT**
  ...
  ```text
  UNCERTAIN — as stated the candidate needs both branches propagating (wave-to-wave conversion), void under the carve; the surviving re-derivation is shear-incident-excites-bound-near-field-A1-response at the wall. Watch row, no means-test — cheap re-scope. Rows :32-:33 (Rayleigh/boundary modes, evanescent head waves) are boundary/evanescent class and not tabled.
  ```
