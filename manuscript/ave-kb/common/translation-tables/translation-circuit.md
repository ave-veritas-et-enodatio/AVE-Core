...
| **INVARIANT-S2 SYM scaling** | Concurrent $\varepsilon$ and $\mu$ modulation = isotropic gradient-index (impedance-matched, $\Gamma = 0$). $Z$ pinned $\Rightarrow$ bond TL $L/C$ pinned **if** those $L,C$ are the pair whose ratio is $Z$ (definitional). Identifying bond TL $L,C$ with $\mu,\varepsilon$ (so they co-scale with $n$) is **channel-scoped**: on the **gapless T2/EM channel** it is this table's own **Class A identity** ($L=\mu_0$, $C=\varepsilon_0$, §10.1 `:811-812`); for the **whole three-channel bond** ($Z_{EM}$/$Z_{shear}$/$Z_{bulk}$, `def-gv1net`) no single $(L,C)$ exists to identify, and the achromatic leaf licenses the $\mu,\varepsilon$ co-scale only. Linear-in-$\varepsilon_{11}$ is Op19, not the Ax4 kernel ([`def-ln3str`](../vocabulary-register.md); [`bond-lc-constitutive-grading.md`](../../vol3/gravity/ch03-macroscopic-relativity/bond-lc-constitutive-grading.md)) | [`alpha-invariance-symmetric-gravity.md`](../../vol3/gravity/ch01-gravity-yield/alpha-invariance-symmetric-gravity.md) |
