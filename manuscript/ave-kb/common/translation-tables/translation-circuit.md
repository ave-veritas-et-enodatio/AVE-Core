...
| strain energy $\tfrac12 E{:}C{:}E$ | dissipated (resistive) or stored (single-sign reactive) power $\tfrac12 \mathbf{E}\cdot\sigma\cdot\mathbf{E}$ |
...
| self-equilibration $\sum_i f_i = 0$ (prereg §1.1; measured `\|Σ_i f_i\| = 3.8e-15`) | **Kirchhoff's current law as the solvability condition** — the Fredholm alternative for a singular Laplacian: a floating current-fed network with net-nonzero injection has NO solution |
