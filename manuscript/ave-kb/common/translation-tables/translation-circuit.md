...
| Mixer / difference-tone conversion (two-tone → beat; inversion-symmetry meter) | Op2 ([`operators.md`](../operators.md):42) — the odd-$\chi^3$ kernel parity | ✓ | **Parity identity (geometry-independent):** the even Ax-4 kernel $\Rightarrow$ odd restoring force $\Rightarrow$ a combination tone at $m\omega_{lo}+n\omega_{hi}$ is allowed iff $m+n$ is odd, so the literal difference tone $\omega_{hi}-\omega_{lo}$ is **FORBIDDEN** sub-yield (by inversion symmetry, *reactive*/lossless — not suppressed). The difference channel is an **inversion-symmetry meter**: the vacuum as a biased-varactor difference-tone mixer, conversion $\propto$ bias — a planted even-$\chi^2$ term lights the forbidden tone $\propto\beta^2$ (exponent 2.000, $R^2 = 0.99999999$). Canonical: [`universal-saturation-kernel-catalog.md` § The parity theorem](../universal-saturation-kernel-catalog.md) (`clm-invmtr`); A⁶ χ³ fingerprint `clm-a6chi3`; DC-bias calibration rides on $C_{ss}=C_0/S^3$ ([`node-up-small-large-signal.md`](../../vol4/circuit-theory/ch1-vacuum-circuit-analysis/node-up-small-large-signal.md):106, `clm-vca7r1`). **SCOPE:** the ✓ is the parity identity; the *frequency form factor* (χ³ vertex vs ω above band) is **interface-scoped / BULK OPEN** (fork A, PR #610 — bulk beat collapses $\approx$17.1 orders toward skin suppression at sep$\ge$3), NOT part of this identity |
