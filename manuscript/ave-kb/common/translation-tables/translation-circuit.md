...
| **INVARIANT-S2 SYM scaling** | Concurrent $\varepsilon$ and $\mu$ modulation = isotropic gradient-index (impedance-matched, $\Gamma = 0$). Hop $L$ and $C$ **co-scale** (ratio pinned); they do not grade apart. Linear-in-$\varepsilon_{11}$ is Op19, not the Ax4 kernel ([`def-ln3str`](../vocabulary-register.md); [`hop-lc-constitutive-grading.md`](../../vol3/gravity/ch03-macroscopic-relativity/hop-lc-constitutive-grading.md)) | [`alpha-invariance-symmetric-gravity.md`](../../vol3/gravity/ch01-gravity-yield/alpha-invariance-symmetric-gravity.md) |
