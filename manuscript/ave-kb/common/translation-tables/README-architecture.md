...
| [phase space](translation-phase-space.md) | **NEW (2026-08-24)** | NO-CLAIM coordinate-discipline register — every row points at a canonical home carrying that home's own status, and **every row — pointer rows included — carries the three §4 per-row fields** (means-test / Ax3 / provenance) inline in its §2.1 (all consistency-class or pointer, no number bought); **no per-row-field exemption is claimed or presumed** — the diagrammatics EXEMPT ruling is per-spoke and NOT inherited, and whether Grant wants to grant this spoke a pointer-row exemption is his question to rule on in the registering PR; Smith-chart-ontology content is pointer-only at the Grant-PARKED `2026-08-18-smith-chart-cp1-canonization` open item; any future row that MINTS a validated mapping is a register-class change and re-audits |
