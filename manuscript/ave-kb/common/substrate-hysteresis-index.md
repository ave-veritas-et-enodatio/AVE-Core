...
| [`peierls-nabarro-paradox.md`](../vol2/appendices/app-b-paradoxes/peierls-nabarro-paradox.md) (line 12) | The moving electron's leading boundary reaches the dielectric-saturation / **matched-impedance** point ($\Gamma \to 0$, Op17 $T^2 = 1-\Gamma^2 \to 1$) and couples through **reactively** → co-moving zero-impedance phase slipstream; the saturation-biased nodes **store and return reversibly** as the soliton passes (sub-yield, lossless, Axiom 3). Resolves the would-be Bremsstrahlung paradox. *(Sub-yield lossless-reactive — NOT the above-yield thixotropic re-freeze case of §3; entry retained here for cross-reference, not as a hysteresis/dissipative mechanism.)* | clm-ghs75o |
