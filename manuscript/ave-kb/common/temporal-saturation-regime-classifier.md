...
> 🔴 **Value correction 2026-08-03 (Rule 12; the struck `$V_{\text{GW}}/V_{\text{snap}} \sim 10^{-24}$` above is preserved, not deleted).** Recomputed from the leaf's own formula chain on canonical constants (`src/ave/core/constants.py`: `L_NODE` $= \hbar/(m_e c) = 3.8616\times10^{-13}$ m, `V_SNAP` $= 510{,}999$ V), at the leaf's own inputs $h = 10^{-21}$, $f = 100$ Hz:
