...
> **Correction — de-orthogonalization on one leg (collapse-batch T5, KEEP-BOTH; the "all three axes are orthogonal" line above is preserved).** The Power-Domain **θ** axis (axis 2, the phase angle between substrate voltage and current at the LC tank, per [`orbital-friction-paradox.md`](../vol4/circuit-theory/ch1-vacuum-circuit-analysis/orbital-friction-paradox.md)) and the **EM loss tangent** $\tan\delta = \sigma/(\omega\varepsilon)$ referenced above are **NOT independent**: for the LC-tank phasor $Z = R + jX$ they are *complementary angles of the one phasor*, $\tan\delta = R/X = \cot\theta$. So the θ-axis and the EM-$\tan\delta$ reading carry the **same** information read two ways — the "all three axes are orthogonal" claim fails on that leg, and a reader must not treat θ and EM-$\tan\delta$ as carrying independent information. **This does NOT collapse the δ_AVE temporal-duty axis:** $\delta_{\text{AVE}} = t_{\text{sat}}/t_{\text{period}}$ is a **duty-at-saturation ratio**, not $\tan\delta$ — a system can spend a *small* duty fraction at *high* instantaneous loss — so the $\delta_{\text{AVE}}\!\leftrightarrow\!\theta$ tie remains the **taxonomic-not-derivational bridge** flagged above (it does not yet derive numerical values from $S(A)$), NOT a phasor identity. The fireable residue is the $\theta\!\leftrightarrow\!$EM-$\tan\delta$ de-orthogonalization only.
...
- [GW propagation lossless](../vol3/gravity/ch08-gravitational-waves/gw-propagation-lossless.md) — LIGO strain $\sim 10^{-21}$ → $V_{\text{GW}}/V_{\text{snap}} \approx 1.42 \times 10^{-28}$ (~~$\sim 10^{-24}$~~ struck 2026-08-03 per Rule 12 — struck value preserved, see note)

> 🔴 **Value correction 2026-08-03 (Rule 12; the struck `$V_{\text{GW}}/V_{\text{snap}} \sim 10^{-24}$` above is preserved, not deleted).** Recomputed from the leaf's own formula chain on canonical constants (`src/ave/core/constants.py`: `L_NODE` $= \hbar/(m_e c) = 3.8616\times10^{-13}$ m, `V_SNAP` $= 510{,}999$ V), at the leaf's own inputs $h = 10^{-21}$, $f = 100$ Hz:  <!-- rule12-freeze: base=32a51a6ce733030e20de89aa498775fa13c96701 region=above offset=0 lines=354 bytes=32799 sha256=535ef5ce3735433d6ea8e10888328d1ecdc61bc221af33c352205eccba9c2552 -->
> $$V_{GW} = h\,c\,\ell_{node}\,2\pi f = 7.274\times10^{-23}\ \text{V} \quad\Longrightarrow\quad \frac{V_{GW}}{V_{snap}} = 1.423\times10^{-28}.$$
> This reproduces the value **six** corpus sites already carry — `vol6/appendix/geometric-inevitability/derived-numerical-constants.md`:27 (formula **and** value), [`ligo-gw-saturation-ratio.md`](../vol3/gravity/ch08-gravitational-waves/ligo-gw-saturation-ratio.md):15, [`gw-detection-antenna.md`](../vol3/gravity/ch08-gravitational-waves/gw-detection-antenna.md):46, [`ch08-gravitational-waves/index.md`](../vol3/gravity/ch08-gravitational-waves/index.md):23, [`vol3/index.md`](../vol3/index.md):31, [`vol3/gravity/index.md`](../vol3/gravity/index.md):38.
> **Diagnosed cause of the outlier family:** a 3-decade $\ell_{node}$ slip ($3.86\times10^{-10}$ m used for $3.86\times10^{-13}$ m) in the linked leaf's intermediate "$V_{GW}\sim10^{-19}$ V"; the slipped chain gives $7.27\times10^{-20}$ V and a ratio $1.42\times10^{-25}$, which is the $\sim 2\times10^{-25}$ carried in the ch08 print. **NOT edited here:** [`gw-propagation-lossless.md`](../vol3/gravity/ch08-gravitational-waves/gw-propagation-lossless.md):42 and `manuscript/vol_3_macroscopic/chapters/08_gravitational_waves.tex`:61/:355 sit inside the gated ch08 ringdown wave — those fixes are **staged with full arithmetic receipt** at [`_orchestration/docket-entries/2026-08-03-mr-handoff-mechanical.md`](../../../_orchestration/docket-entries/2026-08-03-mr-handoff-mechanical.md), to land with that wave.
