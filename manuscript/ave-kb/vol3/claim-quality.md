...
> - **Reading B — the bracket locates the inner EDGE, and "range" means the cloud's radial extent outward from it.** This is the reading `:334`'s *"within (not on)"* phrasing implies, and under it `7,438.9` does sit outside the inner edge, so *"within"* is satisfied — **but the claim is then vacuous**. The satisfied condition is *"`r_sat` is somewhere beyond ~5,000 AU and inside the cloud"*, a target spanning **more than an order of magnitude**; essentially any `a₀`-derived radius in that decade would have "confirmed" it. A test no plausible value could fail is **not evidence**.
>
...
>
> **⚑ Existence gated by the unadjudicated T4 keying fork.** Whether `r_sat` exists at all depends on **internal-vs-total-field keying** of `S(A) = √(1−A²)`: under **total**-local-field keying the Sun already sits at `g_ext ≈ 2.0–2.1 a₀`, so **no saturation transition exists anywhere in the solar system** (`research/2026-07-10_collapse-target-registry.md`:271–290, which calls the keying *"the unadjudicated hinge — flag-don't-fix, Grant's call"*). **Routed to Grant 2026-08-03; not resolved here.**
>
