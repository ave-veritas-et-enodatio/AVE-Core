...
- _Specific Non-Claims and Caveats_
  ...
  - **[2026-08-02 --- Reading-A bulk admixture]** Does NOT claim the transverse shear channel is the *only* radiated channel. Under **Reading-A** (the standing physics as of 2026-07-20) the framework **additionally** predicts a bulk (longitudinal P-wave) admixture radiating on top of the shear decay at $O(1)$ coupling, $F_{\text{bulk}}/F_{\text{shear}} \approx 0.03$--$0.12$; that admixture is *not* part of the observed shear decay, and its standing status is a **LIVE** pulsar exclusion of an independent $O(1)$ bulk radiative port plus an **OPEN** constituent-cage fork (the deep-rail $k$-scaling adjudicator, #775, returned BIN-3 / form-undetermined). The observed-channel = shear identification in the claim bullets above is **Reading-independent and stands untouched**; the same scoping applies to this entry's own propagation bullets above ("Lossless propagation matches LIGO observation."; "The vacuum acts as a perfect lossless transmission line") --- propagation-side prose, preserved, scoped by this caveat rather than struck. What this caveat records is that the leaves' "matching LIGO observations exactly" / "exactly as observed" completeness framing was struck 2026-08-02 (Rule 12 --- preserved, not deleted). **The bulk/shear double-count contradiction (Q1-REVERT) remains LIVE and routed** ([`common/port-register.md`](../common/port-register.md) line 5) --- Reading-A is the standing physics, not a closure of that contradiction. Ruled state: Grant's F4 → option (a) (`_orchestration/2026-07-10_rulings-docket.md:2573`; `_orchestration/2026-07-20_pending-rulings-and-frontier-queue.md:113`), executed in the printed chapter by PR #771 (`manuscript/vol_3_macroscopic/chapters/08_gravitational_waves.tex:107`); the canonical KB statement is the 2026-08-02 banner in [gw-propagation-lossless](./gravity/ch08-gravitational-waves/gw-propagation-lossless.md). This caveat records an already-ruled state and re-scores nothing --- `confidence` is unchanged at 0.9. 🔴 **[DEMOTED 2026-08-11 — R40-B2a: NEEDS RE-DERIVATION, not dead; dated note at the end of this file]**
...
- _Specific Claims (corrected — channel-split)_
  ...
  - The shear/bulk channels **reflect totally** ($|\Gamma| = 1$): a solid$\to$liquid free surface ($G_{shear} \to 0$) is exactly a $Z_{shear} \to 0$ short. GW (transverse shear) modes therefore reflect off $r_{sat}$ — gravitational ringdown **echoes are predicted** (reflect $\Rightarrow$ echo; retrospective, no SHA-pinned forward prereg yet). The $\Gamma \to -1$ mechanism is **not electron-only** — it is the universal $Z \to 0$ short applied in the shear/bulk channels here; the electron applies it in the bulk channel at the knot core. **★ THE ECHO STATEMENT RE-FORMS 2026-08-05 (upgrade wave) — predicted in BOTH density branches, with fork-discriminating certified timing, and the mirror scoped to strain-kernel channels.** This bullet's *"echoes are predicted"* was recorded as **RHO-A-conditional** at [`research/2026-08-04_coldq-axial-rhob_result.md`](../../../research/2026-08-04_coldq-axial-rhob_result.md):332. **It is no longer.** (1) **The mirror is density-profile-independent — measured, not argued:** the RHO-A$-$RHO-B separation of the last-bond reflection is `0.0` *exactly* (`G-COLD`, PR #888), and the ruled scope is the channel-scoped kernel-collapse ruling ([`2026-08-05-ruling-kernel-collapse-rescope.md`](../../../_orchestration/docket-entries/2026-08-05-ruling-kernel-collapse-rescope.md)`:10`–`:21`, PR #897), so *reflect $\Rightarrow$ echoes are predicted* is asserted on **both** branches — **scoped to the strain-kernel channels**, with the rotational channel carved out (unwalled at $r_{sat}$ — **conditional on the separate-kernel (L∞-across-grades) member; the cross-grade combine rule is canon-OPEN at $O(\alpha)$ ([`common/axiom-register.md`](../common/axiom-register.md):190,:232) and on the normalized-L2 member the carve-out does not stand. Fence added by the doc lane 2026-08-05, not in #897's ruled text, routed to Grant**). (2) **The RHO-B delay is a REGULATED FINITE log law on the lattice, and it is certified:** `CFG-A` **and** `CFG-B` are both **`DELAY-CERTIFIED`** and **`BIN-DB-NODE`** is adjudicated — node-governed, achromatic, the chirped branch does not fire ([`research/2026-08-05_echo-delay-v2-reach-through_result.md`](../../../research/2026-08-05_echo-delay-v2-reach-through_result.md), PR #885). The continuum's *"infinite optical distance"* reading is **one half of the `BIN-C-DISJOINT` carve**: what diverges is the $\ell_{node} \to 0$ limit; at finite $\ell_{node}$ the lattice delay is **finite and log-enhanced** (PR #888). (3) **The fork is echo-DISCRIMINABLE BY TIMING:** `BIN-DISC` **FIRES** — $\lvert T_B - T_A\rvert$ exceeds one substrate-native ringdown damping time at **every** mass on the frozen grid, by `38.6` to `43.2`. **What did NOT change and must ride with every quotation of this bullet:** `BIN-DISC` **does not claim an echo EXISTS** (that needs a reflectivity at an outer plane, which its own lane could not pose well — `FLAG-PLANE-RT`, open); **`FLAG-ECO` applies in full** (the RHO-B log law is structurally degenerate with the ECO family, whose cutoff length is a free knob); **neither branch is preferred and FORK-3(b) is open**; the `0.29` s Abedi–Dykaar–Afshordi spacing is **still not an AVE validation** and both branches miss it; and the discrete wall row is **`ROW-NOT-CERTIFIED`** pending the named `G-RHO2` repair while the three load-bearing theorems are measured exact. ⚑ **Surfaced, NOT fixed in this wave:** the *"retrospective, no SHA-pinned forward prereg yet"* qualifier earlier in this same bullet is **stale on its face** — a SHA-pinned forward prereg exists and is cited from the Vol 4 signature leaf — but no record in this wave licenses re-grading the echo claim, so it is left standing and routed to auditor / Grant. **⚑ CURRENCY 2026-08-06 — WHICH RUN THIS QUOTES (doc-lane refresh pass).** The `ROW-NOT-CERTIFIED` state above is the **v1 run's** verdict and stands as that run's true measurement — `research/2026-08-05_last-bond-kernel-collapse_result.md`:24, *"`G-RHO2` FAILS on an injection point this lane sized wrong at freeze"*. **It is superseded as the CURRENT state:** the repair that v1's own §1.3 named — inject `k_0 = ε·ω·Z_1`, not `ε·k_cold` — has run, and TASK 2 is **`ROW-CERTIFIED`**. Read from the landed result document itself, not from a PR title: `research/2026-08-05_last-bond-g-rho2-rerun_result.md`:30, *"`G-RHO2` PASSES. TASK 2 of the last-bond lane is `ROW-CERTIFIED`."* — fitted exponent inside the **unchanged** v1 acceptance interval (PR `#902`, merge commit `b06cbeb1`). **What this does NOT do:** a certified **row** does not certify the **premise scan**, which stays `SCAN-NOT-CERTIFIED` with no bin adjudicated; and it licenses no compressed *"confirmed"* wording here — that stays docket-only. **⚑ FENCE RE-POINT, 2026-08-06 — this routing note now points at the v2 record.** The citable ruled text is the versioned re-issue [`2026-08-06-ruling-kernel-collapse-rescope-v2.md`](../../../_orchestration/docket-entries/2026-08-06-ruling-kernel-collapse-rescope-v2.md):13–29, which **carries the cross-grade combine-member fence inside the ruled text itself**; the 2026-08-05 v1 record is preserved and gains a dated pointer to it. **The earlier *"the omission is at the RULING … routed to Grant for a possible re-issue"* language is RESOLVED — the re-issue happened**, so the fence is now carried AT THE RULING and no print site has to supply it. **Nothing about the physics moves:** the carve-out is still conditional on the per-grade (L∞-across-grades) member, and the cross-grade combine rule is still canon-OPEN. Delta declaration (three deltas from v1, all declared) and the CORRECTED engine-residence map: [`2026-08-06-ruling-kernel-collapse-rescope-v2-correction.md`](../../../_orchestration/docket-entries/2026-08-06-ruling-kernel-collapse-rescope-v2-correction.md) C1/C2 — the engine codes the saturation amplitude **three** ways across two live functionals plus a separate objective, so *"the member the engine actually codes"* is over-broad; the carve-out's receipt is STRUCTURAL, not numerical.
...
- _Specific Non-Claims and Caveats (corrected — channel-split)_
  ...
  - Does claim the **shear/bulk** channels reflect ($\Gamma_{shear} = \Gamma_{bulk} = -1$): GW/shear echoes are predicted. This is **not** the electron mechanism imported — it is the same universal $Z \to 0$ short, channel-resolved (electron: bulk channel at knot core; BH: shear+bulk channels at $r_{sat}$). **⚑ SCOPE AND PLANE, 2026-08-05 (upgrade wave):** the prediction now stands in **both** density branches (see the re-form on `:123`), but it is **scoped to the strain-kernel channels** — the Cosserat rotational channel is **carved out** (unwalled at $r_{sat}$; PR #890, and the ruled scope at [`2026-08-05-ruling-kernel-collapse-rescope.md`](../../../_orchestration/docket-entries/2026-08-05-ruling-kernel-collapse-rescope.md)`:14`–`:15` — **conditional on the separate-kernel (L∞-across-grades) member; the cross-grade combine rule is canon-OPEN at $O(\alpha)$, [`common/axiom-register.md`](../common/axiom-register.md):190,:232, and the fence is NOT in #897's ruled text — added by the doc lane 2026-08-05 and routed to Grant**) — and any **signed** $\Gamma$ quoted from this bullet still travels only with its **reference plane** (`G-PLANE` measures the full $2.0$ inversion across a one-node plane shift). Canonical: [`wall-taxonomy.md`](../common/wall-taxonomy.md) §10.2. **⚑ FENCE RE-POINT, 2026-08-06 — this routing note now points at the v2 record.** The citable ruled text is the versioned re-issue [`2026-08-06-ruling-kernel-collapse-rescope-v2.md`](../../../_orchestration/docket-entries/2026-08-06-ruling-kernel-collapse-rescope-v2.md):13–29, which **carries the cross-grade combine-member fence inside the ruled text itself**; the 2026-08-05 v1 record is preserved and gains a dated pointer to it. **The earlier *"the omission is at the RULING … routed to Grant for a possible re-issue"* language is RESOLVED — the re-issue happened**, so the fence is now carried AT THE RULING and no print site has to supply it. **Nothing about the physics moves:** the carve-out is still conditional on the per-grade (L∞-across-grades) member, and the cross-grade combine rule is still canon-OPEN. Delta declaration (three deltas from v1, all declared) and the CORRECTED engine-residence map: [`2026-08-06-ruling-kernel-collapse-rescope-v2-correction.md`](../../../_orchestration/docket-entries/2026-08-06-ruling-kernel-collapse-rescope-v2-correction.md) C1/C2 — the engine codes the saturation amplitude **three** ways across two live functionals plus a separate objective, so *"the member the engine actually codes"* is over-broad; the carve-out's receipt is STRUCTURAL, not numerical.
...
- _Specific Claims_
  - The Schwarzschild $\ell = 2$ fundamental QNM chain runs $r_{sat} = 7M_g$ (Axiom 4), $r_{eff} = r_{sat}/(1 + \nu_{vac}) = 49M_g/9$ (Poisson), $\omega_R = \ell c/r_{eff}$ (mode). The arithmetic closes with no fitted parameter, but it is **NOT a zero-free-parameter derivation of the eigenfrequency** — that provenance is falsified by a frozen prereg rider, and the card grades the result a **consistency check** (see the line below). 🔴 **Brought into line with :199 on 2026-09-07 under Grant's ruling, verbatim: *"the flip is in scope, propagate it to :198 and :217"*. The card previously read:** *"The Schwarzschild $\ell = 2$ fundamental QNM is derived from Axioms 1–4 with **zero free parameters**: $r_{sat} = 7M_g$ (Axiom 4), $r_{eff} = r_{sat}/(1 + \nu_{vac}) = 49M_g/9$ (Poisson), $\omega_R = \ell c/r_{eff}$ (mode)."* (Rule 12 / R7 2026-08-04: the rendered current-status sentence is rewritten and the prior wording is carried verbatim here.) **Receipt for the substrate reason:** rider `RD-NUVAC-EIGENFREQ`, **FIRED 2026-08-03**, in `_orchestration/fired-riders.json` — a file that is on `main` but **not yet on this branch's base `6b8b49a0`**, so it is quoted here from `main`. Its `measured` field, verbatim: *"omega_R*M_g = 0.2648078872629827 vs 18/49 = 0.3673469387755102 -> -27.91% against a frozen 3% trip level"*, against a **frozen 3% trip level**; its `falsified_chain` field, verbatim: *"r_eff = r_sat/(1+nu_vac) = 49 M_g/9 AS A DERIVATION OF the l=2 eigenfrequency."* That is exactly the middle step this line recites. ★ **SCOPE OF THE RIDER — not overstated.** What fired is that step's standing **as a derivation of the eigenfrequency**. The arithmetic is not disputed and the card is not retracted: the same rider row states, verbatim, *"The VALUE 18/49 is untouched (still -1.69% from GR exact); what is falsified is its PROVENANCE as a zero-free-parameter derivation."* ★ **CATEGORY ONLY — `confidence`, `solidity` and `build_band` are UNTOUCHED here**, exactly as at :199.
  - Schwarzschild value: $0.3673$ vs GR exact $0.3737$, **error 1.7%** — graded a **consistency check**. 🔴 **Category flipped 2026-09-06; the card previously read "a category (iv) derived prediction, not an identity or consistency check"** (Rule 12 / R7 2026-08-04: the rendered current-status note stays, the verbatim prior wording is not duplicated in a comment — the trail lives in version control). Authority: Grant's ringdown-fork ruling, `_orchestration/docket-entries/2026-08-19-ringdown-fork-resolved.md`:11, verbatim — *"The row re-types to consistency_check on the P42 basis like its"* 17 siblings. The machine-readable sibling already carries it: `manuscript/consistency-manifest.yaml` row `id: P_A034_bh_ringdown` / `clm: clm-395gps` reads `type: consistency_check`. ★ **CATEGORY ONLY — `confidence`, `solidity` and `build_band` are UNTOUCHED here.** Re-banding is a separate decision and is not ruled; and it could not be done by hand anyway — `manuscript/ave-kb/.index/SCHEMA.md`:212, verbatim: *"**Solidity is derived, not parsed.**"* (computed by `kb_index_lib.compute_solidity`).
  - Quality factor $Q = \ell$ (so $Q = 2$ for $\ell = 2$); $\omega_I M = 9/98$ vs GR $0.0890$, error 3.2%.
- _Specific Non-Claims and Caveats_
  - Does NOT claim sub-percent agreement on ringdown frequency for spinning remnants. The Kerr-corrected LIGO ringdown comparisons (GW150914, GW170104, GW151226) show **10–18%** frequency error and 10–14% decay-time error, not the sub-percent precision the Schwarzschild case suggests. **🔴 Corrected 2026-07-21 (Ruling B1; original preserved per Rule 12).** The "10–18%" is the **frame-mixed pre-correction artifact** (#774: v1 frequency at source-frame mass vs detector-frame $f_{obs}$ against a corrupt Kerr table), not a dimensionless comparison to true Kerr. Honest per-mapping numbers vs corrected Kerr (frame-independent C-1 comparator): the **standing v1 mapping** (re-selected under Ruling B1) sits **ω_R +2.63% mean** on the primary catalog (a\*=0.64–0.74, inside the |D̄|<3% band) / **+3.36%** secondary (marginal, 3–5%), with **τ a −5.4% open near-miss** (Model-B mΩ law: −5.44% Resultbox / −4.57% ZAMO); the retired **v2 mapping** sits **−9.53%** on ω_R. Mapping-conditional, consistency-class, disclosed-phenomenological — NOT sub-percent, NOT a zero-free-parameter benchmark (only the cold 18/49 eigenvalue is). Refs: [ave-merger-ringdown-eigenvalue § GRANT RULING B1](./cosmology/ch15-black-hole-orbitals/ave-merger-ringdown-eigenvalue.md), `research/2026-07-20_{kerr-table-correction,v1-spin-mapping-adjudication}_result.md`.
  ...
  - ~~The Kerr quality factor matches GR sub-2% only for $a_* = 0.3$–$0.8$; behaviour outside this spin range is not validated in the leaf.~~ **🔴 RETRACTED 2026-07-21 (Ruling B1; original struck, preserved per Rule 12).** The "sub-2%" Kerr-Q match was computed against the corrupt ω_R/ω_I tables + source-frame masses (#774). Corrected picture: the topological **flat $Q = \ell = 2$ is scoped to the a\*=0 cold anchor** (the Ω→0 limit of the mΩ law); at catalog spins it would **fail at $\bar D_Q = -38\%$** (corrected-Kerr Q rises 3.07→3.49). The **spin-refined mΩ law** $\omega_I = (\omega_R - m\Omega)/(2\ell)$ under the standing v1 mapping instead lands at **$-5.44\%$** (Resultbox) / $-4.57\%$ (ZAMO) — an OPEN near-miss tension, the named next ringdown work, NOT a sub-2% match.
...
- rationale: The Schwarzschild $\ell=2$ eigenvalue $18/49 \approx 0.3673$ closes end-to-end from three axiom-grounded steps ($r_{sat} = 7M_g$, $r_{eff} = 49M_g/9$ via Poisson, $\omega_R = \ell c/r_{eff}$) with zero free parameters — but the card grades that closure a **consistency check**, not a category-(iv) derived prediction (see :199, and the 2026-09-07 note at the end of this rationale). Band held below 0.9 because the Kerr extension rests on a **disclosed phenomenological** photon-sphere shift + Cosserat back-reaction fit (v2, refined post-hoc against LIGO), and $Q = \ell$ disagrees with GR overtone structure for $\ell > 2$. The Schwarzschild core is clean; the spinning-remnant comparisons (10–18% pre-Kerr-correction) are the disclosed weak edge. **🔴 Corrected 2026-07-21 (Ruling B1; sentence above preserved per Rule 12).** The "10–18% pre-Kerr-correction" is a **frame-mixed artifact** (#774), not a true-Kerr comparison. Under Ruling B1 the **standing spin mapping is v1** (re-selected over v2), a **consistency-class, mapping-conditional, disclosed-phenomenological** match: ω_R **+2.63% mean** (primary catalog, inside |D̄|<3%) with **τ a −5.4% open near-miss tension** (mΩ law) — the disclosed weak edge is now the τ near-miss + the phenomenological photon-sphere-shift provenance, NOT a 10–18% frequency error. **Solidity 0.55 UNCHANGED** — anchored by the cold 18/49 (−1.69%); the spinning match is consistency-class and does not lift the band either way. **🔴 Category brought into line with :199 on 2026-09-07 under Grant's ruling, verbatim: *"the flip is in scope, propagate it to :198 and :217"*. Prior wording, preserved verbatim per Rule 12 / R7 2026-08-04:** *"with zero free parameters — a genuine 1.7%-error category-(iv) derived prediction."* **The substrate reason is a fired prereg rider, not a re-reading.** `RD-NUVAC-EIGENFREQ` (`_orchestration/fired-riders.json`, on `main`; not yet on this branch's base) **FIRED 2026-08-03** at **−27.91%** against a **frozen 3% trip level**, falsifying — verbatim — *"r_eff = r_sat/(1+nu_vac) = 49 M_g/9 AS A DERIVATION OF the l=2 eigenfrequency."* That is precisely the three-step chain this rationale recites as its derivation. ★ **Scope, not overstated:** the rider hits that step's standing **as a derivation of the eigenfrequency** — not the arithmetic, and not the whole card. The rider's own row records *"The VALUE 18/49 is untouched (still -1.69% from GR exact)"*, and the −1.69% cold anchor named above is unchanged. ★ **`confidence: 0.8`, `solidity: 0.55` and `build_band` are UNTOUCHED by this edit.** Re-banding is a separate decision reserved to its own Grant ruling with an auditor lane preparing it; and it could not be done by hand in any case — `manuscript/ave-kb/.index/SCHEMA.md`:212, verbatim: *"**Solidity is derived, not parsed.**"* (`solidity`, `build_status` and `build_band` are computed by `kb_index_lib.compute_solidity` from the hand-authored `confidence` and the depends-on DAG).
- strengthen-by:
...
- _Specific Claims_
  ...
  - The galactic MOND form $g_{eff} = g_N + \sqrt{g_N a_0}\sqrt{1 - (g_N/a_0)^2}$ is the Axiom 4 saturation operator with $g_N$ as the saturation amplitude and $a_0$ as the yield limit. This is a category (iv) derived prediction. (**Form corrected 2026-07-20** to the QUADRATIC $\sqrt{1-(g_N/a_0)^2}$ per the MOND kernel adjudication: it matches the shipped engine AND this entry's own "$g_N$ is the amplitude, $a_0$ the yield" identification under $S(A)=\sqrt{1-(A/A_\text{yield})^2}$; the two-kernel SPARC contrast was degenerate on Q=1 mean|res|, so the form-level derivation decides — `research/2026-07-20_mond-kernel-adjudication_result.md`.)
    - **Amplitude-identification tag (2026-07-20, #754 R1 qualifier).** The kernel-amplitude identification $A = g_N/a_0$ (which selects the QUADRATIC form) is **substrate-consistent / ANCHOR-FORCED within the established gravity-as-refraction correspondence** — NOT an unconditional "derived." The A1-amplitude trace (`research/2026-07-20_a1-amplitude-trace_result.md`, merged #754) upgrades it **leaf-asserted → substrate-consistent**, resting on one corpus-native mapping: $g_N$ is the A1-tank's DC field-bias (the $E_\text{local}$-analog, both $= -\nabla(\text{potential})$), with $a_0$ the yield field. That **$g_N \leftrightarrow E_\text{local}$ leg is analogy-level / correspondence-transported** (the corpus derives $\Phi \leftrightarrow n$ for the metric channel; the field↔field MOND-amplitude leg is inferred, not independently traced). Under that identification $A = g_N/a_0$ is forced with no free alternative at a DC operating point — but the tag is **anchor-forced-within-the-correspondence**, not stand-alone derived. The $a_0$ VALUE stays Class-E (operating-point projection); the FORM/amplitude is anchor-forced; the framework's usual FORM-derived / VALUE-imported split holds.
...
> **🔴 OORT CONTAINMENT SUB-CLAIM RETRACTED + RELABELED — 2026-08-03 (Rule 12: every claim, caveat, grade and rationale line above and below is PRESERVED UNEDITED; this note recasts their status). Propagation of the merged 2026-07-19 deep-space reactive-bulk MECHANISM-RETRACTED ruling into Vol 3, which never received the banner Vol 1 and Vol 4 have carried since that date. NOT new adjudication.**
>
> **Scope fence first (flag-don't-fix):** this touches the **Oort sub-claim only**. The **Kirkwood** sub-claim (`:319`, `:323`) and the **magnetopause** sub-claim (`:320`, `:322`) are untouched and unaffected.
>
> **`:318` — RETRACTED in its containment half.** Verbatim: *"Oort Cloud inner edge: $r_{sat} = \sqrt{GM_\odot/a_0} \approx 7{,}400$ AU — the radius where solar $g_N$ drops to the MOND threshold. **Falls within the observed Hills Cloud range (2,000–5,000 AU)**; reported as 'consistent with' rather than as a sub-percent match."* The **formula and its value survive**; the **containment assertion does not**. The only bridge in this corpus from a *field isocline* (`g_N = a₀`) to a *population edge* is the dissipative stall, and that stall is **Axiom-3 forbidden** — `vol1/dynamics/ch4-continuum-electrodynamics/magnetic-saturation.md`:34 rules verbatim *"any belt/Oort **position** derived **from** that kinetic-energy loss does not survive as a bulk-dissipation result"* and *"The Asteroid-Belt / Oort-Cloud prediction is **MECHANISM-RETRACTED**"*. **Correct relabel:** `r_sat = 7,438.9 AU` is **the radius at which the Axiom-4 kernel turns on for the solar monopole under internal-field keying — the outer edge of the Newtonian region.** A solar-**field** property; **no claim about where any body sits**. Note also that hedging it as *"consistent with"* does not rescue it: a claim with no mechanism connecting its two sides is not weakly supported, it is **unsupported**.
> **The word "observed" is false**: no object has ever been observed occupying the 2,000–5,000 AU band; the bracket is a **simulation-inferred convention** with roughly factor-10 author spread and is **uncited in this corpus** (`manuscript/bibliography.bib`: **zero** *oort* / *hills* / *comet* entries, two methods). Deliberately **not repaired by adding a citation** — sourcing it sharpens the comparand mismatch rather than relieving it. Per **A47 v11b**, the slot is **NOT refilled**: no replacement bracket, no successor containment claim.
>
> **`:324` — still true, but no longer the binding caveat.** Verbatim: *"Oort Cloud derivation depends on the $a_0$ prediction (which itself carries 10.7% systematic deficit; see MOND entry)."* Accurate, and the arithmetic is now stated explicitly: `r ∝ a₀^(-1/2)`, so the 10.7% `a₀` deficit (`:259`) propagates to a **-5.5% a₀-provenance band** on `r_sat` (7,030.7 AU on the empirical `a₀ = 1.2e-10`). **But the binding limitation is not accuracy — it is that there is no longer any claimed observable to be accurate *about*.** A pin test now carries this band explicitly: `src/tests/test_solar_impedance.py::TestSolarAxiom4OnsetRadius::test_saturation_radius_pin`.
>
> **`:355` — the "lands only within (not on)" hedge is itself WRONG and is retracted.** Verbatim: *"Oort inner edge $r = \sqrt{GM_\odot/a_0} \approx 7400$ AU is a clean one-line formula but inherits the disclosed 10.7% $a_0$ deficit and **lands only 'within' (not on) the observed Hills range**."* This sentence grades the result as a **weak-but-real** match, on a spectrum running *lands-on → lands-within → misses*. **That spectrum does not apply**, and the source sentence it is grading is **ambiguous between two readings — the hedge fails under both, for different reasons.** The Vol-3 print/KB sentence reads *"falls within the observed Hills Cloud **range** (2,000–5,000 AU **inner edge**)"*:
> - **Reading A — the bracket IS the target range.** Then `7,438.9` **does not fall within it**: it is **~1.5× above the 5,000 AU upper end**. The claim is simply false, and *"lands within"* is factually wrong about its own arithmetic.
> - **Reading B — the bracket locates the inner EDGE, and "range" means the cloud's radial extent outward from it.** This is the reading `:355`'s *"within (not on)"* phrasing implies, and under it `7,438.9` does sit outside the inner edge, so *"within"* is satisfied — **but the claim is then vacuous**. The satisfied condition is *"`r_sat` is somewhere beyond ~5,000 AU and inside the cloud"*, a target spanning **more than an order of magnitude**; essentially any `a₀`-derived radius in that decade would have "confirmed" it. A test no plausible value could fail is **not evidence**.
>
> **Neither reading is adjudicated here** — the ambiguity is surfaced, not resolved, because it does not need resolving: **(i)** the claim fails under A as false and under B as non-discriminating; and **(ii)** independently of both, the two quantities are **not comparands** — `r_sat` is an **instantaneous force-balance isocline**, while the Hills inner edge is a **4.6-Gyr emplacement-timescale boundary** contingent on the entire stellar-encounter history, with a torquing rate scaling as `a^{7/2}`. **Agreement between a force-balance radius and a dynamical-history radius would be a coincidence, not a confirmation.**
>
> **⚑ Existence gated by the unadjudicated T4 keying fork.** Whether `r_sat` exists at all depends on **internal-vs-total-field keying** of `S(A) = √(1−A²)`: under **total**-local-field keying the Sun already sits at `g_ext ≈ 1.8–2.0 a₀`, so **no saturation transition exists anywhere in the solar system** (`research/2026-07-10_collapse-target-registry.md`:271–290, which calls the keying *"the unadjudicated hinge — flag-don't-fix, Grant's call"*). **Routed to Grant 2026-08-03; not resolved here.** *(★ Ratio corrected in the 2026-08-03 repair pass: this sentence shipped as `≈ 2.0–2.1 a₀`, which read the **mantissa** of `g_ext ≈ 2.1×10⁻¹⁰ m/s²` as though it were a ratio. The registry's own arithmetic at `:282`–`:283` gives* **`2.1/1.07 = 1.96`** *against the AVE-derived `a₀` and* **`2.1/1.2 = 1.75`** *against the empirical `a₀ = 1.2×10⁻¹⁰` — hence `1.8–2.0`, which is also the figure four merged corpus sites already print (`research/2026-07-11_keying-register-walk_framing.md`:81, `research/2026-07-11_astro-adjudicator-sweep_result.md`:66, `research/2026-07-11_astro-adjudicator-sweep_branch-signature-map_FROZEN.md`:40, `_orchestration/2026-07-11_astro-adjudicator-sweep-handoff.md`:49). **The physics direction is unchanged:** even the smaller ratio, `1.75 a₀`, exceeds `a₀`, so total-field keying still removes the transition from the solar system.)*
>
> **⚑ Cross-volume number conflict — flagged, NOT reconciled.** **Vol 3 books `r_sat ≈ 7,439 AU`; Vol 4 books `~15,000 AU` for the claimed same boundary; one-object-or-two routed to Grant 2026-08-03** ([`vol4/claim-quality.md`](../vol4/claim-quality.md):1123, `boundary-trapping-test.md`:25, `vol_4_engineering/chapters/11_experimental_falsification.tex`:370). Factor 2.0 apart — outside the -5.5% provenance band, so not a precision difference.
>
> **GRADES DELIBERATELY UNMOVED — and flagged as owed.** `confidence: 0.6` and `solidity: 0.55` at `:350`/`:354` are **unchanged**, and no id is minted. This lane is **mechanical propagation of a merged ruling**, and re-banding a claim is **adjudication**, which is not propagation's to do. ⚑ **But the rationale at `:355` explicitly averages the Oort sub-result INTO this band** (*"averaged down for the heterogeneity"*), so the retraction of one of its three legs plausibly moves it. **Re-banding of `clm-3kmt3p` is ROUTED, not decided.** Docket: [`2026-08-03-oort-walkback-propagation.md`](../../../_orchestration/docket-entries/2026-08-03-oort-walkback-propagation.md).
...
> **🔴 DEMOTED 2026-07-19 (deep-space reactive-bulk ruling, Rule-12 — the claims/caveats above are PRESERVED unedited; this note records current status).** The "Lunar Inductive **Joule** Heating" framing is demoted: $P_{topo} \approx 1.04$ TW is a **dissipated power computed from a loss-Q** ($k_2/Q$, $Q\approx38$) and relabelled a *lattice* Joule loss, but a lossless bulk cannot Joule-heat (Axiom 3 — `../common_equations/eq_axiom_3.tex`:24 "never a bulk resistive one"; licensed loss channels = radiative-port / boundary-Joule-at-a-port / Regime-IV rupture, none a sub-yield bulk stall, `common/substrate-native-terminology.md`:31). Per the ruling (Grant, in-chat 2026-07-19, verbatim, *sic*): *"it rings but i think theres a bulk reaction from the lattice that makes it lossless/pure reactance … dofferent passpa danof frequencies for effects, like the rings of saturn vs electron orbitals."* Consequences: **(i)** the bulk-dissipation framing is DEMOTED (Ax3-forbidden); **(ii)** the $1.04$ TW magnitude does not survive as a lattice-Joule result (this entry was already solidity **0.20**, "do not build on"); **(iii)** real lunar heating is *material* tidal dissipation ($k_2/Q$ in the Moon), not vacuum-bulk Joule; whether the $1$–$2$ TW is recoverable via reactive VAR-exchange is a **SPEC'd, not run**, Grant-gated re-derivation. The dependency claims `clm-a71inj` (flyby Sagnac) and `clm-k3p9wz` ($\Gamma_{sagnac}=m_p/m_e$) are reactive phase-shift claims **not** demoted by this ruling. Walk-record: `research/2026-07-19_deep-space-reactive-bulk-walk_RECORD.md`. Regime-IV audit item 90: `research/2026-07-17_regime-iv-dissipation-audit.md`:126. Leaf banner: `lunar-inductive-heating.md` (same date). Solidity unchanged (already floor-band); this is a status/framing demotion, not a re-score.
...
The substrate-mechanism class for the CMB-thermal-running of $\alpha$ ($\delta_{strain} \approx 2.22 \times 10^{-6}$ at $T_{CMB}$), identified via `ave-ee-first-mapping` v1.0 + Grant 2026-05-28 adjudication. Distinct from the magnitude-back-subtraction at clm-009nkt (Vol 1) — this entry is the substrate-mechanism identification; clm-009nkt is the observable identity.

- _Specific Claims_
  - Substrate carries bipartite thermal-mode structure per Ax 1: 3 translational E-DOFs/node (gapless acoustic, thermally populated at any $T > 0$) + 3 microrotational B-DOFs/node (Cosserat couple-stress mass-gap $\omega_m^2 = 4 G_c / I_\omega$ per canonical [`trampoline-framework.md:192`](../common/trampoline-framework.md) — `m_\omega^2 = 4 G_c / I_\omega`; $\omega_m \sim 1$ MeV in physical units). 🔴 **[DEMOTED 2026-08-11 — R40-B2a: NEEDS RE-DERIVATION, not dead; dated note at the end of this file]**
...
- **`:83`** — stamped at `:83`. *(family: Reading-A register row)*  ⚑ **BIAS-DEBT**
  Quoted claim (content verified at HEAD; markup-reduced from the banked audit):
  ```text
  additionally predicts a bulk (longitudinal P-wave) admixture radiating on top of the shear decay at O(1) coupling; that admixture is not part of the observed shear decay
  ```
  Audited rationale, verbatim from the banked worklist:
  ```text
  Register/ledger row recording the imported reading's standing state (LIVE pulsar exclusion + OPEN constituent-cage fork) — the bin definition's named register-row class; a routed row update is owed, not a leaf edit.
  ```
...
- **`:122`** — stamped at `:122`. *(family: Z_bulk = ρc_bulk formula (register))*  ⚑ **BIAS-DEBT**
  Quoted claim (content verified at HEAD; markup-reduced from the banked audit):
  ```text
  and likewise Z_bulk → 0 ⇒ Γ_bulk = -1 at the dielectric rupture
  ```
  Audited rationale, verbatim from the banked worklist:
  ```text
  Register entry derives the bulk wall through the Z = ρc chain (formula-level phantom); the co-located 2026-08-05 repaired form in the same entry ('does not run through Z = ρc at all' — last-bond kernel disconnect, density-independent) is ALREADY carve-compatible and shows what the re-derivation looks like; row update owed, not a re-scoring.
  ```

  **Resolution.** The demoted carrier is the propagating A1 / bulk branch; under Axiom 5 clause G that slot is the **bound response**, so the re-derivation must be re-posed on the bound-sector constitutive law (bias $\varepsilon_{11}$, bound response $\mathbf{u}_0$, mechanism gloss back-reaction) rather than on a compression wave. **⚑ BIAS-DEBT:** this row's re-derivation turns on finite-speed bias dynamics, so the resolution is the ratified axiom **with THE BIAS PROPAGATION THEOREM standing** (clause (c1)) — the replacement is *owed, not held*.

- **`:1220`** — stamped at `:1220`. *(family: thermal bulk band-branch (register); banked `uncertain`)*  ⚑ **BIAS-DEBT**
  Quoted claim (content verified at HEAD; markup-reduced from the banked audit):
  ```text
  3 translational E-DOFs/node (gapless acoustic, thermally populated at any T > 0)
  ```
  Audited rationale, verbatim from the banked worklist:
  ```text
  UNCERTAIN — register mirror of delta-strain-cosmic-tcc.md:34 (same branch-count consumption, same owed DOS re-derivation, same low blast radius given the FT-1 negative closure). One routed fix covers both sites.
  ```
