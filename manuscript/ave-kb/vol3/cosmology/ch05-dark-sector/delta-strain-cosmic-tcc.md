...
- **3 translational E-DOFs** at each K4 node — bond-stretching modes, capacitive storage, source of $\mathbf{E}$ field. Spectrum: gapless acoustic at long wavelength; dispersion $\omega(k) = c_E |k|$ as $|k| \to 0$. Thermal occupation at any $T > 0$. 🔴 **[DEMOTED 2026-08-11 — R40-B2a: NEEDS RE-DERIVATION, not dead; dated note at the end of this file]**
...
$$\delta_{strain} \approx \frac{\eta_\varepsilon}{2} \approx 2.22 \times 10^{-6}$$
...
> **Scope (FT-1, 2026-05-31 — magnitude closed-negative).** The substrate-distinct content of this forward prediction is the **SIGN / existence** of a thermal running, not its magnitude. The $\delta_{strain}(T_{CMB}) \approx 2.22 \times 10^{-6}$ anchor is a **definitional residual** ($1 -$ CODATA$/\alpha_{cold}$), not a thermally-derived observable: the candidate substrate derivation of the genuine thermal drift (Q-DELTA-MAP-1-quant) was **closed negative** (~31 OOM undershoot), so the genuine thermal α-drift is $\sim 10^{-38}$ — **unobservable**. The linear-$T$ scaling and the quasar / collider handles below therefore test the **sign** of the mechanism, **not** a ~2 ppm magnitude (which the $\sim 10^{-38}$ genuine drift cannot support). The 2.2 ppm *magnitude* is the loaded-resonator specification gap (cold vs CODATA in-situ charge-port α), per the Summary's 2026-06-25 Open-C reframe — not a thermal observable. See the Summary + §5/§7 and `vol1/claim-quality.md:105` (clm-009nkt).
...
- **`:34`** — stamped at `:34`. *(family: thermal bulk band-branch; banked `uncertain`)*  ⚑ **BIAS-DEBT**
  ...
  ```text
  UNCERTAIN — included per err-toward-inclusion. Counting all 3 translational branches as gapless propagating acoustic modes with thermal population includes the longitudinal band-branch the carve removes from the spectrum; the E-vs-B gap-asymmetry SIGN mechanism survives, but the mode-count/DOS statement is owed a 2-transverse+constrained re-derivation. Low blast radius: the quantitative route already CLOSED NEGATIVE (FT-1, ~31 OOM).
  ```
