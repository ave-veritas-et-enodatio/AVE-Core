...
1. AVE writes $H_\infty$ in terms of $m_e$, $\alpha$, $G$ (corpus, closed, `consistency-manifest.yaml` entry P23 at 0.7% off TRGB). Classification: **Class E operating-point projection** at $u_0^* \approx 0.187$ — joint-constrained with $\{G, \hat{\Omega}_{\text{freeze}}, \alpha\}$ via the $R_H/\ell_{\text{node}} \sim 10^{39}$ (precisely $\approx 3.456\times10^{38}$) topological bridge per [`omega-freeze-cosmic-grain-cascade.md:11`](../../../common/omega-freeze-cosmic-grain-cascade.md) — *"the substrate's magic-angle operating point"* — not an independent single-observable prediction.
