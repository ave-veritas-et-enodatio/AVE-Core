...
3. ~~V_0 ≠ 0 substrate DC reactive operating point — currently V_0 → 0 assumed~~ **FURTHER PARTIAL CLOSURE 2026-05-26 Phase 2-NA ax4-saturation epic** per [`research/2026-05-26_ax4-saturation-phase-2-na-aperture-aggregate-result.md`](../../../../../research/2026-05-26_ax4-saturation-phase-2-na-aperture-aggregate-result.md) + [`parametric-coupling-kernel.md` §14](../../../vol4/circuit-theory/ch1-vacuum-circuit-analysis/parametric-coupling-kernel.md) (aperture-aggregate prediction); per Phase 0c ax4-saturation epic [`research/2026-05-26_ax4-saturation-phase-0c-pdelta-v-derivation-result.md`](../../../../../research/2026-05-26_ax4-saturation-phase-0c-pdelta-v-derivation-result.md) + [`parametric-coupling-kernel.md` §13](../../../vol4/circuit-theory/ch1-vacuum-circuit-analysis/parametric-coupling-kernel.md) (single-site per-site amplitude-shape). **Phase 0c**: single-site $P(\delta V)$ closed-form + substrate amplitude correlator decomposition $\kappa_3(V_{DC}, A_c) = -3 a \eta_T S_0^{-1/2}$ + $\kappa_4(V_{DC}, A_c) = -3(1 + 4 a^2) \eta_T^2 S_0^{-1}$ + substrate correlation length $\ell_{corr}(V_{DC}) = \ell_{corr}(0) S_0^{3/2}$. **Phase 2-NA**: aperture-aggregate $\kappa_3^{(apt)}/\sigma_{apt}^3 = -3 a \eta_T S_0^{(3d-2)/4}/\sqrt{N_0}$ + dimensionality-dependent peak operating point $a^{(d)}_{peak} = \sqrt{4/(3d+2)}$ (load-bearing $d = 2$ gives $1/\sqrt{2} = 0.707$, within 3% of PONDER-05 canonical $0.687$) + Op14/Op16 metric-lensing convolution against detector frequency response with three-case classification (Case A broadband / Case B narrowband mistuned / Case C narrowband tuned = PONDER-05 architecture by design). **Remaining**: Phase 2-LLCP (substrate critical-point regime, separate sub-epic; Phase 0c Boltzmann-around-V_DC framework does NOT apply at LLCP); Phase 3 KB integration to divergence-test substrate map as new forward-prediction row B7-PONDER-05-EXT; Phase 4 detector-architecture-specific $\mathcal{F}$ derivations for non-PONDER-05 empirical-engagement paths. Three Type E walk-backs documented across Phase 0c + Phase 2-NA (κ_3 LINEAR not cubic; $\ell_{corr}$ shrinking not diverging; peak operating point dimensionality-dependent not uniform). **DEFLATED-IN-PART 2026-06-04 (adjudication #7):** the $\kappa_3/\kappa_4$ + $a^{(2D)}_{peak}=0.707$ **derivation stands** (substrate-geometric, Op17 matched-impedance), but the **PONDER-05 empirical-engagement leg deflates** — PONDER-05's 30 kV biases the *vacuum* to per-node $a\sim10^{-7}$ (0.687 is the quartz material ratio, per the updated INVARIANT-S2), so the "within 3% of 0.687" alignment is a spurious coincidence and the B7-PONDER-05-EXT row is dropped. The prediction is real but **facility-class** ($\sim8\times10^{16}$ V/m), NOT testable at the PONDER-05 bench. See `parametric-coupling-kernel.md` §14.5 banner.
