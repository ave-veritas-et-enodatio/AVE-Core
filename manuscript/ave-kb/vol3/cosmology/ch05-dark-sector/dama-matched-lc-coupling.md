...
3. ~~V_0 ≠ 0 substrate DC reactive operating point — currently V_0 → 0 assumed~~ **PARTIALLY CLOSED 2026-05-26 Phase 0c ax4-saturation epic** per [`research/2026-05-26_ax4-saturation-phase-0c-pdelta-v-derivation-result.md`](../../../../../research/2026-05-26_ax4-saturation-phase-0c-pdelta-v-derivation-result.md) + [`parametric-coupling-kernel.md` §13](../../../vol4/circuit-theory/ch1-vacuum-circuit-analysis/parametric-coupling-kernel.md): single-site per-site amplitude-shape $P(\delta V)$ under DC-biased operating point derived end-to-end from canonical primitives (Ax 4 kernel + Vol 3 Ch 11 boundary-impedance thermalization + cycle-12 substrate-vacuum-varactor); substrate amplitude correlator decomposition $\kappa_3(V_{DC}, A_c)$ + $\kappa_4(V_{DC}, A_c)$ closed-form; substrate correlation length under DC bias $\ell_{corr}(V_{DC}) = \ell_{corr}(0) S_0^{3/2}$. **Remaining**: aperture-aggregate prediction (Phase 2 of ax4-saturation epic) — combining single-site shape with Edgeworth pre-asymptote across N independent boundary sites + mapping to candidate boundary-extraction architectures (SPAD / TES / SNSPD per `translation-instrumentation.md` Category II). Two Type E walk-backs from prereg expectations honestly documented (κ_3 scales LINEARLY in $V_{DC}/A_c$ NOT cubically; $\ell_{corr}$ scales as $S_0^{3/2}$ shrinking toward yield NOT $1/S_0$ diverging).
