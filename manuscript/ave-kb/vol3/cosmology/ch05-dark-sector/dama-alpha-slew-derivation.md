...
| Annual modulation PHASE (June peak) | 🔴 **EXCLUDED as AVE-distinct (2026-07-04)** — the CMB apex peaks in DECEMBER (day ~348), not June; DAMA's May peak (145±5 d) is the standard-HALO phase (June 2, day 153). Claim B's "June" used the Cygnus apex mislabelled as the CMB apex. See §11.3 + `research/2026-07-04_dama-cmb-phase-lock-check_note.md`. *(Prior text preserved per Rule 12: "CONFIRMED AVE-distinct vs SM solar-driven (December peak) — DAMA observed phase matches CMB-velocity phase (§11)")* |
...
~~If independent extra-galactic surveys (globular clusters, halo stars) DO NOT show clustering at αc/(2π) or below, the substrate-equilibrium-velocity prediction is falsified, weakening (but not killing) the α-slew DAMA framing.~~ **TESTED 2026-05-17 night** ([`research/2026-05-17_substrate_equilibrium_velocity_GLOBULAR_CLUSTER_result.md`](../../../../../research/2026-05-17_substrate_equilibrium_velocity_GLOBULAR_CLUSTER_result.md)): GC population (N=165 MW globular clusters per Baumgardt+Vasiliev 2021) cluster at median |v_CMB| = 564 km/s (NOT near αc/(2π) = 348 km/s) — Outcome III (cosmic-flow dominated). **Substrate-velocity prediction does NOT extend to GC-class populations**; scope narrows to LSR-class only. The substrate-velocity prediction at LSR scope SURVIVES with 9% astronomical observation match (375 km/s observed vs 348 km/s predicted, with the 9% gap from cosmic-flow contribution); the α-slew DAMA framing is NOT weakened by GC test outcome (substrate-scale prediction is independent of stellar-population kinematics at galaxy/cluster scales). **Substrate-scale exactness via knot theory (Tier-3 #10 Step 4, 2026-05-17 night; walked back post external reviewer A#1)**: the αc/(2π) value is rigorously exact at substrate scale via the electron unknot Ideal Ropelength = 2π (knot-theory topological invariant per [Cantarella+Kusner+Sullivan 2002](https://doi.org/10.1007/s00222-002-0234-y)); see [`research/2026-05-17_hoop-stress-2pi-step-4-result.md`](../../../../../research/2026-05-17_hoop-stress-2pi-step-4-result.md). The substrate-scale prediction has NO inherent error; astronomical-scope predictions need cosmic-flow corrections. **Walk-back**: Cosmic-scale 2π rigor (originally claimed via "3-sphere great-circle integration") REMAINS OPEN per external reviewer A#1 — cosmic-scale 2π is derived via Unruh-Hawking in `mond-hoop-stress.md §4.5`, not via explicit Hoop-Stress closed-loop integration on de Sitter $S^3$. Per `ave-independence-check` retroactive: only 2 INDEPENDENT instances of motif (cosmic $a_0$ + substrate $\nu_{slew}$); $E_{slew} = \alpha m_e c^2$ has 2π CANCEL via $h = 2\pi\hbar$ identity — derived consequence of $\nu_{slew}$, NOT a separate instance.
...
> 🔴 **PHASE-LOCK DISCRIMINATOR EXCLUDED (2026-07-04; Rule-12 — body preserved below, git is the trail).** The "around early June (day ~152)" figure in Claim B is an **arithmetic error**: it used the galactic-rotation / **Cygnus** apex direction (which does peak in June) while *labelling* it the CMB apex. Computing the phase from the **actual CMB dipole apex** — (l,b)≈(264°,48°) [Planck cross-check (271.9°,30°)], v=370 km/s — with the standard Earth-orbit ephemeris gives peak of $|\vec v_{\rm CMB}+\vec v_{\rm Earth}(t)|$ at **day-of-year ~348 (mid-December)**, because the CMB apex sits at ecliptic longitude λ≈172° while the DM-wind/Cygnus apex sits at λ≈341° — the two are **~170° apart in ecliptic longitude** (nearly antipodal in the orbital plane), so their peak dates are ~172 days apart. The **standard-halo** DM-wind apex (l≈87°, b≈2°, v≈232 km/s) peaks **day ~153 (June 2)**. DAMA/LIBRA-phase2 measures t₀ = **145 ± 5 d** (arXiv:1805.10486; ~mid-May). Thus DAMA's phase matches the **standard halo** (June 2, ~1.6σ) and **EXCLUDES the substrate/CMB frame** (December, ~162 d ≈ 32σ offset). The phase this leaf reported as "AVE-distinct, CONFIRMED by DAMA" is in fact the ordinary SM/particulate-DM halo phase. As a discriminator between the CMB frame and the standard halo, DAMA's phase **favours the halo and dies for the CMB reading.** Full arithmetic: [`research/2026-07-04_dama-cmb-phase-lock-check_note.md`](../../../../../research/2026-07-04_dama-cmb-phase-lock-check_note.md). (The OTHER two §11 discriminators — Z-independence cross-crystal swap [UNTESTED]; solid-vs-liquid G>0 gate — are UNTOUCHED by this note. External context, not load-bearing here: ANAIS-112 is inconsistent with DAMA at ~3σ, arXiv:2404.17348.)  <!-- rule12-freeze: base=20caf88e4baeb81304f3e542bd2fa0a4c6ee2973 region=below offset=34 lines=103 bytes=12715 sha256=3e5b678a59813339ab0d3eeca113c94f8355f08c94d76992c610404084cb230d -->
...
Status: 🔴 **EXCLUDED (2026-07-04)** — see the phase-lock header above. The June peak this claim relies on is the standard-halo/Cygnus phase, NOT the CMB-apex phase (which peaks in December, day ~348). DAMA's measured 145 ± 5 d matches the halo, not the CMB frame; the discriminator fails on the phase arithmetic. Arithmetic: `research/2026-07-04_dama-cmb-phase-lock-check_note.md`. *(Superseded status; prior text preserved per Rule 12: "AVE-distinct claim CONFIRMED by DAMA observation. Replication needed by COSINE-100 + ANAIS-112...")*
...
| CMB-velocity phase-lock (June peak) | Solar-driven: December peak (perihelion); atmospheric-driven: latitude-dependent | June peak matching CMB-velocity phase | 🔴 **EXCLUDED (2026-07-04)** — CMB apex peaks in DECEMBER (day ~348), not June; DAMA's May peak (145±5 d) matches the standard HALO, not the CMB frame. Arithmetic error in Claim B (Cygnus apex mislabelled CMB apex). See §11.3 header + `research/2026-07-04_dama-cmb-phase-lock-check_note.md`. |
...
| **CMB-velocity phase-lock** 🔴 **EXCLUDED 2026-07-04** | *(Grounding preserved per Rule 12; the empirical discriminator FAILS.)* The electron's reactive cycle phase is referenced to the K4 substrate rest frame = CMB rest frame. Earth's CMB-frame velocity modulates the relative phase between electron's α-slew leak and external NaI matched mode. ~~June peak (max CMB-frame velocity) = max matched-coupling efficiency.~~ **Corrected arithmetic (§11.3 header):** the CMB-frame velocity |v_CMB+v_Earth| is MAXIMAL in **DECEMBER (day ~348)**, not June — the CMB apex is ~170° in ecliptic longitude from the Cygnus/DM-wind apex. DAMA's May peak (145±5 d) matches the standard HALO, not the CMB frame; the phase-lock discriminator dies. See `research/2026-07-04_dama-cmb-phase-lock-check_note.md`. |
