...
| Result | Statement |
|---|---|
| Asymptotic Expansion Limit | $H_{\infty} = \frac{1792\pi^3 m_e^3 c G}{\hbar^2 p_c^2}$ |
| Numerical Hubble Evaluation | $H_{\infty} \approx 69.32$ km/s/Mpc |
| Phantom Energy Equation of State | $w_{vac}=-1-\frac{\rho_{latent}}{\rho_{vac}}$ |
| Cosmological Continuity Equation | $\dot{\rho}_{rad} + 4H\rho_{rad} = 3H\rho_{latent}$ |
| Radiation Energy with Latent Floor | $u_{rad}(a) = U_{hot}a^{-4} + \frac{3}{4}\rho_{latent}$ |
| Exponential Inductive Accretion | $M(t) = M_{seed} \cdot e^{t / \tau_{ind}}$ |
| JWST Constraint Equation | $10 = e^{150 / \tau_{ind}}$ |
| Mutual Inductive Accretion Time Constant | $\tau_{ind} = \frac{150}{\ln(10)} \approx 65.1$ Myr |
...
| [Lattice Genesis and Hubble Tension](lattice-genesis-hubble-tension.md) | LC crystallisation mechanism, $H_\infty \approx 69.32$ km/s/Mpc, comparison table (Planck, SH0ES, TRGB) |
...
| [Dark Energy = Latent Heat of Vacuum Crystallization (canonical definition + honest ledger)](dark-energy-latent-heat-definition.md) | CONSOLIDATION leaf (reuses clm-3ii690): states the DE definition once, sector-clean ε-placement, arrow=T2-entropic-sink (NOT chirality), honest ledger ($\rho_\Lambda$=ECHO/consistency, $\rho_{latent}$ symbolic-only, $\Gamma$ asserted), and the one UNBUILT ΛCDM-distinct chord (F6 DE-vs-matter) + 2 open flags |
...
| [Black Holes — Lattice Phase Transition, Not Impedance Mismatch](black-holes-impedance-mismatch.md) | Dielectric saturation, lattice phase transition, information loss |
| [Op14 Cosmic-Horizon Saturation Profile](op14-cosmic-horizon-profile.md) | $Z_{\text{eff}}(r \to R_H)$ during ongoing crystallisation; cosmic-scale (c)-operator-application of canonical Op14 (Vol 1 Ch 6 §1.13); local-clock-freezing at cosmic horizon, asymmetric ε/μ form, dynamic-vs-frozen distinction from BH event horizon |
