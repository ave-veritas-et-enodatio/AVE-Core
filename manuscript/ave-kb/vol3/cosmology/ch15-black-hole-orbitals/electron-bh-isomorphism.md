...
> **↗ NEW quantitative support — the "topology melts" $G_{shear}\to0$ is now quantified on the srs Cauchy tensor (2026-07-04, PR #521 MERGED — the saturated-elastic-tensor arc).** The saturated Born-Huang elastic tensor of the ratified chiral srs-z3 net puts a number on the "$G_{shear}\to0$ / topology melts" picture above: the absolute shear stiffness (Cauchy $C_{44}$) collapses from $0.17661$ (loaded-cold) $\to 0.02536$ (at the $\nu=2/7$ crossing) $\to 4\times10^{-5}$ as $A\to1$ — the lattice goes **floppy near the yield wall**, node-up. The physical picture is **two-faced** (both true, consistent): while the **absolute** moduli scale by the overall saturation factor $S$ and collapse toward zero (topology melts), the dimensionless **ratios freeze** ($\nu$, Zener, $K/G$ are homogeneous degree-0 in the bond stiffnesses, so they depend on $\rho_{eff}$ alone and are unshifted by saturation magnitude) — matter near yield is a **soft region with locked proportions**. And $\mathrm{sign}(K)$ is scale-invariant: the $K<0$ instability boundary sits at $\rho_{eff}=2$, unshifted by saturation magnitude. This is a **quantitative-support / CONSISTENCY** addition to the standing topology-melts picture, not a new claim; no VALUE is derived. Provenance: [`research/2026-07-04_saturated-elastic-tensor_result.md`](../../../../../research/2026-07-04_saturated-elastic-tensor_result.md) §4 (the absolute-scale / floppy-near-yield axis); driver `src/scripts/vol_1_foundations/saturated_elastic_tensor.py`.
