...
Some leaves credit a *derived* quantity to a bare `(Axiom N)` where it should cite the deriving claim + the fundamental: `(clm-XXXXXX via Axiom N[, Axiom M])`. **Done 2026-05-23** — the clear must-fixes: the `p_c = 8πα` packing-fraction class (all 4 sites now carry `clm-9s9apq`, each keeping its local lens — `radial-eigenvalue-solver.md` *via Axiom 3* [α/algebraic]; `chiral-factor.md` / `helium-coupling-first-principles.md` / `spontaneous-symmetry-breaking.md` *via Axiom 4* [saturation]), and the Hopf linking number (`scale-separation.md` → `clm-dfaiwj via Axiom 2`). **Remaining (optional/lower-priority):** ~5–6 `ξ_topo = e/ℓ_node` "(Axiom 2)" sites — defensible as-is (ξ_topo *is* the Axiom-2 isomorphism constant; could optionally gain a `clm-` id for uniformity) — plus any other named derived-identity instances a fuller sweep surfaces. Grep seed: `\(Axiom [1-4]\)` adjacent to a derived symbol/identity (~185 of the 198 hits are legitimate mechanism scoping, NOT D9).
