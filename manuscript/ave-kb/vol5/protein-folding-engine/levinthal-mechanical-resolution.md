...
**The AVE resolution**: the amino-acid sequence does **not** search a random energy landscape. The chain is being **driven** by substrate strain toward the (conjectured-unique) configuration that satisfies impedance matching at every backbone-sidechain junction simultaneously.
...
2. **The 3D fold is the geometry that minimizes the backbone $|S_{11}(\omega_0)|^2$** — the standing-wave reflection at the amide-V frequency. Constructive impedance matching is conjectured to be achieved by a unique 3D placement of residues; destructive mismatches force the chain to bend, twist, or sheet at specific locations.
...
4. **The minimum-$|S_{11}|^2$ configuration is the native fold** — on this picture there is no search; the chain mechanically snaps to its impedance-matched geometry, and the snap IS the protein-folding [A-034 instance](../../common/universal-saturation-kernel-catalog.md).

### Why this resolves Levinthal physically

The chain is **not searching configuration space**; it is **being driven** by substrate strain toward the (conjectured-unique) configuration where the kernel-saturation events on every bond simultaneously satisfy $A_i < 1$ (no further snapping required). This is the impedance-matched, lowest-$|S_{11}|^2$ geometry. The folding timescale ($\mu$s) reflects substrate dielectric relaxation, not configuration enumeration. The uniqueness of that configuration --- the step that carries the Levinthal resolution --- is conjectured, not yet demonstrated by a closed derivation.
