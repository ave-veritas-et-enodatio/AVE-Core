...
The disclosure pattern is structurally identical to Vol 6 nuclear-mass methodology (one fitted scalar per nucleus, with substrate-predicted topology + parameter count): the substrate predicts structure; one numerical scalar is fitted per system; the substrate prediction is falsifiable across the relevant variable. To upgrade $\delta_{\text{strain}}$ from a one-scalar fit (Class A identity at back-substitution) to a full substrate prediction (Class 2 axiom-manifestation on the magnitude as well as the sign), the substrate chain needs to (i) adjudicate the substrate-saturation-class for low-amplitude electromagnetic thermal-bath loading (CMB at $T_{\text{CMB}} \approx 2.725$ K) — open framework-extension question **Q-DELTA-MAP-1** with three candidate substrate-physics paths P1/P2/P3 per [`research/2026-05-28_phase-3-a3-delta-strain-machian-projection-result.md`](../../../research/2026-05-28_phase-3-a3-delta-strain-machian-projection-result.md) §3 — and then (ii) derive the substrate spatial-metric strain magnitude within the chosen class and verify it matches the back-substituted value within tolerance. That substrate-mechanism magnitude derivation does not currently appear in the corpus; it is queued as a `clm-0ktpcn` strengthen-by item ("Adjudicate Q-DELTA-MAP-1 saturation-class then derive δ_strain magnitude"). **2026-05-27 Phase 3-A3 WALK-BACK provenance**: a prior attempt to derive δ_strain via Machian-G operating-point cascade ($u_0^* \to A_0^{cosmic} \to$ INVARIANT-S2 SYM scaling $\to$ α modulation) failed at Step 3.5 — the proposed chain substitutes $c_{shear}$ (group/rest-mass velocity, $= c_0\sqrt{S}$ in SYM per clm-8nkvwy line 113) where $c_{EM}$ (phase velocity, $= c_0/S$ in SYM per clm-8nkvwy line 111) belongs in α. The canonical clm-3zz0f6 SYM α-invariance derivation gives $\alpha = \alpha_0$ exactly invariant; the prework-brief chain reproduces the canonical Pitfall #5 framework-leakage error. See result doc §2 for full audit + §3 for Q-DELTA-MAP-1 candidate paths.
