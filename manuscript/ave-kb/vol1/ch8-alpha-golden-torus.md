...
> → Primary: [Op21 Multi-Mode Mode-Counting at the $\Gamma = -1$ Saturation/TIR Boundary](../vol4/circuit-theory/ch1-vacuum-circuit-analysis/op21-multi-mode-mode-counting.md) — fully-derived substrate-mechanism canonical leaf (Phase 3-A4, 2026-05-27). Promotes the previously paragraph-level Op21 multi-mode generalization to canonical-leaf rigor: five-step substrate-mechanism chain (Ax 1 Nyquist cell size → Ax 3 + Ax 4 forcing $\Gamma = -1$ TIR boundary → per-cycle leak fraction $1/\ell$ → $Q_{\text{mode},\ell} = \ell$ → Nyquist-cell-count = mode-count = dimensionless geometric measure) + Step 5.5 codimensional Nyquist-cell-category independence (replacing the Schur-orthogonality framing that Phase 3-A2 falsified). Derives the $Q_i = \Lambda_i$ identification (`theorem-3-1-q-factor.md:67`) from substrate primitives rather than asserting as natural-unit convention. The substrate-foundational form $Q = \ell$ at $\Gamma = -1$ is cross-scale canonical (electron LC tank + BH ringdown + universal substrate-Q derivation procedure + knot-mode isomorphism); the Cooper-pair-phase-transition Bardeen $Q \sim 1/\ln(Z_1/Z_0)$ form is one cross-scale specialization (Vol 1 Ch 6 §1.21 canonical name "Quality Factor Phase Transition" = substrate Op21 phase-transition mechanism, evaluated at the Cooper-pair scale-instance).
...
**Honest classification per `consistency-vs-emergence` v1.2 (2026-05-27 Phase 3-A4 update).** The additive assembly is **Class 2 substrate-mechanism emergence** (axiom-manifestation; lifted from Class B substrate-mechanism manifestation by Phase 3-A4): the substrate-mechanism path via Op21 mode-counting at saturation boundary is now derived at fully-canonical-leaf rigor at [`op21-multi-mode-mode-counting.md`](../vol4/circuit-theory/ch1-vacuum-circuit-analysis/op21-multi-mode-mode-counting.md), with five-step substrate-mechanism chain (Ax 1 Nyquist cell size → Ax 3 + Ax 4 forcing $\Gamma = -1$ TIR boundary → per-cycle leak fraction $1/\ell$ → $Q_{\text{mode},\ell} = \ell$ → Nyquist-cell-count = mode-count = dimensionless geometric measure) + Step 5.5 codimensional Nyquist-cell-category independence. The substrate-mechanism content of the additive assembly $Q_{\text{tank}} = \sum_k \Lambda_k$ traces to substrate primitives end-to-end; the $Q_i = \Lambda_i$ identification (`theorem-3-1-q-factor.md:67`) is derived from substrate Nyquist-cell-counting in lattice-natural units, not asserted as a natural-unit convention. The numerical match $4\pi^3 + \pi^2 + \pi \approx 137.0363038$ to CODATA $\alpha^{-1} \approx 137.035999$ within $\delta_{\text{strain}} \approx 2.225 \times 10^{-6}$ remains **Class 4 observable consistency** (unchanged by Phase 3-A4; the $\delta_{\text{strain}}$ magnitude derivation is the Phase 3-A3 target).
