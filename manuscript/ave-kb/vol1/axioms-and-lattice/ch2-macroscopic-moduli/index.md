...
The vacuum cannot be a classical Cauchy solid (implosion paradox); it must be a micropolar (Chiral LC) continuum. The topo-kinematic isomorphism maps electromagnetic parameters ($\mu_0, \epsilon_0$) to mechanical moduli, and the geometric trace-reversal packing fraction $p_c = 8\pi\alpha$ is established as a *consistency relation* between the QED Schwinger limit (taken as external input) and $\alpha$'s SI definition. $\alpha$'s value closes at **Class B** named geometric identification in [Ch.8 (Golden Torus closure)](../../ch8-alpha-golden-torus.md) (it rests on $R \cdot r = 1/4$, which the substrate does not independently select; not a first-principles derivation); this chapter shows that the resulting $p_c$ sits at the EMT trace-reversal operating point ($K = 2G$). <!-- 🔴 Rule-12 2026-06-15 alpha: prior "The numerical value of alpha itself is derived from first principles in Ch.8" superseded per Ch.8 keystone. -->
...
| Discrete Voronoi Cell Volume | $V_{node} = 2e^2\hbar^2 / (\epsilon_0 m_e^3 c^4)$ |
| Vacuum Packing Fraction (consistency identity) | $p_c = V_{node}/\ell_{node}^3 = 8\pi\alpha \approx 0.1834$ |
| Inverse Fine-Structure Constant (consistency relation) | $\alpha^{-1} = 8\pi / p_c$ — α's value is a Class B named identification in [Ch.8](../../ch8-alpha-golden-torus.md) (not a first-principles derivation) <!-- 🔴 Rule-12 2026-06-15 alpha: prior "alpha's value derived in Ch.8" superseded per Ch.8 keystone. --> |
| Over-Bracing Factor | $\mathcal{R}_{OB} = p_{Delaunay}/p_c = 0.3068/0.1834 \approx 1.673$ |
| Secondary Link Reach | $r_{secondary} = \sqrt[3]{\mathcal{R}_{OB}} \cdot \ell_{node} \approx 1.187\,\ell_{node}$ |
