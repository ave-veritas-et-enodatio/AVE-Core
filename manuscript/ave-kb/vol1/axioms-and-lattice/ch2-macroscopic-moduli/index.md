[↑ Axioms and Lattice](../index.md)

# Ch.2: Macroscopic Moduli and The Volumetric Energy Collapse

The vacuum cannot be a classical Cauchy solid (implosion paradox); it must be a micropolar (Chiral LC) continuum. The topo-kinematic isomorphism maps electromagnetic parameters ($\mu_0, \epsilon_0$) to mechanical moduli, and the fine-structure constant ($\alpha$) emerges from the geometric trace-reversal packing fraction ($p_c = 8\pi\alpha$) where dielectric rupture forces volumetric energy collapse.

## Key Results

| Result | Statement |
|---|---|
| Impedance Dimensional Isomorphism | $1\,\Omega = \xi_{topo}^{-2}\,\text{kg/s}$ |
| Discrete Voronoi Cell Volume | $V_{node} = 2e^2\hbar^2 / (\epsilon_0 m_e^3 c^4)$ |
| Vacuum Packing Fraction | $p_c = V_{node}/\ell_{node}^3 = 8\pi\alpha \approx 0.1834$ |
| Inverse Fine-Structure Constant | $\alpha^{-1} = 8\pi / p_c$ |
| Over-Bracing Factor | $\mathcal{R}_{OB} = p_{Delaunay}/p_c = 0.3068/0.1834 \approx 1.673$ |
| Secondary Link Reach | $r_{secondary} = \sqrt[3]{\mathcal{R}_{OB}} \cdot \ell_{node} \approx 1.187\,\ell_{node}$ |
| Dielectric Snap Limit | $V_{snap} = m_e c^2 / e \approx 511.0$ kV |

## Derivations and Detail

| Document | Contents |
|---|---|
| [Implosion Paradox](./implosion-paradox.md) | Why the vacuum must be micropolar: MacCullagh's condition yields negative bulk modulus |
| [Constitutive Moduli](./constitutive-moduli.md) | Mapping $\mu_0$, $\epsilon_0$ to mechanical moduli via topo-kinematic isomorphism |
| [Topo-Kinematic Isomorphism](./topo-kinematic-isomorphism.md) | 7-row dimensional mapping table: Voltage to Force, Current to Velocity, etc. |
| [Dielectric Rupture](./dielectric-rupture.md) | Voronoi cell volume, packing fraction $p_c = 8\pi\alpha$, inverse fine-structure constant |
| [Dielectric Snap Limit](./dielectric-snap-limit.md) | Over-bracing factor, secondary link reach, $V_{snap} = 511.0$ kV |
| [Continuous-Springs Reframing](./continuous-springs-reframing.md) | Discrete K4 lattice as a discretization of the continuous Cosserat micropolar field at axiom level; "bonds" are sampling-representation of continuous stress field, not physical springs; $\ell_{node}$ is Nyquist sampling pitch, not fundamental graininess |
| [A-029 Secondary Scale: Geometric Shared-B-Node Distance](./secondary-scale-shared-b-node.md) | The 1.187 secondary-scale ratio is NOT a calibration input — it is the geometric next-nearest-neighbor distance set by K4 graph topology + vacuum packing fraction $p_c = 8\pi\alpha$ + Delaunay reference packing; grounds gravity projection via shared-B-node propagator |
