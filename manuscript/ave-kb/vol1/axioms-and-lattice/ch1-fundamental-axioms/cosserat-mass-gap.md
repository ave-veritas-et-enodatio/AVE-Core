...
> **⚠ CHANNEL→DOF LABEL COLLISION — DISAMBIGUATE, ADJUDICATION PENDING — Grant (2026-07-03; §8 rotation un-conflation, `research/2026-07-03_em-readout-vsector-stage1_prereg.md` §8 correction note item 3, GAP G2; body above PRESERVED unedited).** The line-145 statement **quotes both halves of an inversion in one sentence** and the two halves cannot both be read at face value:
> - **HERE (`cosserat-mass-gap.md:145`, verbatim):** *"the K4 scalar sector remains **separately massless (the photon** per photon-identification)** … $A_1$ (scalar/longitudinal/translational $u$) is massless, $T_2$ (transverse/microrotational $\omega$) carries the mass-gap content."* → the **massless sector is $A_1$**, and this line **labels that massless $A_1$ sector "the photon."**
> - **THERE (`photon-identification.md:11`, verbatim):** *"**The photon is the K4-TLM's stable $T_2$-only bound state** … The $A_1$ scalar/longitudinal sector dissipates … the $T_2$ transverse triplet survives as the photon."* → the **photon is $T_2$**, and $A_1$ **dissipates** (Gauss), it is not "the photon."
...
- **`:132`** — *"the isotropic-solid longitudinal **P-wave** is $c_L = \sqrt{(K + \tfrac{4}{3}G)/\rho} = \sqrt{10/3}\,c \approx 1.826\,c$ ... PR #392 reads it as the acoustic-manifold top"*
  Stamped in place at `:132`.
  **Why it dies (audited row rationale, verbatim):** Prereg-named site: asserts the P-branch EXISTS as a fourth speed / BZ acoustic-manifold top; under the carve the branch is constraint-removed, so the existence claim voids (the 2-vs-3 anti-fusion guard survives vacuously; K=2G provenance is static algebra).
