...
The AVE framework's content is that variables such as $G$, $\alpha$, and $\ell_{node}$ are not fundamental empirical inputs but emergent mathematical properties of the scale-invariant graph topology. As of the current edition this holds for $\ell_{node}$ (scale invariance, Pathway 3) and for $\alpha$ at **Class B substrate-mechanism manifestation level** per Q-EMBED-SEL-1 Phase 1+2+3, with the Class 2 lift candidate workstream identified but out of scope. $G$ remains **quantitatively open** (Chain B', Pathway 2).
