...
> **Kernel vs Op19 (2026-08-28; [`def-ln3str`](../../../common/vocabulary-register.md)).** Solar and white-dwarf rows are Regime I for the **Ax4 kernel** (SYM-table column I frozen; not a derived bond $\delta L/L$). The gravitational index still grades as $n=1+\nu_{\mathrm{vac}}\varepsilon_{11}$ (Op19) inside that same band. Do not read "Regime I" as "$L,C\propto\varepsilon_{11}$." Assembly: [`bond-lc-constitutive-grading.md`](../../../vol3/gravity/ch03-macroscopic-relativity/bond-lc-constitutive-grading.md).
...
> **Correction note (2026-08-03 — predicted-vs-measured; propagating the merged #830 relabel to its own flagged upstream site).** **Near the end of** the 2026-08-01 note above (two sentences follow it), the un-7'd $GM/(c^2R) = 2.59\times10^{-4}$ is called *"which is also the **measured gravitational redshift**, the observable its vol3 companion table's *Physical consequence* column cites"*. **That label carries the same defect (b) above corrects, and the same correction applies:** $2.59\times10^{-4}$ is the **GR-PREDICTED** $z$, not the measured one. Receipt, on the Sirius B parameters this block already tabulates ($M = 1.018\,M_\odot$, $R = 5800$ km): $c\,GM/(c^2R) = 77.72$ km/s, exact-Schwarzschild $77.75$ km/s — against a **measured** line shift of $80.65 \pm 0.77$ km/s $= 2.69\times10^{-4}$, i.e. $3.7\%$ higher and $(80.65 - 77.75)/0.77 = 3.8\sigma$ on the quoted uncertainty. Predicted and measured are **not interchangeable at the printed precision**. The clause's *other* assertion — that the redshift, not the strain, is what the consequence column cites — is **correct and unaffected**; only the predicted-vs-measured word moves.
> **Why this is an additive note and not an in-place relabel.** The 2026-08-01 text is left **BYTE-UNTOUCHED**: the Resolution at the head of this block states that its text is *"preserved unedited per Rule 12"*, so editing it would falsify a merged sentence in this same file. Merged **#830** (`4abc407c`) reached the same conclusion from the other side — its own commit message records the site as left byte-untouched because *"editing it would break the preservation this PR relies on"* — and routed the relabel forward. This note discharges that routing in the shape the constraint allows: the defect is quoted, the correct label and its arithmetic are stated, and the preserved text stays preserved. **No physics moves** — the displaced $3\times10^{-4}$ remains the un-7'd potential and remains a different *quantity* from the strain.
