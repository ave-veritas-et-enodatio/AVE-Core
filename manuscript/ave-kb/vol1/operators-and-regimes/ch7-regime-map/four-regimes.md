...
> **[FLAG 2026-06-11 clarification — Vol 9 Ch 14 §True Phase Diagram]** Regime IV ($r \geq 1$, $S = 0$) is the solid's **local rupture endpoint** along the excitation axis $r = A/A_c$ — the locus where K4 topology is locally destroyed. Its resulting **state** coincides with the substrate's **melt phase** — at this endpoint the **de-bonded state** (bonded once, released); the primordial one is the **pre-bond state** (never bonded) *(R9 rename, 2026-08-06; formerly "pre-geodesic plasma", gloss *pre-geodesic*)* — but it is a distinct *coordinate*: this four-regime map is the **excitation map of the solid phase**, whereas the melt is a thermodynamic phase that **predates the crystal** (per [`cosmic-axes-and-frames-glossary.md:75`](../../../common/cosmic-axes-and-frames-glossary.md), "supercooled pre-geodesic plasma") and therefore predates the $r$-axis itself. A coordinate defined only inside the crystal cannot be the axis on which the pre-crystal phase is located. The substrate's *true* thermodynamic phase diagram (solid / melt / candidate cavitated, on the $(T, \bar\rho)$ plane) is Vol 9 Ch 14 §True Phase Diagram. Re-label per Rule 12; the table body above is preserved. **⚑ Rename note, 2026-08-06:** the inner quotation *"supercooled pre-geodesic plasma"* above is **preserved verbatim** — it quotes [`cosmic-axes-and-frames-glossary.md:75`](../../../common/cosmic-axes-and-frames-glossary.md), which in turn quotes the print source, and a rename inside a quotation falsifies it. The phase it names is the **pre-bond state** under the current vocabulary.
