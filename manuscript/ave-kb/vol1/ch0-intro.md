...
- The framework is structurally zero-parameter (conditional on thermal closure of $\delta_{strain}$ at $T_{CMB}$): $\alpha^{-1}_{\text{ideal}} = 4\pi^3 + \pi^2 + \pi$ derives from Golden Torus geometry (Ch.8), and $\ell_{node}$ and $G$ are also derived; one currently-fitted thermal scalar bridges the cold-lattice asymptote to CODATA pending substrate-physics derivation gated on **Q-DELTA-MAP-1** substrate-saturation-class adjudication for low-amplitude electromagnetic thermal-bath loading (see [`research/2026-05-28_phase-3-a3-delta-strain-machian-projection-result.md`](../../../research/2026-05-28_phase-3-a3-delta-strain-machian-projection-result.md) §3 for the 2026-05-27 Phase 3-A3 WALK-BACK). One-cosmological-parameter framework ($\Omega_{\text{freeze}}$) with three-route framework commitment ($\alpha + G + \mathcal{J}_{\text{cosmic}}$ converging on $u_0^*$).
