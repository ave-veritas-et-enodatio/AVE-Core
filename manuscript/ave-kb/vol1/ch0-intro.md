...
Historically, the framework was presented as a single-parameter EFT (taking $\alpha$ as the one empirical input after $\ell_{node}$ is identified with the electron's Compton wavelength). The zero-parameter closure is **proposed** but **contingent on the open Clifford-torus embedding-selection item** (zero-parameter contingent on the open embedding-selection item — NOT the half-cover chain, which is substrate-derived; see [Ch.8 §"Remaining open formal-rigor sub-item (THE gating item)"](./ch8-alpha-golden-torus.md) for the canonical anchor): $\alpha$ itself is derived from the S₁₁-minimum Golden Torus geometry of the electron's $0_1$ unknot soliton with $(2,3)$ phase-space Clifford-torus winding pattern (Ch.8), $\alpha^{-1}_{\text{ideal}} = 4\pi^3 + \pi^2 + \pi$, with a CMB-induced thermal strain coefficient $\delta_{\text{strain}}$ accounting for the residual to CODATA. Until the embedding-selection step is closed from the classical axioms alone, $\alpha$ operationally remains the framework's one calibration input. From the three derived hardware scales plus four axioms, the geometric relationships defining all other macroscopic constants are derived from topology and continuum mechanics.
