...
- rationale: The 0_1-unknot real-space body + (2,3) phase-space winding is the canonical post-2026-05-17 position and the mass relation m_e = ℏ/(c·ℓ_node) is dimensionally clean. The Finkelstein-Misner spin-1/2 derivation is now explicitly spelled out in the canonical leaf at `vol2/particle-physics/ch01-topological-matter/finkelstein-misner-spin-half-derivation.md` (2026-05-25 promotion of L3 doc 23): extended-unknot twist-and-unwind mechanism + group-theoretic $K_4 \to A_4 \to 2T \subset SU(2)$ chain + gyroscopic-isomorphism numerical anchor at $10^{-8}$ + K4-native-vs-imported decomposition. Three structural caveats remain open per the Non-Claims block: real-space topological protection is absent (the unknot is trivially knotted), phase-space-winding-as-protection is "not yet rigorously established," the spin-1/2/photon-720° compatibility is "a final determination deferred," and the (2,3) winding numbers are asserted not derived. With the FM derivation closed (2026-05-25), 3 structural caveats remain; confidence bumps 0.45 → 0.50. A claim whose own caveats mark its protection mechanism and a key compatibility question as open should not exceed ~0.6.
- strengthen-by:
...
- confidence: 0.70
- depends-on:
  - clm-unk0bd — Electron Body Topology = Unknot (solidity 0.50) [closure condition (c) "via the unknot ground state" formulation]
...
- solidity: 0.50 (use as input only, don't build deeper) [= min(0.85, 0.50)]
...
- confidence: 0.95
- depends-on:
  - clm-0ktpcn — Golden Torus α Derivation (solidity 0.50) [for $V_{yield}$'s $\sqrt{\alpha}$ factor]
- solidity: 0.50 (use as input only, don't build deeper) [= min(0.95, 0.50)]
...
- confidence: 0.90
- depends-on:
  - clm-0ktpcn — Golden Torus α Derivation (solidity 0.50) [for $\xi$'s $\alpha^{-2}$ factor]
  - clm-5xon03 — Zero-Parameter Closure Status (solidity 0.50) [for $\xi$'s $R_H/H_\infty$ factor — $H_\infty$ is a consistency identity conditional on $G$ closure]
...
- confidence: 0.50
- depends-on:
  - clm-0ktpcn — Golden Torus α Derivation (solidity 0.50) [for $p_c = 8\pi\alpha$ in the porosity factor]
- solidity: 0.50 (use as input only, don't build deeper) [= min(0.50, 0.50)]
...
- confidence: 0.60
- depends-on:
  - clm-0ktpcn — Golden Torus α Derivation (solidity 0.50) [for muon's $\alpha\sqrt{3/7}$ and tau's $8\pi/\alpha$ sector factors]
  - clm-unk0bd — Electron Body Topology = Unknot (solidity 0.50) [the "ground-state unknot" framing is load-bearing for the lepton mass formula and the "three structural dimensions of the unknot" Cosserat-sector argument]
...
- confidence: 0.75
- depends-on:
  - clm-0ktpcn — Golden Torus α Derivation (solidity 0.50) [for $r_1 = \sqrt{2\alpha}$ numerical value]
- solidity: 0.50 (use as input only, don't build deeper) [= min(0.75, 0.50)]
...
- confidence: 0.70
- depends-on:
  - clm-b2anl4 — Four-Regime Map (solidity 0.50) [uses $r_1, r_2, r_3$ boundaries to classify domains]
- solidity: 0.50 (use as input only, don't build deeper) [= min(0.70, 0.50)]
...
- solidity: 0.50 (use as input only, don't build deeper) [= min(0.62, 0.50)]
...
- solidity: 0.50 (use as input only, don't build deeper) [= min(0.72, 0.50)]
...
- solidity: 0.50 (use as input only, don't build deeper) [= min(0.74, 0.50)]
