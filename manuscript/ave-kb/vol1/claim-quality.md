...
- depends-on:
  ...
  - clm-8c3yhs — $(2,3)$ torus-knot uniqueness derivation in `vol2/particle-physics/ch01-topological-matter/torus-knot-uniqueness.md` supplies the coprimality + both-windings-$\geq 2$ + minimality + electron-is-lightest chain that forces $(2,3)$ as the electron's phase-space winding (closes the prior strengthen-by item; the $(2,3)$ vs $(4,3)$, $(5,2)$, etc. enumeration is derived, not asserted)
- solidity: 0.55 (use as input only, don't build deeper) [= min(0.55, 0.70)]
- rationale: The 0_1-unknot real-space body + (2,3) phase-space winding is the canonical post-2026-05-17 position and the mass relation m_e = ℏ/(c·ℓ_node) is dimensionally clean. The Finkelstein-Misner spin-1/2 derivation is explicit at `vol2/particle-physics/ch01-topological-matter/finkelstein-misner-spin-half-derivation.md` (2026-05-25 Phase 1 promotion of L3 doc 23): extended-unknot twist-and-unwind mechanism + group-theoretic $K_4 \to A_4 \to 2T \subset SU(2)$ chain + gyroscopic-isomorphism numerical anchor at $10^{-8}$ + K4-native-vs-imported decomposition. **2026-05-25 Phase 2 sub-item 1**: the prior "(2,3) winding asserted, not derived" caveat is closed — `torus-knot-uniqueness.md` (clm-8c3yhs, confidence 0.70) provides the uniqueness derivation via coprimality ($\gcd(p,q)=1$ for a single-component knot, not a link) + both-windings-$\geq 2$ (for non-trivial knotting) + minimality (smallest such pair is $(2,3)$ with crossing number $c=3$) + electron-is-lightest-non-trivial-lepton (forcing the physical identification). depends-on edge wired; clm-unk0bd inherits the higher-confidence derivation through the graph rather than recapitulating it. Two structural caveats remain open per the Non-Claims block: real-space topological protection is absent (the unknot is trivially knotted) and phase-space-winding-as-protection is "not yet rigorously established"; the spin-1/2/photon-720° compatibility is "a final determination deferred." With the FM derivation closed (Phase 1) and the (2,3) uniqueness argument bridged in (Phase 2 sub-item 1), confidence bumps 0.50 → 0.55. A claim whose remaining caveats mark its protection mechanism and a key compatibility question as open should not exceed ~0.6.
- strengthen-by:
