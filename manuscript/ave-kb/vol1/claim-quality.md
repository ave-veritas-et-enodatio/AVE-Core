...
- _Specific Non-Claims and Caveats_
  ...
  - **Spin-½: STRUCTURE derived, SELECTION imported (PEER-WITH-SM) — refined 2026-07-08, [SPIN-HALF-POSITED], #584/#585.** Aligns this entry's spin claim (above) with its `clm-salw2h` / `clm-rkisb8` dependency. The $SU(2) \to SO(3)$ double-cover *structure* is substrate-native (native SO(3) / $2T \subset SU(2)$; lattice-level representability demonstrated #299). But $\pi_1$ of the SO(3)-Cosserat configuration space is $\mathbb{Z}_2$, and $\mathrm{Hom}(\mathbb{Z}_2, U(1))$ has TWO elements — the trivial (boson, $+1$) and the sign (fermion, $-1$) homomorphism — so $\pi_1$ **admits both statistics and forces neither**. Selecting spin-½ requires an imported action-level sign (the half-angle lift $U = \exp(i\,\boldsymbol{\sigma}\cdot\boldsymbol{\omega}/2)$), **PEER-WITH-SM** (SM also posits spin-½). Read "spin-½ arises from the double cover" as *the double-cover structure is native; the fermionic selection is posited*. Full split at `clm-rkisb8` (this file — the 2026-07-08 SELECTION-POSITED strengthening) and `clm-salw2h` ([`../vol2/claim-quality.md`](../vol2/claim-quality.md)).
...
- _Specific Non-Claims and Caveats_
  ...
  - The cold-lattice value $137.0363038$ is the **absolute-zero asymptote**, not the measured CODATA value. CODATA $\alpha^{-1} = 137.035999$ requires the additional CMB-strain correction $\delta_{strain} \approx 2.22\times 10^{-6}$; see Zero-Parameter Closure Status entry above for the predicted/fitted split on $\delta_{strain}$.
...
- $\delta_{\text{strain}} = 1 - \alpha^{-1}_{\text{exp}}/\alpha^{-1}_{\text{ideal}} = 1 - 137.035999/137.0363038 \approx 2.22 \times 10^{-6}$
- _Specific Claims_
  ...
  - **Fitted (one scalar at $T_{\text{CMB}}$):** the numerical magnitude $\delta_{\text{strain}} \approx 2.22 \times 10^{-6}$, computed by back-subtraction from CODATA, $\delta_{\text{strain}} \equiv 1 - \alpha^{-1}_{\text{CODATA}}/\alpha^{-1}_{\text{ideal}}$, definitional given the engine's `DELTA_STRAIN = 1 - (1/ALPHA)/ALPHA_COLD_INV` (`src/ave/core/constants.py`).
...
> **🔴 $X_L$ = the FLYWHEEL (spin/frequency-regulation) sector, NOT the rest-mass store (2026-06-20, Rule 12 — the rationale's "magnetic→$X_L$→rest mass (clm-lv3uw1)" gloss below PRESERVED unedited; Grant-CONFIRMED mass-sector ruling; SYNCS this entry to the PR#260 banner at `common/dual-reactance-storage-taxonomy.md:189`).** Where the rationale glosses the **magnetic** saturation branch ($\mu_{eff}\to0$) inductive sector "$X_L$" as **rest mass**, the **$X_L$ = the inductive FLYWHEEL = the T2 / Cosserat micro-rotation ($\omega$) spin sector** (the spin/Larmor clock), whose mass-gap is the Compton/Larmor *clock* gap, **NOT** the rest-mass store. The **rest mass is the A1 longitudinal DILATATION** depression (`vol1/dynamics/ch4-continuum-electrodynamics/master-equation.md:20` "A1 dilatation-MASS"). The dual-reactance **COUNT** ($\mathcal{V}_{total}=2.0$, $X_C+X_L$) — the load-bearing factor in $\tau_{yield}$ here — is **UNAFFECTED**; only the "$X_L$ = rest mass" *label* is re-scoped. Consistency-class relabel, body preserved per Rule-12.  <!-- rule12-freeze: base=d4c28cf8b86ec2887a29ccd870a9f4cc6ec83034 region=below offset=978 lines=256 bytes=31039 sha256=5e57474a1ccffad2904dad7c87a59acbe3df3ef7177c1292c182c2dc216c03ea -->

> **🔴 PARTIAL DEMOTION 2026-07-19 (deep-space reactive-bulk ruling, Rule-12 — SCOPED to the Asteroid-Belt/Oort corollary only; all claims/caveats above PRESERVED unedited).** The **$\tau_{yield}$ formula, the saturated slipstream ($\eta\to0$), and the dark-matter added-mass reading SURVIVE** — they are reactive and untouched. **Only** the *dissipative* Asteroid-Belt/Oort corollary in the leaf (`magnetic-saturation.md`:38,:40 — detritus "dissipates its kinetic energy via topological Joule heating and becomes physically stalled" against "the resistive deep-space metric"; noted here in the caveat "Asteroid Belt / Oort Cloud as transition traps … interpretive prediction" and the strengthen-by "match asteroid belt / Oort cloud … to $\tau_{yield}$ isoclines") is **MECHANISM-RETRACTED**: a sub-yield bulk resistive stall is Ax3-forbidden (`../common_equations/eq_axiom_3.tex`:24). Per the ruling (Grant, in-chat 2026-07-19): the deep-space bulk coupling to slow matter is **lossless / pure-reactance**; the belt/Oort structure re-derives as **band-structured reactive coupling** (structure-at-resonances, Kirkwood-gap grammar), **SPEC'd, not run**. This is consistent with the leaf's own caveat that "drag is bounded and structured throughout the regimes." Walk-record: `research/2026-07-19_deep-space-reactive-bulk-walk_RECORD.md`. Leaf banner: `magnetic-saturation.md` (same date). Solidity **unchanged** (the τ_yield formula, not the belt corollary, sets this entry's band).  <!-- rule12-freeze: base=aa626690705f170874ee85e923ed170f212e57f3 region=above offset=0 lines=1 bytes=1 sha256=01ba4719c80b6fe911b091a7c05124b64eeece964e09c058ef8f9805daca546b -->
...
- _Specific Non-Claims and Caveats_
  ...
  - **"Cauchy" is a 3-way homonym (collapse-batch T7; reinforces the standing GR-import negative, no values changed).** (i) the *Cauchy elastic solid* (a constitutive class); (ii) the **"Cauchy relation $K=2G$" mislabel** on the AVE vacuum ratio (the :644 claim — actually the GR trace-reversal lock, $\nu=2/7$); (iii) the **genuine classical Cauchy relation** — a central-force lattice ($c_{12}=c_{44}$, every site a centre of symmetry) reduces in the isotropic limit to $\lambda=\mu$, giving $\nu=1/4$ and $K=\tfrac{5}{3}G$. AVE's $K=2G$ instead requires $\lambda=\tfrac{4}{3}\mu$ ($\nu=2/7$). So the one classical-elasticity route that *could* have forced the ratio (the Cauchy relation) forces a **different** value ($\nu=1/4 \neq 2/7$) — naming AVE's $\nu=2/7$ lock "the Cauchy relation" both mis-cashes the namesake AND papers over the very GR-import the echo-register already flags (see strengthen-by: $K=2G$ stays GR-imported, PR #261/#506/#508). This is name-hygiene that *strengthens* the standing negative, not a new chord.
...
- strengthen-by:
  ...
  - Justify the Cauchy relation K_vac = 2G_vac for the chiral micropolar lattice (it is stated as the isotropic-lattice Cauchy relation but the substrate is explicitly non-Cauchy/micropolar per the implosion-paradox argument) — **STAYS OPEN (2026-07-04, PR #508):** the genuine 48×48 chiral micropolar Bloch eigensolve on the ratified srs-z3 net does NOT close it. The full micropolar sector (rotational DOF included — the corpus's own proposed K=2G mechanism, clm-o3q9ul) gives a one-parameter ν_eff(ρ) family, not a forced K=2G; the independent κ_rot is a k→0 Cauchy-grade spectator sourcing no chiral coupling, and the geometry-fixed chiral B moves ν_eff AWAY from 2/7. So the substrate does not supply K=2G at either the Cauchy grade (PR #506) or the micropolar grade (PR #508); K=2G stays GR-imported (PR #261). See `research/2026-07-04_srs-chiral-micropolar_result.md` §2b, §5.
...
- _Specific Claims_
  ...
  - Edge-strain and node-displacement updates take the explicit Symplectic-Euler form $I_{new} = I_{old} + (\Delta t/L)(V_A - V_B)$ and $V_{new} = V_{old} + (\Delta t/C)(\sum I_{in} - \sum I_{out})$ respectively, enforcing the **discrete Gauss-generator conservation** and energy conservation across the discrete crystal. *(Rescoped 2026-08-10 under R43 — previously "enforcing local gauge invariance"; the exact content is the **residual** time-independent gauge family and its Noether charge $\nabla\cdot(\varepsilon_0\partial_t\mathbf{A})$, not full local U(1), which is not a symmetry of the written action. Canonical `eq_axiom_3.tex`. No re-grade: this is a scope correction to a claim BODY, not a confidence change.)*
...
## K4 Double Cover $2T \subset SU(2)$ — Substrate-Native Spin-½ STRUCTURE (selection imported)
<!-- id: clm-rkisb8 -->
...
- rationale: The pure group theory is correct and standard: 1→Z_2→2T→A_4→1, |2T|=24, 2π→−I and 4π→+I are exact SU(2) facts. The physics step that earns the title — fields actually transforming under 2T (substrate-native spin-½) via the Finkelstein–Misner mechanism — is explicitly deferred to the spin-half-paradox leaf and Vol 1 Ch 8, so locally this entry establishes the double-cover structure but only asserts (does not derive) the spin-½ realization. **REPRESENTABILITY established (2026-06-19, #299):** the spin-doublecover gate (`research/2026-06-19_spin-doublecover-gate_result.md`, `CosseratField3D.probe_spin_doublecover_holonomy`) showed the AVE substrate ADMITS the 2T⊂SU(2) double-cover action on the extended unknot's frame as a distinct, lattice-level frame transport — the SU(2)-lift holonomy returns −I@2π and +I@4π and DIFFERS from the trivial-vector (SO(3)/T=A4) baseline (which returns to +I@2π to machine precision), stable across R=4,6,8 cells. This is REPRESENTABILITY (the 2T action is representable and observable), Class-D — it is **NOT dynamics-selection**: the gate does not show the seeded field DYNAMICALLY selects 2T over T (kinematic only — no energy minimization, no time-stepping; that selection is the FM-on-K4 *physical* argument, not re-derived here). So the claim is upgraded from "asserts (does not derive)" to "the necessary lattice-level representability condition is now demonstrated; the dynamical selection remains the FM argument, not derived." **SELECTION-POSITED strengthening (2026-07-08, [SPIN-HALF-POSITED], #584/#585):** the dynamical selection is not merely un-derived — it CANNOT be forced by $\pi_1$ alone. $\pi_1$ of the SO(3)-Cosserat configuration space is $\mathbb{Z}_2$, but $\mathrm{Hom}(\mathbb{Z}_2, U(1))$ has TWO elements — the trivial homomorphism (boson, $+1$ on the noncontractible loop) and the sign homomorphism (fermion, $-1$) — so $\pi_1 = \mathbb{Z}_2$ ADMITS both statistics and FORCES neither. Selecting spin-½ requires an action-level $\mathbb{Z}_2$/Wess–Zumino term, which the corpus IMPORTS via the half-angle lift $U = \exp(i\,\boldsymbol{\sigma}\cdot\boldsymbol{\omega}/2)$ (the ½ is inserted by hand, not forced by the substrate). Confirmed four independent ways: (1) the $\pi_1$-admits-not-forces result (#584); (2) the entire soliton literature imports the sign (Finkelstein–Rubinstein admit-not-force; Witten's Wess–Zumino coefficient matched to $N_c$; Wilczek–Zee's $\theta$ free); (3) the no-derived-weld half-flux run [HALF-FLUX-ECHO] (#585); (4) the pre-sector homotopy argument — no LOCAL term in ANY sector can weld a winding-blind homotopy invariant to a $q$-keyed winding class. So: the double-cover STRUCTURE is derived (native SO(3) / $2T \subset SU(2)$); only the SELECTION (which of the two $\mathrm{Hom}(\mathbb{Z}_2, U(1))$ elements) is imported — PEER-WITH-SM (SM also posits spin-½). CORRECTED SCOPE: the honest statement is NOT "no weld in the transverse sector" (a retracted rescue narrative) but "no LOCAL term in ANY sector welds a winding-blind homotopy invariant to a $q$-keyed winding class."
- strengthen-by:
  ...
  - **[route to a derivation — currently EMPTY]** A genuine substrate derivation of the SELECTION (not just the structure) needs an action-level term tying the $\mathbb{Z}_2$ sign to a $q$-keyed winding class — concretely, an odd-winding texture-class carrying the half-flux. The no-derived-weld half-flux run [HALF-FLUX-ECHO] (#585) shows no such weld emerges from the current engine (echo, not chord); the odd-winding texture-class is the concrete but presently-EMPTY route. Until such a term is derived, spin-½ SELECTION stays imported (PEER-WITH-SM). See clm-salw2h ([`../vol2/claim-quality.md`](../vol2/claim-quality.md)).
---
...
- _Specific Claims_
  ...
  - The Cosserat $\rho, I_\omega$ set the rotational-sector mass scale; the K4 scalar sector ($A_1$, longitudinal/translational $u$) stays massless — the photon — while $T_2$ (transverse/microrotational $\omega$) carries the mass-gap content. **[G2 RESOLVED 2026-07-03 — the photon is the massless *transverse-translational $u$* pair (NOT the microrotational $\omega$, and NOT the $A_1$ *longitudinal* mode which dissipates via Gauss); the gapped $\omega$ is the mechanical Cosserat winding sector. See the ✅ RESOLVED note below + `research/2026-07-03_g2-photon-relabel_note.md`.]**
- _Specific Non-Claims and Caveats_
  ...
  - **⚠ CHANNEL→DOF LABEL COLLISION — DISAMBIGUATE, ADJUDICATION PENDING — Grant (2026-07-03; §8 rotation un-conflation, `research/2026-07-03_em-readout-vsector-stage1_prereg.md` §8 correction-note item 3, GAP G2).** Specific-Claim line 1131 above labels the **massless $A_1$/scalar sector "the photon"** ("the K4 scalar sector ($A_1$…) stays massless — the photon"). This **inverts** the canonical `photon-identification.md:11` (verbatim: *"The photon is the K4-TLM's stable $T_2$-only bound state … the $A_1$ scalar/longitudinal sector dissipates … the $T_2$ transverse triplet survives as the photon"*), where the **photon is $T_2$** and $A_1$ is the **dissipating (non-photon)** sector. The mass-status split ($A_1$ massless / $T_2$ gapped) may be self-consistent under the PROVENANCE-vs-STATE reading (`cosserat-mass-gap.md:147`), but the **photon-label** on the massless-$A_1$ sector is the point of inversion (corpus GAP G2, `_orchestration/2026-06-07_electron-synthesis-epic.md:319`). Surfaced per flag-don't-fix; NOT adjudicated — Grant rules which side is canonical. Mirror site: [`cosserat-mass-gap.md:145`](./axioms-and-lattice/ch1-fundamental-axioms/cosserat-mass-gap.md).
  - **✅ G2 RESOLVED (2026-07-03, Grant-adjudicated — the ADJUDICATION-PENDING note above is PRESERVED unedited per Rule 12; ruling `research/2026-07-03_g2-photon-relabel_note.md`, verified by the Step-1 eigenvector-composition rider `g2_photon_eigvec_composition.py/.json`).** Grant ruled (verbatim): *"the photon = the two massless transverse-TRANSLATIONAL branches (u-family); its magnetic content = the EM-inductive circulation of the u-wave (bond-level curl content), NOT the node micro-rotation; the node ω = the GAPPED mechanical Cosserat sector."* **Canonical side:** the photon is the **massless *transverse-translational $u$*** family — so line 1131's "$A_1$ … translational $u$ … massless — the photon" had the **RIGHT sector-family (translational)**, resolved AGAINST the microrotational-$\omega$ side. The one correction: the photon is the *transverse* subset (the massless shear pair at $c=\sqrt{G/\rho}$), NOT the $A_1$ *longitudinal* mode (which dissipates via Gauss); and the "$T_2$ microrotational $\omega$ carries the mass-gap" clause is re-read as **the gapped mechanical Cosserat sector** (the winding's home), never the photon. **Decisive numbers:** massless transverse branches $u$-dominated ($\omega$-frac max $2.5\times10^{-7}$, $u$-frac $1.000000$); gapped $\omega$-dominated ($1.000000$). Gap VALUE ($m^2=4G_c/I_\omega$) unchanged; confidence unchanged — only the photon-family label is settled. Mirror site: [`cosserat-mass-gap.md:145`](./axioms-and-lattice/ch1-fundamental-axioms/cosserat-mass-gap.md).  <!-- rule12-freeze: base=533b7f2563219532d257654372a39559f5af1948 region=above offset=147 lines=242 bytes=28546 sha256=1799f85aa40a001357443543211c0f4daab353352f3cae2bb1f0c543d213bd49 -->
...
> **🟡 SPIN-½ SELECTION-IMPORT REFINEMENT NOTE (2026-07-09, Rule 12 — flag-for-the-auditor, NOT a claim/rationale edit; the "do NOT edit clm-i4p11y" directive above is held; body byte-untouched).** Sharpening the 2026-06-20 gate-support note above: Specific Claim 3's "spin-½ = Finkelstein–Misner $4\pi$ double-cover" projection is now scoped **STRUCTURE-derived / SELECTION-imported (PEER-WITH-SM)** per [SPIN-HALF-POSITED] (#584/#585). $\pi_1$ of the SO(3)-Cosserat configuration space is $\mathbb{Z}_2$, and $\mathrm{Hom}(\mathbb{Z}_2, U(1))$ has TWO elements — the trivial (boson, $+1$) and the sign (fermion, $-1$) homomorphism — so $\pi_1$ **admits both statistics and forces neither**; selecting spin-½ imports an action-level sign (the half-angle lift), PEER-WITH-SM (SM also posits spin-½). The double-cover REPRESENTABILITY (#299, note above) and the double-cover STRUCTURE are substrate-native; only the fermionic SELECTION is imported. Current scope lives at clm-rkisb8 (the 2026-07-08 SELECTION-POSITED strengthening). Recorded at honest scope; does not convert this claim's solidity band and is not "derived."  <!-- rule12-freeze: base=b72fd4dec5687d12d4ddb208a5ae12e5b4793c84 region=above offset=91 lines=112 bytes=12645 sha256=a585efc77f786c719d8634ee66a0d970f1917c4e4aa691581d899f34969b3275 -->
...
- _Specific Non-Claims and Caveats_
  ...
  - **↗ CARRIER-NATIVE on srs-z3 (2026-07-04, PR #515 — Lorentz-on-srs P1 acceptance gate; the FORM re-clears, the conditional horn stays conditional).** The load-bearing crystallographic step — "the first anisotropic invariant for the cubic point group is the quartic $q_x^4+q_y^4+q_z^4$, so observable anisotropy is $(q\ell_{node})^4$-suppressed" — is a **cubic-point-group fact**, and the ratified chiral **srs-z3** carrier (point group **432**, cubic) delivers it identically to diamond ($m\bar3m$): genuine srs Bloch bond-moments give $\langle(\hat q\cdot\hat d)^2\rangle$ isotropic + $\langle(\hat q\cdot\hat d)^4\rangle$ the pure cubic harmonic ($\kappa_{srs}=-1/12$ vs $\kappa_{diamond}=-2/9$; both ECHOs, both below SME bounds). The $Fd\bar3m$ "diamond-cubic" phrasing over-specifies; **432 ∈ cubic**, so the emergent-Lorentz / isotropy defense survives the migration carrier-native, and the two transverse photon branches are DEGENERATE ($\max|\omega_{T1}-\omega_{T2}|=1.7\times10^{-14}$ — no cold birefringence, the Letter baseline holds). This re-clears the FORM ONLY; the P1b-demoted conditional $(q\ell)^4$ **photon-DISPERSION** horn stays conditional on weak-C (the raw eigensolve is $\mathcal O(k^2)$ on BOTH carriers, gate `wejkhvnfb` OPEN — the 🔴 P1b demotion above is UNCHANGED). Provenance: `research/2026-07-04_lorentz-on-srs_result.md` §4 (S2), driver `src/scripts/vol_4_engineering/lorentz_on_srs.py`.
  - **↗ DOUBLY-PROTECTED LEADING-ORDER ISOTROPY (2026-07-04, PR #516 MERGED — the parent-condition derivation; un-demotes nothing).** The emergent-Lorentz **leading-order isotropy** this claim underwrites (the $q^0$/$q^2$ corrections are isotropic for the cubic point group) holds at the srs net's isotropic-bond operating point $k_s=k_a$, which is now **Axiom-3-FORCED knob-free**: $k_s=k_a$ is the UNIQUE bond-stiffness ratio where the substrate's internal-boundary acoustic reflection $\Gamma_{internal}(\rho_{bond})$ vanishes ($\rho_{bond}=k_a/k_s=1$ to machine precision; ½/¼ knife passes). So the emergent-Lorentz isotropy is now **doubly protected** — cubic-symmetry-automatic (the standing crystallographic step) AND Ax3-forced at the unique zero-internal-reflection point. **SCOPE (KEEP-BOTH):** strengthens the LEADING-ORDER ISOTROPY statement ONLY; the 🔴 weak-C / 🔴 P1b demotions of the $(q\ell_{node})^4$ **forward-prediction** horn (gate `wejkhvnfb`, OPEN) are **UNCHANGED**, and solidity is UNCHANGED (this is the low-$k$ Lorentz emergence the "📌 SCOPE-NARROWING NOTE" in `preferred-frame-and-emergent-lorentz.md` §4.1 already holds separate from the quartic-tell). Provenance: `research/2026-07-04_parent-condition-match-forces-balance_result.md` (VERDICT BOX §1–§2 + §5), driver `src/scripts/vol_4_engineering/parent_condition_match_forces_balance.py`.
...
The chiral srs vacuum net ($I4_132$, $z=3$, 4-site BCC primitive cell) has a **fully characterized linear band structure** in both the scalar (4-band) and vector/Cosserat-translational (12-band) channels. The **scalar** global band top is the closed form $\omega_{\text{top}}^{\text{scalar}}=\pi\sqrt3\,\omega_C=5.4414\,\omega_C=2.781$ MeV at the $H$ point (via the transmission-line arccos map $\omega=\omega_{\text{link}}\arccos(\mu/3)$, $\omega_{\text{link}}=\sqrt3\,\omega_C$); the **vector** band top is a **BRACKET** $[5.441,17.011]\,\omega_C=[2.781,8.693]$ MeV. **NO full internal stop-band exists in EITHER channel** — both manifolds are connected $0\to\text{top}$. The **load-bearing methods fact**: the substrate-native band model is the **transmission-line arccos (coined-quantum-walk) map**, NOT the graph-Laplacian $\omega=\sqrt\lambda$ (which fails the frozen $1/\sqrt3$ velocity gate, giving $1/\sqrt2$). **CONSISTENCY / characterization class** — a measurement of the substrate's own linear dispersion, not a falsification and not an emergence claim. Consolidates the merged scalar (#604) + vector (#607) surveys.

- _Specific Claims_
  - **Scalar band top (closed form):** $\pi\sqrt3\,\omega_C=5.4414\,\omega_C=2.781$ MeV at $H$ ($\mu=-3$, $\lambda=6$). Gates PASS: acoustic $1/\sqrt3$ to $3\times10^{-9}$; $\lambda_{\max}=6.000000$ vs direct `build_srs_net`; 4 bands $\forall\mathbf k$, both enantiomorphs identical to $10^{-9}$.
  - **NO internal gap (both channels):** scalar (4 bands) and vector (12 bands) each form a single connected manifold — all adjacent band-pair envelopes overlap. The $k_a\gg k_s$ split did NOT open a full internal gap.
  - **MODEL ADJUDICATION (methods fact):** the graph-Laplacian $\omega=\sqrt\lambda$ FAILS the frozen $1/\sqrt3$ gate ($1/\sqrt2$, the lumped $\omega\to0$ limit); the distributed transmission-line arccos map PASSES ($1/\sqrt3$) and IS substrate-native. Under it srs band top $=\pi\sqrt3=5.441\,\omega_C$ (lumped would give $\sqrt{12}=3.464$).
  - **Vector band top BRACKET $[5.441,17.011]\,\omega_C$:** the arccos map does NOT cleanly generalize (3 acoustic branches, 2 speeds, anisotropic self-block). Estimates: normalized-arccos 5.441 (LOWER, ρ-independent pin), P-wave-scaled 9.935, lumped-calibrated 12.406, raw-link $\sqrt{\rho^*}$ 17.011 (UPPER). $\rho^*=9.77337$ DERIVED from $\nu_{\text{Hill}}=$`N_NU`$=2/7$.
  - **Per-branch velocity map:** $\sqrt3$ = universal network/coordination projection ($c_{\text{link}}\to c_0$, ALL branches, NOT a channel selector); $\sqrt{10/3}$ = P-wave (longitudinal) $c_P/c_S$ at the VRH average ONLY (a K=2G re-expression, GR-imported PR #261, NOT lattice-emergent); per-direction 1.71/1.85/1.90 (Zener $A=1.23$, lattice-computed); $\sqrt2$ = A1-port bulk sound (`constants.py:770` import, NOT a translational Bloch branch); shear branch $=c_0$ (light-like PROXY; true photon = T2 microrotation, named follow-on).
- _Specific Non-Claims and Caveats_
  - **⚠ Vector single-scale vs stiffness-lifted is PENDING Grant (NOT resolved here).** The bracket is the honest result; Grant's ruling sets the fork-A tone floor (5.441 vs ~17 $\omega_C$) and the pair-channel coexistence window, but does NOT change the band SHAPE, the velocity table, or the gap inventory.
  - **This kills GAP-PINNED carriers only.** No internal gap ⇒ intrinsic-gap-breather candidates UNAVAILABLE in both channels; the **above-band mobile breather** (freq > the top) is NOT falsified and remains the live carrier-fork branch.
  - **Consistency-vs-emergence:** $\omega_C=c_0/\ell_{\text{node}}$ IDENTITY (`OMEGA_C`); $1/\sqrt3$ Class-B manifestation (`ANALYTIC_NETWORK_FACTOR`); $\nu=2/7$ GR-imported (`N_NU`/`NU_VAC`, K=2G, PR #261/#506); $\sqrt{10/3}$ a K=2G re-expression; $\sqrt2$ an A1-port import. No $\alpha$/`Q_TANK`, no CODATA on any verdict path; forward computation only. NOT an emergence claim.
  - **$\omega$-vs-k discipline:** $\pi\sqrt3=5.441\,\omega_C$ is a FREQUENCY band top ($\omega$-space, at $H$); it is a distinct quantity from the k-space Nyquist WAVEVECTOR edge ($\sqrt2\,\pi/\ell_{\text{node}}$ srs, $\sqrt3\,\pi/\ell_{\text{node}}$ cubic) at clm-3bwhad. The $\sqrt3\,\pi\approx5.44$ coincidence spans both axes but they are not the same object.
  - **Scale-label flag (R2 adopted):** $\omega_C=c_0/\ell_{\text{node}}=511$ keV under R2 ($c_0=$ acoustic-branch velocity). Under R1 every $\omega_C$ label divides by $\sqrt3$; only the scale LABEL changes, not the band SHAPE or gap inventory.

> **Leaf references:** [srs-band-structure](./operators-and-regimes/ch6-universal-operators/srs-band-structure.md).

### Quality
- confidence: 0.80
- depends-on:
  - Axiom 1 (Substrate Topology — the chiral srs net $I4_132$, $z=3$, whose $4\times4$ scalar and $12\times12$ vector Bloch dispersion is what this survey diagonalizes)
- solidity: 0.80 (ok to build on, see caveats) [= min(0.80, 1.00)]
- rationale: A clean CONSISTENCY-class characterization: every gate is COMPUTED against an independently-derived canonical number (acoustic velocity factor vs the imported `ANALYTIC_NETWORK_FACTOR`; $\lambda_{\max}$ vs the direct `build_srs_net`; the vector scalar-reduction vs the scalar survey to $2.7\times10^{-15}$; $\nu_{\text{Hill}}$ vs the validated 8-site Born-Huang), both enantiomorphs identical, and the scalar top is a pinned closed form ($\pi\sqrt3$). The load-bearing methods fact (arccos TL map substrate-native; graph-Laplacian fails the $1/\sqrt3$ gate) is a model-selection result, not an assertion. Confidence 0.80 (not higher): the observables are analytic Bloch quadratures, not engine-dynamically-evolved; the vector band top is an honest BRACKET (the arccos map does not cleanly generalize to 3 branches / 2 speeds) with the single-scale-vs-stiffness-lifted fork PENDING Grant; and the vector-bracket upper values ride on the GR-imported $\nu=2/7$ (K=2G, PR #261) — an input, not a lattice-emergent number. depends-on is Axiom 1 (framework, 1.0), so solidity = confidence; the $\nu=2/7$ / $\sqrt{10/3}$ K=2G imports are scoped as ECHO inputs in the caveats rather than solidity-lowering derivation edges (peer with how clm-k4d4ph handles $\omega_C$/`N_NU` imports).
- strengthen-by:
...
- confidence: 0.83
- depends-on:
  - Axiom 1 (Substrate Topology — the chiral srs net $I4_132$, $z=3$ coordination that fixes the counting reflection $\Gamma=-1/3$ and makes the vertex a $C_3$-symmetric 3-port)
...
- _Specific Claims_
  ...
  - **Single-frequency axis:** $|S_{11}(\pi;f)|^2$ touches the $1/9$ floor EXACTLY at $f_{\text{touch}}=\sqrt{3s_L-s_C}/(\pi\,s_L\sqrt{s_C})$ ($=\sqrt2/\pi$ at $s=1$; $=1/(2\pi)$ at $(s_L,s_C)=(2,3)$; governing form = the canonical module `junction_scattering.py:192` / locus $t^2=(3s_L-s_C)/(s_C s_L^2)$, $s_L$ outside the root; `obj1@touch − 1/9 = −4.2e-17`) — a genuine second global minimum (the perfect-square identity, `clm-v3port`), where the section is half-wave at the probe tone and thus impedance-transparent there. The primary objective therefore fires **branch (iv)** (degenerate), not a unique point-junction selection.
