...
- _Specific Claims_
  ...
  - **Fitted (one scalar at $T_{\text{CMB}}$):** the numerical magnitude $\delta_{strain} \approx 2.225 \times 10^{-6}$ is back-subtracted from CODATA, definitional given the engine's `DELTA_STRAIN = 1 - (1/ALPHA)/ALPHA_COLD_INV`. The attribution to substrate spatial-metric response at $T_{\text{CMB}} = 2.725$ K is a substrate-mechanism narrative consistent with the predicted sign — the substrate-saturation-class for this loading is OPEN (**Q-DELTA-MAP-1**, three candidate paths P1/P2/P3 per [`research/2026-05-28_phase-3-a3-delta-strain-machian-projection-result.md`](../../../research/2026-05-28_phase-3-a3-delta-strain-machian-projection-result.md) §3) — not a derivation of magnitude.
...
- _Specific Non-Claims and Caveats_
  - Does NOT claim full ab-initio derivation of the CODATA $\alpha$ from axioms alone in the current edition. The claim is "structurally zero-parameter conditional on thermal closure of $\delta_{strain}$ at $T_{\text{CMB}}$" — the magnitude of $\delta_{strain}$ is an open substrate-physics derivation, framed substrate-natively as **Q-DELTA-MAP-1**: what substrate saturation class does low-amplitude electromagnetic thermal-bath loading fall into? (Three candidate paths surfaced via Phase 3-A3 2026-05-27 WALK-BACK; see [`research/2026-05-28_phase-3-a3-delta-strain-machian-projection-result.md`](../../../research/2026-05-28_phase-3-a3-delta-strain-machian-projection-result.md) §3.) NOT yet present in canonical content.
...
- strengthen-by:
  ...
  - Adjudicate **Q-DELTA-MAP-1** substrate-mechanism class for low-amplitude electromagnetic thermal-bath loading (CMB at $T_{\text{CMB}} \approx 2.725$ K), then derive δ_strain magnitude from the chosen class — closing the cold-lattice → CODATA bridge rather than back-subtracting. (Phase 3-A3 attempted; **WALK-BACK 2026-05-27** — see [`research/2026-05-28_phase-3-a3-delta-strain-machian-projection-result.md`](../../../research/2026-05-28_phase-3-a3-delta-strain-machian-projection-result.md) §3. The prior "G_vac + equipartition" SM-vocabulary framing of this open item was reworded substrate-natively as Q-DELTA-MAP-1 saturation-class adjudication.)
...
- _Specific Claims_
  - The cold-lattice asymptote $\alpha^{-1}_{\text{ideal}} = 4\pi^3 + \pi^2 + \pi \approx 137.0363038$ (substrate-derived; see clm-0ktpcn) is the $T \to 0$ limit; the measured CODATA value $137.035999$ sits below it by the fractional vacuum strain coefficient $\delta_{\text{strain}}$, attributed to **substrate spatial-metric response to the cosmological CMB photon-bath operating environment** at $T_{\text{CMB}} \approx 2.725$ K. The substrate-mechanism class for this loading (saturation-kernel-modulated SYM vs ASYM vs a new third class vs substrate-bond rest-length thermodynamic equilibrium) is **OPEN** — see Q-DELTA-MAP-1 cross-link in rationale below.
...
- _Specific Non-Claims and Caveats_
  - Does NOT claim the *magnitude* is derived: it is one back-subtracted scalar (a Class A identity at the back-substitution step), not yet a substrate-physics derivation. The narrative attribution to substrate spatial-metric response to the CMB photon-bath operating environment is consistent with the predicted sign but is not a magnitude derivation — the substrate-mechanism class for low-amplitude electromagnetic thermal-bath loading is OPEN (Q-DELTA-MAP-1; see [`research/2026-05-28_phase-3-a3-delta-strain-machian-projection-result.md`](../../../research/2026-05-28_phase-3-a3-delta-strain-machian-projection-result.md) §3 for candidate paths).
...
- rationale: definitional-identity (the magnitude). The sign and existence of a positive thermal running of $\alpha^{-1}$ below the cold-lattice asymptote are axiom-derived (substrate spatial-metric response to finite-$T$ photon-bath loading); the magnitude $\delta_{\text{strain}} \approx 2.225 \times 10^{-6}$ is FITTED — one scalar back-subtracted from CODATA, definitional given the engine constant `DELTA_STRAIN`. It is NOT yet derived from substrate primitives. Distinct from QED vacuum polarization and from the proton Skyrme thermal correction $\delta_{th} = 1/(14\pi^2)$. Confidence 0.45 set by the applied-mathematician pass. **2026-05-27 Phase 3-A3 WALK-BACK** (branch `analysis/phase-3-a3-delta-strain-machian-projection`): the prework brief proposed deriving $\delta_{\text{strain}}$ from a Machian-G operating-point cascade ($u_0^* \to A_0^{cosmic}/A_{yield} \to$ INVARIANT-S2 SYM scaling $\to \alpha_{eff}/\alpha_{cold} = 1/S^{3/2}$). Step 3.5 audit found this derivation chain has a substantive substrate-physics inconsistency with canonical clm-3zz0f6 (α Invariance Under Symmetric Gravity, confidence/solidity 0.85): SYM-class scaling produces $\alpha$ exactly invariant via the canonical algebraic substitution $\alpha = e^2/(4\pi (\varepsilon_0 nS) \hbar (c_0/(nS))) = \alpha_0$, NOT modulated. The prework brief's $c_{eff} = c_0\sqrt{S}$ substitution corresponds to $c_{shear}$ (group/rest-mass-freeze velocity per clm-8nkvwy line 113), NOT the $c_{EM}$ phase velocity that enters $\alpha$ ($c_{EM,sym} = c_0/S$ per clm-8nkvwy line 111). The canonical saturation-class taxonomy (SYM mass-energy loading + ASYM strong-EM loading per clm-8nkvwy) does NOT include a third class for low-amplitude electromagnetic thermal-bath loading (CMB at 2.725 K). Phase 3-A3 surfaces this as framework-extension question **Q-DELTA-MAP-1**: three candidate substrate-physics paths P1/P2/P3 (new third class; ASYM at thermal-bath amplitude; substrate-bond rest-length thermal contraction independent of saturation kernel) — see [`research/2026-05-28_phase-3-a3-delta-strain-machian-projection-result.md`](../../../research/2026-05-28_phase-3-a3-delta-strain-machian-projection-result.md) §3 for the full analysis. All three candidates fail at order-of-magnitude under naive estimates, suggesting the substrate-mechanism producing $\delta_{\text{strain}}$ magnitude is structurally different from any of the enumerated candidates — requires Grant adjudication for substrate-mechanism direction-setting. Confidence STAYS at 0.45 (vocabulary-cleanup-only walk-back does NOT lift derivation-class evidence). Class E framework membership in the Machian-G cosmic-substrate family ({α, G, H_∞, Ω̂_freeze}) is canonical at [`omega-freeze-cosmic-grain-cascade.md`](../common/omega-freeze-cosmic-grain-cascade.md); promoting $\delta_{\text{strain}}$ to a 5th joint-constrained projection within that family is GATED on Q-DELTA-MAP-1 substrate-mechanism closure.
- strengthen-by:
  - Adjudicate **Q-DELTA-MAP-1** substrate-mechanism class for low-amplitude electromagnetic thermal-bath loading (candidate paths P1/P2/P3 per [`research/2026-05-28_phase-3-a3-delta-strain-machian-projection-result.md`](../../../research/2026-05-28_phase-3-a3-delta-strain-machian-projection-result.md) §3), then derive $\delta_{\text{strain}}$ magnitude from the chosen class. This replaces the prior "G_vac + equipartition" SM-vocabulary framing of the open derivation step; the substrate-native open item is the saturation-class identification first, then the magnitude derivation within that class. The 2026-05-27 Phase 3-A3 WALK-BACK established that the prior framing leaks SM-vocabulary mechanism (classical thermal-expansion coefficient + heat-capacity equipartition) onto a substrate-physics problem; substrate-native framing requires Q-DELTA-MAP-1 saturation-class adjudication first. SAME open gap as clm-0ktpcn's existing strengthen-by item, also reworded substrate-natively under Phase 3-A3 WALK-BACK.
---
