...
- solidity: 0.60 (use as input only, don't build deeper) [= min(0.85, 0.60)]
...
- confidence: 0.95
- depends-on:
  - clm-0ktpcn — Golden Torus α Derivation (solidity 0.60) [for $V_{yield}$'s $\sqrt{\alpha}$ factor]
- solidity: 0.60 (use as input only, don't build deeper) [= min(0.95, 0.60)]
...
- confidence: 0.90
- depends-on:
  - clm-0ktpcn — Golden Torus α Derivation (solidity 0.60) [for $\xi$'s $\alpha^{-2}$ factor]
...
- solidity: 0.60 (use as input only, don't build deeper) [= min(0.90, 0.60)]
...
- confidence: 0.50
- depends-on:
  - clm-0ktpcn — Golden Torus α Derivation (solidity 0.60) [for $p_c = 8\pi\alpha$ in the porosity factor]
...
- confidence: 0.60
- depends-on:
  - clm-0ktpcn — Golden Torus α Derivation (solidity 0.60) [for muon's $\alpha\sqrt{3/7}$ and tau's $8\pi/\alpha$ sector factors]
...
- confidence: 0.75
- depends-on:
  - clm-0ktpcn — Golden Torus α Derivation (solidity 0.60) [for $r_1 = \sqrt{2\alpha}$ numerical value]
- solidity: 0.60 (use as input only, don't build deeper) [= min(0.75, 0.60)]
...
- confidence: 0.70
- depends-on:
  - clm-b2anl4 — Four-Regime Map (solidity 0.60) [uses $r_1, r_2, r_3$ boundaries to classify domains]
...
- solidity: 0.60 (use as input only, don't build deeper) [= min(0.62, 0.60)]
...
- solidity: 0.60 (use as input only, don't build deeper) [= min(0.80, 0.60)]
...
- solidity: 0.60 (use as input only, don't build deeper) [= min(0.72, 0.60)]
...
- solidity: 0.60 (use as input only, don't build deeper) [= min(0.74, 0.60)]
