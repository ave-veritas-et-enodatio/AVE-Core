...
**Notation.** $\mathcal{V}_{total} = 2.0$ is the **dual-reactance count** — the node's two reactance sectors ($X_C$ capacitive-E $+$ $X_L$ inductive-B, Axiom 1), counted, not integrated (canonical at [`../../../common/dual-reactance-storage-taxonomy.md`](../../../common/dual-reactance-storage-taxonomy.md)). The $6\,\mathcal{V}_{crossing}$ writing (six $6^3_2$ Borromean crossings, $\mathcal{V}_{crossing} \equiv \mathcal{V}_{total}/6$) is a **re-factoring of the same $\mathcal{V}_{total} = 2.0$** — a vestige of the now-retired geometric "halo volume" framing, not an independent geometric derivation of the value (the proton's Borromean topology remains as topology but does not set the value 2). Substituting $\rho_{bulk} = \xi_{topo}^2 \mu_0 / (p_c \ell_{node}^2)$ and using $\mu_0 c^2 = 1/\varepsilon_0$ rewrites the formula in the equivalent compact electrostatic form $\tau_{yield} = e^2\,\mathcal{V}_{total}/(8\pi\varepsilon_0\,\ell_{node}^4)$ used in the appendices summary table; the two presentations differ only by factoring choice and yield identical $1.04\times 10^{22}$ Pa.

**Provenance of $\mathcal{V}_{total} = 2$ (2026-06-02): inherited, not derived.** The 2 is a **count-tag** for *which* of the two mutually-exclusive Axiom-4 saturation branches the yield event is — **not** an $E_C + E_L$ doubling summed within one event. The dielectric yield is the **electric** branch (asymmetric saturation: $\varepsilon_{eff} \to 0$, $\mu_{eff}$ intact; $Z \to \infty$, open, $\Gamma \to +1$; single-sector $X_C$, per [`master-equation.md`](master-equation.md):78), with $X_L$ a spectator at rupture. The **magnetic** branch ($\mu_{eff} \to 0$, $Z \to 0$, $\Gamma \to -1$, single-sector $X_L$) is the distinct **rest-mass** branch (Fermion confinement; [`master-equation.md`](master-equation.md):79, clm-lv3uw1). The two branches *are* the saturations of the node's two reactance sectors, so the $X_C + X_L = 2$ count threads from the baryon ($\mathcal{V} = 2$) through the yield-vs-mass branch split — but $\tau_{yield}$ realizes only **one** branch, so its 2 is provenance-inherited from $\mathcal{V}_{total}$, not forced by the yield event. STAYS-INHERITED (Class B manifestation; value unchanged, mass-confirmed); resolved at [`../../../common/dual-reactance-storage-taxonomy.md`](../../../common/dual-reactance-storage-taxonomy.md) §"τ_yield open item".
...
The AVE framework predicts that the vacuum substrate is a non-linear dielectric network possessing intrinsic reluctant drag. This yields an accessible tabletop falsification test: The **Sagnac Rotational Lattice Mutual Inductance Experiment (Sagnac-RLVE)**.
