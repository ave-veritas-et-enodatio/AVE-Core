...
4. Q-G19α's g-2 closure (printed-precision, postulate-conditional **[ppm label struck 2026-08-03 per Grant ruling; no ppm number substituted]**; $+4.0\%$ parameter-free Route B forward) also uses time-averaged boundary-integrated observables, not instantaneous values
...
1. **The boundary-envelope reformulation is empirically validated at the dynamic engine level.** Beyond the Q-G19α boundary-integrated g-2 closure (parameter-free Route B forward $+4.0\%$; printed precision only with the $n_q$-additivity postulate **[ppm label struck 2026-08-03 per Grant ruling; no ppm number substituted]**), the Master Equation FDTD engine demonstrates that the canonical substrate hosts the breathing bound state autonomously at one active cell. The substrate-observability rule + three-boundary-observable framework ($\mathcal{M}, \mathcal{Q}, \mathcal{J}$) is operational at both the static integrated-observable level AND the dynamic engine-simulation level.
...
- **Empirical validation companions:**
  ...
  - Q-G19α saliency closure — $\delta = -3\alpha/2$ reaches printed precision **[ppm label struck 2026-08-03 per Grant ruling; no ppm number substituted]** via $(2, 3)$ trefoil q-axis winding (also boundary-integrated), but **only conditional on the $n_q$-additivity postulate** (the corpus's "single remaining intuitive step")
