...
| [Photon EE Mapping (free $Z_0$/$\Gamma=0$ vs self-trapped $\Gamma=-1$)](./photon-ee-mapping.md) | Consolidation/translation leaf (the translation-circuit.md:173 "pending" E↔B leaf): free photon (single-sector $T_2$, matched $Z_0$, $\Gamma=0$, NO core) vs self-trapped electron (magnetic-branch $\mu_{eff}\to0$, $\Gamma=-1$ Local Bubble, shorted $\lambda/4$ resonator); carrier × envelope; the photon's own $E$↔$B$ as a LINEAR I/Q quadrature on $(V_{inc},V_{ref})$ / $V\leftrightarrow\Phi_{link}$, disambiguated from the parametric K4↔Cosserat pair-production bridge; carries the R·r≠¼ Class-B + helical-photon-retraction + autoresonance-GAP flags |
