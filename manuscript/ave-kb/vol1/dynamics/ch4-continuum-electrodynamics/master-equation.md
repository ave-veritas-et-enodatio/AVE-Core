...
2. **Particle Assembly ($V \to 43.65\text{ kV}$):** The non-linear regime divides into two distinct saturation symmetries (per Axiom 4 confinement theorem):
   ...
   > **📌 PROVENANCE SURFACE-NOTE (2026-09-07) — the banner above is PRESERVED byte-for-byte; this note records WHO SAID WHAT and takes NO position on which sector owns the cage.** Two statements in the banner above — `The electron mass-cage is the **A1 longitudinal dilatation**` and the reading that `"confinement proceeds via the magnetic/charge sector"` would `break the two-"3"s orthogonality` — are cited across the corpus as PR #260 content. Provenance, each leg verified first-hand by `git show`: **(i)** both sentences were added by commit `5d83b67c` on **2026-06-17**, two days AFTER #260 merged (`04bda99b`, 2026-06-15); **(ii)** `git show --stat 04bda99b` is 7 files and `master-equation.md` is **not** among them — #260's diff never touched this file; **(iii)** Grant's ratification of #260 is recorded as `B3 + the demotion + mute-on-mass-sector + "fix the 2 defects in-lane."` (`_orchestration/2026-06-15_wall-branch-fork.md:142`); and **(iv)** #260's own result doc explicitly LICENSES the other reading — `the wall *confines* the A1 mass, it does not *constitute* it` (`research/2026-06-15_wall-branch-fork_result.md:41`), `the T2/charge-winding sector forms the (sign-−1) wall that confines the A1/mass` as `a chirality-conventional labeling choice` (`:43`), and `this fork is **mute on "which 3 carries the mass"**` (`:43`). **This is a provenance record, NOT a verdict.** It does not say the banner is wrong, does not declare PR #260 superseded, reaffirmed or wrong, retracts nothing, and changes no status marker. Cage OWNERSHIP is OPEN and routed to Grant — `def-vyvsn1` open-ambiguity-flag item (4) in [`vocabulary-register.md`](../../../common/vocabulary-register.md), tracked at `_orchestration/open-items/2026-09-07-cage-ownership-provenance.md`.
