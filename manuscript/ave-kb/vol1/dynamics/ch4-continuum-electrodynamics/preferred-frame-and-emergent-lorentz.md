...
| **C17-PROTOCOL-11** (static-fiber galactic wind) | optical, $\lambda \sim 1$ μm | suppressed by $(q\ell_{node})^4 \sim 10^{-22}$ | NULL — corroborated by Brillet-Hall + Wolf null bounds |
...
- **C18-PROTOCOL-12** ([`divergence-test-substrate-map.md:417`](../../../common/divergence-test-substrate-map.md)) — needs separate audit. The 16.7 mm GEO-sync TOF stretch is at optical wavelength and would seem subject to cubic-symmetry suppression, but the vertical-impedance-integration mechanism is geometrically distinct from horizontal-anisotropy probes; the cubic-symmetry argument may or may not apply directly. Open question.
