...
> **🔴 WEAK-C SCOPE-NOTE (2026-06-15, Rule 12 — the header above AND the "BY DERIVATION" line at §2 are PRESERVED unedited; weak-C canonization, gate `wejkhvnfb`, Grant-confirmed 2026-06-14; prereg at repo-root `research/2026-06-15_weak-c-photon-continuum_prereg.md`).** "DERIVED" / "BY DERIVATION" here scopes ONLY to the **$(q\ell_{node})^4$ observable-wavelength cubic-isotropy / birefringence-magnitude** result (the clm-yr6tu4 crystallographic-group-theory step, solidity 0.78) — i.e. that observable-scale anisotropy is quartic-suppressed to $\delta_{aniso} \approx 2.2 \times 10^{-22}$. It does **NOT** scope to the exact-continuum-limit **no-dispersion theorem** ($\delta = 0$, $\omega = ck$), which remains **OPEN** (no-LIV is carried as a regime-grounded + empirically-corroborated PREDICTION, not a derived theorem — asserting it as derived would be strong-C over-reach; see §4.1).
...
This makes AVE qualitatively different from theories that postulate Lorentz invariance as a primitive: AVE *derives* Lorentz invariance at observable scales, *predicts* the precise OOM at which it must break down ($q \sim \pi/\ell_{node}$, i.e. Trans-Planckian wavelengths), and *names the empirical signature* (GRB dispersion at $\lambda \to \ell_{node}$). [**🔴 Weak-C 2026-06-15, Rule 12 (line above PRESERVED):** the "GRB dispersion at $\lambda \to \ell_{node}$" empirical signature is the **lattice-mode $(q\ell_{node})^2$** breakdown — it applies to **matter carriers** (node-locked), NOT to the free continuum photon (which has no zone-edge dispersion → AVE predicts the GRB NULL, corroborated). The *photon*'s observable-scale signature is the $(q\ell_{node})^4$ optical birefringence ($\delta \approx 2.2 \times 10^{-22}$, §2). See §4.1 for the full PARTIAL/KEEP-BOTH split.]  <!-- rule12-freeze: base=00759b4efa7fe072b89b47fae4ab7ce5f547a112 region=above offset=24 lines=28 bytes=2587 sha256=e4a4078c01cc861d7ab179580768252eda89f400ece4ef5f60d188eaf960c126 -->
...
| **C7-GRB-DISPERSION** (Trans-Planckian) | $\lambda \to \ell_{node}$ | NOT suppressed — at lattice resolution | ~~Surviving forward prediction~~ → **🔴 (qℓ)² GRB-dispersion horn RETRACTED (weak-C 2026-06-15, Rule 12; see §4.1)**: the free photon is the continuum EM field → **no zone-edge $(q\ell_{node})^2$ dispersion**; AVE predicts the GRB **NULL** (corroborated by GeV–TeV nulls). The $(q\ell_{node})^2$ bending is a *lattice-mode* property → it governs **matter carriers** (node-locked), not the continuum photon. The surviving *photon* forward prediction is the $(q\ell_{node})^4$ optical birefringence ($\delta \approx 2.2 \times 10^{-22}$, §2), NOT a GRB dispersion. |
...
- **A2's MECHANISM is rotor-local, NOT preferred-frame.** A2 probes rotor-local mutual-inductance coupling: a spinning tungsten rotor injects a local drift velocity $v_{network} = 0.38$ m/s into the surrounding substrate via mass-density-dependent coupling ($\kappa_{entrain} = \rho_{rotor}/\rho_{bulk}$). The bulk substrate flow past a Earth-bound rotor (uniform 370 km/s) integrates to zero around any closed Sagnac loop (basic geometry — uniform velocity field has zero curl). Only the rotor-induced non-uniform perturbation contributes. A2's prediction is independent of which frame the bulk substrate is at rest in. **This mechanism distinction is correct and unchanged.** What changed (2026-06-03 audit, `AVE-PONDER/research/2026-06-03_sagnac-rlve-fog-question-verdict.md`): the *lifecycle status*. The mechanism distinction does NOT save A2 as a forward kill-switch, because the corpus's own Ch.2 extends the **same** $\kappa$ to **Earth-as-rotor** — Earth is ALSO a rigid rotor (curl $= 2\Omega \neq 0$, rotor-local vorticity, NOT the uniform-bulk case nulled above), so the same coupling predicts a $+7\times10^{-4}$ rotation-rate bias ($\kappa_{earth} = \rho_{earth}/\rho_{bulk} = 6.97\times10^{-4}$) already excluded by existing ring-laser-gyro geodesy (G-ring/ROMY/GINGER, agreeing with VLBI/IERS) by $7\times10^4\times$. **A2 therefore retires to the SAME lifecycle bin as C17/C18 — corroborative null — with different suppression physics** (A2: rotor-local vorticity excluded by existing RLG data; C17: bulk preferred-frame cubic-symmetry-suppressed). The surviving fragment is the paired W-vs-Al $\Psi = 7.15$ self-consistency scaling check, not a forward discriminator.
...
### §4.1 — Weak-C reconciliation: the C7 GRB-dispersion horn vs. the (qℓ)⁴ birefringence horn (2026-06-15, Rule 12)

> **🔴 WEAK-C PARTIAL/KEEP-BOTH WALK-BACK (2026-06-15, Rule 12 — §4 table + bullets above PRESERVED unedited; weak-C canonization, gate `wejkhvnfb`, Grant-confirmed 2026-06-14; prereg at repo-root `research/2026-06-15_weak-c-photon-continuum_prereg.md`).**  <!-- rule12-freeze: base=8cdcef0383429f597e3ffc4562cdb6e55e1b1dc3 region=above offset=5 lines=16 bytes=2577 sha256=c2590b290f2a74f82e1c43b86cd74945ca81b8f3b4d262cb4d4c7c656dc24a56 -->
>
> The C7-GRB-DISPERSION row of this leaf (and clm-yr6tu4) carried **two distinct horns** under one "Trans-Planckian" heading. Weak-C splits them — RETRACT one, KEEP the other:
>
> - **RETRACTED — the $(q\ell_{node})^2$ GRB-dispersion horn (weak-C prereg, edits C/D).** The framing of C7 as AVE *predicting* an energy-dependent GRB arrival-time delay at $\lambda \to \ell_{node}$ ("surviving forward prediction") is **walked back**. Under weak-C the **free photon is the continuum EM field** — the LC-ladder's long-wavelength regime ($L \gg \ell_{node}$, $Z_0$/$c$ $\ell_{node}$-independent: `z0-derivation.md:40`; `master-equation.md:16`), **sub-saturation** ($\Delta\phi \ll \alpha$, $Z_0$-matched, unlocked) — so it has **no zone-edge $(q\ell_{node})^2$ dispersion**. AVE predicts the GRB **NULL**, now empirically corroborated by GeV–TeV photons crossing ~Gpc with no dispersion (GRB090510 31 GeV; LHAASO 221009A 13 TeV). This is the reading already carried at the matrix C7 row ("NULL = no dispersion = AVE survives", `divergence-test-substrate-map.md:538` — ★**cite-rot repair 2026-08-02**: this read `:456`, a blockquote-marker/blank line, never the C7 row; **same defect as the `:399` cite at the C7 bullet below, and NOT in the routing docket's enumeration of six** — found while content-locating that one, repaired in the same pass so the file does not carry one repaired and one rotten cite to the same target) and `binary-kill-switches.md:13`; the "surviving forward prediction" wording here was the contradictory horn, now retracted.
>
> - **RESCOPED to MATTER — the $(q\ell_{node})^2$ zone-edge bending (weak-C prereg, edit E).** The zone-edge group-velocity bending $v_g = c \cdot \cos(k\ell/2)$ is a genuine **lattice-mode** property — but it governs **matter carriers** (which ARE lattice-locked, by the matter = saturation-locked-to-nodes identity), **NOT** the continuum photon. That prediction **survives for matter**, not for free light.
>
> - **KEPT — the $(q\ell_{node})^4$ optical birefringence horn (weak-C prereg, edit D; the surviving forward prediction).** The continuum photon **inherits the lattice's cubic symmetry** ($Fd\bar{3}m$; first anisotropic invariant is quartic, §2), so it retains a tiny **$(q\ell_{node})^4$ optical anisotropy/birefringence**: $\delta_{aniso} \approx 2.2 \times 10^{-22}$ at $\lambda = 633$ nm (§2; clm-pp3qwf). This is distinct from the retracted $(q\ell_{node})^2$ GRB dispersion, is 2–3 OOM below current cavity bounds, and is **NOT falsified** — it is the surviving *photon* forward prediction.
>
> > **🟡 CURRENT STATUS — the KEPT $(q\ell_{node})^4$ photon horn is DEMOTED to CONDITIONAL (P1b eigensolve, 2026-06-24).** The "inherits the lattice's cubic symmetry → first invariant is quartic" step above presumes the continuum photon **decouples from the zone-edge $O(k^2)$ quadratic** — which is exactly the unproven weak-C no-zone-edge theorem (gate `wejkhvnfb`). The P1b genuine 24×24 chiral-srs Bloch eigensolve (`src/scripts/vol_4_engineering/srs_bloch_dispersion.py`, branch `engine/p1b-modes-live`) MEASURES the GENERIC band-edge anisotropy slope $1.9999$ (not 4): the photon does NOT decouple from the $O(k^2)$ term at the lattice level, so the quartic horn is a re-stated inserted exponent, **conditional** on weak-C (OPEN), and $\delta=0$ is NOT exact ($\max|\omega^2/c^2k^2-1|=10^{-3}$ at $k\ell=0.08$). **DEMOTED, not refuted** — the quartic returns if weak-C is proven; `clm-k4d4ph` solidity $0.70\to0.60$. The bond-moment 2nd-moment ISOTROPY of §2 and the small-$k$ emergent-Lorentz isotropy SURVIVE (this is band-edge anisotropy, NOT a low-$k$ Lorentz violation).
>
> **Honest-scope guard (weak-C prereg, edit F; A47 v11b):** the no-dispersion result is **regime-grounded + empirically-corroborated**, NOT a derived theorem — the rigorous proof that the continuum limit is *exact* ($\delta = 0$, $\omega = ck$) is OPEN. Strong-C ($\delta = 0$ / no-LIV-as-derived) would over-reach AND kill the $(q\ell_{node})^4$ birefringence; it is NOT landed. Continuum = a **regime** of the one substrate, NOT a coexisting field; input-count stays 3 {$m_e$, $\alpha$, $G$}. Reconciliation anchor: `k4-port-irrep-decomposition.md` §7 (free-massless vs locked-massive, same $T_2$ sector).

> **📌 SCOPE-NARROWING NOTE (2026-07-03 exposure-audit synthesis, audit B) — the P1b demotion narrows the QUARTIC-TELL, NOT the low-$k$ Lorentz emergence.** The demotion above (`clm-k4d4ph` $0.70\to0.60$) touches only the *distinctiveness/derivation status* of the $(q\ell_{node})^4$-quartic photon horn — whether the free photon's FIRST anisotropic correction is quartic-protected (weak-C-holds) or coexists with an isotropic zone-edge $O(k^2)$ term (weak-C-fails). It does **NOT** narrow the low-$k$ emergent-Lorentz result. The reason: low-$k$ isotropy is protected by the cubic point group's **second-moment identity**, $\Sigma_b(\hat q\cdot\hat d_b)^2 = \tfrac43\mathbb{I}$ (verified to spread $6.7\times10^{-16}$ node-up, `research/2026-06-22_k4-bloch-dispersion-quartic_result.md:56`; the first *anisotropic* invariant enters only at the 4th moment, `:57`) — a **group-theory fact independent of the weak-C gate.** The P1b eigensolve confirms this survives the demotion (the `:104` banner: "The bond-moment 2nd-moment ISOTROPY of §2 and the small-$k$ emergent-Lorentz isotropy SURVIVE (this is band-edge anisotropy, NOT a low-$k$ Lorentz violation)"; leaf blockquote at `k4-bloch-dispersion-quartic.md:114`). **Consequence — both weak-C branches keep the first ANISOTROPIC invariant quartic:** on weak-C-holds the photon's leading anisotropy is $(q\ell)^4$; on weak-C-fails the recovered $O(k^2)$ term is the *isotropic* 2nd moment, so the leading *anisotropic* term is STILL quartic and a Hughes–Drever / cavity anisotropy bound (which tests frame-dependent $\delta c/c$, not an isotropic scalar dispersion) does not collide on either branch. This makes the demoted leaf honestly **stronger**: the demotion cost the quartic-tell its *derivation status*, but the low-$k$ Lorentz emergence it is often read as underwriting was never resting on weak-C in the first place. Full context: `research/2026-07-03_historical-exposure-audits_synthesis.md` §B.
...
- **A2-SAGNAC matrix row** ([`divergence-test-substrate-map.md`](../../../common/divergence-test-substrate-map.md) A2-SAGNAC Predictions/Lifecycle/Execution rows) — **RETIRED to corroborative-null (2026-06-03 audit, `AVE-PONDER/research/2026-06-03_sagnac-rlve-fog-question-verdict.md`).** Severity Ax1 **F → N**; Discriminative **U-D → U-C**. The mechanism remains rotor-local mutual-inductance coupling (NOT bulk Earth-frame entrainment), but the *forward kill-switch status* is walked back: the same $\kappa$ applied to Earth-as-rotor (rigid rotor, curl $= 2\Omega \neq 0$) predicts a $+7\times10^{-4}$ rotation-rate bias excluded by existing RLG geodesy by $7\times10^4\times$. The surviving fragment is the paired W-vs-Al $\Psi = 7.15$ self-consistency scaling check (per PONDER PR #1 `eb7a49b`; GR frame-drag scales as $I \propto \rho$ too, so $\Psi$ is not a forward AVE-vs-GR discriminator). Same lifecycle bin as C17/C18; different suppression physics. Note: this is distinct from the EARTH-FLYBY Regime-IV row below (that row is the planetary-mass-saturated stator-boundary observable, unaffected by this walk-back).
...
- **C7-GRB-DISPERSION** ([`divergence-test-substrate-map.md:538`](../../../common/divergence-test-substrate-map.md) — the Matrix-1 `C7-GRB-DISPERSION` row; ★**cite-rot repair 2026-08-02**: this read `:399`, which was **never** that row — it landed in the D5 HTS/Meissner Discriminator when the label was written and lands on the D4 heading today. Re-pointed by content) — ~~surviving preferred-frame test; framing already correct.~~ **🔴 Weak-C 2026-06-15, Rule 12 (line above PRESERVED):** the matrix C7 row's *no-dispersion-NULL* reading ("NULL = no dispersion = AVE survives") IS correct and unchanged; but the "surviving preferred-frame **test** / forward prediction" framing is **retracted** — under weak-C the continuum photon has no zone-edge $(q\ell_{node})^2$ dispersion, so C7 is a **corroborated NULL** (GeV–TeV GRB), NOT a forward GRB-dispersion prediction. The matrix C7 row + cross-refs (`divergence-test-substrate-map.md`) are OUT OF THIS PR's surgical scope and left for the reconciliation lane; they are already no-dispersion-NULL-compatible. See §4.1 for the PARTIAL/KEEP-BOTH split (the surviving *photon* forward prediction is the $(q\ell_{node})^4$ birefringence).  <!-- rule12-freeze: base=080b69cebf03e41d912ed55a9c3504a3b8566721 region=above offset=0 lines=136 bytes=24625 sha256=4522e45a8ecd7a2b6387ad320971f4de5cb812734e3abf4fc30fbee9b68d0fee -->
