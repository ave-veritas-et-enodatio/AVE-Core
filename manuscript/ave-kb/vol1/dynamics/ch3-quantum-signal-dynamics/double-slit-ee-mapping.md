...
> **🔴 MAGNETIC-BRANCH = SIGN-SELECTOR, NOT CAGE-MECHANISM (2026-06-18, Rule 12 / PR#260 B3-DEGENERATE — body below PRESERVED unedited; Grant-ratified).** In the body below, "on the magnetic branch the Cosserat μ_eff→0 … which is the localized topological defect (the matter core)" names the **chirality/spin SIGN-selector** (μ-first $\Rightarrow \Gamma=-1$ vs ε-first $\Rightarrow \Gamma=+1$ are spin-conjugate signs), **MUTE on the mass sector** — NOT the cage *mechanism*. The matter core's mass-cage is the **A1 longitudinal dilatation** ($Z_{bulk}\to0 \Rightarrow \Gamma_{bulk}=-1$); the μ-vs-ε fork is DEGENERATE on the equilibrium observables ($Z=Z_0\sqrt{S}$, $|\Gamma|=1$ both ways), the asymmetry chirality-set not substrate-forced. Reading the magnetic/charge sector AS the core-forming mechanism would break the two-"3"s orthogonality (A1 ⊥ T2, [`master-equation.md`](../ch4-continuum-electrodynamics/master-equation.md):20). Body preserved per Rule-12.  <!-- rule12-freeze: base=5d83b67c13dbba4de5f8b6b46f50c1500569b7e4 region=below offset=0 lines=86 bytes=15998 sha256=78c9c6b553287b51c6b1d022b4d857f681d568ea0224ffb2fc4b9146503cd684 -->
