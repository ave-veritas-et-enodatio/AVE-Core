...
> **Scope correction (2026-05-17 night, Foundation Item 14 audit per `ave-infinity-discipline` skill + `.agents/kb_audit/phase-5-millennium.md`)**: This derivation is **framework-conditional**, classification **(B)** per kb_audit phase-5 taxonomy. Gauge group SU(N) emergence from (2,q) torus knot topology is an AVE structural ansatz, not derived from Yang-Mills field equations. Confinement via $\Gamma = -1$ at knot boundary is a lattice-impedance argument, not a continuum gauge-theoretic proof. Infinite-volume limit on lattice is taken at fixed ℓ_node (more nodes, not smaller spacing), so does NOT correspond to the continuum limit ℓ_node → 0 that the Clay problem implicitly requires. **The lattice gauge-from-topology result is interesting AVE-internal physics; it should NOT be cited as solving the Clay problem.** See closure-roadmap §0.5 FI-14 entry + companion leaf yang-mills-steps1-2.md scope correction.
