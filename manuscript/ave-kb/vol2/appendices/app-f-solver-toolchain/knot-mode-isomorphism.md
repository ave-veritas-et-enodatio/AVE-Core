...
| First excitation | Cinquefoil $c = 5$ (proton) | $\ell = 3$ (angular multipole — **NOT** an overtone; the radial overtone index $n_r$ is a distinct object, Op6) |

🔴 *[**index cross-wire corrected 2026-07-31** (#814 compact audit). The "First excitation" cell previously read "$\ell = 3$ **(1st GW overtone)**" — struck. $\ell$ is the **ANGULAR** index; the **overtone** index is the radial node count $n_r = n - l - 1$, a distinct object owned by Op6 ([`radial-eigenvalue-solver.md:160`](../../quantum-orbitals/ch07-quantum-mechanics/radial-eigenvalue-solver.md); [`common/operators.md:189`](../../../common/operators.md), *"Op6 (eigenvalue target → radial nodes $n_r$), Op10 ($c=3$ invariant → angular nodes $l$)"*). $\ell = 3$ is a **different angular multipole**, not an overtone of $\ell = 2$; the two indices cannot be identified. The corpus already scopes the $Q = \ell$ claim to the **fundamental** on exactly this ground — [`vol3/claim-quality.md:205`](../../../vol3/claim-quality.md): *"for higher modes ($\ell > 2$) this disagrees with GR overtone structure — the claim is the fundamental mode, not the full QNM spectrum."* **The row's eigenvalue content is untouched** — only the parenthetical gloss is corrected.]*
