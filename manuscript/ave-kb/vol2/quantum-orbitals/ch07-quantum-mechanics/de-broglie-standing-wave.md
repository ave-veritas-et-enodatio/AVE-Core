...
> **[2026-06-11 — three-impedance-law channel note]** The value $Z = 377\,\Omega$ is the **transverse-EM** characteristic impedance $Z_0 \equiv Z_{EM}$, exact for **photons**. A **gravity wave** is a transverse **shear** wave with impedance $Z_{shear} = \rho\,c_{shear}$ (a *different* value), likewise free-space-matched in the uniform vacuum and reflecting at the $V_{SNAP}$ wall ($Z_{shear}\to 0 \Rightarrow \Gamma_{shear} = -1$) — exactly as this paragraph already states. Channel-scope per the three-impedance law: photon $\to Z_{EM}$, gravity wave $\to Z_{shear}$ (field-symbol registry §3.11; vocab-operator-unification audit §4(b) #5). The bulk-modulus matter-wave treatment that follows ($\S$ below) is the corpus's best channel-scoped exemplar ($Z_{bulk}$). 🔴 **[DEMOTED 2026-08-11 — R40-B2a: NEEDS RE-DERIVATION, not dead; dated note at the end of this file]**

By contrast, an electron ($0_1$ unknot) is a massive topological defect. It represents a permanent macroscopic **Impedance Mismatch** ($\Gamma = -1$) to the linear vacuum. It does not travel as a shear wave at $c_0$; instead, its motion displaces the lattice, generating longitudinal acoustic pressure waves governed by the vacuum's **Bulk Modulus**. 🔴 **[DEMOTED 2026-08-11 — R40-B2a: NEEDS RE-DERIVATION, not dead; dated note at the end of this file]**
...
- **`:50`** — stamped at `:50`. *(family: Z_bulk/channel-table formula)*  ⚑ **BIAS-DEBT**
  Quoted claim (content verified at HEAD; markup-reduced from the banked audit):
  ```text
  The bulk-modulus matter-wave treatment that follows ($\S$ below) is the corpus's best channel-scoped exemplar ($Z_{bulk}$).
  ```
  Audited rationale, verbatim from the banked worklist:
  ```text
  Three-impedance-law channel note scoping the bulk sector as a free-space wave channel with its own Z_bulk; formula-level Z_bulk=rho*c_bulk class per the prereg's expected classification.
  ```
...
- **`:52`** — stamped at `:52`. *(family: bulk-acoustic matter-wave)*  ⚑ **BIAS-DEBT**
  Quoted claim (content verified at HEAD; markup-reduced from the banked audit):
  ```text
  its motion displaces the lattice, generating longitudinal acoustic pressure waves governed by the vacuum's **Bulk Modulus**.
  ```
  Audited rationale, verbatim from the banked worklist:
  ```text
  The matter-wave carrier is stated as a propagating bulk pressure wave (and :54 the orbital = trapped bulk-modulus acoustic resonance); the surviving content — de Broglie relation, Bohr spectrum, impedance-cavity quantization — is real, and the leaf's own quantitative dispersion (:183) uses c not c_bulk, so only the carrier ontology owes re-derivation.
  ```
