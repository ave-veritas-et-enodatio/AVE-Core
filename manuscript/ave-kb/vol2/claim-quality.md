...
- strengthen-by:
  - Derive $Q=\ell$ as the unique consequence of the perfect-reflector saturation boundary rather than a consistent identification — and note (Ruling B1, 2026-07-21) that the target is the **cold $a_* = 0$ anchor**: the $1/\ell$-per-cycle leak is currently a scaling assertion with a convention-set constant, never a computed radiated power (#808 scoping F7), and the spin dependence belongs to the m$\Omega$ law, not to flat $Q = \ell$.
