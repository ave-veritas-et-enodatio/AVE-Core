...
- _Specific Non-Claims and Caveats_
  ...
  - **Carve on the three-way Δc_crit=3 identity (2026-08-24, Grant-ruled; leaf carries the dated note):** Δc_crit = connectivity is DERIVED-modulo-premise (the ≤1-unit-per-bond transfer rule is asserted); connectivity = trefoil-crossing is ASSERTED and in tension with the canonical imported-(2,3)-selection disclosure; connectivity = Cosserat-sector-count FAILS the z=6 counterfactual (sector count is owned by D=3/SO(3), and "Cosserat sectors" is itself a "3"-homonym across rotation-axes vs the mode hierarchy). The Specific-Claims bullet's "structurally identical, not independent coincidences" is scoped accordingly: it holds for pair 1 only.
...
- _Specific Non-Claims and Caveats_
  ...
  - **Spin-½ SELECTION is a disclosed import, not derived (2026-07-08, [SPIN-HALF-POSITED], #584/#585).** The Finkelstein–Misner / double-cover argument derives the double-cover **STRUCTURE** (native SO(3) / $2T \subset SU(2)$); the **SELECTION** of the fermionic (spin-½) branch over the bosonic one is **IMPORTED**. $\pi_1$ of the SO(3)-Cosserat configuration space is $\mathbb{Z}_2$, and $\mathrm{Hom}(\mathbb{Z}_2, U(1))$ has two elements — it ADMITS both statistics and FORCES neither; forcing the fermion needs an action-level $\mathbb{Z}_2$/Wess–Zumino term the corpus supplies by hand (half-angle lift $U = \exp(i\,\boldsymbol{\sigma}\cdot\boldsymbol{\omega}/2)$). Confirmed FOUR ways: $\pi_1$-admits-not-forces (#584); the soliton literature imports the sign (Finkelstein–Rubinstein / Witten's Wess–Zumino coefficient matched to $N_c$ / Wilczek–Zee's $\theta$ free); the no-derived-weld half-flux run [HALF-FLUX-ECHO] (#585); the pre-sector homotopy argument (no LOCAL term in ANY sector welds a winding-blind homotopy invariant to a $q$-keyed winding class). **PEER-WITH-SM** (SM also posits spin-½). Structure/selection split canonical at clm-rkisb8 ([`../vol1/claim-quality.md`](../vol1/claim-quality.md)).
...
- _Specific Claims_
  ...
  - **Micropolar stability.** Standard Cauchy elasticity ($K = (5/3)G$) applied to the Delaunay-triangulated AVE substrate causes unbounded contraction within the first calculation frame; the engine must instantiate a Chiral LC micropolar continuum with explicit rotational kinematics $\kappa_{rot}\,\epsilon_{ijk}(\theta_k - \phi_k)$ to enforce $K = 2G$ trace-reversed identity and prevent array implosion.
    > **🔴 NOT-CONFIRMED-BY-SRS-GEOMETRY (2026-07-04, PR #508 — the chiral micropolar Bloch arc; claim PRESERVED, status honestly tagged).** The specific hypothesis that the micropolar rotational sector $\kappa_{rot}\,\epsilon_{ijk}(\theta_k-\phi_k)$ **enforces** $K=2G$ is **NOT confirmed** by the ratified chiral srs-z3 geometry. The genuine 48×48 micropolar Bloch eigensolve (`src/ave/core/micropolar_bloch.py`, driver `src/scripts/vol_1_foundations/srs_chiral_micropolar.py`) sweeps the independent $\kappa_{rot}\in[0,50]$ and finds $\nu_{\text{eff}}$ **completely FLAT at $k\to0$** ($-0.31434$ for all $\kappa_{rot}$ at $\rho=3$) — $\kappa_{rot}$ is a $k\to0$ Cauchy-grade **spectator** (it opens the rotational optic gap only at finite $k$; moduli-hierarchy orthogonality $\ell_c^2=\gamma/G\perp\rho$), and it sources **no Cauchy-grade chiral coupling $B$** ($\sim$1e-32). The geometry-fixed chiral coupling $B$ that IS real on srs rides the $\sigma^A$ lever-arm channel (§`def-ch1crc` row below), not $\kappa_{rot}$, and it moves $\nu_{\text{eff}}$ AWAY from $2/7$ ($+0.389$ at the Stage-1 $K=2G$ point). So the rotational sector is the wrong knob: the ½/¼-tell found **no tunable knob** onto $2/7$. **$K=2G$ stays GR-imported** (co-confirmed with the Cauchy-grade Stage-1 result, PR #506, and the K=2G-provenance closure, PR #261). This claim stays **asserted-not-derived at solidity 0.50** — the micropolar-enforces-$K=2G$ leg is not delivered by the substrate. Provenance: `research/2026-07-04_srs-chiral-micropolar_result.md` §2b, §5.
...
- Five-step universal procedure: (1) identify $\varepsilon_{11}(r)$, (2) locate $r_{sat}$ where $S = \sqrt{1-\varepsilon_{11}^2} = 0$, (3) Poisson correction $r_{eff} = r_{sat}/(1+\nu_{vac})$, (4) eigenfrequency $\omega_0 = \ell c/r_{eff}$, (5) quality factor $Q = \ell$ from lattice phase transition **[spin-scope, Ruling B1 2026-07-21: for the BH-ringdown fire, flat $Q = \ell$ is the cold $a_* = 0$ anchor only — see the scope note under _Specific Claims_ below]**. Universal closed form: $\omega \cdot r_{char} = \ell(1+\nu_{vac})/x_{sat}$.
- _Specific Claims_
  ...
  - The lattice phase transition at $\varepsilon_{11} = 1$ converts the elastic solid ($G > 0$) to a ruptured topology-melted fluid ($G = 0$); transverse shear waves cannot propagate in the ruptured interior, so the saturation boundary acts as a perfect reflector — the QNM is a Stoneley-like surface wave at the elastic/ruptured phase boundary, and $Q = \ell$ falls out of the curvature-radiation-loss-per-cycle scaling. **🟩 SPIN-SCOPE (Grant Ruling B1, 2026-07-21; propagated 2026-07-31 — nothing struck, scope added).** For the BH-ringdown fire this $Q = \ell$ is the **cold $a_* = 0$ anchor**: $\ell$ is an integer **mode count** — cavity-deformable (`common/vocabulary-register.md:261` (def-quant3), sense (1) at `:271`, *"Failure mode: ionization destroys them"*), **not** a deformation-invariant winding. **The discriminator is its INTEGRALITY, not its sense class:** an integer can only jump discretely, while the physical $Q = \omega_R/2\omega_I$ is an impedance ratio that moves continuously with the strain profile; an integer that can only jump cannot track a ratio that moves smoothly, so the equality is a **zero-spin coincidence, not a law** — at catalog spins corrected-Kerr $Q$ rises $3.07 \to 3.49$ and the flat reading fails at $\bar D_Q = -38\%$; the spin dependence is carried by the m$\Omega$ law $\omega_I = (\omega_R - m\Omega)/(2\ell)$ (banked $-5.44\%$/$-4.57\%$, OPEN near-miss). *(Physical reading = orchestrator-walk provenance 2026-07-31, not canon.)* Model banner: [`vol3/claim-quality.md`](../vol3/claim-quality.md) `clm-395gps` at `:204`; scoping doc `research/2026-07-30_qlaw-derivation_scoping.md` §1.5. **[#808 scoping F7/F8 — OPEN FLAG, not a ruling; B1 did not adjudicate this:** the "loss-per-cycle **scaling**" wording is exact — the $1/\ell$ leak fraction is a scaling assertion whose proportionality constant is set to 1 by convention, never a computed radiated power; deriving the cold $Q$ is the named next work. Source `research/2026-07-30_qlaw-derivation_scoping.md` §1.5 — surfaced, unadjudicated.**]**
  ...
  - The Kerr extension applies a co-rotating-frame Park-transform decomposition $\omega_I = (\omega_R - m\Omega)/(2\ell)$ at the Poisson-augmented photon sphere $r_\Omega = r_{ph}\sqrt{1+\nu_{vac}}$; ~~sub-2% accuracy across $a_* = 0.3$–$0.8$~~, with superradiance ~~recovered from first principles~~ **following as a structural consequence of the adopted m$\Omega$ law** at $\omega_R = m\Omega$. **🔴 The "sub-2% accuracy across $a_* = 0.3$–$0.8$" is RETRACTED 2026-07-21 (Grant Ruling B1; original struck, preserved per Rule 12).** That figure was computed against the **corrupt $\omega_R$/$\omega_I$ Kerr reference tables + source-frame masses** (#774). Corrected picture: the topological **flat $Q = \ell = 2$ is scoped to the $a_* = 0$ cold anchor** (the $\Omega \to 0$ limit of the m$\Omega$ law) and would **fail at $\bar D_Q = -38\%$** at catalog spins (corrected-Kerr $Q$ rises $3.07 \to 3.49$); the **spin-refined m$\Omega$ law** under the standing v1 mapping lands at **$-5.44\%$** (Resultbox) / $-4.57\%$ (ZAMO) — an **OPEN near-miss tension**, the named next ringdown work, NOT a sub-2% match and NOT a zero-free-parameter benchmark (only the cold $18/49$ eigenvalue is). **Scope of the strike:** only the accuracy figure is struck. The superradiance clause is deliberately ~~left *unstruck* above~~ **retained above — only its provenance phrase was softened 2026-07-30 (see the 🟡 note below); the clause and the limit themselves stand** — because the m$\Omega$ law and its $\omega_R = m\Omega$ superradiance limit ($\omega_I \to 0$) are the standing Q-law commitment under B1 — the *structure* is retained, the *accuracy number* is not. Honest grade: [`vol3/claim-quality.md`](../vol3/claim-quality.md) `clm-395gps` (model banner at `:204`) and [`ave-merger-ringdown-eigenvalue.md` § GRANT RULING B1](../vol3/cosmology/ch15-black-hole-orbitals/ave-merger-ringdown-eigenvalue.md) (solidity 0.55, `use as input only`, disclosed-phenomenological, *"NOT a zero-free-parameter benchmark"*). **🟡 PROVENANCE SOFTENED 2026-07-30 (Grant ruling [sic]: "soften"; original struck, preserved per Rule 12).** "Recovered from first principles" over-stated **provenance**: the $\omega_R = m\Omega$ superradiance threshold is a *structural consequence* of the **adopted** m$\Omega$ law $\omega_I = (\omega_R - m\Omega)/(2\ell)$, and that law is graded **disclosed-phenomenological, solidity 0.55, `use as input only`** ([`ave-merger-ringdown-eigenvalue.md`](../vol3/cosmology/ch15-black-hole-orbitals/ave-merger-ringdown-eigenvalue.md) `:72-76`: *"it is NOT more first-principles-derived than v2 — both are disclosed phenomenological per `clm-395gps`"*) — it is **grafted on** as a Park-transform decomposition rather than obtained from the Op21 Nyquist-cell count, recorded as *analogised, not derived* by the #808 scoping lane (`research/2026-07-30_qlaw-derivation_scoping.md` §2.5 / Route R4; provenance flag at §1.7). **The threshold and the m$\Omega$ law are RETAINED** as Ruling B1's standing Q-law commitment — provenance wording only, no weakening of the threshold or the limit. Mirrors: [`kerr-q-correction.md`](./appendices/app-f-solver-toolchain/kerr-q-correction.md), [`common/solver-toolchain.md`](../common/solver-toolchain.md), `manuscript/backmatter/05_universal_solver_toolchain.tex`.
...
- _Specific Non-Claims and Caveats_
  - Per-domain percent errors range from $0.1\%$ (protein backbone) to $\sim 5.7\%$ ($a_* = 0.9$ Kerr), with extremal-Kerr $\sim 40\%$ at $a_* = 0.99$ where higher-order coupling is required. Treat the headline as "sub-few-percent agreement across the validated range", not as uniform ppm-level accuracy. **[Corrupt-reference inheritance, 2026-07-21: the two Kerr-$Q$ figures in this sentence ($5.7\%$ at $a_* = 0.9$, $\sim\!40\%$ at $a_* = 0.99$) are read off the same corrupt Kerr table retracted above (#774) — see the banner on the `kerr-q-correction.md` table. They are not honest corrected-Kerr deviations.]**
  - The $Q = \ell$ identity is asserted as an emergent property of the lattice phase transition's perfect-reflector boundary; the leaf does not formally derive that "perfect reflector at the saturation boundary" is the unique resolution mechanism~~, only that it is consistent with the observed quality factors~~ — and the consistency-with-observed-$Q$ reading holds **only at the $a_* = 0$ cold anchor**. **🔴 The "only that it is consistent with the observed quality factors" clause is STRUCK 2026-07-30 (Grant ruling [sic]: "follow rec"; original struck, preserved per Rule 12).** It is an **accuracy assertion**, and Grant Ruling B1 (2026-07-21) contradicts it as written: against the corrected Kerr reference the flat-$Q = \ell = 2$ reading would **fail at $\bar D_Q = -38\%$** at catalog spins (corrected-Kerr $Q$ rises $3.07 \to 3.49$). The clause survives **only** at the $a_* = 0$ cold anchor — the $\Omega \to 0$ limit of the m$\Omega$ law, which is where the topological flat $Q = \ell$ is scoped. **Bucket (the ruled question):** this strike belongs to **this retraction** — the accuracy-class sub-2% retraction — **not** to the #808 $Q = \ell$ **scoping** class. It was PR #809's flag-don't-fix item *"⚠ Needs Grant's bucket ruling — `vol2/claim-quality.md:1101`"*, left unedited there because the bucket was not that lane's call; Grant ruled it into the accuracy bucket 2026-07-30. Honest grade: [`vol3/claim-quality.md`](../vol3/claim-quality.md) `clm-395gps` (model banner at `:204`) and [`ave-merger-ringdown-eigenvalue.md` § GRANT RULING B1](../vol3/cosmology/ch15-black-hole-orbitals/ave-merger-ringdown-eigenvalue.md) (solidity 0.55, `use as input only`, disclosed-phenomenological).
...
- strengthen-by:
  - Derive $Q=\ell$ as the unique consequence of the perfect-reflector saturation boundary rather than a consistent identification — and note (Ruling B1, 2026-07-21) that the target is the **cold $a_* = 0$ anchor**: the spin dependence belongs to the m$\Omega$ law, not to flat $Q = \ell$. Separately (#808 scoping F7 — OPEN FLAG, not a ruling; B1 did not adjudicate it): the $1/\ell$-per-cycle leak is currently a scaling assertion with a convention-set constant, never a computed radiated power.
...
- _Specific Non-Claims and Caveats_
  ...
  - **SCOPED IMPORT, not a computed match (2026-07-14, #685).** The RT-equivalence "match to QED" is re-tagged from argued-match to scoped consistency-import. The QED-TRACE beta-gate ([`research/2026-07-14_qed-trace-beta-gate_RESULT.md`](../../../research/2026-07-14_qed-trace-beta-gate_RESULT.md) §7) proved the two-body pointwise route **WRONG-FORM** (power law, not `ln(q)`; wrong sign on the transfer register). **Boundary (per §7):** the beta gate proved WRONG-FORM for the two-body pointwise route; **the many-body scale-integrated medium-response route is UNPROBED, NOT CLOSED** (not pre-judged). The re-tag does **not** close the log route in general.
    - **UPDATE — screening-sum route CLOSED (2026-07-14, #693, Grant Ruling 5).** The "UNPROBED, NOT CLOSED" boundary above is now **CLOSED for the specific route it named**: the many-body static screening-sum gate ([`research/2026-07-14_screening-sum-gate_RESULT.md`](../../../research/2026-07-14_screening-sum-gate_RESULT.md)) returned **WRONG-FORM**, and Ruling 5 ratified the RELABELED-PAIRWISE reading — the intervening medium is a **relative spectator (~0.02%)**, so nothing accumulates per decade and **no log can form** (the per-decade log-slope collapses ~33×, decisively not-a-log; the naked screening-sum exponent is deliberately not quoted, #693 §2). Scoped to **the STATIC, self-consistent, local-constitutive electric-dipole screening-sum route**; **NOT probed (open):** dynamical/retarded response, and the circulation-keyed inductive (μ-grade) channel with circulating probes. So **BOTH classical routes (pairwise #685, static screening-sum #693) are WRONG-FORM**; the re-tag drops the beta gate's named caveat but reads as a **SCOPED import over the static electric-dipole route, NOT a full-category import**.
...
- _Specific Non-Claims and Caveats_
  ...
  - Does NOT derive $\alpha$ — the loop-regulator FORM is α-CLEAN, but the Stage-2 birefringence MAGNITUDE (v3 headline, single instantaneous footing, OPTION-B re-freeze 2026-07-07: $3.75\pi/\alpha^2\approx2.2\times10^5$; the 2026-07-03 QED-normalization step gave the propagating/mixed-footing $7.5\pi/\alpha^2\approx4.42\times10^5$, exactly double via the $\langle\cos^2\rangle=\tfrac12$ carrier average — convention history; the v1 $7.5/\alpha^3\approx1.93\times10^7$ predated the QED-normalization fix; `research/2026-07-03_birefringence-qed-normalization-correction.md`) is an α-echo (canonical at clm-pp3qwf, a separate leaf; NOT re-minted here).
...
## Common-Mode Twist-Ledger Theorem — the ground-$(2,3)$ twist term cancels from every lepton mass DIFFERENCE **iff** it is generation-independent (derived algebraic identity, CONDITIONAL)
<!-- id: clm-cmtwst -->

> **★★ THE LEDGER THIS CLAIM ASSUMES IS SUPERSEDED — CONDITIONAL (2026-08-03 repair; claim body below PRESERVED per Rule 12).** This claim's budget is **ADDITIVE**, and the additive reading is **not canon's**. Canon's Grant-ratified 2026-06-20 mass-sector ruling makes the lepton tower a **TRANSDUCTION** — *"The flywheel regulates the frequency that *sets* the mass; the mass stays A1"* ([`master-equation.md`](../vol1/dynamics/ch4-continuum-electrodynamics/master-equation.md):34, inside the `:30` ratification header; propagated to five further sites) — and **Grant's 2026-08-03 walk ruled the composition law is a BIAS** (multiplicative; verbatim `[sic]`: *"agree, bias"*). Consequences, **recorded not resolved**:  <!-- rule12-freeze: base=be12518b9c9526869e78502892c38ecad318cd42 region=below offset=13 lines=20 bytes=5338 sha256=cd6537c7fdec6957c8f6c62dfb1c4c1574604fa3203945f964718899caab0276 -->
> - **Condition (C) SURVIVES.** Under transduction/bias, $E_{\text{twist}}\equiv0$ **as an additive slot** ⇒ (C) holds **trivially**, and the identity below is **true but degenerate** ($\Delta mc^2 = \Delta E_{\text{slosh}}$). The claim is not falsified; its **content** is reduced under the ratified reading.
> - **The difference/ratio carve (third Specific Claim) is COMPOSITION-LAW-DEPENDENT, and the ratified law INVERTS it.** Under **bias**, mass **RATIOS** are **blind** (the common factor cancels from $m_j/m_i$) and mass **DIFFERENCES** are **sighted** (every spacing scales with the bias) — the exact inverse of that bullet, which is preserved as the **additive-law** result.
> - **What is NOT closed.** Per Grant 2026-08-03 (*"agree"*), **metering ≠ storage**: the 2026-06-20 ruling closed **metering** only. **Item 13's sector-of-storage stays OPEN**, re-shaped as a **T2-internal DC-inventory** question, with the **"what pumps A1 — AC slosh or total energy density?"** residual open beside it. The **tube-geometry fork** ([`electron-unknot.md`](./particle-physics/ch01-topological-matter/electron-unknot.md):13 vs `:59`) is **OPEN** and moves every $E_{\text{twist}}$ number by $\approx10\times$.
> - **Numbers deliberately NOT changed.** confidence/solidity stay **0.50/0.50**. Whether confidence should drop — given that one of three Specific Claims is now disclosed as composition-law-dependent and the identity is degenerate under the ratified reading — is **surfaced for auditor decision** in `_orchestration/docket-entries/2026-08-02-common-mode-canonization.md`, **not decided by this repair lane**.
>
> Canonical home: [`common-mode-twist-ledger.md`](./particle-physics/ch01-topological-matter/common-mode-twist-ledger.md) §1 **SCOPE BLOCK**.

**The claim, with its condition carried inside it.** *(★ Read the SUPERSEDED-reading header above first — the budget below is the **additive** reading, not canon's ratified one.)* Budget each charged lepton of the shared-ground-$(2,3)$ family as $m_ic^2 = E_{\text{slosh},i} + E_{\text{twist},i} + n_i\varepsilon_{\text{torsion}}$ — canon's reactive LC store ([`mass-closure-theorem.md`](./particle-physics/ch01-topological-matter/mass-closure-theorem.md):52,:54), the couple-stress energy of the **ground** $(2,3)$ winding, and canon's Cosserat torsional **excitation** ladder ($n_e{=}0$, $n_\mu{=}1$; [`torus-knot-uniqueness.md`](./particle-physics/ch01-topological-matter/torus-knot-uniqueness.md):106,:107). **THEN**, for any pair $i,j$ in the family, $(m_j-m_i)c^2 = \Delta E_{\text{slosh}} + \Delta n\,\varepsilon_{\text{torsion}} + (E_{\text{twist},j}-E_{\text{twist},i})$, and the ground-twist bracket vanishes for **every** pair **if and only if** $E_{\text{twist}}$ is **generation-independent** — **condition (C)**. **Under (C) and only under (C)**, every mass-**difference** observable inside the family (rung spacings; in-family decay $Q$-values) is **structurally blind** to the ground-twist term, at any magnitude. **(C) is OPEN**: canon is silent on the muon's real-space loop scale and its two relevant general statements pull opposite ways (canonical home [`common-mode-twist-ledger.md`](./particle-physics/ch01-topological-matter/common-mode-twist-ledger.md) §4). Ordering provenance: Grant 2026-08-02, verbatim `[sic]` — *"this sounds incredibly important to document and derive/cannonize for the paperclip/common mode"*; the derivation is the implementer lane's.

- _Specific Claims_
  - **The conditional identity (the whole claim).** Given the stated ledger, the ground-twist term is absent from every pairwise mass difference **iff** it is generation-independent. Necessity is as load-bearing as sufficiency: a function on a finite set has all pairwise differences zero iff it is constant.
  - **The blind-spot corollary, covering BOTH innocuous readings.** Differences are blind when the term is *outside* the invariant (canon's grade-orthogonality exclusion, [`master-equation.md`](../vol1/dynamics/ch4-continuum-electrodynamics/master-equation.md):41 — i.e. $E_{\text{twist},i}\equiv0$) **and** when it is *inside* but generation-independent. Differences are a probe **only** in the inside-and-generation-dependent case.
  - **The difference/ratio carve.** The blindness is to **differences**. Canon books the ladder **multiplicatively** ($m_\mu/m_e = 1/(\alpha\sqrt{3/7})$, [`lepton-spectrum.md`](./particle-physics/ch06-electroweak-higgs/lepton-spectrum.md):39), and a common additive term does **not** cancel from a ratio; what the ratio face is sensitive to is canon's **mechanism attribution** — *"No torsional excitation is present"* (`:21`) is exact as a **quantum-count** statement and is an **energy** statement only if $E_{\text{twist}}\ll m_ec^2$. **★★ THIS BULLET IS COMPOSITION-LAW-DEPENDENT AND THE RATIFIED LAW INVERTS IT (2026-08-03) — bullet body above PRESERVED as the ADDITIVE-law result.** Under Grant's ratified **BIAS** (multiplicative) law, mass **RATIOS** are the **blind** face (the common factor cancels) and mass **DIFFERENCES** are **sighted** (every spacing scales with the bias) — the exact inverse of what this bullet states. See the **★★ SUPERSEDED-reading header** at the top of this entry.
- _Specific Non-Claims and Caveats_
  - **NOT a claim that $E_{\text{twist}}\neq0$, that it sits inside $m_ec^2$, or that it has any magnitude.** The term's existence-in-the-budget is the **UNACCOUNTED** verdict of `research/2026-08-02_twist-ledger-audit.md` **`[branch:#833]` — implementer-lane, NOT adversarially reviewed, pending its own audit**; the magnitude is gated by the **already-flagged two-$\ell_c$ collision** (`src/ave/core/constants.py`:331-337, *"Surfaced for auditor adjudication … not silently merged"*), a **12-order** bracket. If that audit is overturned, the theorem is unaffected — it is conditional on (C), not on the audit.
  - **NEITHER BRANCH OF (C) IS ADOPTED.** Branch A (scale-common) ⇒ (C) exact, ladder rung-spacings undisturbed, item 13 reduces to a ground-ledger question. Branch B (scale-riding; the leaf derives $E_{\text{twist}} = \tfrac{9\pi}{4}\gamma_c C_{\text{loop}}$, linear in loop circumference for canon's isotropic tube, $\propto1/C_{\text{loop}}$ at fixed cross-section — **both exponents live, and NOT symmetric in consequence**: the isotropic one shrinks the heavier lepton's store, the fixed-cross-section one multiplies it by $\approx207$ and flips the sign) ⇒ the term participates in differences. **The fork and the magnitude gate MULTIPLY**: the ladder is disturbed only if Branch B holds **AND** the lattice-scale $\ell_c$ is the electron's. Routed to Grant.
  - **Resolves NOTHING about open item 13** (sector-of-storage, `⚑ OPEN-IN-WALK`). It supplies item 13's **bookkeeping instrument** — which observables can bear on it — not its answer.
  - **No new axiom (A44); no slot refilled (Rule 12).** The diagnosis is bookkeeping-class, not axiom-class; `clm-ka5zdx`'s open strengthen-by (`:1258`) is untouched, and no existing claim's solidity is flipped.
  - **NOT an emergence result, NOT a chord, NOT a discriminating connection** (`ave-discrimination-check`): a conditional algebraic identity over in-corpus structure, with **no solver**, **no CODATA input**, and **no numerical commitment**. The §4.1 closed form reproducing the `[branch:#833]` R1a row analytically is a **consistency receipt between two evaluations of one integrand**, carrying that doc's pending status — not a result.
  - **Coordinate scope (A46).** The theorem is **coordinate-agnostic** (algebra on a term list). Only the §4 scaling analysis is a real-space couple-stress integral, and it rests on canon's own real-space mechanical-$\omega$ image of the $(2,3)$ ([`master-equation.md`](../vol1/dynamics/ch4-continuum-electrodynamics/master-equation.md):24), with `[branch:#833]` §3.4's `phase-space-coordinate-check` PASS cited as **pending, not adopted**.
  - **PROPOSED / pre-adversarial-review.** Grant ratified the **ordering**, not the derivation.
  - **(2026-08-03) The two OBSERVATIONAL FENCES in the leaf's §4.3 are NOT part of this claim and mint nothing.** The **annihilation ledger** ($e^+e^-$ photons at $2\times511$ keV exactly ⇒ no room for a $20$–$200\ m_ec^2$ additive store to release) and **sign-evenness** (antihydrogen mass-split null at $\sim10^{-12}$ ⇒ twist energetics even in $\mathrm{Tw}$, $\tfrac12k\theta^2$ class) are **fences on the additive reading's magnitude**, folded in at Grant's 2026-08-03 walk (verbatim `[sic]`: *"yes fold in"*). Sign-evenness is a **PASSED NULL** — a constraint met, **not** evidence for the framework — and the annihilation fence's compatibility with the bias reading is a **consistency** observation, **not** an emergence result and **not** a prediction. The *"does the DC pre-load gravitate?"* Eötvös-class channel is **identified and OPEN**; **no bound is asserted and no number is run**.
  - **(2026-08-03) The §4.1 magnitude cross-check is weaker than the shipped leaf said.** It is *"two evaluations of one integrand"* — this leaf's closed form is the symbolic simplification of `[branch:#833]`'s driver product, not an independent route. R1a is additionally a **cross-modulus hybrid** whose own defining relation ($\ell_c^2 = (\beta+\gamma)/[2(\mu+\kappa)]$) yields **R1b** instead ($64/6 = 10.667\times$ larger), and the $T_{EM}\ell_{node}\equiv m_ec^2$ identity **band-forces** any energy of that shape into $10^1$–$10^3\,m_ec^2$ before physics enters. **The honest lattice-scale statement is the bracket $2\times10^1$–$2\times10^2\,m_ec^2$, further uncertain by $\approx\pi^2$ from the unresolved tube geometry** — not the 5-significant-figure number.

> **Leaf references:** [common-mode-twist-ledger](./particle-physics/ch01-topological-matter/common-mode-twist-ledger.md).

### Quality
- confidence: 0.50
- depends-on:
  - clm-ka5zdx — Mass-Closure Theorem: supplies the $E_{\text{slosh}}$ term and the closure the ledger extends; its own rationale (*"no quantity computed"*) is why there is no pre-existing sum for the twist term to sit in (solidity 0.50)
  - clm-zw6mut — Scale Invariance / the three-Cosserat-sector-to-three-lepton-generation identification that carries the excitation ladder; canon's own grade records it as *"asserted, not derived from the four axioms"* (solidity 0.60)
  - clm-8c3yhs — $(2,3)$ torus-knot uniqueness: the shared-ground premise the whole theorem turns on (solidity 0.70)
- solidity: 0.50 (use as input only, don't build deeper) [= min(0.50, 0.50)]
- rationale: **★ SUPERSEDED PREMISE (2026-08-03) — the ADDITIVE ledger this claim budgets is NOT canon's, and under the ratified reading condition (C) holds VACUOUSLY.** Canon's Grant-ratified 2026-06-20 mass-sector ruling makes the lepton tower a **TRANSDUCTION** (*"The flywheel regulates the frequency that *sets* the mass; the mass stays A1"*, [`master-equation.md`](../vol1/dynamics/ch4-continuum-electrodynamics/master-equation.md):34), and Grant's 2026-08-03 walk ruled the composition law is a **BIAS** — multiplicative, not additive (verbatim `[sic]`: *"agree, bias"*). So $E_{\text{twist}}$ dies **as an additive slot**, (C) is satisfied **trivially/vacuously**, the identity is **true but degenerate** ($\Delta mc^2 = \Delta E_{\text{slosh}}$), and the third Specific Claim — the difference/ratio carve — **INVERTS** (under bias, mass RATIOS are the blind face and mass DIFFERENCES are sighted). **A superseded premise is not a retraction:** the algebra below is untouched, and confidence is held at **0.50** per the 2026-08-03 auditor ruling — a thin-but-true conditional identity whose premise canon does not hold; what is reduced is the claim's CONTENT, not its truth. The algebra is exact and trivial — a term-wise subtraction plus the finite-set constancy lemma — and it is stated **with its condition inside the claim**, which is what keeps it honest rather than vacuous. Confidence 0.50 (not higher) because **every input is soft**: the middle term's existence-in-the-budget is a **pending, un-reviewed** implementer-lane verdict `[branch:#833]`; its magnitude is gated by an **unadjudicated 12-order symbol collision** (`constants.py`:331-337); the condition (C) is an **open fork** on which canon is silent at the load-bearing input (the muon's real-space loop scale — two-method sweep, 0 hits) while two of canon's *general* statements pull opposite ways ($1/r$ universality, [`electron-unknot.md`](./particle-physics/ch01-topological-matter/electron-unknot.md):19 + [`scale-invariance.md`](../vol1/operators-and-regimes/ch5-universal-spatial-tension/scale-invariance.md):11, vs. *"nodes … remain fixed at $l_{node}$ spacing"*, [`lepton-spectrum.md`](./particle-physics/ch06-electroweak-higgs/lepton-spectrum.md):17); and the excitation-ladder leg it books is Class C by canon's own tag (`:84`). Confidence 0.50 (not lower) because the identity itself cannot fail, the ledger's terms are canon's own rather than imported, the necessity half is proved (not assumed), and the difference/ratio carve corrects rather than inherits the dispatch's framing. Dep-capped at 0.50 by clm-ka5zdx, which is also where the honest bottleneck is: a ledger built on a closure that was *"reached by identifying … (no quantity computed)"* cannot be more solid than that closure. **Band is correct for its use:** an instrument for routing item 13, not a foundation to build deeper on.
- strengthen-by:
  - **Adjudicate (C)** — Grant's fork call on whether the heavy leptons' $(2,3)$ configuration is real-space scale-common (Branch A) or Compton-scaled (Branch B). This is the single input that converts the conditional identity into an unconditional one (either "the term is invisible to the ladder, full stop" or "the ladder's booking must carry it").
  - **Land the `[branch:#833]` audit through review**, so the middle term's *existence-in-the-budget* status stops being pending; and **adjudicate the two-$\ell_c$ referent** (`constants.py`:331-337), which alone collapses 12 of the fork's orders.
