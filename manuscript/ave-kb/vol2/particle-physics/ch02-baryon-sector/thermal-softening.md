...
The cold ($T=0$) Faddeev-Skyrme solver with $\kappa_{FS} = 8\pi$ evaluates the 1D scalar trace to $\mathcal{I}_{scalar}^{(cold)} \approx 1170.6\,m_e$, yielding a proton ratio of $\approx 1849.70$ ($+0.7377\%$ above the empirical value). This systematic overestimate arises because the solver computes the ideal zero-temperature ground state, whereas the physical proton exists as a localized thermal hotspot within the LC network at an effective core temperature of $T_{core} \sim m_p c^2/k_B \approx 10^{13}\text{ K}$.

🔴 *[value-refresh 2026-07-13 — cold triplet corrected to the live solver. Superseded byte-verbatim:* ~~"evaluates the 1D scalar trace to $\mathcal{I}_{scalar}^{(cold)} \approx 1185\,m_e$, yielding a proton ratio of $\approx 1872$ (approximately $2\%$ above the empirical value)"~~ *— the pre-Ax4-saturation-refactor triplet. Live solver (`src/ave/core/constants.py`; `TopologicalHamiltonian1D` at $\kappa_{FS}^{(cold)}=8\pi$, crossing number 5) gives $\mathcal{I}_{scalar}^{(cold)} = 1170.59\,m_e \to$ ratio $1849.70$ ($+0.7377\%$ vs CODATA $1836.153$); matches [`proton-identification.md`](proton-identification.md) line 13. **Cold-vs-softened caution:** the $\mathcal{I}_{scalar} \approx 1162\,m_e$ trace quoted at `proton-identification.md:68` and `constants.py:910` (`I_SCALAR_1D = 1161.987`) is the thermally-**corrected** trace that feeds the final eigenvalue ($\to 1836.117$), NOT this $\kappa_{FS}=8\pi$ cold trace.]*
...
1. ~~$\nu_{vac} = 2/7$ --- the Poisson ratio of the chiral LC lattice, which sets the anharmonic Gruneisen parameter governing the coupling between thermal fluctuations and elastic stiffness.~~ $\nu_{\text{Hill}} = 2/7$ --- the vacuum Poisson ratio **at the $K = 2G$ operating point** (form-derived; *value* GR-imported, see [`common/form-deriving-value-importing.md`](../../../common/form-deriving-value-importing.md):87), which sets the anharmonic Gruneisen parameter governing the coupling between thermal fluctuations and elastic stiffness. *Naming:* the subscript is "Hill" because $2/7$ is an **isotropic Voigt–Reuss–Hill average** --- an averaging **choice**, not a lattice-emergent bound and not a theorem. Do not read it as the Hill (1963) apparent-modulus *ordering*, which is a different object sharing the name ([`common/theorem-thesaurus.md`](../../../common/theorem-thesaurus.md):227, §"Hill — three distinct objects" at :223).
...
> **[CRIB-1 relabel + provenance — 2026-08-02; KB-lockstep with the merged print correction. VALUE UNCHANGED at $2/7$: this is a naming and attribution correction with zero numerical consequence anywhere in the leaf.]**
>
> **Status of this note.** Discharged-decision propagation of an already-ruled state — it adjudicates **nothing new**. The printed twin of the struck row (`manuscript/vol_2_subatomic/chapters/02_baryon_sector.tex`, the `[RELABEL + PROVENANCE 2026-08-02 (CRIB-1)]` block) was corrected and **merged in PR #840**; this leaf still carried the old symbol and the old attribution. Rule 12: the prior wording is struck in place and preserved verbatim, not deleted.
>
> **Two things were wrong with the struck row, and neither is the number.** **(1) The SYMBOL** — the corpus relabelled $\nu_{vac} \to \nu_{\text{Hill}}$ in #589; the engine carries the relabel in `src/ave/core/constants.py` (`N_NU` / `NU_VAC`, both documented as $\nu_{\text{Hill}}$, *"isotropic Voigt-Reuss-Hill average at K=2G"*). **(2) The ATTRIBUTION** — *"the Poisson ratio of the chiral LC lattice"* reads lattice-emergent, which [`common/theorem-thesaurus.md`](../../../common/theorem-thesaurus.md):227 explicitly denies: the Voigt–Reuss–Hill average is ★*"Not a theorem and not a bound"*, and :231 states the live hazard plainly — $\nu_{\text{Hill}} = 2/7$ is **GR-imported** (PR #261), *"a number fed into the Born-Huang extraction"*, so letting "Hill" drift from the **average** to the **bound** would give an imported calibration input the standing of a variational theorem.
>
> The $\nu_{\text{Hill}}$ symbol is carried through to the product expression below for internal consistency; $\delta_{th} = 1/(14\pi^2) \approx 0.00721$ is unchanged.
