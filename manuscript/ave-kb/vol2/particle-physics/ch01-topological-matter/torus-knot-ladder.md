...
> **Imported-assignment note (winding → particle column).** The integer-$q$ / odd-$q$-only classification of the winding ladder is a derived structural chord (preserved in the SCOPE FLAG below); but **which particle occupies each rung** — the "Particle (real-space body)" column — is an **imported identification** (electron ↔ $(2,3)$, proton ↔ $(2,5)$, …), not substrate-forced. The rung STRUCTURE is derived; the particle SELECTION is imported (PEER).
