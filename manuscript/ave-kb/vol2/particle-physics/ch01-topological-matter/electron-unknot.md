...
This is a Beltrami standing wave where the continuous $\mathbf{E}$ and $\mathbf{B}$ field lines are mutually orthogonal and feed into each other in a closed topological loop ($\nabla \times \mathbf{A} = k\mathbf{A}$), permanently trapping the energy. 🔴 *(2026-06-24: "permanently trapping the energy" is **topology-pinned** (the closed Beltrami loop / Ax2 winding) + boundary, NOT a bulk self-focusing well — the bulk self-trap is a Cartesian artifact, RULED OUT by Stage-2 Mode-III; rides the [`master-equation.md`](../../../vol1/dynamics/ch4-continuum-electrodynamics/master-equation.md) localization-scoping banner; mass = A1 (#260) untouched.)* The unknot has circumference $l_{node}$ and tube radius $l_{node}/(2\pi)$, giving mass $m_e = T_{EM} \cdot l_{node}/c^2 = \hbar/(l_{node} \cdot c)$. The internal electrodynamic circulation of this resonant LC loop produces **gyroscopic precession** (gyromagnetic ratio $g=2$) in the presence of an external magnetic field. Quantum Spin's double-cover **STRUCTURE** is thereby classically realized as the continuous optical circulation of this massive electromagnetic light-loop. 🔴 *(2026-06-21 Rule-12 re-scope + [SPIN-HALF-POSITED] #584/#585: the circulation realizes the spin-½ double-cover **STRUCTURE** (K4-native), but the fermionic **SELECTION** (½ over integer) is imported — $\pi_1=\mathbb{Z}_2$ admits both statistics, forces neither, PEER-WITH-SM. The **$g=2$ VALUE is POSITED** (the imported Dirac value), NOT derived from the circulation: the $2\pi$/$4\pi$ double-cover forces the spin-½ structure, not the $\mu/S$ ratio — proton $g_p\approx5.586$ / neutron $g_n\approx-3.826$ are also spin-½ with the same double-cover yet $g\neq2$. g=2 mechanism-AVE / value peer-with-Dirac. See [clm-rkisb8](../../../vol1/claim-quality.md) and [substrate-perspective-electron.md](substrate-perspective-electron.md) §g-row.)*
