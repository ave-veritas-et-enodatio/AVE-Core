...
<!-- kb-frontmatter
kind: index
subtree-claims: [clm-8c3yhs, clm-8zpicx, clm-ezai5b, clm-gfdplp, clm-h9aqmt, clm-hb2xmj, clm-hvb7q3, clm-jupq56, clm-jwyy6l, clm-k6olj8, clm-ka5zdx, clm-lj4ok5, clm-ou2jym, clm-oygz1i, clm-salw2h, clm-uatcql]
subtree-experiments: []
-->
...
Matter is defined as localised, self-sustaining topological knots of confined electromagnetic waves in the substrate. Every stable elementary particle corresponds to a discrete standing-wave topology, with physical properties derived from non-linear electrodynamics of the resonant structure. Mass emerges from distributed continuous inductance (Lenz's Law), and the fine-structure coupling constant *runs* via dynamic capacitive yielding (a **sketch** — the mechanical *analog* of QED running, **not** a quantitative reproduction of the QED β-function; a rigorous AVE running-coupling derivation is an **open problem**, per [Electron Unknot](electron-unknot.md) §"Deriving the Running Coupling Constant" and `clm-h9aqmt`).
...
| Gauss Linking Number (Hopf Charge) | $Q = \frac{1}{16\pi^2} \int \epsilon_{ijk} \vec{n} \cdot (\partial_i \vec{n} \times \partial_j \vec{n}) \, d^3x$ |
| Dynamic Capacitive Yielding | $C_{eff}(\Delta\phi) = C_0 / \sqrt{1 - (\Delta\phi/\alpha)^2}$ |
| Antimatter Disintegration | $e^{-}(+\boldsymbol{\omega}) + e^{+}(-\boldsymbol{\omega}) \longrightarrow 2\gamma$; $E_{total} = 2 m_e c^2 = 1.022$ MeV |
| Electron unknot circumference | $C_{loop} = \hbar/(m_e c) = l_{node} \approx 3.86 \times 10^{-13}$ m |
| Electrostatic Self-Energy | $U_{AVE} = \oint_{C_{loop}} T_{EM}\ ds = T_{EM} \cdot l_{node} = 1.0 \ m_e c^2$ |
| Spin ↔ Gyroscopic Precession | Classical gyroscope and quantum Dirac spinor ODEs yield identical trajectories ($\Delta < 10^{-8}$) |
...
| [The Electron as a HOLLOW VORTEX — Class-C Binding-Structure Picture](hollow-vortex-binding.md) | **Class-C CONSISTENCY canonicalization** (NOT new physics, NOT a chord): the electron as a self-consistent hollow vortex — cavitated core ($\bar\rho_{cav}=-1/\varphi$) drilled/held-open by the $(2,3)$ circulation ($\propto\Gamma^2/R^3$), closed by void↔vacuum surface tension ($\sigma/R$), equilibrium $R^*=\Gamma/\sqrt\sigma\approx1.6\,\ell_{node}$. Carries the load-bearing honesty demotions: $\sigma=0.18712$ IMPORT-BY-IDENTITY (= #190 shell interface), $R^*/\ell_{node}\approx1.6$ DIMENSIONALLY-FORCED (consistency not discriminator), binding = FORM-consistency NOT a chord. Existence held INDEPENDENTLY by the T2/charge $\Gamma=-1$ wall (cross-linked, not re-derived). 5-attempt existence trail + the forward $R^*$-vs-drive chord-locus (not realized) |
| [Electron as BoundResonator — Property × Bucket Coverage Matrix](electron-bound-resonator-coverage.md) | The electron = `BoundResonator` instance-1 (def-b42e9d). One row per property/kinematic sorted into bucket A (testable-now on the rung-1 A1-scalar cage) / B (winding-sector L4 deferral) / C (NOT-derived gap), each tagged FORM-chord / VALUE-echo|definitional / GAP. Indexes the RUNG-1 EXISTENCE acceptance-suite (T3.3 Γ=−1 wall · T3.4a mass=cutoff · T3.4b cold/α-FREE Q **≈30.8 NOT 137 = clean ECHO-corroboration of Q=1/α being calibration** · T3.4c persistence) + the ranked GAP register. No-claim index leaf; source-of-truth `research/2026-06-17_electron-coverage-matrix.md` |
