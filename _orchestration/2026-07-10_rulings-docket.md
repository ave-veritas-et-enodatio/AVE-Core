...
## Continuation — 2026-07-20: B-MECHANISM DERIVATION lane — what closes the bulk channel's far-field port (ENTRY 30)

**Append-only, dated block (union-safe tail; prior blocks untouched).** New implementer/derivation lane (`research/b-mechanism-derivation`, PR [DO-NOT-MERGE][REVIEW: pending-orchestrator]) — Grant-fired 2026-07-20, verbatim `[sic]`: `"fire"` on the Reading-B mechanism derivation. Executes the #753 §6 sharpened obligation: *find the structure the vacuum HAS and a rock LACKS* that closes the bulk channel's far-field radiative port for gravitating sources. **Canonizes nothing; banks no physics at result-class; mints no `clm-`; READ-ONLY on KB.** Frozen bins committed ALONE (`9ad1831d`) before any derivation. Every `[canon]` anchor two-method verify-before-cite'd at HEAD `e2e870c0`.

> **★ENTRY-number note.** Authored as next-free on main's base (**30**); #750/#751 still hold provisional 27 on their branches and renumber at their own merges (collision-tolerant `merge=union` convention, same class as ENTRY 24/25/29). Read by entry number + content, not file position.

### ENTRY 30 (2026-07-20): implementer lane — Reading-B mechanism derivation (B1/B2/B4 three-trace)

- **Frozen-bin verdict: UNDETERMINED, leaning B1-FORCED.** `research/2026-07-20_b-mechanism-derivation_result.md` (+ pre-reg `_prereg-FROZEN.md` committed alone first; + driver `research/drivers/b_mechanism_k4_commonmode_residual.py` + `_results.json`, `ave.core.constants` read-only, engine byte-UNTOUCHED). Three independent traces run:
  ...
  - **B1 (constraint / GR-parallel) — a DERIVED AVE-native closing mechanism EXISTS.** The **K4 tetrahedral (`T_d`) 4-port common-mode rejection**: the scatter `S = ½·𝟙 − I` has the A1/longitudinal as its `+1` "DC" eigenvector, which common-mode-rejects (destructive interference with neighbors) and **losslessly transduces into the T2/shear triplet** — the **substrate-native Gauss law** (`∇·E=0` forbids longitudinal vacuum EM, enforced automatically by `T_d`; `clm-j550uh`, Ruling-21; descends from the Ax3 curl-only Maxwell Lagrangian `L_node = ½ε₀|∂_tA|² − (1/2μ₀)|∇×A|²`, U(1)-gauge). **★This IS the "structure the vacuum has and a rock lacks"** (a rock is a continuum with no `T_d` 4-port scatter; its P-wave is an independent persisting DOF). It WOULD close the port EXACTLY and is consistent with all gravity canon (static `n(r)` gradient; GW=T2 shear; DM-halo reactive near-field).
- **Why UNDETERMINED not B1-FORCED (the single unforced choice, flag-don't-fix).** The B1 mechanism directly **contradicts** B4's finding: the SAME K4/srs net, read as **Cauchy site-elasticity** (Model β, `clm-bnd5rq`), carries a propagating P-branch (Reading A, port open, EXCLUDED 9–110σ); read as **K4-TLM bond-scatter** (Model α, `clm-j550uh`), rejects the A1 common-mode (port closed exactly). The corpus carries **both models of one net with opposite verdicts** and does NOT adjudicate which owns the *gravitational* A1 coupling. Per the discipline, an unforced load-bearing choice ⇒ STOP at UNDETERMINED, fork stated. The three-trace menu is thereby **collapsed onto ONE sharply-stated model-ownership fork** — the same ruling Q1 (#753 §4) and FLAG-1/D5 (#750) already owed; this lane supplies the *mechanism* Model α would use and names exactly what would force it (§3.4 a/b).
- **Residual (chord-potential): empty either way.** IF B1 governs, the port closes **exactly** (`F_bulk/F_shear ≡ 0`; the exact/unitary K4 scatter drives A1→0; lattice-leakage bound `(ωℓ_node/c)² ~ 10⁻⁴⁸` for HT/J0737 — unobservable at any pulsar precision). **But this exact-null is PEER-with-GR** (GR also predicts pure-tensor) ⇒ a consistency requirement, **NOT an AVE-distinct chord**; and the exact scatter yields **no derived nonzero residual** ⇒ no chord-eligible number for next-gen pulsar timing. IF Reading A governs, `F_bulk/F_shear ≈ 0.03–0.12` is EXCLUDED (framework falsified). Matches the framework-wide "no AVE-distinct chord inside; uniformly peer-with-GR."
- **Anti-seduction fence held.** NONE-DERIVES was genuinely reachable (B4+B2 clear "branch exists" + "source drives it"); only the B1 constraint stands between "exclusion goes live" and "port closes", and its applicability is the unforced choice. The B1 constraint was NOT manufactured (pre-existing DERIVED group theory, the same fact that IDs the photon); two rescue attempts (tracelessness §2.2, sleeper §1.2) were killed against the framework's wish.

**Class:** derivation research-doc (forms derived by `T_d` group theory + multipole algebra; values import/consistency-tagged); canonizes nothing; banks no physics; mints no `clm-`. **PR [DO-NOT-MERGE][REVIEW: pending-orchestrator].** `make verify` green; driver 5/5 asserts pass. **Owed to auditor lane (implementer surfaces; auditor lands):** (1) the B1 model-ownership ruling (Grant-gated sector-ownership adjudication — Model α K4-TLM vs Model β srs-Cauchy for the gravitational A1; resolves Q1 + FLAG-1/D5 together); (2) if Model α ruled, the §3.4(b) follow-on deriving that `T_d` common-mode rejection applies to the mechanical dilatation `∇·u` (new lane, own version chain); (3) the `08_gravitational_waves.tex` warningbox + FLAG-A speed-label reconciliation (resolves with the ruling). This lane lands no KB/tex edits.
