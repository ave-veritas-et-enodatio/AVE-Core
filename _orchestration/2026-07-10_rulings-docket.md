...
| **X44 Komar-source** | install √S Picard weight; reconcile flux↔M_eff | **FIRED 2026-07-12 — bin (iii) UNRECONCILED** (`analysis/x44-komar-source`; prereg `b9f88823` → result doc). Gauss ≡ ∫T√S holds **(install-tautology)**; η_mixed O(1) stable vs N; clock↔bind MATCH fails (~93–97%). **(c) ratification of the source FORM stands**; banked negative = the reconciliation expectation of THIS density-only implementation; faithful test = **X44b** (escalation §7 option 5 — linear-in-ε clock + stress register + virialized family). Post-review repairs R1–R5 landed (recover-GR gate un-vacuated, real shape_dev=6.08e-4; ADD-side one-ledger η=+8.3e-5 shipped; §5b closure chain). DO-NOT-MERGE PR pending orchestrator review | Grant / orchestrator |
