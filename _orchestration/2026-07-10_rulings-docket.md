...
- **FROZEN (a)/(b)-ledger criterion (R-2, Rule-11):** net-per-cycle transfer `≥ tol=3.53e-3` in EVERY driven cell → (a-ledger) never fires; finite bath returns / dense stays at 0.50 → (b-ledger) not clean either. **DEGENERATE.** Magnitude is coupling-scale-governed (same UNDETERMINED as c-magnitude). (c-magnitude) fires on the frozen coupling-model shape split (on-site→Ohmic `J_norm(0.9)=0.31` / strain→super-Ohmic `0.036`).
