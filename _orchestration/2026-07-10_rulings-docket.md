...
**FLAGS (flag-don't-fix).** (1) #748 docs + QUADRATIC-form KB sweep are UNMERGED (`feat/mond-kernel-adjudication`); on main the leaf still shows LINEAR + open flag — orchestrator sequences #748-merge + this trace together. (2) #748 R2 cites the narrative at `galactic_rotation.py:136`; verified on main-base at `:135` (off-by-one, non-load-bearing). (3) Sector-framing note: task=A1, docstring=μ mutual-inductance, catalog=SYM — the amplitude verdict is sector-invariant.
