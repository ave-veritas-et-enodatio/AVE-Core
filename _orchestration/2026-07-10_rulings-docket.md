...
| **G-PERSIST** | genesis N≥14 persistence battery | **LANDED — #670 MERGED. Leans CONFIRMS bin (ii) A-WEAKENED via φ-dispersion.** E recovers with N (0.69→0.84→0.87 = absorber-leakage confirmed) / φ collapses with N (0.87→0.73→0.51 = boundary-clean non-persistence); closed-box = reflecting-cavity artifact-leaning (E≡1.0 conservation-identity, φ→10× N-stable). **PENDING-GRANT:** the **closed-box fork** (echo-chamber vs genesis-under-confinement), the **localization-observable + φ-channel-plant follow-ons**, and the **docket stamp flip** — the stamp flip does **NOT** require the fork (the PML φ-trend is boundary-clean on its own). (`research/2026-07-13_genesis-npersist-n14-battery_RESULT.md`) | **Grant (fork + stamp PENDING)** |
...
- **PENDING-GRANT (his word genuinely owed):** the **G-PERSIST closed-box fork** ruling + the two **follow-on candidates** (localization observable, φ-channel plant) + the **docket stamp flip**; and **Q4** (F6 sink go / `ρ_latent` parameterization) as already docketed above.
- **Landed / in-flight (no Grant word owed to book them):** K-SWEEP, CVR doc set, Q-ladder relabel, killed-ledger booking, F6 tier-1 driver — all merged-or-Grant-GO'd sibling-lane work, tracked here for docket completeness.
