...
- **D1 (PRIMARY, EXISTING DATA):** zero-drag / `v`-independence at the outer-boundary crossing — reactive predicts no `∝v` stall (at most a reactive reflection), demoted resistive predicts `a_drag∝v^{n≥1}`. **Flips vol4 `clm-h55fy1`.** Pioneer/Voyager/New-Horizons null already leans reactive-confirmed.
