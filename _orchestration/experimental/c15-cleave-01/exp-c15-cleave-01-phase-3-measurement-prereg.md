...
Does $V$ step **at all** — monotonically with displacement, repeatably, in the vacuum-gap-only config? The polarity-odd, gap-INDEPENDENT component is classically **0.0** (displacing a neutral conductor in vacuum liberates no net *gap-independent* charge); the raw vacuum charge is **NOT** classically zero — contact-potential-difference (CPD) gives a polarity-odd, gap-DEPENDENT ($\propto V_{CPD}/g^2$) term, the dominant Casimir/Kelvin-probe systematic, separated from the floor by the **gap-sweep**. AVE $\to$ **non-zero, sign-tracking, gap-INDEPENDENT displacement charge** ($e/\ell_{node}$ pure constant). This level needs **no** knowledge of $C$ but DOES need the gap-sweep to separate the gap-independent floor from the $\propto 1/g^2$ CPD background; a gap-independent step surviving the sweep is the load-bearing chord/echo discriminator — **it does not float with $C$** and is not faked by CPD.
