...
| **S3** | does winding+H_couple **PIN** the dispersing A1 core? | `wd0gg25yw` | `wpc5ip174` | #411 ✅merged | 🔴 **DISPERSE-FALSIFIED** (energy-certified, robust; the re-route's central mechanism is wrong) |
| **coupled eigensolve** | does the confined **coupled (mass+charge) eigenmode EXIST** + the V_yield/V_snap/m_e ladder | (emergence scoping `wue1k3pnb` served) | `w4v6abhxs` | 🔴 **DOES-NOT-EXIST (real-space-locus-scoped)** — confined MASS eigenmode exists (a/b/c/e PASS) but the real-space (2,3) winding bleeds off (d FAIL); ⚠ canonical (2,3) is PHASE-SPACE (ave-kb/CLAUDE.md:22) so this tested the wrong locus → phase-space coupling-winding re-test scoped. #413 was a stranded stacked-merge (landed on the dead S3 branch); content re-landed on main via the rescope PR |
...
7. **The pivot to the bench**: the engine's internal routes are characterized; the AVE-distinct chord lives in **forward predictions** (the parity channel), arrived at by the engine exhausting its internal mechanism-hunt, not by assertion. *(2026-06-24 update: "exhausting" completed with the phase-space coupling-winding BREAK (#417) — both internal dynamical loci NEGATIVE; see the PIVOT UPDATE banner below.)*
...
**DONE — coupled eigensolve (`w4v6abhxs`):** **DOES-NOT-EXIST (real-space-locus-scoped).** A confined/gapped/lossless/non-tautological MASS eigenmode exists (gates a/b/c/e PASS; HALT recovers fork-b ω=2.84 on the 2.87 anchor) but the **real-space** (2,3) winding bleeds off it (gate d FAIL). Ladder delivered: V_snap & V_yield both m_e-imported calibration (derives_m_e=False); FORM = mass cage binds at A\*→1 = V_snap cap, coupling front at A≈4/7 = V_yield floor; scale-free (m_e irreducible). **⚠ The (2,3) is canonically a PHASE-SPACE winding (ave-kb/CLAUDE.md:22) — this tested the real-space locus, the wrong one** → phase-space coupling-winding re-test scoped (the proper gate d; Grant's "coupling profile" / resonantly-matched LC-tank winding). Internal routes to a *single* mass+charge eigenmode closed in real space (Stage-2 + S3 + this).

**GIT-STATE NOTE:** the eigensolve PR (#413) was a **stranded stacked-merge** — it merged into the already-merged S3 branch (#411 merged to main 44 min earlier), so its content never reached main. Re-landed on main via the rescope PR (with the real-space-locus caveat baked in). Lesson: stacked PRs must be retargeted to main before merge, or merged base-first in one sitting.

> 🔴 **UPDATE (2026-06-24, post-#417 — supersedes the "essentially complete" closure list below, which predates the phase-space coupling-winding re-test).** The phase-space coupling-winding test (the proper gate-d at the canonical locus, scoped above as pending) has now RUN: **BREAK (#417)** — the dynamical orbit carries the LC **carrier ratio**, not the topological (2,3) (proven by carrier-ratio detuning). So **BOTH internal dynamical loci are NEGATIVE**: real-space eigensolve (#415, gate-d FAIL) + phase-space coupling (#417, BREAK). The closure picture is now: **two-natured electron** — DYNAMICAL A1 mass cavity (localizer = cavity-eigenmode, fork-b EXISTS) + STATIC (2,3) = `Link(∂Ω,F)` charge (does not pin, RIDES the cage; un-walked-back). The **dynamical-orbit hypothesis for charge is REFUTED**; charge = static-Link STANDS. CONSISTENCY-class; Q=137 EMPTY; mass=A1 (#260) untouched. See `research/2026-06-24_engine-reroute-epic-summary.md`.
