...
4. **Substrate amplitude correlator decomposition** (closed forms): $\sigma^2 = k_B T_{eff} S_0^3/C_0$; $\kappa_3/\sigma^3 = -3 a \eta_T S_0^{-1/2}$; $\kappa_4/\sigma^4 = -3 (1 + 4 a^2) \eta_T^2 S_0^{-1}$ — where $\eta_T = \sqrt{k_B T_{eff}/(C_0 V_y^2)} \sim 10^{-6}$ at room T canonical parameters.
...
1. **$\kappa_3/\sigma^3$ scaling: cubic → linear at leading order in $a = V_{DC}/A_c$.** Prereg anticipated $(V_{DC}/A_c)^3$; derived: $-3 a \eta_T \cdot S_0^{-1/2}$ — linear in $a$ multiplied by tiny substrate-thermal-energy ratio $\eta_T \sim 10^{-6}$ at room T. Substrate-mechanical reason: $U''' \propto V_{DC}$ (linear), but dimensionless cumulant divides by $\sigma^3 \propto (k_B T_{eff})^{3/2}$ NOT by $V_y^3$. **Implication for Phase 2**: per-site skewness is ~10⁻⁶ at room T (NOT larger as brief anticipated); aperture-aggregate observability is 2 OOMs harder than initially scoped.
...
- Honest aperture-aggregate observability scoping: room-T narrow-aperture skewness ~10⁻⁶ (structurally limited); observability requires cryogenic regime OR massive statistics OR substrate-engineering of $C_0$ effective scale
- PONDER-05 specific predictions at $a = 0.687$: skewness ~$10^{-6}$, kurtosis-excess ~$10^{-12}$ — testable IF amplitude-statistics extraction across many cycles is feasible at the PONDER-05 bench
...
Phase 2 scoping question for Grant: given the honest room-T observability ~$10^{-6}$ skewness signature, is there a cryogenic regime OR substrate-engineering question worth scoping for Phase 2, or does the room-T scope-limit motivate parking the epic at this empirical-observability threshold? Per epic doc honest framing: "documented as derivation-gated or experimentally-inaccessible" remains a valid outcome.
