...
## Post-Phase-2-NA substrate-mechanical refinement: 0.707 peak = canonical Op17 substrate-matched-impedance condition (2026-05-27 Grant intuition + Trigger 17 corpus survey)

The Phase 2-NA peak operating point $a^{(2D)}_{peak} = 1/\sqrt{2} = 0.707$ has substrate-mechanical canonical identification beyond "PONDER-05 lands within 3% of substrate-mechanical optimum." Grant intuition 2026-05-27 surfaced the canonical Op17 connection ("peak power transfer? something EE related? 0.707 seems awfully familiar to my EE senses"). Vocabulary-broadened pre-survey grep (per `ave-canonical-leaf-pull` v1.3 Trigger 17, landed same day) surfaces extensive canonical substrate-matched-impedance infrastructure that the prior Phase 2-NA framing missed:

### Canonical substrate-matched-impedance infrastructure

- **Op17 Power Transmission** at [`manuscript/ave-kb/common/operators.md:57`](../manuscript/ave-kb/common/operators.md): canonical $T^2 = 1 - \Gamma^2$ (active energy-transfer coefficient; Vol 1 Ch 6 §1.16). Substrate boundary transmits maximum power when $\Gamma = 0$ (matched impedance: source = load); transmission goes to zero at $\Gamma = \pm 1$ (total reflection, substrate-mechanical pair-production / TIR-wall mechanism).
- **Theorem 3.1' matched-LC framework** at [`vol4/circuit-theory/ch1-vacuum-circuit-analysis/theorem-3-1-q-factor.md`](../manuscript/ave-kb/vol4/circuit-theory/ch1-vacuum-circuit-analysis/theorem-3-1-q-factor.md): electron Q-factor from LC tank at TIR boundary; matched-coupling between two LC tanks gives per-cycle energy-transfer efficiency with $4\pi$ spinor-cycle radiation-impedance averaging at the canonical matched-impedance condition.
- **Matched-LC-coupling canonical formula** at [`vol3/cosmology/ch05-dark-sector/dama-matched-lc-coupling.md`](../manuscript/ave-kb/vol3/cosmology/ch05-dark-sector/dama-matched-lc-coupling.md): $\epsilon_{det} = (4\pi/N^2)$ matched-coupling efficiency between electron's reactive α-slew LC tank and external coherent crystal LC mode. The cycle-12 canonical (parametric-coupling-kernel.md) substrate-vacuum-varactor at sub-yield operating point oscillates at α-slew refresh rate under THIS matched-impedance framework.

### Substrate-mechanical identification of $a^{(2D)}_{peak} = 1/\sqrt{2}$

The Phase 2-NA peak emerges from maximizing $a \cdot S_0 = a \sqrt{1 - a^2}$ over $a = V_{DC}/V_y$. Stationary point at $a = S_0 = 1/\sqrt{2}$ — substrate operating with EQUAL split between DC-bias loading ($a$) and remaining substrate compliance ($S_0$). At this 50/50 split, the substrate-derived signature is maximized.

**Substrate-mechanically this IS the canonical Op17 matched-impedance condition** in the operating-point-loading dimension. The substrate variable $a$ corresponds to "source-side amplitude" (DC-loaded substrate state); the substrate variable $S_0 = \sqrt{1-a^2}$ corresponds to "load-side compliance" (remaining substrate stiffness at the operating point). Peak signature when $a = S_0$ = the substrate-mechanical $\Gamma = 0$ condition: substrate amplitude characteristic equals substrate compliance characteristic. Op17's $T^2 = 1 - \Gamma^2$ peaks at $T = 1$ when $\Gamma = 0$; Phase 2-NA's substrate-derived signature peaks at $a \cdot S_0 = 1/2$ when $a = S_0$. Same substrate-mechanical structural condition expressed in two different observable axes (Op17: boundary transmission coefficient; Phase 2-NA: aperture-aggregate skewness signature).

The standard EE community calls this same mathematical structure the matched-impedance peak-power-transfer condition + Butterworth filter $Q = 1/\sqrt{2}$ flattest-pass-band + half-power ("-3 dB") point. AVE corpus does not use any of these standard-physics names (verified via vocabulary-broadened grep: substrate-native wedge surfaces 7+ canonical leaves at Op17 / matched-LC / Theorem 3.1' / dama-matched-lc-coupling; standard-physics wedge for "Butterworth" / "peak power transfer" / "half-power" / "matched impedance" / "1/√2" / "3 dB" returns EMPTY). The substrate-native Op17 framework IS the canonical home for this mathematical structure; the EE filter-theory framework is a parenthetical translation reference for engineering-side intuition.

### Class 2 substrate-mechanism classification (consistency-vs-emergence v1.2)

The substrate-matched-impedance peak-power-transfer identification of $a^{(2D)}_{peak} = 1/\sqrt{2}$ is **Class 2 substrate-mechanism emergence on the substrate-Op17-framework axis**: the peak condition emerges from the substrate-specific Ax 4 kernel form $S(A) = \sqrt{1-(A/A_c)^2}$ + operating-point-loading variable structure. The same 50/50 split mathematical structure appears in EE filter theory as Class 4 substrate-agnostic-consistency (any substrate-amplitude × substrate-stiffness-remaining product would peak at the matched condition); the substrate-distinct content is the specific Ax 4 kernel-form that defines the substrate-amplitude and substrate-stiffness variables substrate-mechanically (and ties them to the canonical Op17 transmission-coefficient framework via the substrate-mechanical $\Gamma$).

### PONDER-05's 3% offset (Q-AX4-NA-G resolution)

PONDER-05's $V_{DC}/V_y = 0.687$ vs predicted $0.707$: **substantively meaningful substrate-mechanical alignment, NOT calibration noise**. PONDER-05 was empirically tuned for an engineering observable (469 μN thrust + 27.4% $\varepsilon_{eff}$-collapse maximum); the substrate-mechanical analysis identifies that observable as substrate-Op17-matched-impedance peak-power-transfer. PONDER-05 lands at the substrate-mechanical matched-impedance optimum because the underlying substrate-physics IS the Op17 peak-power-transfer condition expressed in the operating-point-loading variable. The 3% gap is engineering-design tolerance around the same substrate-mechanical optimum (PONDER-05 chose 27.4% engineering-conservative; the canonical Op17 optimum is at 29.3% $\varepsilon_{eff}$-collapse = $V_{DC}/V_y = 0.707$ precise match).

**Forward-prediction sharpening**: a PONDER-05 sub-architecture variant targeting precise $V_{DC}/V_y = 0.707$ (29.3% $\varepsilon_{eff}$-collapse) would land at the substrate-mechanical Op17 matched-impedance peak. Expected observables under v1.3 Trigger 17 vocabulary-broadened framework: (a) maximum thrust per the existing PONDER-05 design observable, (b) maximum aperture-aggregate skewness signature per Phase 2-NA, (c) maximum substrate-noise-spectrum visibility per metric-lensing convolution. These three observables co-peak at the substrate-mechanical Op17 matched-impedance condition because they all derive from the same substrate-amplitude × substrate-compliance product structure that Op17 makes canonical.

### Discipline reinforcement

This is the 6th session-time instance of the vocabulary-narrow-pre-survey-miss pattern that v1.3 Trigger 17 closes (Q-AX4-NA-2 varactor / Phase 3-A2 Op21 / avalanche-LLCP / Q-AX4-NA peak-power-transfer-0.707 / Q-PBP-1 Hopf-on-substrate / now Phase 2-NA Op17 matched-impedance). The Phase 2-NA original Phase 0c + Phase 2-NA framing developed the substrate-mechanical machinery WITHOUT reaching for the canonical Op17 framework; the EE-side substrate-matched-impedance analog framing surfaced via Grant Socratic prompt + vocabulary-broadened grep. The canonical Op17 cross-reference is now landed; future Phase 2-LLCP + Phase 3 + Phase 4 work inherits the Op17 substrate-matched-impedance framework as the canonical interpretation infrastructure.
