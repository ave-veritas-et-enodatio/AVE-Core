...
- **(ii) S-exponent / c_shear forks — fork-1 resolution recorded (`wkuou8n2l`, 2026-06-17).** The original framing below bundled THREE distinct objects under one "1/4-vs-1/2" symbol; fork-1 (5-lens read, all grep-confirmed) separates them:

  > **🔴 [SUPERSEDED framing, preserved per Rule-12]** the prior text read: "*`master_equation_fdtd.py:165-168` returns `S**0.25` while `c_eff_squared` (`:148-151`) implies n = `S**0.5` … it collides with the T1.6 `c_shear=c₀·S^(1/4)` def-lock (the same 1/4-vs-1/2 ambiguity).*" That conflated a same-argument code bug with a base-notation identity. The split below is the resolved truth.

  - **(ii-a) c_shear "corpus contradiction" — DISCHARGED, RESOLVED (not a contradiction).** `c_shear = c₀·S^(1/2) ≡ c₀·(1−A²)^(1/4)` is ONE curve in two bases (kernel-base S vs amplitude-base (1−A²)), since `S = (1−A²)^(1/2)` — verified identical at A∈{0,0.3,0.5,0.8,0.95}. INVARIANT-S2 (`CLAUDE.md:64-65`, kernel form) and the amplitude form AGREE. Grant's chain-rule-through-the-kernel intuition is confirmed: the apparent "S^(1/4)" is the amplitude exponent, NOT a second physics. **CITE FIX:** the amplitude form lives at `invariant-gravitational-impedance.md:30` (NOT `gw-impedance-perturbation.md:30`, which is a 20-line file with no line 30) — corrected wherever the plan/docstrings cited the wrong leaf. NOTE the T1.6 module/test docstring + print (`test_l1_multiwave.py:44,52,203`) still writes the def-lock as "`c_shear = c₀·S^(1/4)`" with "NOT c₀√S" — that is BACKWARDS relative to this discharge (the corpus form IS `c₀√S = c₀·(1−A²)^(1/4)`); a Rule-12 banner at `:60-72` records the supersession (the frozen S=1 bins are unchanged, so no pass/fail moves), pending a Grant-ratified def-lock rewrite (a logic-claim change, out of this doc-edit scope). Surfaced as a NEW contradiction (flag-don't-fix).
  - **(ii-b) refractive_index() S-exponent — MAGNITUDE resolved, SIGN/ontology PENDING Grant.** `refractive_index()` returns `S**0.25` (`master_equation_fdtd.py:169`; the `:165-168` block above it is the in-code FLAG comment) while its OWN `c_eff_squared` (`:148-151`, `c_eff²=c₀²/S`) forces physical `n = c₀/c_eff = S^0.5`. This IS the one genuine SAME-ARGUMENT static bug (`S^0.25 = (1−A²)^0.125`, off by a factor 2 in the exponent), replicated at `crystal_engine.py:432` + inherited at `boundary_invariants.py:123-126` (~5 sites). **MAGNITUDE:** should be `S^0.5`. **SIGN/ONTOLOGY (open → Grant):** is this method the matter/bending index `n=c₀/c_eff=S^(+1/2)→0` (core slows light, reflective short) OR the canonical binding-wall index `n_eff=S^(−1/2)→∞` (high-Z self-trapping wall, `trampoline-framework.md:802` / `mass-closure-theorem.md:32`)? These go OPPOSITE directions; `S^0.25` matches NEITHER — likely the H3 spin/branch (Γ-sign) selector. **The code fix is task #12, GATED on a Grant ruling — NOT touched in this commit-set.** Wall-depth note: the bug UNDERSTATES |Γ| by ~0.21 at A=0.99 but the chord-vs-echo VERDICT is sign-robust (all core-collapsing exponents drive Γ→−1; corrected S^0.5 DEEPENS, does not flip).
...
- **(iv) fork OPEN → Grant: is L5-genesis in-scope now?** Its closest target `clm-8zpicx` is sol **0.40 / DO-NOT-BUILD** and the genesis self-trap **dynamics-class is FALSIFIED** in the ledger (only the boundary-confinement operator is untested). There is **no solid precursor** to validate an L5 acceptance layer against today.
  ...
  > a separate Grant word). Fork text above preserved (Rule 12); the open-item is closed in the same PR.  <!-- rule12-freeze: base=48b1819a8780577938114f39557c0be72a738779 region=above offset=0 lines=62 bytes=15891 sha256=7f979cbf3466592b2f6593db85710017948845b3e58301885450dfd48ee57499 -->
...
| **L3 mass-cage** | T3.1 longitudinal-mode-is-real | chord (vs QED) | A1 bulk DOF | Ch9/Ch4 | sup-3kq9w7 → clm-crbl60 | I→II (bulk) | ✅ (CHORD — AVE-distinct existence; propagates at c_bulk=√(2G/ρ)=√2·c₀, dω/dk>0; FLIPS T1.7 ⊘) |
| L3 mass-cage | T3.2 c_eff(V) stiffening →∞ | emergent (Ax4) | mass-cage | Ch8/Ch14 | sup-mz4t0p → clm-gz7ryg | II (near-yield) | ✅ (Axiom-4 MANIFESTATION; c_eff=c₀·S^(−½)→∞ at yield, matches (1−A²)^(−¼) <1e-9; the cage precursor) |
| L3 mass-cage | T3.3 binding-wall Γ=−1 (POSITED cage; CONSISTENCY/FORM-chord) | consistency/FORM (was: chord) | Γ=−1 TIR wall | Ch3/Ch2; Ch17 | sup-1ecv2m → clm-kezk9z, clm-uatcql | III–IV (Γ=−1, posited) | ✅ (FORM-chord; α-FREE Γ_min(0.95)=−0.283 crosses gate; NOT self-formation = rung-2 gated ⭐) |
| L3 mass-cage | T3.4a mass = cutoff freq ω₀ | emergent→FORM/definitional | mass observable | Ch2/Ch9; Ch17 | sup-xgx063 → clm-ka5zdx, clm-unk0bd | II→III (near-yield→trap) | ✅ (FORM-chord; ω_cutoff≈2.87 gapped bound mode; m_e VALUE=definitional, never a cage output) |
| L3 mass-cage | T3.4b cold/α-FREE Q (chord-vs-echo decider) | **echo (clean negative)** | Q-factor | Ch5/Ch8; Ch17 | sup-wuy333 → clm-rtdmsn (echo-corrob) | I (linear; cold cavity) | ✅-with-echo-note (FORMS pass: Q finite>0; **VALUE=ECHO — Q≈30.8 NOT 137, α-free ⇒ corpus Q=1/α is an instance-baked calibration-echo; corroborates clm-rtdmsn value-scope; NOT a chord-✅**) |
| L3 mass-cage | T3.4c zero-drive persistence | consistency/FORM | stability (non-radiating) | Ch2/Ch9; Ch17 | sup-x7h9yh → clm-ka5zdx, clm-uatcql | II→III (trap) | ✅ (FORM; late/mid≈1.31 non-radiating standing mode; ≠ topological-winding protection = bucket-B winding-sector) |
...
⭐ = the three chord-deciders (Γ=−1 mass wall · integer charge · forced mass ratio). **Vol 9 chapters with no ladder row** (fork a): Ch6 temperature, Ch8 breakdown-rupture, Ch12 cosmological-values, Ch15 falsification (orthogonal bench axis), Ch16 lookup. **Score (RUNG-1 EXISTENCE completion-ritual 2026-06-17, task #19 + T3.1/T3.2 sup-propagation 2026-06-17 cleanup): 24 ✅ green-confirmed (incl. 1 ✅-with-echo-note) / 2 ⊘ absence-finding / 0 🔧 / 6 ⬜ / 3 🚩 (total 35) — valid medium + RUNG-1 EXISTENCE rung POSITED-cage FORM-chords confirmed; ZERO emergent chords forced (the one chord-vs-echo decider returned a clean ECHO). The 2 prior 🔧 (T3.1/T3.2, committed-green RITUAL-PENDING) are now sup-propagated → ✅ (sup-3kq9w7 → clm-crbl60; sup-mz4t0p → clm-gz7ryg).** This ritual flipped the L3 existence-rung rows (KEEP-BOTH — columns unchanged, only Status/Tag/Spine cells updated + the single T3.4 row SPLIT into T3.4a/b/c so the cold-Q echo reads distinctly; +2 rows ⇒ total 33→35):
> - **T3.3 Γ=−1 wall:** ⬜ ⭐ → **✅ (FORM-chord)** — sup-1ecv2m → clm-kezk9z, clm-uatcql. α-FREE Γ_min(0.95)=−0.283 crosses the −0.25 OP2 gate. Tag corrected chord→**consistency/FORM**: this is the POSITED cage (GIVEN a saturated core, the wall manifests), NOT self-formation (rung-2, still gated) — so the ⭐ chord-decider "does mass SELF-FORM" remains OPEN; the ⭐ is retained on the row as the make-or-break target, but its make-or-break (self-formation) is NOT what this FORM-✅ closes.
> - **T3.4a mass=cutoff:** ⬜ → **✅ (FORM-chord / VALUE-definitional)** — sup-xgx063 → clm-ka5zdx, clm-unk0bd. ω_cutoff≈2.87 gapped bound mode EXISTS; m_e the VALUE is never a cage output.
> - **T3.4b cold/α-FREE Q:** ⬜ → **✅-with-echo-note (NOT a chord-✅)** — sup-wuy333 → clm-rtdmsn (echo-corroboration). The FORM-bin passes (Q finite>0 from cold dynamics); the VALUE is an **ECHO** — Q_ringdown≈30.8, NOT 137, with α never baked ⇒ the corpus Q=1/α (cvr_model.py:72) is an instance-baked calibration-echo, CORROBORATING clm-rtdmsn's already-canonical value-scope (theorem-3-1-q-factor.md:19). The clean chord-vs-echo NEGATIVE the suite hunted; it strengthens NO "Q=137 is forced" reading (there is none).
> - **T3.4c persistence:** ⬜ → **✅ (FORM)** — sup-x7h9yh → clm-ka5zdx, clm-uatcql. late/mid≈1.31 non-radiating standing mode; ≠ topological-winding protection (bucket-B winding-sector).
>
> **✅ RESOLVED 2026-06-17 (was 🚩 FLAG — T3.1 / T3.2 committed-green RITUAL-PENDING):** the L3 precursor tests T3.1 (longitudinal-mode-is-real) + T3.2 (c_eff(V) stiffening) were committed green at `8e852460` (and pass in the 42-green suite run), but their completion-ritual propagation (Vol 9 leaf/Ch17 row + sup- node + figure-wire) was NOT in scope for task #19 (which covered T3.3/T3.4 only) — so they were marked **🔧 (committed-green, RITUAL-PENDING)**. The 2026-06-17 cleanup pass closed this: **sup-3kq9w7 → clm-crbl60** minted for T3.1 (the candidate target was VERIFIED correct — clm-crbl60 = vacuum bulk mass density + shear modulus + the K=2G Cauchy relation, which `c_bulk=√(2G/ρ)=√2·c₀` directly strengthens) and **sup-mz4t0p → clm-gz7ryg** for T3.2 (candidate VERIFIED correct — clm-gz7ryg = the single Axiom-4 kernel S(A)=√(1−A²), which the c_eff stiffening manifests in the longitudinal sector). Both wired into `engine-acceptance-suite.md` (frontmatter + layer index + L3 prose) with matching `### Quality` entries in `vol9/claim-quality.md`, all fractions `*pending*` (INVARIANT-S10). Figures `T3.1_debug.png` / `T3.2_debug.png` inherit sup- provenance through the leaf. Rows flipped 🔧 → ✅.
>
> The two ⊘ rows (T1.7 bulk / T1.8 Cosserat micro-rotation) remain absence-findings (PASS iff DOF ABSENT). **The Op-primitive + scale-invariance tier (`test_operators.py`, `sup-evhfcd`) is GREEN-confirmed** (not a §9 matrix row — runs ALONGSIDE). The remaining chord-deciders ⭐ (T4.2 integer charge · T5.3 forced mass ratio) and all L4/L5 GAP rows remain ⬜/🚩 — Grant-gated, NOT flipped. The full electron property×bucket coverage sheet is the KB leaf `vol2/.../electron-bound-resonator-coverage.md` (electron = BoundResonator instance-1, def-b42e9d).
