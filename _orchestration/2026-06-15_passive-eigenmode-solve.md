> **SURFACE-NOTE — landed 2026-08-19 from tag `archive/analysis/2026-06-15-passive-eigenmode-solve` (`7542990e`) as part of Wave-2 **D15a** (`_orchestration/docket-entries/2026-08-18-wave2-adjudication-sitting.md`); FROZEN at tag state, body below unedited.** Its **tip** status — §8 **Phase 25**, *"SUBSTRATE-PUMP OVERTURNED → SCOPED/UNADJUDICATED"*, keystone RE-OPENED at the methodology level — sits **upstream of later canon and is NOT the current adjudicated state.** Current: the keystone close at `_orchestration/index.md` line 284 (that file FROZEN 2026-08-13) and `manuscript/ave-kb/common/program-arc-map.md` ARC-08 = **"NEGATIVE (leans-falsified)"** — a closure that rests on the independent category-error scoping (landed `19fc78ea`, 2026-06-16 08:43 −07:00 — BEFORE either the freeze-g banking or the Phase-25 overturn, so derived from neither; the interior-field route is sub-Nyquist / phase-space) plus the ARC-12 Mode-III **energy-certified** bulk-cage result (PR #403 / #404), **not** on the `dt→0` plateau that Phase 25 contests. Landed because this log is the only carrier of Phase 25 and of the §12 analytical-toolkit-audit findings; read the body as the 2026-06-16 record.
