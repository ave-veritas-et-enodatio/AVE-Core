...
See "Fix log" below. Site: `vol_4_engineering/chapters/01_vacuum_circuit_analysis.tex:336-355` (pre-fix numbering on origin/main; +3 on this branch after the caveat insertion — e.g. the "maximum hysteresis loss" line sits at `:358` here).
...
- **Drift:** the subsection banks the DISSIPATIVE (first-order / memristive / `Re(Z)` / finite-area-loop) branch of the **registered-OPEN near-yield yield fork** as established fact, with no fork-open note. Contradicts `retention-transition-split.md:61-63` (near-yield loop is fork-gated, fork held OPEN: "do not bank it either way in load-bearing prose"; Grant leans reversible/reactive — n.b. that leaf's RULED-2026-07-19 stamp is the Op3-transduction ruling, not a fork adjudication) and the `#735` result (`research/2026-07-19_yield-fork-discriminators_result.md`: neither leg closes the fork; crux = upstream Flag F).
