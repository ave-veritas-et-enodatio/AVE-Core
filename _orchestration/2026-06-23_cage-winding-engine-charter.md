...
**Created:** 2026-06-23 · orchestrator-tracked · **Status:** 🔴 SUPERSEDED 2026-06-24 (was: SCOPED, awaiting Grant go on Gate 0)
...
> **🔴 SUPERSEDED-BY (2026-06-24).** This charter is superseded by the native-first re-architecture ([`2026-06-23_full-engine-pathway.md`](2026-06-23_full-engine-pathway.md), pathway:4 explicitly supersedes this doc). It is preserved (Rule 12) for the build-history trail. **Do NOT re-adopt the "bulk A1 mass-cage" framing from here.** The Stage-2 native-cage make-or-break RAN and returned 🔴 **MODE-III DISPERSE** (energy-certified; [`research/2026-06-24_engine-stage2-native-cage_result.md`](../research/2026-06-24_engine-stage2-native-cage_result.md)): the **bulk self-trap is a Cartesian artifact (RULED OUT)**. Note the **vindicating** half — this charter's own key move (render the cage as a Γ=−1 *boundary condition*, NOT a bulk c_eff(V) term — §"Architecture", :23/:25) is exactly the route that **SURVIVES**: boundary/topological localization STANDS. mass = A1 (#260) is UNTOUCHED — only the localization MECHANISM (bulk self-focus → boundary + winding + H_couple) is ruled out. The inline 🔴 markers below scope each "mass-cage / bulk" phrasing accordingly.

The electron is a Cosserat (2,3) **charge-winding** confined inside an A1 dilatation **mass-cage** (the Γ=−1 wall). **No built engine carries both** — and that gap is exactly where the AVE-distinct charge chord lives. 🔴 *(2026-06-24: "mass-cage" here = the Γ=−1 **boundary** wall — the SURVIVING boundary/topological localization. The bulk self-focusing-well reading is the FALSIFIED route; mass = A1 untouched.)*

---

## Why it hasn't been built (grounded)

1. **Sector-separation is a derived firewall, not laziness.** `engine-capability-map.md:19` — "No single engine carries more than one or two [DOF]; that gap is the whole point of this map." Enforced by FW1: the A1 cage engine is a *scalar* field (∇×∇V≡0) → irrotational → **mathematically cannot host the winding** (`cavitation_flow.py:12`). 🔴 *(2026-06-24: the A1 cage's LOCALIZATION is now boundary/topological, not bulk self-focus — the bulk self-trap was a Cartesian artifact, RULED OUT. The scalar-firewall claim itself is unaffected; mass = A1 untouched.)*
...
One **shared K4-tetrahedral grid**, extending the winding host. It already carries: (a) the (2,3) winding ansatz (the charge DOF the scalar cage engine structurally can't host); (b) an energy-conserving velocity-Verlet integrator (damping=0, `:940`); (c) the **moving Γ=−1 impedance-boundary cage** as a TIR reflective short via an *exact harmonic LC-reactance Strang-split rotation* (`:953-989`); (d) the **α-separated** topological factor κ̃ = pq/(p+q) = 6/5 (`:99,:127`). 🔴 *(2026-06-24: the Γ=−1 impedance-**boundary** cage (c) is the SURVIVING localization route — boundary/topological STANDS. The Stage-2 falsification rules out only the BULK c_eff(V) self-focusing well, which this charter already declined in favour of the boundary condition.)*

**The key unification:** rendering the cage as a Γ=−1 *boundary condition* (not a bulk c_eff(V) term) both (i) avoids the master-equation detonation-at-the-wall AND (ii) **is the same conservative-rotation form the circulator H_couple needs.** The chiral kernel κ̃=6/5 and the circulator coupling are *one object* — the cage primitive and the coupling are already the same machinery in the host. 🔴 *(2026-06-24: VINDICATED by the Stage-2 falsification — "cage as a Γ=−1 boundary condition, NOT a bulk c_eff(V) term" is precisely the distinction the make-or-break confirmed: the bulk c_eff(V) self-trap is a Cartesian artifact (RULED OUT); the boundary-condition route STANDS.)*
