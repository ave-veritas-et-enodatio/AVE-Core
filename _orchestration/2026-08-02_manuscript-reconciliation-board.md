...
| **Cite-rot fix (#850)** | **PARTLY DISCHARGED.** `strip_target` is unchanged — but it was **bypassed**, not fixed: a separate parser (`iter_line_cites`, `check_line_cites`) now asserts cited lines exist. **11,168 backticked-bare cites are parsed for the first time.** §6's *"~1,800 remain wholly unchecked"* is superseded. Option (1) still not implemented | `docket-entries/2026-08-05-cite-rot-line-existence.md:3,:26`; `verify-md-links.py:726,:652` |
