...
| **C15-CLEAVE-01** (femto-Coulomb electrometer) | [`exp-c15-cleave-01.md`](exp-c15-cleave-01.md) + [Framework-readiness audit](exp-c15-cleave-01-sim-audit.md) + [Phase 0 scaffold brief](exp-c15-cleave-01-phase-0-scaffolding.md) + [Q-C15-01 chamber scoping](exp-c15-cleave-01-q-c15-01-chamber-scoping.md) + [Phase 1 KiCad brief](exp-c15-cleave-01-phase-1-kicad-brief.md) + **sibling repo `AVE-Bench-FemtoElectrometer` LIVE; Phase 1a branch `analysis/phase-1a-kicad-design` at `f743bae` pushed** | Cascade **SIZE** winner (ξ_topo family 6+ rows) — F-severity; **Phase 0 ✓ COMPLETE + Phase 1a ✓ LANDED** (branch pushed; NOT merged): D1.1 reference-design notes + D1.2 finalized BOM ($7670 mid-range) + D1.3 KiCad schematic DRAFT (+ ASCII companion) + D1.7 TEST_PROCEDURE refresh + D1.8 procurement SKU population + D1.9 ORDERING skeleton + D1.10 DESIGN_LOG; 3 new open questions surfaced (Q-C15-07/08/09); KB-leaf prediction verbatim preserved; pure-AVE-corpus zero | Grant pre-merge review on (a) schematic topology, (b) BOM SKUs, (c) Q-C15-07/08/09 adjudication, (d) DESIGN_LOG §3 decision 1.2 (DAC on/off PCBA) → merge `--no-ff` + audit tag → Phase 1b PCB layout (Grant manual) → Phase 1c Gerbers (Grant `kicad-cli`) → Phase 2 fab + assembly |
