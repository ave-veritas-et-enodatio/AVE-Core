...
| **C15-CLEAVE-01** (femto-Coulomb electrometer) | [`exp-c15-cleave-01.md`](exp-c15-cleave-01.md) + [Framework-readiness audit](exp-c15-cleave-01-sim-audit.md) + [Phase 0 scaffold brief](exp-c15-cleave-01-phase-0-scaffolding.md) + [Q-C15-01 chamber scoping](exp-c15-cleave-01-q-c15-01-chamber-scoping.md) + [Phase 1 KiCad brief](exp-c15-cleave-01-phase-1-kicad-brief.md) + **sibling repo `AVE-Bench-FemtoElectrometer` main @ `331a778` (audit tag `audit/2026-05-20_phase-1a-kicad-design`)** | Cascade **SIZE** winner (ξ_topo family 6+ rows) — F-severity; **Phase 1a ✓ MERGED to main** at `331a778`: D1.1 reference-design notes + D1.2 finalized BOM ($7670 mid-range) + D1.3 KiCad schematic DRAFT + ASCII companion + D1.7 TEST_PROCEDURE refresh + D1.8 procurement SKU population + D1.9 ORDERING skeleton + D1.10 DESIGN_LOG + Grant adjudications Q-C15-07/08/09 + Q1.2 all CLOSED (combined Q-C15-08+09 → ALL grounds explicit dedicated paths, no chamber wall in ground network). KB-leaf prediction verbatim preserved; pure-AVE-corpus zero. | Phase 1b Grant manual KiCad GUI: schematic ERC clean + PCB layout + guard-ring polygon + DRC per DESIGN_LOG §5.1-5.2 → Phase 1c Gerbers via `kicad-cli` → Phase 2 fab + assembly |
