...
| **C15-CLEAVE-01** (femto-Coulomb electrometer) | [`exp-c15-cleave-01.md`](exp-c15-cleave-01.md) + [Framework-readiness audit](exp-c15-cleave-01-sim-audit.md) + [Phase 0 scaffold brief](exp-c15-cleave-01-phase-0-scaffolding.md) + [Q-C15-01 chamber scoping](exp-c15-cleave-01-q-c15-01-chamber-scoping.md) + [Phase 1 KiCad brief (predecessor)](exp-c15-cleave-01-phase-1-kicad-brief.md) + [Phase 1a-rev1 atopile walk-back brief](exp-c15-cleave-01-phase-1a-rev1-atopile-walkback-brief.md) + **sibling repos: `AVE-Bench-FemtoElectrometer` main @ `331a778` (Phase 1a) + `AVE-Hardware-Modules` (NEW; Stage A scaffolding)** | Cascade **SIZE** winner (ξ_topo family 6+ rows) — F-severity; **Phase 1a ✓ MERGED** at `331a778` (KiCad-native) → **Phase 1a-rev1 atopile walk-back ACTIVE** (Q-C15-10 Grant adjudicated atopile + new HWMOD sibling repo). Two-stage: Stage A AVE-Hardware-Modules scaffold + 8 modules + tests + remote; Stage B C15 atopile walk-back (`cleave_01.ato` + 8-module composition + decommission KiCad orphans to `_archive/` + Q-C15-10 closure + Q-C15-11 NEW). All prior adjudications honored (Q-C15-01/03/04/05/07/08/09 + Q1.2). | Stage A implementor dispatch → Stage B gated on A return → orchestrator + Grant pre-merge review → merge `--no-ff` + audit tag → Phase 1b/1c KiCad GUI work from `ato build` outputs → Phase 2 fab + assembly |
