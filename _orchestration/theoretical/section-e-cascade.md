...
1. **K4 lattice rest frame** — where the substrate sits at rest. Sun's velocity vector through this frame = CMB dipole at $(l \approx 264°, b \approx 48°)$, $\sim 370$ km/s. Local kinematics. **NOT a fundamental cosmological axis.**
...
Angular separation $|\hat{v}_{\odot \to \text{K4}}, \hat{\Omega}_{\text{freeze}}| \approx 79°$ (minimum, accounting for 180° axis degeneracy).

**Pantheon+ peculiar-velocity correction subtracts $\hat{v}_{\odot \to \text{K4}}$ at the CMB DIPOLE direction.** It does NOT touch $\hat{\Omega}_{\text{freeze}}$. The two are nearly orthogonal observables of the same substrate at different scales (local kinematics vs. cosmic-genesis chirality). Standard pipeline is therefore structurally non-circular for the test "does Hubble bulk-flow direction align with $\hat{\Omega}_{\text{freeze}}$?"
