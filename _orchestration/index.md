...
**Last updated**: 2026-05-20 EOD+++++++++ — **C15-CLEAVE-01 Phase 1a ✓ MERGED to `main` at `331a778`** (audit tag `audit/2026-05-20_phase-1a-kicad-design` at `6d6552f` branch tip; analysis branch deleted local + remote per AVE-HOPF/AVE-Core branching pattern). Grant pre-merge review adjudicated all 4 items: schematic topology OK (KiCad GUI cleanup deferred Phase 1b); BOM $7670 mid-range approved; Q-C15-07 ✓ CLOSED dedicated FT2 feedthrough; Q-C15-08 ✓ CLOSED dedicated PTFE-socket explicit return; Q-C15-09 ✓ CLOSED external-only via FT1 BNC shield; Q1.2 ✓ CLOSED off-PCBA DAC + HV amp (Phase 2.5 consolidation NOT pursued). **Combined Q-C15-08 + Q-C15-09 topology**: ALL grounds (floating-plate signal return + chassis ground) leave the chamber via explicit dedicated paths; no chamber wall in the ground network → cleanest electrical isolation. Phase 1a deliverables D1.1-D1.10 minus D1.4-D1.6 all complete + Grant adjudications captured. **C15 first complete sibling-repo Phase 0 + Phase 1a cycle: scaffold → 9 deliverables → Grant review → adjudication → merge → audit tag → branch cleanup.** Earlier today: Phase 0→1 ✓ PROMOTED + GitHub remote ✓ LIVE; Q-C15-01 ✓ RESOLVED dedicated chamber; Phase 0 ✓ SCAFFOLD LANDED; framework-readiness audit ✓ NO DRIFT; all 3 Experimental-Arc sub-epics audited (A1-HOPF + C11 + C15) — all NO DRIFT. C11-MACH-ZEHNDER Pattern B canonical KB leaf landed (driver live-fire 249.6394 rad). A1-HOPF Phase 0a ✓ COMPLETE + Phase B walk-back. C11 Phase 0 facility partnership search initiating. A1-HOPF Phase 0b ready for Grant fab submission. Phase 1b on C15 pending Grant manual KiCad GUI work (schematic ERC clean + PCB layout + guard-ring + DRC); Phase 1c via `kicad-cli`; Phase 2 fab + assembly (~$7670 full BOM).
...
| ↳ EXP-C15-CLEAVE-01 (cascade SIZE — largest) | [`exp-c15-cleave-01.md`](exp-c15-cleave-01.md) + [Framework-readiness audit](exp-c15-cleave-01-sim-audit.md) + [Phase 0 scaffold brief](exp-c15-cleave-01-phase-0-scaffolding.md) + [Q-C15-01 chamber scoping](exp-c15-cleave-01-q-c15-01-chamber-scoping.md) + [Phase 1 KiCad brief](exp-c15-cleave-01-phase-1-kicad-brief.md) + **sibling repo `AVE-Bench-FemtoElectrometer` main @ `331a778`** | **PHASE 1a ✓ MERGED to main at `331a778`** (audit tag `audit/2026-05-20_phase-1a-kicad-design` at `6d6552f`; analysis branch deleted local + remote). Phase 1a deliverables (D1.1-D1.10 minus D1.4-D1.6) + Grant adjudications all complete: ADA4530-1 reference-design notes + finalized BOM ($7670 mid-range) + KiCad schematic DRAFT + ASCII companion + TEST_PROCEDURE refresh + procurement SKU population + ORDERING skeleton + DESIGN_LOG + Q-C15-07 ✓ dedicated FT2 + Q-C15-08 ✓ dedicated PTFE-socket return + Q-C15-09 ✓ external-only BNC shield ground + Q1.2 ✓ off-PCBA DAC+HVamp. **Combined Q-C15-08+09 topology**: ALL grounds via explicit dedicated paths; no chamber wall in ground network. KB-leaf prediction verbatim preserved; pure-AVE-corpus zero. F-severity (Ax2 dies if 0.0 mV); U-D 41.5 mV/μm; ξ_topo family cascade (B4 + C9 + C16 + B5 + B6 + B7 — 6+ dependents, largest single-row cascade in matrix). | Phase 1a ✓ MERGED 2026-05-20 EOD+++++++++ at `331a778` + audit tag at `6d6552f` | Phase 1b: Grant manual KiCad GUI work (schematic ERC clean per DESIGN_LOG §5.1 + PCB layout per §5.2 + guard-ring polygon + DRC) → Phase 1c: Gerbers via `kicad-cli pcb export gerbers` per §5.3 + ORDERING.md §2 populate → Phase 2 fab + assembly (chamber refurb + PZT + ADA4530-1 + cap + PCB fab; ~$7670 full BOM mid-range) → Phase 3 measurement (ave-prereg + sweep) → Phase 4 outcome adjudication |
