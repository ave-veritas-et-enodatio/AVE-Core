# AVE-Core Orchestration Index  ⛔ **FROZEN 2026-08-13** — superseded by [`BOARD.md`](BOARD.md); full notice at end of file.
...
## 2026-07-20 — Port-register canon promotion + seismology external-anchor toolkit (watch)

**Focus (Grant):** promote the port register (`research/2026-07-20_port-register_draft.md`, merged #753) to a canonical KB leaf; land the A1/T2 ↔ seismological P/S translation row; open one skill-candidate watch. Grant-fired 2026-07-20; canon-promotion gate satisfied by *"lets do it"* `[sic]`.

- Canonical leaf: [`../manuscript/ave-kb/common/port-register.md`](../manuscript/ave-kb/common/port-register.md) — the per-channel × per-port map (**14 rows**: 4 inherent channels + 9 known ports + 1 explicitly-OPEN Q1). **Currency at build (re-verified vs origin/main `1003661c` = #751 merge): 13 rows `[canon]` / 1 `[canon, #750 merged 2026-07-20]` (Q1).** #749 + #751 both MERGED since the draft ⇒ P3/P7 and **P6/P8 flip `[branch]`→`[canon]`** (P8 carries the merged RE-BANK); only Q1 (#750, OPEN) stays branch-tagged. Registered in `common/index.md`; engine-capability-map §1 See-also pointer. Draft preserved (KEEP-BOTH). Docket ENTRY 33.
- Translation row + means-test: `common/translation-tables/translation-circuit.md` §4 (vacuum A1/T2 radiation partition ↔ seismological P/S quadrupole partition; Aki–Richards; `A_ang=2/3`) + §6 #28 (`E_S/E_P≈23.4` external anchor; means-test PASS at value level). Mirror-check: vol2 app-a + vol4 ch1 copies are routing STUBS — skipped.
- **★SKILL-CANDIDATE (watch — posture-B, watch-not-mint, Grant-gated):** **seismology as a standing external-anchor toolkit for substrate elastodynamics.** The elastic-medium sibling discipline supplies non-AVE, textbook validation anchors for the substrate's elastodynamic sector — **mode conversion at Γ walls**; **Rayleigh / boundary waves ↔ boundary-localized modes**; **evanescent head waves**; the **P/S moment-tensor radiation partition** (first confirmed anchor: A1/T2 ↔ P/S, means-test PASS, translation-circuit §6 #28). **No skill minted; watch only** (hard-gated on Grant, per the standing skill-creation gate).

## 2026-07-14 — QED-TRACE charter (Core carries; read QED math as compressed medium data)

**Focus (Grant):** charter the program that reads QED's mathematical machinery (Π(q²), running α, Uehling, propagator, diagram sum) as **compressed data about the vacuum medium**, ontology fence intact (no transverse-only-photon import — the standing no-QED-garbage rule). **Grant rulings 2026-07-14:** (1) **Core carries** — overrides the sibling-repo routing at `research/2026-07-08_electron-halfflux-selection_result.md:152`; (2) **kernel-ON spend GO** — beta-function gate in flight on branch `analysis/qed-trace-beta-gate` (sibling lane; Core carries the program, the gate spends in its own branch); (3) **probe concession = core-session default** (run against the neutral A44 polarization form-factor), Grant veto window open; (4) **g−2 plumbing fork OPEN / HELD** (radiative-port vs on-site dielectric detuning — supersede-vs-degenerate declaration mandatory before any port pre-reg freezes).

- Charter (founding document): [`../research/2026-07-14_qed-trace-charter.md`](../research/2026-07-14_qed-trace-charter.md) — state ledger (5 structures w/ classifications), beta-gate design + frozen bins, g−2 second-target (held on Q4), organizing layer (gauge = charter prose / Ward = dictionary row / anomaly = mint-with-category-flag `parity-selection ≠ anomaly` / series = reframed walk-note), THE FENCE (verbatim no-QED-garbage anchors), string-theory side-walk appendix (walk-class, no claims minted).
- **Honesty rail:** every mapping is consistency-class until it fires; the beta-gate is the ONE chord-class candidate and currently reads as a **category mismatch, not a tuning gap** (every computed scale-object is a power law; one log route already refuted in-corpus; the running-α driver produced the WRONG SIGN). The gate is shaped to test the category question cheaply, not to assume the derivation is live.
- **PENDING-GRANT sign-off:** the reconciliation sentence ("the diagram sum is QED's serialization format for medium response, not a competing ontology") re-scopes `higgs-mass.md:77`'s standing "No Feynman diagrams … required" prose — recorded in the charter, **NOT landed as canon** by it.
- g−2 echo-tag housekeeping (lands regardless of Q4): echo-tags on the a_e = α/2π cites, coverage-matrix g=2 reconciliation, LIVING_REFERENCE/README status sync, electron-2-loop postulate-conditional caveat. Branch `docs/qed-trace-charter-and-g2-tags` · **DO-NOT-MERGE (Grant merges).**
...
**(vii) Hygiene backlog:**
- **Bell-arc citation-hygiene follow-ups** — 3 items from the merged #824/#828 quote-provenance arc: F1 no-gate-binds-`\cite{}`-to-`.bib` (root cause, gate-creation Grant-gated), F2 `historical-precedents.md:30` MM uncited, F3 Abraham 1902/03 citation debt. Full context: [`2026-08-02_bell-arc-followups.md`](2026-08-02_bell-arc-followups.md).
...
- **RANK 3 — the F6 depletion primitive (the real DC→AC chord object) — HARD-BLOCKED.** #86 two-way back-reaction is NOT the make-or-break: it already **LANDED 2026-06-29** (`engine-capability-map.md:139-144`, `clm-w5ez6i`, consistency-class, imports G). The actual DC→AC chord is F6 (irreversible ε→T2 depletion; `dark-energy-latent-heat-definition.md:139-158` §5), which is UNBUILT and HARD-BLOCKED (a bounded norm-preserving depletion primitive that does not detonate + a ρ_latent numeric prerequisite, both ABSENT; the one prior attempt photon_deplete=True DETONATES). A multi-arc build, not a near-term make-or-break. **Update (F6 tier-1, PR #674, §5.4 ruled BOTH 2026-07-13):** the tier-1 *global two-reservoir ODE ledger* executed and landed **bin (i) LEDGER-CONSISTENT** — the DE-tracks-matter FORM *exists* (CONSISTENCY-class, κ free ⇒ not emergence) AND the homogeneous ledger is the **wrong instrument** at late epochs (chord → Λ past τ₀≈300); the a-priori bin-(iii) expectation was FALSIFIED. The **F6 CHORD test remains UNBUILT at tier-1 scope**, relocated to the **spatial-channel/tier-2** (DESI/Euclid DE-vs-matter cross-correlation). Docket `2026-07-10_rulings-docket.md` (two-ruling entry) + `research/2026-07-13_f6-tier1-ledger-driver_result.md`.
...
**The live board is [`BOARD.md`](BOARD.md)**, and it is *generated*.

**Why this one was frozen.** It was declared stale three times and appended to after every one: Benn's 2026-05-23 banner (*"document deprecated"*), then notices stamped 2026-06-16, re-stamped 2026-07-11, addended 2026-07-12. Not a discipline failure — a **shape** failure: nothing here is derived, so nothing here can be *wrong-and-caught*, only *stale-and-believed*.

**What moved where.** Open decisions → [`open-items/`](open-items/) (one file per item, read by the generator). Rulings → [`docket-entries/`](docket-entries/). Claim state → `manuscript/ave-kb/.index/claims.jsonl`. The board is the single *view* over all three.

**★ Why this notice is at the BOTTOM.** Eleven documents cite this file by line number, one of them a FROZEN prereg that cannot be repaired at the citer (`research/2026-08-05_srs-twist-coefficient_prereg-FROZEN.md`). A header would have shifted every one of them. Nothing above this line moved; the H1 carries the marker instead.

**Nothing above is deleted or rewritten** (Rule 12). It remains the session-focus record for 2026-05 → 2026-08-02. **Do not append to it.**
