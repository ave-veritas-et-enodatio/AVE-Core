...
| ↳ EXP-A1-HOPF (cascade × executability) | [`exp-a1-hopf.md`](exp-a1-hopf.md) + [Phase A audit](exp-a1-hopf-repo-audit.md) | **PHASE 0a — Artifact generation pending (3 BLOCKERS per repo audit).** Hardware DESIGN-COMPLETE ($123 BOM verified at AVE-HOPF/hardware/hopf_02_ASSEMBLY_GUIDE.md:21-32; **top-level BOM.md is HOPF-01 $142**); 89 fast tests passing verified. **3 BLOCKERS**: (1) HOPF-02a Gerbers not exported (5 min); (2) HOPF-02a ORDERING.md not drafted (15 min); (3) ave-prereg pre-reg for VNA measurement not drafted (~1 hr, Phase 2 gate). Audit also flagged 4 misdirected citations + 18 IP-pointer-opacity violations across AVE-Core (corpus-wide). | Phase A audit landed 2026-05-20 (425 lines, 23 ✓ / 11 ⚠ / 3 🔴) | Phase 0a artifact generation (Grant inline OR Phase B implementor on `analysis/a1-hopf-audit-walkback-2026-05-20` off `research/hopf-01-testing` per AVE-HOPF AGENTS.md §3) → Phase 0b fab submission |
| ↳ EXP-C15-CLEAVE-01 (cascade SIZE — largest) | [`exp-c15-cleave-01.md`](exp-c15-cleave-01.md) | PHASE 0 — Scoping decision pending. PCBA spec in KB only; needs KiCad design + ~$1-5k bench. F-severity (Ax2 dies if 0.0 mV per KB explicit); U-D 41.5 mV/μm; ξ_topo family cascade (B4 + C9 + C16 + B5 + B6 + B7 — 6+ dependents) | Sub-epic doc established 2026-05-20 | Grant scoping decision → KiCad design from KB spec → fab → bench |
| ↳ EXP-C11-MACH-ZEHNDER (cascade × severity F) | [`exp-c11-mach-zehnder.md`](exp-c11-mach-zehnder.md) | PHASE 0 — Facility partnership search pending. Driver canonical (249.64 rad live-fire vs ~250 rad prediction at 1-m baseline + 100 eV); no terrestrial facility partner. F-severity (Ax3 + Ax1 die); ν_vac=2/7 cascade triangulation (C1 done + C11 + C12 LISA-wait) | Sub-epic doc established 2026-05-20 | Literature survey → facility candidate list (Hasselbach Tübingen / LENS / NIST / TEM holography) → cold-email outreach |
