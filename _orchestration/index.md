...
- Canonical leaf: [`../manuscript/ave-kb/common/port-register.md`](../manuscript/ave-kb/common/port-register.md) — the per-channel × per-port map (**14 rows**: 4 inherent channels + 9 known ports + 1 explicitly-OPEN Q1). **Currency at build (re-verified vs origin/main `1003661c` = #751 merge): 13 rows `[canon]` / 1 `[branch:#750]` (Q1).** #749 + #751 both MERGED since the draft ⇒ P3/P7 and **P6/P8 flip `[branch]`→`[canon]`** (P8 carries the merged RE-BANK); only Q1 (#750, OPEN) stays branch-tagged. Registered in `common/index.md`; engine-capability-map §1 See-also pointer. Draft preserved (KEEP-BOTH). Docket ENTRY 31.
