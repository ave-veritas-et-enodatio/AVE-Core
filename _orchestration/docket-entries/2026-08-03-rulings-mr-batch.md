...
> | 2 | neon print/mirror card mismatch + KB vocabulary in print | `12_neon.tex:65` + `topological-area.md:18` sync note | canon fix |
...
- **`make verify-md-links` gating 0** — `gating errors: 0  warn-only: 207  broken inter: 42` (inter-repo mode: warn) — **all three numbers identical to `origin/main` and to the pre-repair tip**, so the repair introduces no new warn either. **`kbleaf` cites: 1213 → 1214, gating: 0, waived: 1** — the one cite the repair adds (`\kbleaf{ave-kb/common/vocabulary-register.md}` in the electroweak body, step 3′) **resolves**.
