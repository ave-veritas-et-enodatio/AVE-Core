### ENTRY 2026-08-23-theta-fork-ruling (2026-08-23): Grant — the two-θ adjudication: rulings (a)+(b) executed after challenge audit; dressing-investigation program GO

**The question** (surfaced by the Feynman-formalism mapping exercise): `strong-cp.md` asserts
the vacuum angle θ = 0 exactly while `topological-fractionalization.md` runs quark fractional
charge on discrete θ ∈ {0, ±2π/3, ±4π/3} "trapped vacuum" phases — same symbol, no
cross-reference, no register reconciliation.

**RULINGS (Grant, 2026-08-23, verbatim):**
1. *"let's do a and b, but explain them fully, and challenge them against the kb/claims"*
2. (on the two open debts) *"And for the N=3, CP-parity opens, any potential things from the
   vacuum that need to be true and would be worth investigating?"*
...
2. 3-body-only binding: pairwise Link is charge, so neutral multi-loop binding must ride the
   ...
   4-body?
3. Balanced-3-phase reading: the thirds as polyphase angles of a rotating internal field on
   a 3-port junction — would unify "why N=3", "why thirds", and "what the dressing IS".
4. CP-parity via chirality coupling: does the dressing rotation ride the chiral torsional
   dispersion (CP-violating at γ_c order; real EDM confrontation; candidate substrate origin
   of hadronic CP) or the achiral branch (CP-preserving, structural EDM null)? Test = RH-vs-LH
   energy split of a rotating balanced-3-phase boundary pattern on the chiral srs cell.

**Weld hazards recorded with the candidates:** "N=3 because z=3" is the numeral-homonym class
the corpus polices (only a derived embedding obstruction counts); the Δc=3 screening
threshold's numeral is a flagged coincidence-tell, not support.
