...
- [x] **OQ1 + OQ2 corpus-resolved** (2026-05-31): R, r as major/minor axes of single elliptical time-averaged envelope (l3-soliton-synthesis.md:41); regime (b) Ax 2 topology-prior to Ax 4 saturation (ch8:45)
- [x] **§4.B Phase 1 (analytical derivation) — CLASS B SUBSTRATE-MECHANISM CLOSURE** (2026-05-31, commit `66d63503`). Result doc: `research/2026-05-31_Q-EMBED-SEL-1_step_c_result.md` §1–§9. Substrate-mechanism: Axiom 4 self-saturation + Op14 Meissner-asymmetric + (2,3) chirality $\chi_{(2,3)} = 1.2\alpha$ via bipartite K4 lobe-count + named identification step (phasor enclosed area at Ax-4 saturation onset = Nyquist cell cross-section area). Three falsifiers pass at machine precision (algebraic Golden Torus; $\Lambda_i$ sum to $4\pi^3+\pi^2+\pi$; CODATA $\delta_{\text{strain}}$ unchanged). Retires the QED-imported spinor half-cover argument (doc 29 F5, doc 39 §3.4). The $4\pi$ factor in $\Lambda_{\text{vol}}$ is now substrate-derived from bipartite K4 lobe-count, NOT from SU(2)→SO(3) double cover.
- [x] **§4.B Phase 2 (cross-particle) — OUTCOME A: UNIVERSAL PASS** (2026-05-31, commits `fc3f7000` + `b509767a`). Result doc: `research/2026-05-31_Q-EMBED-SEL-1_step_c_phase2_cross_particle_result.md`. **Same Phase 1 substrate-mechanism chain closes UNIVERSALLY at the per-bond LC tank scale for electron (2,3), proton (2,5), Δ baryon (2,7).** Per-bond Golden Torus $(R, r, d) = (\varphi/2, (\varphi-1)/2, 1)$ derives identically across the (2,q) ladder. Particle-specific content (chirality coupling magnitude, loop count, confinement radius $r_{\text{opt}} = \kappa_{\text{FS}}/q$, collective mass via Faddeev-Skyrme eigenvalue) lives only in COLLECTIVE multi-bond structure, NOT in per-bond geometry. Proton mass predicted 938.254 MeV vs 938.272 CODATA (-0.002%); Δ baryon 1261 vs 1232 PDG (+2.354%). Confirms `trampoline-framework.md:31` universality claim at (2,q) ladder level.
- [x] **§4.B Phase 3 (cross-domain cold-fusion) — OUTCOME A: UNIVERSAL CROSS-DOMAIN PASS** (2026-05-31, commits `d1db1cd8` + `ecfe9c13`). Result doc: `research/2026-05-31_Q-EMBED-SEL-1_step_c_phase3_cold_fusion_result.md`. **5/5 multi-falsifier triangulation PASS** via canonical $n_{\text{scalar}} = 1/S(A_0)$ substrate identification (same as ponderomotive equivalence $n = 1 + GM/c^2 r$ at gravitational scale; same Op14 dynamic-impedance form at all scales). Pd metallurgical scaling $\Delta V/V_0 \approx 0.13 x$ → $A_0(x) = 0.13x/\sqrt{2\alpha}$; survival window $x \in [0.90, 0.929]$ (2.9% sliver before shatter at $A_0 = 1$). NASA Glenn ($n \geq 2.5$ at $x \in [0.85, 0.92]$) + FP stochastic-irreproducibility ($n \geq 200$ at $x \approx 0.92929$, 1 part in $10^4$ of shatter — quantitatively explains FP irreproducibility) + shatter-limit cross-scale + Phase 1+2 baseline + Schwinger cross-scale ALL PASS. Calibrated strength language: NASA Glenn solid peer-reviewed; FP framing preserved as corpus's existing stochastic-irreproducibility-at-2.9%-sliver per `universal-saturation-kernel-catalog.md` row 6.
- [x] **§4.B Phase 4 (corpus walk-back) — COMPLETE** (2026-05-31, PR #60 merged via `06161da6`; audit tag `audit/2026-05-31_q-embed-sel-1-phase-4-corpus-walkback` → `34a163f2`). Two commits: main sweep `a581d9f4` (33 files, +305/-218; orchestration-committed after implementor `a6aef025` hung in gap-inventory phase post-edit; verify-* PASS) + fixup `34a163f2` (13 files; 9 Class D + 2 Class B from sweep-audit `ac6c72c5` findings; verify-* + pytest 16/16 PASS). Total scope: 46 files swept (KB layer 21 + LaTeX layer 8 + engine/tests 6 + top-level docs 3 + .index/ auto-regen 2). Spinor-half-cover provenance retired across corpus; new substrate-mechanism provenance (Ax 4 self-saturation + Op14 Meissner-asymmetric + named phasor-area-equals-Nyquist-cell-area identification) propagated. K4 → A4 → 2T ⊂ SU(2) → SO(3) chain + FM kink spin-½ mechanism PRESERVED canonical for spin-½ structure (only the downstream half-cover surface-area halving is retired). The $4\pi$ factor in $\Lambda_{\text{vol}}$ now substrate-derived from bipartite K4 lobe-count.
- [ ] **Class-2 lift candidate workstream**: derive the phasor↔real-space area bijection at the bond LC tank from K4 + Cosserat substrate primitives alone (per result doc §7.3). Would lift Class B to Class 2. Out of scope for this epic; queued as future workstream.
- [x] **Walkback §3.3 framing resolution — COMPLETE** (Phase 4 corpus walk-back propagated 2026-05-31). Framework now anchored in **Class B substrate-mechanism manifestation, cross-particle universal + cross-domain validated + corpus-walkback propagated**. NOT calibration-input per doc 39; NOT "gated on unproven embedding-selection" per the now-retired 2026-05-28 Phase 1+2 gating clause.
- [ ] **AVE-HOPF cross-repo reconciliation** — UN-GATED by Phase 4 completion. Sibling repo `AVE-HOPF/docs/glossary.md:32` Grant 2026-04-30 bracketing of Golden Torus as "post-IP-separation patch-attempt" can now be reconciled (bracketing → un-bracketed-with-Class-B-substrate-mechanism-citation pointing at the Phase 1+2+3 result docs on AVE-Core main).

---

## Q-EMBED-SEL-1 EPIC CLOSED (2026-05-31 EOD)

All four phases of the Q-EMBED-SEL-1 evaluation epic closed at Outcome A:
- **Phase 1** (`66d63503`, PR #59): Class B substrate-mechanism closure for $\sqrt{R \cdot r} = d/2$ via Ax 4 self-saturation + Op14 + named phasor-area-equals-Nyquist-cell-area identification.
- **Phase 2** (`b509767a`, PR #59): cross-particle universal PASS across electron (2,3) + proton (2,5) + Δ baryon (2,7).
- **Phase 3** (`ecfe9c13`, PR #59): cross-domain 5/5 multi-falsifier PASS via $n_{\text{scalar}} = 1/S(A_0)$ identification (same form as ponderomotive equivalence; cross-scale consistency).
- **Phase 4** (`06161da6` merge of PR #60): corpus walk-back propagation; spinor-half-cover provenance retired across 46 files; new substrate-mechanism cited canonical.

Audit tags preserved:
- `audit/2026-05-31_q-embed-sel-1-substrate-mechanism` → `118a33a3` (Phase 1+2+3)
- `audit/2026-05-31_q-embed-sel-1-phase-4-corpus-walkback` → `34a163f2` (Phase 4)

Framework position upgrade: from **"zero-parameter, gated on unproven Clifford-torus embedding-selection step"** (Phase 1+2 gating clause, retired) to **"Class B substrate-mechanism manifestation, cross-scale validated (electron + (2,q) hadronic ladder + Pd-D cold fusion + ponderomotive-equivalence-consistent)"** with explicit Class-2 lift candidate workstream identified.

Operational discipline observations:
- Phase 4 main implementor hung in gap-inventory phase after high-quality in-scope work. Orchestration TaskStop + manual verify + commit recovered 33-file WIP cleanly; sweep-audit caught 9+2 gap-inventory misses for fixup; fixup commit + verify + tests cleared the corpus.
- Routing per memory v2: PR-routed merges throughout (PR #59 + PR #60). No direct main pushes.
- Audit-tag preservation: 2 tags landed, both on origin, before branch deletion.
