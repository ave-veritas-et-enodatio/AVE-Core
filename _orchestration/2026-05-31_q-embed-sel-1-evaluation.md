...
- [ ] **§4.B Phase 3 (cross-domain cold-fusion)** — apply same Axiom 4 self-saturation mechanism to externally-driven Pd-D lattice saturation. Predict Coulomb-barrier reduction factor matching NASA Glenn lattice-confinement (~keV, ~10² reduction). Per result doc §7.2.
- [ ] **§4.B Phase 4 (corpus walk-back)** — once Phase 2+3 validate, propagate the substrate-mechanism replacement across corpus (ch8 step 4 + theorem-3-1-q-factor.md + related leaves), retiring the spinor-half-cover provenance in favor of the Axiom 4 self-saturation provenance. Gated on Phase 2+3 PASS.
