...
- [ ] **§4.B Phase 4 (corpus walk-back) — UN-GATED by Phase 3 PASS**. Propagate the substrate-mechanism replacement across corpus (ch8 step 4 + theorem-3-1-q-factor.md + related leaves), retiring the spinor-half-cover provenance in favor of the Axiom 4 self-saturation provenance per the now-validated Phase 1+2+3 substrate-mechanism. Step 3h-exhaustive corpus sweep + ave-sweep-audit before merge.
- [ ] **Class-2 lift candidate workstream**: derive the phasor↔real-space area bijection at the bond LC tank from K4 + Cosserat substrate primitives alone (per result doc §7.3). Would lift Class B to Class 2.
- [ ] **Walkback §3.3 framing resolution**: framework now anchored in **Class B substrate-mechanism manifestation** (NOT calibration-input per doc 39; NOT "gated on unproven embedding-selection" per Phase 1+2 gating clause). The 2026-05-28 gating clause is superseded; walk-back of corpus to reflect this is Phase 4 above.
