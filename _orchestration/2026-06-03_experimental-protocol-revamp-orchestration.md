...
**Current board (tiered, post-hardening):** LIVE = HOPF (chiral legs) · **Cleave (two-sided — occupation-gap CLOSED, walk-back merged `d2b37f53`)** · IVIM (discrimination sound, magnitude held) · **Q-G42 V²-sign kernel test (decision-ready)**. RETIRED = Sagnac/ROENTGEN-03 (landed) · Casimir (queued). GATED = Protein · Neurology. DONE anchors = SPARC · BH-ringdown. PARK = Schwinger · metric-fusion · baryon-ladder. DEMOTED = muon-g2 · Lamb · PVLAS-static-B · RT-SC · HTS-KI · PONDER-01 · acoustic-rect.

**Pending Grant decisions (parked behind the kernel thread):** IVIM **R-A/R-B** · HOPF **§6.2** doc-correction. ~~Cleave gap-dielectric~~ → **RESOLVED + merged `d2b37f53`**. ~~Casimir retire + cold-fusion split~~ → **AVE-Core §0.5 changelog LANDED** (on origin/main; cross-repo leaf edits remain queued as chips: QED + Metamaterials Casimir, Fusion Pd/D).
