...
- [x] **Corpus walk-back propagation (evaluation epic Phase 4) — COMPLETE** (2026-05-31, PR #60 merged via `06161da6`; audit tag `audit/2026-05-31_q-embed-sel-1-phase-4-corpus-walkback` → `34a163f2`). 46-file sweep across KB (21) + LaTeX (8) + engine/tests (6) + top-level docs (3) + .index (2). Spinor-half-cover provenance retired corpus-wide; Q-EMBED-SEL-1 substrate-mechanism provenance stamped canonical. K4 → A4 → 2T ⊂ SU(2) → SO(3) chain + FM kink spin-½ mechanism preserved canonical (still load-bearing for spin-½ structure).
- [ ] **AVE-HOPF cross-repo reconciliation** — UN-GATED. Reconcile `AVE-HOPF/docs/glossary.md:32` Grant 2026-04-30 bracketing of Golden Torus with the now-validated Class B substrate-mechanism on AVE-Core main. Cross-repo PR; out of scope for the param-count walkback epic.

---

## Phase 3 (presentation-framing): lead-with-the-reduction emphasis shift

**Opened**: 2026-06-02 (Grant approval). **Branch**: `analysis/zero-parameter-headline-reframe` (off `main` @ `a55eea14`). **Worktree**: `../AVE-Core-zp-wt` (isolated). **Status**: implementor branch pushed, NOT merged.

**Distinction from §3 (framing-choice adjudication).** §3.x resolved *which* parameter-count framing is canonical (Class B substrate-mechanism manifestation, Q-EMBED-SEL-1). This Phase 3 is **presentation-framing only**: at the high-visibility FRAMEWORK-LEVEL "zero-parameter" HEADLINE sites, LEAD with the dramatic-but-defensible reduction (~26 SM parameters → ~3 interlocked geometric inputs $\{\ell_{node}/m_e, \alpha, G\}$ + named residuals) and DEMOTE "structurally zero-parameter" to the stated TARGET/aspiration, citing the existing parameter-ledger scorecard. **This is an EMPHASIS shift (lead-with-the-reduction), NOT a demotion of the aspiration** — it respects Grant's 2026-05-28 "Option 2: zero-parameter, gated" call; the aspiration is preserved, the lead is re-ordered. No new accounting leaf created; cites the existing ledger (`backmatter/02_full_derivation_chain.tex` Scorecard ~:1007–1014 / `common/full-derivation-chain.md` Scorecard :835).

**Sites reframed (7 of the ~8 candidate framework-level headline sites):**
1. `manuscript/frontmatter/00_foreword.tex` (~:25) — headline now leads "reduces the SM's ~26 independent empirical parameters to ~3 interlocked geometric inputs"; Zero-Parameter Scale-Invariant Topology demoted to stated target (Class B hedge + $\Omega_{\text{freeze}}$ three-window body preserved verbatim).
2. `manuscript/backmatter/01_appendices.tex:36` section title → "Parameter Accounting: From ~26 SM Parameters to 3 Interlocked Inputs" (body aspiration unchanged); mirror `common/appendices-overview.md:26` title + bald body-tail ("closed Zero-Parameter framework" → gated target).
3. `backmatter/05_universal_solver_toolchain.tex:383` + `vol2/appendices/app-f-solver-toolchain/universal-constants-exchange.md:21` + `common/solver-toolchain.md:334` — three "AVE framework has zero free parameters" bald assertions → lead-with-reduction; tex Class-B hedge preserved; phenomenon-level LIGO-QNM "with zero free parameters" lines (tex:101 / md:116) LEFT ALONE.
4. `vol1/.../ch1-fundamental-axioms/zero-parameter-universe.md` H1 title + lead (:8, :12) — title leads with 26→3; lead leads with reduction then frames emergence as target; hedged Pathways body (:54) LEFT ALONE.

**Site NOT reframed (flagged for Grant):**
5. `vol_1_foundations/chapters/08_alpha_golden_torus.tex:37` + `vol1/ch8-alpha-golden-torus.md:8` — chapter title "Zero-Parameter Closure: $\alpha$ from the Golden Torus". **Left unchanged + flagged**: this is a PHENOMENON-level / per-derivation provenance claim ("derive $\alpha$ with zero free *numerical inputs*" per objectivebox :44), NOT a framework-level 26→3 headline. Reframing it to "From ~26 SM Parameters to 3 Inputs" would misdescribe the chapter (its subject is the single constant $\alpha$, not the 26-parameter SM table). Per the guardrail protecting per-result provenance lines. **Open for Grant adjudication** if the per-derivation "zero numerical inputs" title is nonetheless considered a high-visibility headline warranting a softer reframe (e.g. "Closed-Form $\alpha$ from the Golden Torus").

**Guardrails honored**: existing ledger cited (no new accounting leaf); 378 phenomenon-level "this result used zero fit parameters" lines untouched; already-hedged BODY text (scorecard "Honest framing", zero-parameter-universe:54, ch8 Mathematical Closure) untouched; aspiration preserved; pure-physics; verify-before-cite (every site read before edit). **Verify**: `verify-kb-metadata` PASS (719 files); `verify-md-links` EXIT=0, zero new link issues (183 pre-existing warnings unchanged). `make verify` deliberately NOT used (verify_universe.py:12 StopIteration in worktrees where `.git` is a gitlink file).
