...
1. **Holonomy = π — VERDICT (II)** (`a7467a52`, DONE; PR pending) — chirality is **NECESSARY** (0/400 achiral vs 127/400 chiral; π exact `Z₂`, step-robust) but **PATH-DEPENDENT** (encircling-conditional Berry phase, not a topological spin-½-for-every-orbit). CONSISTENCY-class, not the strong emergence. Real-space path ≠ `(2,3)` (winding ~2,1.3) — non-contradicting, the canonical `(2,3)` is **phase-space** not real-space. Fills the corpus gap `k4-rotation-group.md:119-123` + `finkelstein-misner §8` (open discrete-lattice spin-½). **Does NOT touch the canonical FM/gyroscopic spin-½** (real-space rotation of the extended unknot — different mechanism, untouched).
   - **DECISIVE NEXT TEST (Grant's call — Rule-16 flagged):** the test used a reflection-**EVEN** anisotropy proxy on an **achiral single host** — NOT the true reflection-**ODD** `I4₁32` screw, which lives in the **2nd shell (A/B sublattice)**. So it can't decide robust-topological-π (I) vs path-dependent (II). **Recommended: holonomy on a 2nd-shell A/B screw geometry** — the real chirality. THIS is the (I)-vs-(II) discriminator.
   - **1b. g = 2 — GATED on a robust-π (I)** from the screw test; premature while π is path-dependent. The SAME π double-cover gives half-spin (`S=ℏ/2`) AND double-moment (`g=2`): `μ_s = 2·μ_B·½ = μ_B` — two numbers from one geometry. **Corrects "half-pole-pair":** the electron is a **dipole with a double-cover**, NOT a monopole (a literal half-pole-pair is a forbidden monopole). Fix coherence-doc §5.
