...
| R7 | **Thermal / noise floor** — the entropy sink | sink the thermometer/DE need; peer-label FD/Nyquist only after CHANNEL-BOUNDED | **REFUSED** as-built ✅ (#707) · rung-1 **CHANNEL-BOUNDED** ✅ · rung-2 **BIAS-MOVED** ✅ · Arm A event-gated **BIAS-MOVED** ✅ (bias knife kills; ΔN_occ detector **VOID** — bookkeeping-only, #711 review) | BENCH-∅ | next hypothesis under mode-count discriminator (not Re(Z); not retune Arm A); **detector-rebuild GATE** owed before any CHANNEL-BOUNDED (see §7); thermometer **GATED** |
...
1. **F6 ε→T2 irreversible *field* channel** — Priority 1. Rung-1 ✅; rung-2 BIAS-MOVED; Arm A event-gated **BIAS-MOVED** (`2026-07-16_f6-mode-count-event-gated_prereg_FROZEN.md` — bias knife kills; ΔN_occ detector VOID / bookkeeping-only per #711 review, §7). Next hypothesis under [`../research/2026-07-15_f6-mode-count-door_CHARTER.md`](../research/2026-07-15_f6-mode-count-door_CHARTER.md); **forbid** Re(Z) absorb / Arm A retune.
...
Further echo-classification and consistency polishing (the API is verified); the Re/Im carve (software-language question — parked); standalone r_knee-consumer work (folded into R11's gap design); additional cold-linear census work (scope theorem closed it); walk-level unification prose without a rating attached; **in-Hamiltonian global V scale-down as F6 door** (rung-2 BIAS-MOVED); **event-gated local V scale-down + multi-mode credit** (Arm A BIAS-MOVED — bias knife kills; ΔN_occ detector VOID / bookkeeping-only, #711 review §7); **ℏ = lattice FD constant as workstream** (FORM not derived; unbanked); **pre-naming matched-termination Re(Z) absorb as the next F6 door** (Ax3 retirement knife — Grant 2026-07-15).
