...
**Opened:** 2026-08-16
**Gated on:** Phase C Type D banners + Audit 1 / Checkpoint 2.
...
2. **When Phase C banners a C-row (or a new KB live-wrong site).** Flip that follow-up to **PENDING-READ**. If Phase C adds a deprecation that is not already a row, **append a new F-row** (do not fold it into an existing corpse). This is what "running" means.
...
Seeded from `_orchestration/2026-08-14_electron-identity-kill-list-ledger.md` §2. Status **QUEUED** until the matching Phase C banner exists.
...
|---|---:|---|
| QUEUED | 10 | F-C1 … F-C10 (seed; ten `### F-C` headings, 2026-08-16) |
| PENDING-READ | 0 | — |
| DONE | 0 | — |

Seed count 2026-08-16: **10 QUEUED / 0 PENDING-READ / 0 DONE**. Re-count headings before changing this table.
