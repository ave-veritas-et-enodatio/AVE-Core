...
- **[F6-arc, ch17 §Conservation Canaries] Epistemic-status sharpening, NOT a contradiction.** The F6 §8c.2 finding (standalone-K4 conservation = algebraic identity) SHARPENS ch17's H-drift canary claim ("total H is constant to integrator order") rather than contradicting it: the canary genuinely passes, but ON STANDALONE-K4 it passes by construction, not as a demonstrated lock. The additive note records this + the counting-arrow-OPEN + thermal-floor tri-form as test status. Placed in the EXISTING §Conservation Canaries section (requirement-4 territory), additive-only. The cleanup-ledger tagged ch17 "checked-consistent" — that assessment stands (no contradiction to fix); the vol-9 order authorizes this ADDITIVE currency note on top.
- **★Citation-provenance deviation (flag).** The #727 INSTRUMENT-INCOMPATIBLE source doc (`research/2026-07-19_f6-no-discharge-scan_INSTRUMENT-INCOMPATIBLE.md`) is on the UNMERGED branch `feat/f6-no-discharge-collapse` (engine-capability-map §8c.6 itself marks it "not yet on main"). The ch17 note therefore cites the INSTRUMENT-INCOMPATIBLE finding VIA the merged `engine-capability-map §8c.6`, never the unmerged doc directly. Certificate bands + counting-arrow-OPEN likewise cited via merged §8c; the thermal-floor tri-form via the merged f6-thermal-floor result doc. Every tex citation resolves on merged main.
</content>
</invoke>
