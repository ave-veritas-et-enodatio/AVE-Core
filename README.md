...
**Pre-registration (OpenTimestamps).** The prediction is priority-anchored by an SHA-pinned OpenTimestamps mechanism (`claim-prereg-ots/stamp-claim.sh`): the frozen prediction is appended under the manuscript's commit SHA in `claims_by_hash.md`, the index is stamped, and the receipt is anchored to the Bitcoin blockchain. The birefringence Letter is registered under commit `f34e7559` (PR #499), with the full frozen prediction — scenario flip-probabilities, the `7.5π/α²` ratio, the kill criterion, and the three signatures (tree-level coefficient / static-B transparency / sidereal modulation) — recorded before any pump-on polarimetric data exists. *Receipt status at time of writing: **stamped and pending** — committed against the calendar servers, Bitcoin-block confirmation not yet finalized. The mechanism, the `claims_by_hash.md` ledger, and the pending `.ots` receipt live on branch `ots/stamp-birefringence-letter`; that branch and the confirmed-anchor commit are not yet merged to `main`, so the mechanism files are not present at this README's HEAD.*
