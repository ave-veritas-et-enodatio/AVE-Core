...
def scan_spans(text: str) -> list[str]:
    ...
    This was a single `TOKEN.findall(text)` over the WHOLE document.  The token
    class ``[^`]+`` does NOT exclude newlines, so ONE line carrying an ODD
    back-tick count flips the open/close phase for the ENTIRE REST OF THE DOCUMENT: every numeral below it
...
def main(_text_override: str | None = None, _scanner=None) -> int:
    ...
    text = open(DOC).read() if _text_override is None else _text_override
