...
def main():
    ...
    # THE verdict-bearing seam: f_chain = 7/(4π), profile/chain = 4π exactly
    checks.append(("f_chain == 7/(4*pi)",
                   abs(c3["f_chain_SI_float"] - 7.0 / (4.0 * math.pi)) < 1e-12
                   and c3["f_chain_SI_exact"] == "7/(4*pi)"))
    checks.append(("profile/chain ratio == 4*pi",
    ...
    checks.append(("C3c: zero canon sites declare T00 = 4*pi*density",
                   r["C3_convention_sweep"]["hit_counts"][r"T_\{?00\}?.*=.*4\s?[πp]i"] == 0))
