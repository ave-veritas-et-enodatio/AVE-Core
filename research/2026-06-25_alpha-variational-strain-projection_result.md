...
**Key v2 finding:** B1 **≠** B2 (`|Δu| ≈ 0.026`). Unconstrained maximization of local strain-projection amplitude selects **denser** packing (`p ≈ 0.19`, `K/G ≈ 1.83`) — **not** the trace-reversal lock. Gravity-stable projection is a **constraint** (K/G=2), not the unconstrained optimum of `Π·γ`.
...
Maximizing `s_grav×admissibility` (B1) lands at `K/G ≈ 1.83`, `p ≈ 0.19` — 5% off CODATA. The trace-reversal point has **higher raw** `s_grav` (0.00897 vs 0.00867) but sits at a different `(u,p)` because Π and γ trade off differently vs `K/G`.
