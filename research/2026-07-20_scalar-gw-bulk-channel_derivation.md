...
> **🔵 CURRENCY HEADER (2026-07-20, post-#753-merge — Rule 12: the body below is preserved as authored PRE-#753; read it through this header).** The merged hardening (#753, `research/2026-07-20_q1-pulsar-hardening.md`, docket ENTRY 29) SUPERSEDES four items below: (i) the O(1) coupling estimate → pinned to the textbook-anchored angular partition `A_ang = 2/3` (net prefactor ∈ [0.3, 1.0] — §4.2's "pushes the coupling UP" is wrong-signed as it turned out); (ii) the amplitude floor ~0.2 → hardened flux bracket `F_bulk/F_shear ∈ [0.015, 0.177]` (amplitude [0.12, 0.42]); (iii) "CONSTRAINED, leaning EXCLUDED / not a clean single-event kill" → **Reading A EXCLUDED at 9–110σ (Hulse-Taylor) and 100–1400× the double-pulsar bound** — pulsar timing bypasses the chirp-mass degeneracy §5.2 leaned on; (iv) §8 item 3 (exact-prefactor driver) → EXECUTED by #753 (`q1_pulsar_timing_budget.py`). The Q1 ruling remains Grant's.  <!-- rule12-freeze: base=58f8b65824355078f8839fb70db8b2ec89039a70 region=below offset=0 lines=127 bytes=19431 sha256=926465e27bd8e3cff03b80d61f0fb2b5d2fa44dbb25e418f0b8a69eface2f631 -->
...
> **✅ SECOND-SOURCE VERIFICATION (2026-07-20, riders lane — Abbott figure re-verification; two-method; the single-source WebFetch stamp above is PRESERVED, Rule-12).** The imported figures were re-verified against the **primary source via an independent retrieval channel** distinct from the original WebFetch: the published arXiv PDF (`https://arxiv.org/pdf/1811.00364`) was downloaded and text-extracted locally with `pdftotext`. The paper body §*"Constraints on the Polarization of Gravitational Waves"* reads **verbatim**: *"If the sky location of GW170817 is constrained to NGC 4993, we find overwhelming evidence in favor of pure tensor polarization modes in comparison to pure vector and pure scalar modes with a (base ten) logarithm of the Bayes factor of +20.81 ± 0.08 and +23.09 ± 0.08 respectively. … Indeed if the sky location is unconstrained we find evidence against scalar modes with +5.84 ± 0.09, while the test is inconclusive for vector modes with +0.72 ± 0.09."* **MATCH to every digit** — vector `+20.81 ± 0.08`, scalar `+23.09 ± 0.08` (EM-sky-fixed); sky-unconstrained scalar `+5.84 ± 0.09`, vector inconclusive `+0.72 ± 0.09`; and the *"pure vector and pure scalar … respectively"* ordering independently confirms this doc's vector↔`+20.81` / scalar↔`+23.09` assignment.  <!-- rule12-freeze: base=3a07522986cd6ada1712e61794f357df41e01b3c region=above offset=0 lines=5 bytes=1093 sha256=22f66012cc787463b0779235f69a56c043c1fb5e8fcb28f5c8389c4c4e0c63ed -->
> **Second method (within-paper multi-location):** the *qualitative* import (pure-scalar/vector strongly disfavoured; tensor agrees with GR) is independently stated at two further distinct locations — the **abstract** (*"the polarization content of the gravitational wave signal is studied. The results of all tests performed here show good agreement with GR"*) and the **Conclusions** (*"…and the polarization content of gravitational waves. In all cases we find agreement with the predictions of GR"*).
> **Disclosed gap (honest):** an independent secondary *document* re-quoting the exact numbers was **not surfaced** — the TGR reviews checked (e.g. Krishnendu & Ohme 2021, `arXiv:2201.05418`) cite Abbott 2019 for the polarization result qualitatively but do not restate the Bayes factors, so the exact-number leg rests on **two independent retrieval channels against the one primary source** (WebFetch-HTML + direct-PDF/`pdftotext`), not a distinct secondary numeric restatement. **Verdict: MATCH** — figures byte-identical to source; single import site (this doc, §5.1 here + §7.3 table `:204` + §8 item-4 `:223`), all tracing to this note.
