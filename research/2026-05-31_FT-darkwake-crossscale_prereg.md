...
> **Resolution (vocabulary, post-test):** this test returned **Outcome C** (different objects, shared name). The two phenomena have since been split under a shared genus: the **g-2 sense is now canonically "dark resonance"** (retarded self-Γ, phase-space; canonical home Q-G19α), and the **thrust sense keeps "dark wake"** (real-space motion-trail; canonical home chiral-thrust-derivation). Per the LOCKED **decision-B**, separate symbols: the g-2 / dark-resonance quantity is now $\Sigma_{\text{near}}$ / $-\dot\Sigma_{\text{near}}$ (a reactive-power *rate*, not a stress), and the thrust / dark-wake keeps a genuine Maxwell shear stress, tagged $\tau^{\text{far}}_{zx}$. This prereg deliberately preserves **both** senses under the legacy "dark wake" name below, as it must to frame the same-object question the test asked — do not rename inside this doc. For the genus/species disambiguation see the [Dark Back-Reaction Taxonomy](../manuscript/ave-kb/common/dark-back-reaction-taxonomy.md) glossary leaf.
