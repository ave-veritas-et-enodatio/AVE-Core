...
**Residual (narrowed, not eliminated):** R2 closes the per-channel **coupling** `k=1`. The deeper **`p_c = 8πα`-as-feedback-fraction** identification — *why* the packing fraction is the per-channel loop-gain — remains the standing 1-residual flagged by the V2-closure doc (§4). R2 does not touch it. So the baryon eigenvalue goes from **"2 residuals" (k + p_c-identification)** to **"1 residual" (p_c-identification only)**.
...
- **Ledger (orchestration executes, not this branch):** baryon row `02_full_derivation_chain.tex:1047` → **"derived"** (qualifier "modulo R2" dropped), with the `p_c`-identification residual noted. This *feeds* the Parameter Ledger walk-back; the manuscript PR is separate.
...
- **Next:** land the Step-6 EE correspondence in `translation-circuit.md` (§4 + §4.5 + §6) + mirror to the skill; optional numerical orthogonality-verification driver (would verify `cosθ=0` at the lattice Borromean crossings — fires substrate-native-check + canonical-source).
