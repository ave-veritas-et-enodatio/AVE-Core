...
|---|---|---|
| **Measured Q (self-impedance)** | the bulk leak observable is **dispersion-contaminated and FLAT** across the knob sweep (CV 1.5%) — it reads breather dispersion, not a wall-transmission Q (apparatus-floors 2026-06-10) | a probe that isolates wall transmission from bulk dispersion — an **apparatus REDESIGN** (the S11 prereg `2026-06-10_electron-s11-sweep_prereg.md` is the floor-gated first attempt; if it lands NO-RESPONSE/UNRESOLVED the row stays UNTESTED) |
