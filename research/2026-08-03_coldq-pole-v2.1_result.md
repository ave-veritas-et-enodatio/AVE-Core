...
At `ℓ = 6` and `ℓ = 18` the **mp location path is perfect** (`5.18e-38`, `1.58e-38` against `1e-20`) while the **winding is unstable across contour samplings** (`8/8/5`, `4/4/12`), and at `ℓ = 10, 14` the double-precision seeding **misses** roots (`4` of `5`, `5` of `7`). Both are double-precision failures in the *seeding and counting* path over a box reaching `|Ω| ≈ 22` — the run logs carry `overflow`/`divide by zero` warnings from `slogdet` on that box.
