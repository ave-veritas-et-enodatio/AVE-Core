...
| **C5** | `nu_vac`-cancellation across `x_sat ∈ {5,7,11}` | `1e-9` | `Q` spread **`0.0`**, `Omega` spread **`0.0`** — ⚑ **as reported by the driver at double precision; NOT identically zero, §6 item 3** | **PASS** |
...
| **C9** ★ | graded-representation convergence at 9 rectangle probes | `1e-10` | `1.8993e+01` (**3 of 9** probes pass — 🔴 **corrected 2026-08-03**, original read *"5 of 9 probes pass"*; `3` matches the shipped JSON and this doc's own ✓ marks below) | **FAIL** |
...
| **FT-B** gauge on interior but not the wall-BC row | C2 | `≥ 1e-12` | `1.8294e-14` | **DOES NOT FIRE** — ⚑ **provenance corrected, §3.1**: as coded this is a `λ = 0` vs `λ = 0.25` comparison, not the frozen mutation (which would read exactly `0.0`) |
...
> **⚑ ADDED 2026-08-03 (post-review) — THE VACUITY CONCLUSION SURVIVES; THE NUMBER'S PROVENANCE DID NOT. Two disclosures, and neither was in the original §3.1.**
>
> **(a) The driver does not implement the frozen mutation.** `research/drivers/coldq_pole_v2.py:160-161` and `research/drivers/coldq_pole_v2.py:304` both read `0.0 if lam_bc_skip else lam` — the flag zeroes `λ` for the **whole operator**, interior included. It is not "interior yes, wall row no"; it is `λ = 0` everywhere. **Under the mutation as actually frozen — `λ` applied to the interior, withheld from the wall row — the measurement would be exactly `0.0`**, because the graded wall row carries no `λ` to withhold. **The reported `1.8294e-14` is therefore not a measurement of the frozen mutation at all: it is a plain `λ = 0` versus `λ = 0.25` comparison** (`research/drivers/coldq_pole_v2.py:736-738`), which is one of the very pairs C2 maximizes over. **The number is corrected in its provenance, not in its value; the driver is byte-untouched.**
>
> **(b) FT-B as coded is the logical NEGATION of C2, and can never fire while C2 passes.** FT-B computes `|ψ(λ=0) − ψ(λ=0.25)| / |ψ(λ=0.25)|` against a threshold of `1e-12`, and C2 passes iff the maximum over exactly that gauge set is `≤ 1e-12`. **A self-test whose firing condition is the failure of the gate it guards is not an independent check of that gate.** This is a **second, independent structural vacuity**, distinct from (a) and from the wall-row argument — and it would hold even if the graded wall row did carry `λ`.
>
> **WHAT SURVIVES, AND WHY C2 IS STILL A LIVE GATE.** Review confirmed the wall row is **genuinely `λ`-independent**, so the original vacuity conclusion is correct. Review also built a **correctly-specified half-applied-gauge analog** — a mutation that applies `λ` to the interior and withholds it from a row that actually carries `λ` — and **it FIRES at `6.4e-02` against `1e-12`**. **C2 is sensitive to exactly the coupling a real bug would break; it is a live gate, and this repair does not disturb it.**
>
> **⚑ THE VACUITY WAS KNOWABLE AT FREEZE — from the frozen file's own text, without running anything.** `research/2026-08-03_coldq-pole-v2.1_prereg-FROZEN.md:493` (§9 item 11) asserts that `λ` *"changes the analytic prefactor and therefore every coefficient function **and** the wall-BC row"* and that *"FT-B's half-applied gauge shows C2 is sensitive to exactly the coupling a real bug would break."* **`research/2026-08-03_coldq-pole-v2.1_prereg-FROZEN.md:152` of the same frozen file states the opposite**: *"the traction-free (Z_shear → 0, Γ_shear = −1) condition is EXACTLY `dψ/dη |_{η=0} = 0`"* — a row with no `λ` in it. **Two statements in one frozen document contradict each other, and the self-test was frozen on the false one.** The original §3.1's *"discovered only by running"* framing is therefore too generous to this lane: **a read of the frozen file against itself would have caught it.** Disclosed; the prereg is byte-untouched and stays that way.
...
3. 🔴 **CORRECTED 2026-08-03. Original text, verbatim:** *"**★ The `nu_vac` cancellation is EXACT, to the last bit.** C5 measures `Q` spread and `Omega` spread of **`0.0`** across `x_sat ∈ {5, 7, 11}` — not `1e-8`, not `1e-15`, but zero — while FT-E fires at `6.01e-07`, proving the gate is live and not dead."*

   **Corrected: the `nu_vac` cancellation is measured BELOW THE DRIVER'S DOUBLE-PRECISION REPORTING FLOOR — it is not identically zero.** The shipped `0.0` is an artifact of the complex cast at `research/drivers/coldq_pole_v2.py:612`, which truncates the mp root to Python `complex` before the spread is formed; two values agreeing to better than `~1e-16` therefore report as exactly `0.0`. **Review re-measured at `dps = 50`: `Q` spread `4.466e-47`, `|Ω|` spread `1.381e-46` — small, and non-zero.** **The gate outcome is unaffected** — both sit far inside the frozen `1e-9` and C5 passes either way — but *"exact, to the last bit"* and *"not `1e-15`, but zero"* were claims about the arithmetic that the instrument could not have made. **FT-E still fires at `6.01e-07`, so the gate is live and not dead; #845 measured `1.1058e-08` here and failed its own `1e-9`.** The prereg's structural claim (§0) — that the cancellation is exact **by construction** because the compactified coordinate *is* the Axiom-4 amplitude — is **consistent with** the corrected measurement and is **not** confirmed "at machine level" by a number the driver rounded to zero. **The commit message that shipped the original wording (`bdcfa678`) stays in history as written; history is the audit trail.**
...
| **BIN-2** `Q` vs `2.10021` | **`nu_vac`-FREE — the only emergence-capable axis in this lane** | `Q = Re(Omega)/(2\|Im(Omega)\|)` contains no `r_sat` scale (C5 measures the cancellation as **below the driver's double-precision reporting floor — not identically zero**: mp at `dps = 50` gives `Q` spread `4.466e-47` and `\|Ω\|` spread `1.381e-46`, §6 item 3. 🔴 **Corrected 2026-08-03; original wording, verbatim:** *"C5 measures the cancellation as **exactly** `0.0`"*. **The `nu_vac`-FREE classification is unaffected** — the cancellation holds at `4.466e-47`, far inside the frozen `1e-9` and far below anything BIN-2 could resolve), so a match would be a **value-level result not inherited through the GR-imported `7`** | that AVE *derives* GR. It would mean the canonical profile + Ax-4 kernel + Op16 projection + `Γ=−1` SHORT produce GR's damping ratio with **zero free parameters** — a strong consistency result, and AVE-distinct only if the *form* differs from GR's own derivation | the substrate's cold `Q` differs from GR's. Given `Q = ℓ` is B1-ratified corpus, a fail would be routed to Grant as a **flag on the standing anchor**, not applied as a fix |
...
5. **THE LOCATED POLE STANDS AS AN UNCERTIFIED DIAGNOSTIC.** `Ω = 1.853655 - 1.007257i` is stable to **12 digits across `n = 48 → 80` (~10 digits from `n = 32`), the `n = 80` endpoint from the review's sweep** — 🔴 **RESTATED 2026-08-03, OVER-PRECISE AS WRITTEN. Original text, verbatim:** *"stable to **12 digits across `n = 32 → 80`**"*. **The shipped JSON's own C3 sweep does not support 12 digits at the `n = 32` endpoint**: between `n = 32` and `n = 64` it moves in the tenth significant digit, so `n = 32` carries ~10 digits, not 12, and 12 digits holds only from `n = 48`. **The `n = 80` endpoint is not in the shipped JSON at all** — the C3 sweep stops at `n = 64`; `80` comes from the review's pencil sweep and is attributed to it here. **The `n`-stability conclusion is unchanged**, only its digit count and the provenance of its upper endpoint — and it is the **only** in-box eigenvalue that is `n`-stable. The `6.80e-07` agreement with #845 — two instruments different in kind — likewise stands as a diagnostic. **Both lanes are `SOLVER-NOT-CERTIFIED`, and two uncertified instruments agreeing does not certify either.** That sentence is retained exactly as originally written, and this correction makes it *more* load-bearing, not less.
