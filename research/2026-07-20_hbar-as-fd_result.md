...
---
**Errata + owed-list expansion (2026-07-20, review repairs — the FROZEN derivation doc is NOT edited):**
- **FLAG-1 owed-list ENUMERATED (repair 1):** the auditor-lane landing must carry the SEM caveat to all four `0 → 0.129` propagation sites — `research/2026-07-19_f6-thermal-floor-arm_result.md:114` (table) **and** its `:110` descriptor ("ensemble seed-spread of R_rev" mislabels a `std/√6` SEM as a std), `manuscript/ave-kb/common/engine-capability-map.md:434`, `_orchestration/2026-07-10_rulings-docket.md:2114`, `_orchestration/2026-07-15_hardware-ratings-map.md:93` — not only a new KB-thread note. (All four are fenced no-claim sites; the caveat is: the absolute value is an N_seeds=6 SEM artifact, ∝1/√N; the √ρ SHAPE is N-invariant.)
- **Strengtheners recorded (repair 2, erratum-class — `DERIVATION_FROZEN.md` untouched):** (i) the review's independent derivation shows the √ρ law and the ½:½ quadrature split are ANALYTICALLY EXACT (`std(q)/√ρ = √e_sig·√(Σ1/ω_m²)`, ρ-independent) — the MC's 0.8%/0.4% figures are finite-sample noise around exact laws, not empirical tolerances; (ii) `relax(ρ)` CANCELS in the FORM-MATCH relative error (both prediction and banked datum divide by the same banked denominator), so the match is a clean test of the SEM-numerator's √ρ shape — stronger than framed, and it means the frozen UNDETERMINED bin ("relax dominates") was unreachable by construction (disclosed here as a cannot-trip bin); (iii) erratum to `DERIVATION_FROZEN.md:47`: "floor-level-independent at leading order" is contradicted by the banked `relax(ρ)` (0.684→1.281, +87%) — verdict-harmless per (ii), recorded not re-written.
- **Attribution erratum (repair 3):** `DERIVATION_FROZEN.md:20`'s inline quote "mode loss ≠ system loss, its transduction" [sic] is a NORMALIZED COMPRESSION, not the canonical verbatim; the canonical string is "mode loss should jot equal system loss, its trasnduction right?" [sic] (`retention-transition-split.md:47`). The frozen doc is not edited; read its quote as a paraphrase-cite of RULING-21.
