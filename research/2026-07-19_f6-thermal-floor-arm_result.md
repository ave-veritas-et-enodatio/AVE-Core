...
- **✅ The arm fired the walk-record's OWN designated falsifier.** Walk §2: *"★ The arm's frozen classes include **NO-SUPPRESSION** (ride-on-top) as the **live falsifier**... if a populated floor does not suppress coherent returns, the 'dephasing-against-a-random-floor sets the arrow' mechanism is **dead**, and honest closure (Rule 11) applies."* This arm returns **NO-SUPPRESSION** — the walk's central hypothesis (walk §1c-ii: *"THE LOCAL ARROW = coherent returns dephasing into the pre-occupied random background"*) is **dead** by its own pre-committed kill-shape. No framing conflict: the walk set the arm as adjudicator and named this exact outcome as the falsifier; the arm lands there.
- **✅ The mechanism is the SHARP form of the walk's own §1e precision note.** Walk §1e (PRECISION NOTE): *"In the **linear** engine, per-mode energies do not literally shift frequency... literal spectral downconversion requires the **nonlinear** channel... Conflating the two would overclaim a literal frequency shift the linear engine does not produce."* This arm makes that caveat decisive: in the lossless-reactive (linear, independent-oscillator) bath, the coherent revival is **not dephased at all** — the floor and signal never interact — so the "cross-mode phase decorrelation" the walk hoped would carry the arrow does **not** touch the coherent signal's re-phasing. Dephasing-that-sets-the-arrow needs the interacting/nonlinear channel the walk's §1e already flagged as the only literal one.
- **⚠ PARTIAL CONFLICT surfaced — walk §1c-i "the clamp becomes moot" is comb-dependent (STAGE-1 §D-post Dp-1).** Walk §1c-i: *"a pre-occupied floor changes the endpoint: the cavity relaxes **to the floor**, not to zero. The over-extraction **clamp** then becomes **moot**."* STAGE-1 found this is **only true on partial-transfer combs**: on the arm's `Δω=0.050` comb the floor DOES keep `E_lat` alive (clamp moot — the arm ran clamp-free across [0,5], consistent with the walk); but on the **quasi-continuum / fully-discharging** combs (`Δω≤0.020`) the floor's per-step jitter drives `E_lat→0` and fires the clamp **earlier** (the #727 wall from the floor side) — clamp **NOT** moot there. The walk's clamp-moot intuition holds where the arm operates and is falsified where it does not; the boundary is the FLOOR-METER-VALID-BAND. Both file paths surfaced for adjudication; not silently reconciled.
- **Owed to the auditor lane (implementer surfaces, auditor lands):** the walk-record's §2 falsifier is now **fired** (NO-SUPPRESSION); the §8c "owed placement" pointer (walk KB-pointer section) should record the arm's adjudication once #728 lands §8c on main; the walk §1c-i clamp-moot claim needs the comb-dependent qualifier. This lane does **not** edit the walk-record (a different lane owns it) nor draft the auditor's §8c entry.
