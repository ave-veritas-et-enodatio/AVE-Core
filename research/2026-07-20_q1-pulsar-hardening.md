...
3. **Exact double-pulsar figure re-verification** — the `1.3×10⁻⁴` is a 95%-conf bound; the most recent intrinsic-`Ṗ_b` analysis may be tighter (`~6×10⁻⁵`). Either way the exclusion is unchanged; the exact figure is an owed cite-check before any leaf headlines it. *(Same discipline #750 applied to the LIGO Bayes factor.)* **✅ DISCHARGED 2026-07-20** (tier-B debt batch): re-confirmed via WebFetch against the Kramer et al. 2021 abstract (arXiv:2112.06795), verbatim *"validating GR's prediction at a level of $1.3 \times 10^{-4}$ (95% conf.)"* — the `δ_DP ≈ 1.3×10⁻⁴` figure this doc imports (§1 table, `[import]`-ledger `:194`) is confirmed exactly. (The possibly-tighter `~6×10⁻⁵` intrinsic-`Ṗ_b` reading is a separate refinement; the imported 95%-conf bound is the one carried, now verified. The exclusion is unchanged regardless.)
