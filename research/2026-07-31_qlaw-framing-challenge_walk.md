...
| **FORK-2** *(re-posed 2026-07-31, F8)* | **A5 — the $S$-exponent, reclassified** | *~~At the wall, does the vacuum go **soft** or **stiff-and-opaque**?~~ **Canon already answers that** — for the shear channel it is **soft-short**, stated verbatim at [`vol3/claim-quality.md:123`](../manuscript/ave-kb/vol3/claim-quality.md): "a solid$\to$liquid free surface ($G_{shear}\to0$) is exactly a $Z_{shear}\to0$ **short**". The question the options actually ask is different:* **which constitutive branch does the shear integrand ride — and is $S^{1/4}$ admissible in it at all?** | (a) ratify the $(L,C)$ branch table (§CF-5) ⇒ shear rides $\sqrt S$, $S^{1/4}$ is an ASYM/EM artifact with no constitutive home (CF-6 leg 1); (b) reject the channel assignment and keep $S^{1/4}$ live in the shear integrand; (c) KEEP-BOTH and make the derivation report both. **Note the original question was mis-posed:** soft-vs-stiff is settled for shear, and *that* is why the surviving content is the branch assignment. The genuinely open soft-vs-open question belongs to **FORK-3** (which $\rho$), not here |
