...
**Branch-state honesty (load-bearing; refreshed 2026-07-20 review-repair R2).** Several rows cite content on open PRs (#750 scalar-GW port; #751 J(ω)/arccos). **#749 (ratification batch) MERGED 2026-07-20T16:38:49Z — its rulings are now `[canon]` (P3/P7 refreshed below).** Those cites are tagged `[branch:#NNN]` and state the branch content is **not-yet-canon**; the register does not treat branch content as landed. Fenced per the lane brief: **no edits to #748/#750/#751 branch files.**
...
**🔴 RULED-CONDITIONAL 2026-07-20 (Grant — Part 3 "rule it"). KEEP-BOTH: the UNRULED register-verdict above is preserved (register-state at draft time); this note records the ruling.** Grant ruled 2026-07-20 (**Grant-verbatim three-parter, [sic]: "part 1: loose works / Part 2: bank / part 3: rule it"**; ruling content = the **orchestrator's walk, ratified in chat**). **Reading B is RULED:** *the bulk sector has no far-field radiative port for gravitating sources — the halo is its complete story (reactive near-field; nothing drains).* **CONDITIONAL:** the ruling stands on the B-mechanism lane deriving the closure (**B1** constraint-slaving / **B2** source-projection / **B4** no-independent-P-branch); if that lane lands **NONE-DERIVES, this ruling REVERTS** and the Reading-A exclusion — now **banked** at `manuscript/ave-kb/common/genesis-chord-falsification-ledger.md` (Part 2) — goes live against the framework. Part 1 ("loose works"): the `08_gravitational_waves.tex` step-3 "bulk lattice network" wording is fixed to "transverse shear" and its warningbox is closed RESOLVED-with-receipt. **D1 rider (OPEN):** the compact-object scalar-charge/mass question (surfaced by #750's review) rides the B-lane context and is NOT ruled today.

> **★SYNC-OWED (race note — `docs/port-register-promotion` collision fence).** A concurrent lane (`docs/port-register-promotion`) is transcribing this draft into a canonical `common/` port-register leaf. **That canonical leaf's Q1 row MUST carry this RULED-CONDITIONAL ruling** (Reading B ruled, conditional on the B-lane; Reading A banked as the revert-live exclusion). The promotion lane checks the docket (ENTRY 32) at build; the orchestrator verifies at its review. **This lane does NOT touch `manuscript/ave-kb/common/port-register.md`** even if it exists by build time (collision fence).
...
- P3, P7 receipts cite **#749** docket ENTRY 27 R3/R4 — **#749 MERGED 2026-07-20: the rulings are now `[canon]`** (review-repair R2 refresh; previously branch-state).
...
**Rows I could NOT provenance: NONE.** Every row is provenance-tagged to a content-verified `[canon]` leaf or an honestly-flagged `[branch:#NNN]` source. The honest caveat is the branch-state dependence of P6/P8/Q1 (three rows lean on open PRs #750/#751; P3/P7 became `[canon]` when #749 merged 2026-07-20), NOT an un-provenanced row.
