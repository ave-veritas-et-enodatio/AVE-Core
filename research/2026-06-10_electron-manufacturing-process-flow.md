...
- **Coordinate check (`ave-representation-capability-check`):** `R/r` is a **real-space `O₁` unknot** major/minor ratio compared to the **real-space** Golden-Torus `φ²` — coordinate-matched. The `(2,3)` phase winding does **not** enter this balance. The comparison is legitimate in coordinates; it simply does not close.
