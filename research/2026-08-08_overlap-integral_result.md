...
- **D4 (anisotropy) — NO ZERO.** Measured as the smooth 1.1–12.8% coefficient drift of `ρ_spec/ρ_ref` across the kR sweep (§2.1), consistent with the declared O((kℓ)²) budget (at kR = 2.6, `(kℓ_site)² ≈ 0.094` — the observed 12% deviation IS the declared correction, not a new structure).
...
3. **STATIC-CONTROL floor window: frozen-literal `[t_arr, t_reflect)` → three variants computed SIMULTANEOUSLY, no post-hoc selection** — `frozen_full` (prereg-literal), `late_half` (the v1 driver's definition), and `s_cleared` (`[t0_ramp + 2.5σ_ramp + (r_meas−r_port)/c_S, t_reflect)` — analytic transit term (the c_S spectral speed) plus a disclosed 2.5σ Gaussian-clearing convention and the σ_ramp = T_d/2, t0_ramp = 3σ_ramp ramp conventions; no result-directed freedom). Rationale (the v1→v2 instrument repair, both driver versions in git — v1 committed as-run with the defect named in its commit message): the ramp transient's SLOW S-TAIL (c_S ≈ `0.286` cells/time) is still crossing the measurement shell during the late-half window, and the static run's own spectrum proves the contamination is off-signal — its content peaks at `0.227`·2Ω with band fraction at the signal frequency of order 10⁻² (JSON `band_frac_at_omega_d`), while the commutation signal peaks AT 2Ω. Direction: the S-cleared floor is LOWER (results-favorable for the `R_{c/s}` bin criterion) — therefore ALL variants are reported in §2.2 and the bin is scored with the contamination mechanism named, its analytic basis stated, and the frozen-literal number shown beside it. **Flagged explicitly for the Tier-2 lane as the most attackable step.**
4. **rho_star unpack** (`derive_rho_star()[0]` — the survey helper returns a tuple): mechanical, no physics content.
5. **G-FREQ observable substitution (Tier-2-surfaced, dated 2026-08-08; un-forced, disclosed here):** the prereg froze "FFT of the shell-mean radial velocity"; the shipped driver FFTs the l=2/l=0-PROJECTED radial displacement. Direction: CONSERVATIVE-or-neutral — the Tier-2 review computed the frozen-literal-adjacent variant and it reads STRONGER (peak `0.9972`·2Ω, Ω-content `0.0019`), and the frozen-literal shell-mean observable is structurally near-blind to the l=2 signal (`s00_window_rms` commutation `1.26e-06` vs radial-AC `2.75e-04`), so the substitution did not manufacture the receipt.
6. **Static-drive normalization:** the frozen "equal time-averaged mean-square force" was implemented as equal PLATEAU mean-square for the static control; its realized `drive_ms_timeavg` (`8.466e-09`) is `2.6×` the burst runs' (`3.267e-09`). Direction: CONSERVATIVE for the floor (a stronger static drive can only RAISE the floor, making the `R_{c/s}` bin conjunct harder to pass).
7. **Dated errata notes on the FROZEN prereg (the prereg body is NOT edited; these notes are the Rule-12 repair):** (a) prereg §8 table, kR = 2.2 row: the frozen `0.0862` is a truncation at its own stated 4-s.f. precision; the correct evaluation is `0.0863` (`0.086280`), which the §2.1 result table uses. (b) prereg §8: the frozen "`f_PM(0.6171) = 11.86`" mislabels the eccentricity — `11.8568` is f_PM at the unrounded `e = 0.6171334`; `f_PM(0.6171) = 11.8533`, which Arm C uses consistently. Both errata are gate-neutral (G-SCALE ×3; G-ECC 1%). (c) prereg §3's parenthetical describing the brief's formatting delta is inaccurate: the brief's ellipsis is MEDIAL (it elides the "#750/#758 drivers" reproduction sentence), a second bold is added on "The tracelessness rescue is dead", and the source's closing B1 symmetric-coupling-grade clause is truncated without a marker; the brief excerpt spans `:16-24`. The §3 incumbent quote ITSELF is byte-identical to the source (review-verified both engines) — only the courtesy parenthetical mis-describes the brief's copy. (d) prereg §0 row 4's LC-1 pointer "§3 row 3a" should read "LC-1 §3 (derivation) / §5 channel-enumeration table row 3a" — content verified on the cited branch; locator-only slip.
...
3. **Cross-method (G-XMETHOD):** `R_{c/a} = 0.0878` (time domain) vs `ρ_spec(2.6) = 0.0471` (spectral) — factor `1.86`, inside the frozen ×2; vs the continuum `0.0540` — factor `1.63`, inside the frozen ×3. **Residual attribution (Tier-2 item T7):** near-field excess at `kr_meas = 10.4` (2.4–2.9%) and burst bandwidth (~1%) are quantified INSUFFICIENT to explain the 1.86×; the dominant un-decomposed candidate is the commutation run's S-dominance (receipt 4) leaking quasi-S content into the radial projection through the declared O((kℓ_site)²) ≈ 0.094 finite-k polarization impurity (up to ~40% of `E_P^comm`). The gate passes as frozen; the residual is characterized, not hidden.
...
| **G-NUM** | number check + mutation receipt green; `make verify` green | **FAILED-AS-SHIPPED / REPAIRED (Tier-2 T5, MAJOR):** the shipped checker matched only BARE back-ticked numerals, so composite tokens (`R_{c/s} = 2013`-class — the most verdict-bearing numerals) escaped coverage, and stale allowlist literals created silent-mutation holes. Repaired post-review: composite-token numeral extraction, %-suffix handling, stale-literal purge, a must-appear check on the bin-conjunct numerals, and a composite-targeting mutation receipt; re-run green. The review hand-verified every composite-token verdict numeral against the shipped JSON BEFORE the repair (all reproduce) — instrument defect, not number-honesty defect | **PASS-WITH-REPAIR** |
...
> **★TIER-2 REVIEW-REPAIR ADDENDUM (2026-08-08, PR #927 adversarial review — this doc EDITED per Rule-12 repair-and-bank; the frozen prereg is BYTE-UNTOUCHED, its defects carried as dated notes in §2.0-7).** Five lenses (green-math / receipts / frozen-criteria / physics-knife / code), 22 agents, per-finding adversarial verify. **Disposition: 15 CONFIRMED (1 MAJOR, 14 MINOR after verify) / 2 REFUTED — every confirmed finding EVIDENCE-VOID (repair-and-bank), none CONCLUSION-WRONG. The verdict, all four bin conjuncts, and the divergence-step statement survive every lens.**
>
> - **T1 (the PRE-FLAGGED PRIMARY TARGET — the v1→v2 floor-window instrument repair): ADJUDICATED LEGITIMATE.** No Rule-11 finding; the disclosure structure (three variants computed simultaneously, direction-of-effect declared, both driver versions in git, the `0.009`-vs-`0.808` band-fraction diagnostic) carried it. The review's own independent crosscheck: band-restricting the FROZEN-LITERAL window (zero repaired-window inputs) still clears the bin at `R_{c/s} ≈ 59.6 ≥ 10`. The only confirmed defect on this step is the "zero tuning freedom" wording (MINOR) — softened in §2.0-3 to name the three convention constants.
> - **T2 (MINOR ×4 lenses): the headline "0.4–12%" continuum-match range was not re-derivable** (the flux-weight sensitivity delta masquerading as the match low end) — corrected to `1.1–12.8%` at both occurrences.
> - **T3 (MINOR): two frozen-prereg §8 reference numerals wrong at stated precision** (`0.0862` truncation; `f_PM(0.6171) = 11.86` eccentricity mislabel) — dated errata notes §2.0-7(a,b); prereg body untouched; gate-neutral.
> - **T4 (MINOR): prereg §3's brief-formatting-delta parenthetical inaccurate + the LC-1 "§3 row 3a" locator slip** — dated notes §2.0-7(c,d). The incumbent quote itself is byte-identical to source (review-verified, both engines).
> - **T5 (MAJOR): the G-NUM instrument was blind to composite back-tick tokens and carried stale allowlist literals** — repaired (see the G-NUM gate row); the review hand-verified every escaping verdict numeral against the shipped JSON before the repair (all reproduce).
> - **T6 (MINOR): the Arm-A one-shot k-placement sits ~1% inside the true iso-frequency shell** (small-k-acoustic speeds); dispersion-resolved placement gives `ρ_spec(2.6) = 0.04483` (−4.7%), moves the budget-tracking coefficient from ×1.4 to ×1.8, and thins G-XMETHOD from `1.86`× to ≈`1.96`× of the frozen ×2 — ALL gates and the verdict unchanged under either placement; the shipped JSON is not re-run; the §2.1 characterization sentence now states both placements.
> - **T7 (MINOR): the G-XMETHOD 1.86× residual was un-attributed** — now attributed in §2.2-3 (S-dominance × finite-k polarization impurity as the dominant candidate; near-field and bandwidth quantified insufficient).
> - **T8 (MINOR, downgraded from MAJOR at verify): two silent §2.0 ledger omissions** (the G-FREQ observable substitution; the static-drive plateau normalization) — added as §2.0-5/6 with direction-of-effect; the review's computation shows the frozen-literal G-FREQ variant reads STRONGER (`0.9972`·2Ω peak), so the substitution manufactured nothing.
> - **Refuted (2):** see the review record (one physics-knife sub-claim — the half of T8 alleging the substitution was forced — and one code-lens finding did not survive their verify passes).
>
> **Net effect on the verdict: NONE.** OVERLAP-NONZERO(n = 2) stands on 4.7 decades of nonzero-overlap margin + the D-register adjudication; the divergence-step-NONE statement stands; the eccentricity shape stands (machine-precision f_PM identity untouched); the amplitude inheritance (NOT-DERIVABLE, #919 R23) unchanged. Review record: workflow run `wf_5ce6b55b-783` (22 agents; per-agent results in the session workflow journal).
