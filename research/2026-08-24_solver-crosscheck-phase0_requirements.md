...
**§1 (scalar band top + gap inventory, `:27-47`), §2 (the arccos TL methods fact, `:49-76`) and §6 (`:145-146` the `ANALYTIC_NETWORK_FACTOR` consistency-class line, and `:153-157` the R2 adjudication flag) carry NO demotion marker, and those are the three sections this epic extracts from.** Every OBS row in §5.2 cites §1 or §2 only; **§6 is the third extracted section** — `SCX-REQ-LABEL` (§3.1) quotes its `:153-157` adjudication flag verbatim as the binding R2 convention, and `SCX-REQ-ELEMENTS` (§2) cites its `:145-146` line as the canon-bless for `ANALYTIC_NETWORK_FACTOR`. An earlier draft's "only sections" sentence omitted §6 while the doc extracted from it; corrected here rather than left as a silent completeness overclaim. **Note the standing caveat that travels with the §6 extraction:** `:153-157` is *itself* an open adjudication flag (*"Flagged for adjudication"*), so `SCX-REQ-LABEL` pins a convention that canon has flagged as unsettled — which is precisely why §3.2's FL-1 exists.
...
>
> **★ The nearest live corpus statement, which an earlier draft of this question omitted (checker-audit SCX-06) — and it leans toward (a).** `srs-band-structure.md:136-140` §5(c), **un-demoted**, verbatim: *"this kills GAP-PINNED modes ONLY; the **above-band mobile breather** (freq > the true top, in the semi-infinite gap above the connected manifold) is **NOT falsified** and remains the live carrier-fork branch. The carrier-fork gate must not read no-internal-gap as killing the above-band breather."* So the corpus **does** carry a live object above the band top, and it is load-bearing elsewhere (the carrier fork). Q1 stands as a question — that leaf is about a *breather* in the semi-infinite region above the manifold, not about whether a single bond rings at its full-wave, and the two are not the same claim — but the question may no longer be posed as though the corpus said nothing above the top. **Not resolved by this lane.** If Grant answers (b), note that the answer has a reach beyond this epic: it would put pressure on the above-band branch that §5(c) explicitly preserves, which is an auditor-lane consequence to route, not one to absorb here.
