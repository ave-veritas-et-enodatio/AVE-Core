...
| `SCX-REQ-ELEMENTS` | Every element value is imported by symbol from `src/ave/core/constants.py`, **plus the two canonical engine symbols that are not in that module and are named here explicitly: `ANALYTIC_NETWORK_FACTOR` (src/ave/core/chiral_lattice_dynamics.py:48, canon-blessed at srs-band-structure.md:145-146) and the coordination z (net.degree / srs_band_survey.py:45)**. Nothing is typed. The exporter has zero free parameters | physics-set |
...
**Every element value is imported by symbol from `src/ave/core/constants.py`, plus the two canonical engine symbols that are not in that module and are named here explicitly: `ANALYTIC_NETWORK_FACTOR` (`src/ave/core/chiral_lattice_dynamics.py:48`, canon-blessed at `srs-band-structure.md:145-146`) and the coordination $z$ (`net.degree` / `srs_band_survey.py:45`). Nothing is typed.** Every value below is an import-path citation; the exporter therefore has no free parameter (`ave-canonical-source`; epic §5.4).
...
**§1 (scalar band top + gap inventory, `:27-47`), §2 (the arccos TL methods fact, `:49-76`), §6 (`:145-146` the `ANALYTIC_NETWORK_FACTOR` consistency-class line, and `:153-157` the R2 adjudication flag) and §5(c) (`:136-140`, the gap-breather scoping paragraph) carry NO demotion marker, and those are the four sections this epic extracts from.** Every OBS row in §5.2 cites §1 or §2 only; **§6 is the third extracted section** — `SCX-REQ-LABEL` (§3.1) quotes its `:153-157` adjudication flag verbatim as the binding R2 convention, and `SCX-REQ-ELEMENTS` (§2) cites its `:145-146` line as the canon-bless for `ANALYTIC_NETWORK_FACTOR`. **§5(c) is the fourth** — §8.1's Q1 block-quotes `:136-140` verbatim, which is the same act by which §6 counts as extracted; `:136-140` carries no demotion marker, and it is a *distinct paragraph* from the demoted §5 row, which is `:131` (item (b), the pair-channel coexistence window). An earlier draft's "only sections" sentence omitted §6 while the doc extracted from it; the correction then said "three" while a §5(c) citation was being added to §8.1 in the same commit — both are corrected here rather than left as silent completeness overclaims. **Note the standing caveat that travels with the §6 extraction:** `:153-157` is *itself* an open adjudication flag (*"Flagged for adjudication"*), so `SCX-REQ-LABEL` pins a convention that canon has flagged as unsettled — which is precisely why §3.2's FL-1 exists.

**Demotion check on the second cited leaf, `translation-circuit.md` (checker-audit SCX-03).** The epic cites two rows from it, and both were verified this branch: `:189` (the $\Gamma=(2-z)/z=-1/3$ bare-junction mismatch, consumed by `SCX-REQ-ANCHOR` §4) and `:884-888` (carve 4, the Bragg/half-wave-not-stop-band class fact, consumed by OBS-2). **Both rows are CLEAN — neither carries a demotion marker, and neither sits inside a dated demotion note's line range.** Stated explicitly because `SCX-REQ-OBS` requires *"that path's demotion status"* for every reference path, and an audit that checks only the first leaf leaves the second's status asserted rather than verified.
...
**Therefore the interior edge set and the overlap structure are fixed by the srs geometry — the bond displacement vectors $\boldsymbol\delta$ — and not by the premise set.** A solver reproducing them has reproduced something the **wiring-theorem premise set** {3-regular, bipartite, identical lossless lines} does not force, which is exactly the independence weight epic §4 Phase 2 says the quantitative comparison must carry. *(Wording pinned per checker-audit SCX-07: the earlier phrase "something the exporter's wiring alone does not force" was too strong — the exporter's wiring includes the bond displacement vectors $\boldsymbol\delta$, so in the literal sense the wiring **does** force the interior edges. The claim that the honeycomb contrast actually establishes is the premise-set one, and it is the one epic §4 asks for.)*
...
| srs supercell $L=2$ | 64 / 96 / 192 | 0 (×34); **10** interior values matching the arccos map with doubled multiplicity; π (×34) |
| P1-C (vertex + 3 open stubs) | 4 / 3 / 6 | 0 (×1); π/2 (×4); π (×1) |
| P1-B (one bond, open–open) | 2 / 1 / 2 | 0 (×1); π (×1) |

The $L=2$ interior values reproduce the arccos map exactly (**10** values: 0.635563, 0.729728, 0.955317, 1.230959, 1.432283, 1.709310, 1.910633, 2.186276, 2.411865, 2.506030 — each at exactly twice the adjacency multiplicity), which is the direct verification that **substitute (A) samples the bands**. *(Labelling correction, checker-audit SCX-05: an earlier draft of the row above said "12 interior values", contradicting this list of 10. **12** is the count of distinct arccos $\theta$ values **including** $\theta=0$ and $\theta=\pi$; **10** is the interior count $0<\theta<\pi$, which is what the row means and what F5's interior-only comparison rule consumes. Re-derived at HEAD, not taken from the earlier draft.)*
...
>
> **Why it must be answered before the prereg freezes:** F1 currently caps every comparison at $\pi\,\omega_{link}$ on instrument grounds. If (a), that cap is hiding a real prediction and the epic should say so. If (b), the cap is physics and should be stated as physics. The corpus is not silent but it does not settle this: `srs-band-structure.md:69-76` says $\pi\cdot\omega_{link}$ is a *"first Bragg / half-wave-line resonance, physical, **not** a temporal-Nyquist artifact"* — which asserts the top is real without itself saying whether anything lives above it.
>
> **★ The nearest live corpus statement, which an earlier draft of this question omitted (checker-audit SCX-06). What it does is remove an argument-from-silence — it does NOT lean the fork.** `srs-band-structure.md:136-140` §5(c), **un-demoted**, verbatim: *"this kills GAP-PINNED modes ONLY; the **above-band mobile breather** (freq > the true top, in the semi-infinite gap above the connected manifold) is **NOT falsified** and remains the live carrier-fork branch. The carrier-fork gate must not read no-internal-gap as killing the above-band breather."* So the corpus **does** carry a live object above the band top, and it is load-bearing elsewhere (the carrier fork). Q1 stands as a question, and the fork stays **open** — that leaf is about a *breather* in the semi-infinite region above the manifold, not about whether a single bond rings at its full-wave, and the two are not the same claim. They are not even the same *kind* of object: §5(c)'s above-band mobile breather is a localized carrier-fork candidate above the **band manifold**, whereas Q1(a) is a **linear $n=2$ mode on a single bond**. What the citation changes is only this: the question may no longer be posed as though the corpus said nothing above the top. **Not resolved by this lane.** If Grant answers (b), note that the answer has a reach beyond this epic: it would put pressure on the above-band branch that §5(c) explicitly preserves, which is an auditor-lane consequence to route, not one to absorb here.

### §8.2 — routed flags (surfaced, not fixed)

| # | flag | route |
|---|---|---|
| **FL-1** | **THREE live bond-length/delay conventions in-tree, not two** (checker-audit SCX-08). (i) `bond_lc()` ⇒ bond length $\ell_{node}$, $\mathrm{TD}=1/\omega_C$ (**R1**); (ii) `ANALYTIC_NETWORK_FACTOR` ⇒ $\mathrm{TD}=1/(\sqrt3\,\omega_C)$ (**R2**, adopted by `SCX-REQ-LABEL`); **(iii) [`src/scripts/vol_1_foundations/r10_v8_ee_phase_a.py:255-263`](../src/scripts/vol_1_foundations/r10_v8_ee_phase_a.py) sets `bond_length_SI = np.sqrt(3.0) * L_NODE`** and builds $L_{bond}=\mu_0\cdot$that, $C_{bond}=\varepsilon_0\cdot$that ⇒ $\mathrm{TD}=\sqrt3\,\ell_{node}/c_0=\sqrt3/\omega_C$ — a **third** delay, $\sqrt3\times$ **longer** than R1 and $3\times$ longer than R2, i.e. it moves the label the *opposite* direction from R2. Acceptance test T0.2 additionally asserts the R1 form as a Class-A identity while the band survey uses the R2 form. No in-tree result is known to be wrong (§3.2 scope), but the symbol *named* `bond_lc` is the one an exporter author will reach for, and convention (iii) shows the drift is already three-way rather than a clean binary fork. | auditor lane — is `bond_lc` the emergent-medium constitutive pair mis-named per-bond, or is the R1/R2 fork genuinely open at bond level? **Now additionally: what is the intended bond LENGTH — $\ell_{node}$ or $\sqrt3\,\ell_{node}$ — since (iii) answers that differently from (i) and (ii).** |
