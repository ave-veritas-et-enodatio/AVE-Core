...
- **In-flight, sibling lane.** The arm is being built concurrently on branch `feat/f6-thermal-floor-arm` (its **§D floor-battery**, its prereg, and its docket entries). **This walk-record does not duplicate that content** — it is the WALK/CONTEXT record and cross-references the arm as in-flight. *(As of this freeze the arm branch is not yet on `origin`; the cross-reference is forward-looking. The docket-tail collision expected between this doc's continuation block and the arm lane's is anticipated and normal — see D3.)*
...
| **Provenance** | taking the floor's **level + randomness as given** | the floor's *provenance* (why that level, why random) is **upstream** — the past-hypothesis analog; named candidate = the **crystallization frontier** (`dark-energy-latent-heat-definition.md:30`) |
| **Cosmological rate** | the **local** coupling is effectively constant (`H` invisible locally) | the **growth / node-genesis** term re-homes to the cosmological rate rung, `Γ = 3Hρ_latent` (`dark-energy-latent-heat-definition.md:30,:121`) — a **separate** rung |
