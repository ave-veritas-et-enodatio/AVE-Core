...
| C1 on-site · dense `N=1200` | `+5.4e-1` | **≥ tol** | ~returns (0.50) | ✗ | ✗ |
| C2 strain · finite `N=60` | `+7.5e-2` | **≥ tol** | returns (0.35) | ✗ | ✗ |
| C2 strain · dense `N=1200` | `+3.7e-2` | **≥ tol** | stays up (0.58) | ✗ | **✓** |

**Frozen output (precise): (a-ledger) fires in 0/4 cells; (b-ledger) in only 1/4 — the super-Ohmic (C2) ∞-lattice bath.** Under continuous driving the net-per-cycle transfer EXCEEDS `tol` in **every** cell (even the finite/0D bath transfers above the integrator floor per cycle), so (a-ledger)'s `< tol` conjunct — the "reactive refusal" bin — **fails everywhere**. The finite/0D baths RETURN within the window (return-ratio 0.27–0.35) so (b-ledger)'s "not-returned" clause fails there; C1-dense sits right at the return boundary (0.499, so counts as returning → neither fires); **only the C2 super-Ohmic ∞-lattice** stays up (0.58, sub-recurrence) AND transfers ≥ tol → (b-ledger) fires there. So **3/4 cells fall in the degenerate gap** (transfer above the floor yet returning) and the one clean world-(b) read is coupling-MODEL-specific. **No clean UNIFORM (a)/(b) scope separation** — exactly the muddiness `§4.2` discloses; the clean 70–95 %/0–10 % split came only from the post-hoc ring-down. The transfer MAGNITUDE tracks the undetermined coupling prefactor `ζ` (C2 dense: `5.6e-3` at scale 0.2 → `3.7e-2` at 0.6 → `1.3e-1` at 1.0), i.e. **the frozen tol-gate's quantitative verdict inherits the same indeterminacy as bin (c-magnitude)**. Frozen-tree output: **bin (iii) DEGENERATE**, with (c-magnitude) firing on the coupling-model split (§0.3).
...
**Frozen output (Rule-11): (a-ledger) 0/4 cells, (b-ledger) 1/4.** The net-per-cycle transfer is `≥ tol` in every cell, so **(a-ledger) never fires** (its `<tol` "reactive refusal" conjunct fails even for the finite/0D bath — continuous driving pumps energy above the integrator floor each cycle). The finite/0D baths RETURN within the window (0.27–0.35) → (b-ledger) fails there; C1-dense is at the return boundary (0.499); **only the C2 super-Ohmic ∞-lattice** (0.58, sub-recurrence, ≥ tol) fires (b-ledger). So **3/4 cells fall in the degenerate gap and the one world-(b) read is coupling-model-specific — no clean UNIFORM (a)/(b) scope separation ⇒ bin (iii) DEGENERATE.** The transfer magnitude is coupling-scale-governed (see §4.3), so the frozen tol-gate's quantitative verdict is the same UNDETERMINED as (c-magnitude). **The pre-registered driven instrument does not cleanly separate the scopes** — the muddiness the original ship narrated but never quantified; the clean 70–95 %/0–10 % split came only from the post-hoc ring-down (§4.2).
...
| frozen (a-ledger)/(b-ledger) net-per-cycle transfer | **[DERIVED]** | **DEGENERATE** (a:0/4, b:1/4 super-Ohmic-∞ only; Rule-11, §4.1) | manifestation |
