...
- **C2 (R1 — scope the no-lift claim).** *Superseded:* §0 "**NOT** a parallel-bypass (it does **not** lift) … **refutes** any 'junction lifts the ceiling'." *Correction:* no lift **within the leading-order positive-element lumped class only**. A fuller vertex model (evanescent-mode stub / finite junction volume = a resonant shunt branch) is **not excluded** and could present a bypass. And `f_crit≈0.184` puts `ω_vertex` **inside** the band for `f>0.184`, where the quasi-static lumped separation self-invalidates — most of the 31.4 % swing accrues in that regime. **[Scope superseded 2026-07-10 — see Correction Log C8:** the "not excluded / could present a bypass" escape hatch is **theorem-dead across the ENTIRE lossless reciprocal C₃ vertex class** (matched-lossless-3-port theorem); the only surviving escape class is **non-reciprocity** (circulator/gyrotropic vertex — PENDING-GRANT).]
...
- **C8 (2026-07-10, post-merge correction-PR — the reciprocal-class escape hatch is THEOREM-DEAD; only non-reciprocity survives).** *Superseded (C2's escape hatch, quoted verbatim):* "A fuller vertex model (evanescent-mode stub / finite junction volume = a resonant shunt branch) is **not excluded** and could present a bypass." *Correction (theorem-level, independent of any unmerged work).* The classic **matched-lossless-3-port theorem** of microwave network theory (Pozar-class result — *a lossless, reciprocal, matched 3-port is impossible; for the symmetric C₃ case the vertex reflection is floored at |Γ| ≥ 1/3*) binds **any** lossless, reciprocal, C₃-symmetric vertex network of **any** internal complexity — stubs, finite junction volumes, and resonant shunt branches included. No reciprocal vertex model can present a matched (reflectionless) bypass, so the escape hatch C2 left open is **provably dead across the ENTIRE lossless reciprocal C₃ vertex class**. This is a classic result, **not a new derivation**, and it stands on its own (no unmerged branch is cited). The derivation already frames the vertex as exactly this object — a "symmetric lossless reciprocal 3-port junction … fixed by symmetry up to **two** eigen-reflections" (derivation §2, `2026-07-10_x37-junction-parasitics_derivation.md:37`) — so the theorem applies verbatim to the shipped model and to every fuller *reciprocal* extension of it. **The one surviving escape class is NON-RECIPROCITY:** matched lossless 3-ports *do* exist non-reciprocally — the ideal circulator `S = [[0,0,1],[1,0,0],[0,1,0]]` is unitary, C₃-symmetric, and reflectionless (`S₁₁=0`). A gyrotropic/circulator vertex is the **sole** remaining way a junction could route around the ceiling. **Canon note (named, NOT asserted — PENDING-GRANT):** Axiom-1's srs chirality (right-handed `I4₁32`, `axiom-definitions.md:16`) is **parity-breaking**, but a circulator additionally requires a **time-reversal (T)-breaking** bias; whether the vacuum supplies one is a PENDING-GRANT walk question. The circulator reading is **named as the sole surviving escape class, not asserted here.** **Net scope:** C2's "no lift" becomes **"no lift and no matching bypass within the entire lossless reciprocal C₃ vertex class; the only escape class = non-reciprocity (PENDING-GRANT)."**
...
- **The circuit FORM itself** (positive shunt + positive series) is the leading-order Marcuvitz lumped equivalent, not a from-geometry derivation (C1); a fuller vertex model (evanescent-mode / finite-volume resonant branch) is the named model-fidelity follow-on that could reopen the lift question (C2) — **but only NON-RECIPROCALLY: any reciprocal reopening is theorem-dead across the whole lossless reciprocal C₃ vertex class (see Correction Log C8, 2026-07-10 — matched-lossless-3-port theorem).**
