...
- **Discriminator (ave-discrimination-check):** is it a substrate-vacuum charge-pump (AVE-distinct) or does it reduce to ordinary plasma rectification / radiation pressure? The AVE-distinct signature: the pumped momentum scales with the **substrate** loss R≈α and the per-node yield field, not with a material's properties.
