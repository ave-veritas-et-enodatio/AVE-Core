...
> **⚠ FALSIFIED by Stage-1 (Outcome C, 2026-06-09, `rectifier-stage1-biased-diode_result.md`):** the "one device, four observables" claim above is **wrong**. Rectification (directed momentum) requires **Z≠Z₀** (asymmetric diode → CHROMATIC, n<1 plasma lens); an achromatic gravity lens requires **Z=Z₀** (symmetric loading). **One element cannot be both.** Thrust (asymmetric/plasma) and gravity/lens/time-dilation (symmetric/achromatic) are **different symmetry branches**. The single biased diode is a real but *mundane plasma* rectifier — not engineered gravity. A symmetric engineered-gravity device would produce no reaction thrust; a cascade/taper "rescue" must separate the two branches (and they're mutually exclusive per element). The §6a chromaticity discriminator worked exactly as designed — it caught this.
