...
- new: "Promote the Op21 multi-mode generalization (canonical at `theorem-3-1-q-factor.md` §Op21) from a paragraph-length statement to a fully-derived canonical leaf, formalizing the Nyquist-cell-count additivity at saturation boundary as the substrate-mechanism path for the three-Λ assembly. (Phase 3-A2 result: Schur orthogonality of substrate-symmetry-group irreps is NOT the load-bearing theorem; mode-counting at saturation boundary IS. Closure is future workstream — see this result doc.)"
