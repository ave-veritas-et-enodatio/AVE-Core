...
>
> **The reimplementation lands on v1's root.** G6 measured `6.8032e-07` relative against the v1 shipped comparator — an independent **re-encoding** of the operator, sharing no line of code with either predecessor but **transcribing v2.1's algebra**, agreeing with a *different-in-kind* instrument. **"Different in kind" is earned against v1 (G6), not against v2.1** — see §3.3's disclosure. FT-6 shows the same gate reads `5.8722e-04` under a `1e-3` coefficient corruption, so it is catching transcription error rather than rubber-stamping a known agreement.
>
...
**⚑ ADDED 2026-08-03 (post-review) — and the artifact's drift is worse than "wandering".** The `0.16191` figure is the pair `n = 48` against `n = 80`, and at the `n = 80` rung the polished centre's **real part collapses onto the imaginary axis** (shipped JSON, `self_tests.FT_5.artifact_rows[3].polished`). **The artifact does not merely move — at that order it stops being a resonance at all**, which strengthens rather than softens the pseudo-spectrum reading below. At `n = 96` the polish fails outright and the row falls back to the raw seed with count `0`. **That fallback is a conservatism of the routine, and it never triggers in `G5` proper:** all five `G5` rows carry a non-null polished centre, so `[1, 1, 1, 1, 1]` is measured around polished roots at every order, never around an unpolished seed.
...
> **⚑ DISCLOSED 2026-08-03 (post-review) — HOW INDEPENDENT THIS REIMPLEMENTATION IS, STATED HONESTLY IN BOTH DIRECTIONS.** *"Independent"* here means **no shared line of code**; it does **not** mean an independent derivation of the operator. **This lane transcribed v2.1's algebra**, with attribution comments at every transcription site (`[xcribe v2.1 ...]`), and the arithmetic shows it: FT-2's `4.4038e-04` and FT-8's `6.0137e-07` are **identical to v2.1's measurements of the identical mutations at the precision both docs quote** — the frozen prereg says so in advance at `research/2026-08-03_coldq-pole-v2.2-root_prereg-FROZEN.md:337` and `:343` — and G7(a)'s `0.28424` likewise. **Two encodings of the SAME transcribed algebra agreeing is a re-encoding check, not an independent-derivation check.** What it catches is exactly what FT-6 demonstrates it catches — **transcription and coding error** — and that is worth having, but it is not evidence that the algebra itself is right. **The one comparison in this lane that IS different in kind is `G6`**, against v1's `6.8032e-07`: a different discretization, a different variable set, a different solver. **The PR body's phrase "an independent transcription sharing no line of code" is corrected to this statement.**
...
> **⚑ DISCLOSED 2026-08-03 (post-review) — FT-7's TWO ZEROS ARE A LITERAL CODE-PATH IDENTITY, AND THAT IS WEAKER THAN THE PREREG CLAIMED.** The prereg's §6 FT-7 row says the null mutations *"are exact identities and must return `0` up to arithmetic"*. **They are stronger than that and therefore weaker as evidence: they are the SAME CALL.** In the driver, `spin1_wall=False` is the **default** of `graded_matrices` / `graded_matrices_mp`, so FT-7(a)'s `root(N_PRIMARY, spin1_wall=False)` is byte-for-byte the call that produced `Ω_*` itself; and FT-7(b) divides `strain_to_kinetic(N_PRIMARY, om_c, psi, w2)` by `R2`, which is the value of that identical call three lines above. **Neither is "an identity up to arithmetic" — no arithmetic differs, so `0.0` is forced, and the two zeros cannot be anything else.** What FT-7 therefore demonstrates is **determinism and purity of the call path — which is `G9`'s job** — and **not** that `G7`'s discriminator would return small on a *differently-coded* null. **The prereg's §7.6 reachability claim is over-stated for this one self-test**; verbatim at `research/2026-08-03_coldq-pole-v2.2-root_prereg-FROZEN.md:430`: *"`BIN-F-ROOT` is reachable **and is demonstrated reachable every run**: FT-0…FT-10 each drive an actual gate into its failing state (FT-7 by the reverse construction, stated as such)."* **FT-7 drives nothing; it re-evaluates the unmutated configuration.** The frozen prereg is byte-untouched and this is disclosed rather than edited. **`G7` itself is unaffected** — its two measured discriminations, `0.28424` and `0.19697`, come from genuinely different specifications and sit more than two orders outside the frozen `1e-3`. **The fix — a null coded by an independent path rather than by the default argument — is routed to v2.3 (confirmed dispatched), not attempted here.**
...
> **⚑ DISCLOSED 2026-08-03 (post-review) — THE FROZEN PREREG'S FT-8 NON-VACUITY ARITHMETIC IS WRONG, AND THE PREREG'S "MACHINE-CHECKED" CLAIM DOES NOT REACH IT.** The prereg's §6 FT-8 row at `research/2026-08-03_coldq-pole-v2.2-root_prereg-FROZEN.md:343` argues the mutation `A → A·(1 + 1e-6·(x_sat − 7)/7)` is non-vacuous because *"the perturbation is identically zero at `x_sat = 7` and `±1.43e-06` relative at `x_sat = 5, 11`"*. **Both halves of that numeral are wrong.** The perturbation is `1e-6·(5 − 7)/7 = ` `-2.857143e-07` at `x_sat = 5` and `1e-6·(11 − 7)/7 = ` `5.714286e-07` at `x_sat = 11` — **not symmetric, and roughly `5×` and `2.5×` smaller than quoted.** **The self-test's conclusion is unaffected and if anything conservative:** the mutation is non-vacuous for the reason the row gives — it vanishes at `x_sat = 7` and does not vanish at `x_sat = 5` or `x_sat = 11` — and FT-8 in fact fired at `6.0137e-07`, three orders above its `1e-9` threshold, on the *true* (smaller) perturbation. **Why it survived to here:** the gating number check scans **this result document only** (`DOC` in `research/drivers/coldq_pole_v2p2_root_number_check.py`), never the prereg, so the prereg's §9 disclosure that its pre-freeze arithmetic *"is reproduced by the driver, and is machine-checked by the number check"* **is true of §4.3's and §7.3's numbers, which the driver recomputes, and NOT of §6's non-vacuity algebra, which nothing recomputes.** **The prereg is frozen and byte-untouched; this is disclosed, not edited.**
...
8. **⚑ FLAG-7 — the Makefile contact with the concurrent #854 branch is a REAL MERGE CONFLICT.** This lane's gating number check is wired as its **own** target placed away from #854's insertion points, and **that placement worked**: both recipe bodies auto-merge and **no `research/` or `_orchestration/` file conflicts**. But **three same-point edits collide** — the `.PHONY` list, the `help:` echo block and the `verify:` prerequisite list. **Measured: `git merge-tree --write-tree` between the two branches exits `1` with `CONFLICT (content): Merge conflict in Makefile`; a three-way merge from the true base `583d43dd` produces three conflict hunks.** **Whoever merges second rebases**; both resolutions are mechanical (keep both target names on each list, keep both echo lines). No physics, gate or result depends on it. (🔴 **CORRECTED 2026-08-03, post-review.** Original text, verbatim: *"two single-line list appends (`.PHONY` and the `verify:` prerequisite list) are lines the #854 branch also appends to. **Append-only textual contact on two lines, disclosed before either lane merges, for the orchestrator to sequence.**"* **"Append-only" is the wrong word for a same-line edit, the count was two rather than three, and the `help:` echo contact was not disclosed at all.**)
