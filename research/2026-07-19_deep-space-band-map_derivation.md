...
> **🔴 LEDGER ROW UPDATE (2026-07-20, Rule-12 KEEP-BOTH; the `v_crit/c_ch = 2/π` row above preserved verbatim).** The drag-onset ratio's substrate-native value is now DERIVED (`research/2026-07-20_jomega-derivation_result.md` §9, merged #751): the row should read **`v_crit/c_ch = 0.80` (srs 3D arccos) / `1.0` (1D chain)**, FORM `[derived]` arccos-map min-`v_p` (the "arccos-map value OPEN" qualifier is now closed), VALUE dimensionless / `ℓ_node`-free, class **manifestation**. The cosine-branch `2/π` is superseded (it does not survive the arccos model switch). The `√2`, `√(10/3)` inter-channel ratios are untouched.
