...
> **🔴 ARCCOS DRAG-ONSET UPDATE (2026-07-20, Rule-12 KEEP-BOTH; the cosine-branch `2/π` boxed above is PRESERVED verbatim).** The R2 review-repair note above left the substrate-native (arccos-TL) value of this ratio marked **OPEN**. It has since been **DERIVED** on the corpus-adjudicated arccos map (`research/2026-07-20_jomega-derivation_result.md` §9, merged #751; batched re-derivation on `srs-band-structure.md` `clm-bnd5rq`):  <!-- rule12-freeze: base=1a34f1d5cc1c8cb20342dcfdfe96bf6f321ec9e4 region=above offset=0 lines=113 bytes=15880 sha256=5a77889d1b1ea2698b49eb6fde1de99332c5ab9acde85adb47218e406a916ebf -->
> - **srs 3D acoustic arccos:** `v_{p,min}/c₀ = 0.80` (per-direction min along ⟨100⟩/⟨110⟩; DERIVED, dimensionless, `ℓ_node`-free).
> - **1D-chain arccos (z=2):** `v_{p,min}/c₀ = 1.0000` EXACTLY — `arccos(cos kℓ)=kℓ` is perfectly dispersionless ⇒ **no** vacuum-Cherenkov threshold below `c` at all.
> - **The cosine-branch `2/π ≈ 0.637` does NOT survive the model switch** — the arccos acoustic branch is far more linear (the substrate-native `0.80` is ≈26% HIGHER than the cosine `2/π`). So the AVE-distinct drag-onset ratio is **`0.80` (srs) / `1.0` (chain)**, not `2/π`. Per-channel: multiply by `√2` for the bulk channel (`0.80√2 ≈ 1.13`, i.e.\ dispersion pushes bulk drag-onset above `c` on the srs chain). The deep-space NULL (§3.3, §4) is **unaffected** — deep-space matter is `v ~ 10⁻⁴c` (≪ any of these) AND bandlimited.
...
- **🔴 VALUE SUPERSEDED (2026-07-20, Rule-12 KEEP-BOTH; the `2/π` heading + bullets above preserved verbatim).** The discriminator ratio is the substrate-native **arccos** value, not the cosine-branch `2/π`: **`v_crit/c_ch = 0.80` (srs 3D arccos) / `1.0` (1D-chain arccos, dispersionless — no threshold below `c`)**, per the batched re-derivation `research/2026-07-20_jomega-derivation_result.md` §9 (merged #751), on the corpus-adjudicated `srs-band-structure.md` (`clm-bnd5rq`). The cosine `2/π ≈ 0.637` does NOT survive the model switch (the arccos acoustic branch is far more linear). Everything else in this D4 row is unchanged (still `[derived]`, dimensionless, lattice-universal, protected/unreachable at deep-space scales); only the numeric value is relabeled `0.80`/`1.0`.  <!-- rule12-freeze: base=1a34f1d5cc1c8cb20342dcfdfe96bf6f321ec9e4 region=above offset=0 lines=88 bytes=12362 sha256=ab543d71b316df2e6af281296f5b03c14be4b2d616fce40efe1af88b36e8fe47 -->
...
> **🔴 LEDGER ROW UPDATE (2026-07-20, Rule-12 KEEP-BOTH; the `v_crit/c_ch = 2/π` row above preserved verbatim).** The drag-onset ratio's substrate-native value is now DERIVED (`research/2026-07-20_jomega-derivation_result.md` §9, merged #751): the row should read **`v_crit/c_ch = 0.80` (srs 3D arccos) / `1.0` (1D chain)**, FORM `[derived]` arccos-map min-`v_p` (the "arccos-map value OPEN" qualifier is now closed), VALUE dimensionless / `ℓ_node`-free, class **manifestation**. The cosine-branch `2/π` is superseded (it does not survive the arccos model switch). The `√2`, `√(10/3)` inter-channel ratios are untouched.  <!-- rule12-freeze: base=1a34f1d5cc1c8cb20342dcfdfe96bf6f321ec9e4 region=above offset=0 lines=29 bytes=3294 sha256=4106db7c495234b2d6e5db2201c49e5617f1a66a281558facd40279061191e28 -->
