...
★ **Correction, 2026-08-03 (re-audit finding 2) — that figure is PER-CELL, and the net is smaller still because the two errors cancel.** The `~10⁻¹⁴` above is the contribution of *one* defective cell to the correlation cycle-mean (measured: 2.73×10⁻¹⁴, ≈2.4×10⁻¹² in C₂). It is **not the net**. The two endpoint errors are **equal and opposite** (`+1.5708×10⁻⁵` at index 0, `−1.5708×10⁻⁵` at index −1) and, after the pre-repair `np.roll` by `shift_idx`, land on **adjacent** post-roll indices (`k` and `k−1`), where the kernel-asymmetry weight differs only by O(dt) — so they very nearly cancel. **Measured net** (δ = −3α/2, `n_t` = 2×10⁶, against a wrap-aware central-difference reference): **1.301×10⁻¹⁸ in the correlation / 1.135×10⁻¹⁶ in C₂** — **≈11.4 orders below** the 3.19×10⁻⁵ (97 ppm) C₂ truncation defect, not 9. (Symmetric case: 1.735×10⁻¹⁸ / 1.513×10⁻¹⁶.) The conclusion is unchanged and strengthened: numerically irrelevant, same failure mode.
...
- **§9's classification is unaffected** — and §9 `:126`'s own concession is now load-bearing beyond its original scope: *"the τ_retard = 1/ω_C choice is geometrically **asserted**, not derived."* ★ **(markup normalized — 2026-08-03: `:126` italicizes `*asserted*`; it is promoted to bold here, and the sentence does not end there in the source — it continues *" (the τ-scan in the driver shows…"*. The wording is byte-exact; the emphasis and the terminal period are this addendum's.)** With `dC₂/dτ = −11.4555`, defending even a 60 ppm label on C₂ requires τ known to **1.7 ppm**. No ppm-level label on any C₂ value in this corpus is defensible while τ is asserted.
