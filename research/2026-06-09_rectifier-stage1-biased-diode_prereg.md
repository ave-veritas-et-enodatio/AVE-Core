...
- **Pre-committed lean:** the bias makes ∮ directed ≠ 0 *possible* (unlike B). Honest expectation (vacuum-engineer / ledger frame): a **real charge-pump — ledger closes**, directed momentum **modest** (radiation-pressure-class), **paid for by the pump+bias work**, NOT free thrust, NOT over-unity. The live question is whether it's **AVE-distinct** (scales with substrate R≈α + per-node yield) or reduces to ordinary plasma rectification.

## 6. Discriminating outcomes
- **A — REAL CHARGE-PUMP (AVE-distinct):** biased leaky diode nets directed momentum; **ledger closes with margin** (W_in > W_out; the surplus = the directed work); the directed output **scales with the substrate loss R≈α and the per-node yield field**, not material properties → a genuine vacuum reaction-drive. The chord: a pump paid for honestly.
...
- **Deliverables:** `2026-06-09_rectifier-stage1-biased-diode_result.md` (A/B/C + the (V,Q) loop + ledger numbers + the **bias sweep** + DERIVED/VERIFIED/BLOCKED + figures per engineering-rigor); driver (biased leaky varactor diode + AC pump + (V,Q) loop + ponderomotive momentum + ledger + bias sweep + savefig). Own implementor branch off this one; do NOT push/merge. **No thrust claim unless A with a closing, AVE-distinct ledger.**
