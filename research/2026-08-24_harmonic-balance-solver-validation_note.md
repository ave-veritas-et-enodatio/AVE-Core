...
with the S-field a self-consistent unknown read from the DP-1 cycle-average envelope of the tone phasors (default C-state arm: A_b² = Σ_m |v_m,b|²/2 per yield²; the cross-tone COMBINE rule is derived — distinct canonical tone lines average out — while the normalization arm is an OPEN fork the module supports on both sides, see the caveats). No time axis; no damping device; Ax3 untouched by construction — the graded scatter conserves the Y-weighted energy exactly (tested to 1e-12), and the only sink is a declared matched termination (a boundary condition, checkpoint-10 class, never a bulk loss term). Source-idle machinery measures scaffold-removability: source amplitude, scaffold exchange, the autonomous defect r_auto = ‖e^{iθ}v − M_full v‖/‖v‖, and the power ledger — all computed, thresholds always caller-declared.
...
- **OPEN — the envelope-normalization fork (G2 must freeze it).** Canon holds two envelope forms: the DP-1 C-state projection row (`A²_local = ΣV_inc²/V_SNAP²`) and the DP-3 R2-fix full reactive tank (`A²_V = (V_inc² + Φ_link²/(LC))/V_SNAP²`, substrate-perspective-electron.md:85-87). At fixed v_norm they differ by **exactly √2 in A** — an algebraic identity via `|v_f+v_b|² + |v_f−v_b|² ≡ 2(|v_f|²+|v_b|²)`, content-independent, receipted in the test suite. Which arm feeds the Op2 kernel argument is an OPEN adjudication; the module supports BOTH (`envelope_mode = "c-state" | "full-tank"`), defaults to the C-state arm as PROPOSED, and asserts no resolution. None of this stage's gate receipts depend on the choice (gates 1–2 impose A externally; gate 3 never evaluates the envelope) — it lands squarely on any P2 self-consistent run, which is exactly where the G2 prereg freezes it.
