...
- **All 13 points PASS**, max |Γ_solver − Γ_measured| = **0.00129** (at A = 0.99), a factor ~8 inside the 0.010 tolerance floor; mid-grid deviations are (2–7)e-4. Γ phases 2.975–2.988 rad (≈ π, the polarity-inverted class — matching the measured sign convention at every point). De-embed (two-port model) residuals ≤ 4e-8.
...
- **Termination artifact (measured, then de-embedded).** Bond-matched cuts reflect O(10%) of the Bloch wave; all Γ receipts go through the multi-load de-embedding. A single-load reading from this instrument is NOT calibrated — the module docstring says so.
- **Fit-region discipline.** Traveling-wave fits exclude one cell around interfaces and terminations (evanescent margin); fit residuals are receipts in the JSON.
...
- **D1 (2026-08-24).** None yet — this section exists so any post-run departure lands as a dated entry, never an edit of the run of record.
