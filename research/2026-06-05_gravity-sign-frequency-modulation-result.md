...
**TENSION FLAGGED (flag-don't-fix — do NOT silently reconcile).** The brief's H3 premise that *static external E loads C only / μ_local = μ₀ under DC* is in **direct tension with the canonical Ax 4 small-signal modulation** (C3/C4, CLAUDE.md:58,60), which states that a DC operating-point bias scales **BOTH** μ and ε **symmetrically**:
...
- **(R-a) Regime distinction.** The canonical "DC bias scales both" (C4) is the **per-node operating-point shift A₀** that requires facility-class fields (~8×10¹⁶ V/m to reach appreciable per-node A₀; CLAUDE.md:60). The bench falsification tests (C12) operate at **large-signal apparatus voltages** that drive the *capacitive* sector to rupture (V_yield ≈ 43.65 kV across the apparatus geometry) without a matching B-drive — genuinely **asymmetric** in that regime → Z-step → reflection. Under R-a, the bench (asymmetric, large-signal, C-sector rupture) and gravity (symmetric, both-sector soliton/operating-point) are **consistent** because they live in different drive regimes. This is the reading that makes H3 PASS.
- **(R-b) Genuine inconsistency.** If "static external E loads C only" is meant as a *small-signal* statement, it contradicts C4 head-on (C4 says small-signal DC bias scales both). Under R-b, the H3 bench premise as worded is not canonical.

**H3 verdict: PASS under R-a (the natural regime reading); the C-only-vs-both small-signal wording is FLAGGED.** The asymmetric-bench / symmetric-gravity *consistency* holds (canonical Z taxonomy + soliton-loads-both). The specific "μ_local = μ₀ under DC / loads C only" phrasing needs Grant to confirm it is the large-signal asymmetric-apparatus regime (R-a), not a small-signal claim that would collide with C4. **Not reconciled in this session.**
...
- **H2-FLAG (factor-2 mechanism — surfaced, not a walk-back).** The reactance-counting {1/S,1/√S} factor-2 vs canonical 2/7:1/7 Poisson factor-2: surfaced for Grant's framing call (see pre-test-physics-check question above). NOT a canonical edit; the Poisson mechanism stands unchanged.

- **H3-FLAG (bench small-signal wording).** "static external E loads C only / μ_local = μ₀ under DC" vs canonical C4 (DC bias scales both, SYM). PASS under regime-reading R-a; surfaced for Grant to confirm R-a (large-signal asymmetric apparatus) vs R-b (small-signal, would collide with C4).
