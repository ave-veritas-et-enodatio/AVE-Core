...
The parasitic exists only because `d > 0`. *[Dated correction 2026-07-10, PR #616 review R6: the original clause here — "**canon fixes no transverse bond scale** (constants.py has only ℓ_node and the larger ℓ_c=√6·ℓ_node; no bond radius, core radius, or filling fraction)" — was **factually wrong**. Canon DOES carry transverse scales: a **tube radius `ℓ_node/(2π)`** (constants.py:76, the electron minimal-energy loop) and a **core-tube thickness `d ≡ 1 ℓ_node`** (constants.py:189, the (2,3) trefoil on the discrete grid). Both are **soliton/core-sector** objects; adopting either as the bare vacuum bond's extent would be a **sector-ownership crosswire**. So the correct statement is: the bare bond's transverse extent is not fixed **without a sector-ownership ruling**.]* Consequences:
...
- **Three candidate closures for `f`** (sector-ownership question, PENDING-GRANT): (a) the tube radius → `f = 1/(2π) ≈ 0.159` (right at `f_crit≈0.184`); (b) the core-tube thickness → `f ≈ 1`; (c) neither applies to the bare bond (extent stays an import).
- **Sweep:** `g_scalar(f)` over `f ∈ [0, 0.5]`, sensitivity `dg/df` reported first-class. With `s_L = s_C = 1` the **shipped exact** connected-band ceiling drops **3.8 % at f=0.05, 9.0 % at f=0.1, 16.4 % at f=0.2, 31.4 % at f=0.5** (swing ≫ the 10 % branch-(iii) threshold). *[Dated correction 2026-07-10, PR #616 review R2: the pre-driver numbers written here first — "~18 % at f=0.2, ~38 % at f=0.5" — were the linear-anchor / early wrong-root estimates; the shipped exact connected-band solver gives 16.4 % / 31.4 %. Reconciled to the shipped values.]* **This is the load-bearing finding: the ceiling is extent-dominated ⇒ the junction question is NOT closable without a sector-ownership ruling for the bare-bond extent** — which is exactly why X36 had to install one. The FORM is derived; the MAGNITUDE is not canon-determined.
