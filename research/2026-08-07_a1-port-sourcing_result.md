# A1-PORT SOURCING ADJUDICATION — RESULT

**Date:** 2026-08-07 · **Branch:** `research/a1-port-sourcing` · **Base:** `origin/main` @ `91a910f8`
**Frozen prereg:** [`2026-08-07_a1-port-sourcing_prereg-FROZEN.md`](2026-08-07_a1-port-sourcing_prereg-FROZEN.md),
committed ALONE + pushed at `05803c2a` before any derivation content existed on this branch
(freeze-by-push). **Lane brief:** [`_orchestration/2026-08-06_a1-port-sourcing-brief.md`](../_orchestration/2026-08-06_a1-port-sourcing-brief.md).
**Class:** DERIVATION-ONLY (Tier-2 at adjudication). Engine byte-untouched; NO engine runs (the
stencil-lens fence held). **Mints no `clm-`/`def-`; edits no KB leaf, register, ledger, or ruling;
changes no solidity.** All propagation ROUTED.

---

## REGIME HEADER (restated at the point of reading; SVA §0 of the prereg governs)

**MODE** — compact binary (HT B1913+16 / J0737-3039A/B) as a source; contrast = observed `Ṗ_b`
vs the GR shear-quadrupole. **REGIME** — far field Regime-I cold crystalline linear (S=1);
source region declared separately (NS interior near-yield; exterior static bias falling as the
receipted profile). **PHASE-STATE** — cold-reactive far field (radiation is a legal Ax3 port).
**SECTOR** — source owns A1 dilatation content; observed GW = T2 shear; the two-variable split
(operating-point bias `A₀(r)` vs dynamical displacement `u`) is carried through every step.
...
> object its own context names — the EM-longitudinal `∇·A` (curl-only Lagrangian ⇒ Gauss-killed;
> no longitudinal photon; the Goldhaber–Nieto evasion is intact and this lane does not touch it)
> — and FALSE as a statement about the mechanical A1 dilatation `∇·u`, which is gapless and
> radiative (#761, LC-1, this lane). The map line is a fossil of the true EM-sector claim,
> written before the mechanical channel was resolved; the `def-9a4f07` identification of the two
> objects is the remaining sector-ownership adjudication (Grant; inherited from LC-1 §10).
>
...
## §1 — G-ENUM: the receipted-coupling enumeration (the brief's frozen question, answered per candidate)

Every candidate matter→P-branch source coupling found at HEAD, tagged per the frozen grammar.
Load-bearing quotes verified two-method: corpus-grep sweep (background agent, patterns named
per claim) + first-party `sed`/`grep` reads at worktree HEAD `91a910f8` this session.

| # | Candidate (prereg §4 letter) | Provenance tag | Receipt (file:line, verified verbatim) | Multipole-ladder disposition |
|---|---|---|---|---|
| 1 | Static Komar elliptic source (C-c static side) | **RECEIPTED — Grant-ruled + code, STATIC only, no `clm-`** | `src/ave/gravity/backreaction.py:6-17`: `−∇·[(c⁴/7G)·D(A)·∇ε₁₁] = T₀₀^src`, `T₀₀^src = T₀₀^matter·√S(A)` (*"X44 / Grant RULED (c) 2026-07-12 — Komar / redshift weight"*) | DC bias only; cannot radiate by itself (the brief's own static-mass clause). Its role here: it is the receipted **amplitude anchor** of the static sector — but the bound of §3 does not even need it. |
| 2 | Ponderomotive envelope quadrupole (C-b) | **RECEIPTED structure (def-pndenv, status: proposed) / coefficient ASSUMED** | `vocabulary-register.md:1083-1089`, verbatim: *"the SOURCE STRUCTURE is banked per merged #767; the coupling COEFFICIENT is q1-inherited/ASSUMED, NOT derived"* | Drives the P-branch at n=2 (`Q_ij ∝ ∫x_ix_j|A|²` at 2Ω). The assumed coefficient is exactly what §3 replaces with the shear anchor. |
| 3 | #761 delta-body-force source (C-a) | **RECEIPTED structure / normalization assumed** (conditional on the held sector identity — #761 §6.1: *"NOT driver-validated … held"*) | `2026-07-20_mechanical-commonmode-derivation_result.md` §1.2 | n=2; monopole/dipole killed by `Ṁ=0`, `Ẍ_cm=0` (reproduced §2). |
| 4 | Static exterior profile promoted to time-dependence (C-c dynamic side) | **RECEIPTED static / promotion NOT separately receipted** | exterior receipted as constitutive SYM grading (`einstein-field-equation.md:37-46`, clm-8nkvwy) with `∇·u = 0` deviatoric strain (`forkw-kernel-keying_derivation.md:49` context; `boundary-strain-amplitude_result.md:134` context: *"The exterior is Lamé-∇·u=0"*) | The static `∇·u = 0` is a **DC identity**, not a radiative-order conservation law (§4b). The exterior **deviatoric** texture in motion is a receipted moving stress structure — it is the shear-anchor's source side. |
| 5 | Microscopic pluck operator (C-e) | **ABSENT** | `axiom-register.md:193` open flag; two-method: `grep -rn "pluck"` (GNU grep, fixed string) on `manuscript/ave-kb/claim-quality.md` + `manuscript/ave-kb/*/claim-quality.md` → 0 hits; `grep -rniE "source-coupling\|source operator\|matter.to.substrate"` (GNU grep ERE) over `manuscript/ave-kb/` → only def-pndenv (+1 false positive, 1 table header). The one resolution attempt (auto-resonate, Grant walk 2026-07-04) carries a 🔴 SUPERSEDED verdict (`resonant-tension-law_result.md:3-5`) and its own line: the pluck fork *"is NOT resolved by this arc"* (`:286`). | Not needed for the bound (§3); heads the missing-piece list (§7). |
| 6 | Kinetic stress `ρv_iv_j` (C-d) | **ABSENT as a source** | no corpus doc treats it as a gravitating substrate source (`grep -rniE "kinetic stress\|momentum flux\|v_i v_j\|Reynolds stress"` — ledger-only hits); the rank-2 `T^ij` register is chartered-missing (`tij-x44b_CHARTER.md:28,76-78`); Grant's P0 framing runs against it (`mass-sector-two-body-scattering_T0i_prereg.md:89`) | Excluded from the enumeration as unreceipted; noted §7. |
| 7 | Cosserat rotational outbound (fence line) | **RECEIPTED-CLOSED sub-MeV** | `approach-leak_result.md:77-82`, verbatim: *"the transfer function is REAL, the rotational branch supplies no radiation resistance, and `⟨P⟩_period = 0` exactly"* — a theorem with a measured input (gap strictly positive); MeV-scale explicitly out of scope (`:446-448`, v2 `:384,:535`) | Closes the §4a antisymmetric loophole in-band. **The fence is checked, not crossed.** |
| 8 | EM route via TKI transducer | **BLOCKED** | def-tk1xfm ceiling (`device-circuit-models.md:216`: transduces units, *"does NOT supply a derived coupling MECHANISM"*); charge-sector no-RHS-source no-go (`the-sourced-charge-no-go-cascade.md:81`) | Excluded. |
| 9 | Constituent-cage image-cancellation (C-f) | **OPEN FORK — a modifier, not a coupling** | #770 re-banked verdict: *"WALL-CLASS / RAIL-DEPTH-CONDITIONAL → REOPENED … the BIN-2 route is LIVE again at deep rail. The fork #767 left open REMAINS OPEN"*; advanced by seven post-07-20 lanes (#770→#775→Fork-W→#779→RVE→vessel-state→#802), resolved by none (two-method hit-counts in the sweep record) | Modulates ONLY the isotropic (l=0) radiative moment — a non-negative add-on to the §3 floor (the orthogonality lemma). Inherited OPEN; not resolved here; **cannot rescue** (§3.3). |

**G-NEGCTRL (the enumeration grammar's positive control): PASS.** The same grammar applied to the
T2-shear channel returns its receipted coupling: canon carries the binary→shear-quadrupole chain
at CONSISTENCY-IMPORT grade (the HT `Ṗ_b` reproduction imports Peters–Mathews,
`08_gravitational_waves.tex`; envelope §5 gate: *"rate inherited"*), i.e. the grammar correctly
sees a receipted-consistency coupling with an imported rate on the shear side — it is not blind.
Note what the control exposes: **the shear channel's microscopic sourcing is exactly as
underived as the bulk's** (same pluck gap). That symmetry is what makes the §3 anchor honest:
it consumes only the OBSERVED shear flux, not a derived shear coupling.

## §2 — G-LADDER: the multipole ladder on the receipted structure

Reproducing the receipted kills before any new step (gate condition), then the surviving order:

- **n = 0 (monopole).** The compression monopole is `∫∇·u ∝ M_total` (receipted form, envelope
  §2 Fork-F3): `Ṁ = 0` by total-mass conservation ⇒ no time variation ⇒ dead. Reproduced. ✓
- **n = 1 (dipole).** `∫x ∇·u ∝ M X_cm`; `Ẍ_cm = 0` by momentum conservation ⇒ dead.
  Reproduced. ✓
- **n = 2 (quadrupole).** The coupling integral `Q_ij ∝ ∫x_i x_j s(x,t)` has a traceless part
  rotating at `2Ω` — `⃛Q^{TL} ≠ 0` (#761 §2: the tracelessness rescue is dead; a
  scalar/longitudinal channel radiates via the traceless second moment). **No receipted
  conservation law, coupling-integral identity, or constraint structure kills it:** the §1
  enumeration found none, the #761 structural block forecloses the elliptic-slaving family
  (`K ≠ 0` ⇒ hyperbolic), and the two candidate structures examined in §4 (static `∇·u = 0`
  exterior; trace-cancellation) fail at radiative order by derivation. **The lowest
  non-vanishing multipole order is n = 2**, same order as the observed shear (`Δℓ = 0`, no
  `(Ω/ω)^{2Δℓ}` suppression).

Verdict verb per Step 3.10: the monopole/dipole kills and the quadrupole survival **re-exhibit**
receipted #761/envelope structure (DEMONSTRATED); the per-candidate provenance table (§1) and
the §3 bound are **new adjudication** (ADJUDICATED).

## §3 — ★THE SHEAR-ANCHOR BOUND (the new derivation; eliminates the assumed O(1))
...
- **R2 (#761 §1.1, verbatim `[sic]`).** *"the A1-dilatation `θ = ∇·u` is the longitudinal
  polarization of the vector displacement field — it is nonzero exactly for the P-branch and
  zero for the S-branches. It is NOT a separate scalar DOF; it is a projection of the same
  3-vector `u` the vector survey solves."* One displacement field; two projections.
- **R3 (LC-1, gates PASS).** The far field is the two-eigenspeed Christoffel structure: one
  longitudinal branch at `c_P`, one transverse pair at `c_S = c`, and the radiative row has
  exactly ONE longitudinal speed (LC-1 §8 narrowing 1: the `√2·c` object is constitutive, not a
  Bloch branch; the speed fork is dead).

### §3.2 The bound

In the far field of a medium satisfying R2+R3, a localized time-varying stress distribution
radiates displacement whose P- and S-projections are BOTH functionals of the SAME effective
moment tensor `M_ij(t)` (the far-field equivalent source; symmetric — the antisymmetric part
couples only to the rotation sector, closed in-band per §1 row 7):

$$u^P_i \propto \frac{\gamma_i\,\gamma_p\gamma_q}{\rho c_P^{3}}\,\dot M_{pq},\qquad
u^S_i \propto \frac{(\delta_{ip}-\gamma_i\gamma_p)\,\gamma_q}{\rho c_S^{3}}\,\dot M_{pq}$$

(the receipted q1 §1.1 forms — standard elastodynamic far-field radiation, seismology-anchored
...
2. **Isotropy of the partition integrals** — the `2/3` is the isotropic-medium theorem; the srs
   ...
   an O(1) modulation that never approaches the `10⁻²`–`10⁻⁴` needed to survive the comparator.
   The exact anisotropic angular integrals are NOT computed here (inherited LC-1 §12
   amendment-candidate 3 caveat; owed as a sharpening, verdict-neutral).
3. **The far-field equivalent-source description itself** (a localized stress region radiating
   into the receipted two-branch continuum) — the same description under which canon banks the
   shear-side consistency (G-NEGCTRL); rejecting it un-receipts the observed-GW story too.
...
- **(b) The static `∇·u = 0` exterior as a source-killer.** The receipted Lamé-exterior
  ...
  order (an oscillating pressurized cavity radiates P; a static one stores none — standard
  elastodynamics, same Green structure as §3). And the §3 floor does not touch this question:
  it rides the deviatoric moment alone. Checked; kills nothing.
- **(c) Elliptic slaving (the B1 family).** The receipted static solver
  (`backreaction.py`, Picard elliptic) is a STATICS design choice, not a derived constraint
  theorem; the derived structure runs the other way — `K ≠ 0` gives `∇·u` a genuine restoring
  force, so its dynamics are hyperbolic (#761 §1.3 structural block, review-grade, merged).
  Checked; foreclosed.
- **(d) Trace-cancellation of the deviatoric P-flux.** Impossible: `l = 0` vs `l = 2`
...
Frozen residuals: `δ_HT = 0.0016` (Weisberg & Huang 2016, abstract verbatim in the prereg);
`δ_DP = 1.3×10⁻⁴` (Kramer et al. 2021, abstract verbatim in the prereg). The bound, evaluated
across every receipted speed reading (arithmetic re-derivable from the symbolic forms; G-NUM):
...
3. **The rank-2 `T^ij` stress register** — chartered-missing (`tij-x44b_CHARTER.md:28`).
4. **def-pndenv's coupling coefficient** — still ASSUMED in canon; for the kill arithmetic it is
   SUPERSEDED by the anchor (which does not use it), but the def's own derivation remains owed.
5. **The exact anisotropic partition integrals** on the srs tensor (§3.4-2) — owed sharpening,
   verdict-neutral (bounded by the speed-reading spread).

## §8 — Ledger, gates, routing
...
### §8.2 Gates (UNRUN ≠ PASSED; all five frozen gates run)
...
2. **FLAG-LC1-B → auditor lane + Grant:** fossil-conflation relabel of
   ...
   `def-9a4f07` sector-ownership adjudication is Grant's (inherited from LC-1 §10). No edit
   here.
3. **The Q1 exclusion's O(1)-coupling caveat → auditor lane:** the standing ledger entry's
   *"O(1) coupling"* phrasing can now cite a DERIVED lower bound (this doc §3) instead of an
   assumption — an available strengthening, Grant/auditor-gated, not landed here.
4. **The #770 cage fork:** inherited OPEN; explicitly NOT resolved; its Grant routing stands
   unchanged. This lane adds one derived statement about it (it modulates only the l=0 add-on).
5. **Rotational fence:** checked, not triggered (§4a); nothing routed through the #897 frontier
   line; the MeV-scale rotational question stays exactly as fenced by its own docs.

---

> **Result-doc provenance.** Frozen prereg pushed ALONE at `05803c2a` (freeze-by-push) with the
> comparator retrieved from source abstracts BEFORE any flux number existed; derivation follows
> under the frozen criteria with zero deviations from the frozen protocol (the enumeration
> gained rows 6–8 beyond the prereg's C-list per the prereg's own provision: *"the corpus-grep
...
---
**DATED DISPOSITION NOTE (2026-08-09, orchestrator, per R34/R38):** this record banks
the order verdict and the re-cut as-run. The kill condition subsequently FIRED via
the screening lane's residue (PR #930, merged — the moment ratio this record's §3
anchor assumed is now DERIVED there), and the constitutive PREMISE the chain
consumed (a propagating bulk channel) is under R38 adjudication (the bound-response
lane). The R24 CONTESTED status on the §3.1 anchor premise resolves per R28's audit
(REFINE: one field, two modes, two couplings). This note changes no bin and no
verdict; it dates the record's place in the arc.
