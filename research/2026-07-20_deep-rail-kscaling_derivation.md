...
**Consequence (the crux).** At `k·r_core ~ O(1)` a deep-rail (`Γ_bulk ≈ −1`) cage is a resonant cavity, not a quasistatic Lloyd mirror. The far-field compression coupling is a RESONANCE-DOMINATED comb whose PHASE is a radiation **TROUGH AT** the cavity resonances (`k·r_core ≈ nπ`) with **PEAKS BETWEEN** — the correct signature of a high-Q cavity that TRAPS energy at resonance (a suppressed far-field ON resonance) — NOT the smooth `(k·r_core)²` envelope. (Measured phase, Leg K: the four points bracketing `π` are the four LOWEST `ρ_N`; the peaks sit at `k·r_core ≈ 1.25` and `≈ 4.76`. The specific high-Q account is a CANDIDATE — no cavity `Q`/linewidth/ring-down is measured; what the data establish is *resonance-DOMINATED / non-monotone*.) The two regimes are on OPPOSITE SIDES of the fundamental resonance:
