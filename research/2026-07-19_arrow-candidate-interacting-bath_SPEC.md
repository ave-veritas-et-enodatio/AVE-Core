...
> **🟢 RULED 2026-07-20 (RULING 3 of the 2026-07-20 ratification batch — Decision 2's pre-test-physics question answered).** Grant ratified 2026-07-20 (**Grant-verbatim, [sic]: "3 ratified"**); ruling *content* = the **orchestrator's walk, ratified in chat** — attribution split: the wording here is the ratified walk, NOT Grant-verbatim. **The port is a PHASED ARRAY, not a piston.** The real T2 sink couples via **many independent local contacts** carrying **statically-random per-contact phases** — i.e. Decision 2's option **(b) is the *physical* geometry** (a spatially-resolved, phase-carrying port). This **composes with — it does NOT contradict — Grant's earlier #734 "effectively-constant / static noise floor" aggregate ruling: piston was the *aggregate* level** (the single collective amount-channel the certified meter's scalar port sees), **array is the *port geometry*** (the mode-by-mode reality underneath). Consequences: **(1) #734's structural-inexpressibility RESCOPES to instrument-class** — the arrow question is **live on the real substrate** (a phased-array port *can* imprint the floor's per-contact phases), and is **closed at the certified scalar-port instrument** (barrier B2 is a property of *that meter's* scalar port + global rescale, not of the physics). **(2) The phase-carrying instrument build (Decision 2b's re-cert path) is LICENSED but PARKED** (the focus-check rebalance): sanctioned in principle, but **no build fires without a separate Grant go.** Decisions 1, 3, 4, 5 stay Grant's open calls; this ruling settles only the D2 *physical picture* + the build-park.
