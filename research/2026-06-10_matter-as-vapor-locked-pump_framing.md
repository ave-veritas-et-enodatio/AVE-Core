...
**The reconciliation (Grant 2026-06-10, verbatim-class):**

- **The PERSISTENT SCREENING CURRENT is the FIELD.** In Meissner-class language the standing, non-radiating circulation the broken (saturated) phase sustains — the order-parameter content of §11.1 L3, the standing `V_inc` latent store of §11.2 — is the **field**, not a port.
- **The BACK-EMF is its Faraday-induced PORT signature.** The back-EMF is what is read at the terminal when the screening field **changes** (Faraday–Lenz; zero at steady circulation). It **meters** the field; it is **not** the field (Rule 2; corr `0.117` receipt = distinct objects, `2026-06-10_bemf-feedback-smoke_result.md:79`). This is the same field/port split executed in canon by **R2** this session (`vacuum_engine.py:46,1478,1487`; `claim-quality.md:731`; `dark-wake-bemf-foc-synthesis.md:125`).
- **The Meissner analogy is bounded as Meissner-CLASS — a lens, not an identity.** It is a **picture** (hypothesis-class per novel-objects N11), useful for intuition about standing/expelled circulation in the broken phase — **NOT** a claim that the substrate IS a superconductor or that a BCS condensate maps term-for-term. _Class: lens / analogy, explicitly bounded._

**Status:** the N11 tension (Meissner field-reading **vs** the PORT-only BEMF ruling) → **RECONCILED** by the field-vs-port split above. The registry §3.8 BEMF row + the novel-objects report N11 entry carry the matching RECONCILED status (landed on the PR#176 `analysis/2026-06-10-dark-sector-nomenclature` branch). Cross-ref: `research/2026-06-10_field-symbol-registry.md` §3.8 + §5; `research/2026-06-10_novel-objects-report.md` N11.
