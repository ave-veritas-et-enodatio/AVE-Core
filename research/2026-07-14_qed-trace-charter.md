...
**Anchor / re-verification:** the grounding card was synthesized against AVE-Core HEAD `c12f2bdb` (the PR #676 merge — an ordinary ancestor of the landing HEAD, valid and well inside main's history). This charter was **re-verified at the landing HEAD `db06ba82`** (origin/main at authoring, the PR #682 merge, several merges downstream of `c12f2bdb`); every receipt below was re-grepped at `db06ba82`, which is the operative receipt basis. Line-number drift found on re-verification (the corpus moved between `c12f2bdb` and `db06ba82`) is recorded inline (see the Receipts table footnotes). QED-TRACE confirmed **absent** from the corpus by two methods at the anchor (`rg -l QED-TRACE` repo-wide → 0; grep of `_orchestration/index.md` + boards → 0); this is the founding document.

> **🔴 CORRECTION (2026-07-14, Rule-12 — orchestrator-verified).** An earlier version of this Anchor line and the §6 anchor-honesty note claimed *"`c12f2bdb` is NOT an ancestor of `db06ba82` — the card's HEAD was one merge ahead of origin/main."* **That is FALSE and is withdrawn.** Verified from the main checkout after a fresh fetch (`git merge-base --is-ancestor c12f2bdb origin/main` → true): `c12f2bdb` is the **PR #676 merge, an ordinary ancestor** of `db06ba82` (the #682 merge), several merges upstream. The false characterization was a stale/racing-fetch artifact in the authoring worktree. The re-verification at `db06ba82` **remains the operative receipt basis (unchanged and correct)**; only the "not-an-ancestor / one-merge-ahead / unmerged-or-local" framing is retracted.
...
**Anchor honesty:** the card's receipts were pinned to `c12f2bdb` (the PR #676 merge — an ordinary ancestor of `db06ba82`, several merges upstream; see the 🔴 CORRECTION at the top, which withdraws an earlier false "one merge ahead / not-an-ancestor" framing). This charter is pinned to `db06ba82` (PR #682), the operative receipt basis. The only re-verification drift found is the README/LIVING_REFERENCE line numbers (the corpus moved between the two commits; recorded ⚠ above); every verbatim fence/reconciliation anchor holds at the stated line at `db06ba82`.
