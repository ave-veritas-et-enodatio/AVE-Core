...
2. **α-emergence multi-chain status** (SUPERSEDES prior "K4-TLM α-emergence tautology (κ_chiral hardcoded)" wording — refactored per cross-repo audit 2026-05-18 + K4-TLM audit 2026-05-18; tracked in `research/2026-05-18_c8-class1-vs-class4-audit.md` §8 + closure-roadmap §0.5). The prior single-line flag conflated three structurally-distinct concerns; the κ_chiral piece (item 2c) is actually resolved. Three separate concerns:

   - **(2a) δ_strain CMB-bridge is Class 1 back-fit** (acknowledged): `constants.py:168` defines `DELTA_STRAIN = 1 - (1/ALPHA)/ALPHA_COLD_INV` — comment line 170-172 explicitly admits *"This is definitional given the way DELTA_STRAIN is computed."* The Golden Torus cold asymptote (`α⁻¹ = 4π³+π²+π ≈ 137.0363`, Chain A1, genuine Class 4) is bridged to CODATA `α⁻¹ ≈ 137.035999` via this δ_strain coefficient. Resolution path: first-principles δ_strain derivation from CMB T + lattice elasticity (G_vac + equipartition); currently postulated. KB leaf `vol1/ch8-alpha-golden-torus.md:75` already self-flags this as fitted.
   - **(2b) K4-TLM Q-factor route at 50% precision** (Chain A5): Master Equation FDTD breathing-soliton bound state v14 reaches `Λ_total = 102.8` — structurally agrees with α_cold⁻¹ = 137.036 at 50% precision but does not quantitatively close. Foreword line 106 framing corrected 2026-05-18 to drop the misleading "independently confirms" / "Both engines converge on identical p* = 8πα" language. **Resolution-path correction 2026-05-19** (supersedes the 2026-05-18 night correction which itself cited stale doc 74 follow-ups): the L3 ELECTRON-MODELING BRANCH IS IN COMPREHENSIVE CLOSURE per [doc 79 v5.2](_archive/L3_electron_soliton/79_l3_branch_closure_synthesis.md) (2026-04-29). Three-Layer Convergent Refutation across substrate-geometric (chair-ring Nyquist closure violated 65%), engine-architectural (A28 suppresses V↔B direct coupling per doc 95 §4), and standard-physics-external (far-field 1/r intermediate-regime, not point-particle per doc 95 §2). 10 pre-registered tests at engine-representable corpus GT all Mode III: R7.1 V-block N=32/N=64, R7.1 Cos-block N=32/N=64, R7.2 G-13 pair injection, Test B v2/v3 8-port spatial, path α v1/v2/v3 (V_inc/V_ref / Φ_link/ω_axial / 3D-aligned ω-vector). One structural partial positive: 100% CCW chirality consensus per path α v3 view (c) (Φ_link, |ω|) — Meissner-asymmetric mechanism (§6.7) anchored. Closure shape: *"corpus electron substrate is elsewhere (sub-ℓ_node FDTD per (i-b), OR a different scale entirely)"*. **Grant adjudicated Framework Decision (ii)** per [doc 98:5](_archive/L3_electron_soliton/98_framework_decision_ii_mass_spectrum_activation.md:5) on 2026-04-30: *"(ii) works — Framework Decision (ii) activated as next research track"* — mass spectrum / pair creation. The 50% Λ gap is thus reframed: NOT a geometric-realization gap at chair-ring + K4 + ℓ_node + v8 config, but a wrong-substrate-scale-and-config selection per the convergent refutation. Mass-spectrum direction has positive empirical anchors already on file (proton 0.002%, baryon ladder c=5-13 ±2.4%, atomic orbitals Z=1-10 ±5.5%). The 2026-05-18 ABCD-eigensolver handoff + the 2026-05-18-night retrofit doc §3 Options A/B/C/D were both proposing work that this L3 closure had already resolved; both retired (see [2026-05-18_abcd-handoff-prereg-outcome-corpus-state-retrofit.md](2026-05-18_abcd-handoff-prereg-outcome-corpus-state-retrofit.md) v2 addendum).
   - **(2c) Cosserat κ_chiral = α·κ̃** (RESOLVED 2026-05-02 per commit 6844cc0): κ̃ = 6/5 = pq/(p+q) is now exposed as a purely topological α-decoupled prefactor at `cosserat_field_3d.py:91`; the α multiplicative factor is a downstream CODATA consumer, NOT a hardcoded hidden value. This was the original "κ_chiral hardcoded" concern in the prior wording — superseded; no longer a weak spot. Cosserat κ_chiral is correctly an α-consumer, not a failed α-producer.

   **Additional cross-repo finding** (logged 2026-05-18): AVE-Core carries FIVE distinct α-derivation chains simultaneously (Chain A1 Golden Torus, A2 δ_strain bridge, A3 SI substitution at backmatter `02_full_derivation_chain.tex:272` — was A47 v17 catch site at line 864, moved not retracted, A4 `p_c/(8π)` EMT at line 75/278/877, A5 K4-TLM Q-factor). Chain A1 vs A4 dependency at SM-parameter table line 877 is unclear (does p_c derive α via A4, or does α derive from Golden Torus via A1 and p_c follow?). Foreword reconciles via `Ω_freeze → u_0* → three-route closure` promise (line 148-151) but no implementation in `constants.py` actually starts from `u_0*`. Resolution: A1-vs-A4 reconciliation pass + Ω_freeze closure implementation (multi-session research-doc queue).
