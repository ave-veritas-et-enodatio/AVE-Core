...
2. **α-emergence multi-chain status** (SUPERSEDES prior "K4-TLM α-emergence tautology (κ_chiral hardcoded)" wording — refactored per cross-repo audit 2026-05-18 + K4-TLM audit 2026-05-18; tracked in `research/2026-05-18_c8-class1-vs-class4-audit.md` §8 + closure-roadmap §0.5). The prior single-line flag conflated three structurally-distinct concerns; the κ_chiral piece (item 2c) is actually resolved. Three separate concerns:
   ...
   - **(2b) K4-TLM Q-factor route at 50% precision** (Chain A5, audit-pending): Master Equation FDTD breathing-soliton bound state v14 reaches `Λ_total = 102.8` — structurally agrees with α_cold⁻¹ = 137.036 at 50% precision but does not quantitatively close. Foreword line 106 framing corrected 2026-05-18 to drop the misleading "independently confirms" / "Both engines converge on identical p* = 8πα" language. **Resolution-path correction 2026-05-18 night**: the prior "finer-grid FDTD convergence study OR derivation of expected discrepancy" framing was stale relative to docs 71-78. The corpus-canonical state is (i) Golden Torus is topologically-quantized INPUT per [doc 03 §4.3](_archive/L3_electron_soliton/03_existence_proof.md) + [doc 72 §1.3](_archive/L3_electron_soliton/72_vacuum_impedance_design_space.md:51), NOT gradient-flow OUTPUT — F17-K Phase 5 (commits `6158465` → `2c873cf` → `4c9fbea`) empirically falsified both Cosserat-energy and coupled-S₁₁ descent's ability to land at GT; (ii) R7.1 (block Helmholtz, [doc 73](_archive/L3_electron_soliton/73_discrete_k4_tlm_lctank_operator.md) / [doc 74](_archive/L3_electron_soliton/74_r7_k4tlm_lctank_run_result.md)) returned Mode III at N=32 across all seeds, V-block comprehensive but Cos-block bottom-100-only. Actual canonical open follow-ups per [doc 74_ §4.5](_archive/L3_electron_soliton/74_r7_k4tlm_lctank_run_result.md): Cos-block comprehensive shift-invert at sigma=ω_C², hybrid V≠0 ∧ ω≠0 seed test (Round 8), larger-N sensitivity sweep (N=64/N=80), or Round 8 architectural rework. The 2026-05-18 night ABCD-eigensolver workstream handoff (also stale per same coordinate-system error) was retired before any code was written; see [2026-05-18_abcd-handoff-prereg-outcome-corpus-state-retrofit.md](2026-05-18_abcd-handoff-prereg-outcome-corpus-state-retrofit.md) §3 for next-step adjudication options pending Grant.
