...
4. **★ AND THE MODE THAT IS MISSING IS A NAMED, SPECIFIC OBJECT, NOT A VAGUE "SOME OTHER MODE".** This lane computes the **toroidal** (odd-parity, purely shear) branch. The **spheroidal** branch — even-parity, with the P and SV polarizations *coupled*, so the compressional `A1` dilatation channel and the shear channel move together — is **not built anywhere in this arc**, and it is the branch that in the elastodynamic analogue carries the lower-frequency, longer-lived `ℓ = 2` fundamental. **If a graded-cavity mode sits at GR's value, that is the most likely place for it to sit**, and this lane's `BIN-1`/`BIN-2` misses say nothing whatever about it. **This is the single most valuable unrun object named in this document**, and it is named here rather than only in the closing sentence, because a limits list that omits the specific candidate is a limits list that reads as rhetorical.
...
> **⚑ ROUTED SUCCESSOR ARGUMENT, 2026-08-03 — NOT MADE HERE, and deliberately so.** The adversarial review offered a reciprocal reading of G6 that this lane is **fenced from using**: *two differently-formulated instruments agree on `Ω` to* `6.803231574438666e-07`, *so the miss is unlikely to be a formulation artifact.* The fence is the prereg's own §2.3(1), frozen: this lane claims **no** implementation independence and makes **no** claim about any predecessor's, and G6's agreement is recorded as a **regression check**, not as corroboration — which forbids turning that same agreement around and spending it as evidence that the misses are real. **A successor that has not frozen that fence may make the argument; this lane records it as available and does not take it.** It is logged here because an argument a lane is barred from making is exactly the kind of thing that otherwise disappears.
...
>
> **THE COUNTS, CORRECTED.** The figure published with this PR was (markdown emphasis stripped): *"number check 71 sites, 60 distinct tokens, 45 registered, 26 allow-listed, 0 unregistered"*. **That figure is withdrawn as a coverage claim** — it counted a document the instrument was only half-reading. The repaired instrument, on this document as it now stands, reports **189 sites, 134 distinct tokens, 111 registered, 78 allow-listed, 0 unregistered, and all 78 registered keys exercised.** The site count rose by a factor of about two and a half, and the registered count by about two and a half, **without a single measured value changing** — which is the whole shape of the finding: the numbers were always right, and the gate that was supposed to prove they were right was not reading most of them. **These five counts are written WITHOUT back-ticks and are deliberately not registered**, for the same reason `_runtime_sec` is not: a back-ticked count would become its own site, and the checker's own output is not a value of the shipped JSON.
>
