...
## ✅ REPRODUCTION RECEIPT (2026-07-20 — riders lane; `ave-reproduction-gate`; discharges the #759-flagged genesis-24 receipt-refresh GRANT CALL, Grant-fired verbatim `[sic]` "lets do em")

**Bank BOTH (Rule 11 — the banked receipt is NOT overwritten; the committed `genesis_24_saturated_seed_results.json` still holds the 2026-06-21 `−2` C1 values, restored via `git checkout` after the re-run).** The #759 F1-verification refire (`research/2026-07-20_f1-verification-rerun_result.md`; `engine-capability-map.md` §8c.13) flagged the genesis-24 JSON receipt-refresh as an owed **GRANT CALL** and did not action it. Grant called it 2026-07-20. Reproduction gate run:

- **LOCATE.** Banked receipt: `src/scripts/vol_1_foundations/genesis_24_saturated_seed_results.json`, the `−2` Lenz C1 state generated **2026-06-21** by the sign-correction commit `5a8b16e3`. Config frozen in the JSON (`N=24, steps=40, emit_steps=60, A_photon=3.0, fracs=[0.3,0.6,0.85,0.95], eps_machine=1e-9`, canonical `V_SNAP/V_YIELD/R_I/α` from `ave.core.constants`). Seed set: the genesis-23 analytic `_seed_v_partner` / `_seed_photon` shapes — **deterministic, no RNG on the genesis-24 path** (the only engine RNG, `vacuum_engine.py` `initialize_thermal`, is off-path); bit-compare therefore applies (not the flicker/independent-run class).
- **RE-RUN.** Same driver, same config, on current **HEAD `60b30b4b`** (`PYTHONPATH=<worktree>/src`, engine imported read-only; driver byte-untouched). Engine delta since the 2026-06-21 bank: the F1 DEFECT fix (`external_z_local` True under `CoupledK4Cosserat`, `79435ee3`/`20395fe2`) + Op4-pairwise blessing (`3f67242f`) — the latter the **only** `src/ave/` commit between #759's test commit `e2e870c0` and HEAD.
- **BIT-COMPARE — verdict-class quantities ALL BIT-STABLE:** `verdict='C1'`; the full `verdict_msg` ("FALSIFIER — dE_V<=eps_machine at deep saturation: GAP-1 STRUCTURAL"); `smoke.is_C1=True`; `emission_reverse.reverses=True` (bounded, **not** the `+2`-bug detonation); `ledger.closes=False`; `ledger.L_bounded=False`; `task1_toroidal_resolution.case='C_subgate'`; `arm3_null` (`E_V=0`, `max_V_inc=0`, the genesis-23 GAP-1 reproduction); `arm5_pump_only_structure=False`; `arm4_charge_flip.any_flip=True`; `arm2_topology_null=True`; `headline_monotone=False`; `headline_deep_positive=False`.
- **DRIFT — continuous intermediates (verdict-INVARIANT):**

  | quantity | banked (`−2` C1, 5a8b16e3) | re-run (HEAD 60b30b4b) | drift |
  |---|---|---|---|
  | `smoke.dE_V` = `headline_dEV[0.85]` | `−0.170550` | `−0.147318` | +13.6% (both ≪ 0 ⇒ C1) |
  | `headline_dEV[0.95]` | `−0.238724` | `−0.212998` | +10.8% |
  | `headline_dEV[0.60]` | `−0.064000` | `−0.048685` | +23.9% |
  | `headline_dEV[0.30]` | `−0.010094` | `−0.005836` | +42.2% |
  | `smoke.E_V_arm1` | `5.730800` | `5.754031` | +0.41% |
  | `smoke.E_V_arm2` (control) | `5.901349` | `5.901349` | **bit-identical** |
  | `emission_reverse.max_V_inc_peak` | `0.403872` | `0.410493` | +1.6% (bounded ~0.4; **not** 1.08e4) |
  | `emission_reverse.E_V_peak` / `E_V_final` | `12.0769` / `5.1982` | `12.0774` / `5.2116` | +0.004% / +0.26% |
  | `ledger.L_max` / `v_secular_ratio` | `42.931` / `0.7879` | `43.000` / `0.7980` | +0.16% / +1.3% |

- **CLASSIFY = verdict-invariant DRIFT.** The `dE_V` values (an amplified difference of two ~5.9 quantities, largest fractional drift at the shallowest frac) shifted by +10.8…+42.2% but stay decisively `≤ eps_machine` — the **C1 falsifier verdict is unchanged**, and every emission/ledger/toroidal/arm classification is bit-stable. The Arm-2 control is bit-identical; `max_V_inc` stays in the bounded ~0.4 class. The **+10.8…+42.2% range matches #759's `e2e870c0` characterization exactly**, so the drift is F1-defect-fix-attributable with bit-identical to the `e2e870c0` (#759) run across all compared quantities, so the one intervening engine commit `3f67242f` is provably inert on the genesis-24 operator path (`3f67242f`). Per `ave-reproduction-gate`, a verdict-invariant drift is recorded (not Grant-routed — no bin/verdict/headline flipped); the sign-independent §8 structural finding **(2,3) does not close** stands untouched. **Receipt-refresh DISCHARGED: the banked `−2` C1 reproduces on HEAD.**
