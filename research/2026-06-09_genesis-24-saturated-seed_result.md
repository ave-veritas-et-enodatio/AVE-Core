...
- **CLASSIFY = verdict-invariant DRIFT.** The `dE_V` values (an amplified difference of two ~5.9 quantities, largest fractional drift at the shallowest frac) shifted by +10.8…+42.2% but stay decisively `≤ eps_machine` — the **C1 falsifier verdict is unchanged**, and every emission/ledger/toroidal/arm classification is bit-stable. The Arm-2 control is bit-identical; `max_V_inc` stays in the bounded ~0.4 class. The **+10.8…+42.2% range matches #759's `e2e870c0` characterization exactly**, so the drift is F1-defect-fix-attributable with negligible incremental contribution from the one intervening engine commit (`3f67242f`). Per `ave-reproduction-gate`, a verdict-invariant drift is recorded (not Grant-routed — no bin/verdict/headline flipped); the sign-independent §8 structural finding **(2,3) does not close** stands untouched. **Receipt-refresh DISCHARGED: the banked `−2` C1 reproduces on HEAD.**
