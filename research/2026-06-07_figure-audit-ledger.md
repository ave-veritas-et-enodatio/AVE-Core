...
1. **Place the ~9 PLACE-NEW** (montage→vol1 ch03, photon-strip→vol1 ch04, ie_validation→vol2 ch07, lithium_7→vol6 lithium, be8_decay→vol6 beryllium, orbital_jitter→vol2 ch07, ~~hulse_taylor→vol3 ch08~~ **(EXECUTED 2026-08-05 — see the struck row above; regenerated generator-first, not placed as-is)**, sonoluminescence→vol4 ch11, k4_native_chirality→vol4 ch01). Fix the `05_lithium.tex` duplicate-figure bug while there.
