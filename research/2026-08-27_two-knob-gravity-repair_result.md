...
4. **"superluminal phase velocity is a problem."** It is not; $v_pv_g = c_{\rm eff}^2$ is the ordinary gapped-branch relation, and Mercury's packet has $v_p = 6263c$ carrying no signal. Replaced with the branch-**slope** question — which canon answers two different ways (`port-register.md`:49 writes the gapped branch with $c_\kappa = \sqrt2 c$; `cosserat-mass-gap.md`:18,:42,:59 write the same branch with $c$).
