...
**⚑ The repair round broke CI, and the break was CLS-1's own failure mode committed inside the gate written to close CLS-1.** Recorded rather than quietly amended, because it is the most instructive thing in this section. The CLS-1 registry originally recomputed the defect counts from `build_srs_net(3)`'s **live** eigenvalues. Whether the defect reproduces depends on which side of $\mu=\pm3$ a given LAPACK build lands on: this repo's dev machine returns $\mu_{\min}=$ `-2.9999999999999987` and the defect reproduces (`215` / `20` / $\theta=\pi$ block empty), while a build that returns $\mu_{\min}$ at or beyond $-3$ has `np.clip` fire and the defect does **not** reproduce (`214` / `19` / one). So the registry contained `215` locally and not on CI — green here, red there, on a doc that had not changed. **A classification riding on where a float lands at the ULP level is exactly the defect CLS-1 is, and I reproduced it in the checker.** Fixed by taking srs $L=3$'s two band-edge eigenvalues from the **record** — which is what the doc's sentence names (*"this driver's OWN shipped value `reproduction_gate.fresh.srs_L3_mu_min`"*) — while the engine still supplies the spectrum and the driver still supplies the classifier. The substitution **reconciles rather than overrides**: if the live extremes disagree with the record beyond ULP noise the checker RAISES, because that would mean the graph changed. Four **determinism receipt** arms now hold the checker's verdict fixed under exactly that perturbation, and reverting the fix turns all four red with the verbatim CI error — so the receipt is load-bearing, not decorative.
