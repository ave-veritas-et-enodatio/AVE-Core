...
2. **The substrate default is symmetric CO-SATURATION, locked at the operating point by K=2G.** At the trace-free condition the saturation kernel acts symmetrically on ε and μ — *"both saturate at the same A value, locked by the trace-free condition"* [`A-034`:14]. So absent a symmetry-breaking bias the two sectors saturate **together** — the fork is genuinely degenerate. (Qualifier per A4: K=2G is a *downstream consistency* the lattice *sits at* given α, not a primitive axiom — `dual-reactance-storage-taxonomy.md`:62-67 — so the lock is **operating-point-true, not axiom-forced**.)
...
- **Therefore the wall-formation-order fork, whatever its verdict, cannot re-assign the mass sector.** Mass is A1 by ontology; the wall *confines* the A1 mass, it does not *constitute* it. Under B3 the wall is co-built from both sectors, so there is no unique wall-sector to identify with mass anyway.
...
1. **Ratify B3 DEGENERATE** — the magnetic-vs-capacitive wall is symmetric co-saturation at the operating point, with the μ-vs-ε choice a chirality-set SIGN/spin selector, not an independent substrate-forced branch.
...
**Plumber's one-liner:** it's one short-circuit wall, and the substrate builds it from both the electric and magnetic "windings" at once (they hit the wall together because the vacuum is tuned to K=2G). Calling it "the magnetic wall" is just naming which winding leads by a hair — and that hair is the electron's handedness (spin), not a separate kind of wall. The mass it traps is the longitudinal compression (A1) regardless.
