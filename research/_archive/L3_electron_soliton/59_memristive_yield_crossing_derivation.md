...
> **🔴 STALENESS NOTE — dated 2026-07-19 (append; §10.1–§10.6 scope preserved below).** This section's "documented, NOT executed / deferred per derive-before-simulate" framing is now **STALE**:
> - **The engine capability now EXISTS.** The dynamic Level-2 `S(t)` relaxation-ODE this section scopes as future work is implemented — `K4Lattice3D(use_memristive_saturation=True)` (`src/ave/core/k4_tlm.py:266–296`, backward Euler at `:291`), added since this doc was written. The §10 "no engine changes land" bound no longer holds for the Level-2 relaxation-ODE; the yield-fork discriminator drivers reproduce the engine's own memristive update with the engine byte-untouched (`research/2026-07-19_yield-fork-discriminators_result.md` §5, merged #735).
> - **The §11 / §6.3 downstream prediction status PREDATES the #735 outcome.** `P_phase5_memristor_loop_area` was run in that discriminator arc and is now **DEMOTED** on a both-and basis: (a) falsified-as-registered / zero-information in its stated `(r,S)` plane — the loop-area peak is pinned at $\omega\tau \approx 1.0014$ across the drive family, so $[0.85, 0.95]$ is a-priori unreachable for any first-order kernel (a theorem of the observable); (b) the $0.9$ window center was mis-registered (imported from doc-48's $A^2_{\cos}$, while this doc's own Eq 6.3 gives $0.954$–$0.978$ at the registered drive). The one datum leaning toward the memristive window is the testable $(V,I)$ plane peak at $\omega\tau = 0.911$, INSIDE $[0.85, 0.95]$ (F-B2 no-origin-pinch caveat). See the dated demotion at `manuscript/ave-kb/vol4/circuit-theory/ch1-vacuum-circuit-analysis/tau-relax-derivation.md` §5 + the #735 result doc.
> - **★ Flag F (§12) is now the CRUX.** Both discriminator legs converged on relocating the fork (finite-area memristive loop vs zero-area saturating reactance) to **Flag F** — whether the near-yield `S`-dynamics are first-order overdamped (dissipative, Eq 2.1 as asserted here) or second-order reactive ($I_S \neq 0$, lossless). That is the load-bearing OPEN item; the decisive $d^2S/dt^2$ second-order reactive contrast **has never been run** (SPEC'd for the Flag-F derivation branch). Ruling stays Grant's; the fork is OPEN.
...
>
> **Flag F — post-#751/#758 state (dated 2026-07-20; J(ω) now DERIVED; still OPEN, still NEVER "RESOLVED").** The z=3 bath spectral density J(ω) the #744 note said the fork awaited has been derived and re-banked (`research/2026-07-20_jomega-derivation_result.md`, merged #751; sibling merge #758). It ADVANCES Flag F past OPEN-XOR but does **not** close it:
> - **World (c) — an axiom-level resistor / rate-independent plastic loss — remains EXCLUDED (corroborated, not new).** The informative new content: a lossless (γ=0) second-order S traces a **finite** reactive loop (∮S dr finite, existence-grade), so **loop-area ≠ dissipation** — the finite pinched loop of this doc's §2/§6.3 does NOT license a resistor. The world-(c) exclusion rests on the independent drive-work ledger closure (W_drive ≈ W_diss to ≤2% for γ>0), not the definitional W_diss=0 leg (`…jomega-derivation_result.md` §0.2, §5, §6).
> - **The frozen (a-ledger)/(b-ledger) net-per-cycle-transfer criterion is DEGENERATE.** Run exactly as its prereg specifies, (a-ledger) fires in **0/4** cells and (b-ledger) in **1/4** (only the super-Ohmic C2 ∞-lattice bath); 3/4 cells fall in the degenerate gap. No clean UNIFORM (a)/(b) scope separation ⇒ bin (iii) DEGENERATE (`…jomega-derivation_result.md` §0.1, §4.1).
> - **The (a)/(b) crux is a coupling-model + scope question, both UNDETERMINED.** The crossing shape splits on the one unforced choice — S→bath coupling **on-site (Ohmic, s≈1) vs strain (super-Ohmic, s≈3)** — which decides world-(b)-channel-LIVE vs world-(a)-suppression. Deriving it needs the full engine constitutive tensor; the physical bath SCOPE (0D few-mode reactive-return vs ∞-lattice transductive) is a separate open question routed to Grant. Grant's reversible-reactive lean at the 0D scope is ASSERTED, not frozen-adjudicated.
> - **Flag F stays PARTIALLY discharged, awaiting Grant ratification. NEVER "RESOLVED."** The kinetic-term / I_S provenance (§2.3 / this §12) is also still OPEN — the derivation did not stretch scope onto it.
