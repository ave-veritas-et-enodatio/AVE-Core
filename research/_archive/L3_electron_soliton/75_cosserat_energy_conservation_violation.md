# 75 — Cosserat sector saturation asymmetry: V·S, T·1 violates Ax 4's symmetric L/C requirement (Noether-broken energy conservation downstream), BUT empirically negligible at relevant amplitudes (Diag A Mode I)

> **Framing note (added 2026-04-27 per axiom homologation P3+):** earlier versions of this doc framed the bug as "violates Ax 3 energy conservation." Per canonical Scheme A ([axiom homologation](../../manuscript/ave-kb/common/axiom-homologation.md), [doc 77_](77_lattice_to_axiom4_bridge.md)): Ax 3 is the Effective Action Principle (the Lagrangian itself), NOT energy conservation. Energy conservation is a Noether consequence of Ax 3's time-translation symmetry. The engine bug is more precisely stated as: **Ax 4's saturation kernel is implemented asymmetrically across L (motion-side, T_kinetic, unmodulated) and C (rest-side, V_potential, S-modulated), violating the symmetric L/C scaling that Ax 4 + Symmetric Gravity (Ax 1 + Ax 4 combined) require for c invariance.** The asymmetric implementation makes the Lagrangian explicitly time-dependent through `S(A²(x,t))`, breaking the time-translation symmetry of Ax 3, which by Noether breaks energy conservation. The original doc text below uses the older "Ax 3 violation" shorthand; per Rule 12 (retraction-preserves-body) it is preserved with this framing note.
...
**TL;DR:** The V·S, T·1 saturation asymmetry diagnosed in [`cosserat_field_3d.py:545-587`](../../src/ave/topological/cosserat_field_3d.py#L545) (V_potential gets S factor) vs [`cosserat_field_3d.py:1204-1209`](../../src/ave/topological/cosserat_field_3d.py#L1204) (T_kinetic does NOT) is an in-principle violation of Ax 4's symmetric L/C scaling requirement (Noether-broken energy conservation downstream — see framing note above). But empirically, the wave-speed drift it predicts is ~50× weaker than the analytical estimate suggested at low-to-moderate amplitudes, and only becomes detectable at A ≥ 4 (per supplementary scan). **Move 11's 5.5% H_cos drift CANNOT be primarily from this mechanism, and the Round 7+8 universal Mode III pattern is NOT explained by V·S, T·1 wave-speed drift.** Both findings need alternative explanations.
...
Ax 3 (Effective Action Principle) gives the macroscopic hardware Lagrangian $\mathcal{L}_{node}$; energy conservation follows by Noether from the Lagrangian's time-translation symmetry. In a saturated medium where saturation factor `S(A²(x,t))` is dynamic, the Lagrangian's coefficients become time-dependent through `S`. For energy conservation to hold, `S` must enter `T` and `V` in a structurally consistent way — typically symmetrically (both ·S, both /S, etc.) so the EOM is just the rescaled bare wave equation. Per canonical Scheme A, this symmetric-L/C requirement is set by Ax 4 (saturation kernel) combined with Ax 1 (LC network Z₀ invariance) — i.e., Symmetric Gravity per [`eq_gravity_derived.tex`](../../manuscript/common_equations/eq_gravity_derived.tex). The Ax 3 → Noether → energy-conservation chain is the consequence; the load-bearing requirement is at the Ax 4 implementation level.
...
This is an empirical statement about the engine at its prescribed lattice resolution, not a falsification of the corpus framework. [Doc 77_](77_lattice_to_axiom4_bridge.md) (lattice → Ax 4 bridge under canonical Scheme A; supersedes doc 76_'s Scheme B framing) provides one route for reframing — corpus electron lives at an integrated-along-loop scale that single-cell engine extraction may not see. Other routes (N=128+ escalation, non-corpus aspect ratio, different IC topology) remain open as separate framework questions.
