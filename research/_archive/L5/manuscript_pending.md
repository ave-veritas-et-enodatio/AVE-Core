...
- **[E-094] Substrate-native vocabulary discipline — AVE-QED App G propagation to AVE-Core**
  ...
  - **Status:** **APPLIED 2026-05-15** — all 7 propagation targets landed:
    (1) Vol 1 Ch 1 §sec:substrate_vocab_box_ch1 — 9-row substrate-native/EE/ME vocab box + three invariants pointer;
    (2) Vol 1 Ch 4 — two new paragraphs near eq:master_wave (Master Equation FDTD canonical engine reference per E-095/E-097 + substrate-observability rule cross-ref per A-026);
    (3) Vol 2 Ch 1 §eq:M_electron_identity — canonical $\mathcal{M}_{\text{electron}} \cdot c^2 = T_{EM} \cdot \ell_{\text{node}}$ identity + full triplet $(\mathcal{M}, \mathcal{Q}, \mathcal{J})$ for canonical electron;
    (4) Vol 3 Ch 2 §eq:M_to_M_ADM — $\mathcal{M} \to M_{ADM}$ projection identity; substrate-observability applied at gravitational boundaries (ordinary matter, BH limit, cosmic);
    (5) Vol 4 Ch 1 §sec:rosetta_stone_3col — full 14-row 3-column Rosetta-stone (substrate-native / EE / ME) + three substrate invariants canonical table;
    (6) `docs/glossary.md` CREATED (180 lines) mirroring AVE-QED §5m + canonical entries A-026/A-028/A-031;
    (7) `src/ave/core/constants.py` module docstring updated with structural-closure framing (one cosmological IC + one scale + 4 axioms; three observational routes; substrate-native vocab cross-refs).
  - **Cross-refs:** E-085 (Vol 4 Ch 1 Virial sum already canonical), E-095 (Master Eq FDTD), E-096 (boundary-envelope), E-101 (engine module), E-102 (Vol 3 Ch 4 cosmic-𝒥), E-103 (Vol 3 Ch 21 epistemic-horizon); A-026 (substrate-observability rule), A-028 (three substrate invariants), A-029 (shared-spring), A-030 (α+G joint), A-031 (cosmic-𝒥 IC); doc 109 §13 (Grant-confirmed canonical)
