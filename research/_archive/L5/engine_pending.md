...
### `master_equation_fdtd.py` (NEW MODULE, 2026-05-14)

- **[E-099] Master Equation FDTD engine — canonical bound-state regime engine (LANDED)**
  - **Sources:** [doc 111_ §3-§4 audit](../L3_electron_soliton/111_master_equation_audit_and_engine_gap.md) (`3815158`, 2026-05-14, audit identifying c_eff(V) gap in K4-TLM-only architecture); [doc 112_](../L3_electron_soliton/112_master_equation_fdtd_first_iteration.md) (`6a40610`, 2026-05-14, Path B first iteration); [doc 113_ §0-§4](../L3_electron_soliton/113_v14_closure_master_equation_fdtd_mode_I.md) (`345d55d`, 2026-05-14, Mode I PASS canonical closure on breathing-soliton criterion); [src/ave/core/master_equation_fdtd.py](../../src/ave/core/master_equation_fdtd.py) (canonical engine module, ~9.7KB)
  - **Action:** NEW MODULE `MasterEquationFDTD` class implementing 3D leapfrog integration on Vol 1 Ch 4 eq:master_wave $\nabla^2 V - \mu_0\varepsilon_0 \sqrt{1-(V/V_{\text{yield}})^2}\,\partial_t^2 V = 0$. Key methods: `__init__(N, dx, V_yield, c0, cfl_safety, pml_thickness, A_cap, S_min, source_mode)`, `step()`, `run()`, `inject_gaussian()`, `inject_localized_blob()`, `saturation_kernel(V)`, `c_eff_squared(V)`, `refractive_index()`. Update rule: `V_new = 2*V - V_prev + dt² * c_eff_squared(V) * laplacian(V)` with PML damping. Hosts the bound-state regime (A → 1 saturation). v14 Mode I PASS on breathing-soliton criterion (doc 113, 4/4 strict on breathing-appropriate criterion). Smoke test passes on merged research branch (imports + instantiates clean). Black + isort applied; flake8 deferred.
  - **Tests needed:** existing v14 driver scripts at `src/scripts/vol_1_foundations/r10_master_equation_v14_*.py` serve as integration tests. Add `src/tests/test_master_equation_v14_mode_i.py` as regression test (asserts breathing-soliton seed produces V_peak mean > 0.2, FWHM stable, n(r) gradient measurable). Wire into `make verify` per doc 114 §4.5.
  - **Status:** **APPLIED 2026-05-14** via commit `6a40610` (engine + first iteration), `88c331b` (source_mode hard/soft injection), `345d55d` (v14 Mode I PASS closure). Module canonical-engine status confirmed by doc 113 + doc 114 §2 status table.
  - **Cross-refs:** E-095 (manuscript-side canonical statement in Vol 1 Ch 4), E-097 (two-engine architecture), E-100 (k4_tlm.py companion v14 changes), A-027 (axiom-status entry for two-engine architecture)

### `k4_tlm.py` — v14 additions (LANDED, 2026-05-14)

- **[E-100] K4-TLM v14 additions — V_SNAP plumbing + op3_bond_reflection path + memristive saturation**
  - **Sources:** [doc 110_](../L3_electron_soliton/110_v14_single_cell_boundary_empirical_results.md) (`d1bc623`, 2026-05-14, K4-TLM Mode III decisive across 4 variants); [doc 110 §v14e seven-mode seed](../L3_electron_soliton/110_v14_single_cell_boundary_empirical_results.md) (`25a3c67`, 2026-05-14, Grant pushback 7-mode seed); [src/ave/core/k4_tlm.py](../../src/ave/core/k4_tlm.py) (current canonical state on merged research branch)
  - **Action:** Multiple v14 additions landed on `k4_tlm.py`: **(1) `op3_bond_reflection=True` option** in `K4Lattice3D.__init__` — applies Op3 reflection at each bond based on local impedance mismatch between adjacent sites ($Z_{\text{eff}} = Z_0/\sqrt{S}$ per Op14); the missing mechanism for bound solitons on K4 substrate; default False for backwards-compat with existing validators; **(2) `use_memristive_saturation=True` option** — opt-in dynamical $S(t)$ per doc 59_ §1 ($dS/dt = (S_{\text{eq}}(V) - S(t))/\tau_{\text{relax}}$ with backward Euler); slow-drive limit reduces to current Op14; near rupture or simulation $\omega \cdot \tau_{\text{relax}} \sim 1$ exhibits pinched hysteresis loop; **(3) `V_SNAP` parameter override** — rupture voltage normalization for saturation kernel in `_update_z_local_field` and `_scatter_all`; when K4Lattice3D is instantiated by CoupledK4Cosserat in engine natural-units context (engine V_SNAP=1), CoupledK4Cosserat passes 1.0 here so saturation strain calculation matches engine convention; **(4) `tau_relax` parameter** — defaults to dx/c per Ax1+Ax3 derivation (doc 59 §1); SI mode ≈ 3.34e-9 s; native units = 1.0; **(5) `Phi_link` accumulator** — per-bond magnetic flux linkage stored at A-sites for memristive observables; **(6) seven-mode seed support** via existing infrastructure — Grant's bubble-mode pushback "did you forget your seven modes of compliance?" surfaced that prior single-cell tests seeded only 2/7 modes (translational only); v14e added full 7-mode seed (3 translational + 3 rotational + 1 volumetric breathing). Result: K4-TLM alone remains Mode III decisive (doc 110); engine needed Master Equation FDTD for bound-state hosting (E-099).
  - **Tests needed:** v14e seven-mode seed driver at `src/scripts/vol_1_foundations/r10_path_alpha_v14e_seven_mode_seed.py` documents the empirical Mode III state. No new pytest needed (informational empirical record, not regression-protective).
  - **Status:** **APPLIED 2026-05-14** via commits `d1bc623` (v14 baseline), `25a3c67` (v14e seven-mode seed). Engine canonical for sub-saturation bench regime per E-097 two-engine architecture.
  - **Cross-refs:** E-095, E-097, E-099 (Master Equation FDTD companion), C-? (K4-TLM Mode III at v14 — preserved empirical finding, NOT framework failure per doc 113 §3.2 two-engine architecture explanation)

### `master_equation_fdtd.py` observables extension — NEW QUEUED

- **[E-101] Three substrate invariants $\mathcal{M}, \mathcal{Q}, \mathcal{J}$ — engine-side observable computation**
  - **Sources:** [AVE-QED `manuscript/vol_qed_replacement/appendices/G_substrate_vocabulary.tex` §4](../../../AVE-QED/manuscript/vol_qed_replacement/appendices/G_substrate_vocabulary.tex) (`ce34645`, 2026-05-14, canonical definitions); [doc 109 §13-§15](../L3_electron_soliton/109_elastic_substrate_finite_strain_investigation.md) (`ad90c87`, 2026-05-14, boundary-envelope reformulation)
  - **Action:** Add observables module (`src/ave/core/boundary_invariants.py` or similar) computing the three canonical integrated invariants for any localized boundary $\partial\Omega \subset \mathcal{M}_A$: **$\mathcal{M} = \int_\Omega (n(\mathbf{r}) - 1)\,dV$** (integrated strain integral; mass-equivalent observable); **$\mathcal{Q} = \mathrm{Link}(\partial\Omega, \mathbf{F}_{\text{substrate}})$** (boundary linking number; integer per boundary; charge analog); **$\mathcal{J} = \mathrm{Wind}(\partial\Omega)$** (boundary winding number; half-integer per SU(2) double-cover; spin analog). Operationalizes the substrate-observability rule: a substrate-native simulation reports these three numbers per boundary, not interior eigenmode structure. Used as: assertion targets in v14-like driver scripts (e.g., compute $\mathcal{M}$ on the breathing-soliton boundary; should approach $\mathcal{M}_{\text{electron}} = T_{EM} \cdot \ell_{\text{node}}$ as the engine converges).
  - **Tests needed:** new `src/tests/test_boundary_invariants.py` with: (a) known-analytical-boundary tests (uniform sphere → $\mathcal{M}$ proportional to volume; unknot vs trefoil → distinct $\mathcal{Q}$ values; etc.); (b) v14 breathing-soliton integration test asserting $\mathcal{M}, \mathcal{Q}, \mathcal{J}$ are conserved across breathing cycle within numerical tolerance; (c) cross-check against canonical electron values ($\mathcal{M}_e = m_e c^2 / c^2 = 9.109e-31$ kg-equivalent; $\mathcal{Q}_e = 1$ (per |e|); $\mathcal{J}_e = 1/2$).
  - **Status:** **APPLIED 2026-05-15** — landed as `src/ave/core/boundary_invariants.py` (270 lines: BoundaryInvariants dataclass + compute_M (rigorous volume integral of n_grav-1) + compute_Q (first-pass proxy via scipy.ndimage.label connected components) + compute_J (first-pass proxy via inertia-tensor eigenvalue anisotropy) + compute_all_invariants convenience). Tests: `src/tests/test_boundary_invariants.py` (13/13 PASS): unstrained → all 0; single sphere → M>0, Q=1, J≈0; two spheres → M doubles, Q=2; elongated → J>0; v14 breathing-soliton seed integration test. Rigorous Q (Link) and J (Hopf invariant) implementations deferred until Cosserat coupling on Master Equation FDTD lands (doc 113 §5.4); module docs flag this explicitly.
  - **Cross-refs:** E-094 (manuscript-side substrate-vocab discipline), E-095 (Master Equation FDTD engine), E-096 (boundary-envelope reformulation), A-026, A-027, A-028, A-031 (cosmic-scale application uses same definitions)
