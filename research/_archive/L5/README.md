...
- **Audit playbooks** (`.agents/workflows/audit-*.md` + `peer-review.md`). Referenced from many docs as authoritative methodology. When a tracker entry lands changes that affect a playbook's known-stale list (e.g., fixing a numeric value the audit-math playbook flags), update the playbook in the same commit. Currently NOT a separate file.
  - **[DATED POINTER NOTE 2026-08-18 — Wave-2 PR-3, ruling D6-3]** The playbooks moved from the gitignored `.agents/workflows/` scratch to the tracked `docs/workflows/` (`audit-code.md`, `audit-full.md`, `audit-latex.md`, `audit-math.md`, `peer-review.md`). Content unchanged; git history follows the renames. The "authoritative methodology" status and the same-commit maintenance obligation stated above are UNCHANGED — only the home moved. The bullet above is preserved as written (this archived tracker is frozen); this note is the additive path correction.
