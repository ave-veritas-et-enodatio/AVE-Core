...
2. **The RADIATED remainder** (`9/10`) disperses into a **matched quasi-continuum** (`R_rad ≡ Z_0`) with **reconvergence ≈ 0** — the home-leaf premise of the mode-count arrow (`…tier-1…CHARTER.md:172` "reconvergence probability effectively zero"; `dark-energy-latent-heat-definition.md:84-86` cited there).
...
> **🟢 RULED 2026-07-20 (RULING 4 of the 2026-07-20 ratification batch — Decision 2 = 2a, scoped).** Grant ratified 2026-07-20 (**Grant-verbatim, [sic]: "4 ratified"**); ruling *content* = the **orchestrator's walk, ratified in chat** — attribution split: the wording here is the ratified walk, NOT Grant-verbatim. **Decision 2 = (2a).** The click's radiated remainder (`9/10`) terminates into the **matched real port `R_rad ≡ Z_0`** — the canon's **Ax3-legal SYSTEM-loss row** (a real port at the *system boundary*; **NOT a bulk valve**; consistent with the banked X40 instrument's existing matched stubs, `retention-transition-split.md:60`). **Consequence — the cost-of-genesis ledger becomes well-defined:** the **radiated majority (`9/10`) = the genesis cost**, and the **trapped fraction (`1/10 = f_E = 1/N`) is MESH-GEOMETRY, NOT `m_e`** — **mass stays A1 / definitional** (the trapped-DC-loop label is a topological count, not a mass derivation; sector-ownership A1⊥Cosserat-winding preserved — do NOT read the trapped fraction as an electron-mass emergence). **Keystone fence RIDES:** formation-route claims stay **echo-scoped** — the α-keystone / electron-genesis-from-precursor fence is untouched; the click quantifies a *cost ledger*, it does not mint an electron-existence or a value-level chord. The **click-arm prereg's load-bearing gate (this Decision-2 fork) is thereby CLEARED**; the **BUILD stays PARKED** per the same focus-check rebalance (Decisions 1/3/4 remain Grant's open calls; no build fires without a separate Grant go).
