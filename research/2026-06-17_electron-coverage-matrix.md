...
| **g=2** (gyromagnetic) | 2.00231930436 | **B** (winding-sector; g=N_cover FORM-forced conditional on the canonical double-cover — reconciled 2026-07-14, note ‡) | clm-uatcql 0.70 (SO(3) 2π vs spinor 4π ratio) | FORM=chord (g=N_cover, forced conditional on the canonical double-cover; NOT a bare group-theory "derives") / VALUE=peer-with-Dirac (N/A distinct) | NOT cage-testable — magnetic moment = Cosserat-B winding sector |
...
> **‡ g=2 provenance reconciliation (2026-07-14, QED-TRACE housekeeping).** The prior internal tension — the **g=2** row above reading "the ratio derives" (FORM=chord) while the **compositeness** row's (iii) MOMENT sub-bullet read "g=2 POSITED (imported)" — is reconciled per the g−2 self-orbit result's own resolution (`research/2026-07-08_electron-g2-selforbit_result.md` §6/§7): the ratio does **NOT "derive" as a bare group-theory statement**, but the FORM g=2 **IS forced** by the double-cover integer given the A1⊥T2 mechanical construction (g = N_cover; μ cover-immune, S cover-halved). It is **neither "purely imported" nor "independently derived from nothing" — a FORM forced conditional on the canonically-established double-cover**; VALUE is peer-with-Dirac (not an AVE-distinct number). Both cells now carry this single statement (KEEP-BOTH — the two legacy readings are preserved in the git history + named here, reconciled rather than silently redefined).
