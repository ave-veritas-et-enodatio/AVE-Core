> **SURFACE-NOTE 2026-08-19 (Wave-2 D15a EXTENSION — flagged for the pre-merge audit + Grant; landed from tag `archive/analysis/2026-06-15-eigenmode-heldbc`; frozen at tag state incl. its own Rule-12 correction banner).** The ruling's family-2 list did not name this file, but landing its PREREG without its RESULT would recreate the exact stale-read hazard the honest-set landing exists to fix. One word strikes it. Current adjudicated state: `program-arc-map.md` ARC-08; NOT stamped with the K3 kill (static-existence lane — the same cross-wiring guard the sibling prereg's note observes).
...
>
> Read the NEGATIVE-A text below through both corrections (mis-binned **and** superseded). The §5 FLAG-WIND / FLAG-RES flags it raises remain valid empirical observations; the bin LABEL is what is corrected.
>
