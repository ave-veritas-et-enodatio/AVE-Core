...
**Status**: definitional leaf landed; forward falsification-tests scoped (NOT executed). Phase 4 LANDED (PR #60 + closure PR #61, `main` @ 7a2722df) — FT-1 + FT-2 execution now UN-GATED. Bell/determinism foundation probe resolved (§8).
...
- **Phase 4 dependency — CLEARED**: FT-1's α-chain sat on the ch8 / theorem-3-1 / op21 provenance the Phase 4 walk-back was rewriting. Phase 4 landed (PR #60/#61, `main` @ 7a2722df), so **FT-1 and FT-2 execution are now un-gated**; `constants.py` is stable on main.
- **Merge touchpoint — RESOLVED**: this branch + Phase 4 both edited `common/index.md`; rebased onto `main` @ 7a2722df with clean git auto-merge (my Contents row + Phase 4's frontmatter/Key-Results both intact); `verify-kb-metadata` + `verify-md-links` re-run PASS against the post-Phase-4 claim-DAG.
...
- **Bell/determinism foundation probe — RESULT** (corpus-grep 2026-05-31): AVE is **not silent** on Bell — it self-classifies (foreword `manuscript/frontmatter/00_foreword.tex`; canonical leaf `phase-locked-topological-thread.md`, clm-zuf7g1) as a **nonlocal deterministic hidden-variable theory**: CHSH $=2\sqrt2$ (Tsirelson) derived from substrate primitives, no-signalling preserved, NOT superdeterministic, NOT preferred-frame-mediated. The deterministic-substrate stance (leaf §1) therefore *survives* Bell by conceding locality — foundational consistency confirmed (the chord is playable). CHSH is not an AVE-vs-QM discriminator. Integrated into leaf §4 + §7.
- **OPEN PROBE — preferred-frame seam** (forward, from the Bell probe): a nonlocal deterministic hidden-variable theory generally needs a preferred foliation to order nonlocal updates covariantly; AVE *has* one (K4 = CMB rest frame) yet the corpus says the topological thread is frame-agnostic and does NOT invoke it for the correlation. Open question on clm-zuf7g1: does the thread require the K4 frame, and if so does that yield a testable consequence? Either a genuine advantage over Bohmian mechanics or an unexamined gap — worth a dedicated probe.
