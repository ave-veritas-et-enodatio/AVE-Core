...
> **→ RATIFIED (Grant, 2026-06-13):** the Gaussian `seed_bulk` self-create is **FALSIFIED** (does not nucleate); the sech eigen-profile self-focus is **CONSISTENCY-confirmed** (v14 Mode I). The basin is **profile-selective** — recorded as a structural finding, **not** generic emergence. This is the bridge to **T2** (genesis self-lock): the CP8 generative precursor is a **sub-threshold generic field with a photon incident at it** (photon-**at-a-field** — pair-production needs a field; `crystal_engine` correctly nulls photon-**from-vacuum** via the `g_front` gate, the engine encoding γ+Z→e⁺e⁻), **not** the eigen-profile end-state.
...
> **→ RATIFIED (Grant + auditor, 2026-06-13):** the energy-flat reading is **superseded by boundedness + persistence** (prereg **Amendment 3**, Rule-12, **result-invariant** — the classifier already gated on boundedness, not energy-flat). Energy-ledger drift is reported, not binned; pump-exclusion = the a/b/c triad (converter-OFF, bounded-at-`V_yield`, decreases-with-amplitude).
