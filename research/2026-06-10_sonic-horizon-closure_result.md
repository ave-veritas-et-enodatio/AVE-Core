...
**Gate conclusion: every new BC knob has a ZERO false-positive floor at the sub-crossing drive.** A pocket/`E_diss`/persistence signature in the physics runs cannot be a sub-crossing artifact of these knobs.
...
- **The crossing is CONTINUOUS — no latent-release step (FLASH signature ii NEGATIVE).** KE declines smoothly (0.04876 → 0.04645), PE_exact rises smoothly (0.01435 → 0.01589), KE+PE eases down (0.063105 → 0.062330) with **no step, spike, or kink** at the `ρ̄_cav` crossing. The reactance pair (CP6: KE = L-state ↔ PE_exact = C-state) exchanges smoothly.
- **The one-way `E_diss` is real but a minority sink:** it accrues **gradually** (2.7e-5 → 1.65e-4 across the crossing), accounting for ≈ **23%** of the ≈ 7.3e-4 total KE+PE decline over the window. The remaining ≈ 77% is acoustic radiation into the sponge + `nu_art` shock-capturing + the (small, bounded) mass-clamp. **There is no discontinuous latent heat** — the EOS `c²=0` root is a smooth softening point, not a first-order phase boundary, even with the void clamp and shock dissipation active.
...
- Handedness: `R_co = R_counter` to the 0.000 static-mirror floor at M=0.9/1.0/χ=0 → **BLIND**.
...
4. **FLAG — representation-capability:** the handedness BLIND is for rotating-horizon **frame-dragging**, NOT the I4₁32 **cholesteric-Bragg** lattice selection rule. The vapor-lock v5 "chirality valve = the mirror's selection rule" claim is **NOT** supported by this engine; it would need the chiral-crystal engine. Do not headline v5 on this BLIND.
