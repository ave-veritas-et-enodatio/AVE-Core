...
```mermaid
flowchart TB
  subgraph macro["Macro — cosmic rim (clm-s4n33u, clm-u86caq)"]
    H["H∞ bulk drift"]
    HS["Hoop ÷2π → a₀"]
    LH["Latent heat → DE"]
    H --> HS
  ...
  subgraph gal["Galactic — clm-u86caq / clm-tt8j0v limb (i)"]
    OUT["Outer: gₙ ≪ a₀ — UNSATURATED"]
    DRAG["η_eff drag → DM curves"]
    INN["Inner: gₙ ≫ a₀ — SATURATED"]
    KEPL["η_eff → 0 — Keplerian"]
    OUT --> DRAG
    INN --> KEPL
  end

  subgraph micro["Micro — electron cell"]
    SAT["Saturated T₂ on one node"]
    TRAP["Γ_bulk → −1 trap"]
    SAT --> TRAP
  end

  KERNEL["Shared: K4 Cosserat + S(A)=√(1−A²)"]
  KERNEL --> OUT
  KERNEL --> INN
  KERNEL --> SAT
  HS --> OUT
