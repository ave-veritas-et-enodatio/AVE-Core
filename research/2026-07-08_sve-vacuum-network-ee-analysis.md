...
- **★ Sector ratios ν = 2/7, v_L/v_T = √(10/3): RATIOS-DEVIATE — NOT forced forms.** ν and v_L/v_T are a **one-parameter family in the free bond ratio k_a/k_s** (Maxwell sub-isostatic freedom: z=4 < 2d=6). ν = 2/7 (⇒ √(10/3)) requires a **tuned, lattice-specific** k_a/k_s (16.36 diamond vs 11.63/9.77 srs), not geometrically distinguished. The "ν = 2/7 exact" identity is a **tautology of imported K=2G**; the direct anisotropic-lattice reading *deviates* (ν_C = 0.338 at K=2G, Zener A = 1.755 ≠ 1), and the two "ν=2/7" conditions (isotropic-identity vs direct) sit at *different* knobs — mutually inconsistent. **K=2G is confirmed lattice-un-forced** (a tuned/imported point — consistent with K=2G = GR-imported, PR #261/#506). **[lattice tag]** these anisotropy numbers (A≈1.755, ν_C direct-[100]) are the **diamond-z4 instrument / direct-reading**, NOT the canonical carrier: the ratified **srs-z3** value is ν_Hill = 2/7, Zener A ≈ 1.23 at ρ*≈9.77 (isotropic Voigt-Reuss-Hill average, #506). **KEEP-BOTH** — the diamond/direct numbers do not supersede the canonical srs-Hill; the qualitative finding (ν a free family, K=2G un-forced) holds on both. So this sector is **value-imported AND ratio-imported**; the only lattice-forced FORM the vector sector adds is *qualitative* — the existence of a Poisson-like relationship v_L/v_T = f(k_a/k_s) and the isotropic tetrahedral (4/3)I 2nd-moment structure.
