...
**Manual revision:** r8.8 (2026-04-25, evening)
**Engine version:** 4.0.4 (current HEAD = `675141e` on branch `research/l3-electron-soliton`; r8.8 catches up on one Round 7 commit since r8.7 — doc 72_ design-space articulation + reframe 3 of R7.1 (`675141e`). **External-audit-driven reframe**: the multi-seed Hessian-of-W framing frozen in r8.7 (`P_phase6_eigensolver_multiseed`) was itself a Rule 6 instance — Hessian-of-W is SM/QM-style energy-stationarity framing on a wave-propagation substrate. Doc 72_ articulates the four AVE-native concepts (wave eigenmode via Helmholtz NOT Hessian; impedance match via S₁₁-min NOT energy-min; topological quantization as input ansatz NOT dynamical attractor; AVE basin = S₁₁-min NOT W-min) and the 3D Smith chart for the vacuum (Extension A `(Re(Γ), Im(Γ), ω)` primary). **Reframe 3 deliverables:** R7.1 operator changed from Hessian-of-W to **block Helmholtz on joint (V, ω) state** (per audit Q1 hybrid-coupled-mode pushback; cross-blocks encode Op14 coupling explicitly; V=0 seed decouples into V-block + ω-block eigenmodes simultaneously per §3.1.1 footnote — strict superset of single-sector Helmholtz). Topological crossing-count `c_eigvec=3` replaces shape correlation as binary PASS criterion; shape correlation > 0.60 retained as informational diagnostic (audit Q4 two-tier). Sector-energy split diagnostic added (audit Q1). LOC budget ~290. **Rule 10 commitment language landed verbatim in BOTH doc 72_ §6.1 AND this §13.5l**: this is reframe 3, none has run; fresh-session run committed to operator choice barring catastrophic methodology error. New **A36** finding (operator-choice Rule 6 violation — Hessian-of-W on wave substrate; family with A22+A30+A35; caught earliest in cycle via external audit on commit `c69e79c`). Engine-flag commits still unchanged since r8.3.)
...
**Working tree (r8.8):** clean. Doc 72_ design-space + reframe 3 commit (`675141e`) committed. Manual itself tracked since r8 (`4ba20f8`).

**Critical-path status (r8.8) — Round 7 Stage 1 reframed (reframe 3) to multi-seed block Helmholtz on joint (V, ω); Hessian-of-W retracted as wrong-substrate operator; Rule-10 commitment locks operator choice for fresh-session run:**
...
| **Remaining critical-path total (r8.8)** | — | **gated on R7.1 fresh-session run** | Round 6 + Phase 5 resume close discipline-completely. R7.1 reframed (reframe 3) to multi-seed block Helmholtz; frozen pre-reg `P_phase6_helmholtz_eigenmode_sweep`. Rule-10 commitment locks operator choice for fresh-session run. R7.2 still needs P_phase5_topological_injection pre-reg before fresh-session run. Both R7.1 + R7.2 independent — can run in either order or in parallel. R7.1 multi-seed block Helmholtz answers basin-vs-corpus geometry question + (2,3) eigenmode existence question + sector character (V-dominant / hybrid / ω-dominant) of eigenmode in one run. |
