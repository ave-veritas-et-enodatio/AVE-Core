...
**Manual revision:** r8.7 (2026-04-25, afternoon)
**Engine version:** 4.0.4 (current HEAD = `c69e79c` on branch `research/l3-electron-soliton`; r8.7 catches up on two Round 7 commits since r8.6 — basin-audit Stage 0 scaffold (`1bc1652`) + reframe pivot to multi-seed sparse eigensolver (`c69e79c`). **Mid-cycle self-audit caught a Rule 6/8/10 framing error** before it propagated into R7.1 implementation: basin-audit-as-Stage-0 was a SM-style minimization framing on a wave-propagation substrate (Rule 6); bypassed R7.1's corpus-canonical eigensolver tool by building parallel basin-mapping infrastructure (Rule 8 inverse); carried across multiple turns without being pressure-tested against "just run R7.1 at multiple seeds" (Rule 10 creeper compound). **Reframe deliverables:** doc 71_ renamed `71_basin_audit_methodology.md` → `71_multi_seed_eigenmode_sweep.md` (§1-§12 retained as Rule 12 audit trail; §13 ACTIVE multi-seed R7.1); `predictions.yaml` carries `P_basin_audit_GT_stationarity` (RETRACTED) + `P_phase6_eigensolver_multiseed` (frozen pre-registration, four seeds, three-mode falsification); R7.1 strengthened from single-seed (GT only) to multi-seed (GT + F17K_cos + F17K_s11 + vacuum_control) at N=32 with three pass-criteria (ω_Compton ± α, Q ± 5%, shape correlation > 0.85). New **A35** finding (Rule 6/8/10 violation — corpus-bypass family with A22 + A30; caught earlier in cycle than predecessors per within-session self-audit). Engine-flag commits still unchanged since r8.3.)
...
**Working tree (r8.7):** clean. Round 7 Stage 0 scaffold + reframe (`1bc1652`, `c69e79c`) all committed. Manual itself tracked since r8 (`4ba20f8`).

**Critical-path status (r8.7) — Round 7 Stage 1 strengthened to multi-seed sparse eigensolver; basin-audit Stage 0 retracted; R7.1 + R7.2 fresh-session candidates scoped:**
...
| F17-K v3 (ii) Phase 6 sparse eigensolver | **Round 7 Stage 1 candidate (STRENGTHENED r8.7 to MULTI-SEED)** | ~300 LOC, ~3-4 hours fresh session | Eigenvalue problem `Au=λBu` at fixed cavity geometry (per acoustic-cavity / Helmholtz framing per doc 67_ §23.4) extracts (2,3) eigenmode without requiring dynamical stability. Empirically motivated by A32 (Golden Torus geometrically unstable). r8.7: scope strengthened from single-seed (GT only) to four seeds in same run (GT_corpus + F17K_cos_endpoint + F17K_s11_endpoint + vacuum_control negative control) at N=32, with three-mode falsification structure frozen in `P_phase6_eigensolver_multiseed` (doc 71_ §13). |
...
| **Remaining critical-path total (r8.7)** | — | **gated on Round 7 Stage selection** | Round 6 + Phase 5 resume close discipline-completely. R7.1 strengthened to multi-seed sparse eigensolver (frozen pre-reg). R7.2 still needs P_phase5_topological_injection pre-reg before fresh-session run. Both R7.1 + R7.2 independent — can run in either order or in parallel. R7.1 multi-seed answers basin-vs-corpus geometry question + (2,3) eigenmode existence question in one run. |
...
- [71_multi_seed_eigenmode_sweep.md](71_multi_seed_eigenmode_sweep.md) — **r8.7: filename + scope changed.** Originally landed `1bc1652` as `71_basin_audit_methodology.md` (Round 7 Stage 0 basin-audit-as-precondition framing); reframed in `c69e79c` to multi-seed R7.1 sparse eigensolver after within-session self-audit caught Rule 6/8/10 violation. §1-§12 retained as Rule 12 audit trail (basin-audit framing, v1 fresh-session run halted by A26 guard, v2 draft never executed); §13 ACTIVE — multi-seed R7.1 sparse eigensolver methodology with three-mode falsification structure frozen in `P_phase6_eigensolver_multiseed`; §14 driver scope notes for fresh-session implementer (K4-amplitude-zero pitfall flagged + Jacobian-block sanity check mandate).
...
**Critical-path blockers (r8.7) — Round 7 Stage 0 RETRACTED; R7.1 + R7.2 fresh-session candidates scoped with frozen pre-registrations:**

1. **Round 7 Stage 1 — F17-K Phase 6 multi-seed sparse eigensolver implementation** (~300 LOC, fresh session) — STRENGTHENED in r8.7 from single-seed (GT only) to four seeds at SAME run: `GT_corpus`, `F17K_cos_endpoint` (R/r=3.40), `F17K_s11_endpoint` (R/r=1.03), `vacuum_control` (negative control). Closes basin-vs-corpus geometry question + (2,3) eigenmode existence question in one run. Three-mode falsification structure frozen in `P_phase6_eigensolver_multiseed` (N=32, R_anchor=10, ω_Compton ± α tolerance, Q ± 5%, shape correlation > 0.85). Mode (I) GT_corpus passes → corpus vindicated; (II) F17K endpoints pass / GT fails → engine basin ≠ corpus geometry; (III) no seed passes → Round 8 architectural rework. Negative-control failure indicates eigensolver methodology bug. Two open auditor flags for fresh-session implementer (per §13.5k + A35): mandatory Jacobian-block sanity check before `eigsh` (K4↔Cosserat off-diagonal block norms must be nonzero, else eigensolver returns Cosserat-only modes and mode (III) interpretation is contaminated); shape-correlation > 0.85 PASS gate may be over-strict against pre-coupling X4a profile (record as informational, flag adjudication if other criteria pass alone).
...
**r5 → r6 → r7 → r8 → r8.3 → r8.4 → r8.5 → r8.6 → r8.7 retraction / closure summary:**
...
*End of manual r8.7 (synchronous edit after Round 7 Stage 0 self-audit retraction + R7.1 multi-seed reframe). **Mid-cycle self-audit caught Rule 6/8/10 framing error before propagating into ~300 LOC of R7.1 implementation.** Next update (r8.8) triggered by: R7.1 multi-seed sparse eigensolver fresh-session run result (three-mode falsification); R7.2 topological pair injection driver pre-registration + result; A26 fix commit + retroactive caller audit; `use_lagrangian_emf_coupling` cleanup; default-flip of `disable_cosserat_lc_force`; Flag 62-A first-law closure attempt; or any engine commit per §1.2 maintenance protocol.*

*Full r9 rewrite of §3 (physical model under three-storage-mode framing per [doc 66_ §17.2.1](66_single_electron_first_pivot.md)) and §15 (derivation chain with three-entropy distinction + area theorem from Ax1+Ax4 + K4-TLM exhaustion + A28 double-counting structural finding + A30 corpus-duality falsification + A32 Golden Torus geometric instability + A33 smallest-coupled-oscillator structural reading + A34 G-13 injection-profile-upgrade requirement + A35 corpus-bypass-at-precondition-level methodology lesson) still deferred until Round 7 closes — multi-seed Phase 6 eigensolver + topological pair injection are the gates. See §1.5 for canonical Round 6 content pointers.*
...
*Round 7 Stage 0 self-audit milestone (r8.7): mid-cycle pivot caught corpus-bypass framing **before** R7.1 implementation committed to wrong scope. Two commits and ~2 hours of work spanning the scaffold-and-retract cycle: basin-audit-as-Stage-0 framing (`1bc1652`) → within-session COLLABORATION_NOTES re-read triggered by Grant directive ("review COLLABORATION_NOTES, are you trapped in known patterns?") → Rule 6/8/10 self-audit → reframe pivot to multi-seed R7.1 (`c69e79c`). Frozen pre-registration `P_phase6_eigensolver_multiseed` replaces retracted `P_basin_audit_GT_stationarity` (Rule 12 body preservation); R7.1 strengthening from single-seed (GT only) to four seeds (GT + F17K_cos + F17K_s11 + vacuum_control) at N=32 with three-mode falsification. **A35 caught earlier in cycle than A22 / A30 predecessors** — within-session, before implementation. Audit-first + creeper-checking discipline working at full strength.*

*Methodology arc summary across Round 6 + Phase 5 resume + Round 7 Stage 0 (r8.0 → r8.7): A28 + A29 + A30 + A32 + A33 + A34 + A35 — seven depth-level corrections + empirical anchorings, all corpus-grounded, each producing an empirical or methodology finding rather than just a fix. Audit-first discipline working at full strength: each cycle catches the prior framing's hidden assumption and produces evidence. The unifying finding across A30, A32, A34: **dynamics don't stabilize unprotected localized topology; topology must be encoded as persistent structural feature (ansatz)**. This holds at three scales: single-electron eigenmode (Round 6); single-electron linear stability (Round 6); pair injection profile (Phase 5 resume). Confirms doc 03 §4.3 framing across the framework's full topology-encoding stack. The unifying finding across A22 + A30 + A35: **corpus-bypass errors at three different layers — operator level (A22), corpus-claim falsification level (A30), methodology-precondition level (A35)** — caught at decreasing latency from production to within-session.*

*Notable methodology meta-lessons (per A33 + A35 + auditor framing): (1) bootstrap-chain calibration anchor (Q=137 sanity check at single-bond level) should be a bring-up gate for any numerical AVE engine, not a post-hoc realization 8 sessions in; (2) gate-firing investigations should decouple (α) mechanism / (β) persistence / (γ) reach BEFORE running, not conflate them in one driver; (3) **before scaffolding a "diagnostic precondition" upstream of an existing R7.x stage, grep the corpus for the AVE-native tool that addresses the precondition's question; if it exists, strengthen R7.x's scope rather than building parallel infrastructure** (per A35). All three are COLLABORATION_NOTES Rule 8 strengthening candidates for Round 7+.*
