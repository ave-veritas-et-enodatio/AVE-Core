...
- **(a) is better for the hand-audit, and is probed-viable on the AC path.** T1's probe put an SI-scale `T` element through `.AC` and got the closed-form resonance to within the grid step, with no tolerance overrides. So the a-priori conditioning worry is **de-risked** *for AC* — same register as the T1 pin at `:53` (checker-audit SCX-09): this is evidence from an **untracked, non-banked Phase-0 scratch probe**, not a retirement. Retiring **the epic §8 PARK trigger** is a Phase-1 act on a tracked driver under the epic §5.2 reproduction gate. *(This bullet sits under T6; the commit that first harmonized it mis-titled the location as the T4 row.)*
