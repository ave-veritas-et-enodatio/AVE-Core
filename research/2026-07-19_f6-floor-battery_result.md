...
🔴 **R-3 (floor/E0 corrected; same corruption as the arm result).** The prior `floor/E0` column {0.14, 0.47, 0.93, 1.40, 2.33} was inflated **~2.55×**. Banked values (`e_floor_expected/e0` on the shared `Δω=0.050` comb, `e0=2.14143`): {0.055, 0.18, 0.37, 0.55, 0.92}. The per-mode ρ framing is unchanged (ρ = E_floor_per_mode/E_signal_per_mode).

| ρ | floor/E0 (banked) | all-seeds clean | max drift | #id-fail | #over-tx | #clamp | excess-identity | jitter (CoV) | c_excess finite | broke-tare shown |
|---|---|---|---|---|---|---|---|---|---|---|
| 0.3 | 0.055 | ✅ | 2.3e-14 | 0 | 0 | 0 | 2.3e-14 | 0.038 (0.13) | ✅ | — |
| 1.0 | 0.18 | ✅ | 1.8e-14 | 0 | 0 | 0 | 1.8e-14 | 0.046 (0.17) | ✅ | — |
| 2.0 | 0.37 | ✅ | 2.0e-14 | 0 | 0 | 0 | 2.0e-14 | 0.056 (0.18) | ✅ | — |
| 3.0 | 0.55 | ✅ | 1.6e-14 | 0 | 0 | 0 | 1.6e-14 | 0.066 (0.19) | ✅ | — |
| 5.0 | 0.92 | ✅ | 1.5e-14 | 0 | 0 | 0 | 1.5e-14 | 0.082 (0.21) | ✅ | ✅ |
...
| **FB3** excess-tare (Dp-3 corrected) 🔴R-3/R-7 | `c_excess` finite & ≥0; broke-tare shown; *(form-agreement = checklist, not a gate — see below)* | `c` finite, **`c∈[0,1]` at every ρ on this comb (`c_range01_all=True`)**; the alternate `√(1−E_bath/E0)` form breaks at ρ=5 (broke-tare shown ✅) | ✅ FB3 |
| **FB4** statistics-not-realization (Dp-2 corrected) 🔴R-7 finding-1 | realizations differ; ensemble mean stable — **`SEM/mean < 0.10` (derived fireable gate, §Dp-6)**; CoV finite (secondary sanity) | realizations differ (`‖Δx‖ 0.98→2.19`); **`SEM/mean 0.069→0.093 < 0.10` ✅** (ρ=5 at `0.093` is near the edge — the 6-seed ensemble is marginal, the gate is fireable); **CoV 0.170→0.228** (arm budget) | ✅ FB4 |
...
## 6 · Amendment §D-post-review (adversarial review wf_932a21ad, PR #734; append-only — the §D charter body and the §2 battery are frozen; corrections stated here, Rule-11/Rule-12)

The review of the arm consuming this certificate surfaced three floor-battery repairs. No PASS gate is loosened; the one gate change (§Dp-6) is a **tightening** and the banked data still passes, so the `FLOOR-METER-VALID-BAND[0,5]` verdict is UNCHANGED (numerics reproduce bit-for-bit).

- **§Dp-6 — FB4 SEM/mean fireable hardening (finding 1; the one prescribed gate change).** The frozen `stats_carry = realization_differs AND CoV < COV_CHAOS(=1.0)` sub-gate was a **checklist, not a gate**: CoV 0.17–0.23 ≪ 1.0 is trivially satisfied — nothing realistic could fail it. Hardened to require the **ensemble mean to be a stable read**: `SEM/mean < SEM_MEAN_MAX = 0.10` at every ρ. Derivation (fireable, not tuned-to-pass): the ensemble-mean read carries the FB4 arm-ensemble requirement iff its own fractional uncertainty `SEM/mean = CoV/√N` is below 10%; with the frozen budget `CoV≈0.23` and `N=6` the *expected* value is `0.23/√6 = 0.094`, so the 6-seed ensemble sits **at the edge of adequacy** — a genuinely fireable bound (a noisier bath, or fewer seeds, fails it). Banked: `SEM/mean = 0.069 (ρ=1) → 0.093 (ρ=5)`, both `< 0.10` ✅ (ρ=5 marginal). `COV_CHAOS` is **relabelled** a secondary sanity bound (finite-CoV guard), not the FB4 pass condition. The rows (`excess_plateau_sem_over_mean`) were already banked; only the gate composition changed.
- **FB3 form-agreement relabel (finding 3).** The FB3 "the two D2 tare forms agree `< 0.02`" leg is relabelled a **checklist, not a gate**: `c_form_diff_max ≈ 1e-16` (machine-zero) because the two forms are algebraically identical at the working window — "they agree" is a tautology, not a discriminating test. FB3's load-bearing content is `c_excess` finite & ≥0 (+ `c∈[0,1]` on this comb, R-3) and the broke-tare demonstration.
- **Finding-4 both-ways sharpening (for Grant's flip adjudication — FLOOR-LEDGER-ARTIFACT vs FLOOR-METER-VALID-BAND).** The frozen-literal verdict is `FLOOR-LEDGER-ARTIFACT` on the `Δω=0.030` ladder `{0.3,1.0,3.0,10.0,30.0}`. Both directions now stated: **(pro-flip / toward VALID)** the frozen-literal FB3/FB4 *failures* live at `ρ=10,30` — **outside** any readable floor band (regime-discipline: an unreadable-regime cell cannot condemn the meter), so the artifact verdict is driven by cells the meter was never certified for. **(anti-flip / toward ARTIFACT)** the in-band evidence is not clean either: on the **swapped denser comb** `Δω=0.030` the identity already breaks at `ρ=3` (2/6 seeds clamp) — an *in-band-adjacent* FB1 failure, so the readable band is comb-fragile, not universal. Both banked; the flip is Grant's at merge (the lane STOPS at FLOOR-LEDGER-ARTIFACT if the frozen-literal binary is preferred, and the arm does not fire).
