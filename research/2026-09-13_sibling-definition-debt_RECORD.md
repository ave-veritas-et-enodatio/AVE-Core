...
> **Docket receipt:** [`2026-09-13-ruling-r60-sibling-def-debt-o1-o2.md`](../_orchestration/docket-entries/2026-09-13-ruling-r60-sibling-def-debt-o1-o2.md) (minted after the 2026-09-13 audit; number provisional until the Orchestrator confirms). That entry also records the audit's Call 1 (the fence reaches #1034's "A1 2-port" / G-PORTS → blocks-merge there) and Call 2 (the "reviewed" trigger stays unsatisfied until #1034's body matches its head and Grant/Orchestrator sign).
>
...
**Grep receipt (the debt's own name has no corpus home; re-run at the 2026-09-13 Claims Gate).**
At `origin/main` @ `451bf480`, `git grep -n -iE` over `manuscript/`, `research/`, `_orchestration/`:
`sibling[ -]definition`, `definition[ -]debt`, `sibling[ -]far[ -]field` → **0 hits**;
`lattice[ -]free` → **1 raw hit**, the substring of *"lattice freeze-in"* at
`research/2026-06-06_simulation-assumptions-audit.md:158` (`\b`-anchored → 0) — **0 hits in the
north-star sense**; `port famil` → **1 raw hit**, the hyphenated compound *"licensed-port family"* at
`research/2026-07-17_regime-iv-dissipation-audit_items.json:1752` (a different head noun) — **0 hits
in the brief's sense**; `two-port famil` / `Link[–-]Γ` / `dilatation *→ *Z` → **0 hits**; `two ports`
→ hits in 11 files, the EM↔mechanical domain-boundary rows (e.g. `device-circuit-models.md:216`) plus
K4 **bond-port** uses — none in the family sense. The superseded wording family of this record
(`far-field read-out`, `read-out famil`, `two sector contents`, `one enclosing boundary`) → **0 hits**.
This is a **name-level** sweep: the packaging below is the first in-repo statement of the split **by
name**; the content it restates is canon, pinned line by line in §3, and it derives nothing.
...
| main moved (2026-09-13, gate time) | `origin/main` advanced `451bf480` → `5619dd56` (#1041 fired-riders own job; #1042 inbound cite-shift checker; six commits). No canon leaf changed (`git diff --stat 451bf480..5619dd56` = `.github/workflows/verify.yml`, `Makefile`, one docket entry, one tools script); merge-base of this branch unchanged; `git merge-tree` vs the new main: 0 conflicts. Pins below hold at both SHAs. |
| Claims Gate round (2026-09-13) | seven-dimension read-only gate + completeness critic + three-lens adversarial verification (packet: [`2026-09-13_sibling-definition-debt_GATE-PACKET.md`](2026-09-13_sibling-definition-debt_GATE-PACKET.md)). Grade **unchanged: WALK / DEFINITION-DEBT**. 1 FAIL (the "three copies" count above) and 20 FLAGs survived verification, 19 findings were refuted, 37 hygiene FLAGs were left unverified by the verify cap; the surviving repairs are applied in this commit and itemised in §2 C9(e). |
...
| L6 | [`vocabulary-register.md`](../manuscript/ave-kb/common/vocabulary-register.md) | `def-portmp` `:506-518`, `def-tk1xfm` `:434-452`, `def-1mpanl` `:938-950`, `def-q1escn` `:490-502`, `def-9a4f07` `:583-600`, `def-3638f2` `:241-256`, `def-69f472` `:160-172`, `def-cmdiff` `:1533-1549` | the proposed port↔DOF↔sector map (the closest existing object to a qualified "port family"; the A1 slot is a bound response with no register port — the O1 R38/R40 fence); the impedance-analogy sign convention; the common-mode MODE-vs-INFLUENCE hazard | `2e480851` 2026-08-25 |
...
| L9 | [`electron-identification.md`](../manuscript/ave-kb/vol2/particle-physics/ch01-topological-matter/electron-identification.md) | `:1-120` | "self-trapped" = boundary-condition sense; the three orthogonal identity labels; core vs far-field (D3 COEXIST); the three-way FORM/VALUE accounting. **Leaf-level fence carried into every cite of it here** (`:24` — *"Everything below characterizes the electron's *interior* as a finite, mechanistic, **honest PEER** re-representation"*; no property of that leaf may be read as an interior AVE-distinct claim) | — |
...
Canon for the mass side: [`master-equation.md:138-141`](../manuscript/ave-kb/vol1/dynamics/ch4-continuum-electrodynamics/master-equation.md)
...
— `| **MASS** — A1 dilatation | trace-of-translation … | longitudinal bond compliance $\to Z_{\mathrm{bulk}}$ |`, and `:58-59` — *"the MASS, CHARGE, and deviatoric-shear rows are **mechanical**"* (the sentence runs on at `:59` under a 🔴 R40-B2a NEEDS-RE-DERIVATION stamp on its ρ×speed half; used here for the **sector attribution** only — which rows are mechanical — never for a bulk speed).
...
Also the pair interaction the corpus HAS is screened: [`form-deriving-value-importing.md:588-589`](../manuscript/ave-kb/common/form-deriving-value-importing.md) — *"Yukawa screening ⇒ no source-free far-field"*.
...
`:168-169` (EE-mapping comment) — `an electret's frozen state is a` / `DIPOLE (net-zero monopole), while clause S deposits a NET MONOPOLE flux`.
→ in this packaging "sibling" means *shared far-field FORM and shared Gauss-surface read*, not shared source mechanism.
...
The mass family IS the A1 common-mode **irrep** ([`k4-port-irrep-decomposition.md:11`](../manuscript/ave-kb/vol1/operators-and-regimes/ch6-universal-operators/k4-port-irrep-decomposition.md) — *"the $+1$ eigenvector is the $A_1$ "common mode" (all ports equal)"*) and it is read as a **differential** bias ([`translation-circuit.md:116`](../manuscript/ave-kb/common/translation-tables/translation-circuit.md) — `**Differential bias** — a bias gradient across the ladder (differential varactor operating points; the load-transfer / gravity-ledger direction)`). Both words are needed; keep them apart.
...
(d) **Stamp-placement residue (corrected at the 2026-09-13 gate — the earlier "unadjudicated" reading of this item is withdrawn):** [`def-9a4f07`](../manuscript/ave-kb/common/vocabulary-register.md) `:594` (sharpened 2026-08-07) still carries *"it is a propagating P-branch wave"* and carries no inline R40 stamp — but the register's own R40 batch-2a row for `:870` (`:1492-1503`) **names it**: *"covers :594 def-9a4f07 sharpen ("it is a propagating P-branch wave")"*, with the re-derivation routed to the bound-response constitutive law. So the clause IS adjudicated under R40 (the same "P-branch-propagates" family demoted at [`master-equation.md:178-188`](../manuscript/ave-kb/vol1/dynamics/ch4-continuum-electrodynamics/master-equation.md)); only the inline stamp at `:594` is missing. Flag for the R40 batch owner as stamp placement, not as an open claim.
(e) **This record's own pins, caught at the 2026-09-13 Claims Gate and repaired in place** (superseded → repaired): `k4-port-irrep-decomposition.md:10` → `:11` (×2; `:10` is blank); `refractive-index-of-gravity.md:14` → `:15` (×2; `:14` is the `$$` opener); `form-deriving-value-importing.md:589` → `:588-589`; `node-up-small-large-signal.md:57` → `:58-59`; `eq_axiom_5.tex:169-170` → `:168-169`; `2026-08-29_picture-lock-spillover.md:65-70` → `:66-70` (×2) and `:65` → `:130` for the S9 join phrase; `vocabulary-register.md:438` → `:445` for the def-tk1xfm ceiling phrase; `master-equation.md:139-143` → `:138-141` (full sentence containment); the L11 read-range for `gravitational-coupling-constant.md` (`:1-25`, exceeding the 22-line leaf) → `full`; a missing pin added for the `clm-ze4clw` caveat (`common/claim-quality.md:606`). Same defect class this section catalogues; recorded rather than hidden.

**C10 — in-flight overlap on the same join.** PR **#1034** (`analysis/2026-08-29-tank-state-h1-join`; H1 CLOSES — *"three circuit objects, not one mesh"*, KEEP-ALL, WALK-GRADE) and PR **#1033** (`research/2026-08-28-qpoint-constitutive`; ε₁₁ as A1 Q-point) work the circuit-object side of A1 ⊥ T2. Neither is on `main`. This record is the **far-field / observable side** of the same split; it pre-empts none of H2–H6 ([`2026-08-29_picture-lock-spillover.md:66-70`](2026-08-29_picture-lock-spillover.md)).
...
Whatever is inside, the far field reads **boundary data** only — `:59` (electron-identification) — *"identity is topological boundary data, carried by the far-field projection."* "One enclosing boundary" is canon's own $\partial\Omega$ — [`boundary-observables-m-q-j.md:33`](../manuscript/ave-kb/common/boundary-observables-m-q-j.md) *"For any localized region $\Omega$ in the substrate enclosed by a $\Gamma = -1$ saturation surface"* — and the electron's property-3 TIR cavity (electron-identification `:32`); it is not a coinage.
...
Group-theoretic root: [`k4-port-irrep-decomposition.md:11`](../manuscript/ave-kb/vol1/operators-and-regimes/ch6-universal-operators/k4-port-irrep-decomposition.md) — `$V_{\text{4-port}} = A_1 \oplus T_2$` (`clm-j550uh` 0.85).
**Carrier disclosure (collision-set item 13).** The "K4 4-port" decomposition is the abstract $T_d$ scattering-amplitude decomposition, not a bond count — [`vol9/ch1-general-description/index.md:20`](../manuscript/ave-kb/vol9/ch1-general-description/index.md) *"The physical connectivity is $z=3$ (three nearest-neighbour bonds). The "K4 4-port" $A_1\oplus T_2$ irrep decomposition (ch3/ch11) is the abstract $T_d$ scattering-amplitude decomposition of the K4-TLM scatter matrix ($\dim 4 = 1+3$), *not* a physical bond count"*; the A1 ⊥ T2 orthogonality this packaging rests on is ratified at `master-equation.md:20,:33,:36` independently of the coordination number.

| | **Family A — weight / inertia (A1 dilatation)** — a "port family" in the qualified def-portmp sense only; a **bound response, NOT a register PORT** (R38/R40 fence) | **Family B — charge / EM** (the static Link on the **gapped mechanical Cosserat ω** sector, read through the **massless EM $T_2$** channel — two rotation flavours, C4) — a "port family" in the qualified def-portmp sense; its EM read-out channel is the register's P1 port |
|---|---|---|
| **content** | rest-mass **store** = A1 longitudinal dilatation — [`master-equation.md:31`](../manuscript/ave-kb/vol1/dynamics/ch4-continuum-electrodynamics/master-equation.md) *(grade-ASSIGNMENT, PR#260 — `"mass = A1" is RATIFIED-CONSISTENCY — the adjudicated grade-ASSIGNMENT (PR#260), **NOT driver-validated**`)*; the **work-doing reactive MASS STORE**, [`resonant-lc-solitons.md:129`](../manuscript/ave-kb/vol4/circuit-theory/ch1-vacuum-circuit-analysis/resonant-lc-solitons.md) | the **static Link integer** $\mathcal{Q}=\mathrm{Link}(\partial\Omega,\mathbf F)\in\mathbb Z$, [`boundary-observables-m-q-j.md:20`](../manuscript/ave-kb/common/boundary-observables-m-q-j.md) (`clm-ze4clw`); STATIC and reactive — [`def-3638f2`](../manuscript/ave-kb/common/vocabulary-register.md) `:256` (a **PROPOSED SHARPEN**, 2026-06-24 — *"awaiting Grant ratification; NOT yet canonical"* — the line's own opening fence; cf. K3) *"the $(2,3)$ phase-space winding is **STATIC** (a deformation-invariant texture, NOT a dynamical/energetic time-orbit) and is the boundary linking integer"*; the **static reactive CHARGE BOUNDARY**, [`resonant-lc-solitons.md:128`](../manuscript/ave-kb/vol4/circuit-theory/ch1-vacuum-circuit-analysis/resonant-lc-solitons.md) |
...
| **domain / impedance** | **mechanical** $Z_{\mathrm{bulk}}$ ([`node-up-small-large-signal.md:51`](../manuscript/ave-kb/vol4/circuit-theory/ch1-vacuum-circuit-analysis/node-up-small-large-signal.md)); at the cage $Z_{bulk}\to0\Rightarrow\Gamma_{bulk}=-1$ — [`bulk-impedance-at-saturation-boundary.md:76`](../manuscript/ave-kb/vol3/cosmology/ch15-black-hole-orbitals/bulk-impedance-at-saturation-boundary.md) `| Bulk channel | $Z_{bulk} \to 0$ at TIR wall $\Rightarrow \Gamma_{bulk} = -1$ |` | **mechanical** $Z_{\mathrm{shear}}$ carries the Link ([`node-up-small-large-signal.md:52`](../manuscript/ave-kb/vol4/circuit-theory/ch1-vacuum-circuit-analysis/node-up-small-large-signal.md)); the **EM** $Z_{\mathrm{EM}}\equiv Z_0$ is the read-out channel — [`resonant-lc-solitons.md:127`](../manuscript/ave-kb/vol4/circuit-theory/ch1-vacuum-circuit-analysis/resonant-lc-solitons.md) `**$Z_{\mathrm{EM}}\equiv Z_0$ = the radiative CARRIER PORT**`; bridged only by the TKI transducer ([`def-tk1xfm`](../manuscript/ave-kb/common/vocabulary-register.md) `:445` *"identity-by-translation, NOT a derivation"*) |
...
both far fields are $1/r$ in FORM (the charge side's $1/r$ is **asserted, not derived** — O3; the mass side's $1/r$ bias is the elliptic solve's free geometry — K5); both are FORM-derived / VALUE-imported
...
(`trampoline-framework.md` is a picture-first canonical-reference leaf whose frontmatter states it *"asserts no independent derivation of its own"*; the sentence is cited as the **ratified wording** — skill check 6 — not as a derivation; §5.3's own pointers are Vol 3 Ch 3/4.)
...
$\mathrm{Link}(\partial\Omega,\mathbf F)$ is an integer ([`boundary-observables-m-q-j.md:20`](../manuscript/ave-kb/common/boundary-observables-m-q-j.md) `$\mathrm{Link}(\partial\Omega, \mathbf{F}_{\text{substrate}}) \in \mathbb{Z}$`); $\oint_S\mathbf u\cdot\hat n=4\pi B(M)$ is a flux *"over any enclosing exterior surface"* ([`eq_axiom_5.tex:71`](../manuscript/common_equations/eq_axiom_5.tex)).
$\ell_{node}$ enters only through the VALUE imports — $\xi_{topo}\equiv e/\ell_{node}$ on the charge side; on the mass side through $\kappa=c^4/7G$, whose $G$ is value-fitted via the Machian $\xi = 4\pi(R_H/\ell_{node})\alpha^{-2}$ ([`gravitational-coupling-constant.md:10`](../manuscript/ave-kb/vol3/gravity/ch01-gravity-yield/gravitational-coupling-constant.md)), and through the $\ell_{node}\equiv\hbar/(m_e c)$ anchor. $\mathcal{A}_g$ is **not** an $\ell_{node}$ route: it is unvalued, *"not a falloff"* (eq_axiom_5 `:96`), and the $\mathcal{A}_g = c\,\ell_{node}^2$ hypothesis is NOT SUPPORTED.
...
(`:23` — *"The three dimensions are exhaustive: there is no fourth integrated boundary observable at this scale-invariant structure."*) is asserted at **input-only** grade (`clm-ze4clw` 0.55 — the register's own caveat at [`common/claim-quality.md:606`](../manuscript/ave-kb/common/claim-quality.md) — *"Does NOT claim a formal proof that no fourth observable exists"*).
...
([`2026-08-29_picture-lock-spillover.md:130`](2026-08-29_picture-lock-spillover.md) — *"A1 Q-point grades T2 constitutives (photoelastic), not one voltage on one capacitor"*; H1 itself is held at `:65`; closed WALK-grade on `[branch:#1034]`) — the spillover's word is "photoelastic"; on `[branch:#1034]` that materials word is retired as load-bearing in favour of the EE-native lattice names **varactor-biased GRIN / impedance-gradient**. Either way: a constitutive grading, not a shared port. This record adds no join and, per O2, asserts no co-ownership.

---

## §4 — Ledger: CANON / RESTATEMENT vs OWED

*(The ledger grades **statements**, not leaves; each pinned leaf keeps its own grade in its frontmatter and `claim-quality.md` entry, and every input-only row is used as input only.)*

| # | statement | grade | pin | what would move it |
|---|---|---|---|---|
| K1 | A1 ⊥ T2 grade orthogonality; no shared phasor; FOC-independent | **CANON** (Grant-ratified) | `master-equation.md:20,:33,:36` (`:20` also carries a routed R40 ledger row, ⚑ BIAS-DEBT / PAST-WALL, for its *"trapped acoustic compression energy"* clause — `:166-176`; the two-objects sentence is untouched by it) | — |
...
| K5 | clause S / G / Q; bound response, no port; $u_0=B\hat r/r^2$; $\varepsilon_{11}=7GM/c^2r$ | **CANON** (source law, POSTULATED; R38/R40/R43–R55) | `eq_axiom_5.tex:68,:75-77,:84,:94,:100`; `axiom-register.md:338-342,:423`; the 'no port' half at `master-equation.md:138-141` + R38 docket `:14-16` | — (postulated, not derivable from Ax 1–4 by its own status line) |
| K6 | $n(r)=1+2GM/c^2r$; compression = impedance gradient; $\Gamma_{\mathrm{EM}}=0$ under SYM | **CANON** (`clm-rd9cjm` 0.55, `clm-3bwhad` 0.55) | `refractive-index-of-gravity.md:15`; `boundary-observables-m-q-j.md:110`; `master-equation.md:115` | — |
| K7 | two clocks (A1-Op14 time dilation; T2 flywheel spin) | **CANON** | `master-equation.md:38` | — |
| K8 | weight ≡ inertia derived via /7 | **CANON** (`clm-rd9cjm` 0.55 input-only), **wording stale** | `ponderomotive-equivalence.md:12,:14,:30` | Rule-12 banner on "inductive rest mass" (hygiene lane) — the same stale wording is live in `manuscript/vol_2_subatomic/chapters/03_neutrino_sector.tex:126` and `manuscript/vol_3_macroscopic/chapters/03_macroscopic_relativity.tex:54` |
...
| R2 | three-channel ROLES (carrier port / charge boundary / mass store) | **RESTATEMENT, consistency-class** (the leaf's $Z_{bulk}$ **formula** row is R40-B2a-stamped at `:123`; the batch note's scope is that row only, `:209-219`; the ROLE bullets `:127-129` carry no stamp) | `resonant-lc-solitons.md:123,:127-129` | R40 re-derivation of the Z_bulk row |
...
| R4 | §3 P0–P5 above (the packaging sentence, O1/O2-compliant) | statement-by-statement **RESTATEMENT** of K1–K11; the assembly itself is **WALK-LEVEL** (skill tag) = the record's WALK / DEFINITION-DEBT grade | this file | Claims Gate (noun RULED, O1; co-ownership excluded, O2) |
| **O1** | **the one-place definition** — two sector contents (two objects), two far-field read-outs at one boundary — stated nowhere in `ave-kb` (grep receipt above). **NOUN RULED 2026-09-13:** qualified "port family" only; R38/R40 fence mandatory on every use (A1 bound response is NOT a register PORT, $\mathrm{Re}\,Z>0$); never unqualified for the A1 slot; "read-out family" not adopted | **RULED (noun) / OWED-DEFINITION (the sentence's KB landing)** | docket entry `_orchestration/docket-entries/2026-09-13-ruling-r60-sibling-def-debt-o1-o2.md` (R60, provisional) + the ruling block at the top of this file; `port-register.md:21,:26`; `master-equation.md:138-141` | the sentence lands in a KB leaf under review **only after #1034 (H1)**; Claims Gate |
...
| **O6** | the 1:1 Kirchhoff map of DC strain; H2–H6 | **OPEN-IN-WALK** (H1 closed WALK-grade on `[branch:#1034]`) | open-item `acdc-gravity-circuit-map`; spillover `:66-70` | #1034 review; H2–H6 walks |
| **O7** | vocabulary: "port family" — noun RULED (O1) but still without a `def-` node (`def-portmp` is proposed); "weight" (no `def-` entry); $B(M)$ glyph across the A1 ⊥ T2 fence | **OWED-VOCAB / ROUTED** (noun ruled; nothing minted here) | `def-portmp` proposed; open-item `axiom5-b-glyph` | a vocabulary PR under review (after #1034); Grant glyph call |
| **O8** | the four cite-shifts, the `def-9a4f07:594` stamp placement (C9), and the thirteen stale `.claude/worktrees/wf_*` copies in the primary checkout that trip `r40_quote_claim_strength_number_check.py` (§0) | **HYGIENE, routed** | C9 (a)–(e); §0 commit-gate row | auditor / R40 batch owner / whoever owns the stale workflow worktrees |
