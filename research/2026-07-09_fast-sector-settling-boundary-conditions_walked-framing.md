...
> **🔴 Rule-12 banked-negative upgrade (2026-07-10) — W5-iii is now a BANKED NEGATIVE, upgraded from "unsupported-at-prereg-strength" (KEEP-BOTH).** The W5-iii proposal above (item iii: in the doubly-bounded cavity BOTH (2,3) integers become discrete, knobless BC-quantized mode indices, so #417's ratio-tracks-detuning kill "structurally cannot fire on them") has been tested twice; this note records the two-axis history and upgrades the retraction. Body above preserved verbatim per Rule 12.  <!-- rule12-freeze: base=f5e518f95f1fe0a665e6756910986f295268adff region=above offset=0 lines=15 bytes=6219 sha256=e6a9b1b89cdcf6819e10e111d65154d32e098b5b4ee3ed9363f26979b8fd3195 -->
>
> **First read — x34 / PR #612 (result: [`2026-07-09_tethered-pivot-mode-locking_result.md`](2026-07-09_tethered-pivot-mode-locking_result.md) §2, §4.1).** W5-iii was withdrawn at **"UNSUPPORTED-AT-PREREG-STRENGTH, amended axis leaning negative"** — NOT a hard-fired TRACK branch. Two-axis read (KEEP-BOTH): the frozen prereg-detector returned **PARTIAL** (Signature-1's absolute `staircase_fraction`=0.4286 confounded by the free-control saturation artifact the anchored run shares), while the frozen-clean tracking leg (`track_R²`=0.9799) and the amended control-subtracted axis (`excess_staircase`=0.0714) both leaned negative. A walked PROPOSAL needs only be unsupported-at-prereg-strength to withdraw, so a clean fired negative was not claimed then.
>
> **Second read — x34b / PR #626, merged 2026-07-10 (prereg: [`2026-07-10_tethered-pivot-rerun_prereg.md`](2026-07-10_tethered-pivot-rerun_prereg.md); result doc §10 consequence).** The **control-subtracted excess detector was FROZEN A PRIORI as THE primary rule on a SINGLE axis** (roles inverted vs #612 — the excess axis is now the pre-registered primary; the #612 two-axis output is preserved intact, not redefined-in-place). Banked on the non-saturated window (ω_s ≤ 1.275, 24 pts): **TRACK — `excess_staircase`=0.0435, `track_R²`=0.9901, `excess_jumps`=0.** Lock-fireability proven in-window: the planted-LOCK positive control fired **LOCK** (staircase 0.7857, excess 0.6429, 3 jumps), so the null is a real read, not a dead detector. The anchored (2,3) traversal tracks the carrier knob just as the free orbit does — a BC-quantized *knobless* mode index cannot follow a continuous knob (the forbidden-knob logic), so **#417's detuning-kill DOES fire on the (2,3) here.** W5-iii is therefore **BANKED NEGATIVE**, the 7th entry on the orchestrator's miss-ledger (0-for-6 → 0-for-7; `ave-discrimination-check` v1.8).
>
> **Sector fence intact (per (d) above — the negative is scoped, not overreaching).** Untouched: **charge = Link(∂Ω, F) ∈ ℤ** (the STATIC real-space linking integer, a different coordinate from the phase-space temporal traversal tested here — still exactly quantized); **mass = A1** (#260); the **(2,3) as a geometric Clifford-torus embedding** (the rigid template still carries it). What is dead is *only* the claim that a wall-anchored traversal promotes the (2,3) to a knobless dynamical mode index. **Substitution-not-retraction (Rule 12):** no successor hypothesis is refilled into the tested-negative slot.  <!-- rule12-freeze: base=f5e518f95f1fe0a665e6756910986f295268adff region=above offset=0 lines=5 bytes=1904 sha256=0a60751eca7af4a36e30c4cde4dd8c7195ca560fb4a2134dd8639caffaa6e1f9 -->
