...
**Rule-12 amendment 2026-09-13 (PR #1034 Lane A review packet, 12 items; Grant + Math GO).** Bins, gates, and identities are **not** re-frozen; this is vocabulary, provenance, and pins. (1) **PORT-noun fence (R38/R40; blocks-merge item).** Wherever this freeze wrote “2-port” of Object 1, read **2-terminal element** (the A1 bond-compliance capacitor). The A1 slot is a bound response — *“no independent propagating branch, no port and zero longitudinal characteristic speed”* (`master-equation.md:138–141`, R40); R38 docket `_orchestration/docket-entries/2026-08-09-ruling-r38-bound-response.md:14–16`: *“no autonomy, no independent flux, no port”* — and a register PORT is *“RADIATIVE iff \(\mathrm{Re}(Z)>0\)”* (`port-register.md:21`). G-PORTS substance is unchanged: Object 1 terminals \(\neq\) Object 2/3 terminals. Sites edited in place with the superseded words quoted: §0.9, §0.11, §4 Bin A, §5 positive control. (2) **§3.1 “rung T2”** means the T2 **irrep / photon-port sector** (`k4-port-irrep-decomposition.md:11`: \(V_{4\text{-port}}=A_1\oplus T_2\)), **not** the `translation-phase-space.md:100` ladder rung T2 (the Golden-Torus bond-pair phasor plane); the §3.1 text is left as frozen. (3) **§0.11 pin:** `H1 :104` is `@ 48eead40` (the paragraph *“Do not identify Object 3 with Object 1”*; `:106 @ 28990d1e` after H1’s 2026-09-04 header insert). (4) **Footer restored** to its 2026-09-04 (`7ffca034`) wording. The 2026-09-04 characterization commit (`75ba11cc`) had rewritten the frozen provenance footer to *“characterization commits second as `research/2026-09-04_s9-tank-state-chart-join_WALK.md`. **Rule-12 amend 2026-09-04** (G-PORTS / cite path / H1 Op16 alignment) is on this same freeze file.”*, dropping *“if any”* and *“still no characterization.”*; the characterization pointer lives in the 2026-09-04 (characterization) amendment line above, so the provenance footer reverts. (5) The superseded G-PORTS wording is now quoted in the 2026-09-04 (Findings 1–3) amendment above; it was owed under Rule 12.
...
1. **SECTOR / OWNERSHIP:** three **named** circuit objects on one K4/srs graph, KEEP-ALL from H1. The orthogonal split is A1 ⊥ T2, not three orthogonal 2-ports. **Object 1 (A1)** owns DC prestress: bond-compliance capacitor \(C_{\mathrm{eff}}=C_0/S\) (`CLAUDE.md:73`; `nonlinear-vacuum-capacitance.md:14,:26–28`). **Object 2 (EM-transverse photon port)** owns light’s traveling-wave chart \((V_{\mathrm{inc}},V_{\mathrm{ref}})\) (`photon-ee-mapping.md:31–38,:69–71`; `z0-derivation.md:100–102`; `port-register.md:47` channel 1). **Object 3** is the lumped Faraday \(L\)-strut of that same photon-port line (\(\omega\tau\ll 1\) limit of Object 2; KEEP-BOTH descriptions, `z0-derivation.md:115–124`) — short at DC (`kirchhoff-network-method.md:18–19,:30–32`). A1 ⊥ T2 (`master-equation.md:20` — never wire the winding into the breather’s own \((V_{\mathrm{inc}},V_{\mathrm{ref}})\)). EE is operational, not ontological (`def-1mpanl` ANALOGY). **Cross-wiring check:** Object 1’s DC \(V_Q\) is not Object 2’s \(V_{\mathrm{inc}}\). Kirchhoff node \(C\) is not A1 \(C_{\mathrm{eff}}\). Identifying Object 2 with Object 3 as lumped/distributed is **not** a cross-wiring fail. The two English uses of \(T_2\) (photon-port irrep vs charge/winding) stay KEEP-ALL (H3).
...
9. **CERTIFICATION PLAN:** bins and gates frozen here, before any characterization prose exists beyond H1. **UNRUN ≠ PASSED.** This freeze has **no engine numbers** and **no driver**. Negative control named at §5: a model that places static \(\nabla V\) on Object 3, or identifies Object 1 with Object 2’s capacitor. Positive control: Object 1 and Object 2/3 remain exhibitable as distinct terminal pairs (A1 terminals \(\neq\) T2 terminals; superseded 2026-09-13: “distinct 2-ports” — Object 1 is a 2-terminal element, not a PORT, R38/R40 fence) from the cited leaves. Object 2 vs Object 3 may share the photon-port line (lumped vs distributed).
...
11. **NUMERICAL CONDITIONING:** no iterated map, no residual fit, no scan regex. Working precision N/A. Named cancellation if a later engine were to host the join: Object 1 DC vs Object 2 AC must not share a terminal pair (superseded 2026-09-13: “a 2-port”) or the inductor shorts the prestress (H1 `:104 @ 48eead40`, *“Do not identify Object 3 with Object 1”*; `:106 @ 28990d1e`). **No engine on this freeze** (`engine-capability-map.md:51,:55`: Master-Equation FDTD has the A1 cage and no winding; VacuumEngine3D has `v_scalar_from_v_inc(V_inc)` and **no independent A1 field**). A paper join is not an engine demonstration.
...
| Gravity two slots (H2) | `clm-acdc07` `form-deriving-value-importing.md:284` / `common/claim-quality.md:1374–1379` vs Op19 `operators.md:59` / `refractive-index-of-gravity.md:15` | **b** fork | REFINES — fork-record-both; do not pick |
...
**What is genuinely open and is this lane’s to close:** a **walk-grade characterization** of (i) the tank-state chart as Object 2’s coordinates with Object 1 as external grader, and (ii) how bulk \(n,Z\), clocks, and gravity-as-differential sit on that join **without** collapsing Object 1 into Object 2/3 (A1 ≠ T2). Not a new primitive. Not an engine run. Not a constitutive-slot pick (H2).
...
|---|---|---|
| **A — JOIN-HOLDS** (expected) | Chart = Object 2 state; \(A_0\) = external grader; coupling = GRIN / impedance-gradient; Smith = ratio; gravity-as-interaction = AC-read \(\nabla A\); bulk \(n,Z\), clocks **exhibited as consequences or explicitly left on H2** without a third phasor or a shared terminal pair (superseded 2026-09-13: “a shared 2-port”; Object 1 is a 2-terminal element, R38/R40 fence). | **FIREABLE** as a *modeling* verdict. Sub-claims that only restate W6 / A1 ⊥ T2 are **ENTAILED** and must be verb’d **DEMONSTRATED**. |
...
**Masquerade guard (Step 3.6, pre-armed):** Bin C and Bin D must not be absorbed into A via “persists” / “essentially the same mesh.” Primary rung is **Object 1 terminals \(\neq\) Object 2/3 terminals** (A1 ≠ T2). Object 2 vs Object 3 as lumped/distributed of one photon-port line is KEEP-BOTH, not a Bin C tell.
...
**Positive control (Step 3.8(a)):** the characterization can point at Object 1 as a distinct 2-terminal element from Object 2/3 in the cited leaves (superseded 2026-09-13: “a distinct 2-port”; not a PORT — R38/R40 fence) (Object 1 capacitor holds DC; Object 2 TL carries \((V_{\mathrm{inc}},V_{\mathrm{ref}})\); Object 3 is that line’s lumped \(L\), \(V_A=V_B\) at DC) **without identifying Object 1’s terminals with Object 2/3**. Identifying Object 2 with Object 3 as lumped/distributed is **not** a fail. H1 already ran this exhibit; the characterization must **preserve** it.
...
|---|---|
| **G-PORTS** | Object 1 terminals \(\neq\) Object 2/3 terminals. Sharing A1 terminals with the photon-port line ⇒ Bin C, not A. Object 2 vs Object 3 as distributed vs lumped of **one** T2 line is KEEP-BOTH, not Bin C. |
