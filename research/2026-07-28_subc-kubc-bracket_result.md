...
**★WHICH MODULUS THIS `K` IS — stated once here and repeated at every headline.** The frozen §1.2 fixes both ends of the hydrostatic mode to the pure dilatational response (`E = ε·I` ⇒ `dev E = 0`; `Σ = σ·I` ⇒ `dev Σ = 0`), so the bracketed modulus is the **BULK modulus `K`** — **NOT** the P-wave / longitudinal modulus `M = K + 4G/3` (`= C11` for an isotropic average) that a normal-incidence impedance `Z = ρc_P` carries, and **NOT** Young's `E`. The `T2` companion is the pure deviatoric `G`. A supplementary `M = K + 4G/3` axis is shipped ADDITIVELY in §7 — it is **not** the frozen deliverable and amends nothing.
...
### §6.1 — The P-wave / VRH modulus `M = K + 4G/3` (the modulus-identity question, ROUTED)
...
2. **★The modulus-identity question (§6.1)** — the corpus `r_Z` is built on bulk `K`, but a normal-incidence impedance carries `M = K + 4G/3`, and the axis choice moves the bin at the headline arm (`r_Z ∈ [0.513, 0.644]` on `K` vs `[0.625, 0.704]` on `M`). **A definition question, surfaced with data, not settled here.**
