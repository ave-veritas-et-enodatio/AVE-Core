...
3. **(c) ENGINE.** What remains. **The L1–L2 analytic anchor localises (a)-vs-(c) before either is trusted at L3/L4 scale** — subject to the FL-3 caveat in §2.1, and with **L0 sitting below the anchor** as the solver-vs-arithmetic numerics smoke test that clears **the lumped-LC dynamic-range arm of** suspect (b) first (`SCX-REQ-ANCHOR.2/.3`; consistency propagation of the §2.1 correction). **An L0 pass does not clear (b) globally**: L0/P1-A is a lumped $L_{CELL}$/$C_{CELL}$ tank, while L1/L2 export the bond as a lossless TL element $(Z_0,\mathrm{TD})$ and L3/L4 add network scale and `.AC` peak extraction, so step (b) still runs its own integrator/tolerance sweep at divergence time (epic §5 item 3(b): *"solver numerics (checked by integrator/tolerance sweep on the solver side, and where available a second solver)"*).
