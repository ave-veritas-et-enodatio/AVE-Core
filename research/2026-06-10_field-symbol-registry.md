...
> `cavitation_flow.py:64` and consumed by the **merged** Vol-4 probe `cavitation_core_probe.py:35,73,241,253`
...
| C-0.117 | 0.117 | corr$(\mathrm{bemf\_emf},\tau_{zx})=+0.117$ **vs** Op14 onset $\sqrt{2\alpha}\approx0.1208$ **vs** $p_G=6/z_0$ rigidity threshold — the coincidence is **WEAKER than a "three 0.117s" reading**: the Op14-onset leg is $\sqrt{2\alpha}\approx0.1208$, **NOT** 0.117 (**corrected 2026-07-14, Wall-A hygiene** — the prior "$\sqrt{2\alpha}\approx0.117$" was a value drift; $\sqrt{2\times1/137.036}=0.1208$). So the near-miss is a $+0.117$ correlation next to a $0.1208$ amplitude threshold — a coincidence-magnet, and a looser one than the drift value suggested | near-miss | `2026-06-10_bemf-feedback-smoke_result.md:79` [branch]; coincidence-magnet flag |
