...
1. **[`_orchestration/BOARD.md`](_orchestration/BOARD.md)** — the live program board: what is experimentally supported, claim-solidity distribution, what we are waiting on, open PRs, and ruling-propagation debt. **Generated** — regenerate with `python3 _orchestration/tools/generate_board.py` if the `origin/main` SHA in its header is behind. Open decisions live one-per-file in [`_orchestration/open-items/`](_orchestration/open-items/). *(`_orchestration/index.md` was FROZEN 2026-08-13 — historical session-focus record only.)*
