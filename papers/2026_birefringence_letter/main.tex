...
\emph{(iv) Two open honesty items.} The QED denominator $\alpha/(15\pi)$ is a
...
the order of magnitude. And, from the paper's self-contained vantage, the elliptic
form $\sqrt{1-(E/\Ec)^2}$ is one \emph{chosen} member of a one-parameter family of
saturating laws (e.g.\ $1/(1+x^2)$, $\tanh$); it is a stated hypothesis, not a
forced one. Both are exactly the kind of assumption the experiment adjudicates.
...
$\delta n\sim\num{3e-23}$ --- far below any current bound and orders of magnitude
below QED's own static-$\mathbf B$ prediction. The frame dependence is therefore
operationally negligible for realistic experiments; the static-$\mathbf B$
transparency of Eq.~\eqref{eq:staticB} is not resurrected by the boost.
...
the $\sim\SI{10}{\kilo\electronvolt}$ probe; this assumes the probe frequency lies
well below the saturation-response scale, i.e.\ the AC-Kerr coefficient reduces to
the DC one. A dispersive correction would enter only through Kramers--Kronig
partners of the saturation edge and is not modeled here. \emph{Common-mode
...
static-field experiments are unaffected by the substrate boost. The model is thus
not already excluded by any published precision-optics bound~\cite{Fouche2016}.
