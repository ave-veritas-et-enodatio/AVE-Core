...
class TestYangMillsHamiltonian:
    ...
    def test_vacuum_energy_zero(self):
        ...
        assert H_vac == pytest.approx(0.0, abs=1e-50), \
            "Vacuum energy must be zero"
...
class TestYangMillsSpectralGap:
    ...
    def test_gap_equals_electron_mass(self):
        ...
        assert gap["gap_MeV"] == pytest.approx(m_e_MeV, rel=1e-6), \
            "Mass gap must equal m_e c²"
    ...
    def test_bogomolnyi_bounds_satisfied(self):
        ...
        for c_val, data in gap["torus_knot_bounds"].items():
            assert data["bound_satisfied"], \
                f"Bogomol'nyi bound violated for crossing number {c_val}"
...
class TestYangMillsInfiniteVolume:
    def test_energy_volume_independent(self):
        ...
        assert result["volume_independent"], \
            "Defect energy must be independent of box volume"

    def test_max_spread_below_threshold(self):
        from ave.axioms.yang_mills import defect_energy_vs_volume
        result = defect_energy_vs_volume(crossing_number=5)
        assert result["max_spread"] < 1e-10, \
            "I_scalar must be identical across all box sizes"
...
class TestOsterwalderSchrader:
    ...
    def test_os1_analyticity(self, os_result):
        assert os_result["OS1_analyticity"]["satisfied"], \
            "OS1 (analyticity) must be satisfied: H ≥ 0 → Wick converges"

    def test_os2_covariance(self, os_result):
        assert os_result["OS2_covariance"]["satisfied"], \
            "OS2 (Euclidean covariance) must be satisfied: Z₀ is scalar"

    def test_os3_reflection_positivity(self, os_result):
        assert os_result["OS3_reflection_positivity"]["satisfied"], \
    ...
    def test_os4_symmetry(self, os_result):
        assert os_result["OS4_symmetry"]["satisfied"], \
    ...
    def test_os4_fermion_antisymmetry(self, os_result):
        ...
        assert abs(phase + 1.0) < 1e-10, \
            "Trefoil exchange phase must be -1 (fermion antisymmetry)"

    def test_os5_cluster_decomposition(self, os_result):
        assert os_result["OS5_cluster_decomposition"]["satisfied"], \
            "OS5 (cluster decomposition) must be satisfied"
    ...
    def test_all_os_satisfied(self, os_result):
        assert os_result["all_OS_satisfied"], \
            "All 5 OS axioms must be satisfied"
    ...
    def test_full_mass_gap_proof_flag(self):
        ...
        assert "Part_E_Osterwalder_Schrader" in proof, \
            "Part E must be present in full proof output"
...
class TestNSLatticeRegularization:
    ...
    def test_laplacian_norm_formula(self):
        ...
        assert norm == pytest.approx(4.0 / dx**2, rel=1e-10), \
            "‖∇²‖ = 4/dx²"
    ...
    def test_laplacian_1d_boundary_nodes(self):
        ...
        assert np.allclose(lap, 0.0, atol=1e-20), \
            "Laplacian of constant field must be zero"
...
class TestNSVelocityBound:
    def test_max_velocity_equals_c(self):
        ...
        assert v_max == pytest.approx(C_0, rel=1e-10), \
            "Maximum lattice velocity must equal c"
    ...
    def test_enstrophy_finite(self):
        ...
        assert np.isfinite(omega) and omega > 0, \
            "Enstrophy maximum must be finite and positive"
...
class TestNSGlobalExistence:
    def test_picard_lindelof_applies(self):
        ...
        assert result["picard_lindelof_applies"], \
            "Picard-Lindelöf conditions must all be satisfied"

    def test_global_existence_proven(self):
        from ave.axioms.navier_stokes import lattice_ns_global_existence
        result = lattice_ns_global_existence(N=100, dx=L_NODE)
        assert result["GLOBAL_EXISTENCE_PROVEN"], \
            "Global existence flag must be True"
...
class TestNSSobolevBound:
    ...
    def test_h1_norm_zero_for_zero_field(self):
        ...
        u = np.zeros(50)
        norm = sobolev_h1_norm(u, L_NODE)
        assert norm == pytest.approx(0.0, abs=1e-30), \
            "H¹ norm of zero field must be zero"
    ...
    def test_uniform_sobolev_bound(self):
        ...
        assert result["uniform_bound_holds"], \
            "H¹ norm must be ≤ analytical bound for all N"
    ...
    def test_ns_smoothness_proven_flag(self):
        ...
        assert proof["NS_SMOOTHNESS_PROVEN"], \
            "NS_SMOOTHNESS_PROVEN must be True after Sobolev upgrade"
...
class TestRiemannSpectralBoundary:
    def test_sigma_cutoff_is_half(self):
        ...
        assert spectral_cutoff_sigma() == pytest.approx(0.5, rel=1e-10), \
            "Spectral cutoff must be σ_c = 1/2"
    ...
    def test_functional_equation_reciprocity(self):
        ...
        assert result["network_reciprocal"], \
            "ABCD det must equal 1 (lossless reciprocal network)"
        assert result["mirror_symmetry_verified"], \
            "Mirror symmetry of Riemann zero must be verified"
        assert result["FUNCTIONAL_EQUATION_FROM_RECIPROCITY"], \
            "Functional equation flag must be True"
    ...
    def test_zero_free_region_physical_establishment(self):
        ...
        assert result["ZERO_FREE_REGION_PHYSICALLY_ESTABLISHED"], \
            "Zero-free region must be physically established"
        assert result["axiom_4_forbids_sigma_below_half"], \
            "Axiom 4 must forbid σ < 1/2"
    ...
    def test_power_converges_above_critical(self):
        ...
        result = zero_free_region_equivalence()
        cases = result["sigma_test_cases"]
        assert cases["sigma_0.6 (physical)"]["convergent"], \
            "Power sum must converge for σ > 1/2"
...
class TestFormalProofOrchestration:
    ...
    def test_yang_mills_proven_in_summary(self, summary):
        assert summary["Yang_Mills"]["proof_complete"], \
            "Yang-Mills must be marked proof_complete in formal summary"

    def test_navier_stokes_proven_in_summary(self, summary):
        assert summary["Navier_Stokes"]["proof_complete"], \
            "Navier-Stokes must be marked proof_complete in formal summary"

    def test_riemann_not_complete(self, summary):
        assert not summary["Riemann_Hypothesis"]["proof_complete"], \
            "Riemann must NOT be marked complete (Phragmén-Lindelöf gap)"

    def test_poincare_complete(self, summary):
        assert summary["Poincare_Conjecture"]["proof_complete"], \
            "Poincaré must be marked complete (Perelman 2003)"

    def test_proof_counts(self, summary):
        counts = summary["formal_proof_count"]
        assert counts["fully_proven"] == 2, \
            "Exactly 2 problems fully proven (Y-M, N-S)"
        assert counts["conditional"] == 1, \
            "Exactly 1 conditional (Riemann)"
        assert counts["solved_externally"] == 1, \
            "Exactly 1 solved externally (Poincaré)"
    ...
    def test_clay_gaps_not_empty(self, summary):
        for key in ("Yang_Mills", "Navier_Stokes", "Riemann_Hypothesis"):
            ...
            assert gap and len(gap) > 20, \
                f"Clay gap for {key} must be a non-trivial string"

    def test_all_os_in_ym_summary(self, summary):
        ym = summary["Yang_Mills"]
        for axiom in ("OS1_satisfied", "OS2_satisfied", "OS3_satisfied",
                      "OS4_satisfied", "OS5_satisfied"):
    ...
    def test_full_millennium_includes_formal(self):
        ...
        assert "formal_proof_status" in result, \
            "full_millennium_means_test must include formal_proof_status"
