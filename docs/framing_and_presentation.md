...
> 🟡 **SUPERSEDED 2026-05-31 by Q-EMBED-SEL-1.** The 2026-04-19 remediation-status snapshot below describes the framework's then-current $\pi^2$ provenance via "spin-1/2 half-cover of the standard Clifford torus." That provenance is **retired as load-bearing** per Q-EMBED-SEL-1 Phase 1+2+3 closure (`research/2026-05-31_Q-EMBED-SEL-1_step_c_result.md` §2.3 — Class B substrate-mechanism manifestation: Axiom 4 self-saturation + Op14 Meissner-asymmetric coupling + named phasor-area-equals-Nyquist-cell-area identification gives $R \cdot r = 1/4$ at $d = 1\,\ell_\text{node}$; Phase 2 cross-particle PASS at `..._phase2_cross_particle_result.md`; Phase 3 cross-domain PASS at `..._phase3_cold_fusion_result.md`). The snapshot is preserved verbatim below as a frozen dated remediation-status record; the current canonical provenance lives in the linked Q-EMBED-SEL-1 result docs and at [`manuscript/ave-kb/vol1/ch8-alpha-golden-torus.md`](../manuscript/ave-kb/vol1/ch8-alpha-golden-torus.md).
...
> **🔴 WEAK-C SCOPE-NOTE (2026-06-15, Rule 12 — paragraph above PRESERVED unedited; weak-C canonization, gate `wejkhvnfb`, Grant-confirmed 2026-06-14; prereg at repo-root `research/2026-06-15_weak-c-photon-continuum_prereg.md`).** One substrate, **two regimes** — the "lattice" the paragraph above invokes is the **matter** substrate (saturation **locked** to nodes; the standing entrained structure, smallest = electron). It is the right object for the Millennium-problem results, which are *matter*-sector: the mass gap (Yang-Mills nuclear modes), regular NS flow with velocity cap, and the unique θ=0 vacuum are all **lattice-conditional**, and the B2 reject-the-continuum-limit argument above stands UNCHANGED for them. The **free photon**, by contrast, is the **unlocked / sub-saturation** ($\Delta\phi \ll \alpha$, $Z_0$-matched) regime: it does not lock to nodes, and its propagation description is the discrete LC-ladder's **long-wavelength regime** ($L \gg \ell_{\text{node}}$), which IS the Maxwell continuum — where $Z_0 = \sqrt{\mu/\varepsilon}$ and $c = 1/\sqrt{LC}$ are $\ell_{\text{node}}$-independent (`z0-derivation.md:40`; `master-equation.md:16, :61`). So the same substrate is **lattice-conditional for matter** and **continuum-regime for free light** — NOT two substrates, and NOT a continuum limit demanded of the matter proofs. The continuum here is a *regime* of the one substrate; the input-parameter count is unchanged at 3 {$m_e$, $\alpha$, $G$}. Reconciliation anchor: `k4-port-irrep-decomposition.md` §7 (massless-when-free $T_2$ vs the massive mode the same $T_2$ sector hosts at saturation).  <!-- rule12-freeze: base=8cdcef0383429f597e3ffc4562cdb6e55e1b1dc3 region=above offset=29 lines=116 bytes=11928 sha256=ab77b0b3f64e4eba3475aa2af7ebcf1ab44f2e56da16381db1fa70c99a234909 -->
...
**Status (2026-05-18 late evening — RESOLVED).** ✅ Per-spacecraft table now provided by [`src/scripts/verify/flyby_anomaly_anderson_anchor.py`](../src/scripts/verify/flyby_anomaly_anderson_anchor.py) (driver) + [`research/2026-05-18_flyby-anomaly-anderson-anchor-result.md`](../research/2026-05-18_flyby-anomaly-anderson-anchor-result.md) (full result doc). Walk-back commit on `analysis/flyby-anomaly-driver` branch corrected: (a) the geometric notation from `cos(α)cos(δ)` to Anderson empirical `(cos δ_in − cos δ_out)` form; (b) the "13.4 mm/s" headline to honest per-spacecraft framing (2/6 within 1σ, 3/6 within 2σ across the Anderson 2008 6-spacecraft anchor: Galileo I/II, NEAR, Cassini, Rosetta I, MESSENGER); (c) the mechanism description with explicit Regime-IV stator-boundary framing + Q-G24 reconciliation + cross-reference to the canonical Gravitational Stator (`vol3/cosmology/ch06-solar-system/plasma-standoff-vs-gravitational-stator.md`) and sibling-observable geodynamo Back-EMF (`vol3/applied-physics/ch13-geophysics/geodynamo-vca-back-emf.md`) leaves. Original E1 anti-pattern critique was valid; corpus walk-back addressed it. **Prior status (2026-04-19): ⏳ DEFERRED.**
